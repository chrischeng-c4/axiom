...
# preview installer - downloads the right prebuilt binary from GitHub
...
#
# Env overrides:
#   PREVIEW_VERSION   tag to install (default: latest preview@* release)
#   PREVIEW_INSTALL   install dir (default: $HOME/.local/bin)
#   PREVIEW_REPO      gh repo (default: chrischeng-c4/axiom)
...
REPO="${PREVIEW_REPO:-chrischeng-c4/axiom}"
...
# Preview's CI runner path is Linux-first, and local SRE bootstrap is Apple
# Silicon-first. Keep the same Intel macOS boundary as jet/lumen until a release
# job produces that artifact.
if [ "${target}" = "x86_64-apple-darwin" ]; then
  die "preview provides no Intel-macOS binary yet. Use an arm64 Mac, or run the Linux binary." 2
fi
...
if [ "${VERSION}" = "latest" ]; then
  ...
  tag="$(
    auth_curl_api "${api}" \
      | grep -E '"tag_name": "preview@[^"]+"' \
      | head -n 1 \
      | sed -E 's/.*"tag_name": "([^"]+)".*/\1/'
  )" || true
  if [ -z "${tag}" ]; then
    if [ -z "${TOKEN}" ]; then
      die "could not find a preview@* release in ${REPO} (if the repo is private - export GH_TOKEN or \`gh auth login\`)"
    fi
    die "could not find a preview@* release in ${REPO}"
  fi
else
  tag="${VERSION}"
fi
...
# ---- download + verify -------------------------------------------------
asset="preview-${target}.tar.gz"
url="https://github.com/${REPO}/releases/download/${tag}/${asset}"
sha_url="${url}.sha256"

tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t preview-install)"
...
bin="${tmpdir}/preview-${target}/preview"
[ -f "${bin}" ] || die "binary not found in archive: ${bin}"
chmod +x "${bin}"

mkdir -p "${INSTALL_DIR}"
mv "${bin}" "${INSTALL_DIR}/preview"
...
if "${INSTALL_DIR}/preview" --version >/dev/null 2>&1; then
  ver="$("${INSTALL_DIR}/preview" --version 2>/dev/null || echo unknown)"
  say "ready: ${ver}"
fi
