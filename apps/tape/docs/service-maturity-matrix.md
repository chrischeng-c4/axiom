...
|---|---|---|---|---|---|---|
| HTTP/1.1 + h2c, errors, probes, drain | `service-http`, `server-http`, `server-lifecycle`, `transport-h2c` | `apps/lumen/e2e/protocol_transport_e2e.rs`, `spec_cli.rs` | `tests/http_transport.rs` | `tests/http2_transport.rs` | `tests/http_api.rs` | parity |
...
| Durable consensus, snapshot/restart, committed ownership/fencing | `raft-core`, `raft-runtime`, `storage-durable` | `apps/lumen/e2e/wal_nats_e2e.rs` | `tests/raft_cluster.rs`, `raft_persistence.rs`, `raft_failover.rs` | `tests/raft_cluster.rs`, `raft_persistence.rs` | `tests/raft_scheduler.rs` (same-directory restart + repeated failover) | parity; app owns state machine only |
