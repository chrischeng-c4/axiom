#!/usr/bin/env bash
# HANDWRITE-BEGIN gap="missing-generator:logic:a3fc8dfd" tracker="pending-tracker" reason="New script (mirrors apps/lumen/scripts/dev-single.sh): single-node local dev, embedded file-backed journal via TAPE_STORE, TAPE_BIND default 127.0.0.1:7137, TAPE_AUTH=off, runs cargo run -p tape --bin tape -- serve."
