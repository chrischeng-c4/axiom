#!/usr/bin/env bash
# Verify only a run-scoped Tape release candidate. This script never creates
...
verify_archive() {
  ...
  [[ "$members" == "$(printf '%s\n' "tape-${target}/" "tape-${target}/README.md" "tape-${target}/tape" | LC_ALL=C sort)" ]] || fail "archive members changed: ${archive##*/}"
  mode="$(tar -tvzf "$archive" | awk -v path="tape-${target}/tape" '$NF == path { print $1 }')" || fail "archive metadata cannot be read: ${archive##*/}"
...
validate_manifest() {
  ...
  if jq -e 'has("jobs")' "$MANIFEST" >/dev/null; then
    ...
    jq -e '.jobs == {identity:"success",build:"success","tape-release-gates":"success",manifest:"success","ghcr-image-and-attest":"success","verify-candidate":"success","verify-libraries":"success","kind-amd64":"success","kind-arm64":"success",result:"success"}' "$MANIFEST" >/dev/null || fail "final candidate manifest does not bind all successful jobs"
  fi
  [[ "$actual_keys" == "$expected_keys" ]] || fail "candidate manifest keys changed: $actual_keys"
  targets="$(target_list | jq -Rsc 'split("\n") | map(select(length > 0))')"
  jq -e --arg repo "$REPO" --arg version "$VERSION" --arg commit "$COMMIT" --arg run_id "$RUN_ID" --arg attempt "$RUN_ATTEMPT" --arg tag "tape@${VERSION}" --arg candidate "release-candidate-${RUN_ID}-${RUN_ATTEMPT}" --arg workflow_ref "${REPO}/.github/workflows/tape-release-candidate.yml@refs/heads/main" --argjson targets "$targets" '
    .schema == "cclab.tape.candidate-manifest.v1" and
    .repository == $repo and .workflow_path == ".github/workflows/tape-release-candidate.yml" and
    ...
    (.artifacts | type == "array" and length == 5 and map(.target) == $targets and all(.[];
      ...
      (.archive == ("tape-" + .target + ".tar.gz")) and
...
verify_local_artifacts() {
  ...
  binary="$unpack_dir/tape-${target}/tape"
  ...
  [[ "$version_line" == "tape $VERSION" ]] || fail "candidate binary version mismatch: $version_line"
...
EXPECTED_CERT_ID="https://github.com/${REPO}/.github/workflows/tape-release-candidate.yml@refs/heads/main"
verify_local_artifacts
if [[ "$MODE" == full ]]; then
  verify_full_supply_chain
