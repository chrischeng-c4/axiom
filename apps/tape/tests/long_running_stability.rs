...
async fn wait_leader(raft: &TapeRaft) {
    ...
    while !raft.is_leader().await {
        assert!(Instant::now() < deadline, "single node never elected a leader");
...
async fn repeated_restarts_preserve_append_history_and_checkpoint_progress() {
    ...
    for cycle in 0..CYCLES {
        ...
        let raft = TapeRaft::spawn(Arc::clone(&journal), &dir.path().join("raft"), 0, membership(), HashMap::new(), TapeRaft::host_config(8)).unwrap();
        wait_leader(&raft).await;
        for _ in 0..APPENDS_PER_CYCLE {
            let (_, outcome) = raft.propose_append("orders".into(), None, serde_json::json!({ "n": total }), total).await.unwrap();
            assert!(matches!(outcome, Some(TapeOutcome::Appended(_))));
            total += 1;
        }
        let (_, checkpoint) = raft.propose_checkpoint("orders".into(), "worker".into(), total, 10_000 + cycle).await.unwrap();
        assert!(matches!(checkpoint, Some(TapeOutcome::Checkpoint(Ok(_)))));
        drop(raft);

        let recovered = Arc::new(Mutex::new(TapeJournal::default()));
        let raft = TapeRaft::spawn(Arc::clone(&recovered), &dir.path().join("raft"), 0, membership(), HashMap::new(), TapeRaft::host_config(8)).unwrap();
        let deadline = Instant::now() + Duration::from_secs(8);
        while raft.journal().lock().unwrap().end_offset("orders") != total {
            assert!(Instant::now() < deadline, "cycle {cycle} lost or duplicated journal state");
            tokio::time::sleep(Duration::from_millis(20)).await;
        }
        let journal = raft.journal();
        let journal = journal.lock().unwrap();
        assert_eq!(journal.checkpoint("orders", "worker").unwrap().offset, total);
