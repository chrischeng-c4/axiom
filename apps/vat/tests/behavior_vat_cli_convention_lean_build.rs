...
// Contract: vat compiles without default features; the self-update HTTPS path is absent but the verbs still parse and the pure helpers remain testable.
#[test]
#[ignore = "AW EC gate: run via `aw health --verify-ec` or `cargo test -- --ignored`"]
fn vat_cli_convention_lean_build() {
    let command = "cargo build -p vat --no-default-features";
    let id = "vat-cli-convention-lean-build";
