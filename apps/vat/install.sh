...
if [ "${VERSION}" = "latest" ]; then
  ...
  # API filter so we only pick `vat@*` tags (not mamba/jet/etc).
  ...
  if [ -z "${tag}" ]; then
    if [ -z "${TOKEN}" ]; then
      die "could not find a vat@* release in ${REPO} (repo may be private — export GH_TOKEN or \`gh auth login\`)"
    fi
    die "could not find a vat@* release in ${REPO}"
  fi
else
  tag="${VERSION}"
fi
...
tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t vat-install)"
...
bin="${tmpdir}/vat-${target}/vat"
[ -f "${bin}" ] || die "binary not found in archive: ${bin}"
chmod +x "${bin}"

mkdir -p "${INSTALL_DIR}"
mv "${bin}" "${INSTALL_DIR}/vat"
say "installed: ${INSTALL_DIR}/vat"
...
# ---- show version ------------------------------------------------------
if "${INSTALL_DIR}/vat" --version >/dev/null 2>&1; then
