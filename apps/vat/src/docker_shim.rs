...
mod tests {
    ...
    fn docker_stats_timeout_detaches_an_escaped_pipe_holder_without_reusing_its_pgid() {
        ...
        let result = capture_docker_native_json_command(
            &mut command,
            // This launches a second copy of the Rust test harness before the
            // helper can publish its detached-child PID. Keep the production
            // timeout path bounded, but allow enough startup time on loaded
            // CI hosts for that explicit readiness marker to be written.
