// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/logic/issues/next_id_source.md#source
// CODEGEN-BEGIN
//! Phase B: monotonic id allocator backed by `/tmp/aw/.../issues/.next-id`.
//!
...
//! atomic via:
//!   1. fs2 file lock on the ephemeral issue store's `.next-id.lock`
...
mod tests {
    ...
    fn counter_survives_across_processes_simulated_via_reload() {
        ...
        let counter =
            read_counter(&crate::shared::workspace::issues_path(tmp.path()).join(".next-id"))
                .unwrap()
                .unwrap();
