// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/interfaces/issues/issues_module_preamble_source.md#source
// CODEGEN-BEGIN
...
// CODEGEN-END
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/interfaces/issues/issues_module_runtime_source.md#source
// CODEGEN-BEGIN
pub mod backend;
pub mod backends;
pub mod labels;
...
pub fn resolve_default_backend(
    ...
    if !config_path.exists() {
        anyhow::bail!(
            "no aw.toml found at {} — run `aw conf sync` and configure [agentic_workflow.issue_platform] or [agentic_workflow.repo_platform]",
    ...
    if let Some(ref s) = workflow {
        if let Some(ref rp) = s.repo_platform {
            ...
            eprintln!(
                "[aw wi] no [agentic_workflow.issue_platform] in aw.toml; \
    ...
    anyhow::bail!(
        "must configure [agentic_workflow.repo_platform] (or [agentic_workflow.issue_platform]) in aw.toml \
...
"#,
    ...
    fn missing_config_file_errors() {
        ...
        assert!(err.contains("no aw.toml"), "got: {err}");
    }
