// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/interfaces/workflow/merge.md#source
// CODEGEN-BEGIN
...
pub fn handle(
    ...
    let archive_path = workspace::archive_path(project_root)
        .join(format!("{}-{}", Utc::now().format("%Y%m%d"), change_id))
        .display()
        .to_string();
    ...
    match action {
        Action::BeginMerge => {
            ...
            base["target_dir"] = json!(
                workspace::tech_design_path(project_root)
                    .display()
                    .to_string()
            );
            ...
            base["prompt"] = json!(format!(
                "# Task: Begin Merge for Change '{cid}'\n\n\
                 ...
                 4. Apply merge based on strategy:\n\
                    - `new` (default): Create a new spec file in the configured tech-design root\n\
        ...
        Action::MergeComplete => {
            ...
            base["prompt"] = json!(format!(
                "# Merge Complete for Change '{}'\n\n\
                 ...
                 1. Move the runtime change directory to archive:\n\
                    mv {change_dir} {archive}\n\
