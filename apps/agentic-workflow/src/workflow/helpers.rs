// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/interfaces/workflow/helpers.md#source
// CODEGEN-BEGIN
...
///
/// `{ "cli": "score ...", "args": {...} }`
/// @spec apps/agentic-workflow/tech-design/core/interfaces/workflow/helpers.md#source
pub fn next_action(
...
pub fn format_cli_command(tool: &str, args: &serde_json::Value) -> String {
    if tool == RUN_CHANGE_TOOL {
        ...
        let change_base = crate::shared::workspace::change_path(project_root, change_id);
        let payload_path = match group_id {
            Some(gid) => change_base
                .join("groups")
                .join(gid)
                .join("payloads")
                .join(format!("{}.json", action_kebab)),
            None => change_base
                .join("payloads")
                .join(format!("{}.json", action_kebab)),
        };
...
/// `/tmp/aw/workspaces/<workspace>/changes/{change_id}/groups/{gid}/payloads/{action_kebab}.json`.
...
/// @spec apps/agentic-workflow/tech-design/core/interfaces/workflow/helpers.md#schema
#[derive(Debug, Clone)]
pub struct SpecInfo {
    /// Spec identifier.
    pub id: String,
