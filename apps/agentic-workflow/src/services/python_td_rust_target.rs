...
pub fn emit_python_td_rust_target(ir: &PythonTdIr, root: &Path) -> Result<RustTdTarget> {
    ...
    for module in ir.modules.iter().filter(|m| m.path.starts_with("src/")) {
        ...
        for d in &module.declarations {
            if !ident(&d.name) {
                bail!("unsupported Python TD declaration `{}` in `{}` for Rust target", d.name, module.id);
            }
            ...
            if !d.annotations.is_empty() || !d.decorators.is_empty() {
                body.push_str(&format!("// python metadata: {:?} {:?}\n", d.annotations, d.decorators));
            }
            match &d.kind {
                PythonTdDeclarationKind::Class => {
                    body.push_str(&format!("#[derive(Default)] pub struct {};\n", d.name));
                    body.push_str(&format!("impl GeneratedModuleContract for {} {{}}\n", d.name));
                }
                PythonTdDeclarationKind::Function if d.is_async => body.push_str(&format!("pub async fn {}() -> Result<(), GeneratedError> {{ Err(GeneratedError) }}\n", d.name)),
                PythonTdDeclarationKind::Function => body.push_str(&format!("pub fn {}() -> Result<(), GeneratedError> {{ Err(GeneratedError) }}\n", d.name)),
            }
        }
        body.push_str("#[derive(Debug)] pub struct GeneratedError;\n// CODEGEN-END python-ir-rust-target\n");
        ...
        files.entry(format!("src/{role}/mod.rs")).or_insert_with(String::new).push_str(&format!("pub mod {name};\n"));
        roles.insert(role);
    }
    if files.is_empty() { bail!("Python TD IR has no src/* modules to lower to Rust"); }
    let lib = roles.into_iter().map(|role| format!("pub mod {role};\n")).collect();
    ...
    let manifest = files.iter().map(|(p,c)| RustTdTargetFile { path:p.clone(), digest:digest(c.as_bytes()) }).collect::<Vec<_>>();
    for (path, content) in &files { let output=root.join(path); fs::create_dir_all(output.parent().unwrap()).with_context(|| format!("create {}",output.display()))?; fs::write(output,content)?; }
    Ok(RustTdTarget { digest:digest(&serde_json::to_vec(&manifest)?), files:manifest })
}
fn ident(value:&str)->bool { !value.is_empty() && value.bytes().enumerate().all(|(i,b)| b.is_ascii_alphabetic()||b==b'_'||(i>0&&b.is_ascii_digit())) }
