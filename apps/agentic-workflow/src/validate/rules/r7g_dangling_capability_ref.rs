// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/validate/source/projects-sdd-src-validate-rules-r7g_dangling_capability_ref-rs.md#source
// CODEGEN-BEGIN
...
//! from `spec_path` for `aw.toml`, resolve the owning project name
...
//!
//! Both the `aw.toml` project-path table and the parsed capability
//! document are memoized per project root for the process lifetime, so a
//! whole-tree `aw td check` run reads+parses `aw.toml` and each
...
/// @spec apps/agentic-workflow/tech-design/core/validate/source/projects-sdd-src-validate-rules-r7g_dangling_capability_ref-rs.md#source
...
/// Walk up from `spec_path` looking for the owning `aw.toml`. Same
...
/// table, keyed by project root. Reading and parsing `aw.toml` is
...
/// Read `<project_root>/aw.toml` and extract `(name, path)` for
...
"#;
    ...
    /// Set up `<tempdir>/aw.toml` + `<tempdir>/proj/CAPABILITIES.md`
...
"#
    ...
    fn td_outside_any_project_passes_silently() {
        let tmp = tempfile::TempDir::new().unwrap();
        // No `aw.toml` anywhere above this path — not every TD tree
