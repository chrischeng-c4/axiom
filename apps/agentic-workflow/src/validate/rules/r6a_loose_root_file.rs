// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/validate/source/projects-sdd-src-validate-rules-r6a_loose_root_file-rs.md#source
// CODEGEN-BEGIN
...
pub(super) fn locate_in_crate_spec_root(spec_path: &Path) -> Option<(String, String)> {
    ...
    let root_idx = parts
        ...
        .rev()
        .find(|(i, part)| {
            (**part == "apps" || **part == "crates" || **part == "projects") && i + 1 < parts.len()
        })
        .map(|(i, _)| i)?;
