// SPEC-MANAGED: apps/agentic-workflow/tech-design/surface/interfaces/src/hook.md#source
// CODEGEN-BEGIN
...
//!
//! @spec apps/agentic-workflow/tech-design/core/specs/score-hook-pretooluse-write-scope.md
...
#[derive(Debug, Args)]
// @spec apps/agentic-workflow/tech-design/surface/interfaces/src/hook.md#source
pub struct HookArgs {
    #[command(subcommand)]
    pub event: HookEvent,
}
...
pub enum PretooluseKind {
    /// Enforce project-<name> branch ↔ edit scope per aw.toml
...
async fn run_workflow_guard() -> Decision {
    match read_json_payload() {
        Ok(payload) => match crate::cli::workflow_guard::hook_pretooluse_workflow_guard(
...
async fn run_workflow_apply() -> Decision {
    match read_json_payload() {
        Ok(payload) => match crate::cli::workflow_guard::hook_posttooluse_workflow_apply(
            &std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")),
            &payload,
        )
        .await
...
fn workflow_hook_decision(decision: crate::cli::workflow_guard::HookDecision) -> Decision {
    match decision {
        crate::cli::workflow_guard::HookDecision::Allow => Decision::Allow,
        crate::cli::workflow_guard::HookDecision::AllowWithOutput(out) => {
            Decision::AllowWithOutput(out)
        }
        crate::cli::workflow_guard::HookDecision::Block(reason) => Decision::Block(reason),
    }
}
...
mod write_scope {
    ...
    pub fn decide() -> Decision {
        ...
        // Step 7: load aw.toml. Missing → allow (a repo
        ...
        let cfg = match path_scope::load_scope(&canon_root) {
            ...
            Err(e) => {
                return Decision::FailOpen(format!("failed to load aw.toml: {e}"));
            }
        };
            ...
            None => {
                return Decision::Block(format!(
                    "branch '{branch}' has no matching [[projects]] entry \
                     (name='{project_name}') in aw.toml — refusing edit of {rel}"
                ));
            }
        };
        ...
        if scope.contains(&rel) {
            ...
            Decision::Block(format!(
                "branch '{branch}' restricts edits to {} (per aw.toml \
