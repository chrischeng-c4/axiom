// SPEC-MANAGED: apps/agentic-workflow/tech-design/surface/interfaces/src/chat_members.md#source
// CODEGEN-BEGIN
...
// key. `aw.toml` `[team] name` lives in the repo and is shared
...
// 3. `aw.toml` `[team] name` — only used when not in a git tree
...
pub fn resolve_identity(cwd: &Path) -> Result<String> {
    ...
    let caller_config_path = toplevel.join("aw.toml");
