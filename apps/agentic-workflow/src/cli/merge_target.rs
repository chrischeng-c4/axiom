// SPEC-MANAGED: apps/agentic-workflow/tech-design/surface/interfaces/src/merge_target.md#source
// CODEGEN-BEGIN
...
//! `apps/agentic-workflow/tech-design/core/issues/issue-merge-target.md`):
...
//!   4. Detached HEAD: read `aw.toml`, extract
...
///
// @spec apps/agentic-workflow/tech-design/core/issues/issue-merge-target.md#logic
pub fn resolve_merge_target(
    ...
    // Step 4: detached HEAD — fall back to aw.toml default_branch.
    let config_path = project_root.join("aw.toml");
    if config_path.exists() {
        let content = std::fs::read_to_string(&config_path)
            .map_err(|e| anyhow::anyhow!("failed to read aw.toml: {}", e))?;
        let parsed: toml::Value = toml::from_str(&content)
            .map_err(|e| anyhow::anyhow!("failed to parse aw.toml: {}", e))?;
    ...
    Err(anyhow::anyhow!(
        "cannot determine merge target: detached HEAD and no default_branch in aw.toml"
    ))
}
...
mod tests {
    ...
    /// Write a minimal aw.toml with an optional default_branch.
    fn write_config(dir: &std::path::Path, default_branch: Option<&str>) {
        ...
        std::fs::write(dir.join("aw.toml"), content).unwrap();
    }
