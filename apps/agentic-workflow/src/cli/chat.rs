// SPEC-MANAGED: apps/agentic-workflow/tech-design/surface/interfaces/src/chat.md#source
// CODEGEN-BEGIN
...
mod tests {
    ...
    // REQ: apps/agentic-workflow/tech-design/surface/specs/score-chat.md#logic — identity fallback when no aw.toml
    #[test]
    ...
    fn test_detect_team_identity_reads_config() {
        ...
        std::fs::write(tmp.path().join("aw.toml"), "[team]\nname = \"myteam\"\n").unwrap();
        let no_members = tmp.path().join("no-members.yaml");
        let identity = detect_team_identity(tmp.path(), &no_members).unwrap();
    ...
    fn test_resolve_identity_uses_branch_not_sibling_config() {
        ...
        // Create a temp dir as a fake WT with its own git repo and aw.toml
        let tmp = tempfile::tempdir().unwrap();
        ...
        // Write aw.toml with [team] name = "fake-team-name"
