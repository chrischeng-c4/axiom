// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/create_change_merge/definition.md#source
// CODEGEN-BEGIN
...
//! 4. Writes cleaned specs to `tech-design/{main_spec_ref}`
...
/// @spec apps/agentic-workflow/tech-design/core/tools/create_change_merge/definition.md#source
pub fn workflow_definition() -> ToolDefinition {
    ToolDefinition {
        name: "sdd_workflow_create_change_merge".to_string(),
        description:
            "Programmatic merge: copy all change specs to tech-design/ and archive the change"
                .to_string(),
        input_schema: json!({
            "type": "object",
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/create_change_merge/workflow.md#source
// CODEGEN-BEGIN
// ─── Workflow (programmatic merge) ────────────────────────────────────────────
...
/// 4. Write to `tech-design/{main_spec_ref}`
/// 5. Update phase → ChangeArchived
/// 6. Return archive instructions
/// @spec apps/agentic-workflow/tech-design/core/tools/create_change_merge/workflow.md#source
pub async fn execute_workflow(args: &Value, project_root: &Path) -> Result<String> {
    ...
    // project_root (legacy / test fallback). Runtime writes (specs,
    ...
    let change_dir = workspace::change_path(&work_root, &change_id);
    ...
    if spec_paths.is_empty() {
        ...
        let archive_rel = build_archive_path(&work_root, &change_id);
        let archive_abs = PathBuf::from(&archive_rel);
        ...
        // working copy BEFORE post_archive_git_ops so the branch commit picks
    ...
    // the branch commit stages the rename into the cclab/<slug> branch
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/create_change_merge/preflight.md#source
// CODEGEN-BEGIN
// ─── Pre-flight Gates (R6, R7, R8) ───────────────────────────────────────────
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/create_change_merge/issue-close.md#source
// CODEGEN-BEGIN
// ─── Issue Closing (part of step 5) ──────────────────────────────────────────
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/create_change_merge/merge-helpers.md#source
// CODEGEN-BEGIN
// ─── 3-Way Merge Support ─────────────────────────────────────────────────────
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/create_change_merge/alignment-and-archive.md#source
// CODEGEN-BEGIN
// ─── Alignment Check Helpers ─────────────────────────────────────────────────
...
fn build_archive_path(project_root: &Path, change_id: &str) -> String {
    workspace::archive_path(project_root)
        .join(format!(
            "{}-{}",
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/create_change_merge/tests.md#source
// CODEGEN-BEGIN
#[cfg(test)]
mod workflow_tests {
    ...
    async fn test_programmatic_merge_with_main_spec_ref() {
        ...
        assert_eq!(
            ...
            "tech-design/sdd/workflow/auth-flow.md"
        );

        // Verify target file exists and change-spec-only fields are stripped
        let target = tmp.path().join("tech-design/sdd/workflow/auth-flow.md");
    ...
    async fn test_root_level_path_rejected() {
        ...
        let flat_target = tmp.path().join("tech-design/flat-spec.md");
        assert!(
            !flat_target.exists(),
    ...
    async fn test_audit_log_section_merge() {
        ...
        let target_dir = tmp.path().join("tech-design/sdd/logic");
    ...
    async fn test_validation_aborts_before_write() {
        ...
        let valid_target = tmp.path().join("tech-design/sdd/valid/valid-spec.md");
        assert!(
            !valid_target.exists(),
...
mod preflight_worktree_tests {
    ...
    path = "tech-design"
    "#;
        std::fs::write(main.join("aw.toml"), config_content).unwrap();
    ...
    // the worktree, and main's tech-design/ is NOT touched by
    ...
    path = "tech-design"
    "#;
        std::fs::write(main_root.join("aw.toml"), config_content).unwrap();
        ...
        let wt_target = wt_root.join("tech-design/sdd/logic/wt-spec.md");
        let main_target = main_root.join("tech-design/sdd/logic/wt-spec.md");
        assert!(
            wt_target.exists(),
        ...
        assert!(
            !main_target.exists(),
            "spec must NOT be written to main/tech-design/ — git merge \
