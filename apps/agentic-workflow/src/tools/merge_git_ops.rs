// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/merge_git_ops/types-and-post-archive.md#source
// CODEGEN-BEGIN
...
//!
//! The 5-step merge sequence (see `apps/agentic-workflow/tech-design/core/tools/merge_git_ops/types-and-post-archive.md`):
...
/// @spec apps/agentic-workflow/tech-design/core/tools/merge_git_ops/types-and-post-archive.md#source
impl GitOpsResult {
...
pub(super) fn post_archive_git_ops(
    ...
    // REQ: bug-create-change-merge-archive-moves-not-committed-sp
    // Runtime AW artifacts live under /tmp/aw and are not git-tracked. Commit
    // only project-local changes that the merge produced in the worktree.
    ...
    if has_dirty {
        // Stage all project-local changes. Runtime AW artifacts are outside
        // the worktree, so this cannot re-add repo-root `.aw/`.
            ...
            .output()
        {
            ...
            Err(e) => return Ok(GitOpsResult::warning(format!("git add failed: {}", e))),
        }
    ...
    // @spec apps/agentic-workflow/tech-design/core/logic/merge-gaps-fix.md#R2
    ...
    if worktree_dir.is_some() {
        match merge_worktree_branch(&git_bin, project_root, change_id, &config.default_branch) {
            MergeOutcome::Ok => {
                // @spec apps/agentic-workflow/tech-design/core/logic/merge-gaps-fix.md#R1
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/merge_git_ops/worktree-merge-cleanup.md#source
// CODEGEN-BEGIN
// ─── Worktree Merge & Cleanup (Steps 3 & 4) ──────────────────────────────────
...
pub(super) fn resolve_worktree_dir(project_root: &Path, change_id: &str) -> Option<PathBuf> {
    for wt in [
        workspace::worktree_path(project_root, change_id),
        project_root.join(".aw/worktrees").join(change_id),
    ] {
        if wt.is_dir() {
            return Some(wt);
        }
...
fn cleanup_merged_worktree(
    ...
    let branch = format!("cclab/{}", change_id);
    let worktree_abs = workspace::worktree_path(project_root, change_id);
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/merge_git_ops/binary-and-pr.md#source
// CODEGEN-BEGIN
// ─── Git Binary Detection ───────────────────────────────────────────────────
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/merge_git_ops/commit-and-pr-body.md#source
// CODEGEN-BEGIN
// ─── Commit Message & PR Body ───────────────────────────────────────────────
...
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/tools/merge_git_ops/tests.md#source
// CODEGEN-BEGIN
// ─── Tests ──────────────────────────────────────────────────────────────────
...
path = "tech-design"
"#;
        std::fs::write(tmp.path().join("aw.toml"), toml_content).unwrap();
