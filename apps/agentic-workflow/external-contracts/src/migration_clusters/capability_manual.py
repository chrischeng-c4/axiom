    ...
    if case_id.startswith("capability-control-plane"):
        ...
        if case_id == "capability-control-plane-capability-project-sweep":
            ...
            assertions = [
                ...
                "a relocated capability keeps its own Status, the prose prelude above its fields, the prose postlude a different capability of that same document carries below its work-root table -- the other half of a carry-through rule that was asserted on one side only, leaving the whole postlude renderer deletable -- the `aw ec` efficiency backfill slot a third capability carries in that same position, which is not prose at all but is lifted out of it before either prose side is read and re-emitted by its own renderer, so deleting that renderer rendered the identical document until one capability declared a slot, together with the `efficiency` EC dimension the merge appends to that capability's own contract, and the Impl and Verification columns derived from that status, asserted on the one input whose capabilities are not uniformly verified -- three distinct derived pairs across four statuses, so neither column can be a constant -- and, on the one capability whose status exempts it from the contract requirement, the absence of the `Surfaces:` field it declared none of, because every capability of every other document declares one and forcing that field's emptiness guard true was unobservable",
