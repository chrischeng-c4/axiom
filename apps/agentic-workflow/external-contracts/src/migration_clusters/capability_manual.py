...
def _lumen_check(root: Path, cap_path: Path, document: str) -> dict[str, Any]:
    """Read the same document through `aw capability check`.

    `expect_success` is `None` rather than `False`: this fixture's project has
    no Python EC or TD root, so `check` exits non-zero on *every* document here,
    valid ones included. Its return code therefore carries no signal about the
    feature roots and is deliberately not asserted -- the envelope's blockers
    are, after the pinned environment prefixes are subtracted.
    """
    cap_path.write_text(document, encoding="utf-8")
    return final_json(
        run_aw(
...
def _lumen_feature_class_snapshot() -> dict[str, Any]:
    ...
    with project_fixture() as root:
        ...
        # Everything above reads the root rules through `capability report`.
        # Epic #2887's AC2 names `capability check`, which no leg here drove at
        # all until this one -- an entire verb, reachable by these exact
        # documents, undisclosed. The four conditions are re-read through it
        # with the *same* expectation functions, because the two verbs return
        # the same blocker strings from the same validator; what this binds is
        # that `check` still reaches it.
        lumen_reference.assert_check_surfaces_the_same_root_diagnostics(
            {
                "valid": _lumen_check(
                    root, cap_path, lumen_reference.REFERENCE_DOCUMENT
                ),
                "duplicate_root": _lumen_check(
                    root, cap_path, lumen_reference.DUPLICATE_ROOT_DOCUMENT
                ),
                "missing_non_core_root": _lumen_check(
                    root, cap_path, lumen_reference.MISSING_NON_CORE_ROOT_DOCUMENT
                ),
                "unknown_feature_root": _lumen_check(
                    root, cap_path, lumen_reference.UNKNOWN_FEATURE_ROOT_DOCUMENT
                ),
                "both_roots": _lumen_check(
                    root, cap_path, lumen_reference.MULTIPLY_CLASSIFIED_DOCUMENT
                ),
            }
        )
        ...
        # because it blanks that state before rendering. Thirteen shapes now,
        # eleven of which classify nothing and two of which classify; the loop
        # grew one shape at a time as later rounds bound more of the branch, so
        # read its length from the tuple rather than from this comment. The
        # original four are still the ones that make the branch's distinguishable
        # behaviors observable: nothing classified, partially classified, the
        # `Root WI` fallback, and all three render groups populated at once. That
        # last one is what makes the *order* of `capabilities_in_render_order`
        # observable -- everywhere else grouped order and raw document order are
        # the same list, so permuting the array renders the identical document.
        relocated_sections = {}
        for name, readme_document, declares_any_class, expected_order in (
            ...
            with project_fixture() as section_root:
                ...
                if name == "mixed_class":
                    ...
                    # is the only shape that declares *both* class values --
                    # `partially_classified` declares two `core` and no
                    # `non_core` -- so it is where a present class of either
                    # value gets its position in the block bound.
                    lumen_reference.assert_relocation_renders_the_canonical_field_block(
                        section_text, lumen_reference.MIXED_CANONICAL_BLOCK_SUBJECTS
                    )
                # The roots are emitted exactly when the input classified
                # something. Eleven of these shapes classify nothing and two
                # classify, so both directions of that conditional are pinned;
                # the flag travels with each shape in the tuple rather than
                # being counted here, so adding a shape cannot make this stale.
    ...
    if case_id.startswith("capability-control-plane"):
        ...
        if case_id == "capability-control-plane-capability-project-sweep":
            ...
            assertions = [
                ...
                "the Lumen reference fixture attributes its domain search promises to core and every archetype service baseline it names to non-core, with the capability and claim pairs each summing to their retained total; the fixture carries seven baseline ids, not four -- the four it classifies non-core and three more it names only to be rejected when declared core -- and the size of the union registry those are drawn from is a fact about the Rust `doc_mirror` tables that no assertion here reads, so this label does not state it; measured against epic #2887's AC8, which names six non-core categories, the attribution is stated per id rather than as a count, because the mapping is not one-to-one: `kubernetes-native-deployment` and `security-hardening` land on AC8's Kubernetes-native deployment and security exactly, `ec-gates-configured` belongs to family `contract-quality-assurance` and answers to no AC8 category at all, and `standard-operational-endpoints` straddles two of them -- its trait promises `/healthz`, `/readyz`, `/metrics`, `/openapi.json` and `/docs` on one port while its family is `operations-observability`, so it touches both observability and standard CLI/API integration without either getting a capability of its own; durability/replication/backup is carried by nothing, and developer/agent experience appears only as the subject of a declared-core rejection, never as an accepted member -- so two of AC8's six are attributed outright, two are only grazed by a shared capability, and two are not attributed at all, tracked in #3286, which has to move the pairwise-distinct operand values every count here pins",
                ...
                "a heading at a feature root's own level closes that root, so a capability title repeated under a later sibling heading is not read as a member of both roots -- falsified by an implementation that reports too much rather than too little, which is the direction most legs here cannot see; this label no longer counts its peers, because two successive revisions of that count were wrong and the set is not local to this leg, and it names the guard instead: the `declares_any_class` early return at capability.rs:10048 is bound by the unclassified-default and legacy-row legs, and deleting it spams every pre-migration document with untrue missing-root blockers",
                ...
                "a Capability Index that declares no Notes column at all falls back to each capability's own promise in that cell, which is the only input that reaches the emptiness filter guarding that fallback -- a blank cell does not, because an empty cell is read as `-`, and the fallback itself is reached by every index-less document here -- with the four columns the document still declares carried through unchanged",
                ...
                "the arms of the gap verification fold are tried in the order the product declares them, asserted on a YAML-fenced capability that is itself `verified` and carries all three gaps whose rendering the order decides and nothing else -- a closed gap reads `verified` rather than `passing`, a deferred gap reads `verified` rather than `blocked`, and a blocked gap still reads `blocked` rather than `verified` -- which together pin the blocked arm ahead of the capability-status arm and the capability-status arm ahead of both the closed and the deferred arm; three, because the fold is `(Blocked,_) -> (_,Verified) -> (Closed,_) -> (Deferred,_)` and every arm but the first overlaps the capability-status arm, and an earlier revision of this label said two while declaring only two, leaving the deferred swap free to compile and change a rendered cell with this case green; this is what an arm-by-arm assertion structurally cannot express, and it is the whole of what this leg adds: the arm *values* of both folds, and the `epic` kind that marks a gap-derived row, are bound already by the capability that declares no work-root table at all, whose synthetic gap enters the same rendering block -- established by mutating each in turn and finding the case still failed with this leg neutralized, so the five gap statuses declared here are declared to reach the two ordering subjects and this label claims no credit for the rest of them",
                ...
                "`aw capability check` -- the verb epic #2887's AC2 names, which no leg of this case drove at all before this one -- surfaces the same four feature-root diagnostics `aw capability report` does, read from the identical documents and asserted with the identical expectation functions: a duplicated root, an absent root with its four stranded capabilities, an unknown root, and one capability under both, plus the valid document yielding an empty list so an implementation emitting all four unconditionally would fail; this claims the wiring and not the rules, because the two verbs return the same blocker strings from the same validator -- verified by running both over each document and comparing the subtracted lists, equal in all four cases -- so what it binds is that the verb AC2 names still reaches that validator, which an implementation leaving `check` silent while `report` stayed correct would break with every other leg here still green; the return code is deliberately not asserted, since this fixture project has no Python EC or TD root and `check` therefore exits non-zero on the valid document too, and the TD-capability-ref half of `check` is subtracted as an environment blocker and covered by nothing in this case; the *drafting* verbs AC4 names in the same breath as this checker -- `aw capability draft` and `apply-draft` -- are driven by no leg here and cannot be, because they are currently wrong: `draft` reads the two canonical feature roots back as candidate capabilities and re-emits them as capability sections, and a document applied from that draft passes `check` with zero blockers and zero core capabilities, filed as #3285; the *write* half of this schema is undriven here too and was undisclosed until round 41 -- `set-type`, `set-status`, `set-surface`, `set-ec-dimension` and `set-wi-ref` all serialize into the Markdown this case parses, and `set-surface` provably cannot round-trip one of the columns asserted here, since its argument struct declares no verification input and the surface it builds hardcodes an empty one, filed as #3287 -- so every claim in this case is a claim about reading the schema, never about writing it",
                "Lumen's production capability contract is byte-identical before and after the fixture run -- a property of this external contract rather than of `aw capability`, listed so a reader can see the fixture is barred from mutating a real project's contract, and counting toward nothing the product promises; and, since this list is what the evidence envelope publishes as `assertion_count` and digests as `assertions_digest` (`runner.py:107` digests these strings, and the driver above discards the snapshot's return value), neither number is derived from the assertions themselves -- deleting every `assert` in the oracle changes neither, so read them as identifying which revision of this case ran and never as evidence that coverage grew or held",
            ]
