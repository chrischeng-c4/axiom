    ...
    if case_id.startswith("capability-control-plane"):
        ...
        if case_id == "capability-control-plane-capability-project-sweep":
            ...
            assertions = [
                ...
                "the Lumen reference fixture attributes its domain search promises to core and every archetype service baseline it names to non-core -- four of the fifteen ids the union registry holds, which is every baseline this fixture carries and not every baseline that exists, with the capability and claim pairs each summing to their retained total; measured against epic #2887's AC8, which names six non-core categories by name, those four cover Kubernetes-native deployment, security and standard operational endpoints, while durability/replication/backup and observability are carried by no capability here and developer/agent experience appears only as the subject of a declared-core rejection, never as an accepted member -- three of AC8's six are therefore unattributed by this fixture and are tracked in #3286, which has to move the pairwise-distinct operand values every count here pins",
