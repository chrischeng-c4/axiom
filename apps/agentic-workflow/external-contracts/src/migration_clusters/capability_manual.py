    ...
    if case_id.startswith("capability-control-plane"):
        ...
        if case_id == "capability-control-plane-capability-project-sweep":
            ...
            assertions = [
                ...
                "a heading at a feature root's own level closes that root, so a capability title repeated under a later sibling heading is not read as a member of both roots -- one of the two assertions in this case falsified by an implementation that reports too much rather than too little, the other being the fenced-heading document, whose empty blocker list is reachable only when the mask holds; an earlier revision of this label called it the only one",
