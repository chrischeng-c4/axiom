...
| Terminal EC process liveness | bug | #1579 | in progress | pending | smoke | `cargo test -p agentic-workflow --lib ec_verify_bounds_a_wrapper_after_its_child_exits`; terminal EC inventory is single-flight and emits a bounded failure instead of hanging after a VAT child exits |
| Default TD target-plan queue | bug | #1598 | implemented | verified | smoke | `cargo test -p agentic-workflow --lib merge_spec_section_preserves_complete_default_queue_from_empty_skeleton -- --nocapture`; fresh TD lifecycle dispatches `logic`, `changes`, then `unit-test` so codegen receives explicit targets |
