...
| XML HANDWRITE fill protocol | bug | #1898 | implemented | verified | smoke | `cargo test -p agentic-workflow --lib pending_xml_handwrite -- --nocapture`; `cargo test -p agentic-workflow --lib xml_handwrite_marker_ -- --nocapture`; pending XML scaffold markers enter a unique per-target fill queue and bind their tracker to the owning work item after payload application |
| Existing XML HANDWRITE adoption | bug | #1900 | implemented | verified | smoke | `cargo test -p agentic-workflow --lib adopt_existing_ -- --nocapture`; existing implementations enclosed by pending XML markers use an explicit adoption payload that preserves the body and binds it to the owning work item |
| XML adoption source-diff guard | bug | #1901 | implemented | verified | smoke | `cargo test -p agentic-workflow --lib pending_xml_ -- --nocapture`; `aw td fill <id> --apply` permits only the active marker source path to be dirty, so adoption commits a bounded implementation diff with its tracker update |
| Marker-free post-fill evidence | bug | #1904 | implemented | verified | smoke | `cargo test -p agentic-workflow --lib marker_free_brief_ -- --nocapture`; after the last marker, including re-entry from `cb_filled`, `aw td fill` accepts only declared Changes paths and stages them with the terminal fill lifecycle commit |
