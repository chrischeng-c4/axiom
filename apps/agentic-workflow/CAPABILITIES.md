...
| Rebased TD lifecycle recovery | bug | #1602 | implemented | verified | smoke | `cargo test -p agentic-workflow --lib reachable_td_init_ignores_lifecycle_commits_lost_after_branch_rewrite`; an active remote TD phase is reset and re-provisioned when its exact `Td-Init` baseline is no longer reachable from `HEAD` |
