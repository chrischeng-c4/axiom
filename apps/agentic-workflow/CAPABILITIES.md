...
| Marker-free post-fill evidence | change | #1904 | implemented | verified | smoke | `cargo test -p agentic-workflow --lib marker_free_brief_ -- --nocapture`; after the last marker, including re-entry from `cb_filled`, `aw td fill` accepts only declared Changes paths and stages them with the terminal fill lifecycle commit |
