// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/validate/project_registry_test.md#tests
// CODEGEN-BEGIN
...
fn marker_upsert_first_run() {
    ...
    assert!(
        ...
        "aw.toml must contain BEGIN AW SYNC marker after first sync;\ngot:\n{content}"
    );

    // R2: END marker must be present
    assert!(
        content.contains(SYNC_END_MARKER),
        "aw.toml must contain END AW SYNC marker after first sync;\ngot:\n{content}"
    );

    // R1: [[projects]] entries present
    assert!(
        content.contains("[[projects]]"),
        "aw.toml must contain [[projects]] entries;\ngot:\n{content}"
    );
    assert!(
        content.contains("\"sdd\""),
        "aw.toml must contain discovered project name;\ngot:\n{content}"
    );
...
#[test]
fn load_projects_reads_workspace_codegen_profile() {
    let tmp = make_score_root();
    write_config(
        tmp.path(),
        r#"
...
// ---------------------------------------------------------------------------
// Additional: check_drift targets aw.toml (T22 prerequisite)
// Verifies R9, R11
// ---------------------------------------------------------------------------
...
// REQ: REQ-011
#[test]
fn check_drift_references_config_toml_in_output() {
    let tmp = make_score_root();
    ...
    // R9/R11: diff output must reference aw.toml (not projects.toml)
    assert!(
        drift.contains("aw.toml"),
        "--check output must reference aw.toml, not projects.toml;\ngot:\n{drift}"
    );
    assert!(
