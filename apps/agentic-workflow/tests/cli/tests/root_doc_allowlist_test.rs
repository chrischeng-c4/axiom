// SPEC-MANAGED: apps/agentic-workflow/tech-design/surface/validate/tests/root_doc_allowlist_test.md#source
// CODEGEN-BEGIN
...
fn project_layer_does_not_define_live_agent_docs() {
    ...
    collect_live_agent_docs(&root, &root.join("apps"), &mut found);
    collect_live_agent_docs(&root, &root.join("projects"), &mut found);
    assert!(
        found.is_empty(),
        "project/app-layer CLAUDE.md/AGENTS.md files are not allowed; these names \
...
fn agentic_workflow_readme_links_project_layer_meta_docs() {
    ...
    let contributing_path = root.join("apps/agentic-workflow/CONTRIBUTING.md");
    let contributing = fs::read_to_string(&contributing_path)
        .unwrap_or_else(|err| panic!("read {}: {err}", contributing_path.display()));
    let capabilities_path = root.join("apps/agentic-workflow/CAPABILITIES.md");
