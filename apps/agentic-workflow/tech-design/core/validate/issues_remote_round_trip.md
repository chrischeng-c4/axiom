---
id: projects-sdd-tests-issues-remote-round-trip-rs
...
# GitHub Issue Remote Round-Trip Test

## Overview
<!-- type: overview lang: markdown -->

Codegenerated ignored integration test for `GitHubBackend` round-tripping
CRRR state through GitHub issue labels. The test is intentionally manual-gated:
it requires `gh auth status` to pass and `SDD_TEST_GH_REPO=owner/repo` to point
at a disposable sandbox repository.

The generated file owns the module documentation, imports, helper functions,
fixture issue construction, and the single ignored async test body. Rust-only
attributes and raw bodies are handled as generator template data, not as new
cross-language section types.
...
preamble: |
  ...
  use agentic_workflow::issues::{GitHubBackend, Issue, IssueBackend, IssuePatch, IssueSection, IssueState, IssueType};
...
changes:
  - path: apps/agentic-workflow/tests/issues_remote_round_trip.rs
    ...
    impl_mode: codegen
    description: |
      Generate the complete ignored GitHub remote round-trip integration test
      from the Tests section. The target file contains only the
      CODEGEN-BEGIN/CODEGEN-END block for this section.
```
