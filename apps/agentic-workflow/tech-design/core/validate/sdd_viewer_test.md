---
id: projects-sdd-tests-sdd-viewer-test-rs
...
# SDD Viewer Integration Tests

## Overview
<!-- type: overview lang: markdown -->

Codegenerated integration tests for SDD viewer directory fixtures, file-node
serialization, JSON response shape, LaTeX-containing Markdown fixtures, and
basic navigation scenarios. The file is emitted through the Rust tests template
with a module wrapper and module-local helper preamble.
...
imports: []
tests:
  - name: test_project_tree_generation
    body: |
  ...
  - name: test_sdd_directory_structure
    body: |
      let (_temp, sdd_dir) = setup_test_sdd();
  ...
  - name: test_markdown_file_detection
    body: |
      let (_temp, sdd_dir) = setup_test_sdd();
  ...
  - name: test_tree_node_serialization
    body: |
  ...
  - name: test_latex_pattern_recognition
    body: |
  ...
  - name: test_tree_navigation_scenario
    body: |
  ...
  - name: test_html_template_structure
    body: |
...
changes:
  - path: apps/agentic-workflow/tests/sdd_viewer_test.rs
    ...
    description: |
      Emit the SDD viewer tests from the Rust tests template using module
      wrapper support and raw Markdown fixture preservation.
```
