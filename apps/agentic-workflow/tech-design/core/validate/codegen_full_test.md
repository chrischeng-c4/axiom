---
id: projects-sdd-tests-codegen-full-test-rs
...
# Codegen Full Pipeline Tests

## Overview
<!-- type: overview lang: markdown -->

Codegenerated integration tests that run codegen apply across the tech design
fixture set. The file is emitted through the Rust tests template with a raw
Rust preamble and raw test bodies; this strengthens the template path while
keeping `tests` as the cross-language section type.
...
preamble: |
  ...
  /// Derive project root from CARGO_MANIFEST_DIR (apps/agentic-workflow/ → project root).
...
tests:
  ...
  - name: test_gentest_produces_all_files
    body: |
      ...
      let state_content =
          std::fs::read_to_string(tmpdir.path().join("src/order/state.rs")).unwrap();
      assert!(
  ...
  - name: test_task_state_machine_roundtrip
    body: |
      ...
      let tmpdir = tempfile::TempDir::new().unwrap();
      let report = run_apply(&spec, tmpdir.path(), false)
          .expect("task-state-machine apply should succeed");
  ...
  - action: annotate
    ...
    description: "Traceability metadata edge for the unit-test section."
      ...
      let report = run_apply(&spec_path, out_dir.path(), false)
          .expect("minimal frontmatter should not crash");
  ...
  - name: test_python_backend_models_apply_dispatches_python_emitter
    body: |
      let root = project_root();
      let spec_rel =
          Path::new("examples/fixture_platform/tech_design/backend/automations/video_qc/api_models.md");
      let target_rel =
          Path::new("examples/fixture_platform/backend/src/features/automations/video_qc/api_models.py");
      let source_spec = root.join(spec_rel);
      let source_target = root.join(target_rel);
      let expected = std::fs::read_to_string(&source_target).unwrap();
      ...
      std::fs::write(&temp_spec, std::fs::read_to_string(&source_spec).unwrap()).unwrap();
      std::fs::write(
          out_dir
              .path()
              .join("examples/fixture_platform/backend/pyproject.toml"),
          "[project]\nname = \"fixture-platform-fixture\"\n",
      )
      .unwrap();

      let spec_ref =
          "examples/fixture_platform/tech_design/backend/automations/video_qc/api_models.md#models";
      std::fs::write(
          &temp_target,
          format!(
      ...
      assert_eq!(report.files.len(), 1, "one Python model file should be routed");
      assert!(
...
changes:
  - path: apps/agentic-workflow/tests/codegen_full_test.rs
    ...
    impl_mode: codegen
    description: |
      Generate the complete codegen full integration test file from the Tests
