...
nodes:
  ...
  load_config:
    kind: process
    label: "load <root>/.aw/config.toml via apps/agentic-workflow/src/services/path_scope::load_scope; on missing exit 0; on parse error exit 0 + stderr fail-open"
  resolve_project:
    kind: decision
...
**new** module `apps/agentic-workflow/src/services/path_scope.rs` that owns:
...
namespace_contract:
  registration: "top-level aw CLI enum in apps/agentic-workflow/src/cli/commands.rs"
  module: "apps/agentic-workflow/src/cli/hook.rs"
  service_dependency: "apps/agentic-workflow/src/services/path_scope.rs"
io_contract:
...
- [changes] Resolved by keeping `aw hook` in the main Agentic Workflow CLI module instead of a separate wrapper crate. The `catch_unwind` boundary wraps the handler dispatch in `apps/agentic-workflow/src/cli/hook.rs`, preserving allow/block exit-code semantics.
- [changes] Resolved by making `globset` a direct `apps/agentic-workflow` dependency.
