...
`apps/agentic-workflow/src/generate/gen/rust/schema.rs`. The current generator
...
`apps/agentic-workflow/src/td_ast/types.rs`, a discriminated enum with nine typed
...
`apps/agentic-workflow/src/td_ast/types.md` § Schema, replacing the HANDWRITE block
...
`apps/agentic-workflow` drops to zero. One new `missing-generator:logic` marker is
...
changes:
  ...
  - path: apps/agentic-workflow/src/td_ast/types.md
    action: modify
    section: schema
    impl_mode: hand-written
    description: |
