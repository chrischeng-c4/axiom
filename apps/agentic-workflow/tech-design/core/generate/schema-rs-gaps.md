...
Closes thirteen gaps in `apps/agentic-workflow/src/generate/gen/rust/schema.rs` and
...
The primary target file is `apps/agentic-workflow/src/generate/gen/rust/schema.rs`. The `RustTypeTranslator` integer-type fix (R5) resides in the same `apps/agentic-workflow/src/generate/gen/rust/` directory.
## Schema
...
description: |
  ...
  (apps/agentic-workflow/src/generate/gen/rust/schema.rs) and RustTypeTranslator.
...
- [changes] R5 is split into a second `modify` entry for the same `apps/agentic-workflow/src/generate/gen/rust/schema.rs` path. The split is harmless — `infer_rust_type_with_nullable` is confirmed to be defined in `schema.rs` — but consolidating into one entry would eliminate any ambiguity about two-pass application.
- [overview] Reference to `apps/agentic-workflow/tech-design/core/validate/rule.md` is present in the issue Reference Context and confirmed on disk; the dogfood linkage is clear.
