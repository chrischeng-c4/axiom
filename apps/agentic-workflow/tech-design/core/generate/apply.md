...
first target. Until #1634 supplies canonical `generates:` unit ownership, more
than one whole-section Schema or CLI CODEGEN destination is rejected with a
typed error carrying the section, sorted targets, and executable remediation.
Single-target, CODEGEN-plus-HANDWRITE, entry-local `rust_source`, and legacy
Logic/source inference remain compatible. A no-Changes Schema/CLI plan uses the
same read-only exact spec-ref inference as the executor: one existing managed
target is admitted, multiple inferred targets are rejected as ambiguous, and
no inferred target receives explicit Changes remediation before lifecycle
mutation.
...
```````rust
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/generate/apply.md#source
// CODEGEN-BEGIN
