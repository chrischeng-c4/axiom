...
main_spec_ref: apps/agentic-workflow/logic/codegen-markers.md
merge_strategy: new
capability_refs:
...
All emitted markers are written to an ephemeral inventory at `/tmp/aw/workspaces/<workspace>/codegen_markers.yaml` for local diagnostics. Reducing marker count over time indicates improving TD quality.
## Requirements
...
requirements:
  ...
  R5:
    text: All SPEC-REF markers are inventoried under /tmp/aw
    type: functional
    priority: medium
    risk: low
    verification: test
    notes: |
      After each gen apply, update /tmp/aw/workspaces/<workspace>/codegen_markers.yaml with all emitted markers.
      Format: spec-path -> [{ section, file, line, task }]
      Enables local diagnostic visibility of remaining TODOs without writing repo state.
  R6:
...
changes:
  ...
  - path: apps/agentic-workflow/src/generate/apply.rs
    ...
    impl_mode: hand-written
    description: |
      Writes /tmp/aw/workspaces/<workspace>/codegen_markers.yaml from collected SPEC-REF markers after apply.
