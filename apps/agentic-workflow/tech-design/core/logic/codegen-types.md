...
main_spec_ref: apps/agentic-workflow/logic/codegen-types.md
merge_strategy: new
capability_refs:
...
Type translation lives in `apps/agentic-workflow/src/generate/types.rs`. The `AbstractType` enum is parsed from YAML `type` fields in schema frontmatter. The `RustTypeTranslator` implements `TypeTranslator<String>` for Rust output.
## Requirements
