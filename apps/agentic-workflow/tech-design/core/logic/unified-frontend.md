...
main_spec_ref: "apps/agentic-workflow/tech-design/core/logic/unified-frontend.md"
merge_strategy: new
capability_refs:
...
Unifies the Score plan viewer (vanilla JS, axum-served, `apps/agentic-workflow/src/ui/viewer/`) and the Conductor frontend (React, `projects/conductor/fe/`) into a single React application under `apps/agentic-workflow/packages/@score/`.
...
|----|-------|-------------|----------|
| R1 | Move @cclab packages to @score | Move `packages/@cclab/{ui,spec-viewer,pipeline}` to `apps/agentic-workflow/packages/@score/{ui,spec-viewer,pipeline}`. Update all import paths. Packages become SDD-owned, co-located with the score crate. | P0 |
...
| R6 | Replace viewer assets with @score/app build | Remove `apps/agentic-workflow/src/ui/viewer/assets/{index.html,app.js,styles.css}`. Replace with `@score/app` dist/ output. Update `include_str!()` paths in `mod.rs` to reference new build artifacts. Pre-built dist/ committed to repo. | P0 |
...
| S4 | cclab jet build produces dist bundle | `apps/agentic-workflow/packages/@score/app/` contains the React SPA source with all dependencies | User runs `cclab jet build -p @score/app` | `dist/` directory created with `index.html`, bundled JS, and CSS. Bundle is self-contained (no external CDN deps). Output is suitable for `include_str!()` embedding. |
...
changes:
  ...
  - file: apps/agentic-workflow/packages/@score/core/package.json
    impl_mode: hand-written
    action: create
    section: config
    description: Package manifest for @score/core.

  - file: apps/agentic-workflow/packages/@score/core/src/data-source.ts
    impl_mode: hand-written
    action: create
    section: state-machine
    description: SddDataSource interface definition with all method signatures.

  - file: apps/agentic-workflow/packages/@score/core/src/types.ts
    impl_mode: hand-written
    action: create
    section: dependency
    description: Shared TypeScript types (TechDesign, Change, Issue, LineageGraph, ProjectInfo).

  - file: apps/agentic-workflow/packages/@score/core/src/context.ts
    impl_mode: hand-written
    action: create
    section: db-model
    description: React Context for SddDataSource. Provider and useDataSource hook.

  - file: apps/agentic-workflow/packages/@score/core/src/index.ts
    impl_mode: hand-written
    action: create
    section: interaction
  ...
  # R3: LocalDataSource
  - file: apps/agentic-workflow/packages/@score/core/src/local-data-source.ts
    impl_mode: hand-written
    action: create
    section: component
  ...
  # R4: RemoteDataSource
  - file: apps/agentic-workflow/packages/@score/core/src/remote-data-source.ts
    impl_mode: hand-written
    action: create
    section: wireframe
  ...
  # R5: Create @score/app
  - file: apps/agentic-workflow/packages/@score/app/package.json
    impl_mode: hand-written
    action: create
    section: design-token
    description: Package manifest for @score/app. Dependencies on @score/core, @score/ui, @score/spec-viewer, @score/pipeline.

  - file: apps/agentic-workflow/packages/@score/app/src/routes.tsx
    impl_mode: hand-written
    action: create
    section: doc
    description: React Router route definitions. Exports RouteObject[] and ScoreRoutes component.

  - file: apps/agentic-workflow/packages/@score/app/src/pages/TechDesignList.tsx
    impl_mode: hand-written
    action: create
    section: logic
    description: List page for tech designs.

  - file: apps/agentic-workflow/packages/@score/app/src/pages/TechDesignDetail.tsx
    impl_mode: hand-written
    action: create
    section: cli
    description: Detail page for a single tech design with spec viewer.

  - file: apps/agentic-workflow/packages/@score/app/src/pages/ChangeList.tsx
    impl_mode: hand-written
    action: create
    section: schema
    description: List page for SDD changes.

  - file: apps/agentic-workflow/packages/@score/app/src/pages/ChangeDetail.tsx
    impl_mode: hand-written
    action: create
    section: rpc-api
    description: Detail page for a single change with specs, annotations, review actions.

  - file: apps/agentic-workflow/packages/@score/app/src/pages/IssueList.tsx
    impl_mode: hand-written
    action: create
    section: rest-api
    description: List page for issues.

  - file: apps/agentic-workflow/packages/@score/app/src/pages/IssueDetail.tsx
    impl_mode: hand-written
    action: create
    section: async-api
    description: Detail page for a single issue.

  - file: apps/agentic-workflow/packages/@score/app/src/pages/Lineage.tsx
    impl_mode: hand-written
    action: create
    section: config
    description: Lineage DAG visualization page.

  - file: apps/agentic-workflow/packages/@score/app/src/main.tsx
    impl_mode: hand-written
    action: create
    section: state-machine
    description: Standalone entry point for score view. Mounts ScoreRoutes with LocalDataSource.

  - file: apps/agentic-workflow/packages/@score/app/src/index.ts
    impl_mode: hand-written
    action: create
    section: dependency
    description: Library entry point. Exports ScoreRoutes and route config for Conductor embedding.

  - file: apps/agentic-workflow/packages/@score/app/index.html
    impl_mode: hand-written
    action: create
    section: db-model
  ...
  - file: apps/agentic-workflow/src/ui/viewer/assets/index.html
    impl_mode: hand-written
    action: delete
    section: interaction
    description: Replaced by @score/app dist/index.html.

  - file: apps/agentic-workflow/src/ui/viewer/assets/app.js
    impl_mode: hand-written
    action: delete
    section: component
    description: Replaced by @score/app dist/ bundle.

  - file: apps/agentic-workflow/src/ui/viewer/assets/styles.css
    impl_mode: hand-written
    action: delete
    section: wireframe
    description: Replaced by @score/app dist/ bundle.

  - file: apps/agentic-workflow/src/ui/viewer/assets/highlight.min.css
    impl_mode: hand-written
    action: delete
    section: design-token
    description: No longer needed — bundled in @score/app.

  - file: apps/agentic-workflow/src/ui/viewer/assets/highlight.min.js
    impl_mode: hand-written
    action: delete
    section: doc
    description: No longer needed — bundled in @score/app.

  - file: apps/agentic-workflow/src/ui/viewer/assets/mermaid.min.js
    impl_mode: hand-written
    action: delete
    section: logic
    description: No longer needed — bundled in @score/app.

  - file: apps/agentic-workflow/src/ui/viewer/dist/
    impl_mode: hand-written
    action: create
    section: cli
    description: Pre-built @score/app output (index.html + JS + CSS). Committed to repo.

  - file: apps/agentic-workflow/src/ui/viewer/mod.rs
    section: source
    impl_mode: hand-written
    action: modify
  ...
  - file: apps/agentic-workflow/src/ui/viewer/render.rs
    section: source
    impl_mode: hand-written
    action: modify
    description: Add functions to read and serialize tech designs, changes, issues from .aw/ directory.

  - file: apps/agentic-workflow/src/ui/viewer/manager.rs
    section: source
    impl_mode: hand-written
    action: modify
