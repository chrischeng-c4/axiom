...
main_spec_ref: apps/agentic-workflow/logic/spec-structure.md
merge_strategy: new
capability_refs:
...
Reorder `SectionType::fill_order()` in `apps/agentic-workflow/src/models/spec_rules.rs` so sections fill in human-reasoning order: overview (0) → requirements (1) → scenarios (2) → mindmap (3) → state-machine (4) → interaction (5) → logic (6) → dependency (7) → db-model (8) → schema (9) → rest-api (10) → rpc-api (11) → async-api (12) → cli (13) → wireframe (14) → component (15) → design-token (16) → config (17) → test-plan (18) → changes (19) → doc (20).

**Priority**: high

### R2: ALL_SECTIONS constant updated

Update `ALL_SECTIONS` constant in `apps/agentic-workflow/src/tools/common_change_spec.rs` to reflect the new top-down order matching `fill_order()` values.
...
files:
  ...
  - path: apps/agentic-workflow/src/models/spec_rules.rs (tests)
