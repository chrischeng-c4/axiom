...
main_spec_ref: apps/agentic-workflow/logic/codegen-behavioral.md
merge_strategy: new
capability_refs:
...
requirements:
  ...
  R4:
    ...
    notes: |
      ...
      Markers inventoried under /tmp/aw/workspaces/<workspace>/codegen_markers.yaml.
  R5:
...
changes:
  ...
  - path: /tmp/aw/workspaces/<workspace>/codegen_markers.yaml
    action: create
    section: logic
    impl_mode: codegen
    description: |
      Ephemeral inventory for all SPEC-REF markers emitted across all specs.
      Updated by aw td gen/apply. Used for local diagnostic visibility.
  - action: annotate
    section: async-api
    impl_mode: hand-written
