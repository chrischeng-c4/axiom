...
main_spec_ref: apps/agentic-workflow/logic/check-alignment.md
merge_strategy: new
capability_refs:
...
Change `SectionType::default_lang()` in `apps/agentic-workflow/src/models/spec_rules.rs` so that `schema`, `rpc-api`, `config`, `component`, and `design-token` return `"yaml"` instead of `"json"`.

**Priority**: high

### R2: format_rules.rs updated for yaml

Update `REQUIRED_CODE_BLOCK_TYPES` in `apps/agentic-workflow/src/spec_alignment/format_rules.rs`:
