---
id: sdd-logic-issues-push-through-source
...
capability_refs:
  ...
  - id: td-cb-lifecycle-automation
    ...
    coverage: full
    rationale: "Remote normalization preserves workflow-local implements ownership that issue trackers cannot represent."
---

# Issue Push-Through Source

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `apps/agentic-workflow/src/issues/push_through.rs` generated from AST during Score force-regeneration standardization.

Remote issue refresh restores the workflow-local `implements` paths after
server normalization. GitHub and GitLab do not model that ownership field, so
discarding it would force later TD commands to infer scope from unrelated
checkout history.
...
|------|--------|------|------------|------|-----------|
| `push_through` | apps/agentic-workflow/src/issues/push_through.rs | function | pub | 38 | push_through(     issue_path: &Path,     backend: &dyn IssueBackend,     slug: &str, ) -> Result<Issue> |
## Source
<!-- type: source lang: rust -->
<!-- source-from-target: strip-managed-markers -->

<!-- source-snapshot: path=apps/agentic-workflow/src/issues/push_through.rs -->
```rust
...
/// @spec apps/agentic-workflow/tech-design/core/logic/issues/push_through_source.md#source
...
```yaml
changes:
  - path: apps/agentic-workflow/src/issues/push_through.rs
    action: modify
    section: source
    impl_mode: codegen
    description: "Source template owns the full issue push-through module."
```
