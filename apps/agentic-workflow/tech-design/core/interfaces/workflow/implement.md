---
id: projects-sdd-src-workflow-implement-rs
...
# Standardized apps/agentic-workflow/src/workflow/implement.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `apps/agentic-workflow/src/workflow/implement.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `handle` | apps/agentic-workflow/src/workflow/implement.rs | function | pub | 29 | handle(change_dir: &Path, change_id: &str, project_path: &str) -> Result<Value> |
## Source
<!-- type: source lang: rust -->
<!-- source-from-target: strip-managed-markers -->

<!-- source-snapshot: path=apps/agentic-workflow/src/workflow/implement.rs -->
~~~rust
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/interfaces/workflow/implement.md#source
// CODEGEN-BEGIN
...
/// @spec apps/agentic-workflow/tech-design/core/interfaces/workflow/implement.md#source
pub fn handle(change_dir: &Path, change_id: &str, project_path: &str) -> Result<Value> {
    ...
    let task_ctx = if has_tasks {
        build_task_context(change_dir, state.current_task_id.as_deref(), &state.task_revisions)
    } else {
        None
    };
    ...
    build_response(&action, &mut base, change_id, project_path, &task_ctx, change_dir);
...
enum Action {
    BeginImplementation,
    ImplementTask { task_id: String },
    /// Structured codegen via Prism: task has a SpecIR-eligible spec_ref
    ImplementTaskWithCodegen { task_id: String, spec_ref: String },
    ReviewTask { task_id: String },
    ReviseTask { task_id: String, issues: Vec<String> },
    TaskTerminalFailure { task_id: String, revisions: u32 },
    AllTasksDone,
    /// Legacy: no tasks.md or unrecognized phase
    LegacyReview,
    LegacyResolve { issues: Vec<String> },
    LegacyComplete,
}
        ...
        StatePhase::ChangeImplementationCreated => {
            if ctx.all_done {
                ...
                if ctx.current_task_revisions > MAX_TASK_REVISIONS {
                    ...
                    if impl_path.exists() {
                        Action::ReviewTask { task_id: task_id.clone() }
                        ...
                        if helpers::is_codegen_eligible(change_dir, spec_ref) {
                            ...
                            Action::ImplementTask { task_id: task_id.clone() }
                        }
                    } else {
                        Action::ImplementTask { task_id: task_id.clone() }
                    }
...
fn build_response(
    ...
    match action {
        ...
        Action::LegacyReview => {
            ...
            base["next_phase"] = json!("change_implementation_approved or change_implementation_reviewed");
...
```
    ...
    fn test_codegen_eligible_task_routes_to_codegen() {
        ...
        assert!(result["spec_ref"].as_str().unwrap().contains("codegen-spec"));
    }
...
```yaml
changes:
  - path: apps/agentic-workflow/src/workflow/implement.rs
    action: modify
    section: source
    impl_mode: codegen
    description: |
      Source template owns the complete implementation workflow router.
```
