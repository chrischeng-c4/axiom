...
Public API manifest for `apps/agentic-workflow/src/workflow/scope.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `ScopeInfo` | apps/agentic-workflow/src/workflow/scope.rs | struct | pub | 17 |  |
| `build_spec_dir_tree` | apps/agentic-workflow/src/workflow/scope.rs | function | pub | 265 | build_spec_dir_tree(     spec_groups: &[String],     project_root: &Path,     config: Option<&SddConfig>, ) -> String |
| `extract_crate_labels_from_content` | apps/agentic-workflow/src/workflow/scope.rs | function | pub | 494 | extract_crate_labels_from_content(content: &str) -> Option<Vec<String>> |
| `extract_scope` | apps/agentic-workflow/src/workflow/scope.rs | function | pub | 33 | extract_scope(change_dir: &Path) -> ScopeInfo |
| `pre_filter_specs` | apps/agentic-workflow/src/workflow/scope.rs | function | pub | 206 | pre_filter_specs(     spec_groups: &[String],     project_root: &Path,     config: Option<&SddConfig>, ) -> String |
| `resolve_spec_dir` | apps/agentic-workflow/src/workflow/scope.rs | function | pub | 137 | resolve_spec_dir(     group: &str,     specs_base: &Path,     scopes: &HashMap<String, String>, ) -> Option<PathBuf> |
| `resolve_spec_dir_for_root` | apps/agentic-workflow/src/workflow/scope.rs | function | pub | 171 | resolve_spec_dir_for_root(     group: &str,     project_root: &Path,     specs_base: &Path,     scopes: &HashMap<String, String>, ) -> Option<PathBuf> |
| `suggest_topics` | apps/agentic-workflow/src/workflow/scope.rs | function | pub | 369 | suggest_topics(description: &str) -> Vec<&'static str> |
## Schema
...
/// @spec apps/agentic-workflow/tech-design/core/interfaces/workflow/scope.md#schema
#[derive(Debug, Clone, Default)]
...
mod tests {
    ...
    fn test_extract_scope_combined() {
        ...
        std::fs::write(
            change_dir.join("pre_clarifications.md"),
            "## Scope\n\nAffected: cclab-lens and apps/agentic-workflow/src/mcp/\n",
        )
        .unwrap();
    ...
    fn test_resolve_spec_dir_config_hit() {
        ...
        // Create the configured path: specs_base/apps/agentic-workflow
...
"#,
    ...
    fn test_pre_filter_specs_with_project_td_path() {
        let temp = TempDir::new().unwrap();
        let specs_dir = temp.path().join("apps/agentic-workflow/tech-design/logic");
        std::fs::create_dir_all(&specs_dir).unwrap();
        std::fs::write(specs_dir.join("state.md"), "# State\n").unwrap();
        std::fs::create_dir_all(temp.path().join(".aw")).unwrap();
        std::fs::write(
...
"#,
    ...
    fn test_pre_filter_specs_config_present_but_group_not_in_scopes() {
        ...
        let specs_dir = temp.path().join("apps/agentic-workflow/tech-design/core");
        std::fs::create_dir_all(&specs_dir).unwrap();
...
```yaml
changes:
  - path: apps/agentic-workflow/src/workflow/scope.rs
    action: modify
    section: source
    impl_mode: codegen
    description: |
