...
changes:
  - path: apps/agentic-workflow/src/issues/types.rs
    ...
    description: |
      ...
      The field is folded into `apps/agentic-workflow/tech-design/core/interfaces/issues/types.md#schema`
  ...
  - path: apps/agentic-workflow/tests/issue_create_worktree_test.rs
    action: modify
    section: test-plan
    impl_mode: hand-written
    description: |
...
- [changes] The decomposition covers all required touch points (types, CLI, resolver, three test files). The duplicate entry for `apps/agentic-workflow/src/issues/types.rs` is intentional (struct fields vs. test block) and unambiguous.
