---
id: sdd-interfaces-issues-backends-local-runtime-source
...
<!-- source-from-target: handwrite-gap local-backend-runtime -->
...
changes:
  - path: apps/agentic-workflow/src/issues/backends/local.rs
    ...
    impl_mode: codegen
    replaces:
      - "<handwrite-gap:local-backend-runtime>"
    description: "Source template owns local backend constructors and CRUD/search behavior."
```
