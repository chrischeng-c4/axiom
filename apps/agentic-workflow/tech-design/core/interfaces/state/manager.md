...
Public API manifest for `apps/agentic-workflow/src/state/manager.rs` generated from AST during Score force-regeneration standardization.
...
definitions:
  AgentLock:
    type: object
    required: [_file]
    description: File-based agent lock. Released automatically when dropped.
    properties:
      _file:
        type: object
        x-rust-type: "File"
        x-rust-visibility: private
        description: "Locked file handle."
    x-rust-struct:
      derive: []

  StateManager:
    type: object
    required: [change_dir, state, dirty, issue_slug, project_root]
    description: State manager for a single change.
    properties:
      change_dir:
        type: string
        x-rust-type: "PathBuf"
      ...
      dirty:
        type: boolean
        x-rust-visibility: private
        description: "Whether the state has unsaved changes."
      issue_slug:
        type: string
        x-rust-type: "Option<String>"
        x-rust-visibility: private
        description: "Issue slug, equals change_id."
      project_root:
        type: string
        x-rust-type: "Option<PathBuf>"
        x-rust-visibility: private
        description: "Project root derived from change_dir."
    x-rust-struct:
      derive: []

  StalenessReport:
    type: object
    required: [stale_files, missing_checksums, up_to_date]
    description: Report of file freshness vs recorded checksums.
    properties:
      stale_files:
        type: array
        items: { type: string }
        x-rust-type: "Vec<String>"
        description: "Files that have changed since last checksum."
      missing_checksums:
        type: array
        items: { type: string }
        x-rust-type: "Vec<String>"
        description: "Files without recorded checksums."
      up_to_date:
        type: array
        items: { type: string }
        x-rust-type: "Vec<String>"
        description: "Files that are up to date."
    x-rust-struct:
      derive: [Debug, Clone]
```
...
//! @spec apps/agentic-workflow/tech-design/core/interfaces/models/state.md#R5
//! @spec apps/agentic-workflow/tech-design/core/interfaces/models/state.md#R6
...
/// @spec apps/agentic-workflow/tech-design/core/interfaces/state/manager.md#source
...
/// @spec apps/agentic-workflow/tech-design/core/interfaces/state/manager.md#schema
        ...
        if state_path.exists() {
            anyhow::bail!(
                "STATE.yaml is deprecated. Migrate via `score changes migrate-legacy` \
                 ...
                 apps/agentic-workflow/tech-design/core/logic/issue-centric-workflow.md"
            );
        }
...
"#;
    ...
    // @spec apps/agentic-workflow/tech-design/core/interfaces/models/state.md#R11
...
```yaml
changes:
  - path: apps/agentic-workflow/src/state/manager.rs
    action: modify
    section: source
    impl_mode: codegen
    description: |
      Source template owns the complete state manager module.
  - action: annotate
    section: schema
...
- [overview] 3 types: lock, manager, report.
- [schema] All in `required:`; private foreign-type fields incl File and PathBuf.
