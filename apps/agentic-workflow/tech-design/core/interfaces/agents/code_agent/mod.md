...
definitions:
  CodeAgentConfig:
    type: object
    required: [model, max_tokens, temperature, base_branch, branch_prefix]
  ...
  CodeAgent:
    type: object
    required: [config, provider, platform]
...
~~~~~rust
// SPEC-MANAGED: apps/agentic-workflow/tech-design/core/interfaces/agents/code_agent/mod.md#source
// CODEGEN-BEGIN
...
//!   │
//!   ├─ LLM generation → multi-file XML artifact
//!   │
...
impl Agent for CodeAgent {
    ...
    async fn run_with_handler(
        ...
        // 3. Generate the candidate artifact once. EC is the implementation
        // oracle; code-level review/revise loops are intentionally absent.
        let generator = InlineLLMAgent {
        ...
        let artifact = generator.run(&generation_prompt).await?;

        // 4. Parse XML file blocks.
        let file_blocks = parse_file_blocks(&artifact)?;
        ...
        let pr = self
            ...
            .create_pull_request(&PullRequestParams {
                ...
                body: "Automated implementation generated by CodeAgent; external-contract verification owns acceptance.".to_string(),
                head: branch_name,
...
impl CodeAgentBuilder {
    ...
    /// Set the LLM provider used for code generation.
...
changes:
  - path: apps/agentic-workflow/src/agents/code_agent/mod.rs
    ...
    impl_mode: codegen
    description: |
      Source template owns the linear code implementation; semantic
