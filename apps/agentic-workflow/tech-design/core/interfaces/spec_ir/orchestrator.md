...
# Codegen Strategy Type

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `apps/agentic-workflow/src/spec_ir/orchestrator.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `CodegenStrategy` | apps/agentic-workflow/src/spec_ir/orchestrator.rs | enum | pub | 15 |  |
| `detect_yaml_ir` | apps/agentic-workflow/src/spec_ir/orchestrator.rs | function | pub | 28 | detect_yaml_ir(change_dir: &Path) -> Vec<PathBuf> |
| `has_yaml_ir` | apps/agentic-workflow/src/spec_ir/orchestrator.rs | function | pub | 81 | has_yaml_ir(change_dir: &Path) -> bool |
| `resolve_strategy` | apps/agentic-workflow/src/spec_ir/orchestrator.rs | function | pub | 63 | resolve_strategy(change_dir: &Path, legacy_allowed: bool) -> CodegenStrategy |
## Schema
...
definitions:
  CodegenStrategy:
    type: string
    enum: [YamlPipeline, LegacyFallback, NoIrError]
    description: |
    ...
    x-rust-enum:
      ...
      variants:
        - name: YamlPipeline
          kind: struct
          doc: "YAML IR files found — invoke Lens with these paths."
          fields:
            - { name: manifest_paths, rust_type: "Vec<PathBuf>", doc: "Paths to manifest files." }
        - { name: LegacyFallback, doc: "No YAML IR, but legacy fallback is allowed." }
        - { name: NoIrError, doc: "No YAML IR and legacy is disabled — error state." }
```
...
<!-- source-snapshot: path=apps/agentic-workflow/src/spec_ir/orchestrator.rs -->
```rust
...
/// Strategy resolved by the orchestrator.
/// @spec apps/agentic-workflow/tech-design/core/interfaces/spec_ir/orchestrator.md#schema
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// @spec apps/agentic-workflow/tech-design/core/interfaces/spec_ir/orchestrator.md#source
...
```yaml
changes:
  - path: apps/agentic-workflow/src/spec_ir/orchestrator.rs
    action: modify
    section: source
    impl_mode: codegen
    description: |
      Source template owns the complete SpecIR orchestration module.
  - action: annotate
    section: schema
...
- [overview] Single enum with mixed unit + struct variants.
- [schema] x-rust-enum.variants with kind=struct for YamlPipeline.
