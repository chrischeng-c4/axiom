...
main_spec_ref: apps/agentic-workflow/tech-design/surface/specs/issue-cli-envelope.md
merge_strategy: new
...
> all emit sites in `apps/agentic-workflow/src/cli/`. New code MUST NOT rely on
...
| File | Action | Purpose |
|------|--------|---------|
| `apps/agentic-workflow/src/cli/issues.rs` | modify | `IssueEnvelope` enum, envelope emission in `run_create`, new `FillSection` subcommand + `run_fill_section` (brief + apply) |
| `CLAUDE.md` | modify | Insert "Score envelope (mainthread protocol)" section |
| `apps/agentic-workflow/templates/mainthread/CLAUDE.md.tmpl` | modify | Same as CLAUDE.md (template source of truth) |
| `apps/agentic-workflow/templates/mainthread/agents/score-issue-author.md` | modify | Rewrite Inputs/Process/Output to consume envelope JSON + write `.aw/payloads/<slug>/body.md`. Add `Write` tool + PreToolUse guard |
| `apps/agentic-workflow/templates/mainthread/skills/score-issue/SKILL.md` | modify | Slim to intent-router; delegate envelope handling to CLAUDE.md |
| `apps/agentic-workflow/templates/mainthread/settings.json` | modify | Register `score-issue-author` SubagentStop hook ahead of generic `score-*` validate hook |
| `apps/agentic-workflow/templates/mainthread/hooks/agents/issue-author/pretooluse-write-guard.sh` | create | Allow Write only to `.aw/payloads/<slug>/body.md` |
| `apps/agentic-workflow/templates/mainthread/hooks/agents/issue-author/subagentstop-apply.sh` | create | Parse transcript first row as envelope JSON, call `fill-section --apply`, return hook decision |
| `apps/agentic-workflow/templates/mainthread/hooks/global/subagentstop-validate.sh` | modify | Add early-skip for `score-issue-author` so the new specific hook owns it |
| `apps/agentic-workflow/src/cli/init.rs` | modify | `include_str!` + installation for the two new hook scripts |
| `apps/agentic-workflow/tech-design/surface/specs/issue-cli-envelope.md` | create | This TD |
