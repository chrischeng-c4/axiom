---
id: apps-agentic-workflow-tests-cli-tests-wi-close-remote-test-rs
...
capability_refs:
  - id: work-item-planning
    ...
    coverage: full
    rationale: "Real-CLI tests prove configured remote issue rehydration, explicit repository routing, idempotent close and reason posting, actionable missing diagnostics, and preserved local close behavior."
---

# Standardized apps/agentic-workflow/tests/cli/tests/wi_close_remote_test.rs

## Overview
<!-- type: overview lang: markdown -->

Offline integration coverage for `aw wi close`: a temp-HOME `gh` adapter forwards the real binary's backend calls to an in-process HTTP fixture server, so numeric remote lookup and mutation are verified without touching GitHub.

## Source
<!-- type: source lang: rust -->
<!-- source-from-target: strip-handwrite -->

<!-- source-snapshot: path=apps/agentic-workflow/tests/cli/tests/wi_close_remote_test.rs -->
````rust
...
changes:
  - path: apps/agentic-workflow/tests/cli/tests/wi_close_remote_test.rs
    ...
    section: source
    description: |
      Issue #1551 regression coverage drives the real aw binary against a
      loopback-only GitHub backend fixture. Issue #1583 is the same defect and
      is retained as a second field reproducer rather than a duplicate fix.
```
