...
fn reconcile_producer(
    ...
    let desired = desired_document(existing.as_deref(), target_name, producer, document_root)?;
    match desired {
...
fn desired_document(
    ...
    if matches!(
        ...
        let desired = render_agent_document(existing, producer.kind);
...
fn render_agent_document(existing: Option<&str>, kind: ProducerKind) -> String {
    let claude_section = rendered_claude_section();
    let (section, fresh) = match kind {
        ProducerKind::RepoClaude => (claude_section, rendered_claude_document()),
        ProducerKind::RepoAgents => {
