...
`aw td create <wi>` is now a Python TD producer. It initializes one
WI-bound module under the resolved project-local `tech-design/src` package,
records that `.py` file as the workflow payload, and advances only after
`--apply` removes the bounded fill marker and the full Python TD root compiles.
The producer no longer authors Markdown TD skeletons or JSON section payloads;
legacy `--phase`, `--section`, and `--from-source` inputs fail with migration
guidance instead of selecting the retired path.

`aw td check` and `aw td ast` now compile only the complete canonical Python
TD project root. A configured project must match that exact root; Markdown
specs, slugs, and individual Python files are rejected instead of selecting a
second parser. The retired readers remain reachable only through the isolated
test-only legacy seam while corpus migration is in progress.
...
{}",
    ...
    fn preserve_dest_rel_falls_back_to_label_dir_when_src_outside_td() {
        ...
        assert_eq!(
            dest, "apps/agentic-workflow/tech-design/logic/external-spec.md",
...
changes:
  - path: apps/agentic-workflow/src/cli/td.rs
    ...
    section: source
    description: |
      Issue #2713 makes the complete Python TD project root the sole public
      input to `aw td check` and `aw td ast`. Configured projects must resolve
      to that exact root; Markdown files, slugs, individual Python modules, and
      the retired section-type reader fail instead of selecting an implicit
      legacy parser.
  - path: apps/agentic-workflow/src/cli/td.rs
    action: modify
    impl_mode: codegen
    section: source
    description: |
      Issue #2711 makes `aw td create` initialize and apply one WI-bound,
      project-local Python TD module. The lifecycle payload is the `.py`
      source itself; apply rejects the bounded fill marker, validates WI and
      artifact identity, compiles the complete Python TD root, and routes to
      `aw td check` without creating Markdown or JSON authoring artifacts.
  - path: apps/agentic-workflow/src/cli/td.rs
    action: modify
    impl_mode: codegen
    section: source
    description: |
      ...
      section-level lifecycle trailers. Adds JSON-payload rendering for the
      Mermaid Plus contract/unit-test section class (#1097): agents write only
      structured requirements JSON under the fill-loop payload path
      (`<section>.json`); `aw td create --apply` deterministically renders the
