---
id: projects-meter-examples-baseline-demo-rs
...
# Standardized apps/meter/examples/baseline_demo.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `apps/meter/examples/baseline_demo.rs` generated from AST during Score force-regeneration standardization.
...
fn main() {
    ...
    match store.load_baseline("demo", "latest") {
        Ok(baseline) => {
            ...
            let report =
                RegressionDetector::detect_regressions(&baseline, &new_results, &thresholds);
            ...
            let p99_thresholds =
                RegressionThresholds::default().with_percentile(PercentileType::P99);

            let p99_report =
                RegressionDetector::detect_regressions(&baseline, &new_results, &p99_thresholds);
            ...
            if !p99_report.regressions.is_empty() {
                ...
                for reg in &p99_report.regressions {
                    println!(
                        "  - {}: {:.1}ms → {:.1}ms ({:+.1}%)",
                        reg.name, reg.baseline_value_ms, reg.current_value_ms, reg.percent_change
                    );
                }
            } else {
            ...
            let p95_thresholds =
                RegressionThresholds::default().with_percentile(PercentileType::P95);

            let p95_report =
                RegressionDetector::detect_regressions(&baseline, &new_results, &p95_thresholds);
...
```yaml
changes:
  - path: apps/meter/examples/baseline_demo.rs
    action: modify
    section: rust-source-unit
    impl_mode: codegen
    description: |
      Source template for `apps/meter/examples/baseline_demo.rs` captured during meter full-codegen standardization.
```
