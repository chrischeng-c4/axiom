---
id: projects-meter-tests-async-fuzzing-test-rs
...
# Standardized apps/meter/tests/async_fuzzing_test.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `apps/meter/tests/async_fuzzing_test.rs` generated from AST during Score force-regeneration standardization.
...
async fn test_async_fuzzing_basic() {
    ...
    println!(
        "Completed {} iterations, found {} crashes",
        result.iterations,
        result.crashes.len()
    );
...
async fn test_parallel_fuzzing() {
    ...
    println!(
        "Parallel fuzzing: {} iterations in {}ms",
        result.iterations,
        elapsed.as_millis()
    );
...
async fn test_timeout_handling() {
    ...
    println!(
        "Timeout test: {} crashes (all should be timeouts)",
        result.crashes.len()
    );
...
async fn test_empty_corpus_handling() {
    let config = AsyncFuzzConfig::new().with_iterations(10).with_seed(42);
...
```yaml
changes:
  - path: apps/meter/tests/async_fuzzing_test.rs
    action: modify
    section: rust-source-unit
    impl_mode: codegen
    description: |
      Source template for `apps/meter/tests/async_fuzzing_test.rs` captured during meter full-codegen standardization.
```
