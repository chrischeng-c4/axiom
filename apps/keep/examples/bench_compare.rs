...
async fn bench_keep(args: &Args, val: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pool = h2c::H2cPool::with_connections_and(n_clients, Some(Duration::from_secs(10)), None)?;
        ...
        for w in 0..args.concurrency {
            ...
            handles.push(tokio::spawn(async move {
                ...
                for r in 0..n_req {
                    ...
                    let t = Instant::now();
                    if batch == 1 {
                        let url = format!("{base_url}/kv/{}", keys[0]);
                        if phase == "SET" {
                        ...
                        let _ = client
                            .post(format!("{base_url}/kv:mset"))
                            .json(&json!({ "entries": entries }))
                            .send()
                            .await;
                    } else {
                        let _ = client
                            .post(format!("{base_url}/kv:mget"))
                            .json(&json!({ "keys": keys }))
                            .send()
                            .await;
                    }
