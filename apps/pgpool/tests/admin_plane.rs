...
#[test]
fn admin plane serves all R1 routes on h2c and HTTP/1.1() {
    let mut cmd = Command::new("sh");
    cmd.arg("-c").arg("cargo test -p pgpool --test admin_plane all_routes_respond_on_h2c_and_http1 -- --nocapture");
    let output = cmd.output().expect("e2e command should run");
...
#[test]
fn POST /drain flips /readyz to 503 while an in-flight transaction completes, then the process exits cleanly() {
    let mut cmd = Command::new("sh");
    cmd.arg("-c").arg("cargo test -p pgpool --test admin_plane drain_flips_readyz_and_process_exits_cleanly -- --nocapture");
    let output = cmd.output().expect("e2e command should run");
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert_eq!(output.status.code().unwrap_or(-1), 0, "stdout:\n{}\nstderr:\n{}", stdout, stderr);
...
#[test]
fn served /openapi.json and route set are byte-identical to the offline pgpool spec inventory() {
    let mut cmd = Command::new("sh");
    cmd.arg("-c").arg("cargo test -p pgpool --test admin_plane served_contract_matches_offline_spec -- --nocapture");
    let output = cmd.output().expect("e2e command should run");
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert_eq!(output.status.code().unwrap_or(-1), 0, "stdout:\n{}\nstderr:\n{}", stdout, stderr);
...
#[test]
fn GET /metrics exposes pool gauges in Prometheus text format() {
    let mut cmd = Command::new("sh");
    cmd.arg("-c").arg("cargo test -p pgpool --test admin_plane metrics_exposes_prometheus_pool_gauges -- --nocapture");
    let output = cmd.output().expect("e2e command should run");
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert_eq!(output.status.code().unwrap_or(-1), 0, "stdout:\n{}\nstderr:\n{}", stdout, stderr);
