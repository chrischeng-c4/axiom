...
// Contract: lumen llm outline publishes a cclab.llm.v2 task manifest and each advertised topic preserves the task-navigation contract.
// Contract: lumen llm integrate-source-db recommends the Postgres/AlloyDB boundary: database commit/outbox or CDC, external adapter-owned Pub/Sub retry/DLQ, HTTP writes into lumen, and no direct external publishing to lumen's internal broker WAL.
#[test]
