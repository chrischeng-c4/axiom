...
struct FakeControl {
    ...
    /// Forces exactly one CR-spec patch failure. This covers the write-ahead
    /// boundary for #1485: a failed durable claim must not fire the external
    /// rolling-restart side effect that it is meant to bound.
...
impl FakeControl {
    fn new(cluster: Arc<Mutex<Lumen>>, shard_urls: Vec<String>) -> Self {
        Self {
            ...
            restart_flips: None,
            fail_next_spec_patch: AtomicBool::new(false),
        }
    }
    ...
    fn fail_next_spec_patch(&self) {
        self.fail_next_spec_patch.store(true, Ordering::SeqCst);
    }
...
impl ClusterControl for FakeControl {
    async fn patch_spec(
        ...
        if self.fail_next_spec_patch.swap(false, Ordering::SeqCst) {
            anyhow::bail!("forced spec patch failure");
        }
...
async fn convergence_stall_triggers_exactly_one_remediation_restart_then_converges() {
    ...
    // A failed write-ahead claim must not fire a restart. This is the failure
    // ordering that used to permit duplicate restarts: the old implementation
    // triggered first and only then tried to persist count=1.
    control.fail_next_spec_patch();
    let lumen = control.snapshot();
    let outcome = drive_tick(&control, &http, &lumen).await;
    assert!(
        matches!(outcome, DriveOutcome::Blocked(ref message) if message.contains("restart claim")),
    ...
    assert_eq!(
        ...
        0,
        "the failed patch must not consume the remediation slot"
    );

    // Next tick: stall budget exceeded + rollout converged + pods still
    // stale -> the successful write-ahead claim permits exactly one bounded
    // remediation restart to fire.
    let lumen = control.snapshot();
    let outcome = drive_tick(&control, &http, &lumen).await;
    assert_eq!(
        outcome,
