// SPEC-MANAGED: apps/lumen/external-contracts/claim-closure/production-claims.md#lumen-claim-k8s-topology-hpa-handoff
// CODEGEN-BEGIN
// AW-EC-BEGIN
...
// Contract: The Kubernetes reconcile loop deletes stale autoscaling state when fixed storage topology takes over.
#[test]
#[ignore = "AW EC gate: run via `aw health --verify-ec` or `cargo test -- --ignored`"]
fn lumen_claim_k8s_topology_hpa_handoff() {
    let command =
        "cargo test -p lumen --features operator --lib prune_stale_hpa_deletes_operator_rendered_hpa_on_multi_shard -- --nocapture";
    let id = "lumen-claim-k8s-topology-hpa-handoff";
