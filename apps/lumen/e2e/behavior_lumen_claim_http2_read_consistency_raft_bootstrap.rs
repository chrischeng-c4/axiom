...
// @contract http2-read-consistency-raft-bootstrap
// @category behavior
// @required_for_production true
// @command cargo test -p lumen --test api_e2e bounded_zero_rejects_lagged_follower -- --exact --nocapture
// AW-EC-END

// Contract: The serving bootstrap supplies raft state to the API so bounded reads reject a lagged follower.
#[test]
#[ignore = "AW EC gate: run via `aw health --verify-ec` or `cargo test -- --ignored`"]
fn lumen_claim_http2_read_consistency_raft_bootstrap() {
    let command =
        "cargo test -p lumen --test api_e2e bounded_zero_rejects_lagged_follower -- --exact --nocapture";
    let id = "lumen-claim-http2-read-consistency-raft-bootstrap";
