...
fn dx_llm_composes_library_owned_provider_content() {
    ...
    for needle in [
        ...
        ".github/workflows/lumen-release-recovery.yml",
        "fixed-identity",
        ".github/workflows/lumen-release-recovery.yml is only for lumen@0.4.28",
        ".github/workflows/lumen-release-0.4.29-recovery.yml is only for lumen@0.4.29",
        ".github/workflows/lumen-release-0.4.30-recovery.yml is only for lumen@0.4.30",
    ...
    for needle in [
        ...
        "For the bare binary only, no data directory means replacement loses state",
        "LUMEN_WAL=embedded",
        "LUMEN_DATA_DIR=/var/lib/lumen/data",
        "LUMEN_PERSISTENCE=segment",
        "Stop the old container cleanly",
        "VOLUME [\"/var/lib/lumen/data\"]",
        "Standalone GKE uses the shared StatefulInstancePlan boundary to render one StatefulSet plus a separately owned PVC instance",
        ...
        "public config has no image field",
        "manual, controller-run, and paid",
        "candidate CI makes no GKE or gcloud claim",
