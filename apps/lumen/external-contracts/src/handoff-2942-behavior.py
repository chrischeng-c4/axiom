...
from lumen.topology.availability import availability_promise, planned_handoff_availability
...
HANDOFF_2942_BEHAVIOR_MATRIX = (
    ...
    ("protect_new_voter_set_advances_only_to_transfer_leadership_routing", "transfer_leadership_routing"),
    ("transfer_leadership_routing_advances_only_to_commit_membership", "commit_membership"),
    ("commit_membership_advances_only_to_drain_old", "drain_old"),
    ("drain_old_advances_only_to_remove_old", "remove_old"),
    ("remove_old_advances_only_to_clean_up", "clean_up"),
    ("post_protection_phase_skip_is_rejected", "out_of_order_phase"),
    ...
    ("one_voter_unexpected_node_loss_has_no_availability_promise", "no_promise_on_unexpected_node_loss"),
    ("rollback_status_reports_old_generation_as_authoritative", ("rollback", "generation-7")),
)
...
def verify_handoff_2942_behavior() -> dict:
    ...
    # 1-9. R1 -- the persisted phase machine permits only the stated order.
    ...
    fourth = decide_handoff_transition(HandoffPhase.PROTECT_NEW_VOTER_SET, HandoffPhase.TRANSFER_LEADERSHIP_ROUTING, transition_facts)
    obs4 = _outcome(fourth); exp4 = HANDOFF_2942_BEHAVIOR_MATRIX[3][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[3][0], "expected": exp4, "observed": obs4, "passed": obs4 == exp4})
    fifth = decide_handoff_transition(HandoffPhase.TRANSFER_LEADERSHIP_ROUTING, HandoffPhase.COMMIT_MEMBERSHIP, transition_facts)
    obs5 = _outcome(fifth); exp5 = HANDOFF_2942_BEHAVIOR_MATRIX[4][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[4][0], "expected": exp5, "observed": obs5, "passed": obs5 == exp5})
    sixth = decide_handoff_transition(HandoffPhase.COMMIT_MEMBERSHIP, HandoffPhase.DRAIN_OLD, transition_facts)
    obs6 = _outcome(sixth); exp6 = HANDOFF_2942_BEHAVIOR_MATRIX[5][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[5][0], "expected": exp6, "observed": obs6, "passed": obs6 == exp6})
    seventh = decide_handoff_transition(HandoffPhase.DRAIN_OLD, HandoffPhase.REMOVE_OLD, transition_facts)
    obs7 = _outcome(seventh); exp7 = HANDOFF_2942_BEHAVIOR_MATRIX[6][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[6][0], "expected": exp7, "observed": obs7, "passed": obs7 == exp7})
    eighth = decide_handoff_transition(HandoffPhase.REMOVE_OLD, HandoffPhase.CLEAN_UP, transition_facts)
    obs8 = _outcome(eighth); exp8 = HANDOFF_2942_BEHAVIOR_MATRIX[7][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[7][0], "expected": exp8, "observed": obs8, "passed": obs8 == exp8})
    skipped_after_protection = decide_handoff_transition(HandoffPhase.PROTECT_NEW_VOTER_SET, HandoffPhase.DRAIN_OLD, transition_facts)
    obs9 = _outcome(skipped_after_protection); exp9 = HANDOFF_2942_BEHAVIOR_MATRIX[8][1]
    ...
    # 10-12. R2/AC4 -- a surge is visible and convergence is exactly one steady generation.
    obs10 = surge.active_generation_roles; exp10 = HANDOFF_2942_BEHAVIOR_MATRIX[9][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[9][0], "expected": exp10, "observed": obs10, "passed": obs10 == exp10})
    obs11 = surge.temporary_generation_cost; exp11 = HANDOFF_2942_BEHAVIOR_MATRIX[10][1]
    ...
    obs12 = converged.steady_generation_count; exp12 = HANDOFF_2942_BEHAVIOR_MATRIX[11][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[11][0], "expected": exp12, "observed": obs12, "passed": obs12 == exp12})
    ...
    # 13-14. R3 -- a learner is explicit input, but never changes the protected quorum.
    obs13 = voter.protected_committed_voters if not isinstance(voter, Rejection) else ()
    exp13 = HANDOFF_2942_BEHAVIOR_MATRIX[12][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[12][0], "expected": exp13, "observed": obs13, "passed": obs13 == exp13})
    obs14 = voter.quorum_voters if not isinstance(voter, Rejection) else ()
    exp14 = HANDOFF_2942_BEHAVIOR_MATRIX[13][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[13][0], "expected": exp14, "observed": obs14, "passed": obs14 == exp14})

    # 15. R4/AC2 -- replaying a persisted phase yields one action, not a second member add.
    ...
    obs15 = _outcome(resume); exp15 = HANDOFF_2942_BEHAVIOR_MATRIX[14][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[14][0], "expected": exp15, "observed": obs15, "passed": obs15 == exp15})

    # 16. R5/AC4 -- an explicit unschedulable target leaves the known-good old member authoritative.
    ...
    obs16 = (failed.action, failed.authoritative_member) if not isinstance(failed, Rejection) else (failed.reason.value, "")
    exp16 = HANDOFF_2942_BEHAVIOR_MATRIX[15][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[15][0], "expected": exp16, "observed": obs16, "passed": obs16 == exp16})

    # 17-18. R6 -- planned movement is distinct from the no-promise unexpected-loss policy.
    obs17 = planned_handoff_availability(voters=1, target_healthy=True, routing_transferred=False)
    exp17 = HANDOFF_2942_BEHAVIOR_MATRIX[16][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[16][0], "expected": exp17, "observed": obs17, "passed": obs17 == exp17})
    obs18 = availability_promise(1)
    exp18 = HANDOFF_2942_BEHAVIOR_MATRIX[17][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[17][0], "expected": exp18, "observed": obs18, "passed": obs18 == exp18})

    # 19. AC4 -- rollback status itself, not merely admission, retains old-generation authority.
    rollback = derive_handoff_status(HandoffFacts(
        old_member="old-a", target_member="new-c", old_generation="generation-7", target_generation="generation-8",
        active_generation_roles=(("generation-7", "old"), ("generation-8", "failed_target")),
        old_voters=("old-a",), new_voters=("new-c",), committed_voters=("old-a",), learners=(),
        protected_voters=("old-a",), target_healthy=False, phase=HandoffPhase.PROVISION_TARGET,
    ))
    obs19 = (rollback.state, rollback.authoritative_generation)
    exp19 = HANDOFF_2942_BEHAVIOR_MATRIX[18][1]
    checks.append({"name": HANDOFF_2942_BEHAVIOR_MATRIX[18][0], "expected": exp19, "observed": obs19, "passed": obs19 == exp19})
