...
capability_refs:
  - id: "security-hardening"
    ...
    coverage: partial
    rationale: "auth.rs owns Lumen's half of delegated request auth: the audience, the collection/admin resource mapping, the role-to-verb translation, and the fail-closed wiring around service-auth's TokenReview/SubjectAccessReview mechanics."
...
|------|--------|------|------------|------|-----------|
| `AUDIENCE` | apps/lumen/src/auth.rs | const | pub | 89 | AUDIENCE: &str |
| `API_GROUP` | apps/lumen/src/auth.rs | const | pub | 92 | API_GROUP: &str |
| `COLLECTIONS_RESOURCE` | apps/lumen/src/auth.rs | const | pub | 97 | COLLECTIONS_RESOURCE: &str |
| `ADMIN_RESOURCE` | apps/lumen/src/auth.rs | const | pub | 101 | ADMIN_RESOURCE: &str |
| `AuthTarget` | apps/lumen/src/auth.rs | enum | pub | 112 |  |
| `attributes` | apps/lumen/src/auth.rs | function | pub | 120 | attributes(&self, namespace: &str, needed: Role) -> ResourceAttributes |
| `verb` | apps/lumen/src/auth.rs | function | pub | 148 | verb(role: Role) -> &'static str |
| `AuthConfig` | apps/lumen/src/auth.rs | struct | pub | 158 |  |
| `open` | apps/lumen/src/auth.rs | function | pub | 176 | open() -> Self |
| `required_in` | apps/lumen/src/auth.rs | function | pub | 186 | required_in(namespace: impl Into<String>) -> Self |
| `from_env` | apps/lumen/src/auth.rs | function | pub | 193 | from_env() -> Result<Self> |
| `LumenVerifier` | apps/lumen/src/auth.rs | struct | pub | 261 |  |
| `new` | apps/lumen/src/auth.rs | function | pub | 277 | new(cfg: Arc<AuthConfig>) -> Self |
| `delegated` | apps/lumen/src/auth.rs | function | pub | 288 | delegated(namespace: &str, backend: Arc<dyn ReviewBackend>) -> Result<Self> |
| `with_authenticator` | apps/lumen/src/auth.rs | function | pub | 299 | with_authenticator(namespace: &str, authenticator: Arc<DelegatedAuthenticator>) -> Self |
| `connect` | apps/lumen/src/auth.rs | function | pub | 314 | connect(cfg: &AuthConfig) -> Result<Self> |
| `render_metrics` | apps/lumen/src/auth.rs | function | pub | 341 | render_metrics(&self) -> String |
| `AuthContext` | apps/lumen/src/auth.rs | enum | pub | 393 |  |
| `ensure` | apps/lumen/src/auth.rs | function | pub | 425 | ensure(&self, collection_id: &str, needed: Role) -> Result<(), AuthErr> |
| `ensure_admin` | apps/lumen/src/auth.rs | function | pub | 432 | ensure_admin(&self, needed: Role) -> Result<(), AuthErr> |
| `subject` | apps/lumen/src/auth.rs | function | pub | 453 | subject(&self) -> Option<&str> |
| `auth_middleware` | apps/lumen/src/auth.rs | function | pub | 462 | auth_middleware(     State(verifier): State<Arc<LumenVerifier>>,     req: Request,     next: Next, ) -> Response |
| `AuthErr` | apps/lumen/src/auth.rs | enum | pub | 480 |  |
| `wire` | apps/lumen/src/auth.rs | function | pub | 522 | wire(&self) -> (StatusCode, &'static str, String) |
## Source
