---
id: projects-lumen-src-operator-fleet-rs
capability_refs:
  - id: "long-running-stability"
    role: primary
    claim: "kustomize-base-overlays-hpa"
    coverage: partial
    rationale: "Cluster-scoped LumenFleet: the control-plane object that declares which data-plane namespaces exist and what settings each one gets, so a multi-namespace install is one apply rather than one Lumen CR per team."
fill_sections: [overview, source, changes]
---

# Standardized apps/lumen/src/operator/fleet.rs

## Overview
<!-- type: overview lang: markdown -->

`LumenFleet` — one cluster-scoped object, applied into the control-plane
namespace by the platform team, that declares every data-plane namespace and
the settings each one gets. The fleet loop materializes one namespaced `Lumen`
per entry; the existing per-instance controller then reconciles each one
unchanged.

### Load-bearing decisions

| Decision | Why | Test that pins it |
|---|---|---|
| The CRD is `scope: Cluster` | A namespaced owner cannot legally own objects in other namespaces — Kubernetes emits `OwnerRefInvalidNamespace` and its GC treats the owner as absent, deleting the dependents | `the_fleet_crd_is_cluster_scoped` |
| Ownership is the `lumen.dev/fleet` label, **not** an `ownerReference` | An owner reference makes deleting one cluster-scoped object cascade-delete every tenant's index and PVCs; `prunePolicy: Retain` would be a lie | `a_materialized_instance_carries_no_owner_reference` |
| The steady-state apply omits `shardCount`, `shardMap`, `reshardPolicy.workflow` | The reshard driver writes those at runtime; an applier that listed them would revert a completed split, reset the map version, and re-migrate data that already moved | `the_steady_state_apply_never_claims_a_field_the_reshard_driver_owns` |
| Create uses a different field manager (`lumen-fleet-seed`) from apply (`lumen-fleet`) | Under server-side apply, a manager that once owned a field and later omits it *removes* it. Seeding the initial topology under a manager that never applies again keeps the steady-state apply-set from pruning it | `the_seed_create_carries_the_initial_topology` |
| `instances[].spec` is a free-form JSON Merge Patch, not an enumerated override struct | It covers every `LumenSpec` field, including ones added later, without a list that goes stale | `an_override_replaces_only_the_field_it_names` |
| A merged spec is round-trip diffed against a `LumenSpec` | serde ignores unknown keys, so `serving.cpuu: "8"` would otherwise be accepted, dropped, and the tenant would silently run on the default it thought it had changed | `a_misspelled_override_is_rejected_rather_than_ignored` |
| Deserialization goes through `serde_path_to_error` | Plain serde reports `invalid type: integer, expected a string` with no field path, leaving whoever reads the fleet's status to guess which of ~40 fields it means | `a_value_the_schema_rejects_names_the_field` |
| The fleet never creates namespaces | Namespace lifecycle (quotas, labels, Workload Identity bindings) belongs to whatever provisions the cluster; granting `create` would make one CR edit a cluster-wide namespace-creation privilege | RBAC grants `namespaces: [get, list, watch]` only |
| A pre-existing `Lumen` without this fleet's label is never overwritten | A hand-authored instance at the same name would otherwise be silently replaced by the fleet's defaults | `NotAdopted` branch in `converge` |
| Rejections are per-entry | One tenant's bad override must not stop every other tenant from converging | `one_rejected_entry_does_not_stop_the_others` |
| A poll loop, not `service_k8s::run` | `service_k8s::controller::reconcile` requires `metadata.namespace`, namespaces every child apply, and uses `Api::namespaced` for `patch_status` — none of which a cluster-scoped CR can satisfy | n/a (structural) |

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `FLEET_LABEL` | apps/lumen/src/operator/fleet.rs | const | pub | 62 | `&str` |
| `FLEET_MANAGER` | apps/lumen/src/operator/fleet.rs | const | pub | 65 | `&str` |
| `FLEET_SEED_MANAGER` | apps/lumen/src/operator/fleet.rs | const | pub | 71 | `&str` |
| `LumenFleetSpec` | apps/lumen/src/operator/fleet.rs | struct | pub | 102 | derives `CustomResource` -> `LumenFleet` |
| `FleetInstance` | apps/lumen/src/operator/fleet.rs | struct | pub | 123 | |
| `PrunePolicy` | apps/lumen/src/operator/fleet.rs | enum | pub | 155 | `Retain` \| `Delete` |
| `LumenFleetStatus` | apps/lumen/src/operator/fleet.rs | struct | pub | 170 | |
| `FleetEntryStatus` | apps/lumen/src/operator/fleet.rs | struct | pub | 193 | |
| `PlannedInstance` | apps/lumen/src/operator/fleet.rs | struct | pub | 207 | |
| `PlanOutcome` | apps/lumen/src/operator/fleet.rs | enum | pub | 215 | `Ready(Value)` \| `Rejected(String)` |
| `plan` | apps/lumen/src/operator/fleet.rs | function | pub | 243 | plan(fleet: &LumenFleet) -> Vec<PlannedInstance> |
| `seed_object` | apps/lumen/src/operator/fleet.rs | function | pub | 371 | seed_object(fleet: &str, planned: &PlannedInstance, spec: &Value) -> Value |
| `apply_object` | apps/lumen/src/operator/fleet.rs | function | pub | 389 | apply_object(fleet: &str, planned: &PlannedInstance, spec: &Value) -> Value |
| `spawn_fleet_loop` | apps/lumen/src/operator/fleet.rs | function | pub | 427 | spawn_fleet_loop(client: kube::Client) |
| `fleet_crd_yaml` | apps/lumen/src/operator/fleet.rs | function | pub | 637 | fleet_crd_yaml() -> String |
...
changes:
  - path: apps/lumen/src/operator/fleet.rs
    ...
    description: |
      Centralized control-plane configuration: one cluster-scoped `LumenFleet`
      in the control-plane namespace declares every data-plane namespace and
      the settings each one gets, replacing "one hand-authored Lumen CR per
      team namespace". Platform-owned settings (image, node pool,
      StorageClass, ServiceAccount) live in `spec.defaults`; app-team settings
      (CPU/memory request, disk size, credential source) live in each
      `spec.instances[].spec` as an RFC 7386 merge patch over the defaults.
      The steady-state apply deliberately omits `spec.shardCount`,
      `spec.shardMap`, and `spec.reshardPolicy.workflow` so server-side apply
      can never revert a reshard the driver completed.
```
