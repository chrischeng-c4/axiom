...
| **S5** | 規則 ＋ ratchet | `.claude/rules/`、`.claude/aw/verification/` | 「source 自己承載設計」成為有消費者會退件的規則 |
...
| 33 | **我自己的殘留掃描帶副檔名過濾，漏掉一份出貨契約** | L4d 之後我報「活物件殘留 0」，那是一支帶副檔名白名單的掃描算出來的，`.json` 不在裡面。`apps/lumen/clients/openapi.json` 有 **58 行** `@spec apps/lumen/tech-design/…` —— 那是出貨給使用者的 client 契約，不是文件。成因：L4a 從 `apps/lumen/src/types.rs` 拔掉 60 行 `@spec` doc comment，那些字串本來會被折進 OpenAPI 的 `description`，刪了之後產出就變了，而 L4a／L4b／L4c／L4d 沒有一輪重跑產生器。`apps/lumen/e2e/spec_cli.rs:1150` 的 `openapi_committed_snapshot_matches_live_generation` 一直在守這件事，它是對的；錯的是那四輪沒有人跑含它的那套測試。 | L4e（`acbe54e5ac`）用它自己的產生器重算，58 刪 / 36 增逐條對得上（36 條掉尾巴、22 條整個 `description` key 消失，與 L4b 的 CRD 同形），`@spec` 58 → 0，紅→綠都在同一棵樹上量過。掃描本身也換掉：現在走 `git ls-files` 全部 55,172 個 tracked 檔、**零副檔名過濾**。這是本戰役第四次同一種毛病（地雷 29／30／31 是前三次），而這一次犯的人是我。 |
| 34 | **同一棵 worktree 上有第二個活的 session 在寫，我的每一次 `git status` 都是髒的量測** | L4e 收工後 `git status` 冒出兩個我沒編輯過的檔（`apps/lumen/README.md`、`libs/cli-std/README.md`），後者的 mtime 距我看它只有 **21 秒**。查 `ps` 抓到元凶：另一個 Claude Code session （peer 清單上的 `app-lumen-e9 [3bbf98]`，session dir `5492d6d0-…`，已跑 2 小時）**cwd 就是這棵 worktree**，正在跑 `cargo test -p lumen --features operator --no-fail-fast`，scratchpad 裡是 `libaudit.py`／`consumers.py` 與 #3091–#3146 的 issue 草稿。兩分鐘內它從 2 個髒檔長到 **15 個**：`apps/lumen/README.md` ＋ 14 個 `libs/*/README.md`，共 +496 行。 | **先停，不是先修。** 這違反 `.claude/rules/operations/persistent-branches.md` 的 one-project-one-worktree 配置，也違反「不要平行跑寫入範圍重疊的 worker」。已核對我這四顆 commit （`6c0c64f3b0` / `c2b6a066df` / `acbe54e5ac` / `ba67a5715a`）的檔案清單，**沒有一個檔屬於對方** —— 污染沒有進 commit，但它進了我的量測：S4 每一輪的「樹是乾淨的」都是在有第二個寫者的情況下說的。S5 在人做出取捨前不開工：它的產出是一條要對整棵樹生效的 ratchet，而樹正在動，在會動的樹上量出來的 baseline 是假的。 |
...
| S4 / L4e 出貨契約重算 | **完成** | `acbe54e5ac`，2 檔 / +38 / −60。地雷 33 指名的 `apps/lumen/clients/openapi.json` 58 行斷指標，用 `./target/debug/lumen spec --format openapi > apps/lumen/clients/openapi.json` 重算，sha256 與事前獨立擷取的產生器 stdout 相同（`eacd9e68…2cea`）。同一輪順手修掉 `.claude/skills/dispatch-to-agy/scripts/make_profile.py:20` 的 usage 範例 —— 它同時指向已刪的 `libs/service-auth/external-contracts/` 與跑不起來的 `uv run pytest`，兩半都換成活的。**重掃（零副檔名過濾，55,172 檔）：16 檔 / 22 行**，全部歸得了戶 —— 儀器 12、本文件 7、`verify-lumen-auth.sh:726` 的 `SPEC-MANAGED` 1（依 USER DECISION 不管）、`apps/pgpool/tech-design/logic/adopt-shared-labeled-prometheus-metrics-encoder.md:66` 1。最後這條**是真的斷的**（它指向 L1 刪掉的 `libs/metrics-prometheus/tech-design/…`），刻意不動：pgpool 不在 D1 指名的 15 個 project 內，那一行在 pgpool 自己的活設計紀錄裡，改它等於改別人的歷史。本輪整套 `cargo test -p lumen --no-fail-fast`：167 個 target、**0 個 `error[`**、3 條紅 —— openapi 那條本輪修掉並重跑證明綠（44 passed / 0 failed）；`no_retired_credential_symbol_survives_unaccounted`（`retired_credential_surface.rs:397` 指 `README.md:368`）是 S3 之前就紅的；`gen_py_writes_pydantic_h2c_client`（`spec_gen_e2e.rs:76`）是環境問題，`ModuleNotFoundError: No module named 'pydantic'`。 |
| S5 規則 ＋ ratchet | **完成** | ratchet 是 `apps/lumen/e2e/design_trees_stay_retired.rs`（登記在 `apps/lumen/Cargo.toml`，stanza 數 163 → 164），規則是`.claude/rules/authoring/source-carries-its-own-design.md`。**範圍矛盾用量的解掉了**：S5 原本宣告的兩個 write root 裡，`.claude/aw/verification/run_all.py` 全 repo 沒有人呼叫（無 CI、無 git hook、無 phase script），所以 ratchet 放那裡等於沒有人會退件；改放 `apps/lumen/e2e/`，它被 `apps/lumen/CONTRIBUTING.md:18` 宣告的 `cargo test -p lumen` 直接跑到。規則檔仍然寫在 `.claude/rules/`，但它只說理由，退件的是 gate。**範圍是 15 個 project 不是整個 repo**，這也是量的：HEAD 上還有 **560 個**設計樹檔案活在**26 個**別的 owner（`apps/tape` 72、`apps/pgpool` 61、`apps/jet` 58、`projects/mamba` 24 …），15 個退役 project 裡是 **0**；寫成 repo-wide 第一次跑就紅，而且會一直紅。三條斷言：`no_retired_project_carries_a_design_tree`（結構性，一個 byte 都不讀）、`no_retired_project_carries_a_spec_pointer`（掃 15 個 project 底下每一個文字檔，零副檔名過濾）、`the_sweep_is_not_vacuous`（量儀器本身：15 個 root 都要存在、2 條豁免都要指到真檔、走訪檔數 ≥ 400，HEAD 實測 593）。第三條是為地雷 29／30／31／33 而存在的 —— 那四次全是同一個形狀，掃描面比宣告面窄。 |
| S5 negative control | **完成** | 三條斷言逐條證明會紅，不是推測。① 種一個 `libs/cli-std/tech-design/` 空目錄 → `no_retired_project_carries_a_design_tree` FAILED，訊息指名 `["libs/cli-std/tech-design"]`。② 三行 `@spec` 是**天然的紅**，不用種：`libs/service-http/src/transport.rs:3` 與 `libs/transport-h2c/src/server.rs:5` 都指向 **已刪除的** `apps/agentic-workflow/tech-design/…`，`libs/service-k8s/src/service.rs:37` 指向 `apps/pgpool/tech-design/…`（目標還活著，但那是把一個共用 lib 綁到某個 app 的私有設計文件上）。三行全部刪掉 → 綠。③ 把 sweep list 的 `libs/cli-std` 改成 `libs/cli-std-typo` → `the_sweep_is_not_vacuous` FAILED。全部還原後 `shasum -a 256 -c` 驗過 byte-identical：`b81dedeee9f1ada5550a7c083fb031d1aa60cf9e25b72d44cc31c3203fa81a2a`。順手修掉 `libs/service-auth/aw.toml` 的註解 —— 它說契約清單住在 `external-contracts/pyproject.toml`，那個檔 S4 就刪了。 |
| ~~S5 規則 ＋ ratchet~~ | **重複列，已作廢** | 這一列是 S5 開工前寫下的進場條件，S5 做完之後沒有清掉，於是同一個階段在這張表上同時是「完成」與「未開始」。以上面那兩列（`S5 規則 ＋ ratchet`、`S5 negative control`）的實測為準。地雷 32 依 USER DECISION 不補，這一句仍然成立。 |
| S6 拆掉 AW-EC 殼（70 檔） | **完成** | 進場基線 `ee2cec2aea`：`apps/lumen/e2e/` 164 個 `.rs` 裡有 **102** 個帶 `#[ignore = "AW EC gate: …"]`。其中 **101** 個是殼 —— 檔案主體就是 `sh -c "<@command>"` ＋ 斷言子行程的 exit status，沒有自己的觀測面。第 102 個 `spec_gen_e2e.rs` 是**字串誤判**：它是真的測試，`:168` 的 ignore 理由是「opens a local h2c listener」，剛好也以 “AW EC gate” 開頭。101 條 `@command` 依「`cargo test -p lumen` 到底跑不跑得到」四分：**A 67**（`--test <已宣告 target>`）、**B 5**（`--lib` / `--bin`）、**C 15**（`--features operator`，gate 根本不編譯 operator 模組）、**D 14**（`kustomize` / `target/debug/vat` / `scripts/kind-e2e.sh` / `cargo run`）。A＋B 是純冗餘：殼跑的東西 gate 已經在跑，唯一的獨有內容是那句 `// Contract:`。依 D3 把 70 句散文搬進它指名的 target 自己的 `//!`（25 個 owner；`e2e/lumen_bench_cli.rs` 原本 0 行 `//!`，替它開了一段），再刪殼。C＋D 這 29 個**沒有刪** —— 它們是全樹唯一記錄「這些檢查該跑」的東西，其中 C 那 15 個直接對上 `cargo test -p lumen` 不編譯 operator 這個洞。數字：`e2e/*.rs` 164 → 94、`Cargo.toml` `[[test]]` 164 → 94（雙向零差異）、`aw.toml` `[[aw.ec.generated.cases]]` 101 → 31（＝殘存 31 個殼，一對一）、帶 “AW EC gate” 的檔 102 → 32（31 ＋ `spec_gen_e2e.rs`）。 |
| S6 分類器的鑑別力缺口 | **已修，扣掉 2 個** | 第一版分類器先問 `"--test " in cmd`、才問「這條指令到底是不是 `cargo test`」，於是任何**外部 runner** 只要把 `--test <已宣告 target>` 透傳給 cargo 就會被歸成「冗餘」。從 HEAD 把 72 個候選逐檔讀回來重問「指令的第一個字」，抓到兩個假 A：① `efficiency_lumen_claim_elastic_disk_tier.rs` 的指令是 `target/debug/meter test -- -p lumen --test disk_scale_proof -- --ignored` —— 既不是 cargo，而且帶 `--ignored`，跑的是 `cargo test -p lumen` **會跳過**的重量級 `scale_proof_reopen_rss_is_bounded`；② `behavior_lumen_claim_cli_connect_query_workflow.rs` 在 `&&` 後面接了兩條 `cargo run -p lumen --bin lumen -- {connect,query} --help`，那是 exit-status-only 檢查，沒有別的東西在做。兩個都還原，實刪 **70** 不是 72。同一輪順便量了另一個形狀：70 個裡有 **36** 個帶名稱過濾器（`--exact` 或裸 substring）。過濾器是**收窄**，gate 跑的是超集，所以這 36 個的冗餘成立 —— 方向反過來（gate 比殼窄）才是問題。這是地雷 29／30／31／33 的第五個實例，形狀完全一樣：**掃描面比宣告面窄**。 |
| S6 指標修補 | **完成** | 刪檔會讓 5 條活的指標指向空氣，事後掃描（全 worktree、排除 `apps/lumen/e2e`）逐條修：`apps/lumen/README.md` 3 條 capability gate —— `--test security_lumen_claim_security_tls_rustls` → `--lib`（殼的指令本體是 `cargo test -p lumen --lib tls`）、兩條 `--test stability_lumen_topology_existing_raft_replica_sync` → `--test wal_nats_e2e`；`apps/tape/docs/service-maturity-matrix.md` 2 列 Lumen reference —— `e2e/behavior_lumen_cli_interface_protocol_transport.rs` → `e2e/protocol_transport_e2e.rs`、`tests/stability_lumen_topology_existing_raft_replica_sync.rs` → `apps/lumen/e2e/wal_nats_e2e.rs`（後者的 `tests/` 前綴在 S3 之後就已經是錯的，這輪連目錄一起修對）。**跨 project 修別人的 doc 是刻意的**：那兩列是我這輪刪檔弄斷的，不是既存的斷指標。修完重掃：指向已刪殼的殘留 **0**。 |
| S6 殘存 31 個殼的盤點 | **不刪，記錄** | 留下來的 31 個殼（102 − 70 − `spec_gen_e2e.rs`）是全樹**唯一**寫下「這些檢查該跑」的地方，所以一個都沒動。它們的 `@command` 去重後只有 **19 條**，31 → 19 的落差全是同一條指令被抄進好幾個 EC id：`--test operator_render`（無過濾器）一條就佔了 5 個殼，`kind-e2e.sh` 佔 3 個，`statefulset_wires_serving_contract_single_member`、`prune_stale_hpa_…`、`vat run rig-endurance`、`vat run rig-resilience`、`vat run ec-efficiency-meter`、kustomize 五連 各佔 2 個。**C（15 殼／9 條）** 全部帶 `--features operator`，`cargo test -p lumen` 連編都不編那個模組。但 C 當成 to-do 清單也是**破的**：`feature_gated_targets_are_registered.rs` 的 `REGISTRY` 列了 11 個 operator-gated target，C 只點名其中 **3** 個（`operator_render`、`reshard_driver_e2e`、`routed_shard_e2e`），另外 8 個（`access_render_cli`、`body_limit_configurable`、`capacity_catalog_client`、`capacity_catalog_contract`、`capacity_retire_hpa`、`cli_client_ksa_token`、`operator_backup_kubernetes_wiring`、`operator_retired_credential_projection`）**沒有任何殼、任何 gate 點名**——又是一次掃描面比宣告面窄。一條 `cargo test -p lumen --features operator` 就蓋掉 C 的全部 9 條。**D（16 殼／10 條）** 是 `cargo test` 以外的 runner：kustomize 五連＋單 overlay 各一條、`apps/lumen/scripts/kind-e2e.sh`、`target/debug/vat run {ec-efficiency-meter, rig-endurance, rig-load, rig-resilience}`、`target/debug/meter test … --ignored`、兩條 `cargo run … --help` 的 exit-status 檢查。D 沒有單一替代指令，得先有人決定這些 runner 誰還活著。 |
| S7 收掉 operator 那個洞 | **完成** | S6 的 C 類指向的不是「15 個殼」，是**根因**：`apps/lumen/Cargo.toml` `default = []`，9 個 feature，而 94 個 `[[test]]` stanza **一個都沒有 `required-features`**（`grep -c required-features` = 0）。所以 `cargo test -p lumen` **不是跳過**那 11 個 `#![cfg(feature = …)]` 的 target —— 它把每一個都編成**空的 binary**，印 `test result: ok. 0 passed`，然後 exit 0。四次全量測（同一棵樹 `d1f407f6cf`，全部 `--no-fail-fast`，各 97 個 binary，跑在隔離的 `CARGO_TARGET_DIR` 免得跟另一個 session 互相打壞 fingerprint）：`cargo test -p lumen` **627 p／6 f／43 ign／4 紅**；`--features operator` **922／4／43／3 紅**；`--features "operator delegated-auth"` **929／4／43／3 紅**；`--all-features` **930／5／43／4 紅**。11 個登記過的 gated target 在第二列底下 **0 → 177**：`operator_render` 73、`operator_backup_kubernetes_wiring` 19、`routed_shard_e2e` 17、`reshard_driver_e2e` 16、`capacity_catalog_contract` 14、`access_render_cli` 13、`cli_client_ksa_token` 7、`body_limit_configurable` 6、`capacity_catalog_client` 5、`capacity_retire_hpa` 4、`operator_retired_credential_projection` 3。這 177 條全部**不需要 cluster、不需要 kubeconfig、不需要外部網路、不需要另外 build 一個 binary**（`CARGO_BIN_EXE_lumen` 由同一次 cargo invocation 產出）；11 個裡**零個新紅**，那 3 個紅（`capability_shared_ownership`、`retired_credential_surface`、`spec_gen_e2e`）是第一列 4 個紅的子集，panic 訊息逐字相同。 |
| S7 為什麼是這兩個 feature，不是別的 | **量出來的，不是選的** | `--features operator` **蓋不到 11 個**，只蓋到 10 個。`e2e/cli_client_ksa_token.rs:29` 的 gate 是 `#![cfg(all(unix, feature = "delegated-auth", feature = "backup"))]`，而 `operator = [… "backup"]` 拉得進 `backup`、拉不進 `delegated-auth`（`delegated-auth` 是**刻意**分開的，Cargo.toml 的註解寫了理由：serving auth 不是 control-plane 的事）。所以 §6 第 299 列那句「一條 `cargo test -p lumen --features operator` 就蓋掉 C 的全部 9 條」對 **C 的 9 條指令**成立，對 **11 個 target 不成立** —— 拿它當 to-do 清單會少一個。`"operator delegated-auth"` 是能編出全部 11 個 gate 字串的**最小**組合。**`--all-features` 不能用**：它會連 `jieba` 一起開，而 `e2e/jieba_bigram_fallback_e2e.rs` 的 `//!` 明寫「feature 關掉才是重點」，檔案裡卻**一個 `cfg` 都沒有**，所以 `--all-features` 讓它在 `:94` 紅掉 —— 是假紅，不是回歸。**兩列都要留，誰也不是誰的超集**：feature 開著就編不到「關著才跑」的那些碼 —— `e2e/auth_e2e.rs:278` 是 `#[cfg(not(feature = "delegated-auth"))]`（`:285` 有活的雙胞胎），`src/bin/lumen.rs` 有 **15** 個 `#[cfg(not(feature = …))]` 的拒絕樁。 |
| S7 落地 ＋ 順手修掉的兩條假綠 | **完成** | `apps/lumen/CONTRIBUTING.md` 的 Verification 表從一列變兩列，新的一列附上上面的量測與三個紅的名字（GHAN 要求：baseline 不綠就逐一點名，不准只講數量）。新列**接在第 18 行後面**是刻意的 —— `.claude/rules/authoring/source-carries-its-own-design.md` 用行號引「`apps/lumen/CONTRIBUTING.md:18`」，沒有任何東西偵測這個引用漂移，append 才讓它繼續成立。另外 `apps/lumen/README.md` 兩條 `- Gate:` 本身就是同一個洞的實例：`:149` 是 `cargo test -p lumen --test operator_render`，**少了 `--features operator`**，而 `e2e/operator_render.rs:6` 是 `#![cfg(feature = "operator")]` —— 那條指令編一個空 binary、跑 0 個測試、exit 0；`:290` 指的是殼 `behavior_lumen_claim_cli_deployment_operator_command_surface`，它唯一的 `#[test]` 帶 `#[ignore]`，印 `0 passed; 1 ignored`、exit 0，而它自己的 `@command` 就是修好後的 `:149`。兩條都改成 `cargo test -p lumen --features operator --test operator_render`。改完 `meta.py check` **=> CLEAN**（184 META-docs ＋ 64 README，M1–M7 全 0），`capability_shared_ownership` 與 `retired_credential_surface` 重編重跑，失敗訊息與改前**逐字相同**（含 `README.md:310` 這個行號，兩條編輯都是原地換行、沒有位移）。 |
| S7 沒修的、以及第 299 列的一個錯 | **記錄** | **第 299 列數錯一個**：它說有 8 個 target「沒有任何殼、任何 gate 點名」，但 `operator_backup_kubernetes_wiring` 被 `apps/lumen/README.md:129` 與 `:269` 兩條 `- Gate:` 點名。真正沒人點名的是 **7** 個。拿 299 當工作清單會多做一個。**兩個假綠形狀報了但沒修**，因為 `e2e/feature_gated_targets_are_registered.rs` 這一輪不能動：① `jieba_bigram_fallback_e2e.rs` 該補的 `#![cfg(not(feature = "jieba"))]` 補不下去 —— 補了 `all_gated_files_in_tree_are_registered` 就會要求 REGISTRY 多一列，而那個檔在禁改清單裡；② `all_registered_targets_declare_non_zero_test_rows` 數的是 `#[test]` **原始碼行數**，`cli_client_ksa_token.rs` 有 7 行所以它綠，但宣告的 gate 實際執行 **0** 個 —— 量語法不量事實。**更好的修法沒做**：在那 11 個 `[[test]]` stanza 加 `required-features`，會把「沒開 feature」從**無聲**（編成空 binary）變成**有聲**（cargo 印 `skipping, required features not enabled`）；沒做是因為它同時改變裸 `cargo test -p lumen` 會 build 什麼，要自己一輪量測。**還有一件事這兩列都做不到**：全樹**沒有任何執行者**。實測：`.github/workflows/` 18 個檔零個 `cargo test -p lumen`（兩個 lumen workflow 只 `cargo build --release`，而 `cargo build` 從不編 `[[test]]` target —— 就是地雷 31）；`core.hooksPath` 空、共用 hooks 目錄只有 `*.sample`；`.pre-commit-config.yaml` 沒宣告 cargo test 而且沒安裝；沒有 Makefile／justfile；`.claude/settings.json` 沒有 `hooks` key。兩列都是**人跑的**。這一輪把 gate 從「沒宣告」變成「宣告了、跑起來會退件」，僅此而已。 |
| S8 收掉 C 組 15 個殼 | **完成** | S6 第 299 列把 C 那 15 個留下來，理由寫得很清楚：它們是全樹唯一記錄「這些檢查該跑」的東西，因為 `cargo test -p lumen` 不編譯 operator。S7 把那個洞補起來了 —— `apps/lumen/CONTRIBUTING.md` 的 Verification 表加了第二列 `cargo test -p lumen --features "operator delegated-auth"`，而那一列直接跑得到這 15 個 `@command` 指名的 target。留下來的理由消失，殼就只剩「gate 已經在跑的東西，再用子行程跑一次」。刪。數字：`e2e/*.rs` 94 → 79、`Cargo.toml` `[[test]]` 94 → 79（用 `tomllib` 對兩個方向的集合差，都是空）、`aw.toml` `[[aw.ec.generated.cases]]` 31 → 16。15 個 EC id 在全樹各出現剛好一次，位置都是新落地的 `//!`。`meta.py check` `=> CLEAN`（184 份 META-doc ＋ 64 份 README，M1–M7 全 0；M6 = 0 表示沒有任何 README 的 gate 指到被刪掉的 target）。 |
| S8 的三次 gate 量測 | **完成** | 全部帶 `--no-fail-fast`，並各自用獨立的 `CARGO_TARGET_DIR`，避開另一個 session 正在攪的那顆 191 G 共用 target。第 2 列 BEFORE（在 `db414b17db`）：97 個 binary、`929 passed; 4 failed; 43 ignored`、3 個紅 target。第 2 列 AFTER：82 個 binary、`929 passed; 4 failed; 28 ignored`、同樣 3 個紅。**passed 一個沒動、ignored 少 15、binary 少 15** —— 這就是「只刪掉冗餘」的簽名：每個殼只有一個 `#[test]`，而且是 `#[ignore]`，對 passed 的貢獻本來就是 0。4 個紅的測試名與 `file:line` 兩邊完全相同，斷言訊息把 thread id 與 hex hash 正規化後 `diff` 為空，sha256 `5451da50…b645` 兩邊一致。第 1 列 AFTER：82 個 binary、`629 passed; 4 failed; 28 ignored`、同樣那 3 個紅；第 2 列比第 1 列多跑 `+300`，其中 177 個來自那 11 個在第 1 列被編成空 binary 的 target。`apps/lumen/CONTRIBUTING.md` 那段量測數字同步改掉（97 → 82、`+302` → `+300`）。 |
| S8 落地位置，與一句自己寫錯的話 | **完成** | 15 句 `// Contract:` 依各自 `@command` 指名的 target 落地：`e2e/operator_render.rs` 9 句、`e2e/reshard_driver_e2e.rs` 3 句、`e2e/routed_shard_e2e.rs` 1 句、`src/operator/reconcile.rs` 2 句。格式沿用 S6 在 `e2e/reshard_admin_e2e.rs` 立下的那個 —— 標題 `## Contracts inherited from the retired EC shells`，每句前面掛它原本的 EC id。兩個細節記著：`routed_shard_e2e` 那個殼的指令帶 `-- --exact` 只跑一個測試，gate 跑的是它的超集；`reshard_durability` 那個殼還串了 `--test reshard_admin_e2e` 和 `--lib segment_rdb`，兩個一樣被同一列 gate 直接跑到，所以那句歸在它指名的第一個 target。**產生器套的共同前言寫錯一句，自己抓回來**：它說 `cargo test -p lumen` 把殼指名的 target「編成一個印 `0 passed` 的空 binary」—— 這對那 3 個 e2e target 成立，對 `src/operator/reconcile.rs` 不成立。`src/lib.rs:66` 的 `pub mod operator;` 沒有 cfg，但 `src/operator/mod.rs:26` 的 `pub mod reconcile;` 掛在 `#[cfg(feature = "operator")]` 底下，所以那個模組**根本不編譯**，殼的 `--lib` 名稱過濾器一個都對不到、印 `0 passed`、exit 0。兩種不同的隱形機制，同樣的 exit 0。已針對 `reconcile.rs` 改成後者。 |
| S8 量測時抓到的一個假紅 | **記錄** | S7 記在 `CONTRIBUTING.md` 的第 1 列基線是 `627 passed; 6 failed; 43 ignored`、4 個紅，比 S8 之後多一個 `auth_e2e`。刪掉 15 個 `#[ignore]` 的殼不可能讓 `auth_e2e` 變綠，所以先查了才寫。查出來是儀器的問題：`e2e/auth_e2e.rs:182` 給 spawn 出來的 `lumen serve` **20 秒** wall-clock 去拒絕啟動並退出，逾時就在 `:210` panic。而那個 panic 會把子行程的 stderr 印出來 —— 也就是**正確的拒絕訊息** —— 所以它一邊說 server「kept running」，一邊引用了它沒有 kept running 的證據。`d1f407f6cf` 與 `db414b17db` 之間只差 3 個 `.md`，編出來的程式碼一模一樣，S7 那個 commit 自己也改不動它；那次量測與另一個 session 共用機器和 workspace `target/`。換成獨立 `CARGO_TARGET_DIR` 之後，這個 target 跑 3 次都是 `6 passed`、約 1.1 秒 —— 離 20 秒的 deadline 有 18 倍。這條寫進 `CONTRIBUTING.md` 了，因為它是一個基線讀數會隨機器負載改變的 gate，不記下來下一個人會把它當成回歸。 |
| S8 沒動的 16 個殼 | **記錄** | 剩下的 16 個去重之後是 **10 個相異指令**：`kustomize build …` 3 個、`cd apps/lumen && ../../target/debug/vat run {ec-efficiency-meter, rig-endurance, rig-resilience, rig-load}` 7 個、`apps/lumen/scripts/kind-e2e.sh` 3 個、`cargo run -p lumen --bin lumen -- connect/query --help` 2 個、`target/debug/meter test -- -p lumen --test disk_scale_proof -- --ignored` 1 個。這些都不是 `cargo test -p lumen` 跑得到的東西：要 `kustomize`、要別的專案先建出 `vat` / `meter` binary、要一座 kind cluster。**而且沒有執行者** —— 殼自己的 ignore 理由寫「run via `aw health --verify-ec`」，但 `aw` binary 已經刪掉了（`CLAUDE.md`「`aw` 是 `.claude/aw` 加 `.claude/skills/aw:*`」），所以這 16 個是沒有人跑的 `#[ignore]` 測試，它們的 `@command` 是全樹唯一記錄這些檢查該跑的東西。16 個 `test_path` 指到的檔案全部存在。順帶記一筆：`e2e/feature_gated_targets_are_registered.rs:184` 的 `all_e2e_test_files_are_declared_in_cargo_toml` 是**單向**的 —— 它只斷言每個 `e2e/*.rs` 都有 `[[test]]`，反向（宣告了但檔案不在）由 cargo build 自己擋。所以刪檔案和刪 stanza 必須同時做，少做一邊會從不同的地方紅。 |
