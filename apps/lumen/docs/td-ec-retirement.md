...
| 16 | **`apps/lumen/scripts/dx-contract-gate.sh` 今天就是紅的** | `:89-91` 釘死 7 欄舊值 `want='md=102 lock=1 py=51 hdr=589 files=105 other=19 embed=1'`，現況印 `GATE FAIL`。 | 不要修它去追新數字 —— #3708 的處置是把這個腳本刪掉。 |
...
| S1 知識地板（14 檔） | **完成** | 14 個檔全部補上 `//!`，共 316 行；`src/**/*.rs` 缺 `//!` 的數量 14 → **0**（15 個 in-scope root 全掃）。四個 crate build exit 0，且 `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` 全綠（`service-backup` 另加 `--document-private-items`，因為它的 module 全是私有的，不加就不會檢查那些 `//!` 裡的連結）。無 code fence、`lumen-bench` 塊零方括號。 |
...
| 6 | `apps/lumen/e2e/ec_claim_closure_consistency.rs` 必須維持 0 行 `//!` | 它是 6 條 deferred 之一；補 `//!` 會讓 gate 的 `reasons` 列變紅 |
