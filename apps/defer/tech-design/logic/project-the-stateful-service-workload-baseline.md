...
requirements:
  contract_composes_existing_evidence:
    id: R1
    text: "The capability root resolves the stateful_storage profile by composing current shared mechanisms and existing Defer domain evidence without duplicating runtime policy."
    kind: regression
    risk: low
    verify: aw capability check --project defer --skip-issue-inventory
  stateful_evidence_is_executable:
    id: R2
    text: "The referenced Defer StatefulSet, Raft scheduler, and shared authorization paths continue to pass their targeted tests."
    kind: regression
    risk: medium
...
flowchart TD
    ...
    r2[R2 stateful evidence is executable] --> cargo_test_p_defer_test_direct_k8s_assets_test_raft_scheduler_test_service_auth[cargo test -p defer --test direct_k8s_assets --test raft_scheduler --test service_auth]
```
