//! Build script: stamp `BEAM_GIT_SHA`, `BEAM_BUILT_AT`, and `BEAM_TARGET` into
//! the binary so the standard CLI ops (`upgrade` picks the matching release
//! asset; `issue` reports provenance) work without a server.
//!
...
fn main() {
    ...
    println!("cargo:rustc-env=BEAM_GIT_SHA={git_sha}");
    ...
    println!("cargo:rustc-env=BEAM_BUILT_AT={built_at}");

    // The exact target triple cargo built for, so `beam upgrade` can select the
    // matching `beam-<target>.tar.gz` asset. Cargo always sets `TARGET` here.
