#!/usr/bin/env sh
# SPEC-MANAGED: apps/cap/tech-design/semantic/cap-install-script.md#text-source-unit
# CODEGEN-BEGIN
# cap installer — downloads the right prebuilt binary from GitHub
# Releases and drops it on your PATH.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/chrischeng-c4/axiom/main/apps/cap/install.sh | sh
#
# Env overrides:
...
if [ "${VERSION}" = "latest" ]; then
  ...
  # API filter so we only pick `cap@*` tags (not mamba/jet/etc).
  ...
  if [ -z "${tag}" ]; then
    if [ -z "${TOKEN}" ]; then
      die "could not find a cap@* release in ${REPO} (repo may be private — export GH_TOKEN or \`gh auth login\`)"
    fi
    die "could not find a cap@* release in ${REPO}"
  fi
else
  tag="${VERSION}"
fi
...
tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t cap-install)"
...
# ---- show version ------------------------------------------------------
if "${INSTALL_DIR}/cap" --version >/dev/null 2>&1; then
