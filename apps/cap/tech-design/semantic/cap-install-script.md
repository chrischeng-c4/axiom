---
id: semantic-cap-install-script
summary: Lossless source-unit coverage for the cap project install script.
capability_refs:
  - id: daemon-lifecycle-and-status
    ...
    coverage: partial
    rationale: "The project installer keeps cap's CLI lifecycle and release install flow reproducible from TD."
fill_sections: [text-source-unit, changes]
---

# Semantic TD: cap/install.sh
...
changes:
  - path: "apps/cap/install.sh"
    action: modify
    section: text-source-unit
    description: "Regenerate the cap installer script from a TD-owned text source unit."
    impl_mode: codegen
```
