...
def main():
    gate_dir = os.path.join(
        REPO_ROOT,
        "apps/mamba/tests/governance/gates/t1_explicit_any_acceptance_matrix",
    )
    ...
    lock_path = os.path.join(
        evidence_dir, "mamba-t1-explicit-any-acceptance-lock.json"
    )
    ...
    for family in FAMILIES:
        ...
        fixture_records.append(
            {"family": family, "path": exp_rel, "sha256": exp_sha}
        )

        row = {
            "case_id": f"explicit_any_acceptance__{family}",
            "channel": "behavior",
            "expected_probe_anchor": f"{family}:1:two",
            "expected_terminal_classification": "accepted_and_behavior_matched",
            "fixture_or_probe_path": exp_rel,
            "mamba_binary_sha256": MAMBA_BINARY_SHA256,
            "mamba_git_sha": MAMBA_GIT_SHA,
            "oracle_command": f"{CPYTHON312_EXECUTABLE} {exp_rel}",
            "oracle_executable": CPYTHON312_EXECUTABLE,
            "oracle_kind": "cpython312_reference",
            "oracle_version": CPYTHON312_VERSION,
            "pair_id": pair_id,
            "paired_template_sha256": paired_template_sha,
            "platform": PLATFORM,
            "probe_id": f"explicit_any_acceptance__{family}",
            "sample_role": "positive",
            "source_identity": exp_sha,
            "source_set": "explicit_any_acceptance_matrix",
            "sut_command": f"target/release/mamba run {exp_rel}",
        }
    ...
    manifest_content = (
        f"schema_version = 1\n"
        f'inventory_path = "apps/mamba/tests/governance/gates/t1_explicit_any_acceptance_matrix/cases.jsonl"\n'
        f'inventory_sha256 = "{inventory_sha256}"\n'
        f"row_count = 7\n"
        f'required_channels = ["compile", "behavior", "concurrency", "performance"]\n'
        f'source_set = "explicit_any_acceptance_matrix"\n'
        f"migration_inputs = [\n"
    ...
    lock_data = {
        ...
        "evidence_path": "apps/mamba/external-contracts/evidence/mamba-t1-explicit-any-acceptance-lock.json",
        "fixture_records": fixture_records,
        "generated_at": GENERATED_AT,
        "inventory_path": "apps/mamba/tests/governance/gates/t1_explicit_any_acceptance_matrix/cases.jsonl",
        "inventory_sha256": inventory_sha256,
        "mamba_binary_sha256": MAMBA_BINARY_SHA256,
        "mamba_git_sha": MAMBA_GIT_SHA,
        "manifest_path": "apps/mamba/tests/governance/gates/t1_explicit_any_acceptance_matrix/manifest.toml",
    ...
    print(f"Generated explicit any acceptance matrix successfully.")
