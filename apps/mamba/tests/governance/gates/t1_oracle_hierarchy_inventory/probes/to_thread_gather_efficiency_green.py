...
async def main() -> None:
    ...
    assert speedup >= 1.50, (
        f"Wall speedup {speedup:.3f}x below required threshold 1.50x "
        f"(serial={serial_wall:.4f}s, parallel={parallel_wall:.4f}s)"
    )

    print(
        f"MAMBA-T1-FT-GATHER-EFFICIENCY serial={serial_wall:.4f}s "
        f"parallel={parallel_wall:.4f}s speedup={speedup:.3f}x"
    )
