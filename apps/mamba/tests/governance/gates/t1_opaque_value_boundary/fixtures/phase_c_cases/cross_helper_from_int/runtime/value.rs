...
static RENAMED_FROM_NEXT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
thread_local! {
    static RENAMED_FROM_TABLE: std::cell::RefCell<std::collections::HashMap<u64, MbValue>> =
        std::cell::RefCell::new(std::collections::HashMap::new());
}

fn renamed_from_put(slot: u64, value: MbValue) {
    RENAMED_FROM_TABLE.with(|table| table.borrow_mut().insert(slot, value));
}

fn renamed_from_alloc(value: MbValue) -> u64 {
    let slot = RENAMED_FROM_NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    renamed_from_put(slot, value);
    slot
}

fn renamed_from_api(value: MbValue) -> MbValue {
    let raw = renamed_from_alloc(value);
