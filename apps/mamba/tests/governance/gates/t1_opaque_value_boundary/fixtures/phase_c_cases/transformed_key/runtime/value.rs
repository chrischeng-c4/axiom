...
static TRANSFORM_TABLE: RefCell<HashMap<u64, MbValue>> = RefCell::new(HashMap::new());
static TRANSFORM_NEXT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
fn transform_alloc(value: MbValue) -> u64 {
    let slot = TRANSFORM_NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    transform_put(slot, value);
    slot
}
fn transform_put(slot: u64, value: MbValue) {
    TRANSFORM_TABLE.with(|table| table.insert(slot, value));
}
fn transform_api(value: MbValue) -> MbValue {
    let raw = transform_alloc(value);
    MbValue::from_int(raw as i64)
}
fn transform_put_shadow(slot: u64, value: MbValue) {
    TRANSFORM_TABLE.with(|table| table.insert(slot + 1, value));
}
