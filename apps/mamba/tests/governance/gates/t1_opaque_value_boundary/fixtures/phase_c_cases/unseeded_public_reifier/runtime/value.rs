...
thread_local! {
    static UNSEEDED_PUBLIC: std::cell::RefCell<std::collections::HashMap<u64, MbValue>> =
...
fn public_from_int_reifier(value: MbValue) -> MbValue {
    let id = UNSEEDED_NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    UNSEEDED_PUBLIC.with(|store| store.borrow_mut().insert(id, value));
