...
static AMBIG_NEXT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
thread_local! {
    static AMBIG_TABLE: std::cell::RefCell<std::collections::HashMap<u64, MbValue>> =
        std::cell::RefCell::new(std::collections::HashMap::new());
}
fn ambig_put(slot: u64, value: MbValue) {
    AMBIG_TABLE.with(|table| table.borrow_mut().insert(slot, value));
}
fn ambig_api(value: MbValue) -> MbValue {
    let raw = ambig_alloc(value);
    ambig_put(1, value);
