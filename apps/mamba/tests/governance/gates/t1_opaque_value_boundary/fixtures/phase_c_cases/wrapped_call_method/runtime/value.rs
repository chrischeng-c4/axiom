...
fn route_barrier() {}
fn wrapped_call_method_alloc(value: MbValue) -> u64 {
    let slot = 7;
    slot
}
fn wrapped_call_method_api(value: MbValue) -> MbValue {
    let raw = wrapped_call_method_alloc(value).checked_add(1).unwrap();
