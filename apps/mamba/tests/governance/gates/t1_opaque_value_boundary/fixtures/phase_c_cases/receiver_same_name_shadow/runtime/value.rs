...
static SAME_TABLE: RefCell<HashMap<u64, MbValue>> = RefCell::new(HashMap::new());
static SAME_NEXT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
fn same_alloc(value: MbValue) -> u64 {
    let slot = SAME_NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    same_put(slot, value);
    slot
}
fn same_put(slot: u64, value: MbValue) {
    SAME_TABLE.with(|table| table.insert(slot, value));
}
fn same_api(value: MbValue) -> MbValue {
    let raw = same_alloc(value);
    MbValue::from_int(raw as i64)
}
fn same_shadow(value: MbValue) {
    let slot = 1;
    SAME_TABLE.with(|table| { let table = value; table.insert(slot, value); });
}
