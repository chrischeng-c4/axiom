...
static MULTI_TABLE: RefCell<HashMap<u64, MbValue>> = RefCell::new(HashMap::new());
static MULTI_NEXT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
fn multi_alloc(value: MbValue) -> u64 {
    let slot = MULTI_NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    multi_put(slot, value);
    slot
}
fn multi_put(slot: u64, value: MbValue) {
    MULTI_TABLE.with(|table| table.insert(slot, value));
    MULTI_TABLE.with(|table| table.insert(slot, value));
}
fn multi_api(value: MbValue) -> MbValue {
    let raw = multi_alloc(value);
