...
thread_local! {
    static SEEDED_FROM_TABLE_A: std::cell::RefCell<std::collections::HashMap<u64, MbValue>> =
        std::cell::RefCell::new(std::collections::HashMap::new());
    static SEEDED_FROM_TABLE_B: std::cell::RefCell<std::collections::HashMap<u64, MbValue>> =
        std::cell::RefCell::new(std::collections::HashMap::new());
}

fn seeded_from_alloc(value: MbValue) -> u64 {
    let slot = SEEDED_FROM_NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    SEEDED_FROM_TABLE_B.with(|table| table.borrow_mut().insert(slot, value));
    slot
}
