...
thread_local! {
    static UNSEEDED_TABLE: std::cell::RefCell<std::collections::HashMap<u64, MbValue>> =
...
fn unseeded_side_table(value: MbValue) -> MbValue {
    let id = UNSEEDED_NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    UNSEEDED_TABLE.with(|table| table.borrow_mut().insert(id, value));
