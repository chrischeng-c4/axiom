...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_dataclasses_typing_behavioral")
    );
...
fn dataclass_definition_is_pinned_and_sample_covers_every_aspect() {
    let doc = crate::common::load_toml(&manifest_path());
    let dc = doc
        .get("dataclass_definition")
        .and_then(|v| v.as_table())
        .expect("[dataclass_definition] missing");
    assert_eq!(dc.get("class_name").and_then(|v| v.as_str()), Some("Point"));
    for k in &[
        "field_a_name",
        "field_a_type",
        "field_a_default_python_repr",
        "field_b_name",
        "field_b_type",
        "field_b_default_python_repr",
        "field_c_name",
        "field_c_type",
        "field_c_default_factory_python_repr",
    ] {
        assert!(
            dc.get(*k).and_then(|v| v.as_str()).is_some(),
            "dataclass_definition.{k} missing"
        );
    }
    ...
    let specs: &[(&str, &[&str])] = &[
        (
            "instance_creation_cases",
            &[
                "call",
                "expected_x",
                "expected_y",
                "expected_tags_python_repr",
            ],
        ),
        ("repr_cases", &["call", "expected_repr"]),
        (
            "equality_cases",
            &["left_call", "right_call", "expected_relation"],
        ),
        (
            "get_type_hints_cases",
            &["target", "expected_hints_python_repr", "supported"],
        ),
    ];
...
fn fixture_fails_on_wrong_initialization_or_equality() {
    ...
    assert_eq!(
        sorted.len(),
        exits.len(),
        "failure exit codes must all differ"
    );
}
...
fn unsupported_typing_behavior_marked_with_linked_blocker() {
    let doc = crate::common::load_toml(&manifest_path());
    let b = doc
        .get("unsupported_typing_blocker_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    for k in &[
        ...
        assert_eq!(
            b.get(*k).and_then(|v| v.as_bool()),
            Some(true),
            "{k} must be true"
        );
    }
    assert_eq!(
        b.get("blocker_link_field_name").and_then(|v| v.as_str()),
        Some("blocker_issue")
    );
    assert_eq!(
        b.get("unsupported_typing_outcome_value")
            .and_then(|v| v.as_str()),
        Some("xfail")
    );
    let exit = b
    ...
    assert_eq!(exit, 98);
    assert_eq!(
        b.get("missing_blocker_link_failure_kind")
            .and_then(|v| v.as_str()),
        Some("unsupported_typing_missing_blocker_link")
    );
    let allowed: Vec<&str> = b
        .get("allowed_unsupported_typing_paths")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    assert!(
        !allowed.is_empty(),
        "allowed_unsupported_typing_paths must enumerate at least one path"
    );
}
...
fn fixture_is_part_of_ecosystem_summary() {
    let doc = crate::common::load_toml(&manifest_path());
    let e = doc
        .get("ecosystem_summary_inclusion_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        e.get("must_be_listed_in_ecosystem_summary")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        e.get("ecosystem_summary_fixture_issue")
            .and_then(|v| v.as_integer()),
        Some(2814)
    );
    assert_eq!(
        e.get("must_emit_summary_record").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        e.get("summary_record_format").and_then(|v| v.as_str()),
        Some("json")
    );
    ...
    let covered: Vec<&str> = e
        .get("covered_modules_in_summary")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for m in &["dataclasses", "typing"] {
        assert!(
            covered.contains(m),
            "covered_modules_in_summary must include {m}"
        );
    }
    let exit = e
        .get("missing_summary_record_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 99);
    assert_eq!(
        e.get("missing_summary_record_failure_kind")
            .and_then(|v| v.as_str()),
        Some("ecosystem_summary_missing_dataclasses_typing")
    );
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    let outcomes: Vec<&str> = c
        ...
        .unwrap_or_default();
    assert!(
        outcomes.contains(&"xfail"),
        "runner_contract.outcome_values must include xfail"
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("exhaustive_typing_semantics")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
