...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("fail_new_ignore_without_work_item")
    );
...
fn cross_references_inventory_and_reference_fixtures() {
    ...
    assert_eq!(
        i.get("fixture_issue").and_then(|v| v.as_integer()),
        Some(2602)
    );
    assert_eq!(
        i.get("must_consume_inventory_output")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        i.get("must_be_consistent_with_inventory_marker_kinds")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let shared: Vec<&str> = i
    ...
    let r = doc
        .get("reference_cross_reference")
        .and_then(|v| v.as_table())
        .expect("[reference_cross_reference] missing");
    assert_eq!(
        r.get("fixture_issue").and_then(|v| v.as_integer()),
        Some(2601)
    );
    assert_eq!(
        r.get("must_share_work_item_reference_definition")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn baseline_block_is_committed_versioned_and_checksummed() {
    ...
    let path = b
        .get("baseline_relative_path_within_repo")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(path.starts_with("apps/mamba/tests/"));
    assert!(path.ends_with("baseline.json"));
    assert_eq!(
        b.get("baseline_format").and_then(|v| v.as_str()),
        Some("json")
    );
    assert_eq!(
        b.get("must_be_committed_to_repo").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        b.get("must_be_versioned").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        b.get("schema_version_field_name").and_then(|v| v.as_str()),
        Some("schema_version")
    );
    assert_eq!(
        b.get("must_emit_baseline_checksum")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        b.get("checksum_field_name").and_then(|v| v.as_str()),
        Some("checksum")
    );
}
...
fn new_ignore_without_work_item_fails_guard() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc
        .get("new_ignore_without_work_item_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        n.get("must_compare_current_inventory_to_baseline")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        n.get("must_emit_added_markers_diff")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        n.get("diff_record_format").and_then(|v| v.as_str()),
        Some("json")
    );
    let required: Vec<&str> = n
        .get("diff_required_fields")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for f in &[
        ...
        assert!(
            required.contains(f),
            "diff_required_fields must include {f}"
        );
    }
    assert_eq!(
        n.get("must_fail_when_new_ignore_lacks_work_item")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let exit = n
        .get("new_ignore_without_work_item_exit_code")
...
fn new_xfail_without_work_item_fails_guard() {
    let doc = crate::common::load_toml(&manifest_path());
    let x = doc
        .get("new_xfail_without_work_item_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        x.get("must_fail_when_new_xfail_lacks_work_item")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let exit = x
        .get("new_xfail_without_work_item_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 57);
    assert_eq!(
        x.get("must_distinguish_new_xfail_from_new_ignore")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        x.get("must_apply_same_work_item_rule_to_all_marker_kinds")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let ignore_exit = doc
    ...
    assert_ne!(
        exit, ignore_exit,
        "new-xfail and new-ignore exit codes must differ"
    );
}
...
fn baseline_update_is_explicit_review_step() {
    ...
    let exit = b
        .get("silent_baseline_update_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 58);
    assert_eq!(
        b.get("review_required_message_field_name")
            .and_then(|v| v.as_str()),
        Some("review_required_message")
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("making_all_existing_markers_pass_immediately")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
