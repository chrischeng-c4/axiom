...
fn binding_pins_user_facing_import_shape() {
    ...
    assert!(
        stmt.starts_with("from mambalibs import "),
        "must use the user-facing import path; got {stmt:?}"
    );
...
fn fails_when_import_path_is_unavailable() {
    ...
    let stmt = c
        .get("must_use_user_facing_import_statement")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(stmt.starts_with("from mambalibs import "));
    assert_eq!(
        c.get("expected_outcome_when_import_succeeds")
            .and_then(|v| v.as_str()),
        Some("pass")
    );
    assert_eq!(
        c.get("expected_outcome_when_import_unavailable")
            .and_then(|v| v.as_str()),
        Some("fail")
    );
    assert_eq!(
        c.get("import_unavailable_failure_kind")
            .and_then(|v| v.as_str()),
        Some("import_failure")
    );
    assert_eq!(
        c.get("import_unavailable_exit_code")
            .and_then(|v| v.as_integer()),
        Some(2)
    );
    assert_eq!(
        c.get("import_unavailable_diagnostic_must_name_module")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_not_mask_runtime_call_failures")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn fails_when_exported_function_cannot_be_called() {
    ...
    let must_call = c
        .get("must_call_exported_function")
        .and_then(|v| v.as_str())
        .unwrap();
    let exported = doc
        .get("binding")
    ...
    assert_eq!(
        must_call, exported,
        "must_call_exported_function must match [binding].exported_function"
    );
    assert_eq!(
        c.get("must_assert_exact_returned_value")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let asserted = c
    ...
    assert_eq!(
        asserted, expected,
        "asserted_value must match [expected_return_value].value"
    );
    assert_eq!(
        c.get("expected_outcome_when_call_returns_expected")
            .and_then(|v| v.as_str()),
        Some("pass")
    );
    assert_eq!(
        c.get("expected_outcome_when_call_fails")
            .and_then(|v| v.as_str()),
        Some("fail")
    );
    assert_eq!(
        c.get("call_failure_kind").and_then(|v| v.as_str()),
        Some("runtime_call_failure")
    );
    assert_eq!(
        c.get("call_failure_exit_code").and_then(|v| v.as_integer()),
        Some(3)
    );
    assert_eq!(
        c.get("call_failure_diagnostic_must_name_function")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_not_mask_import_failures")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn output_distinguishes_import_from_runtime_call_failure() {
    ...
    assert_eq!(
        d.get("must_distinguish_import_vs_runtime_call_failure")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let kinds: Vec<&str> = d
        .get("allowed_failure_kinds")
    ...
    assert_eq!(
        d.get("must_emit_failure_kind_in_summary")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("must_emit_distinct_exit_codes")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let imp_code = d
        ...
        .unwrap();
    let rt_code = d
        .get("runtime_call_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let clean = d
        .get("clean_exit_code")
    ...
    assert_ne!(
        imp_code, rt_code,
        "import and runtime call exit codes must differ"
    );
    assert_ne!(imp_code, clean);
    assert_ne!(rt_code, clean);

    // Cross-check with per-case exit codes.
    let imp_case_code = doc
        .get("import_case")
        .and_then(|v| v.get("import_unavailable_exit_code"))
        .and_then(|v| v.as_integer())
        .unwrap();
    let call_case_code = doc
        .get("call_case")
        .and_then(|v| v.get("call_failure_exit_code"))
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(
        imp_case_code, imp_code,
        "[import_case].import_unavailable_exit_code must match the cross-cutting code"
    );
    assert_eq!(
        call_case_code, rt_code,
        "[call_case].call_failure_exit_code must match the cross-cutting code"
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("importing_every_cclab_library")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
