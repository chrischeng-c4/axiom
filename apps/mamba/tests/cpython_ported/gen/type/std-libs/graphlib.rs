use super::super::super::super::harness::*;

/// Ported from `tests/cpython/type/std-libs/graphlib/TopologicalSorter__init__graph_as_SupportsItems_wrong.py`.
#[test]
fn test_gen_type_std_libs_graphlib_TopologicalSorter__init__graph_as_SupportsItems_wrong() {
    let out = run_type_wall_fixture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "graphlib"
# dimension = "type"
# case = "TopologicalSorter__init__graph_as_SupportsItems_wrong"
# subject = "graphlib.TopologicalSorter.__init__(graph: SupportsItems)"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "vendor/typeshed/stdlib/graphlib.pyi"
# status = "filled"
# ///
# mamba-strict-type: TypeError
"""Type wall: graphlib.TopologicalSorter.__init__(graph: SupportsItems); call it with the wrong type.

typeshed contract: graph is SupportsItems. mamba is force-typed, so a wrong-typed
argument MUST raise TypeError (CPython may accept or raise — mamba's to enforce)."""

class _W:
    pass


from graphlib import TopologicalSorter
try:
    TopologicalSorter(_W())  # graph: SupportsItems <- wrong-typed
    print("no_typeerror:")  # CPython accepted the wrong-typed arg; mamba must raise
except TypeError as e:
    print("typeerror:", type(e).__name__)
except Exception as e:
    print("setup_or_other:", type(e).__name__)
"###);
    assert!(out == "STRICT_TYPE_REJECTED" || out.starts_with("RUNTIME_REJECTED"),
        "type wall did not hold: {out}");
}

/// Ported from `tests/cpython/type/std-libs/graphlib/TopologicalSorter__init__graph_as_typed_wrong.py`.
#[test]
fn test_gen_type_std_libs_graphlib_TopologicalSorter__init__graph_as_typed_wrong() {
    let out = run_type_wall_fixture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "graphlib"
# dimension = "type"
# case = "TopologicalSorter__init__graph_as_typed_wrong"
# subject = "graphlib.TopologicalSorter.__init__(graph: typed)"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "vendor/typeshed/stdlib/graphlib.pyi"
# status = "filled"
# ///
# mamba-strict-type: TypeError
"""Type wall: graphlib.TopologicalSorter.__init__(graph: typed); call it with the wrong type.

typeshed contract: graph is typed. mamba is force-typed, so a wrong-typed
argument MUST raise TypeError (CPython may accept or raise — mamba's to enforce)."""

class _W:
    pass


from graphlib import TopologicalSorter
try:
    TopologicalSorter(_W())  # graph: typed <- wrong-typed
    print("no_typeerror:")  # CPython accepted the wrong-typed arg; mamba must raise
except TypeError as e:
    print("typeerror:", type(e).__name__)
except Exception as e:
    print("setup_or_other:", type(e).__name__)
"###);
    assert!(out == "STRICT_TYPE_REJECTED" || out.starts_with("RUNTIME_REJECTED"),
        "type wall did not hold: {out}");
}
