use super::super::super::super::harness::*;

/// Ported from `tests/cpython/behavior/std-libs/ntpath/path_like_tests__test_path_commonpath.py`.
#[test]
fn test_gen_behavior_std_libs_ntpath_path_like_tests__test_path_commonpath() {
    let out = jit_capture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "ntpath"
# dimension = "behavior"
# case = "path_like_tests__test_path_commonpath"
# subject = "cpython.test_ntpath.PathLikeTests.test_path_commonpath"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "Lib/test/test_ntpath.py"
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_ntpath.py::PathLikeTests::test_path_commonpath
"""Auto-ported test: PathLikeTests::test_path_commonpath (CPython 3.12 oracle)."""


import inspect
import ntpath
import os
import string
import subprocess
import sys
import unittest
import warnings
from ntpath import ALLOW_MISSING
from test.support import cpython_only, os_helper
from test.support import TestFailed, is_emscripten
from test.support.os_helper import FakePath
from test import test_genericpath
from tempfile import TemporaryFile


try:
    import nt
except ImportError:
    nt = None

try:
    ntpath._getfinalpathname
except AttributeError:
    HAVE_GETFINALPATHNAME = False
else:
    HAVE_GETFINALPATHNAME = True

try:
    import ctypes
except ImportError:
    HAVE_GETSHORTPATHNAME = False
else:
    HAVE_GETSHORTPATHNAME = True

    def _getshortpathname(path):
        GSPN = ctypes.WinDLL('kernel32', use_last_error=True).GetShortPathNameW
        GSPN.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_uint32]
        GSPN.restype = ctypes.c_uint32
        result_len = GSPN(path, None, 0)
        if not result_len:
            raise OSError('failed to get short path name 0x{:08X}'.format(ctypes.get_last_error()))
        result = ctypes.create_unicode_buffer(result_len)
        result_len = GSPN(path, result, result_len)
        return result[:result_len]

def _norm(path):
    if isinstance(path, (bytes, str, os.PathLike)):
        return ntpath.normcase(os.fsdecode(path))
    elif hasattr(path, '__iter__'):
        return tuple((ntpath.normcase(os.fsdecode(p)) for p in path))
    return path

def tester(fn, wantResult):
    fn = fn.replace('\\', '\\\\')
    gotResult = eval(fn)
    if wantResult != gotResult and _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), str(gotResult)))
    fn = fn.replace("('", "(b'")
    fn = fn.replace('("', '(b"')
    fn = fn.replace("['", "[b'")
    fn = fn.replace('["', '[b"')
    fn = fn.replace(", '", ", b'")
    fn = fn.replace(', "', ', b"')
    fn = os.fsencode(fn).decode('latin1')
    fn = fn.encode('ascii', 'backslashreplace').decode('ascii')
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', DeprecationWarning)
        gotResult = eval(fn)
    if _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), repr(gotResult)))

def _parameterize(*parameters):
    """Simplistic decorator to parametrize a test

    Runs the decorated test multiple times in subTest, with a value from
    'parameters' passed as an extra positional argument.
    Calls doCleanups() after each run.

    Not for general use. Intended to avoid indenting for easier backports.

    See https://discuss.python.org/t/91827 for discussing generalizations.
    """

    def _parametrize_decorator(func):

        def _parameterized(self, *args, **kwargs):
            for parameter in parameters:
                with self.subTest(parameter):
                    func(self, *args, parameter, **kwargs)
                self.doCleanups()
        return _parameterized
    return _parametrize_decorator


# --- test body ---
path = ntpath

def _check_function(func):
    assertPathEqual(func(self_file_path), func(self_file_name))

def assertPathEqual(path1, path2):
    if path1 == path2 or _norm(path1) == _norm(path2):
        return

    assert path1 == path2

def assertPathIn(path, pathset):

    assert _norm(path) in _norm(pathset)
self_file_name = os_helper.TESTFN
self_file_path = FakePath(os_helper.TESTFN)
pass
with open(self_file_name, 'xb', 0) as file:
    file.write(b'test_ntpath.PathLikeTests')
common_path = path.commonpath([self_file_path, self_file_name])
assertPathEqual(common_path, self_file_name)
print("PathLikeTests::test_path_commonpath: ok")
"###);
    assert_output(&out, r###"PathLikeTests::test_path_commonpath: ok
"###);
}

/// Ported from `tests/cpython/behavior/std-libs/ntpath/path_like_tests__test_path_dirname.py`.
#[test]
fn test_gen_behavior_std_libs_ntpath_path_like_tests__test_path_dirname() {
    let out = jit_capture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "ntpath"
# dimension = "behavior"
# case = "path_like_tests__test_path_dirname"
# subject = "cpython.test_ntpath.PathLikeTests.test_path_dirname"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "Lib/test/test_ntpath.py"
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_ntpath.py::PathLikeTests::test_path_dirname
"""Auto-ported test: PathLikeTests::test_path_dirname (CPython 3.12 oracle)."""


import inspect
import ntpath
import os
import string
import subprocess
import sys
import unittest
import warnings
from ntpath import ALLOW_MISSING
from test.support import cpython_only, os_helper
from test.support import TestFailed, is_emscripten
from test.support.os_helper import FakePath
from test import test_genericpath
from tempfile import TemporaryFile


try:
    import nt
except ImportError:
    nt = None

try:
    ntpath._getfinalpathname
except AttributeError:
    HAVE_GETFINALPATHNAME = False
else:
    HAVE_GETFINALPATHNAME = True

try:
    import ctypes
except ImportError:
    HAVE_GETSHORTPATHNAME = False
else:
    HAVE_GETSHORTPATHNAME = True

    def _getshortpathname(path):
        GSPN = ctypes.WinDLL('kernel32', use_last_error=True).GetShortPathNameW
        GSPN.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_uint32]
        GSPN.restype = ctypes.c_uint32
        result_len = GSPN(path, None, 0)
        if not result_len:
            raise OSError('failed to get short path name 0x{:08X}'.format(ctypes.get_last_error()))
        result = ctypes.create_unicode_buffer(result_len)
        result_len = GSPN(path, result, result_len)
        return result[:result_len]

def _norm(path):
    if isinstance(path, (bytes, str, os.PathLike)):
        return ntpath.normcase(os.fsdecode(path))
    elif hasattr(path, '__iter__'):
        return tuple((ntpath.normcase(os.fsdecode(p)) for p in path))
    return path

def tester(fn, wantResult):
    fn = fn.replace('\\', '\\\\')
    gotResult = eval(fn)
    if wantResult != gotResult and _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), str(gotResult)))
    fn = fn.replace("('", "(b'")
    fn = fn.replace('("', '(b"')
    fn = fn.replace("['", "[b'")
    fn = fn.replace('["', '[b"')
    fn = fn.replace(", '", ", b'")
    fn = fn.replace(', "', ', b"')
    fn = os.fsencode(fn).decode('latin1')
    fn = fn.encode('ascii', 'backslashreplace').decode('ascii')
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', DeprecationWarning)
        gotResult = eval(fn)
    if _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), repr(gotResult)))

def _parameterize(*parameters):
    """Simplistic decorator to parametrize a test

    Runs the decorated test multiple times in subTest, with a value from
    'parameters' passed as an extra positional argument.
    Calls doCleanups() after each run.

    Not for general use. Intended to avoid indenting for easier backports.

    See https://discuss.python.org/t/91827 for discussing generalizations.
    """

    def _parametrize_decorator(func):

        def _parameterized(self, *args, **kwargs):
            for parameter in parameters:
                with self.subTest(parameter):
                    func(self, *args, parameter, **kwargs)
                self.doCleanups()
        return _parameterized
    return _parametrize_decorator


# --- test body ---
path = ntpath

def _check_function(func):
    assertPathEqual(func(self_file_path), func(self_file_name))

def assertPathEqual(path1, path2):
    if path1 == path2 or _norm(path1) == _norm(path2):
        return

    assert path1 == path2

def assertPathIn(path, pathset):

    assert _norm(path) in _norm(pathset)
self_file_name = os_helper.TESTFN
self_file_path = FakePath(os_helper.TESTFN)
pass
with open(self_file_name, 'xb', 0) as file:
    file.write(b'test_ntpath.PathLikeTests')
_check_function(path.dirname)
print("PathLikeTests::test_path_dirname: ok")
"###);
    assert_output(&out, r###"PathLikeTests::test_path_dirname: ok
"###);
}

/// Ported from `tests/cpython/behavior/std-libs/ntpath/path_like_tests__test_path_isabs.py`.
#[test]
fn test_gen_behavior_std_libs_ntpath_path_like_tests__test_path_isabs() {
    let out = jit_capture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "ntpath"
# dimension = "behavior"
# case = "path_like_tests__test_path_isabs"
# subject = "cpython.test_ntpath.PathLikeTests.test_path_isabs"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "Lib/test/test_ntpath.py"
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_ntpath.py::PathLikeTests::test_path_isabs
"""Auto-ported test: PathLikeTests::test_path_isabs (CPython 3.12 oracle)."""


import inspect
import ntpath
import os
import string
import subprocess
import sys
import unittest
import warnings
from ntpath import ALLOW_MISSING
from test.support import cpython_only, os_helper
from test.support import TestFailed, is_emscripten
from test.support.os_helper import FakePath
from test import test_genericpath
from tempfile import TemporaryFile


try:
    import nt
except ImportError:
    nt = None

try:
    ntpath._getfinalpathname
except AttributeError:
    HAVE_GETFINALPATHNAME = False
else:
    HAVE_GETFINALPATHNAME = True

try:
    import ctypes
except ImportError:
    HAVE_GETSHORTPATHNAME = False
else:
    HAVE_GETSHORTPATHNAME = True

    def _getshortpathname(path):
        GSPN = ctypes.WinDLL('kernel32', use_last_error=True).GetShortPathNameW
        GSPN.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_uint32]
        GSPN.restype = ctypes.c_uint32
        result_len = GSPN(path, None, 0)
        if not result_len:
            raise OSError('failed to get short path name 0x{:08X}'.format(ctypes.get_last_error()))
        result = ctypes.create_unicode_buffer(result_len)
        result_len = GSPN(path, result, result_len)
        return result[:result_len]

def _norm(path):
    if isinstance(path, (bytes, str, os.PathLike)):
        return ntpath.normcase(os.fsdecode(path))
    elif hasattr(path, '__iter__'):
        return tuple((ntpath.normcase(os.fsdecode(p)) for p in path))
    return path

def tester(fn, wantResult):
    fn = fn.replace('\\', '\\\\')
    gotResult = eval(fn)
    if wantResult != gotResult and _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), str(gotResult)))
    fn = fn.replace("('", "(b'")
    fn = fn.replace('("', '(b"')
    fn = fn.replace("['", "[b'")
    fn = fn.replace('["', '[b"')
    fn = fn.replace(", '", ", b'")
    fn = fn.replace(', "', ', b"')
    fn = os.fsencode(fn).decode('latin1')
    fn = fn.encode('ascii', 'backslashreplace').decode('ascii')
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', DeprecationWarning)
        gotResult = eval(fn)
    if _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), repr(gotResult)))

def _parameterize(*parameters):
    """Simplistic decorator to parametrize a test

    Runs the decorated test multiple times in subTest, with a value from
    'parameters' passed as an extra positional argument.
    Calls doCleanups() after each run.

    Not for general use. Intended to avoid indenting for easier backports.

    See https://discuss.python.org/t/91827 for discussing generalizations.
    """

    def _parametrize_decorator(func):

        def _parameterized(self, *args, **kwargs):
            for parameter in parameters:
                with self.subTest(parameter):
                    func(self, *args, parameter, **kwargs)
                self.doCleanups()
        return _parameterized
    return _parametrize_decorator


# --- test body ---
path = ntpath

def _check_function(func):
    assertPathEqual(func(self_file_path), func(self_file_name))

def assertPathEqual(path1, path2):
    if path1 == path2 or _norm(path1) == _norm(path2):
        return

    assert path1 == path2

def assertPathIn(path, pathset):

    assert _norm(path) in _norm(pathset)
self_file_name = os_helper.TESTFN
self_file_path = FakePath(os_helper.TESTFN)
pass
with open(self_file_name, 'xb', 0) as file:
    file.write(b'test_ntpath.PathLikeTests')
_check_function(path.isabs)
print("PathLikeTests::test_path_isabs: ok")
"###);
    assert_output(&out, r###"PathLikeTests::test_path_isabs: ok
"###);
}

/// Ported from `tests/cpython/behavior/std-libs/ntpath/path_like_tests__test_path_isdir.py`.
#[test]
fn test_gen_behavior_std_libs_ntpath_path_like_tests__test_path_isdir() {
    let out = jit_capture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "ntpath"
# dimension = "behavior"
# case = "path_like_tests__test_path_isdir"
# subject = "cpython.test_ntpath.PathLikeTests.test_path_isdir"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "Lib/test/test_ntpath.py"
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_ntpath.py::PathLikeTests::test_path_isdir
"""Auto-ported test: PathLikeTests::test_path_isdir (CPython 3.12 oracle)."""


import inspect
import ntpath
import os
import string
import subprocess
import sys
import unittest
import warnings
from ntpath import ALLOW_MISSING
from test.support import cpython_only, os_helper
from test.support import TestFailed, is_emscripten
from test.support.os_helper import FakePath
from test import test_genericpath
from tempfile import TemporaryFile


try:
    import nt
except ImportError:
    nt = None

try:
    ntpath._getfinalpathname
except AttributeError:
    HAVE_GETFINALPATHNAME = False
else:
    HAVE_GETFINALPATHNAME = True

try:
    import ctypes
except ImportError:
    HAVE_GETSHORTPATHNAME = False
else:
    HAVE_GETSHORTPATHNAME = True

    def _getshortpathname(path):
        GSPN = ctypes.WinDLL('kernel32', use_last_error=True).GetShortPathNameW
        GSPN.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_uint32]
        GSPN.restype = ctypes.c_uint32
        result_len = GSPN(path, None, 0)
        if not result_len:
            raise OSError('failed to get short path name 0x{:08X}'.format(ctypes.get_last_error()))
        result = ctypes.create_unicode_buffer(result_len)
        result_len = GSPN(path, result, result_len)
        return result[:result_len]

def _norm(path):
    if isinstance(path, (bytes, str, os.PathLike)):
        return ntpath.normcase(os.fsdecode(path))
    elif hasattr(path, '__iter__'):
        return tuple((ntpath.normcase(os.fsdecode(p)) for p in path))
    return path

def tester(fn, wantResult):
    fn = fn.replace('\\', '\\\\')
    gotResult = eval(fn)
    if wantResult != gotResult and _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), str(gotResult)))
    fn = fn.replace("('", "(b'")
    fn = fn.replace('("', '(b"')
    fn = fn.replace("['", "[b'")
    fn = fn.replace('["', '[b"')
    fn = fn.replace(", '", ", b'")
    fn = fn.replace(', "', ', b"')
    fn = os.fsencode(fn).decode('latin1')
    fn = fn.encode('ascii', 'backslashreplace').decode('ascii')
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', DeprecationWarning)
        gotResult = eval(fn)
    if _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), repr(gotResult)))

def _parameterize(*parameters):
    """Simplistic decorator to parametrize a test

    Runs the decorated test multiple times in subTest, with a value from
    'parameters' passed as an extra positional argument.
    Calls doCleanups() after each run.

    Not for general use. Intended to avoid indenting for easier backports.

    See https://discuss.python.org/t/91827 for discussing generalizations.
    """

    def _parametrize_decorator(func):

        def _parameterized(self, *args, **kwargs):
            for parameter in parameters:
                with self.subTest(parameter):
                    func(self, *args, parameter, **kwargs)
                self.doCleanups()
        return _parameterized
    return _parametrize_decorator


# --- test body ---
path = ntpath

def _check_function(func):
    assertPathEqual(func(self_file_path), func(self_file_name))

def assertPathEqual(path1, path2):
    if path1 == path2 or _norm(path1) == _norm(path2):
        return

    assert path1 == path2

def assertPathIn(path, pathset):

    assert _norm(path) in _norm(pathset)
self_file_name = os_helper.TESTFN
self_file_path = FakePath(os_helper.TESTFN)
pass
with open(self_file_name, 'xb', 0) as file:
    file.write(b'test_ntpath.PathLikeTests')
_check_function(path.isdir)
print("PathLikeTests::test_path_isdir: ok")
"###);
    assert_output(&out, r###"PathLikeTests::test_path_isdir: ok
"###);
}

/// Ported from `tests/cpython/behavior/std-libs/ntpath/path_like_tests__test_path_islink.py`.
#[test]
fn test_gen_behavior_std_libs_ntpath_path_like_tests__test_path_islink() {
    let out = jit_capture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "ntpath"
# dimension = "behavior"
# case = "path_like_tests__test_path_islink"
# subject = "cpython.test_ntpath.PathLikeTests.test_path_islink"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "Lib/test/test_ntpath.py"
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_ntpath.py::PathLikeTests::test_path_islink
"""Auto-ported test: PathLikeTests::test_path_islink (CPython 3.12 oracle)."""


import inspect
import ntpath
import os
import string
import subprocess
import sys
import unittest
import warnings
from ntpath import ALLOW_MISSING
from test.support import cpython_only, os_helper
from test.support import TestFailed, is_emscripten
from test.support.os_helper import FakePath
from test import test_genericpath
from tempfile import TemporaryFile


try:
    import nt
except ImportError:
    nt = None

try:
    ntpath._getfinalpathname
except AttributeError:
    HAVE_GETFINALPATHNAME = False
else:
    HAVE_GETFINALPATHNAME = True

try:
    import ctypes
except ImportError:
    HAVE_GETSHORTPATHNAME = False
else:
    HAVE_GETSHORTPATHNAME = True

    def _getshortpathname(path):
        GSPN = ctypes.WinDLL('kernel32', use_last_error=True).GetShortPathNameW
        GSPN.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_uint32]
        GSPN.restype = ctypes.c_uint32
        result_len = GSPN(path, None, 0)
        if not result_len:
            raise OSError('failed to get short path name 0x{:08X}'.format(ctypes.get_last_error()))
        result = ctypes.create_unicode_buffer(result_len)
        result_len = GSPN(path, result, result_len)
        return result[:result_len]

def _norm(path):
    if isinstance(path, (bytes, str, os.PathLike)):
        return ntpath.normcase(os.fsdecode(path))
    elif hasattr(path, '__iter__'):
        return tuple((ntpath.normcase(os.fsdecode(p)) for p in path))
    return path

def tester(fn, wantResult):
    fn = fn.replace('\\', '\\\\')
    gotResult = eval(fn)
    if wantResult != gotResult and _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), str(gotResult)))
    fn = fn.replace("('", "(b'")
    fn = fn.replace('("', '(b"')
    fn = fn.replace("['", "[b'")
    fn = fn.replace('["', '[b"')
    fn = fn.replace(", '", ", b'")
    fn = fn.replace(', "', ', b"')
    fn = os.fsencode(fn).decode('latin1')
    fn = fn.encode('ascii', 'backslashreplace').decode('ascii')
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', DeprecationWarning)
        gotResult = eval(fn)
    if _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), repr(gotResult)))

def _parameterize(*parameters):
    """Simplistic decorator to parametrize a test

    Runs the decorated test multiple times in subTest, with a value from
    'parameters' passed as an extra positional argument.
    Calls doCleanups() after each run.

    Not for general use. Intended to avoid indenting for easier backports.

    See https://discuss.python.org/t/91827 for discussing generalizations.
    """

    def _parametrize_decorator(func):

        def _parameterized(self, *args, **kwargs):
            for parameter in parameters:
                with self.subTest(parameter):
                    func(self, *args, parameter, **kwargs)
                self.doCleanups()
        return _parameterized
    return _parametrize_decorator


# --- test body ---
path = ntpath

def _check_function(func):
    assertPathEqual(func(self_file_path), func(self_file_name))

def assertPathEqual(path1, path2):
    if path1 == path2 or _norm(path1) == _norm(path2):
        return

    assert path1 == path2

def assertPathIn(path, pathset):

    assert _norm(path) in _norm(pathset)
self_file_name = os_helper.TESTFN
self_file_path = FakePath(os_helper.TESTFN)
pass
with open(self_file_name, 'xb', 0) as file:
    file.write(b'test_ntpath.PathLikeTests')
_check_function(path.islink)
print("PathLikeTests::test_path_islink: ok")
"###);
    assert_output(&out, r###"PathLikeTests::test_path_islink: ok
"###);
}

/// Ported from `tests/cpython/behavior/std-libs/ntpath/path_like_tests__test_path_ismount.py`.
#[test]
fn test_gen_behavior_std_libs_ntpath_path_like_tests__test_path_ismount() {
    let out = jit_capture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "ntpath"
# dimension = "behavior"
# case = "path_like_tests__test_path_ismount"
# subject = "cpython.test_ntpath.PathLikeTests.test_path_ismount"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "Lib/test/test_ntpath.py"
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_ntpath.py::PathLikeTests::test_path_ismount
"""Auto-ported test: PathLikeTests::test_path_ismount (CPython 3.12 oracle)."""


import inspect
import ntpath
import os
import string
import subprocess
import sys
import unittest
import warnings
from ntpath import ALLOW_MISSING
from test.support import cpython_only, os_helper
from test.support import TestFailed, is_emscripten
from test.support.os_helper import FakePath
from test import test_genericpath
from tempfile import TemporaryFile


try:
    import nt
except ImportError:
    nt = None

try:
    ntpath._getfinalpathname
except AttributeError:
    HAVE_GETFINALPATHNAME = False
else:
    HAVE_GETFINALPATHNAME = True

try:
    import ctypes
except ImportError:
    HAVE_GETSHORTPATHNAME = False
else:
    HAVE_GETSHORTPATHNAME = True

    def _getshortpathname(path):
        GSPN = ctypes.WinDLL('kernel32', use_last_error=True).GetShortPathNameW
        GSPN.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_uint32]
        GSPN.restype = ctypes.c_uint32
        result_len = GSPN(path, None, 0)
        if not result_len:
            raise OSError('failed to get short path name 0x{:08X}'.format(ctypes.get_last_error()))
        result = ctypes.create_unicode_buffer(result_len)
        result_len = GSPN(path, result, result_len)
        return result[:result_len]

def _norm(path):
    if isinstance(path, (bytes, str, os.PathLike)):
        return ntpath.normcase(os.fsdecode(path))
    elif hasattr(path, '__iter__'):
        return tuple((ntpath.normcase(os.fsdecode(p)) for p in path))
    return path

def tester(fn, wantResult):
    fn = fn.replace('\\', '\\\\')
    gotResult = eval(fn)
    if wantResult != gotResult and _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), str(gotResult)))
    fn = fn.replace("('", "(b'")
    fn = fn.replace('("', '(b"')
    fn = fn.replace("['", "[b'")
    fn = fn.replace('["', '[b"')
    fn = fn.replace(", '", ", b'")
    fn = fn.replace(', "', ', b"')
    fn = os.fsencode(fn).decode('latin1')
    fn = fn.encode('ascii', 'backslashreplace').decode('ascii')
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', DeprecationWarning)
        gotResult = eval(fn)
    if _norm(wantResult) != _norm(gotResult):
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn), str(wantResult), repr(gotResult)))

def _parameterize(*parameters):
    """Simplistic decorator to parametrize a test

    Runs the decorated test multiple times in subTest, with a value from
    'parameters' passed as an extra positional argument.
    Calls doCleanups() after each run.

    Not for general use. Intended to avoid indenting for easier backports.

    See https://discuss.python.org/t/91827 for discussing generalizations.
    """

    def _parametrize_decorator(func):

        def _parameterized(self, *args, **kwargs):
            for parameter in parameters:
                with self.subTest(parameter):
                    func(self, *args, parameter, **kwargs)
                self.doCleanups()
        return _parameterized
    return _parametrize_decorator


# --- test body ---
path = ntpath

def _check_function(func):
    assertPathEqual(func(self_file_path), func(self_file_name))

def assertPathEqual(path1, path2):
    if path1 == path2 or _norm(path1) == _norm(path2):
        return

    assert path1 == path2

def assertPathIn(path, pathset):

    assert _norm(path) in _norm(pathset)
self_file_name = os_helper.TESTFN
self_file_path = FakePath(os_helper.TESTFN)
pass
with open(self_file_name, 'xb', 0) as file:
    file.write(b'test_ntpath.PathLikeTests')
_check_function(path.ismount)
print("PathLikeTests::test_path_ismount: ok")
"###);
    assert_output(&out, r###"PathLikeTests::test_path_ismount: ok
"###);
}
