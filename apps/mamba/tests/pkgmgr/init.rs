...
//!      stderr) and preserves pyproject.toml / README.md / src/__init__.py
//!      byte-for-byte.
...
fn init_creates_expected_files_on_fresh_directory() {
    ...
    assert!(
        manifest.contains("[dependency-groups]\ndev = []"),
        "dev deps default"
    );
}
...
fn init_uses_directory_name_as_project_name() {
    ...
    let manifest = std::fs::read_to_string(project_dir.join("pyproject.toml")).unwrap();
    assert!(
