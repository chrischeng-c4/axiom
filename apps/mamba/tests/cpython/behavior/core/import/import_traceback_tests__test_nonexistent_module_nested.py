...
# lib = "import"
# dimension = "behavior"
# case = "import_traceback_tests__test_nonexistent_module_nested"
# subject = "cpython.__init__.ImportTracebackTests.test_nonexistent_module_nested"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 __init__.py::ImportTracebackTests::test_nonexistent_module_nested
"""Auto-ported test: ImportTracebackTests::test_nonexistent_module_nested (CPython 3.12 oracle)."""
...
# --- test body ---
def _setup_broken_package(parent, child):
...
def assert_traceback(tb, files):
    ...
    assert len(deduped_files) == len(files)
    for fn, pat in zip(deduped_files, files):

        assert pat in fn

def create_module(mod, contents, ext='.py'):
...
create_module('foo', 'import nonexistent_xyzzy')
try:
    import foo
...
assert_traceback(tb, [__file__, 'foo.py'])
print("ImportTracebackTests::test_nonexistent_module_nested: ok")
