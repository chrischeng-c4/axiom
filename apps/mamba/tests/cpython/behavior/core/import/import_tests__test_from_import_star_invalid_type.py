...
# lib = "import"
# dimension = "behavior"
# case = "import_tests__test_from_import_star_invalid_type"
# subject = "cpython.__init__.ImportTests.test_from_import_star_invalid_type"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 __init__.py::ImportTests::test_from_import_star_invalid_type
"""Auto-ported test: ImportTests::test_from_import_star_invalid_type (CPython 3.12 oracle)."""
...
with ready_to_import() as (name, path):
    ...
    try:
        ...
        import re as _re_aR
        assert _re_aR.search(f'{re.escape(name)}\\.__all__ must be str', str(_aR_e))

    assert b'invalid_type' not in globals
with ready_to_import() as (name, path):
    with open(path, 'w', encoding='utf-8') as f:
    ...
    try:
        ...
        import re as _re_aR
        assert _re_aR.search(f'{re.escape(name)}\\.__dict__ must be str', str(_aR_e))

    assert b'invalid_type' not in globals
print("ImportTests::test_from_import_star_invalid_type: ok")
