...
# lib = "exception_group"
# dimension = "behavior"
# case = "exception_group_split_tests__test_basics_split_by_type_match"
# subject = "cpython.test_exception_group.ExceptionGroupSplitTests.test_basics_split_by_type__match"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_exception_group.py::ExceptionGroupSplitTests::test_basics_split_by_type__match
"""Auto-ported test: ExceptionGroupSplitTests::test_basics_split_by_type__match (CPython 3.12 oracle)."""
...
VE = ValueError
TE = TypeError
testcases = [(VE, [VE(1), VE(2)], [TE(int)]), (TE, [TE(int)], [VE(1), VE(2)]), ((VE, TE), self_eg_template, None), ((OSError, VE), [VE(1), VE(2)], [TE(int)])]
for match_type, match_template, rest_template in testcases:
    match, rest = eg.split(match_type)

    assert match.message == eg.message
    assertMatchesTemplate(match, ExceptionGroup, match_template)
    if rest_template is not None:

        assert rest.message == eg.message
        assertMatchesTemplate(rest, ExceptionGroup, rest_template)
    else:

        assert rest is None
print("ExceptionGroupSplitTests::test_basics_split_by_type__match: ok")
