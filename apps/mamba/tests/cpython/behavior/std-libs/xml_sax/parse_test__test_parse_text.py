...
# lib = "xml_sax"
# dimension = "behavior"
# case = "parse_test__test_parse_text"
# subject = "cpython.test_sax.ParseTest.test_parse_text"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_sax.py::ParseTest::test_parse_text
"""Auto-ported test: ParseTest::test_parse_text (CPython 3.12 oracle)."""
...
for encoding in encodings:
    check_parse(StringIO(xml_str(data, encoding)))
    make_xml_file(data, encoding)
    with open(TESTFN, 'r', encoding=encoding) as f:
        check_parse(f)
    check_parse(StringIO(data))
    make_xml_file(data, encoding, None)
    with open(TESTFN, 'r', encoding=encoding) as f:
        check_parse(f)
print("ParseTest::test_parse_text: ok")
