...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_chain_reducible"
# subject = "cpython.test_itertools.TestBasicOps.test_chain_reducible"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_chain_reducible
"""Auto-ported test: TestBasicOps::test_chain_reducible (CPython 3.12 oracle)."""
...
for oper in [copy.deepcopy] + picklecopiers:
    ...
    assert list(oper(it)) == list('abcdef')

    assert next(it) == 'a'

    assert list(oper(it)) == list('bcdef')

    assert list(oper(chain(''))) == []

    assert take(4, oper(chain('abc', 'def'))) == list('abcd')
...
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    pickletest(proto, chain('abc', 'def'), compare=list('abcdef'))
print("TestBasicOps::test_chain_reducible: ok")
