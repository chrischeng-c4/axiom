...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_permutations"
# subject = "cpython.test_itertools.TestBasicOps.test_permutations"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_permutations
"""Auto-ported test: TestBasicOps::test_permutations (CPython 3.12 oracle)."""
...
for n in range(7):
    ...
    for r in range(n + 2):
        ...
        assert len(result) == (0 if r > n else fact(n) / fact(n - r))
        ...
        for p in result:
            ...
            assert all((e in values for e in p))

        assert result == list(permutations1(values, r))

        assert result == list(permutations2(values, r))
        if r == n:

            assert result == list(permutations(values, None))

            assert result == list(permutations(values))
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            pickletest(proto, permutations(values, r))
print("TestBasicOps::test_permutations: ok")
