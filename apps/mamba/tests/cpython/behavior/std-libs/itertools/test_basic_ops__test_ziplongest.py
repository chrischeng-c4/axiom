...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_ziplongest"
# subject = "cpython.test_itertools.TestBasicOps.test_ziplongest"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_ziplongest
"""Auto-ported test: TestBasicOps::test_ziplongest (CPython 3.12 oracle)."""
...
# --- test body ---
for args in [['abc', range(6)], [range(6), 'abc'], [range(1000), range(2000, 2100), range(3000, 3050)], [range(1000), range(0), range(3000, 3050), range(1200), range(1500)], [range(1000), range(0), range(3000, 3050), range(1200), range(1500), range(0)]]:
...
assert take(3, zip_longest('abcdef', count())) == list(zip('abcdef', range(3)))
...
assert list(zip_longest('abc', 'defg', **{})) == list(zip(list('abc') + [None], 'defg'))
...
try:
    zip_longest(range(3), 3)
    raise AssertionError('expected TypeError')
except TypeError:
    pass
for stmt in ["zip_longest('abc', fv=1)", "zip_longest('abc', fillvalue=1, bogus_keyword=None)"]:
    try:
        ...
        raise AssertionError('Did not raise Type in:  ' + stmt)

assert [tuple(list(pair)) for pair in zip_longest('abc', 'def')] == list(zip('abc', 'def'))

assert [pair for pair in zip_longest('abc', 'def')] == list(zip('abc', 'def'))
print("TestBasicOps::test_ziplongest: ok")
