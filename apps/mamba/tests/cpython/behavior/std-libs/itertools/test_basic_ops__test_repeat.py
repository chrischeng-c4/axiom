...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_repeat"
# subject = "cpython.test_itertools.TestBasicOps.test_repeat"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_repeat
"""Auto-ported test: TestBasicOps::test_repeat (CPython 3.12 oracle)."""
...
assert repr(r) == 'repeat((1+0j))'
r = repeat(1 + 0j, 5)

assert repr(r) == 'repeat((1+0j), 5)'
list(r)

assert repr(r) == 'repeat((1+0j), 0)'
c = repeat(object='a', times=10)

assert next(c) == 'a'

assert take(2, copy.copy(c)) == list('a' * 2)

assert take(2, copy.deepcopy(c)) == list('a' * 2)
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    pickletest(proto, repeat(object='a', times=10))
print("TestBasicOps::test_repeat: ok")
