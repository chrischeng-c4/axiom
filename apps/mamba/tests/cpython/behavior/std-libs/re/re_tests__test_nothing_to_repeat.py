...
# lib = "re"
# dimension = "behavior"
# case = "re_tests__test_nothing_to_repeat"
# subject = "cpython.test_re.ReTests.test_nothing_to_repeat"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
...
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_re.py::ReTests::test_nothing_to_repeat
"""Auto-ported test: ReTests::test_nothing_to_repeat (CPython 3.12 oracle)."""
...
for reps in ('*', '+', '?', '{1,2}'):
    for mod in ('', '?'):
        checkPatternError('%s%s' % (reps, mod), 'nothing to repeat', 0)
        checkPatternError('(?:%s%s)' % (reps, mod), 'nothing to repeat', 3)
print("ReTests::test_nothing_to_repeat: ok")
