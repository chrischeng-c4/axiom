...
# lib = "optparse"
# dimension = "behavior"
# case = "test_conflict_override__test_conflict_override_opts"
# subject = "cpython.test_optparse.TestConflictOverride.test_conflict_override_opts"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_optparse.py::TestConflictOverride::test_conflict_override_opts
"""Auto-ported test: TestConflictOverride::test_conflict_override_opts (CPython 3.12 oracle)."""
...
# --- test body ---
self_parser = InterceptingOptionParser(usage=SUPPRESS_USAGE)
self_parser.set_conflict_handler('resolve')
self_parser.add_option('-n', '--dry-run', action='store_true', dest='dry_run', help="don't do anything")
self_parser.add_option('--dry-run', '-n', action='store_const', const=42, dest='dry_run', help='dry run mode')
opt = self_parser.get_option('--dry-run')

assert opt._short_opts == ['-n']

assert opt._long_opts == ['--dry-run']
print("TestConflictOverride::test_conflict_override_opts: ok")
