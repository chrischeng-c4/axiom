...
# lib = "curses"
# dimension = "behavior"
# case = "test_curses__test_overlay"
# subject = "cpython.test_curses.TestCurses.test_overlay"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_curses.py::TestCurses::test_overlay
"""Auto-ported test: TestCurses::test_overlay (CPython 3.12 oracle)."""
...
assert dstwin.instr(0, 0) == b'sectetur_________'

assert dstwin.instr(1, 0) == b'piscing_elit,____'

assert dstwin.instr(2, 0) == b'_do_eiusmod______'

assert dstwin.instr(3, 0) == b'_________________'
srcwin.overwrite(dstwin)

assert dstwin.instr(0, 0) == b'sectetur       __'

assert dstwin.instr(1, 0) == b'piscing elit,  __'

assert dstwin.instr(2, 0) == b' do eiusmod    __'

assert dstwin.instr(3, 0) == b'_________________'
srcwin.overlay(dstwin, 1, 4, 3, 2, 4, 11)

assert dstwin.instr(3, 0) == b'__r_sit_amet_____'

assert dstwin.instr(4, 0) == b'__ectetur________'

assert dstwin.instr(5, 0) == b'_________________'
srcwin.overwrite(dstwin, 1, 4, 3, 2, 4, 11)

assert dstwin.instr(3, 0) == b'__r sit amet_____'

assert dstwin.instr(4, 0) == b'__ectetur   _____'

assert dstwin.instr(5, 0) == b'_________________'
print("TestCurses::test_overlay: ok")
