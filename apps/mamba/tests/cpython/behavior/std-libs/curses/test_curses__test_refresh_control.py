...
# lib = "curses"
# dimension = "behavior"
# case = "test_curses__test_refresh_control"
# subject = "cpython.test_curses.TestCurses.test_refresh_control"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_curses.py::TestCurses::test_refresh_control
"""Auto-ported test: TestCurses::test_refresh_control (CPython 3.12 oracle)."""
...
assert stdscr.is_wintouched() is False
stdscr.touchwin()

assert stdscr.is_wintouched() is True
stdscr.refresh()
...
assert stdscr.is_linetouched(5) is True

assert stdscr.is_linetouched(6) is True

assert stdscr.is_wintouched() is True
stdscr.touchline(5, 1, False)

assert stdscr.is_linetouched(5) is False
...
assert win.is_wintouched() is True
...
assert win2.is_wintouched() is True
if hasattr(stdscr, 'syncok') and (not sys.platform.startswith('sunos')):
    ...
    for syncok in [False, True]:
        ...
        assert win.is_wintouched() is syncok

        assert stdscr.is_wintouched() is syncok
print("TestCurses::test_refresh_control: ok")
