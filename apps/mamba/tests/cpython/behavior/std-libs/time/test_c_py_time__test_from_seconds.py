...
# lib = "time"
# dimension = "behavior"
# case = "test_c_py_time__test_from_seconds"
# subject = "cpython.test_time.TestCPyTime.test_FromSeconds"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_time.py::TestCPyTime::test_FromSeconds
"""Auto-ported test: TestCPyTime::test_FromSeconds (CPython 3.12 oracle)."""
...
def _check_rounding(pytime_converter, expected_func, use_float, unit_to_sec, value_filter=None):
    ...
    def convert_values(ns_timestamps):
        ...
        if value_filter:
            values = filter(value_filter, values)
        return sorted(set(values))
    ns_timestamps = _rounding_values(use_float)
    valid_values = convert_values(ns_timestamps)
    for time_rnd, decimal_rnd in ROUNDING_MODES:
        with decimal.localcontext() as context:
            context.rounding = decimal_rnd
            for value in valid_values:
                ...
                try:
                    ...
                    raise AssertionError('Error on timestamp conversion: %s' % debug_info)

                assert result == expected
    ns = OVERFLOW_SECONDS * SEC_TO_NS
    ns_timestamps = (-ns, ns)
    overflow_values = convert_values(ns_timestamps)
    for time_rnd, _ in ROUNDING_MODES:
        for value in overflow_values:
...
def _rounding_values(use_float):
    """Build timestamps used to test rounding."""
    units = [1, US_TO_NS, MS_TO_NS, SEC_TO_NS]
    if use_float:
        units.append(0.001)
...
def check_float_rounding(pytime_converter, expected_func, unit_to_sec=1, value_filter=None):
    _check_rounding(pytime_converter, expected_func, True, unit_to_sec, value_filter)

def check_int_rounding(pytime_converter, expected_func, unit_to_sec=1, value_filter=None):
    _check_rounding(pytime_converter, expected_func, False, unit_to_sec, value_filter)

def create_decimal_converter(denominator):
...
def time_t_filter(seconds):
    return self_time_t_min <= seconds <= self_time_t_max
from _testcapi import SIZEOF_TIME_T
bits = SIZEOF_TIME_T * 8 - 1
self_time_t_min = -2 ** bits
self_time_t_max = 2 ** bits - 1
from _testcapi import PyTime_FromSeconds

def c_int_filter(secs):
    return _testcapi.INT_MIN <= secs <= _testcapi.INT_MAX
check_int_rounding(lambda secs, rnd: PyTime_FromSeconds(secs), lambda secs: secs * SEC_TO_NS, value_filter=c_int_filter)
for time_rnd, _ in ROUNDING_MODES:
    try:
        ...
        pass
print("TestCPyTime::test_FromSeconds: ok")
