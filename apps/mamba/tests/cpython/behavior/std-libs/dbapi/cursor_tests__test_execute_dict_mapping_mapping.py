...
# lib = "dbapi"
# dimension = "behavior"
# case = "cursor_tests__test_execute_dict_mapping_mapping"
# subject = "cpython.test_dbapi.CursorTests.test_execute_dict_mapping_mapping"
# kind = "semantic"
...
class D(dict):
    ...
    def __missing__(self, key):
        return 'foo'
self_cu.execute("insert into test(name) values ('foo')")
self_cu.execute('select name from test where name=:name', D())
row = self_cu.fetchone()
assert row[0] == 'foo'

print("CursorTests::test_execute_dict_mapping_mapping: ok")
