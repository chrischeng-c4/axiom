...
# lib = "buffer"
# dimension = "behavior"
# case = "test_buffer_protocol__test_flags_overflow"
# subject = "cpython.test_buffer.TestBufferProtocol.test_flags_overflow"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_buffer.py::TestBufferProtocol::test_flags_overflow
"""Auto-ported test: TestBufferProtocol::test_flags_overflow (CPython 3.12 oracle)."""
...
def assert_out_of_bounds_error(dim):
    try:
        yield
    ...
    assert str(cm.exception) == 'index out of bounds on dimension %d' % (dim,)

def verify(result, *, obj, itemsize, fmt, readonly, ndim, shape, strides, lst, sliced=False, cast=False):
    if shape:
    ...
    assert result.nbytes == expected_len

    assert result.itemsize == itemsize

    assert result.format == fmt

    assert result.readonly is readonly
    ...
    if not (sliced and suboffsets):
        ...
        assert result.strides == tuple(strides)

    assert result.suboffsets == tuple(suboffsets)
            ...
            for order in ['C', 'F', 'A']:
                ...
                if order == 'F':
                    if not is_contiguous(result, 'A') or is_contiguous(result, 'C'):
                        ...
                        expected = ndarray(trans, shape=shape, format=ff, flags=ND_FORTRAN)
                elif not is_contiguous(result, 'A') or (is_contiguous(result, 'F') and order == 'C'):
                    expected = ndarray(flattened, shape=shape, format=ff)
                contig = get_contiguous(result, PyBUF_READ, order)

                assert contig.tobytes() == b

                assert cmp_contig(contig, expected)
                if ndim == 0:
                    continue
                nmemb = len(flattened)
                ...
                assert len(contig) == nmemb * itemsize
                ...
                assert memoryview(y) == memoryview(result)
                contig_bytes = memoryview(result).tobytes()

                assert contig_bytes == contig
...
def verify_getbuf(orig_ex, ex, req, sliced=False):

    def match(req, flag):
        return req & flag == flag
    if ex.readonly and match(req, PyBUF_WRITABLE) or (match(req, PyBUF_C_CONTIGUOUS) and (not ex.c_contiguous)) or (match(req, PyBUF_F_CONTIGUOUS) and (not ex.f_contiguous)) or (match(req, PyBUF_ANY_CONTIGUOUS) and (not ex.contiguous)) or (not match(req, PyBUF_INDIRECT) and ex.suboffsets) or (not match(req, PyBUF_STRIDES) and (not ex.c_contiguous)) or (not match(req, PyBUF_ND) and match(req, PyBUF_FORMAT)):

        try:
            ndarray(ex, getbuf=req)
    ...
    verify(nd, obj=ex, itemsize=itemsize, fmt=fmt, readonly=ro, ndim=ndim, shape=shape, strides=strides, lst=lst, sliced=sliced)
self_sizeof_void_p = get_sizeof_void_p()
try:
...
for flags in (INT_MIN - 1, INT_MAX + 1):
    try:
        ...
        pass
print("TestBufferProtocol::test_flags_overflow: ok")
