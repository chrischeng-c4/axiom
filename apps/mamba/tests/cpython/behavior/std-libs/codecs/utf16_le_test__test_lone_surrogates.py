...
# lib = "codecs"
# dimension = "behavior"
# case = "utf16_le_test__test_lone_surrogates"
# subject = "cpython.test_codecs.UTF16LETest.test_lone_surrogates"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_codecs.py::UTF16LETest::test_lone_surrogates
"""Auto-ported test: UTF16LETest::test_lone_surrogates (CPython 3.12 oracle)."""
...
assert '[\udc80]'.encode(encoding, 'backslashreplace') == '[\\udc80]'.encode(encoding)

assert '[\udc80]'.encode(encoding, 'namereplace') == '[\\udc80]'.encode(encoding)

assert '[\udc80]'.encode(encoding, 'xmlcharrefreplace') == '[&#56448;]'.encode(encoding)

assert '[\udc80]'.encode(encoding, 'ignore') == '[]'.encode(encoding)

assert '[\udc80]'.encode(encoding, 'replace') == '[?]'.encode(encoding)

assert '[\ud800\udc80]'.encode(encoding, 'ignore') == '[]'.encode(encoding)

assert '[\ud800\udc80]'.encode(encoding, 'replace') == '[??]'.encode(encoding)
bom = ''.encode(encoding)
for before, after in [('\U00010fff', 'A'), ('[', ']'), ('A', '\U00010fff')]:
    ...
    test_sequence = bom + before_sequence + ill_formed_sequence + after_sequence

    try:
        test_sequence.decode(encoding)
    ...
    assert test_sequence.decode(encoding, 'replace') == before + ill_formed_sequence_replace + after
    backslashreplace = ''.join(('\\x%02x' % b for b in ill_formed_sequence))

    assert test_sequence.decode(encoding, 'backslashreplace') == before + backslashreplace + after
print("UTF16LETest::test_lone_surrogates: ok")
