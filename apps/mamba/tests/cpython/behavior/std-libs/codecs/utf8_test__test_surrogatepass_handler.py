...
# lib = "codecs"
# dimension = "behavior"
# case = "utf8_test__test_surrogatepass_handler"
# subject = "cpython.test_codecs.UTF8Test.test_surrogatepass_handler"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_codecs.py::UTF8Test::test_surrogatepass_handler
"""Auto-ported test: UTF8Test::test_surrogatepass_handler (CPython 3.12 oracle)."""
...
assert 'abc\ud800def'.encode(encoding, 'surrogatepass') == BOM + b'abc\xed\xa0\x80def'

assert '\U00010fff\ud800'.encode(encoding, 'surrogatepass') == BOM + b'\xf0\x90\xbf\xbf\xed\xa0\x80'

assert '[\ud800\udc80]'.encode(encoding, 'surrogatepass') == BOM + b'[\xed\xa0\x80\xed\xb2\x80]'

assert b'abc\xed\xa0\x80def'.decode(encoding, 'surrogatepass') == 'abc\ud800def'

assert b'\xf0\x90\xbf\xbf\xed\xa0\x80'.decode(encoding, 'surrogatepass') == '\U00010fff\ud800'

assert codecs.lookup_error('surrogatepass')
try:
    b'abc\xed\xa0'.decode(encoding, 'surrogatepass')
    raise AssertionError('expected UnicodeDecodeError')
except UnicodeDecodeError:
    pass
try:
    b'abc\xed\xa0z'.decode(encoding, 'surrogatepass')
    raise AssertionError('expected UnicodeDecodeError')
except UnicodeDecodeError:
    pass
print("UTF8Test::test_surrogatepass_handler: ok")
