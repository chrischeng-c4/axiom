...
# lib = "codecs"
# dimension = "behavior"
# case = "exception_notes_test__test_unflagged_non_text_codec_handling"
# subject = "cpython.test_codecs.ExceptionNotesTest.test_unflagged_non_text_codec_handling"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_codecs.py::ExceptionNotesTest::test_unflagged_non_text_codec_handling
"""Auto-ported test: ExceptionNotesTest::test_unflagged_non_text_codec_handling (CPython 3.12 oracle)."""
...
# --- test body ---
def assertNoted(operation, exc_type, msg):
    full_msg = '{} with {!r} codec failed'.format(operation, self_codec_name)
    try:
        yield caught
        raise AssertionError('expected exc_type')
    except exc_type as _aR_e:
        import types as _types_aR
        caught = _types_aR.SimpleNamespace(exception=_aR_e)

    assert full_msg in caught.exception.__notes__[0]
    caught.exception.__notes__.clear()

def check_note(obj_to_raise, msg, exc_type=RuntimeError):
    self_obj_to_raise = obj_to_raise
    set_codec(raise_obj, raise_obj)
    with assertNoted('encoding', exc_type, msg):
        'str_input'.encode(self_codec_name)
    with assertNoted('encoding', exc_type, msg):
        codecs.encode('str_input', self_codec_name)
    with assertNoted('decoding', exc_type, msg):
        b'bytes input'.decode(self_codec_name)
    with assertNoted('decoding', exc_type, msg):
        codecs.decode(b'bytes input', self_codec_name)

def raise_obj(*args, **kwds):
    raise self_obj_to_raise

def set_codec(encode, decode):
    codec_info = codecs.CodecInfo(encode, decode, name=self_codec_name)
    _TEST_CODECS[self_codec_name] = codec_info
self_codec_name = 'exception_notes_test'
codecs.register(_get_test_codec)
pass
self_obj_to_raise = RuntimeError
...
def decode_to_bytes(*args, **kwds):
    return (b'not str!', 0)
set_codec(encode_to_str, decode_to_bytes)
encoded = codecs.encode(None, self_codec_name)

assert encoded == 'not bytes!'
decoded = codecs.decode(None, self_codec_name)

assert decoded == b'not str!'
fmt = "^{!r} encoder returned 'str' instead of 'bytes'; use codecs.encode\\(\\) to encode to arbitrary types$"
msg = fmt.format(self_codec_name)
try:
    'str_input'.encode(self_codec_name)
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search(msg, str(_aR_e))
print("ExceptionNotesTest::test_unflagged_non_text_codec_handling: ok")
