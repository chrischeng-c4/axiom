...
# lib = "codecs"
# dimension = "behavior"
# case = "idna_codec_test__test_incremental_decode"
# subject = "cpython.test_codecs.IDNACodecTest.test_incremental_decode"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_codecs.py::IDNACodecTest::test_incremental_decode
"""Auto-ported test: IDNACodecTest::test_incremental_decode (CPython 3.12 oracle)."""
...
assert ''.join(codecs.iterdecode((bytes([c]) for c in b'python.org'), 'idna')) == 'python.org'

assert ''.join(codecs.iterdecode((bytes([c]) for c in b'python.org.'), 'idna')) == 'python.org.'

assert ''.join(codecs.iterdecode((bytes([c]) for c in b'xn--pythn-mua.org.'), 'idna')) == 'pythön.org.'

assert ''.join(codecs.iterdecode((bytes([c]) for c in b'xn--pythn-mua.org.'), 'idna')) == 'pythön.org.'
decoder = codecs.getincrementaldecoder('idna')()

assert decoder.decode(b'xn--xam') == ''

assert decoder.decode(b'ple-9ta.o') == 'äxample.'

assert decoder.decode(b'rg') == ''

assert decoder.decode(b'', True) == 'org'
decoder.reset()
...
assert decoder.decode(b'rg.') == 'org.'

assert decoder.decode(b'', True) == ''
print("IDNACodecTest::test_incremental_decode: ok")
