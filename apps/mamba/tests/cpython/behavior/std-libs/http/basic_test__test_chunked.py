...
# lib = "http"
# dimension = "behavior"
# case = "basic_test__test_chunked"
# subject = "cpython.test_httplib.BasicTest.test_chunked"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_httplib.py::BasicTest::test_chunked
"""Auto-ported test: BasicTest::test_chunked (CPython 3.12 oracle)."""
...
assert resp.read1(-1) == b'hello worl'
resp.close()
for n in range(1, 12):
    sock = FakeSocket(chunked_start + last_chunk + chunked_end)
    resp = client.HTTPResponse(sock, method='GET')
    ...
    assert resp.read(n) + resp.read(n) + resp.read() == expected
    resp.close()
for x in ('', 'foo\r\n'):
    ...
    try:
        ...
        assert i.partial == expected
        expected_message = 'IncompleteRead(%d bytes read)' % len(expected)

        assert repr(i) == expected_message

        assert str(i) == expected_message
    else:

        raise AssertionError('IncompleteRead expected')
    finally:
        resp.close()
print("BasicTest::test_chunked: ok")
