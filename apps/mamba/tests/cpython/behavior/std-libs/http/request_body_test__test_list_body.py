...
# lib = "http"
# dimension = "behavior"
# case = "request_body_test__test_list_body"
# subject = "cpython.test_httplib.RequestBodyTest.test_list_body"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_httplib.py::RequestBodyTest::test_list_body
"""Auto-ported test: RequestBodyTest::test_list_body (CPython 3.12 oracle)."""
...
for body, expected in cases:
    self_conn = client.HTTPConnection('example.com')
    self_conn.sock = self_sock = FakeSocket('')
    self_conn.request('PUT', '/url', body)
    msg, f = get_headers_and_fp()

    assert 'Content-Type' not in msg

    assert 'Content-Length' not in msg

    assert msg.get('Transfer-Encoding') == 'chunked'

    assert expected == f.read()
print("RequestBodyTest::test_list_body: ok")
