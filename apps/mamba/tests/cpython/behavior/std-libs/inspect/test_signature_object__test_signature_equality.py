...
# lib = "inspect"
# dimension = "behavior"
# case = "test_signature_object__test_signature_equality"
# subject = "cpython.test_inspect.TestSignatureObject.test_signature_equality"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_inspect.py::TestSignatureObject::test_signature_equality
"""Auto-ported test: TestSignatureObject::test_signature_equality (CPython 3.12 oracle)."""
...
assert not inspect.signature(foo) == 42

assert inspect.signature(foo) != 42

assert inspect.signature(foo) == ALWAYS_EQ

assert not inspect.signature(foo) != ALWAYS_EQ
...
assert not inspect.signature(foo) != inspect.signature(bar)

assert hash(inspect.signature(foo)) == hash(inspect.signature(bar))
...
assert not inspect.signature(spam) == inspect.signature(bar)

assert inspect.signature(spam) != inspect.signature(bar)

assert hash(inspect.signature(spam)) != hash(inspect.signature(bar))
...
assert hash(inspect.signature(foo)) == hash(inspect.signature(bar))
print("TestSignatureObject::test_signature_equality: ok")
