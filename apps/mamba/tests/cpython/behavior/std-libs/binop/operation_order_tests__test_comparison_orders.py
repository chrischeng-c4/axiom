...
# lib = "binop"
# dimension = "behavior"
# case = "operation_order_tests__test_comparison_orders"
# subject = "cpython.test_binop.OperationOrderTests.test_comparison_orders"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_binop.py::OperationOrderTests::test_comparison_orders
"""Auto-ported test: OperationOrderTests::test_comparison_orders (CPython 3.12 oracle)."""
...
assert op_sequence(eq, A, A) == ['A.__eq__', 'A.__eq__']

assert op_sequence(eq, A, B) == ['A.__eq__', 'B.__eq__']

assert op_sequence(eq, B, A) == ['B.__eq__', 'A.__eq__']

assert op_sequence(eq, B, C) == ['C.__eq__', 'B.__eq__']

assert op_sequence(eq, C, B) == ['C.__eq__', 'B.__eq__']

assert op_sequence(le, A, A) == ['A.__le__', 'A.__ge__']

assert op_sequence(le, A, B) == ['A.__le__', 'B.__ge__']

assert op_sequence(le, B, A) == ['B.__le__', 'A.__ge__']

assert op_sequence(le, B, C) == ['C.__ge__', 'B.__le__']

assert op_sequence(le, C, B) == ['C.__le__', 'B.__ge__']

assert issubclass(V, B)

assert op_sequence(eq, B, V) == ['B.__eq__', 'V.__eq__']

assert op_sequence(le, B, V) == ['B.__le__', 'V.__ge__']
print("OperationOrderTests::test_comparison_orders: ok")
