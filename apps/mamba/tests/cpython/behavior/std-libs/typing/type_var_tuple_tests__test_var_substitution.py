...
# lib = "typing"
# dimension = "behavior"
# case = "type_var_tuple_tests__test_var_substitution"
# subject = "cpython.test_typing.TypeVarTupleTests.test_var_substitution"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::TypeVarTupleTests::test_var_substitution
"""Auto-ported test: TypeVarTupleTests::test_var_substitution (CPython 3.12 oracle)."""
...
for A in (G1, G2, Tuple, tuple):
    ...
    assert B[float] == A[float]

    assert B[float, str] == A[float, str]
    C = A[Unpack[Ts]]

    assert C[()] == A[()]

    assert C[float] == A[float]

    assert C[float, str] == A[float, str]
    D = list[A[*Ts,]]

    assert D[()] == list[A[()]]

    assert D[float] == list[A[float]]

    assert D[float, str] == list[A[float, str]]
    E = List[A[Unpack[Ts]]]

    assert E[()] == List[A[()]]

    assert E[float] == List[A[float]]

    assert E[float, str] == List[A[float, str]]
    F = A[T, *Ts, T2]
    try:
        F[()]
    ...
    assert F[float, str] == A[float, str]

    assert F[float, str, int] == A[float, str, int]

    assert F[float, str, int, bytes] == A[float, str, int, bytes]
    G = A[T, Unpack[Ts], T2]
    try:
        G[()]
        raise AssertionError('expected TypeError')
    ...
    assert G[float, str] == A[float, str]

    assert G[float, str, int] == A[float, str, int]

    assert G[float, str, int, bytes] == A[float, str, int, bytes]
    ...
    if A != Tuple:
        ...
        assert H[float, str] == tuple[list[float], A[()], list[str]]

    assert H[float, str, int] == tuple[list[float], A[str], list[int]]

    assert H[float, str, int, bytes] == tuple[list[float], A[str, int], list[bytes]]
    ...
    if A != Tuple:
        ...
        assert I[float, str] == Tuple[List[float], A[()], List[str]]

    assert I[float, str, int] == Tuple[List[float], A[str], List[int]]

    assert I[float, str, int, bytes] == Tuple[List[float], A[str, int], List[bytes]]
print("TypeVarTupleTests::test_var_substitution: ok")
