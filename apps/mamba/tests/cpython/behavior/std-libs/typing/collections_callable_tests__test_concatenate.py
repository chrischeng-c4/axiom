...
# lib = "typing"
# dimension = "behavior"
# case = "collections_callable_tests__test_concatenate"
# subject = "cpython.test_typing.CollectionsCallableTests.test_concatenate"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::CollectionsCallableTests::test_concatenate
"""Auto-ported test: CollectionsCallableTests::test_concatenate (CPython 3.12 oracle)."""
...
assert repr(C) == f'{fullname}[typing.Concatenate[int, ~P], ~T]'

assert C[P2, int] == Callable[Concatenate[int, P2], int]

assert C[[str, float], int] == Callable[[int, str, float], int]

assert C[[], int] == Callable[[int], int]

assert C[Concatenate[str, P2], int] == Callable[Concatenate[int, str, P2], int]

assert C[..., int] == Callable[Concatenate[int, ...], int]
C = Callable[Concatenate[int, P], int]

assert repr(C) == f'{fullname}[typing.Concatenate[int, ~P], int]'

assert C[P2] == Callable[Concatenate[int, P2], int]

assert C[[str, float]] == Callable[[int, str, float], int]

assert C[str, float] == Callable[[int, str, float], int]

assert C[[]] == Callable[[int], int]

assert C[Concatenate[str, P2]] == Callable[Concatenate[int, str, P2], int]

assert C[...] == Callable[Concatenate[int, ...], int]
print("CollectionsCallableTests::test_concatenate: ok")
