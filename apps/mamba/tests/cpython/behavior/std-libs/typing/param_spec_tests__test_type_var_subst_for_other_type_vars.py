...
# lib = "typing"
# dimension = "behavior"
# case = "param_spec_tests__test_type_var_subst_for_other_type_vars"
# subject = "cpython.test_typing.ParamSpecTests.test_type_var_subst_for_other_type_vars"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::ParamSpecTests::test_type_var_subst_for_other_type_vars
"""Auto-ported test: ParamSpecTests::test_type_var_subst_for_other_type_vars (CPython 3.12 oracle)."""
...
assert A1.__parameters__ == (T,)

assert A1.__args__ == ((T,),)

assert A1[int] == Base[int]
A2 = Base[[T]]

assert A2.__parameters__ == (T,)

assert A2.__args__ == ((T,),)

assert A2[int] == Base[int]
A3 = Base[[int, T]]

assert A3.__parameters__ == (T,)

assert A3.__args__ == ((int, T),)

assert A3[str] == Base[[int, str]]
A4 = Base[[T, int, T2]]

assert A4.__parameters__ == (T, T2)

assert A4.__args__ == ((T, int, T2),)

assert A4[str, bool] == Base[[str, int, bool]]
A5 = Base[[*Ts, int]]

assert A5.__parameters__ == (Ts,)

assert A5.__args__ == ((*Ts, int),)

assert A5[str, bool] == Base[[str, bool, int]]
A5_2 = Base[[int, *Ts]]

assert A5_2.__parameters__ == (Ts,)

assert A5_2.__args__ == ((int, *Ts),)

assert A5_2[str, bool] == Base[[int, str, bool]]
A6 = Base[[T, *Ts]]

assert A6.__parameters__ == (T, Ts)

assert A6.__args__ == ((T, *Ts),)

assert A6[int, str, bool] == Base[[int, str, bool]]
A7 = Base[[T, T]]

assert A7.__parameters__ == (T,)

assert A7.__args__ == ((T, T),)

assert A7[int] == Base[[int, int]]
A8 = Base[[T, list[T]]]

assert A8.__parameters__ == (T,)

assert A8.__args__ == ((T, list[T]),)

assert A8[int] == Base[[int, list[int]]]
A9 = Base[[Tuple[*Ts,], *Ts]]

assert A9.__parameters__ == (Ts,)

assert A9.__args__ == ((Tuple[*Ts,], *Ts),)

assert A9[int, str] == Base[Tuple[int, str], int, str]
A10 = Base[P2]

assert A10.__parameters__ == (P2,)

assert A10.__args__ == (P2,)

assert A10[[int, str]] == Base[[int, str]]

class DoubleP(Generic[P, P2]):
    pass
B1 = DoubleP[P, P2]

assert B1.__parameters__ == (P, P2)

assert B1.__args__ == (P, P2)

assert B1[[int, str], [bool]] == DoubleP[[int, str], [bool]]

assert B1[[], []] == DoubleP[[], []]
B2 = DoubleP[[int, str], P2]

assert B2.__parameters__ == (P2,)

assert B2.__args__ == ((int, str), P2)

assert B2[[bool, bool]] == DoubleP[[int, str], [bool, bool]]

assert B2[[]] == DoubleP[[int, str], []]
B3 = DoubleP[P, [bool, bool]]

assert B3.__parameters__ == (P,)

assert B3.__args__ == (P, (bool, bool))

assert B3[[int, str]] == DoubleP[[int, str], [bool, bool]]

assert B3[[]] == DoubleP[[], [bool, bool]]
B4 = DoubleP[[T, int], [bool, T2]]

assert B4.__parameters__ == (T, T2)

assert B4.__args__ == ((T, int), (bool, T2))

assert B4[str, float] == DoubleP[[str, int], [bool, float]]
B5 = DoubleP[[*Ts, int], [bool, T2]]

assert B5.__parameters__ == (Ts, T2)

assert B5.__args__ == ((*Ts, int), (bool, T2))

assert B5[str, bytes, float] == DoubleP[[str, bytes, int], [bool, float]]
B6 = DoubleP[[T, int], [bool, *Ts]]

assert B6.__parameters__ == (T, Ts)

assert B6.__args__ == ((T, int), (bool, *Ts))

assert B6[str, bytes, float] == DoubleP[[str, int], [bool, bytes, float]]

class PandT(Generic[P, T]):
    pass
C1 = PandT[P, T]

assert C1.__parameters__ == (P, T)

assert C1.__args__ == (P, T)

assert C1[[int, str], bool] == PandT[[int, str], bool]
C2 = PandT[[int, T], T]

assert C2.__parameters__ == (T,)

assert C2.__args__ == ((int, T), T)

assert C2[str] == PandT[[int, str], str]
C3 = PandT[[int, *Ts], T]

assert C3.__parameters__ == (Ts, T)

assert C3.__args__ == ((int, *Ts), T)

assert C3[str, bool, bytes] == PandT[[int, str, bool], bytes]
print("ParamSpecTests::test_type_var_subst_for_other_type_vars: ok")
