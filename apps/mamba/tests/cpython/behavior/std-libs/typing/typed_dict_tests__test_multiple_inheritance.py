...
# lib = "typing"
# dimension = "behavior"
# case = "typed_dict_tests__test_multiple_inheritance"
# subject = "cpython.test_typing.TypedDictTests.test_multiple_inheritance"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::TypedDictTests::test_multiple_inheritance
"""Auto-ported test: TypedDictTests::test_multiple_inheritance (CPython 3.12 oracle)."""
...
assert Child.__required_keys__ == frozenset(['one', 'two', 'child'])

assert Child.__optional_keys__ == frozenset([])

assert Child.__annotations__ == {'one': int, 'two': str, 'child': bool}

class ChildWithOptional(One, Untotal):
    child: bool

assert ChildWithOptional.__required_keys__ == frozenset(['one', 'child'])

assert ChildWithOptional.__optional_keys__ == frozenset(['untotal'])

assert ChildWithOptional.__annotations__ == {'one': int, 'untotal': str, 'child': bool}

class ChildWithTotalFalse(One, Untotal, total=False):
    child: bool

assert ChildWithTotalFalse.__required_keys__ == frozenset(['one'])

assert ChildWithTotalFalse.__optional_keys__ == frozenset(['untotal', 'child'])

assert ChildWithTotalFalse.__annotations__ == {'one': int, 'untotal': str, 'child': bool}

class ChildWithInlineAndOptional(Untotal, Inline):
    child: bool

assert ChildWithInlineAndOptional.__required_keys__ == frozenset(['inline', 'child'])

assert ChildWithInlineAndOptional.__optional_keys__ == frozenset(['untotal'])

assert ChildWithInlineAndOptional.__annotations__ == {'inline': bool, 'untotal': str, 'child': bool}
...
for bases in wrong_bases:
    try:
        ...
        import re as _re_aR
        assert _re_aR.search('cannot inherit from both a TypedDict type and a non-TypedDict', str(_aR_e))
print("TypedDictTests::test_multiple_inheritance: ok")
