...
# lib = "typing"
# dimension = "behavior"
# case = "generic_tests__test_non_generic_subscript"
# subject = "cpython.test_typing.GenericTests.test_non_generic_subscript"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::GenericTests::test_non_generic_subscript
"""Auto-ported test: GenericTests::test_non_generic_subscript (CPython 3.12 oracle)."""
...
class A:
    __parameters__ = (T,)
for s in (int, G, A, List, list, TypeVar, TypeVarTuple, ParamSpec, types.GenericAlias, types.UnionType):
    for t in (Tuple, tuple):

        assert t[s, T][int] == t[s, int]

        assert t[T, s][int] == t[int, s]
        a = t[s]
        try:
            a[int]
    ...
    for c in (Callable, collections.abc.Callable):
        ...
        assert c[[s], T][int] == c[[s], int]

        assert c[[T], s][int] == c[[int], s]
        a = c[[s], s]
        try:
            a[int]
            raise AssertionError('expected TypeError')
            ...
            pass
print("GenericTests::test_non_generic_subscript: ok")
