...
# lib = "typing"
# dimension = "behavior"
# case = "get_utilities_test_case__test_get_origin"
# subject = "cpython.test_typing.GetUtilitiesTestCase.test_get_origin"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::GetUtilitiesTestCase::test_get_origin
"""Auto-ported test: GetUtilitiesTestCase::test_get_origin (CPython 3.12 oracle)."""
...
assert get_origin(C[int]) is C

assert get_origin(C[T]) is C
...
assert get_origin(Final[List[int]]) is Final
...
assert get_origin(Annotated[T, 'thing']) is Annotated

assert get_origin(List) is list

assert get_origin(Tuple) is tuple

assert get_origin(Callable) is collections.abc.Callable

assert get_origin(list[int]) is list

assert get_origin(list) is None

assert get_origin(list | str) is types.UnionType
...
assert get_origin((*Ts,)[0]) is Unpack

assert get_origin(Unpack[Ts]) is Unpack

assert get_origin((*tuple[*Ts,],)[0]) is tuple

assert get_origin(Unpack[Tuple[Unpack[Ts]]]) is Unpack
print("GetUtilitiesTestCase::test_get_origin: ok")
