...
# lib = "decimal"
# dimension = "behavior"
# case = "c_usability_test__test_subclassing"
# subject = "cpython.test_decimal.CUsabilityTest.test_subclassing"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::CUsabilityTest::test_subclassing
"""Auto-ported test: CUsabilityTest::test_subclassing (CPython 3.12 oracle)."""
...
assert type(d) is Decimal
d = d1.max(d2)

assert type(d) is Decimal
d = copy.copy(d1)

assert type(d) is MyDecimal
...
assert type(x) is Decimal

assert x == d
m = MyDecimal(d)

assert type(m) is MyDecimal
...
assert type(x) is MyDecimal

assert x == d

assert x.y is None
print("CUsabilityTest::test_subclassing: ok")
