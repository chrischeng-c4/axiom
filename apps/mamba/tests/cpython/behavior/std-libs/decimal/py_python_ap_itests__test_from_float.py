...
# lib = "decimal"
# dimension = "behavior"
# case = "py_python_ap_itests__test_from_float"
# subject = "cpython.test_decimal.PyPythonAPItests.test_from_float"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::PyPythonAPItests::test_from_float
"""Auto-ported test: PyPythonAPItests::test_from_float (CPython 3.12 oracle)."""
...
assert issubclass(MyDecimal, Decimal)
r = MyDecimal.from_float(0.1)

assert type(r) == MyDecimal

assert str(r) == '0.1000000000000000055511151231257827021181583404541015625'

assert r.x == 'y'
bigint = 12345678901234567890123456789

assert MyDecimal.from_float(bigint) == MyDecimal(bigint)

assert MyDecimal.from_float(float('nan')).is_qnan()

assert MyDecimal.from_float(float('inf')).is_infinite()

assert MyDecimal.from_float(float('-inf')).is_infinite()

assert str(MyDecimal.from_float(float('nan'))) == str(Decimal('NaN'))

assert str(MyDecimal.from_float(float('inf'))) == str(Decimal('Infinity'))

assert str(MyDecimal.from_float(float('-inf'))) == str(Decimal('-Infinity'))

try:
    MyDecimal.from_float('abc')
...
for i in range(200):
    ...
    assert x == float(MyDecimal.from_float(x))
print("PyPythonAPItests::test_from_float: ok")
