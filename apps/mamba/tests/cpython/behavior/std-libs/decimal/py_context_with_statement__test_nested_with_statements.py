...
# lib = "decimal"
# dimension = "behavior"
# case = "py_context_with_statement__test_nested_with_statements"
# subject = "cpython.test_decimal.PyContextWithStatement.test_nested_with_statements"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::PyContextWithStatement::test_nested_with_statements
"""Auto-ported test: PyContextWithStatement::test_nested_with_statements (CPython 3.12 oracle)."""
...
with localcontext() as c1:
    ...
    assert c1.flags == orig_ctx.flags

    assert c1.traps == orig_ctx.traps
    c1.traps[Clamped] = True
    c1.Emin = -383

    assert orig_ctx.Emin != -383

    try:
        c1.create_decimal('0e-999')
        raise AssertionError('expected Clamped')
    except Clamped:
        pass

    assert c1.flags[Clamped]
    with localcontext(new_ctx) as c2:

        assert c2.flags == new_ctx.flags

        assert c2.traps == new_ctx.traps

        try:
            c2.power(Decimal('3.4e200'), 2)
            raise AssertionError('expected Overflow')
        except Overflow:
            pass

        assert not c2.flags[Clamped]

        assert c2.flags[Overflow]
        del c2

    assert not c1.flags[Overflow]
    del c1

assert orig_ctx.Emin != -383

assert not orig_ctx.flags[Clamped]

assert not orig_ctx.flags[Overflow]

assert not new_ctx.flags[Clamped]

assert not new_ctx.flags[Overflow]
print("PyContextWithStatement::test_nested_with_statements: ok")
