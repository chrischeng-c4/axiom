...
# lib = "fileinput"
# dimension = "behavior"
# case = "file_input_tests__test_inplace_binary_write_mode"
# subject = "cpython.test_fileinput.FileInputTests.test_inplace_binary_write_mode"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_fileinput.py::FileInputTests::test_inplace_binary_write_mode
"""Auto-ported test: FileInputTests::test_inplace_binary_write_mode (CPython 3.12 oracle)."""
...
def writeTmp(content, *, mode='w'):
    ...
    with open(fd, mode, encoding=encoding) as f:
        f.write(content)
    return name
temp_file = writeTmp(b'Initial text.', mode='wb')
with FileInput(temp_file, mode='rb', inplace=True) as fobj:
    line = fobj.readline()

    assert line == b'Initial text.'
    sys.stdout.write(b'New line.')
with open(temp_file, 'rb') as f:

    assert f.read() == b'New line.'
print("FileInputTests::test_inplace_binary_write_mode: ok")
