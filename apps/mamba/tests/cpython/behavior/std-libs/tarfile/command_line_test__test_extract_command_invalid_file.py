...
# lib = "tarfile"
# dimension = "behavior"
# case = "command_line_test__test_extract_command_invalid_file"
# subject = "cpython.test_tarfile.CommandLineTest.test_extract_command_invalid_file"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_tarfile.py::CommandLineTest::test_extract_command_invalid_file
"""Auto-ported test: CommandLineTest::test_extract_command_invalid_file (CPython 3.12 oracle)."""
...
# --- test body ---
def make_evil_tarfile(tar_name):
    pass
...
def make_simple_tarfile(tar_name):
    files = [support.findfile('tokenize_tests.txt', subdir='tokenizedata'), support.findfile('tokenize_tests-no-coding-cookie-and-utf8-bom-sig-only.txt', subdir='tokenizedata')]
    pass
    with tarfile.open(tar_name, 'w') as tf:
        for tardata in files:
            tf.add(tardata, arcname=os.path.basename(tardata))

def tarfilecmd(*args, **kwargs):
    rc, out, err = script_helper.assert_python_ok('-m', 'tarfile', *args, **kwargs)
    return out.replace(os.linesep.encode(), b'\n')

def tarfilecmd_failure(*args):
...
with os_helper.temp_cwd(tarextdir):
    rc, out, err = tarfilecmd_failure('-e', zipname)

assert b' is not a tar archive.' in err

assert out == b''

assert rc == 1
print("CommandLineTest::test_extract_command_invalid_file: ok")
