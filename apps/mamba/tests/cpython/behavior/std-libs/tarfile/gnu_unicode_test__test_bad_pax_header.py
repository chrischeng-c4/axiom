...
# lib = "tarfile"
# dimension = "behavior"
# case = "gnu_unicode_test__test_bad_pax_header"
# subject = "cpython.test_tarfile.GNUUnicodeTest.test_bad_pax_header"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_tarfile.py::GNUUnicodeTest::test_bad_pax_header
"""Auto-ported test: GNUUnicodeTest::test_bad_pax_header (CPython 3.12 oracle)."""
...
def _test_unicode_filename(encoding):
    tar = tarfile.open(tmpname, 'w', format=format, encoding=encoding, errors='strict')
    try:
        name = 'äöü'
...
for encoding, name in (('utf-8', 'pax/bad-pax-\udce4\udcf6\udcfc'), ('iso8859-1', 'pax/bad-pax-äöü')):
    with tarfile.open(tarname, encoding=encoding, errors='surrogateescape') as tar:
        try:
            ...
            raise AssertionError('unable to read bad GNU tar pax header')
print("GNUUnicodeTest::test_bad_pax_header: ok")
