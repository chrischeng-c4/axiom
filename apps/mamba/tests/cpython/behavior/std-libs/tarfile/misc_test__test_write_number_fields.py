...
# lib = "tarfile"
# dimension = "behavior"
# case = "misc_test__test_write_number_fields"
# subject = "cpython.test_tarfile.MiscTest.test_write_number_fields"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_tarfile.py::MiscTest::test_write_number_fields
"""Auto-ported test: MiscTest::test_write_number_fields (CPython 3.12 oracle)."""
...
assert tarfile.itn(1) == b'0000001\x00'

assert tarfile.itn(2097151) == b'7777777\x00'

assert tarfile.itn(2097152, format=tarfile.GNU_FORMAT) == b'\x80\x00\x00\x00\x00 \x00\x00'

assert tarfile.itn(4294967295, format=tarfile.GNU_FORMAT) == b'\x80\x00\x00\x00\xff\xff\xff\xff'

assert tarfile.itn(-1, format=tarfile.GNU_FORMAT) == b'\xff\xff\xff\xff\xff\xff\xff\xff'

assert tarfile.itn(-100, format=tarfile.GNU_FORMAT) == b'\xff\xff\xff\xff\xff\xff\xff\x9c'

assert tarfile.itn(-72057594037927936, format=tarfile.GNU_FORMAT) == b'\xff\x00\x00\x00\x00\x00\x00\x00'

assert tarfile.itn(-100.0, format=tarfile.GNU_FORMAT) == b'\xff\xff\xff\xff\xff\xff\xff\x9c'

assert tarfile.itn(8 ** 12 + 0.0, format=tarfile.GNU_FORMAT) == b'\x80\x00\x00\x10\x00\x00\x00\x00'

assert tarfile.nti(tarfile.itn(-0.1, format=tarfile.GNU_FORMAT)) == 0
print("MiscTest::test_write_number_fields: ok")
