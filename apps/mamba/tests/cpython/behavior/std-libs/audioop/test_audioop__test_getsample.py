...
# lib = "audioop"
# dimension = "behavior"
# case = "test_audioop__test_getsample"
# subject = "cpython.test_audioop.TestAudioop.test_getsample"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_audioop.py::TestAudioop::test_getsample
"""Auto-ported test: TestAudioop::test_getsample (CPython 3.12 oracle)."""
...
for w in (1, 2, 3, 4):
    ...
    assert audioop.getsample(data, w, 0) == 0

    assert audioop.getsample(bytearray(data), w, 0) == 0

    assert audioop.getsample(memoryview(data), w, 0) == 0

    assert audioop.getsample(data, w, 1) == 1

    assert audioop.getsample(data, w, 2) == -1

    assert audioop.getsample(data, w, 3) == maxvalues[w]

    assert audioop.getsample(data, w, 4) == minvalues[w]
print("TestAudioop::test_getsample: ok")
