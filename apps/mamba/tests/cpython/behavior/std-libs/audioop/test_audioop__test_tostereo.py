...
# lib = "audioop"
# dimension = "behavior"
# case = "test_audioop__test_tostereo"
# subject = "cpython.test_audioop.TestAudioop.test_tostereo"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_audioop.py::TestAudioop::test_tostereo
"""Auto-ported test: TestAudioop::test_tostereo (CPython 3.12 oracle)."""
...
for w in (1, 2, 3, 4):
    ...
    assert audioop.tostereo(data1, w, 1, 0) == data2

    assert audioop.tostereo(data1, w, 0, 0) == b'\x00' * len(data2)
    for k in range(w):
        data2[k + w::2 * w] = data1[k::w]

    assert audioop.tostereo(data1, w, 1, 1) == data2

    assert audioop.tostereo(bytearray(data1), w, 1, 1) == data2

    assert audioop.tostereo(memoryview(data1), w, 1, 1) == data2
print("TestAudioop::test_tostereo: ok")
