...
# lib = "audioop"
# dimension = "behavior"
# case = "test_audioop__test_cross"
# subject = "cpython.test_audioop.TestAudioop.test_cross"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_audioop.py::TestAudioop::test_cross
"""Auto-ported test: TestAudioop::test_cross (CPython 3.12 oracle)."""
...
for w in (1, 2, 3, 4):
    ...
    assert audioop.cross(b'', w) == -1

    assert audioop.cross(bytearray(), w) == -1

    assert audioop.cross(memoryview(b''), w) == -1
    p = packs[w]

    assert audioop.cross(p(0, 1, 2), w) == 0

    assert audioop.cross(p(1, 2, -3, -4), w) == 1

    assert audioop.cross(p(-1, -2, 3, 4), w) == 1

    assert audioop.cross(p(0, minvalues[w]), w) == 1

    assert audioop.cross(p(minvalues[w], maxvalues[w]), w) == 1
print("TestAudioop::test_cross: ok")
