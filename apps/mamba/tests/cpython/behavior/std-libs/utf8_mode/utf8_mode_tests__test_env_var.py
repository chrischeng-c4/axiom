...
# lib = "utf8_mode"
# dimension = "behavior"
# case = "utf8_mode_tests__test_env_var"
# subject = "cpython.test_utf8_mode.UTF8ModeTests.test_env_var"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_utf8_mode.py::UTF8ModeTests::test_env_var
"""Auto-ported test: UTF8ModeTests::test_env_var (CPython 3.12 oracle)."""
...
out = get_output('-c', code, PYTHONUTF8='1')

assert out == '1'
out = get_output('-c', code, PYTHONUTF8='0')

assert out == '0'
out = get_output('-X', 'utf8=0', '-c', code, PYTHONUTF8='1')

assert out == '0'
if MS_WINDOWS:
    out = get_output('-X', 'utf8', '-c', code, PYTHONUTF8='1', PYTHONLEGACYWINDOWSFSENCODING='1')

    assert out == '0'
if not posix_locale():
    out = get_output('-E', '-c', code, PYTHONUTF8='1')

    assert out == '0'
out = get_output('-c', code, PYTHONUTF8='xxx', failure=True)

assert 'invalid PYTHONUTF8 environment variable value' in out.rstrip()
print("UTF8ModeTests::test_env_var: ok")
