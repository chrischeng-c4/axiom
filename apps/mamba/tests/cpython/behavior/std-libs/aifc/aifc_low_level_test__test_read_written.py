...
# lib = "aifc"
# dimension = "behavior"
# case = "aifc_low_level_test__test_read_written"
# subject = "cpython.test_aifc.AIFCLowLevelTest.test_read_written"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_aifc.py::AIFCLowLevelTest::test_read_written
"""Auto-ported test: AIFCLowLevelTest::test_read_written (CPython 3.12 oracle)."""
...
for x in (-1, 0, 0.1, 1):
    ...
    assert read_written(x, 'float') == x
for x in (float('NaN'), float('Inf')):

    assert read_written(x, 'float') == aifc._HUGE_VAL
for x in (b'', b'foo', b'a' * 255):

    assert read_written(x, 'string') == x
for x in (-2147483647, -1, 0, 1, 2147483647):

    assert read_written(x, 'long') == x
for x in (0, 1, 4294967295):

    assert read_written(x, 'ulong') == x
for x in (-32767, -1, 0, 1, 32767):

    assert read_written(x, 'short') == x
for x in (0, 1, 65535):

    assert read_written(x, 'ushort') == x
print("AIFCLowLevelTest::test_read_written: ok")
