...
# lib = "configparser"
# dimension = "behavior"
# case = "config_parser_test_case_non_standard_default_section__test_interpolation_missing_value"
# subject = "cpython.test_configparser.ConfigParserTestCaseNonStandardDefaultSection.test_interpolation_missing_value"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_configparser.py::ConfigParserTestCaseNonStandardDefaultSection::test_interpolation_missing_value
"""Auto-ported test: ConfigParserTestCaseNonStandardDefaultSection::test_interpolation_missing_value (CPython 3.12 oracle)."""
...
def parse_error(cf, exc, src):
    ...
    try:
        ...
        cm = _types_aR.SimpleNamespace(exception=_aR_e)
    return cm.exception
cf = get_interpolation_config()
e = get_error(cf, configparser.InterpolationMissingOptionError, 'Interpolation Error', 'name')

assert e.reference == 'reference'

assert e.section == 'Interpolation Error'

assert e.option == 'name'
if interpolation == configparser._UNSET:

    assert e.args == ('name', 'Interpolation Error', '%(reference)s', 'reference')
elif isinstance(interpolation, configparser.LegacyInterpolation):

    assert e.args == ('name', 'Interpolation Error', '%(reference)s', 'reference')
print("ConfigParserTestCaseNonStandardDefaultSection::test_interpolation_missing_value: ok")
