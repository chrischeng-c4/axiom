...
# lib = "long"
# dimension = "behavior"
# case = "long_test__test_bit_length"
# subject = "cpython.test_long.LongTest.test_bit_length"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_long.py::LongTest::test_bit_length
"""Auto-ported test: LongTest::test_bit_length (CPython 3.12 oracle)."""
...
for x in range(-65000, 65000):
    ...
    if x != 0:
        ...
        assert 2 ** (k - 1) <= abs(x) < 2 ** k
    else:

        assert k == 0
    if x != 0:

        assert k == 1 + math.floor(math.log(abs(x)) / math.log(2) + tiny)
...
for i in [2, 3, 15, 16, 17, 31, 32, 33, 63, 64, 234]:
    ...
    assert (a - 1).bit_length() == i

    assert (1 - a).bit_length() == i

    assert a.bit_length() == i + 1

    assert (-a).bit_length() == i + 1

    assert (a + 1).bit_length() == i + 1

    assert (-a - 1).bit_length() == i + 1
print("LongTest::test_bit_length: ok")
