...
# lib = "ssl"
# dimension = "behavior"
# case = "context_tests__test_create_stdlib_context"
# subject = "cpython.test_ssl.ContextTests.test__create_stdlib_context"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_ssl.py::ContextTests::test__create_stdlib_context
"""Auto-ported test: ContextTests::test__create_stdlib_context (CPython 3.12 oracle)."""
...
# --- test body ---
def _assert_context_options(ctx):

    assert ctx.options & ssl.OP_NO_SSLv2 == ssl.OP_NO_SSLv2
    if OP_NO_COMPRESSION != 0:

        assert ctx.options & OP_NO_COMPRESSION == OP_NO_COMPRESSION
    if OP_SINGLE_DH_USE != 0:

        assert ctx.options & OP_SINGLE_DH_USE == OP_SINGLE_DH_USE
    if OP_SINGLE_ECDH_USE != 0:

        assert ctx.options & OP_SINGLE_ECDH_USE == OP_SINGLE_ECDH_USE
    if OP_CIPHER_SERVER_PREFERENCE != 0:

        assert ctx.options & OP_CIPHER_SERVER_PREFERENCE == OP_CIPHER_SERVER_PREFERENCE

    assert ctx.options & ssl.OP_LEGACY_SERVER_CONNECT == (0 if IS_OPENSSL_3_0_0 else ssl.OP_LEGACY_SERVER_CONNECT)
...
assert ctx.protocol == ssl.PROTOCOL_TLSv1_2
...
assert ctx.verify_mode == ssl.CERT_NONE
_assert_context_options(ctx)
print("ContextTests::test__create_stdlib_context: ok")
