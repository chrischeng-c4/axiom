...
# lib = "urllib_parse"
# dimension = "behavior"
# case = "utility_tests__test_splittag"
# subject = "cpython.test_urlparse.Utility_Tests.test_splittag"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_urlparse.py::Utility_Tests::test_splittag
"""Auto-ported test: Utility_Tests::test_splittag (CPython 3.12 oracle)."""
...
assert splittag('http://example.com?foo=bar#baz') == ('http://example.com?foo=bar', 'baz')

assert splittag('http://example.com?foo=bar#') == ('http://example.com?foo=bar', '')

assert splittag('#baz') == ('', 'baz')

assert splittag('http://example.com?foo=bar') == ('http://example.com?foo=bar', None)

assert splittag('http://example.com?foo=bar#baz#boo') == ('http://example.com?foo=bar#baz', 'boo')
print("Utility_Tests::test_splittag: ok")
