...
# lib = "wsgiref"
# dimension = "behavior"
# case = "utility_tests__test_hop_by_hop"
# subject = "cpython.test_wsgiref.UtilityTests.testHopByHop"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_wsgiref.py::UtilityTests::testHopByHop
"""Auto-ported test: UtilityTests::testHopByHop (CPython 3.12 oracle)."""
...
for hop in 'Connection Keep-Alive Proxy-Authenticate Proxy-Authorization TE Trailers Transfer-Encoding Upgrade'.split():
    for alt in (hop, hop.title(), hop.upper(), hop.lower()):
        ...
        assert util.is_hop_by_hop(alt)
for hop in 'Accept Cache-Control Date Pragma Trailer Via Warning'.split():
    for alt in (hop, hop.title(), hop.upper(), hop.lower()):

        assert not util.is_hop_by_hop(alt)
print("UtilityTests::testHopByHop: ok")
