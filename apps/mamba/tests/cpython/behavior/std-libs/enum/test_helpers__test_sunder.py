...
# lib = "enum"
# dimension = "behavior"
# case = "test_helpers__test_sunder"
# subject = "cpython.test_enum.TestHelpers.test_sunder"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_enum.py::TestHelpers::test_sunder
"""Auto-ported test: TestHelpers::test_sunder (CPython 3.12 oracle)."""
...
for name in sunder_names + private_and_sunder_names:

    assert enum._is_sunder(name)
for name in dunder_names + private_names + random_names:

    assert not enum._is_sunder(name)
for s in ('_a_', '_aa_'):

    assert enum._is_sunder(s)
for s in ('a', 'a_', '_a', '__a', 'a__', '__a__', '_a__', '__a_', '_', '__', '___', '____', '_____'):

    assert not enum._is_sunder(s)
print("TestHelpers::test_sunder: ok")
