...
# lib = "enum"
# dimension = "behavior"
# case = "test_helpers__test_dunder"
# subject = "cpython.test_enum.TestHelpers.test_dunder"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_enum.py::TestHelpers::test_dunder
"""Auto-ported test: TestHelpers::test_dunder (CPython 3.12 oracle)."""
...
for name in dunder_names:
    ...
    assert enum._is_dunder(name)
for name in sunder_names + private_names + private_and_sunder_names + random_names:

    assert not enum._is_dunder(name)
for s in ('__a__', '__aa__'):

    assert enum._is_dunder(s)
for s in ('a', 'a_', '_a', '__a', 'a__', '_a_', '_a__', '__a_', '_', '__', '___', '____', '_____'):

    assert not enum._is_dunder(s)
print("TestHelpers::test_dunder: ok")
