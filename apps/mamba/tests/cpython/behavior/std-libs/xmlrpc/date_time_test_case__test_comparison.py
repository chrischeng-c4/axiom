...
# lib = "xmlrpc"
# dimension = "behavior"
# case = "date_time_test_case__test_comparison"
# subject = "cpython.test_xmlrpc.DateTimeTestCase.test_comparison"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_xmlrpc.py::DateTimeTestCase::test_comparison
"""Auto-ported test: DateTimeTestCase::test_comparison (CPython 3.12 oracle)."""
...
assert not dtime == 1970

assert dtime != dbytes

assert not dtime == bytearray(dbytes)
...
assert dtime == ALWAYS_EQ

assert not dtime != ALWAYS_EQ

assert dtime < LARGEST

assert not dtime > LARGEST

assert dtime <= LARGEST

assert not dtime >= LARGEST

assert not dtime < SMALLEST

assert dtime > SMALLEST

assert not dtime <= SMALLEST

assert dtime >= SMALLEST
print("DateTimeTestCase::test_comparison: ok")
