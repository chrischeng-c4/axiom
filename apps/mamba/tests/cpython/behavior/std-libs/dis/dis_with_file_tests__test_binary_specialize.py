...
# lib = "dis"
# dimension = "behavior"
# case = "dis_with_file_tests__test_binary_specialize"
# subject = "cpython.test_dis.DisWithFileTests.test_binary_specialize"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_dis.py::DisWithFileTests::test_binary_specialize
"""Auto-ported test: DisWithFileTests::test_binary_specialize (CPython 3.12 oracle)."""
...
def strip_offsets(text):
    ...
    return ''.join(res)
binary_op_quicken = '  0           0 RESUME                   0\n\n  1           2 LOAD_NAME                0 (a)\n              4 LOAD_NAME                1 (b)\n              6 %s\n             10 RETURN_VALUE\n'
co_int = compile('a + b', '<int>', 'eval')
code_quicken(lambda: exec(co_int, {}, {'a': 1, 'b': 2}))
got = get_disassembly(co_int, adaptive=True)
do_disassembly_compare(got, binary_op_quicken % 'BINARY_OP_ADD_INT        0 (+)', True)
co_unicode = compile('a + b', '<unicode>', 'eval')
code_quicken(lambda: exec(co_unicode, {}, {'a': 'a', 'b': 'b'}))
got = get_disassembly(co_unicode, adaptive=True)
do_disassembly_compare(got, binary_op_quicken % 'BINARY_OP_ADD_UNICODE     0 (+)', True)
binary_subscr_quicken = '  0           0 RESUME                   0\n\n  1           2 LOAD_NAME                0 (a)\n              4 LOAD_CONST               0 (0)\n              6 %s\n             10 RETURN_VALUE\n'
co_list = compile('a[0]', '<list>', 'eval')
code_quicken(lambda: exec(co_list, {}, {'a': [0]}))
got = get_disassembly(co_list, adaptive=True)
do_disassembly_compare(got, binary_subscr_quicken % 'BINARY_SUBSCR_LIST_INT', True)
co_dict = compile('a[0]', '<dict>', 'eval')
code_quicken(lambda: exec(co_dict, {}, {'a': {0: '1'}}))
got = get_disassembly(co_dict, adaptive=True)
do_disassembly_compare(got, binary_subscr_quicken % 'BINARY_SUBSCR_DICT', True)
print("DisWithFileTests::test_binary_specialize: ok")
