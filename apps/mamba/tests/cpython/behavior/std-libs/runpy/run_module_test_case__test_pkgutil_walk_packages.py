...
# lib = "runpy"
# dimension = "behavior"
# case = "run_module_test_case__test_pkgutil_walk_packages"
# subject = "cpython.test_runpy.RunModuleTestCase.test_pkgutil_walk_packages"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_runpy.py::RunModuleTestCase::test_pkgutil_walk_packages
"""Auto-ported test: RunModuleTestCase::test_pkgutil_walk_packages (CPython 3.12 oracle)."""
...
pkg_dir, mod_fname, mod_name, mod_spec = _make_pkg('', max_depth)
pass
for depth in range(2, max_depth + 1):
    _add_relative_modules(pkg_dir, '', depth)
for moduleinfo in pkgutil.walk_packages([pkg_dir]):

    assert isinstance(moduleinfo, pkgutil.ModuleInfo)

    assert isinstance(moduleinfo.module_finder, importlib.machinery.FileFinder)
    if moduleinfo.ispkg:
...
print("RunModuleTestCase::test_pkgutil_walk_packages: ok")
