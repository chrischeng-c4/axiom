...
# lib = "pprint"
# dimension = "behavior"
# case = "query_test_case__test_container_repr_override_called"
# subject = "cpython.test_pprint.QueryTestCase.test_container_repr_override_called"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_pprint.py::QueryTestCase::test_container_repr_override_called
"""Auto-ported test: QueryTestCase::test_container_repr_override_called (CPython 3.12 oracle)."""
...
for cont in (list_custom_repr(), list_custom_repr([1, 2, 3]), list_custom_repr(range(N)), tuple_custom_repr(), tuple_custom_repr([1, 2, 3]), tuple_custom_repr(range(N)), set_custom_repr(), set_custom_repr([1, 2, 3]), set_custom_repr(range(N)), frozenset_custom_repr(), frozenset_custom_repr([1, 2, 3]), frozenset_custom_repr(range(N)), dict_custom_repr(), dict_custom_repr({5: 6}), dict_custom_repr(zip(range(N), range(N)))):
    ...
    assert pprint.pformat(cont) == expected

    assert pprint.pformat(cont, width=1, indent=0) == expected

    assert pprint.saferepr(cont) == expected
print("QueryTestCase::test_container_repr_override_called: ok")
