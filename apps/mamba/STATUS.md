...
project `.venv` — is the shipped outcome `mambalibs-cpython-wheels`, measured
by the `mambalibs-cpython-wheels` row below: one row for the kits' CPython
carrier, so it changes when a kit gains or loses a wheel.
...
|---|---|---|---|---|---|
| Project dependencies | `project-dependencies` | Supported | `init`, `add`, `remove`, `lock`, `export`, `tree`, and `workspace` over a PEP 621 `pyproject.toml` (`[project].dependencies`, PEP 735 `[dependency-groups]`, `[project.optional-dependencies]`, `[tool.mamba.sources]`) and `mamba.lock`, editing the manifest in place so other tables and comments survive; `add` takes several specs or `-r <FILE>` and lands them in `[project]`, the `dev` group (`--dev`), a named group (`--group`), or an extra (`--optional`); `remove` takes several names with the same scopes; `migrate` converts a retired `mamba.toml` once, resolving from a frozen `--index <DIR>`, an explicit `--index-url` (a registry base or its `…/simple` URL), or a local wheel path; `lock` and `add` pin the transitive closure with a paired `sha256` and `url` or `path` per package and record each package's `project`, `groups`, and `extras` membership, `lock` keeps the pins `mamba.lock` already records until `--upgrade` or `--upgrade-package <NAME>` frees them, `remove` keeps the remaining roots' closure and digests, and `pkgmgr-validate` accepts the PEP 503 canonical name the lock records. | Nothing reaches PyPI implicitly; a bare name needs `--index`, `--index-url`, or `MAMBA_FROZEN_INDEX` / `MAMBA_INDEX_URL`, and `pyproject.toml` records no index. A `mamba.toml` beside a `mamba.lock` is refused until `mamba migrate` converts it. A bare `add <name>` writes the exact `name==<resolved>` pin, where `uv` writes `>=`. `export` emits `requirements.txt` or `pylock.toml`, not `uv.lock`. | `cargo test -p mamba --test pkgmgr`, `cargo test -p mamba --test pkgmgr_flag_surface_uv_parity`, `cargo test -p mamba --test pkgmgr_lock_frozen_transitive`, `cargo test -p mamba --test pkgmgr_index_url_simple`, `cargo test -p mamba --test pkgmgr_lock_registry_sha_pairs_url`, `cargo test -p mamba --test pkgmgr_add_registry_transitive`, `cargo test -p mamba --test pkgmgr_remove_keeps_remaining_closure`, `cargo test -p mamba --test pkgmgr_validate_auth_family` |
| Environment and run | `environment-and-run` | Supported | `venv` seeds `.venv` from the first `python` on `PATH` or from `--python`; `sync` installs the locked wheels into `.venv`'s `lib/pythonX.Y/site-packages`, is a no-op on the second run, removes the packages `mamba.lock` no longer pins, and `sync --check` reports drift without writing; `sync` installs the project closure plus the `dev` group by default, `--no-dev`, `--group <NAME>`, `--all-groups`, `--extra <EXTRA>`, and `--all-extras` pick the selection and the environment converges on exactly it, and `sync --locked` refuses a `mamba.lock` whose recorded input hash no longer matches `pyproject.toml`; `run -- <cmd>` runs a command inside the synced environment; `run --no-project` skips the sync preflight and leaves the child's `PATH` alone, and `run --python <PATH or NAME> <file>` runs the file on that interpreter instead; inside a project `run <file>` executes the file on the `.venv` interpreter and `run --compile <file>` compiles it with mamba; outside a project `run <file>` uses the first `python` on `PATH`. | Only `sync --locked` compares `mamba.lock` to `pyproject.toml`; a plain `sync` compares `.venv` to `mamba.lock`, so an edited manifest needs `lock` first. `sync --frozen` is accepted and changes nothing, because `sync` never re-locks. `run --with <SPEC>` is not offered. | `cargo test -p mamba --test pkgmgr`, `cargo test -p mamba --test pkgmgr_flag_surface_uv_parity`, `cargo test -p mamba --test pkgmgr_sync_real_install`, `cargo test -p mamba --test pkgmgr_sync_prune`, `cargo test -p mamba --test pkgmgr_run_file_venv` |
...
| mambalibs CPython wheels | `mambalibs-cpython-wheels` | Supported | arraykit, scikit, and plotkit each ship as a PyO3 `cp312-abi3` wheel — `mambalibs-arraykit` (`import mambalibs.array`), `mambalibs-scikit` (`import mambalibs.sci`), and `mambalibs-plotkit` (`import mambalibs.plot`) — built by `maturin build` from the kit's core crate, frozen by `index build`, installed by `add` and `sync` into `.venv`'s `lib/pythonX.Y/site-packages`, and importable side by side under the PEP 420 `mambalibs` namespace; each kit's Python cases pass under pytest on the `.venv` interpreter. | The wheels are served only from a frozen local index; publication to PyPI has no case. The gates need `maturin` on `PATH` and a `python3.12` that can `import pytest`, and fail rather than skip when either is missing. cryptokit, mediakit, and mongokit have no wheel. `mamba package` does not build these wheels; the cases run `maturin build` themselves. | `cargo test -p mamba --test mambalibs_array_wheel`, `cargo test -p mamba --test mambalibs_sci_wheel`, `cargo test -p mamba --test mambalibs_plot_wheel` |
