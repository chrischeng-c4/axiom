...
**Method lookup**: `lookup_method` (mod.rs:11988) = METHOD_CACHE → MRO walk over methods then class_attrs; `lookup_method_including_none` (:12026) for `__hash__ = None` sentinels. MRO: `compute_mro` (mod.rs:12208) — duplicate-base TypeError first, linear chain for ≤1 base, else `c3_merge` (:12295); inconsistent hierarchy sets a catchable TypeError and may leave recovery-only fallback state, but the lowering barrier aborts the class statement before body/finalizers/decorators/name binding; valid paths always append `"object"`. See `class-construction.md` §MRO rejection barrier.
...
| Missing exception propagation after class define/update-bases | Invalid C3 state becomes a provisionally usable class: body/finalizers/decorators/name binding can run despite pending TypeError. Both MRO-producing calls are transactional barriers; see `class-construction.md` §MRO rejection barrier. |
