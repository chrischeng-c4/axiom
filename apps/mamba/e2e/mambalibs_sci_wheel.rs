//! Black-box contract: `mambalibs-scikit` ships as a PyO3 `abi3` wheel that the
//! mamba package manager can freeze, resolve, install, and import as
//! `mambalibs.sci` — *beside* the already-shipping `mambalibs.array`, in one
//! environment, out of one index.
//!
//! # What this case owns
//!
//! The case drives real tools over a throwaway tree and reads only what they
//! wrote. It links no product type: two wheels are built by `maturin` from
//! `apps/mamba/mambalibs/arraykit/pyo3` and `apps/mamba/mambalibs/scikit/pyo3`,
//! frozen together by one `mamba index build`, locked by one `mamba add
//! --index` naming both distributions, installed by one `mamba sync`, and
//! finally judged by the project interpreter itself. A green run means those
//! five steps agree on two artifacts that share a namespace — a wrong ABI tag,
//! a wheel that lands outside the environment, a wheel that ships
//! `mambalibs/__init__.py` and shadows its sibling, or an import that resolves
//! from the source tree each fails one named step.
//!
//! # The observation point
//!
//! The environment's interpreter is asked where each module came from
//! (`mambalibs.sci.__file__` and `mambalibs.array.__file__`), and those paths
//! are compared against this case's own `.venv` and against each other.
//! Nothing is derived from a layout constant, so an implementation that
//! invents its own directory cannot satisfy the assertions by agreeing with
//! the test's arithmetic.
//!
//! # Steps, in order
//!
//! 1. `maturin build` in the arraykit crate produces exactly one
//!    `mambalibs_arraykit-*` wheel. This is the sibling the new wheel has to
...
//!    directory: exactly one `mambalibs_scikit-*`, tagged `cp312-abi3`, with
//!    the arraykit wheel still beside it.
//! 3. `mamba index build` freezes both wheels into one local index.
//! 4. `mamba init` scaffolds a PEP 621 `pyproject.toml`; `mamba venv
//!    --system-site-packages` builds the environment on `python3.12`.
//! 5. `mamba add mambalibs-arraykit mambalibs-scikit --index <dir>` locks both
//!    index entries in one call, so neither can come from anywhere else.
//! 6. `mamba sync` installs them.
//! 7. `python -c "import mambalibs.array, mambalibs.sci"` prints both
//!    `__file__`s: each is under this case's `.venv`, they share one
//!    `mambalibs/` directory, and that directory holds no `__init__.py`
//!    (PEP 420).
//! 8. `python -m pytest -p no:cacheprovider` over `e2e/mambalibs_sci/`
//!    collects at least one test and exits 0. Those files are the wheel's
//!    Python API contract over the nine feature-gated modules.
//!
//! # Facets
//!
//! - **Behavior**: the eight steps above. The wheel is the only source of
//!   `mambalibs.sci`: the case asserts `__file__` is under its own `.venv`, so
//!   an import satisfied by the source tree, by `target/`, or by an ambient
//!   install fails rather than passes. Coexistence is asserted twice over —
//!   once on the artifacts (two wheels, one index, one `mamba add`) and once
...
//!   sibling silently and would otherwise read as success.
...
//!
//! # Relationship to `mambalibs_array_wheel.rs`
//!
//! That case is landed evidence and is not edited by this one. The helpers
//! below are copied rather than shared: two cases reaching through one helper
//! stop being independent, and the arraykit contract has to keep failing for
//! arraykit reasons alone.
...
fn pytest_dir() -> PathBuf {
    manifest_dir().join("e2e").join("mambalibs_sci")
}
...
/// The host interpreter must be able to `import pytest`: step 8 runs it
...
/// as well: both `cp312-abi3` wheels must be accepted by the shipped selector.
...
#[test]
fn scikit_ships_as_an_abi3_wheel_that_coexists_with_mambalibs_array() {
    ...
    // --- step 1: the sibling wheel, built from source --------------------
    ...
    assert!(
        ...
        "step `maturin build` needs the landed arraykit crate at {}, and it does not exist. \
         This case installs `mambalibs-arraykit` beside the new wheel to show the two share \
    ...
    // --- step 2: the wheel under contract --------------------------------
    ...
    assert!(
        ...
        "step `maturin build` needs the PyO3 crate at {}, and it does not exist. \
         `mambalibs-scikit` has no wheel to build until that crate ships: a cdylib bound \
         with pyo3's abi3-py312 feature, depending on `scikit` with `features = [\"full\"]` \
         so all nine feature-gated modules are wrapped, and a pyproject.toml whose \
         [tool.maturin] puts the module at `mambalibs.sci` with no `mambalibs/__init__.py`.",
        scikit_dir.display()
    );
    ...
    let sci_wheel_name = file_name_of(&sci_wheel);
    assert!(
        sci_wheel_name.contains("cp312-abi3"),
    ...
    assert_eq!(
        built.len(),
        2,
        "the two builds must leave exactly two wheels in {}, found {:?}",
        dist.display(),
        built.iter().map(|p| file_name_of(p)).collect::<Vec<_>>()
    );
    assert!(
        array_wheel.is_file(),
        "building the scikit wheel must not disturb the arraykit wheel beside it; {} is gone",
        array_wheel.display()
    );

    // --- step 3: one index over both wheels ------------------------------
    ...
    // --- step 5: mamba add, both distributions, one index ----------------
    let args = [
        "add",
        "mambalibs-arraykit",
        "mambalibs-scikit",
    ...
    // --- step 7: import, from the environment and nowhere else -----------
    ...
    expect_ok(
        "import mambalibs.array, mambalibs.sci",
        "python",
        &import_args,
        &out,
    );
    ...
    assert_eq!(
        lines.len(),
        2,
        "step `import mambalibs.array, mambalibs.sci` must print one `__file__` per module: {}",
        render("python", &import_args, &out)
    );

    let venv_real = std::fs::canonicalize(&venv).unwrap_or_else(|_| venv.clone());
    let mut resolved: Vec<PathBuf> = Vec::new();
    for (module, line) in ["mambalibs.array", "mambalibs.sci"].iter().zip(&lines) {
    ...
    let sci_file = resolved[1].clone();
    let package_dir = sci_file
        .parent()
        .unwrap_or_else(|| panic!("fixture: {} has no parent", sci_file.display()))
        .to_path_buf();
    assert_eq!(
    ...
    assert_eq!(
        ...
        "both kits must live in one namespace directory, or installing the second has \
         shadowed the first.\n  mambalibs.array = {}\n  mambalibs.sci   = {}",
        resolved[0].display(),
        sci_file.display()
    );

    // --- step 8: the Python API contract ---------------------------------
