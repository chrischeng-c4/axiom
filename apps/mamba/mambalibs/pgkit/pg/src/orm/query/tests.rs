...
fn test_select_with_multiple_where() {
    let qb = QueryBuilder::new("users")
        .unwrap()
        .where_clause("age", Operator::Gt, ExtractedValue::Int(18))
        .unwrap()
        .where_clause(
            "status",
            Operator::Eq,
...
#[test]
fn test_filter_by_builds_equality_conditions() {
    let qb = QueryBuilder::new("users")
        .unwrap()
        .filter_by(&[
            ("active", ExtractedValue::Bool(true)),
            ("account_type", ExtractedValue::String("admin".to_string())),
        ])
        .unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(
        sql,
        "SELECT * FROM \"users\" WHERE \"active\" = $1 AND \"account_type\" = $2"
    );
    assert_eq!(
        params,
        vec![
            ExtractedValue::Bool(true),
            ExtractedValue::String("admin".to_string()),
        ]
    );
}

#[test]
fn test_filter_by_null_uses_is_null() {
    let qb = QueryBuilder::new("users")
        .unwrap()
        .filter_by(&[
...
#[test]
fn test_where_between_builds_two_bound_params() {
    ...
    assert_eq!(
        params,
        vec![ExtractedValue::Int(10), ExtractedValue::Int(20)]
    );
}

#[test]
fn test_where_not_between_builds_two_bound_params() {
    ...
    assert_eq!(
        sql,
        "SELECT * FROM \"events\" WHERE \"created_day\" NOT BETWEEN $1 AND $2"
    );
    assert_eq!(params, vec![ExtractedValue::Int(1), ExtractedValue::Int(7)]);
}

#[test]
fn test_query_builder_driver_fetch_methods_are_exposed() {
    let _fetch_rows = QueryBuilder::fetch_rows;
    let _fetch_optional_row = QueryBuilder::fetch_optional_row;
}
...
fn test_select_with_limit_offset() {
    let qb = QueryBuilder::new("users").unwrap().limit(10).offset(20);
    let (sql, params) = qb.build_select();
...
fn test_new_with_schema_qualified_table() {
    ...
    let qb = QueryBuilder::new("public.bench_insert_one_db")
        .unwrap()
    ...
    assert_eq!(
        sql,
        "SELECT * FROM \"public\".\"bench_insert_one_db\" WHERE \"id\" = $1"
    );
    assert_eq!(params.len(), 1);
}
...
fn test_operators() {
    ...
    assert_eq!(Operator::Between.to_sql(), "BETWEEN");
    assert_eq!(Operator::NotBetween.to_sql(), "NOT BETWEEN");
...
fn test_complex_where_4_conditions() {
    let qb = QueryBuilder::new("users")
        ...
        .unwrap()
        .where_clause("score", Operator::Lt, ExtractedValue::Int(100))
        .unwrap()
        .where_clause("verified", Operator::Eq, ExtractedValue::Bool(true))
        .unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(
            sql,
...
fn test_schema_qualified_table_names() {
    ...
    let qb = QueryBuilder::new("public.users")
        .unwrap()
        .select(vec!["id".to_string(), "name".to_string()])
        .unwrap()
        .where_clause("active", Operator::Eq, ExtractedValue::Bool(true))
    ...
    let values = vec![
        (
            "name".to_string(),
            ExtractedValue::String("Widget".to_string()),
        ),
    ...
    assert_eq!(
        sql,
        "INSERT INTO \"myschema\".\"products\" (\"name\", \"price\") VALUES ($1, $2) RETURNING *"
    );
    assert_eq!(params.len(), 2);

    // Test UPDATE with schema-qualified table
    let qb = QueryBuilder::new("analytics.events")
        .unwrap()
        .where_clause("id", Operator::Eq, ExtractedValue::Int(42))
        .unwrap();
    let values = vec![("processed".to_string(), ExtractedValue::Bool(true))];
    let (sql, params) = qb.build_update(&values).unwrap();
    assert_eq!(
        sql,
        "UPDATE \"analytics\".\"events\" SET \"processed\" = $1 WHERE \"id\" = $2"
    );
    assert_eq!(params.len(), 2);

    // Test DELETE with schema-qualified table
    let qb = QueryBuilder::new("logs.audit_log")
        .unwrap()
        .where_clause(
            "created_at",
            Operator::Lt,
            ExtractedValue::String("2020-01-01".to_string()),
        )
        .unwrap();
    let (sql, params) = qb.build_delete();
    assert_eq!(
        sql,
        "DELETE FROM \"logs\".\"audit_log\" WHERE \"created_at\" < $1"
    );
    assert_eq!(params.len(), 1);
}
...
fn test_very_large_limit() {
    // Test with i64::MAX
    let qb = QueryBuilder::new("users").unwrap().limit(i64::MAX);
    let (sql, params) = qb.build_select();
    ...
    let qb = QueryBuilder::new("users").unwrap().limit(1_000_000_000);
    let (sql, params) = qb.build_select();
    assert_eq!(sql, "SELECT * FROM \"users\" LIMIT $1");
    assert_eq!(params.len(), 1);
    match &params[0] {
...
fn test_zero_limit() {
    ...
    let qb = QueryBuilder::new("users").unwrap().limit(0);
    let (sql, params) = qb.build_select();
    assert_eq!(sql, "SELECT * FROM \"users\" LIMIT $1");
    assert_eq!(params.len(), 1);
    match &params[0] {
    ...
    let qb = QueryBuilder::new("users").unwrap().limit(0).offset(10);
    let (sql, params) = qb.build_select();
    assert_eq!(sql, "SELECT * FROM \"users\" LIMIT $1 OFFSET $2");
    assert_eq!(params.len(), 2);
    match &params[0] {
...
fn test_negative_offset() {
    ...
    let qb = QueryBuilder::new("users").unwrap().offset(-10);
    let (sql, params) = qb.build_select();
    ...
    let qb = QueryBuilder::new("users").unwrap().limit(20).offset(-5);
    let (sql, params) = qb.build_select();
    assert_eq!(sql, "SELECT * FROM \"users\" LIMIT $1 OFFSET $2");
    assert_eq!(params.len(), 2);
}
...
fn test_empty_select_columns() {
    ...
    let qb = QueryBuilder::new("users").unwrap().select(vec![]).unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(sql, "SELECT * FROM \"users\"");
    assert_eq!(params.len(), 0);
...
fn test_in_operator_with_empty_list() {
    ...
    let qb = QueryBuilder::new("users")
        .unwrap()
        .where_clause("user_role", Operator::NotIn, ExtractedValue::Array(vec![]))
        .unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(
        sql,
        "SELECT * FROM \"users\" WHERE \"user_role\" NOT IN ($1)"
    );
    assert_eq!(params.len(), 1);
}
...
fn test_in_operator_with_many_values() {
    ...
    let values: Vec<ExtractedValue> = (0..150).map(|i| ExtractedValue::Int(i)).collect();

    let qb = QueryBuilder::new("users")
        .unwrap()
        .where_clause("id", Operator::In, ExtractedValue::Array(values.clone()))
        .unwrap();
    let (sql, params) = qb.build_select();
    ...
    let qb = QueryBuilder::new("users")
        .unwrap()
        .where_clause("status", Operator::In, ExtractedValue::Array(string_values))
        .unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(sql, "SELECT * FROM \"users\" WHERE \"status\" IN ($1)");
    assert_eq!(params.len(), 1);
...
fn test_like_patterns_escaping() {
    ...
    let qb = QueryBuilder::new("users")
        ...
        .where_clause(
            "email",
            Operator::Like,
            ExtractedValue::String("user_@%.com".to_string()),
        )
        .unwrap();
    let (sql, params) = qb.build_select();
    ...
    let qb = QueryBuilder::new("users")
        ...
        .where_clause(
            "first_name",
            Operator::Like,
            ExtractedValue::String("J%".to_string()),
        )
        .unwrap()
        .where_clause(
            "last_name",
            Operator::Like,
            ExtractedValue::String("%son".to_string()),
        )
        .unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(
        sql,
        "SELECT * FROM \"users\" WHERE \"first_name\" LIKE $1 AND \"last_name\" LIKE $2"
    );
    assert_eq!(params.len(), 2);
}
...
fn test_ilike_case_insensitive() {
    ...
    let qb = QueryBuilder::new("emails")
        .unwrap()
        .where_clause(
            "address",
            Operator::ILike,
            ExtractedValue::String("USER_@example.com".to_string()),
        )
        .unwrap();
    let (sql, params) = qb.build_select();
    ...
    let qb = QueryBuilder::new("articles")
        .unwrap()
        .where_clause(
            "title",
            Operator::ILike,
        ...
        .unwrap()
        .where_clause("published", Operator::Eq, ExtractedValue::Bool(true))
        .unwrap()
        .order_by("created_at", OrderDirection::Desc)
        .unwrap()
        .limit(10);
...
fn test_order_by_multiple_columns() {
    ...
    let qb = QueryBuilder::new("users")
        .unwrap()
        .order_by("department", OrderDirection::Asc)
        .unwrap()
    ...
    let qb = QueryBuilder::new("products")
        ...
        .unwrap()
        .order_by("subcategory", OrderDirection::Asc)
        .unwrap()
        .order_by("price", OrderDirection::Desc)
        .unwrap()
        .order_by("rating", OrderDirection::Desc)
        .unwrap()
        .order_by("name", OrderDirection::Asc)
        .unwrap();
    let (sql, params) = qb.build_select();
    ...
    assert_eq!(
        sql,
        "SELECT * FROM \"users\" WHERE \"active\" = $1 ORDER BY \"created_at\" DESC, \"id\" ASC"
    );
    assert_eq!(params.len(), 1);
}
...
fn test_combined_query_builder() {
    ...
    let qb = QueryBuilder::new("orders")
        ...
        .unwrap()
        .where_clause("total", Operator::Gte, ExtractedValue::Int(100))
        .unwrap()
        .where_clause("customer_id", Operator::Ne, ExtractedValue::Int(0))
        .unwrap()
        .where_not_null("payment_method")
        .unwrap()
        .order_by("created_at", OrderDirection::Desc)
        .unwrap()
    ...
    let qb = QueryBuilder::new("public.analytics_events")
        .unwrap()
        .select(vec![
        ...
        .where_clause(
            "event_type",
            Operator::Like,
            ExtractedValue::String("click%".to_string()),
        )
        .unwrap()
        .where_clause(
            "timestamp",
            Operator::Gte,
            ExtractedValue::String("2024-01-01".to_string()),
        )
        .unwrap()
...
fn test_upsert_selective_update() {
    ...
    let (sql, params) = qb
        .build_upsert(&values, &conflict_target, Some(&update_cols))
        .unwrap();
...
fn test_upsert_composite_key() {
    ...
    let values = vec![
        ...
        (
            ...
            ExtractedValue::String("Engineering".to_string()),
        ),
        (
            "name".to_string(),
            ExtractedValue::String("Alice".to_string()),
...
fn test_inner_join() {
    ...
    let qb = QueryBuilder::new("posts")
        ...
        .unwrap()
        .inner_join("users", None, condition)
        .unwrap();
...
fn test_left_join_with_alias() {
    ...
    let qb = QueryBuilder::new("posts")
        .unwrap()
        .select(vec!["p.id".to_string(), "u.name".to_string()])
        .unwrap()
        .left_join("users", Some("u"), condition)
        .unwrap();
...
fn test_multiple_joins() {
    ...
    let qb = QueryBuilder::new("posts")
        .unwrap()
        .inner_join("users", Some("u"), condition1)
        .unwrap()
        .left_join("categories", Some("c"), condition2)
        .unwrap();
...
fn test_right_join() {
    ...
    let qb = QueryBuilder::new("posts")
        .unwrap()
        .select(vec!["posts.id".to_string(), "users.name".to_string()])
        .unwrap()
        .right_join("users", None, condition)
        .unwrap();
...
fn test_full_join() {
    ...
    let qb = QueryBuilder::new("posts")
        ...
        .select(vec!["posts.id".to_string(), "users.name".to_string()])
        .unwrap()
        .full_join("users", None, condition)
        .unwrap();
...
fn test_join_with_where() {
    ...
    let qb = QueryBuilder::new("posts")
        ...
        .unwrap()
        .where_clause("posts.published", Operator::Eq, ExtractedValue::Bool(true))
        .unwrap();
...
fn test_join_with_order_and_limit() {
    ...
    let qb = QueryBuilder::new("posts")
        ...
        .inner_join("users", Some("u"), condition)
        .unwrap()
        .order_by("posts.created_at", OrderDirection::Desc)
        .unwrap()
        .limit(10);
...
fn test_join_invalid_table_name() {
    let result = QueryBuilder::new("posts").unwrap().inner_join(
        "drop",
        None,
        JoinCondition::new("author_id", "users", "id").unwrap(),
    );
    assert!(result.is_err());
}
...
fn test_join_with_schema_qualified_table() {
    let condition = JoinCondition::new("author_id", "u", "id").unwrap();
    let qb = QueryBuilder::new("public.posts")
        .unwrap()
        .select(vec!["p.id".to_string(), "u.name".to_string()])
        .unwrap()
        .inner_join("public.users", Some("u"), condition)
        .unwrap();
...
fn test_aggregate_count_with_alias() {
    let qb = QueryBuilder::new("users")
        .unwrap()
        .aggregate(AggregateFunction::Count, Some("total"))
        .unwrap();
    let (sql, params) = qb.build_select();
...
fn test_aggregate_count_column() {
    let qb = QueryBuilder::new("users")
        .unwrap()
        .aggregate(
            AggregateFunction::CountColumn("email".to_string()),
            Some("email_count"),
        )
        .unwrap();
    let (sql, params) = qb.build_select();
...
fn test_aggregate_count_distinct() {
    let qb = QueryBuilder::new("orders")
        .unwrap()
        .aggregate(
            AggregateFunction::CountDistinct("customer_id".to_string()),
            Some("unique_customers"),
        )
        .unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(
...
fn test_aggregate_sum() {
    let qb = QueryBuilder::new("orders")
        .unwrap()
        .aggregate(AggregateFunction::Sum("total".to_string()), Some("revenue"))
        .unwrap();
    let (sql, params) = qb.build_select();
...
fn test_aggregate_min_max() {
    let qb = QueryBuilder::new("temperatures")
        .unwrap()
        .aggregate(
...
fn test_aggregate_with_where() {
    let qb = QueryBuilder::new("orders")
        .unwrap()
        .aggregate(AggregateFunction::Count, Some("order_count"))
        .unwrap()
        .where_clause(
            "status",
            Operator::Eq,
    ...
    assert_eq!(
        sql,
        "SELECT COUNT(*) AS \"order_count\" FROM \"orders\" WHERE \"status\" = $1"
    );
    assert_eq!(params.len(), 1);
}
...
fn test_aggregate_with_where_and_group_by() {
    let qb = QueryBuilder::new("sales")
        .unwrap()
        .aggregate(AggregateFunction::Sum("revenue".to_string()), Some("total"))
        .unwrap()
        .aggregate(
            AggregateFunction::Avg("revenue".to_string()),
            Some("average"),
        )
        .unwrap()
        .where_clause("year", Operator::Eq, ExtractedValue::Int(2024))
        .unwrap()
        .group_by(&["department"])
        .unwrap();
    let (sql, params) = qb.build_select();
...
fn test_multiple_aggregates_no_alias() {
    let qb = QueryBuilder::new("sales")
        ...
        .aggregate(AggregateFunction::Count, None)
        .unwrap()
        .aggregate(AggregateFunction::Sum("amount".to_string()), None)
        .unwrap()
        .aggregate(AggregateFunction::Avg("amount".to_string()), None)
        .unwrap();
    let (sql, params) = qb.build_select();
    assert_eq!(
        sql,
...
fn test_complex_aggregate_query() {
    let qb = QueryBuilder::new("orders")
        .unwrap()
        .aggregate(AggregateFunction::Count, Some("total_orders"))
        .unwrap()
        .aggregate(
            AggregateFunction::Sum("amount".to_string()),
            Some("total_revenue"),
        ...
        .unwrap()
        .aggregate(
            AggregateFunction::Avg("amount".to_string()),
            Some("avg_order_value"),
        )
        .unwrap()
        .aggregate(
            AggregateFunction::CountDistinct("customer_id".to_string()),
...
fn test_having_with_avg() {
    let qb = QueryBuilder::new("students")
        .unwrap()
        .aggregate(
            AggregateFunction::Avg("score".to_string()),
            Some("avg_score"),
        )
        .unwrap()
        .group_by(&["class_id"])
...
fn test_having_invalid_column() {
    let result = QueryBuilder::new("orders")
        .unwrap()
        .aggregate(AggregateFunction::Count, None)
        .unwrap()
        .group_by(&["status"])
        .unwrap()
        .having(
            AggregateFunction::Sum("drop".to_string()),
            Operator::Gt,
            ExtractedValue::Int(100),
        );
    assert!(result.is_err());
...
fn test_complex_having_query() {
    let qb = QueryBuilder::new("sales")
        .unwrap()
        .aggregate(
            AggregateFunction::Sum("revenue".to_string()),
            Some("total_revenue"),
        )
        .unwrap()
        .aggregate(AggregateFunction::Count, Some("sale_count"))
        .unwrap()
        .aggregate(
            AggregateFunction::Avg("revenue".to_string()),
            Some("avg_revenue"),
...
fn test_distinct_with_where_and_order() {
    let qb = QueryBuilder::new("users")
        .unwrap()
        .select(vec!["email".to_string(), "name".to_string()])
        .unwrap()
        .distinct()
        .where_clause("active", Operator::Eq, ExtractedValue::Bool(true))
        .unwrap()
...
fn test_cte_basic() {
    ...
    let main_query = QueryBuilder::new("order_totals")
        .unwrap()
        .with_cte("order_totals", cte_query)
        .unwrap();
...
fn test_cte_with_where() {
    let cte_query = QueryBuilder::new("orders")
        .unwrap()
        .select(vec!["user_id".to_string(), "amount".to_string()])
        .unwrap()
        .where_clause(
            "status",
            Operator::Eq,
    ...
    let main_query = QueryBuilder::new("completed_orders")
        .unwrap()
        .with_cte("completed_orders", cte_query)
        .unwrap()
        .where_clause("amount", Operator::Gt, ExtractedValue::Float(100.0))
        .unwrap();
...
fn test_cte_name_validation() {
    ...
    let result = QueryBuilder::new("test")
        .unwrap()
...
fn test_multiple_ctes() {
    let cte1 = QueryBuilder::new("orders")
        .unwrap()
        .where_clause(
            "status",
            Operator::Eq,
        ...
        .unwrap();
    let cte2 = QueryBuilder::new("users")
        .unwrap()
        .where_clause("active", Operator::Eq, ExtractedValue::Bool(true))
        .unwrap();

    let main = QueryBuilder::new("completed_orders")
        .unwrap()
        .with_cte("completed_orders", cte1)
        .unwrap()
        .with_cte("active_users", cte2)
        .unwrap();
...
fn test_clear_ctes() {
    let cte_query = QueryBuilder::new("orders").unwrap();
    let main_query = QueryBuilder::new("test")
        .unwrap()
        .with_cte("cte1", cte_query)
        .unwrap()
        .clear_ctes();
...
fn test_cte_parameter_adjustment() {
    ...
    let cte1 = QueryBuilder::new("t1")
        .unwrap()
        .where_clause("a", Operator::Eq, ExtractedValue::Int(1))
        .unwrap();
    let cte2 = QueryBuilder::new("t2")
        .unwrap()
        .where_clause("b", Operator::Eq, ExtractedValue::Int(2))
        .unwrap();

    let main = QueryBuilder::new("result")
        .unwrap()
        .with_cte("cte1", cte1)
        .unwrap()
        .with_cte("cte2", cte2)
        .unwrap()
        .where_clause("c", Operator::Eq, ExtractedValue::Int(3))
        .unwrap();
...
fn test_subquery_with_multiple_params() {
    let subquery = QueryBuilder::new("orders")
        .unwrap()
        .where_clause("total", Operator::Gt, ExtractedValue::Float(100.0))
        .unwrap()
        .where_clause(
            "status",
            Operator::Eq,
...
fn test_subquery_parameter_index_adjustment() {
    ...
    let subquery = QueryBuilder::new("orders")
        .unwrap()
        .where_clause("total", Operator::Gt, ExtractedValue::Float(500.0))
        .unwrap();
...
fn test_multiple_subqueries() {
    let subquery1 = QueryBuilder::new("orders")
        .unwrap()
        .select(vec!["user_id".to_string()])
        .unwrap()
        .where_clause("total", Operator::Gt, ExtractedValue::Float(1000.0))
    ...
    let subquery2 = QueryBuilder::new("banned_users")
        .unwrap()
        .select(vec!["user_id".to_string()])
        .unwrap();
    ...
    let main = QueryBuilder::new("users")
        .unwrap()
        .where_in_subquery("id", subquery1)
        .unwrap()
        .where_not_in_subquery("id", subquery2)
        .unwrap();
...
#[test]
fn test_union_basic() {
    let q1 = QueryBuilder::new("active_users")
        .unwrap()
        .select(vec!["id".to_string(), "name".to_string()])
        .unwrap();
    let q2 = QueryBuilder::new("archived_users")
        .unwrap()
        .select(vec!["id".to_string(), "name".to_string()])
        .unwrap();
...
#[test]
fn test_intersect() {
    let q1 = QueryBuilder::new("all_orders")
        .unwrap()
        .select(vec!["id".to_string()])
        .unwrap();
    let q2 = QueryBuilder::new("paid_orders")
        .unwrap()
        .select(vec!["id".to_string()])
        .unwrap();
...
fn test_union_with_params() {
    let q1 = QueryBuilder::new("users")
        .unwrap()
        .where_clause("active", Operator::Eq, ExtractedValue::Bool(true))
        .unwrap();
    let q2 = QueryBuilder::new("admins")
        .unwrap()
        .where_clause(
            "user_role",
            Operator::Eq,
...
fn test_json_contains() {
    ...
    assert!(
        sql.contains(r#"'{"role": "admin"}'"#),
        "SQL should contain JSON value"
    );
}
...
fn test_json_contains_with_quote_escaping() {
    let qb = QueryBuilder::new("users")
        .unwrap()
        .where_json_contains("metadata", r#"{"name": "O'Brien"}"#)
        .unwrap();
    let (sql, _) = qb.build_select();
...
fn test_multiple_json_conditions() {
    ...
    assert!(
        sql.contains("AND"),
        "SQL should have AND between conditions"
    );
...
fn debug_json_sql() {
    ...
    let qb = QueryBuilder::new("users")
        .unwrap()
        .where_json_contained_by("settings", r#"{"theme": "dark"}"#)
        .unwrap();
    let (sql, params) = qb.build_select();
    ...
    let qb = QueryBuilder::new("users")
        .unwrap()
        .where_json_all_keys_exist("metadata", &["email", "name", "age"])
        .unwrap();
    let (sql, params) = qb.build_select();
    ...
    let qb = QueryBuilder::new("users")
        ...
        .where_json_contains("metadata", r#"{"role": "admin"}"#)
        .unwrap()
        .where_json_key_exists("metadata", "verified")
        .unwrap()
        .where_json_all_keys_exist("profile", &["name", "email"])
        .unwrap();
    let (sql, params) = qb.build_select();
...
fn test_update_returning_specific_columns() {
    ...
    let (sql, _) = qb
        .build_update(&[(
            "status".to_string(),
            ExtractedValue::String("inactive".to_string()),
        )])
        .unwrap();
    assert!(sql.contains("RETURNING"));
...
fn test_returning_allows_asterisk() {
    let result = QueryBuilder::new("users").unwrap().returning(&["*"]);
    assert!(result.is_ok());
...
#[test]
fn test_count_distinct_convenience() {
    let qb = QueryBuilder::new("events")
        .unwrap()
    ...
    assert!(sql.contains("AS \"unique_users\""));
}

#[test]
fn test_sum_convenience() {
    let qb = QueryBuilder::new("orders")
        .unwrap()
...
#[test]
fn test_min_max_convenience() {
    let qb = QueryBuilder::new("products")
        .unwrap()
        .min("price", Some("min_price"))
        .unwrap()
        .max("price", Some("max_price"))
        .unwrap();
    let (sql, _) = qb.build_select();
...
#[test]
fn test_having_count_convenience() {
    let qb = QueryBuilder::new("orders")
        .unwrap()
...
#[test]
fn test_having_sum_convenience() {
    let qb = QueryBuilder::new("orders")
        .unwrap()
...
#[test]
fn test_row_number_convenience() {
...
#[test]
fn test_rank_convenience() {
    let spec = WindowSpec::new().order_by("score", OrderDirection::Desc);
    let qb = QueryBuilder::new("students")
        .unwrap()
        .rank(spec, "student_rank")
        .unwrap();
    let (sql, _) = qb.build_select();
    assert!(sql.contains("RANK()"));
    assert!(sql.contains("AS \"student_rank\""));
}

#[test]
fn test_lag_convenience() {
    let spec = WindowSpec::new().order_by("timestamp", OrderDirection::Asc);
    let qb = QueryBuilder::new("stock_prices")
        .unwrap()
        .lag(
            "price",
            Some(1),
            Some(ExtractedValue::Int(0)),
            spec,
            "prev_price",
        )
        .unwrap();
    let (sql, _) = qb.build_select();
    assert!(sql.contains("LAG(\"price\""));
    assert!(sql.contains("AS \"prev_price\""));
}

#[test]
fn test_lead_convenience() {
    let spec = WindowSpec::new().order_by("timestamp", OrderDirection::Asc);
    let qb = QueryBuilder::new("stock_prices")
        .unwrap()
        .lead("price", Some(1), None, spec, "next_price")
        .unwrap();
    let (sql, _) = qb.build_select();
    assert!(sql.contains("LEAD(\"price\""));
    assert!(sql.contains("AS \"next_price\""));
}

#[test]
fn test_window_sum_convenience() {
    let spec = WindowSpec::new().partition_by(&["category"]);
    let qb = QueryBuilder::new("sales")
        .unwrap()
        .window_sum("amount", spec, "running_total")
        .unwrap();
    let (sql, _) = qb.build_select();
    assert!(sql.contains("SUM(\"amount\")"));
...
#[test]
fn test_from_cte_convenience() {
    let subquery = QueryBuilder::new("orders")
        .unwrap()
        .where_clause("total", Operator::Gt, ExtractedValue::Int(1000))
        .unwrap();

    let qb = QueryBuilder::from_cte("high_value_orders", subquery)
        .unwrap()
        .order_by("total", OrderDirection::Desc)
        .unwrap()
        .limit(10);
    ...
    let (sql, params) = qb.build_select();
    assert!(sql.starts_with("WITH \"high_value_orders\" AS"));
    assert!(sql.contains("SELECT * FROM \"high_value_orders\""));
    assert!(sql.contains("ORDER BY \"total\" DESC"));
    assert!(sql.contains("LIMIT"));
    assert_eq!(params.len(), 2); // $1 for 1000, $2 for limit 10
}

#[test]
fn test_combined_convenience_methods() {
    // Test a complex query using multiple convenience methods
    let qb = QueryBuilder::new("orders")
        .unwrap()
        .sum("amount", Some("total"))
        .unwrap()
        .avg("amount", Some("average"))
        .unwrap()
    ...
    assert!(sql.contains("AVG(\"amount\") AS \"average\""));
