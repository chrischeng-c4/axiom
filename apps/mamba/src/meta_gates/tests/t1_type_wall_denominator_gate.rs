...
// Locks the shape of the T1 Type Wall Denominator Gate fixture
// pinned by tests/governance/gates/t1_type_wall_denominator/manifest.toml.
// Closes #2760.
...
fn header_is_well_formed() {
    ...
    assert_eq!(m["fixture"].as_str(), Some("t1_type_wall_denominator"));
    assert_eq!(m["issue"].as_integer(), Some(2760));
    assert_eq!(m["capability"].as_str(), Some("mamba-core-semantics"));
    assert_eq!(m["work_root_wi"].as_integer(), Some(1942));
    assert_eq!(m["profile"].as_str(), Some("regression-prevention"));
    assert_eq!(m["family"].as_str(), Some("t1_type_wall_denominator"));
}
