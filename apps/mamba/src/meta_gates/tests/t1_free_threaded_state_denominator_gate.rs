...
// Locks the shape of the T1 Free Threaded State Denominator Gate fixture
// pinned by tests/governance/gates/t1_free_threaded_state_denominator/manifest.toml.
// Closes #2761.
...
fn manifest_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/governance/gates/t1_free_threaded_state_denominator/manifest.toml")
}

fn denominator_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/governance/gates/t1_free_threaded_state_denominator/denominator.txt")
}
...
fn header_is_well_formed() {
    ...
    assert_eq!(m["fixture"].as_str(), Some("t1_free_threaded_state_denominator"));
    assert_eq!(m["issue"].as_integer(), Some(2761));
    assert_eq!(m["capability"].as_str(), Some("mamba-core-semantics"));
    assert_eq!(m["work_root_wi"].as_integer(), Some(2024));
    assert_eq!(m["profile"].as_str(), Some("regression-prevention"));
    assert_eq!(m["family"].as_str(), Some("t1_free_threaded_state_denominator"));
}
