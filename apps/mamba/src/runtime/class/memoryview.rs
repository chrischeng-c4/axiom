...
pub(crate) fn memoryview_field(view: MbValue, name: &str) -> Option<MbValue> {
    let ptr = view.as_ptr()?;
    unsafe {
...
pub(crate) fn is_memoryview_instance(value: MbValue) -> bool {
...
pub(crate) fn memoryview_tuple(values: &[i64]) -> MbValue {
    MbValue::from_ptr(MbObject::new_tuple(
        values.iter().map(|v| MbValue::from_int(*v)).collect(),
    ))
}

pub(crate) fn memoryview_i64_items(value: MbValue) -> Option<Vec<i64>> {
...
pub(crate) fn memoryview_dict_str_get(value: MbValue, key: &str) -> Option<MbValue> {
    let ptr = value.as_ptr()?;
    unsafe {
...
pub(crate) fn memoryview_is_dict(value: MbValue) -> bool {
    value
        .as_ptr()
        .is_some_and(|ptr| unsafe { matches!(&(*ptr).data, ObjData::Dict(_)) })
}

pub(crate) fn memoryview_format_itemsize(format: &str) -> Option<i64> {
...
pub(crate) fn memoryview_format(view: MbValue) -> String {
    memoryview_field(view, "_format")
        .and_then(extract_str)
        .unwrap_or_else(|| "B".to_string())
}

pub(crate) fn memoryview_itemsize(view: MbValue) -> i64 {
...
pub(crate) fn memoryview_nbytes(view: MbValue) -> i64 {
...
pub(crate) fn memoryview_shape_values(view: MbValue) -> Vec<i64> {
...
pub(crate) fn memoryview_strides_from_shape(itemsize: i64, shape: &[i64]) -> Vec<i64> {
    if shape.is_empty() {
        return vec![];
    }
...
pub(crate) fn memoryview_strides_values(view: MbValue) -> Vec<i64> {
...
pub(crate) fn memoryview_readonly(view: MbValue) -> bool {
...
pub(crate) fn memoryview_released(view: MbValue) -> bool {
    memoryview_field(view, "_released")
        .map(|v| v.as_bool() == Some(true))
        .unwrap_or(false)
}

pub(crate) fn memoryview_raise_released() {
...
pub(crate) fn memoryview_slice_item_count(
    len: i64,
    start: MbValue,
    stop: MbValue,
    step: MbValue,
...
pub(crate) fn memoryview_element_at(data: &[u8], format: &str, index: usize) -> MbValue {
...
pub(crate) fn memoryview_flat_items(data: &[u8], format: &str) -> Vec<MbValue> {
...
pub(crate) fn memoryview_nested_list(flat: &[MbValue], shape: &[i64]) -> MbValue {
    if shape.len() <= 1 {
...
pub(crate) fn memoryview_slice(
    ...
    let itemsize = memoryview_itemsize(view).max(1);
    let parent_offset = memoryview_field(view, "_offset")
        .and_then(|v| v.as_int())
        .unwrap_or(0);
    let shape_values = memoryview_shape_values(view);
    let len = shape_values.first().copied().unwrap_or(0);
    let stride = step.as_int().unwrap_or(1);
    let stride = if stride == 0 { 1 } else { stride };
    let normalize = |value: MbValue, default: i64| -> i64 {
        let mut idx = value.as_int().unwrap_or(default);
    ...
    let count = memoryview_slice_item_count(len, start, stop, step);
    let new_offset = parent_offset + start_idx * itemsize;
    let shape = vec![count];
    let format = memoryview_format(view);
    let item_stride = memoryview_field(view, "_stride")
        .and_then(|v| v.as_int())
        .unwrap_or(1);
    let total_stride = stride * item_stride;
    let strides = vec![total_stride * itemsize];
    let inst = MbObject::new_instance("memoryview".to_string());
    unsafe {
        if let ObjData::Instance { fields, .. } = &(*inst).data {
            let mut f = fields.write().unwrap();
            crate::runtime::rc::retain_if_ptr(buf);
            f.insert("_buffer".to_string(), buf);
            let obj = memoryview_field(view, "_obj").unwrap_or(buf);
            crate::runtime::rc::retain_if_ptr(obj);
            f.insert("_obj".to_string(), obj);
            f.insert("_offset".to_string(), MbValue::from_int(new_offset));
            f.insert(
                "_readonly".to_string(),
                MbValue::from_bool(memoryview_readonly(view)),
            );
            f.insert("_contiguous".to_string(), MbValue::from_bool(total_stride == 1));
            f.insert("_stride".to_string(), MbValue::from_int(total_stride));
            f.insert(
                "_format".to_string(),
