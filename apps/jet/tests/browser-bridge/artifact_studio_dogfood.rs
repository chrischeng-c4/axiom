...
// CODEGEN-BEGIN
//! Dogfood the e2e evidence pipeline against the Artifact Studio
...
fn fixture_spec_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../examples/jet-e2e-demo/artifact-studio.spec.js")
        .canonicalize()
        .expect("artifact-studio.spec.js must exist alongside the jet crate")
}

fn summary_with(outcome_for_promote: Outcome) -> Summary {
    ...
    if matches!(outcome_for_promote, Outcome::Failed) {
        promote_report.error = Some(TestError {
            ...
            stack: Some(
                "Error: Expected work-state to be 'shipped'\n    at /tmp/examples/artifact-studio.spec.js:60:5"
                    .to_string(),
            ),
            diff: Some("- shipped\n+ reviewing".to_string()),
...
fn assert_bundle_shape(bundle: &E2eEvidenceBundle, expected_mode: E2eMode) {
    ...
    assert_eq!(bundle.cases.len(), 2, "two artifact studio cases");
    ...
    for case in &bundle.cases {
        ...
        assert!(case.title.starts_with("Artifact Studio"));
    }
}
...
fn fixture_is_present_and_self_contained() {
    ...
    assert!(body.contains("Artifact Studio"));
...
#[test]
fn run_mode_evidence_carries_product_steps_for_artifact_studio_flow() {
    let summary = summary_with(Outcome::Passed);
...
fn failure_path_carries_assertion_context() {
    let summary = summary_with(Outcome::Failed);
