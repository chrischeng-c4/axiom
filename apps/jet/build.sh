...
usage() {
  cat <<'EOF'
Usage: apps/jet/build.sh <debug|release>
...
fail_hint() {
  ...
  echo ""
  echo "Build failed."
  echo "Retry with: apps/jet/build.sh ${mode}"
...
if [[ "$MODE" == "debug" ]]; then
  VERSION_FILES=(apps/jet/Cargo.toml)
  CURRENT_VERSION="$(project_build_read_version apps/jet/Cargo.toml)"
