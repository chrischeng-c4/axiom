...
| Code Splitting At Dynamic Import Boundaries | epic | #1906 | implemented | verified | corpus | `cargo test -p jet --test code_splitting -- --nocapture` — web-target `jet build` chunk-splits by default when the module graph has dynamic `import()`s (entry chunk + async/shared chunks + `__jet__` runtime loader + chunk manifest); `--no-splitting` and `[build].splitting` in jet.toml are the escape hatches (flag > config > target default; non-web targets stay opt-in only); a graph with no dynamic imports falls back to the pre-existing single-file output byte-for-byte; large-corpus fixture (200 generated dynamic-import modules) budgets chunk count, entry-payload exclusion, and build wall-time |
| Manual Chunks Config Routing | change | #1948 | implemented | verified | conformance | `cargo test -p jet --lib bundler::splitting -- --nocapture`<br>`cargo test -p jet --lib task_runner::config -- --nocapture`<br>`cargo test -p jet --test code_splitting -- manual_chunks --nocapture` — `[build.manual_chunks]` in jet.toml (chunk name → glob patterns, matched against each module's absolute filesystem path, vite `manualChunks`-style) routes matched modules into a named `ChunkType::Shared` chunk instead of the entry/auto-`shared` chunk; entry excludes the routed module's code and gains a `<link rel="preload">` hint for the named chunk; unrelated async chunks are unaffected; no config leaves `--splitting` output byte-for-byte unchanged; a chunk whose glob patterns fail to compile degrades gracefully (GH #3300 warn-and-drop, matches zero modules) instead of failing the build |
...
| Fix CSS Layer Statement Form Parse Error | change | #1377 | implemented | verified | conformance | `cargo test -p jet --lib css::directives -- --nocapture` — the bare CSS Cascade Layers order statement (`@layer theme, base, components, utilities;`) emitted by Tailwind v4 is recognized and dropped by the directive pipeline before the final `lightningcss` parse step, alongside unchanged block-form `@layer name { ... }` inlining — see `apps/jet/tech-design/logic/jet-css-parser-fails-to-parse-tailwind-css-v4-layer-directives.md` |
| Fix CSS Bare-Specifier Import Resolution For Package Directories | change | #1375 | planned | none | smoke | `cargo test -p jet --lib css::import_resolver` — a bare-specifier `@import` (e.g. Tailwind v4's `@import "tailwindcss";`) whose `node_modules/<pkg>` path is a directory resolves via the package's `package.json` `exports`/`style`/`main` map instead of raising "Is a directory (os error 21)" — see `apps/jet/tech-design/logic/jet-build-fails-to-resolve-tailwind-css-import-tailwindcss.md` |
| RHS Normalization Shape Predicate | change | #2168 | planned | none | smoke | `cargo test -p jet --lib bundler::scope_hoist_opt::rhs_normalization_tests -- --nocapture`<br>`cargo test -p jet --test entry_flatten -- --nocapture`<br>`cargo test -p jet --test production_build_regression -- --nocapture` — relaxes the RHS-normalization purity gate in the flat-region export-elision pipeline (`is_pure_normalizable_export_rhs` → shape-based `is_shape_normalizable_export_rhs`, `scope_hoist_opt.rs`) from a 3-shape allow-list to a 3-check deny-list (top-level comma / chained assignment / await-yield), on the evaluation-position argument that the `var __jx = <RHS>; <exports_obj>.key = __jx;` rewrite never moves/duplicates/delays/skips RHS evaluation, unlocking the previously-rejected `ComplexRhs` bucket (536 keys / ~13.7KB, #2139 attribution) — see `apps/jet/tech-design/logic/drop-the-rhs-normalization-purity-gate-evaluation-position-argum.md` |
...
Promise:
Jet's project architecture and authoring guidance (the top-level path-role map plus crate/package naming conventions) lives in scoped project docs rather than a project-root uppercase meta doc. `apps/jet/docs/architecture/layout.md` is the discoverable, README-linked home for that guidance; `apps/jet/LAYOUT.md` no longer exists, and README/CAPABILITIES.md remain the only Jet project-root uppercase meta docs.
Gate Inventory:
- `apps/jet/docs/architecture/layout.md`
- `test ! -e apps/jet/LAYOUT.md && ! grep -rl "apps/jet/LAYOUT.md" --include=*.md . | grep -v apps/jet/tech-design/logic/move-root-layout-meta-doc-into-scoped-architecture-documentation.md`
Surfaces:
- Docs: `apps/jet/docs/architecture/layout.md` - Scoped path-role map and crate/package naming conventions, linked from this README's Source map.
EC Dimensions:
- behavior: `test ! -e apps/jet/LAYOUT.md && ! grep -rl "apps/jet/LAYOUT.md" --include=*.md . | grep -v apps/jet/tech-design/logic/move-root-layout-meta-doc-into-scoped-architecture-documentation.md` - No project-root uppercase meta doc remains and no live reference still points at the retired path.

| Work Root | Kind | WI | Impl | Verification | Maturity | Gate / Evidence |
|---|---|---:|---|---|---|---|
| Move root layout meta doc into scoped architecture documentation | change | #1169 | implemented | verified | smoke | `apps/jet/docs/architecture/layout.md` exists with the preserved path-role map and naming conventions; `apps/jet/LAYOUT.md` removed; README Source map row repointed — see `apps/jet/tech-design/logic/move-root-layout-meta-doc-into-scoped-architecture-documentation.md` |
| Rebrand jet docs site and nav-link orphaned design notes | change | #1083 | planned | none | smoke | `apps/jet/docs/index.md` hero/features, `.vitepress/config.mjs` title, and `package.json` name carry jet identity (no `cclab` branding, no non-jet placeholder features); every pre-existing hand-written design-note markdown file under `apps/jet/docs/` that the doc-relocation left unlinked is reachable from the VitePress nav/sidebar — see `apps/jet/tech-design/logic/jet-docs-site-carries-cclab-era-branding-after-move-to-projects.md` |
