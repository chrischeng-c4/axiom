...
changes:
  - path: projects/jet/src/bundler/dts.rs
    ...
    reason: "Empirically re-verified WI #1262's exact claim (jet build --lib --dts silently truncates every object-literal property after one whose value is Object.assign({}, ...arr.map(cb)), with no error and exit code 0) at the WI's OWN stated scale, not the reduced 2-property scale the prior #1238 TD validated. Built the issue's exact 3-property minimal repro fresh with target/debug/jet build --lib --format esm --dts on current app/jet HEAD: all 3 members emitted, matching the issue's stated tsc ground truth exactly -- no truncation. Reconstructed the issue's own \"Real-world impact\" description (fe-shared's 11-method _Query) and built it the same way: all 11 members emitted, cross-checked byte-for-byte against a direct tsc --isolatedDeclarations --declaration --emitDeclarationOnly run on the identical source (tsc emits unrelated TS2550 lib-target diagnostics for Object.assign/Object.entries in this environment but still emits the correct, complete .d.ts) -- jet's output matches the compiler's own ground truth. Additionally stress-tested a 12-property variant carrying TWO separate Object.assign-valued members in the same literal (not required by the issue, but rules out a member-count-relative or Object.assign-count-relative edge the #1238 TD's single-Object.assign 2-property probe could not detect): all 12 members emitted correctly. Root cause (why this is already fixed at scale, no Object.assign-specific logic change needed): infer_arrow_function_type_from_text resolves each member's own arrow head from its explicit return type and never inspects the Object.assign(...) body, so the Object.assign call was never actually the obstacle; the real defect lived in split_top_level(inner, ','), the single shared routine every object-literal member-splitting call site uses, which WI #1264's fix (commit 75cb5e5ca) corrected to stop miscounting the arrow token's own trailing '>' as a generic-close that decrements bracket depth. That correction is applied once per '>' character scan with no dependency on member count or Object.assign-member count, so it generalizes uniformly rather than coincidentally working only at small scale. WI #1262's own filed report reproduces jet 0.4.16's PRE-#1264 state; on current app/jet HEAD (post commits 75cb5e5ca and d9ac6afea, same fix as the #1238/#1263/#1264 siblings) the defect this WI describes is already fixed as a side effect of that shared-routine correction. This mirrors the established WI #937/#1264/#1263/#1238 pattern in this TD family: an already-implemented fix, closed with regression-locking tests rather than a new logic change. Add the R1-R2 regression tests specified in the unit-test section to the existing mod tests block in dts.rs: R1 pins WI #1262's real-world-scale (11-member) positive case with both per-member substring assertions and an explicit emitted-member-count assertion (== 11) so a partial-truncation regression that happens to preserve unrelated member text still fails the test; R2 is a non-regression stress control proving the fix also holds when TWO Object.assign-valued members exist in the same literal, a shape the #1238 TD's single-Object.assign probes never covered. FAMILY TERMINUS: this closes the jet --lib --dts isolatedDeclarations false-positive/truncation TD family -- all four sibling WIs (#937 expr-as-Type explicit-annotation false positive, #1264 arrow-body-returns-typed-object-literal false positive, #1263 nested-plain-object-literal false positive, #1238 Object.assign+computed-key arrow-property false positive/truncation) are td_merged, and this TD closes the one remaining open thread #1238 explicitly deferred: WI #1262's real-world at-scale (11-method) silent-truncation symptom. No further open WI in this family references projects/jet/src/bundler/dts.rs's object-literal member-splitting path as of this TD."
```
