...
pub const TOPICS: &[cli_std::llm::Topic] = &[
    ...
    cli_std::llm::Topic {
        ...
        body: "# relay — operating the server\n\n\
            ...
            - `--grace-secs` (`RELAY_GRACE_SECS`, default `10`) — graceful-drain window: \
            ...
            the downward API) and the same `relay` bin runs a raft group — publish, \
            lease, ack/nack, heartbeat, and expiry/reclaim all commit through Raft, \
            ...
            Executor node + monotonic epoch fence stale outcomes; assignment commits \
            before a work frame is sent. Delivery stays at-least-once for ambiguous \
            effects and expiry redelivery.\n\n\
            ...
            CronJob). Restore feeds the artifact to `load_live` for exact state \
            replacement, preserving seq/next-seq, committed offsets, active fenced \
            leases, and retry/delay state.\n\n\
