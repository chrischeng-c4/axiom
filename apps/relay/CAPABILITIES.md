...
shared `service-http::server_timing` contract) is not yet wired into relay's
...
- HTTP: Server-Timing response attribution — shared `service-http::server_timing` contract (`Server-Timing: app;dur=` per-response latency), wiring pending (#2490 adoption batch).
EC Dimensions:
