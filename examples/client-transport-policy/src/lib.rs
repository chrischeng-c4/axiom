...
pub struct ClientTransportSample {
    pub language: &'static str,
    pub file_count: usize,
    pub transport_contract: bool,
}

pub fn lumen_client_transport_samples() -> Result<Vec<ClientTransportSample>> {
    let spec = lumen::spec::openapi_json();
    Ok(vec![
        inspect_generated_client("typescript", generate(&spec, &opts(Lang::Ts))?)?,
        inspect_generated_client("python", generate(&spec, &opts(Lang::Py))?)?,
        inspect_generated_client("rust", generate(&spec, &opts(Lang::Rust))?)?,
    ])
}
...
fn inspect_generated_client(
    ...
    let text = output
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let transport_contract = match language {
        "typescript" => contains_all(
            &text,
            &[
                "export interface TransportPolicy",
                "const DEFAULT_MAX_CONNECTIONS = 128;",
                "const DEFAULT_MAX_KEEPALIVE_CONNECTIONS = 16;",
                "maxInFlightPerOrigin",
                "poolTimeoutMs",
                "recommendedH2Connections",
            ],
        ),
        "python" => contains_all(
            &text,
            &[
                ...
                "def recommended_h2c_connections(",
        ...
        "rust" => contains_all(
            ...
            &[
                ...
                "const DEFAULT_MAX_KEEPALIVE_CONNECTIONS: usize = 16;",
                "max_in_flight_per_origin",
                "PoolTimeout(Duration)",
                "recommended_h2_connections",
            ],
        ),
        other => return Err(anyhow!("unsupported language sample {other}")),
    };
    Ok(ClientTransportSample {
        language,
...
mod tests {
    ...
    impl LumenServer {
        async fn start() -> Self {
            ...
            let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
                .await
                .expect("bind test lumen server");
            let addr: SocketAddr = listener.local_addr().expect("lumen server local addr");
        ...
        async fn wait_ready(&self) {
            ...
            loop {
                ...
                assert!(
                    ...
                    "lumen server did not become ready at {url}"
                );
        ...
        async fn shutdown(mut self) {
            ...
            let _ = tokio::time::timeout(std::time::Duration::from_secs(6), self.handle).await;
        }
    }
    ...
    fn unique_temp_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "axiom-client-transport-policy-{label}-{}-{nanos}-{serial}",
            std::process::id()
        ))
    }
    ...
    #[test]
    fn real_lumen_openapi_generates_transport_policy_in_all_clients() {
        let samples = lumen_client_transport_samples().unwrap();
        assert_eq!(samples.len(), 3);
        for sample in samples {
            assert!(
                sample.file_count >= 3,
                "{} should generate a real client package",
                sample.language
            );
            assert!(
                sample.transport_contract,
                "{} generated client missing shared transport policy",
                sample.language
            );
        }
    }
    ...
    #[test]
    fn h2_physical_connections_are_log_shaped_below_the_abstract_cap() {
        let hint = h2_physical_connection_hint();
        assert_eq!(hint, 5);
        assert!(hint < EXAMPLE_TARGET_CONCURRENCY);
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn generated_python_client_runs_crud_happypath_against_real_lumen_server() {
        if !command_exists("python3") {
            eprintln!("skip python generated-client smoke: python3 not found");
            return;
        }
        let pydantic = Command::new("python3")
            ...
            .expect("run python3");
        if !pydantic.success() {
            eprintln!("skip python generated-client smoke: pydantic not installed");
            return;
        }
...
"#,
        ...
        let output = Command::new("python3")
            ...
            .arg(script)
            .output()
            .expect("run generated Python client smoke");
        assert!(
            output.status.success(),
            "generated Python client smoke failed\nstdout:\n{}\nstderr:\n{}",
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn generated_typescript_client_runs_crud_happypath_against_real_lumen_server() {
        if !command_exists("node") || !command_exists("tsc") {
            eprintln!("skip TypeScript generated-client smoke: node or tsc not found");
            return;
        }
...
"#,
        ...
        let tsc = Command::new("tsc")
            ...
            .output()
            .expect("run tsc for generated TypeScript client smoke");
        assert!(
            tsc.status.success(),
        ...
        let node = Command::new("node")
            ...
            .output()
            .expect("run generated TypeScript client smoke");
        assert!(
            node.status.success(),
            "generated TypeScript client smoke failed\nstdout:\n{}\nstderr:\n{}",
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn generated_rust_client_runs_crud_happypath_against_real_lumen_server() {
        if !command_exists("cargo") {
            eprintln!("skip Rust generated-client smoke: cargo not found");
            return;
        }
...
"#,
        ...
        .expect("write generated Rust smoke Cargo.toml");
        fs::write(
            src.join("main.rs"),
            format!(
                r#"
...
"#,
        ...
        .expect("write generated Rust smoke main");
        let output = Command::new("cargo")
            .arg("run")
            .arg("--quiet")
            .current_dir(&dir)
            .output()
            .expect("run generated Rust client smoke");
        assert!(
            output.status.success(),
            "generated Rust client smoke failed\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
        let _ = fs::remove_dir_all(&dir);
