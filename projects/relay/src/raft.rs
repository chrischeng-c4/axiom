...
// HANDWRITE-BEGIN gap="missing-generator:logic:786bf09f" tracker="pending-tracker" reason="Single-shard Raft consensus core. The step-driven engine was extracted to the shared `raft_core` crate (libs/raft-core, serde-only) so relay and keep share one verified implementation; relay re-exports it and supplies the glue (raft_driver h2c transport, raft_store persistence, raft_config k8s)."
//! Single-shard Raft consensus core for relay.
//!
//! The step-driven engine now lives in the shared [`raft_core`] crate
//! (`libs/raft-core`, serde-only) so relay and keep share one verified
