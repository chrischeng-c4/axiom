//! `lumen upgrade` — self-update the installed binary from its own GitHub
//! releases.
//!
//! lumen publishes one `lumen-<target>.tar.gz` (+ `.sha256`) per release tag
//! (`lumen@X.Y.Z`), with the inner layout `lumen-<target>/{lumen,README.md}`.
//! This module resolves the running target + version, picks the latest stable
//! (or a pinned) release, downloads the matching asset, verifies its sha256,
//! extracts the inner binary, and atomically replaces the running executable.
//!
...
//! (it needs the otherwise-optional HTTP client).
//!
...
/// GitHub repository that publishes lumen release assets.
pub const DEFAULT_REPO: &str = "chrischeng-c4/axiom";

/// Release-tag prefix; tags look like `lumen@0.4.3`.
const TAG_PREFIX: &str = "lumen@";
...
/// The release-asset filename for a target, e.g. `lumen-aarch64-apple-darwin.tar.gz`.
pub fn asset_name(target: &str) -> String {
    format!("lumen-{target}.tar.gz")
}
...
/// Parse a release tag (`lumen@X.Y.Z`) into a semver. Returns `None` for tags
/// that are not lumen releases or are not valid semver — callers skip those
/// rather than treating them as an error.
pub fn parse_tag(tag: &str) -> Option<Version> {
    Version::parse(tag.strip_prefix(TAG_PREFIX)?).ok()
}

/// Pick the `(tag, version)` to install. With `pin` set, match that exact
/// version (accepting either `X.Y.Z` or `lumen@X.Y.Z`); otherwise choose the
/// highest stable (non-prerelease) release.
...
/// Extract the inner `lumen-<target>/lumen` binary bytes from a gzip tarball.
/// Errors if the expected entry is absent.
pub fn extract_binary(tar_gz: &[u8], target: &str) -> Result<Vec<u8>> {
    let want = format!("lumen-{target}/lumen");
...
/// Run `lumen upgrade` against the default release repository.
#[cfg(feature = "self-update")]
pub async fn run(opts: Options) -> Result<()> {
    run_with_repo(opts, DEFAULT_REPO).await
}

/// Without the `self-update` feature the HTTP client is not linked, so the
/// command degrades clearly instead of silently missing (mirrors `lumen k8s`).
#[cfg(not(feature = "self-update"))]
pub async fn run(_opts: Options) -> Result<()> {
    anyhow::bail!(
        "this lumen build was compiled without self-update support; rebuild with \
         `--features self-update` (the published binary includes it)"
    )
}
...
async fn run_with_repo(opts: Options, repo: &str) -> Result<()> {
    ...
    let Some((tag, selected)) = select_version(&tags, opts.tag.as_deref()) else {
        match opts.tag.as_deref() {
            Some(t) => bail!(
                "no lumen release matching `{t}` (scanned {} tags)",
                tags.len()
            ),
            None => bail!(
                "no stable lumen release found (scanned {} tags)",
                tags.len()
            ),
        }
    };
...
fn confirm(current: &Version, selected: &Version) -> Result<bool> {
    ...
    print!("upgrade lumen {current} → {selected}? [y/N] ");
...
fn install_binary(bin: &[u8]) -> Result<()> {
    ...
    let tmp = dir.join(format!(".lumen-upgrade-{}.tmp", std::process::id()));
    ...
    if e.kind() == std::io::ErrorKind::PermissionDenied {
        anyhow::anyhow!(
            "cannot replace {}: permission denied. Re-run with elevated permissions \
             (e.g. `sudo lumen upgrade`) or reinstall manually.",
            exe.display()
        )
    } else {
...
mod tests {
    ...
    fn asset_and_sha_names() {
        assert_eq!(
            ...
            "lumen-aarch64-apple-darwin.tar.gz"
        );
        assert_eq!(
            sha_name("lumen-aarch64-apple-darwin.tar.gz"),
            "lumen-aarch64-apple-darwin.tar.gz.sha256"
        );
    }

    #[test]
    fn parse_tag_skips_non_lumen_and_malformed() {
        assert_eq!(parse_tag("lumen@1.2.3"), Some(Version::new(1, 2, 3)));
        assert!(parse_tag("vat@0.3.62").is_none());
        assert!(parse_tag("lumen@not-semver").is_none());
        assert!(parse_tag("0.4.3").is_none());
    }

    #[test]
    fn select_latest_stable() {
        let tags = vec![
            "lumen@0.4.0".to_string(),
            "lumen@0.4.3".to_string(),
            "lumen@0.4.10".to_string(),
            "vat@9.9.9".to_string(),
        ];
    ...
    fn select_exact_tag_override() {
        ...
        // bare version
        let (tag, v) = select_version(&tags, Some("0.4.3")).unwrap();
        assert_eq!(tag, "lumen@0.4.3");
        assert_eq!(v, Version::new(0, 4, 3));
        // lumen@-prefixed pin
        let (tag2, _) = select_version(&tags, Some("lumen@0.4.0")).unwrap();
        assert_eq!(tag2, "lumen@0.4.0");
        // unknown pin → None
    ...
    fn prerelease_excluded_from_latest_but_pinnable() {
        let tags = vec!["lumen@1.0.0".to_string(), "lumen@1.1.0-rc.1".to_string()];
        let (tag, _) = select_version(&tags, None).unwrap();
        assert_eq!(tag, "lumen@1.0.0");
        let (pin, _) = select_version(&tags, Some("1.1.0-rc.1")).unwrap();
        assert_eq!(pin, "lumen@1.1.0-rc.1");
    }
    ...
    fn extract_binary_reads_inner_and_errs_when_absent() {
        ...
        let gz = make_tar_gz(&[(format!("lumen-{target}/lumen"), payload.as_slice())]);
        assert_eq!(extract_binary(&gz, target).unwrap(), payload);

        let gz_missing = make_tar_gz(&[("lumen-other/lumen".to_string(), payload.as_slice())]);
