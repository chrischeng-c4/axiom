//! Durable storage for a lumen Raft node's hard state.
//!
...
//! The driver calls [`RaftStore::save`] *before* flushing the node's outbox, so
//! no vote or ack is sent before the decision that produced it is durable.
//! Mirrors relay's `raft_store` (lumen has no `FsyncPolicy` of its own, so a
//! minimal one lives here).

use std::fs::{create_dir_all, File};
use std::io::{self, Write};
use std::path::PathBuf;

use crate::raft_core::{NodeId, PersistedState};
