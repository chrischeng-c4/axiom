...
// HANDWRITE-BEGIN gap="missing-generator:logic:54088576" tracker="pending-tracker" reason="RelayWal: a WalLog backed by relay's broadcast. publish POSTs CBOR to relay /v1/{subject}/publish (payload=lumen WalRecord binary envelope); subscribe GETs /v1/{subject}/subscribe and decodes relay's length-prefixed CBOR LogEntry frames (relay::wire::decode_frames), mapping each to (seq+1, WalRecord). Plaintext h2c, no TLS."
