...
// @command cargo test -p lumen --test stability_lumen_search_stability_backpressure -- --ignored
// AW-EC-END

// Contract: GAP (d): under concurrent load beyond steady-state capacity the server sheds/bounds (error rate + p99 bounded), never OOMs or crashes. Scenario being added.
#[test]
#[ignore = "AW EC gate: run via `aw health --verify-ec` or `cargo test -- --ignored`"]
fn lumen_search_stability_overload_backpressure() {
    let command = "cargo test -p lumen --test stability_lumen_search_stability_backpressure -- --ignored";
