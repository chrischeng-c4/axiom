...
|------|--------|------|------------|------|-----------|
| `DriveOutcome` | projects/lumen/src/operator/reshard_driver.rs | enum | pub | 383 |  |
...
after eviction), plus three new #1396 R1/R2 private helpers:
`reshard_fence_call`, `set_write_fence` (arm/clear `POST
/admin/reshard:fence` on every current source shard), and
...
now brackets with a fence arm/clear).
