...
capability_refs:
  - id: "long-running-stability"
    ...
    coverage: partial
    rationale: >
      New hand-written module (#809): a `spec.serving.raftStorage` CR edit
      does not, by itself, resize existing per-pod PVCs because StatefulSet
      `volumeClaimTemplates` are immutable after creation. This module is the
      detect-and-patch tool for that gap, backing the new
      `lumen k8s operator resize-storage` CLI verb. No generator primitive
      exists yet for this PVC-list + quantity-compare + conditional-patch
      shape, so it stays HANDWRITE per CLAUDE.md ("no skip state for source
...
is a silent no-op for that field. This module is the detect-and-patch tool
for the gap: pure quantity comparison ([`parse_storage_bytes`], [`decide`])
plus an impure driver ([`resize_instance`]) that lists the instance's live
`raft-<name>-<n>` PVCs, compares each to the CR's declared size, and patches
`spec.resources.requests.storage` directly on PVCs whose bound
`StorageClass` allows expansion. PVC shrink is never attempted — Kubernetes
does not support it. Gated `#[cfg(feature = "operator")]`; the default
(no-feature) build links no kube client.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `ResizeAction` | projects/lumen/src/operator/resize.rs | enum | pub | 28 |  |
| `parse_storage_bytes` | projects/lumen/src/operator/resize.rs | function | pub | 47 | parse_storage_bytes(qty: &str) -> Result<u64> |
| `decide` | projects/lumen/src/operator/resize.rs | function | pub | 84 | decide(current: &str, desired: &str) -> ResizeAction |
| `PvcResizeOutcome` | projects/lumen/src/operator/resize.rs | struct | pub | 116 |  |
| `resize_instance` | projects/lumen/src/operator/resize.rs | function | pub async | 131 | resize_instance(client: kube::Client, namespace: &str, name: &str, dry_run: bool) -> Result<Vec<PvcResizeOutcome>> |
...
changes:
  - path: projects/lumen/src/operator/resize.rs
    ...
    description: |
      New module (#809), hand-written: no generator primitive exists yet for
      a PVC-list + Kubernetes-quantity-compare + conditional-patch shape.
      parse_storage_bytes/decide are pure and unit-tested; resize_instance is
      the impure driver that lists an instance's raft-<name>-<n> PVCs,
      compares each against the CR's spec.serving.raftStorage, and patches
      spec.resources.requests.storage on PVCs whose bound StorageClass allows
      expansion, honoring --dry-run.
```
<!-- HANDWRITE-END -->
