...
async fn query_logs(client: &reqwest::Client, base: &str, trace_id: Option<&str>) -> Value {
    let mut query = json!({ "project": "project-a", "environment": "prod", "limit": 50 });
    if let Some(trace_id) = trace_id {
        query["trace_id"] = Value::String(trace_id.to_string());
    }
    let response = client
        .post(format!("{base}/v1/logs:query"))
        .json(&query)
        .send()
        .await
...
async fn cri_partial_rotation_trace_metadata_and_cloud_coexistence() {
    ...
    let duplicate_response: Value = client
        .post(format!("{base}/v1/events:write"))
        .header("x-sift-project", "project-a")
        .json(&json!({ "events": [gcp_duplicate] }))
        .send()
        .await
        .unwrap()
        .json()
        .await
        .unwrap();
    assert_eq!(duplicate_response["duplicates"], 1);
