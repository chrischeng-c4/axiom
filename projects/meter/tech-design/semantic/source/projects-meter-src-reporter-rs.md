---
id: projects-meter-src-reporter-rs
...
# Standardized projects/meter/src/reporter.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `projects/meter/src/reporter.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `AgentEvalReport` | projects/meter/src/reporter.rs | struct | pub | 197 |  |
| `CoverageInfo` | projects/meter/src/reporter.rs | struct | pub | 97 |  |
| `EnvironmentInfo` | projects/meter/src/reporter.rs | struct | pub | 67 |  |
| `FileCoverage` | projects/meter/src/reporter.rs | struct | pub | 81 |  |
| `ReportFormat` | projects/meter/src/reporter.rs | enum | pub | 13 |  |
| `Reporter` | projects/meter/src/reporter.rs | struct | pub | 247 |  |
| `TestReport` | projects/meter/src/reporter.rs | struct | pub | 46 |  |
| `all_passed` | projects/meter/src/reporter.rs | function | pub | 239 | all_passed(&self) -> bool |
| `console` | projects/meter/src/reporter.rs | function | pub | 284 | console() -> Self |
| `failed_results` | projects/meter/src/reporter.rs | function | pub | 168 | failed_results(&self) -> Vec<&TestResult> |
...
| `markdown` | projects/meter/src/reporter.rs | function | pub | 259 | markdown() -> Self |
| `new` | projects/meter/src/reporter.rs | function | pub | 113 | new(suite_name: impl Into<String>, results: Vec<TestResult>) -> Self |
| `new` | projects/meter/src/reporter.rs | function | pub | 213 | new(         suite_name: impl Into<String>,         results: Vec<AgentEvalResult>,         metrics: AgentEvalMetrics,     ) -> Self |
| `new` | projects/meter/src/reporter.rs | function | pub | 254 | new(format: ReportFormat) -> Self |
| `results_by_type` | projects/meter/src/reporter.rs | function | pub | 160 | results_by_type(&self, test_type: TestType) -> Vec<&TestResult> |
| `set_coverage` | projects/meter/src/reporter.rs | function | pub | 155 | set_coverage(&mut self, coverage: CoverageInfo) |
| `with_coverage` | projects/meter/src/reporter.rs | function | pub | 149 | with_coverage(mut self, coverage: CoverageInfo) -> Self |
| `with_environment` | projects/meter/src/reporter.rs | function | pub | 143 | with_environment(mut self, env: EnvironmentInfo) -> Self |
...
impl Reporter {
                ...
                for file in &coverage.files {
                    ...
                    if !file.missing_lines.is_empty() {
                        let missing: Vec<String> =
                            file.missing_lines.iter().map(|n| n.to_string()).collect();
    ...
    fn generate_console(&self, report: &TestReport) -> String {
        ...
        // Header
        let status_color = if report.summary.all_passed() {
            GREEN
        } else {
            RED
        };
        let status_text = if report.summary.all_passed() {
            "PASSED"
        } else {
            "FAILED"
        };
        ...
        if let Some(ref coverage) = report.coverage {
            ...
            // Low coverage files
            let low_coverage: Vec<_> = coverage
                .files
                .iter()
        ...
        for result in &report.results {
            ...
            writeln!(
                output,
                "  {}{} {}{}  {}{}ms{}",
                color, icon, result.meta.name, RESET, DIM, result.duration_ms, RESET
            )
            .unwrap();
    ...
    fn generate_agent_eval_markdown(&self, report: &AgentEvalReport) -> String {
        ...
        writeln!(
            output,
            "| Pass Rate | {:.1}% |",
            report.metrics.pass_rate * 100.0
        )
        .unwrap();
        writeln!(
            output,
            "| Correctness Rate | {:.1}% |",
            report.metrics.correctness.rate * 100.0
        )
        .unwrap();
        writeln!(
            output,
            "| Avg Latency (P50) | {:.0}ms |",
            report.metrics.latency_stats.median_ms
        )
        .unwrap();
        writeln!(
            output,
            "| Avg Latency (P95) | {:.0}ms |",
            report.metrics.latency_stats.p95_ms
        )
        .unwrap();
        writeln!(
            output,
            "| Total Cost | ${:.4} |",
            report.metrics.cost_stats.total_cost_usd
        )
        .unwrap();
        writeln!(
            output,
            "| Avg Cost/Case | ${:.4} |",
            report.metrics.cost_stats.avg_cost_per_case_usd
        )
        .unwrap();
        writeln!(output).unwrap();
        ...
        writeln!(
            output,
            "| Precision | {:.2} |",
            report.metrics.tool_usage.avg_precision
        )
        .unwrap();
        writeln!(
            output,
            "| Recall | {:.2} |",
            report.metrics.tool_usage.avg_recall
        )
        .unwrap();
        writeln!(
            output,
            "| F1 Score | {:.2} |",
            report.metrics.tool_usage.avg_f1_score
        )
        .unwrap();
        writeln!(output).unwrap();
        ...
        if !failed.is_empty() {
            ...
            for result in failed {
                writeln!(
                    output,
                    "### ❌ {} ({})",
                    result.test_case_name, result.test_case_id
                )
                .unwrap();
    ...
    fn generate_agent_eval_html(&self, report: &AgentEvalReport) -> String {
        let status_color = if report.all_passed() {
            "#22c55e"
        } else {
            "#ef4444"
        };
...
<body>
    ...
    fn generate_agent_eval_console(&self, report: &AgentEvalReport) -> String {
        ...
        writeln!(
            ...
            DIM, RESET, report.metrics.latency_stats.median_ms, report.metrics.latency_stats.p95_ms
        )
        .unwrap();
        writeln!(
            output,
            "  {}Total Cost:{}        ${:.4}",
            DIM, RESET, report.metrics.cost_stats.total_cost_usd
        )
        .unwrap();
        writeln!(
            output,
            "  {}Tool F1 Score:{}     {:.2}",
            DIM, RESET, report.metrics.tool_usage.avg_f1_score
        )
        .unwrap();
        writeln!(output).unwrap();
        ...
        if !failed.is_empty() {
            ...
            for result in failed.iter().take(5) {
                writeln!(
                    output,
                    "  {}{}✗{} {}",
                    RED, BOLD, RESET, result.test_case_name
                )
                .unwrap();
                if let Some(ref reason) = result.failure_reason {
...
mod tests {
    ...
    fn test_markdown_generation() {
        let results = vec![TestResult::passed(TestMeta::new("test_example"), 100)];
    ...
    fn test_json_generation() {
        let results = vec![TestResult::passed(TestMeta::new("test_json"), 50)];
    ...
    fn test_agent_eval_report_creation() {
        ...
        let results = vec![AgentEvalResult {
