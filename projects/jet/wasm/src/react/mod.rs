...
// CODEGEN-BEGIN
//! React-specific runtime: fiber tree + hooks + mount/flush commit loop.
//!
//! The framework-agnostic substrate of this crate lives at the
//! crate root (`Element`, `Props`, `Callback`, `Component`, and the
//! `renderer` module). This module adds React 18's fiber + hooks
//! flavour on top of that substrate. Other framework adapters
//! (`vue`, `angular`, `solid`) would live in sibling modules with
//! the same "generic primitives + framework-specific runtime"
//! pattern.
//!
//! Module body was lifted verbatim from the crate's earlier flat
//! layout (pre framework-agnostic split). Types and functions
//! reach back into the crate root via `crate::{Element, Props,
//! Callback, Component}`.

use crate::{Callback, Component, Element};
...
/// @spec .aw/tech-design/projects/jet/semantic/jet-wasm-src-react.md#schema
#[cfg(feature = "debug")]
pub(crate) struct DebugFiberMeta {
    pub id: u64,
...
/// @spec .aw/tech-design/projects/jet/semantic/jet-wasm-src-react.md#schema
#[cfg(feature = "debug")]
pub(crate) fn debug_snapshot_fibers() -> Vec<DebugFiberMeta> {
    RUNTIME.with(|r| {
        let rt = r.borrow();
        rt.fibers
            .iter()
...
/// @spec .aw/tech-design/projects/jet/semantic/jet-wasm-src-react.md#schema
#[cfg(feature = "debug")]
pub(crate) struct DebugHookSummary {
    pub kind: &'static str,
...
/// @spec .aw/tech-design/projects/jet/semantic/jet-wasm-src-react.md#schema
#[cfg(feature = "debug")]
pub(crate) fn debug_snapshot_hooks(fiber_id: u64) -> Vec<DebugHookSummary> {
    RUNTIME.with(|r| {
        let rt = r.borrow();
