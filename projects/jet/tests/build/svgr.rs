...
fn attribute_conversion_kebab_to_camel() {
    let svg =
        r#"<svg><path stroke-width="2" clip-rule="evenodd" fill-opacity="0.5" d="M0 0"/></svg>"#;
    let module = transform_svg_to_component(svg, SvgrExportType::Named).unwrap();
...
fn attribute_conversion_style_string_to_object() {
    ...
    let module = transform_svg_to_component(svg, SvgrExportType::Named).unwrap();
    assert!(
        module.contains("style={{"),
        "style must be an object literal:\n{module}"
    );
...
fn nested_elements_and_groups_are_rendered() {
    let svg = r#"<svg><g fill="none"><path d="M1 1"/><circle cx="1" cy="2" r="3"/></g></svg>"#;
    let module = transform_svg_to_component(svg, SvgrExportType::Named).unwrap();
