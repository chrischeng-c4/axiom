    ...
    json!({
        ...
        "shapes": [
            { "name": "set", "description": "store a scalar/JSON value (optional TTL in ms)",
              "request": { "method": "PUT", "path": "/kv/{key}", "body": { "value": "hello", "ttl_ms": 60000 } } },
            { "name": "get", "description": "read a value",
              "request": { "method": "GET", "path": "/kv/{key}" } },
            { "name": "delete", "description": "remove a key",
              "request": { "method": "DELETE", "path": "/kv/{key}" } },
            { "name": "incr", "description": "atomic integer add (negative delta decrements)",
              "request": { "method": "POST", "path": "/kv/{key}/incr", "body": { "delta": 1 } } },
            { "name": "cas", "description": "compare-and-swap: write `new` iff the current value equals `expected`",
              "request": { "method": "POST", "path": "/kv/{key}/cas", "body": { "expected": "old", "new": "new" } } },
            { "name": "setnx", "description": "set only if the key is absent",
              "request": { "method": "POST", "path": "/kv/{key}/setnx", "body": { "value": "once" } } },
            { "name": "mset", "description": "batch set",
              "request": { "method": "POST", "path": "/kv:mset", "body": { "entries": { "a": 1, "b": 2 } } } },
            { "name": "mget", "description": "batch get (parallel to keys; null where absent)",
              "request": { "method": "POST", "path": "/kv:mget", "body": { "keys": ["a", "b"] } } },
            { "name": "mdel", "description": "batch delete",
              "request": { "method": "POST", "path": "/kv:mdel", "body": { "keys": ["a", "b"] } } },
            { "name": "scan", "description": "prefix scan (keys only)",
              "request": { "method": "GET", "path": "/kv?prefix=user:&limit=100" } },
            { "name": "expire", "description": "set a TTL on any existing key (seconds or ms)",
              "request": { "method": "POST", "path": "/kv/{key}/expire", "body": { "ms": 60000 } } },
            { "name": "lock", "description": "acquire an owner+TTL advisory lease",
              "request": { "method": "POST", "path": "/locks/{name}", "body": { "owner": "worker-1", "ttl_ms": 30000 } } },
            { "name": "lpush", "description": "prepend values to a list",
              "request": { "method": "POST", "path": "/lists/{key}/lpush", "body": { "values": ["a", "b"] } } },
            { "name": "hset", "description": "write hash fields",
              "request": { "method": "POST", "path": "/hashes/{key}", "body": { "fields": { "name": "ada" } } } },
            { "name": "sadd", "description": "add set members",
              "request": { "method": "POST", "path": "/sets/{key}", "body": { "members": ["x", "y"] } } },
            { "name": "zadd", "description": "add scored members to a sorted set",
              "request": { "method": "POST", "path": "/zsets/{key}", "body": { "members": [ { "member": "alice", "score": 10.0 } ] } } },
            { "name": "claim_check_input", "description": "worker job input payload by message id (bytes-first)",
              "request": { "method": "PUT", "path": "/inputs/{id}", "body": "<application/octet-stream bytes>" } },
            { "name": "claim_check_result", "description": "worker job result payload by message id (bytes-first)",
              "request": { "method": "PUT", "path": "/results/{id}", "body": "<application/octet-stream bytes>" } }
        ]
    })
}
...
pub fn value_catalog() -> Value {
    json!({
        "note": "Scalars are set/read on /kv/{key}; aggregates use their typed collection endpoints. JSON integers map to int, fractional numbers to float, and bool to int.",
        "value_types": [
            ...
            { "type": "list", "purpose": "ordered list", "endpoint": "/lists/{key}" },
            { "type": "map", "purpose": "field -> value hash", "endpoint": "/hashes/{key}" },
            { "type": "set", "purpose": "unordered unique strings", "endpoint": "/sets/{key}" },
            { "type": "sorted_set", "purpose": "scored members in score order", "endpoint": "/zsets/{key}" },
