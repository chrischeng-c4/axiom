//! Schema gate for the mambalibs fetch import gate fixture —
//! closes #2844.
...
const LIBRARY: &str = "fetch";
const ISSUE: i64 = 2844;
const FIXTURE: &str = "mambalibs_fetch_import_gate";
const STATUS_FIELD: &str = "fetch_import_status";
const SUPPORTED_CASE: &str = "fetch_import_supported";
const BLOCKED_CASE: &str = "fetch_import_blocked";
const OOS_FLAG: &str = "http_behavior_coverage";
...
#[test]
fn network_policy_forbids_external_io() {
    let doc = load_toml(&manifest_path());
    let block = doc.get("network_policy").and_then(|v| v.as_table()).expect(
        "[network_policy] missing — acceptance: \
         \"Test performs no external network I/O.\"",
    );
    for f in &[
        "must_not_open_tcp_connection",
        "must_not_perform_dns_lookup",
        "must_not_emit_http_request",
        "must_not_read_proxy_env_vars",
        "must_run_offline",
    ] {
