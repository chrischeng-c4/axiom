...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("mamba_build_e2e_harness"));
...
fn fixture_project_targets_the_local_binding_crate() {
    let doc = load_toml(&manifest_path());
    let f = doc.get("fixture_project").and_then(|v| v.as_table()).expect("[fixture_project] missing");
    assert_eq!(
        f.get("local_binding_crate_fixture_issue").and_then(|v| v.as_integer()),
        Some(2577),
        "must cross-reference #2577",
    );
    assert_eq!(
        f.get("mamba_toml_fixture_issue").and_then(|v| v.as_integer()),
        Some(2575),
        "must cross-reference #2575",
    );
    assert!(f.get("relative_path").and_then(|v| v.as_str()).is_some());
    assert_eq!(f.get("must_be_offline").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(f.get("must_be_deterministic").and_then(|v| v.as_bool()), Some(true));
...
fn build_command_is_the_smallest_e2e_command_and_captures_everything() {
    let doc = load_toml(&manifest_path());
    let b = doc.get("build_command").and_then(|v| v.as_table()).expect("[build_command] missing");
...
fn missing_build_artifact_fails_the_test() {
    ...
    assert_eq!(c.get("must_assert_artifact_path_is_recorded").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("must_assert_artifact_path_exists_on_disk").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("expected_outcome_when_artifact_present").and_then(|v| v.as_str()), Some("pass"));
    assert_eq!(c.get("expected_outcome_when_artifact_missing").and_then(|v| v.as_str()), Some("fail"));
    assert_eq!(c.get("artifact_missing_failure_kind").and_then(|v| v.as_str()), Some("artifact_missing"));
    assert_eq!(c.get("artifact_missing_exit_code").and_then(|v| v.as_integer()), Some(4));
    assert_eq!(c.get("artifact_missing_diagnostic_must_name_path").and_then(|v| v.as_bool()), Some(true));
}
...
fn nonzero_build_exit_fails_with_logs() {
    let doc = load_toml(&manifest_path());
    let c = doc.get("nonzero_exit_case").and_then(|v| v.as_table()).expect(
        "[nonzero_exit_case] missing — acceptance: \
         \"Nonzero build exit fails the test with logs.\"",
    );
    assert_eq!(c.get("must_assert_clean_exit_code").and_then(|v| v.as_integer()), Some(0));
    assert_eq!(c.get("expected_outcome_when_build_exits_clean").and_then(|v| v.as_str()), Some("pass"));
    assert_eq!(c.get("expected_outcome_when_build_exits_nonzero").and_then(|v| v.as_str()), Some("fail"));
    assert_eq!(c.get("nonzero_exit_failure_kind").and_then(|v| v.as_str()), Some("nonzero_build_exit"));
    assert_eq!(c.get("nonzero_exit_must_attach_captured_stdout").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("nonzero_exit_must_attach_captured_stderr").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("nonzero_exit_diagnostic_must_name_command").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("nonzero_exit_exit_code").and_then(|v| v.as_integer()), Some(5));
    assert_eq!(c.get("must_not_drop_logs").and_then(|v| v.as_bool()), Some(true));
}
...
fn isolated_from_user_level_mamba_caches() {
    let doc = load_toml(&manifest_path());
    let u = doc.get("user_cache_isolation").and_then(|v| v.as_table()).expect(
    ...
    for f in &[
        ...
        assert_eq!(u.get(*f).and_then(|v| v.as_bool()), Some(true), "{f} must be true");
    }
    let forbidden: Vec<&str> = u.get("forbidden_env_vars").and_then(|v| v.as_array())
...
fn runner_contract_declares_keys_and_cases() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for required in &[
        "outcome", "case", "program", "subcommand",
        "stdout_captured", "stderr_captured", "exit_status",
        "artifact_path", "failure_kind", "exit_code",
    ] {
    ...
    let cases: Vec<&str> = c.get("case_values").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for required in &["build_produces_artifact", "build_exits_clean_or_logs_failure"] {
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("publishing_or_global_install").and_then(|v| v.as_bool()), Some(true));
}
