//! Schema gate for the mambalibs array import gate fixture —
//! closes #2842.
...
const LIBRARY: &str = "array";
const ISSUE: i64 = 2842;
const FIXTURE: &str = "mambalibs_array_import_gate";
const FAMILY: &str = "array_import_gate";
const STATUS_FIELD: &str = "array_import_status";
const SUPPORTED_CASE: &str = "array_import_supported";
const BLOCKED_CASE: &str = "array_import_blocked";
const OOS_FLAG: &str = "numeric_kernel_correctness";
...
fn sample_policy_pins_tiny_in_memory() {
    ...
    let block = doc.get("sample_policy").and_then(|v| v.as_table()).expect(
        "[sample_policy] missing — acceptance: \
         \"Test uses a tiny in-memory sample only if needed.\"",
    );
    for f in &[
        "must_be_tiny",
        "must_be_in_memory",
