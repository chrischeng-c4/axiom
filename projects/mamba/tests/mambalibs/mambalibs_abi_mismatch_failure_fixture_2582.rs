...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("abi_mismatch_failure"));
...
fn binding_supplies_small_non_platform_specific_fake_artifact() {
    ...
    assert_eq!(b.get("must_supply_fake_artifact").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(b.get("fake_artifact_must_be_small").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(b.get("fake_artifact_must_not_be_platform_specific").and_then(|v| v.as_bool()), Some(true));
}

#[test]
fn abi_versions_must_differ() {
    let doc = load_toml(&manifest_path());
    let a = doc.get("abi").and_then(|v| v.as_table()).expect("[abi] missing");
    let exp = a.get("expected_abi_version").and_then(|v| v.as_str()).unwrap();
    let found = a.get("found_abi_version").and_then(|v| v.as_str()).unwrap();
    assert_ne!(exp, found, "expected_abi_version must differ from found_abi_version");
    let exp_schema = a.get("expected_schema_version").and_then(|v| v.as_integer()).unwrap();
    let found_schema = a.get("found_schema_version").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exp_schema, found_schema, "expected_schema_version must differ from found_schema_version");
    assert_eq!(a.get("versions_must_differ").and_then(|v| v.as_bool()), Some(true));
}
...
fn failure_message_names_expected_and_found_abi_version() {
    ...
    assert_eq!(e.get("exception_message_must_name_expected_version").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(e.get("exception_message_must_name_found_version").and_then(|v| v.as_bool()), Some(true));
    let exp_substr = e.get("expected_version_substring").and_then(|v| v.as_str()).unwrap();
    let found_substr = e.get("found_version_substring").and_then(|v| v.as_str()).unwrap();
    let abi_exp = doc.get("abi").and_then(|v| v.get("expected_abi_version")).and_then(|v| v.as_str()).unwrap();
    let abi_found = doc.get("abi").and_then(|v| v.get("found_abi_version")).and_then(|v| v.as_str()).unwrap();
    assert_eq!(exp_substr, abi_exp, "expected_version_substring must match [abi].expected_abi_version");
    assert_eq!(found_substr, abi_found, "found_version_substring must match [abi].found_abi_version");
    assert_ne!(exp_substr, found_substr, "expected and found substrings must differ");
    assert_eq!(e.get("exception_message_must_distinguish_expected_from_found").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(e.get("must_be_deterministic").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(e.get("abi_mismatch_failure_kind").and_then(|v| v.as_str()), Some("abi_mismatch"));
    let exit = e.get("abi_mismatch_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 9);
}
...
fn positive_mode_2_fixture_remains_unaffected() {
    ...
    assert_eq!(p.get("this_fixture_must_not_register_globally").and_then(|v| v.as_bool()), Some(true));
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        "outcome", "case", "module",
        "expected_abi_version", "found_abi_version",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("designing_final_abi_compatibility_policy").and_then(|v| v.as_bool()), Some(true));
}
