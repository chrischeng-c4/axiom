...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("multiple_mambalibs_import")
    );
...
fn binding_runs_in_single_interpreter_and_is_offline_deterministic() {
    ...
    assert_eq!(
        b.get("must_run_in_single_interpreter")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        b.get("must_be_deterministic").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        b.get("must_be_offline").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
...
fn modules_declare_two_distinct_fixtures_with_distinct_values() {
    ...
    let names: Vec<&str> = mods
        ...
        .filter_map(|m| {
            m.as_table()
                .and_then(|t| t.get("fixture_module"))
                .and_then(|v| v.as_str())
        })
        .collect();
    ...
    let funcs: Vec<&str> = mods
        .iter()
        .filter_map(|m| {
            m.as_table()
                .and_then(|t| t.get("exported_function"))
                .and_then(|v| v.as_str())
        })
        .collect();
    ...
    let values: Vec<i64> = mods
        .iter()
        .filter_map(|m| {
            m.as_table()
                .and_then(|t| t.get("expected_return_value"))
                .and_then(|v| v.as_integer())
        })
        .collect();
    ...
    assert_ne!(
        values[0], values[1],
        "expected_return_value must differ to prove no collision"
    );
...
fn both_imports_succeed_in_single_interpreter() {
    ...
    assert_eq!(
        c.get("must_import_both_in_single_interpreter")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_assert_each_exported_function_returns_own_value")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("expected_outcome_when_both_imports_succeed")
            .and_then(|v| v.as_str()),
        Some("pass")
    );
    assert_eq!(
        c.get("expected_outcome_when_either_import_fails")
            .and_then(|v| v.as_str()),
        Some("fail")
    );
    assert_eq!(
        c.get("both_imports_failure_kind").and_then(|v| v.as_str()),
        Some("import_failure")
    );
    assert_eq!(
        c.get("both_imports_failure_exit_code")
            .and_then(|v| v.as_integer()),
        Some(2)
    );
}
...
fn namespace_collision_fails_with_clear_test_failure() {
    ...
    assert_eq!(
        c.get("must_detect_namespace_collision")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_emit_clear_failure_diagnostic")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("collision_failure_kind").and_then(|v| v.as_str()),
        Some("namespace_collision")
    );
    let exit = c
        .get("collision_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(exit, 0);
    assert_ne!(
        exit, 2,
        "collision exit code must differ from import_failure to distinguish failure modes"
    );
    assert_eq!(
        c.get("collision_diagnostic_must_name_colliding_modules")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("collision_diagnostic_must_distinguish_from_import_failure")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_not_silently_overwrite_module_binding")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("expected_outcome_when_collision_detected")
            .and_then(|v| v.as_str()),
        Some("fail")
    );
}
...
fn fixture_stays_offline_and_deterministic() {
    let doc = load_toml(&manifest_path());
    let i = doc
        .get("offline_and_deterministic_invariant")
        .and_then(|v| v.as_table())
        .expect(
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "returned_value_a",
        "returned_value_b",
        "expected_returned_value_a",
        "expected_returned_value_b",
        "failure_kind",
        "exit_code",
    ] {
        assert!(
    ...
    let cases: Vec<&str> = c
        ...
        .unwrap_or_default();
    for required in &[
        "both_modules_import_in_one_run",
        "namespace_collision_is_clear_failure",
    ] {
        assert!(
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("testing_all_cclab_libraries")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
