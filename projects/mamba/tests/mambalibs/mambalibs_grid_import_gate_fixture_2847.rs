//! Schema gate for the mambalibs grid import gate fixture — closes
//! #2847.
...
const LIBRARY: &str = "grid";
const ISSUE: i64 = 2847;
const FIXTURE: &str = "mambalibs_grid_import_gate";
const STATUS_FIELD: &str = "grid_import_status";
const SUPPORTED_CASE: &str = "grid_import_supported";
const BLOCKED_CASE: &str = "grid_import_blocked";
const OOS_FLAG: &str = "grid_rendering_or_wasm_coverage";
...
#[test]
fn browser_policy_forbids_browser_and_wasm() {
    let doc = load_toml(&manifest_path());
    let block = doc.get("browser_policy").and_then(|v| v.as_table()).expect(
        "[browser_policy] missing — acceptance: \
         \"Test avoids browser or WASM build requirements unless \
         explicitly isolated.\"",
    );
    for f in &[
        "must_not_require_browser_runtime",
        "must_not_require_wasm_build",
        "must_not_launch_headless_browser",
        "must_not_invoke_wasm_pack",
        "must_run_in_native_python",
    ] {
        assert_eq!(block.get(*f).and_then(|v| v.as_bool()), Some(true), "{f}");
    }
    let forbidden: Vec<&str> = block.get("forbidden_env_vars").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for v in &["BROWSER", "WASM_PACK"] {
