//! Schema gate for the mambalibs learn import gate fixture — closes
//! #2848.
...
const LIBRARY: &str = "learn";
const ISSUE: i64 = 2848;
const FIXTURE: &str = "mambalibs_learn_import_gate";
const STATUS_FIELD: &str = "learn_import_status";
const SUPPORTED_CASE: &str = "learn_import_supported";
const BLOCKED_CASE: &str = "learn_import_blocked";
const OOS_FLAG: &str = "ml_algorithm_coverage";
...
#[test]
fn dataset_policy_forbids_models_and_datasets() {
    let doc = load_toml(&manifest_path());
    let block = doc.get("dataset_policy").and_then(|v| v.as_table()).expect(
        "[dataset_policy] missing — acceptance: \
         \"Test avoids large model or dataset dependencies.\"",
    );
    for f in &[
        "must_not_download_model",
        "must_not_download_dataset",
        "must_not_load_pretrained_weights",
        "must_run_offline",
    ] {
        assert_eq!(block.get(*f).and_then(|v| v.as_bool()), Some(true), "{f}");
    }
    assert_eq!(block.get("max_model_parameters").and_then(|v| v.as_integer()), Some(0));
    let forbidden: Vec<&str> = block.get("forbidden_env_vars").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for v in &["HF_HOME", "TRANSFORMERS_CACHE", "TORCH_HOME"] {
