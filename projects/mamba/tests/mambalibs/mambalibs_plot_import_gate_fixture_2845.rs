//! Schema gate for the mambalibs plot import gate fixture — closes
//! #2845.
...
const LIBRARY: &str = "plot";
const ISSUE: i64 = 2845;
const FIXTURE: &str = "mambalibs_plot_import_gate";
const STATUS_FIELD: &str = "plot_import_status";
const SUPPORTED_CASE: &str = "plot_import_supported";
const BLOCKED_CASE: &str = "plot_import_blocked";
const OOS_FLAG: &str = "rendering_correctness";
...
#[test]
fn gui_policy_forbids_display_and_window_server() {
    let doc = load_toml(&manifest_path());
    let block = doc.get("gui_policy").and_then(|v| v.as_table()).expect(
        "[gui_policy] missing — acceptance: \
         \"Test does not require GUI or display services.\"",
    );
    for f in &[
        "must_not_open_display",
        "must_not_require_window_server",
        "must_not_initialize_gui_backend",
        "must_not_read_display_env_vars",
        "must_run_headless",
    ] {
