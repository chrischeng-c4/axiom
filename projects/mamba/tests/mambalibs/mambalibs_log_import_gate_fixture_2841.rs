...
fn binding_uses_documented_surface() {
    ...
    assert!(!bind.get("minimal_exported_symbol").and_then(|v| v.as_str()).unwrap().is_empty());
}
...
fn side_effect_policy_blocks_global_logger_mutation() {
    let doc = load_toml(&manifest_path());
    let block = doc.get("side_effect_policy").and_then(|v| v.as_table()).expect(
    ...
    assert!(block.get("rationale").and_then(|v| v.as_str()).map(|s| !s.is_empty()).unwrap_or(false));
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get(OOS_FLAG).and_then(|v| v.as_bool()), Some(true), "{OOS_FLAG}");
}
