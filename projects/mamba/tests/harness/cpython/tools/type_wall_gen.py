...
Create or verify the pinned typeshed checkout first with:
...
def verify_typeshed_corpus() -> TypeshedLock:
    """Verify the active generator input before any denominator is consumed."""
    return verify_typeshed_stdlib(TYPESHED_STDLIB)
...
def render_specs_json(lock: TypeshedLock) -> str:
...
def render_specs_rust(specs_json: str) -> str:
    ...
    typeshed = manifest["provenance"]["typeshed"]
    return (
        ...
        "//!\n"
        f"//! typeshed-repository: {typeshed['repository']}\n"
        f"//! typeshed-revision: {typeshed['revision']}\n"
        f"//! typeshed-stdlib-pyi-count: {typeshed['stdlib_pyi_count']}\n"
        f"//! typeshed-stdlib-pyi-sha256: {typeshed['stdlib_pyi_sha256']}\n"
        "//!\n"
...
def render_rust(lock: TypeshedLock) -> str:
    ...
    lines.append(
        ...
        "//!   python3.12 tests/harness/cpython/tools/type_wall_gen.py --emit-rust\n"
        "//!\n"
        f"//! typeshed-repository: {lock.repository}\n"
        f"//! typeshed-revision: {lock.revision}\n"
        f"//! typeshed-stdlib-pyi-count: {lock.stdlib_pyi_count}\n"
        f"//! typeshed-stdlib-pyi-sha256: {lock.stdlib_pyi_sha256}\n"
        "//!\n"
...
def emit_rust(check: bool, lock: TypeshedLock) -> int:
    text = render_rust(lock)
    specs_data = render_specs_json(lock)
...
def main() -> int:
    ...
    try:
        typeshed_lock = verify_typeshed_corpus()
    except TypeshedLockError as error:
        print(f"typeshed lock verification failed: {error}", file=sys.stderr)
        return 2

    # Additive Rust-table modes. Disjoint from fixture generation above.
    if args.check_rust:
        return emit_rust(check=True, lock=typeshed_lock)
    if getattr(args, "emit_rust", False):
        return emit_rust(check=False, lock=typeshed_lock)
