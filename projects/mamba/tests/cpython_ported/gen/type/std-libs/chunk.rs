use super::super::super::super::harness::*;

/// Ported from `tests/cpython/type/std-libs/chunk/Chunk__init__file_as_IO_wrong.py`.
#[test]
fn test_gen_type_std_libs_chunk_Chunk__init__file_as_IO_wrong() {
    let out = run_type_wall_fixture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "chunk"
# dimension = "type"
# case = "Chunk__init__file_as_IO_wrong"
# subject = "chunk.Chunk.__init__(file: IO)"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "vendor/typeshed/stdlib/chunk.pyi"
# status = "filled"
# ///
# mamba-strict-type: TypeError
"""Type wall: chunk.Chunk.__init__(file: IO); call it with the wrong type.

typeshed contract: file is IO. mamba is force-typed, so a wrong-typed
argument MUST raise TypeError (CPython may accept or raise — mamba's to enforce)."""

class _W:
    pass


from chunk import Chunk
try:
    Chunk(_W())  # file: IO <- wrong-typed
    print("no_typeerror:")  # CPython accepted the wrong-typed arg; mamba must raise
except TypeError as e:
    print("typeerror:", type(e).__name__)
except Exception as e:
    print("setup_or_other:", type(e).__name__)
"###);
    assert!(out == "STRICT_TYPE_REJECTED" || out.starts_with("RUNTIME_REJECTED"),
        "type wall did not hold: {out}");
}

/// Ported from `tests/cpython/type/std-libs/chunk/Chunk__read__size_as_int_wrong.py`.
#[test]
fn test_gen_type_std_libs_chunk_Chunk__read__size_as_int_wrong() {
    let out = run_type_wall_fixture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "chunk"
# dimension = "type"
# case = "Chunk__read__size_as_int_wrong"
# subject = "chunk.Chunk.read(size: int)"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "vendor/typeshed/stdlib/chunk.pyi"
# status = "filled"
# ///
# mamba-strict-type: TypeError
"""Type wall: chunk.Chunk.read(size: int); call it with the wrong type.

typeshed contract: size is int. mamba is force-typed, so a wrong-typed
argument MUST raise TypeError (CPython may accept or raise — mamba's to enforce)."""

from chunk import Chunk
obj = object.__new__(Chunk)
try:
    obj.read("not_an_int")  # size: int <- wrong-typed
    print("no_typeerror:")  # CPython accepted the wrong-typed arg; mamba must raise
except TypeError as e:
    print("typeerror:", type(e).__name__)
except Exception as e:
    print("setup_or_other:", type(e).__name__)
"###);
    assert!(out == "STRICT_TYPE_REJECTED" || out.starts_with("RUNTIME_REJECTED"),
        "type wall did not hold: {out}");
}

/// Ported from `tests/cpython/type/std-libs/chunk/Chunk__seek__pos_as_int_wrong.py`.
#[test]
fn test_gen_type_std_libs_chunk_Chunk__seek__pos_as_int_wrong() {
    let out = run_type_wall_fixture(r###"# /// script
# requires-python = ">=3.12"
# dependencies = []
#
# [tool.mamba]
# bucket = "std-libs"
# lib = "chunk"
# dimension = "type"
# case = "Chunk__seek__pos_as_int_wrong"
# subject = "chunk.Chunk.seek(pos: int)"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
# source = "vendor/typeshed/stdlib/chunk.pyi"
# status = "filled"
# ///
# mamba-strict-type: TypeError
"""Type wall: chunk.Chunk.seek(pos: int); call it with the wrong type.

typeshed contract: pos is int. mamba is force-typed, so a wrong-typed
argument MUST raise TypeError (CPython may accept or raise — mamba's to enforce)."""

from chunk import Chunk
obj = object.__new__(Chunk)
try:
    obj.seek("not_an_int")  # pos: int <- wrong-typed
    print("no_typeerror:")  # CPython accepted the wrong-typed arg; mamba must raise
except TypeError as e:
    print("typeerror:", type(e).__name__)
except Exception as e:
    print("setup_or_other:", type(e).__name__)
"###);
    assert!(out == "STRICT_TYPE_REJECTED" || out.starts_with("RUNTIME_REJECTED"),
        "type wall did not hold: {out}");
}
