...
# lib = "builtins"
# dimension = "behavior"
# case = "builtin_test__test_zip_strict_error_handling"
# subject = "cpython.test_builtin.BuiltinTest.test_zip_strict_error_handling"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_builtin.py::BuiltinTest::test_zip_strict_error_handling
"""Auto-ported test: BuiltinTest::test_zip_strict_error_handling (CPython 3.12 oracle)."""
...
class Iter:
    ...
    def __next__(self):
        ...
        if self.size < 0:
            raise Error
        return self.size
l1 = iter_error(zip('AB', Iter(1), strict=True), Error)

assert l1 == [('A', 0)]
l2 = iter_error(zip('AB', Iter(2), 'A', strict=True), ValueError)

assert l2 == [('A', 1, 'A')]
l3 = iter_error(zip('AB', Iter(2), 'ABC', strict=True), Error)

assert l3 == [('A', 1, 'A'), ('B', 0, 'B')]
l4 = iter_error(zip('AB', Iter(3), strict=True), ValueError)

assert l4 == [('A', 2), ('B', 1)]
l5 = iter_error(zip(Iter(1), 'AB', strict=True), Error)

assert l5 == [(0, 'A')]
l6 = iter_error(zip(Iter(2), 'A', strict=True), ValueError)

assert l6 == [(1, 'A')]
l7 = iter_error(zip(Iter(2), 'ABC', strict=True), Error)

assert l7 == [(1, 'A'), (0, 'B')]
l8 = iter_error(zip(Iter(3), 'AB', strict=True), ValueError)

assert l8 == [(2, 'A'), (1, 'B')]
print("BuiltinTest::test_zip_strict_error_handling: ok")
