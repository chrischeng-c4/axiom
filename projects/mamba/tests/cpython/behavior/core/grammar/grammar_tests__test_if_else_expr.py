...
# lib = "grammar"
# dimension = "behavior"
# case = "grammar_tests__test_if_else_expr"
# subject = "cpython.test_grammar.GrammarTests.test_if_else_expr"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_grammar.py::GrammarTests::test_if_else_expr
"""Auto-ported test: GrammarTests::test_if_else_expr (CPython 3.12 oracle)."""
...
assert [x(False) for x in (lambda x: False if x else True, lambda x: True if x else False) if x(False)] == [True]
...
assert (1 or _checkeval('check 4', 2) if 1 else _checkeval('check 5', 3)) == 1

assert (0 or 5 if 1 else _checkeval('check 6', 3)) == 5

assert (not 5 if 1 else 1) == False
...
print("GrammarTests::test_if_else_expr: ok")
