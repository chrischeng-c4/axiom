...
# lib = "io"
# dimension = "behavior"
# case = "c_text_io_wrapper_test__test_create_at_shutdown_without_encoding"
# subject = "cpython.test_io.CTextIOWrapperTest.test_create_at_shutdown_without_encoding"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_io.py::CTextIOWrapperTest::test_create_at_shutdown_without_encoding
"""Auto-ported test: CTextIOWrapperTest::test_create_at_shutdown_without_encoding (CPython 3.12 oracle)."""
...
def _check_create_at_shutdown(**kwargs):
    iomod = io.__name__
    code = 'if 1:\n            import codecs\n            import {iomod} as io\n\n            # Avoid looking up codecs at shutdown\n            codecs.lookup(\'utf-8\')\n\n            class C:\n                def __init__(self):\n                    self.buf = io.BytesIO()\n                def __del__(self):\n                    io.TextIOWrapper(self.buf, **{kwargs})\n                    print("ok")\n            c = C()\n            '.format(iomod=iomod, kwargs=kwargs)
    return assert_python_ok('-c', code)

def multi_line_test(f, enc):
    f.seek(0)
    f.truncate()
    sample = 'sÿ\u0fff\uffff'
    wlines = []
    ...
    assert rlines == wlines
self_testdata = b'AAA\r\nBBB\rCCC\r\nDDD\nEEE\r\n'
self_normalized = b'AAA\nBBB\nCCC\nDDD\nEEE\n'.decode('ascii')
os_helper.unlink(os_helper.TESTFN)
codecs.register(lookupTestDecoder)
pass
rc, out, err = _check_create_at_shutdown()
if err:

    assert shutdown_error in err.decode()
else:

    assert 'ok' == out.decode().strip()
print("CTextIOWrapperTest::test_create_at_shutdown_without_encoding: ok")
