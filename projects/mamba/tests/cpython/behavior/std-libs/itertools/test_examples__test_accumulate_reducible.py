...
# lib = "itertools"
# dimension = "behavior"
# case = "test_examples__test_accumulate_reducible"
# subject = "cpython.test_itertools.TestExamples.test_accumulate_reducible"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestExamples::test_accumulate_reducible
"""Auto-ported test: TestExamples::test_accumulate_reducible (CPython 3.12 oracle)."""
...
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    ...
    assert list(pickle.loads(pickle.dumps(it, proto))) == accumulated[:]

    assert next(it) == 1

    assert list(pickle.loads(pickle.dumps(it, proto))) == accumulated[1:]
it = accumulate(data)

assert next(it) == 1

assert list(copy.deepcopy(it)) == accumulated[1:]

assert list(copy.copy(it)) == accumulated[1:]
print("TestExamples::test_accumulate_reducible: ok")
