...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_filter"
# subject = "cpython.test_itertools.TestBasicOps.test_filter"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_filter
"""Auto-ported test: TestBasicOps::test_filter (CPython 3.12 oracle)."""
...
assert list(filter(None, [0, 1, 0, 2, 0])) == [1, 2]

assert list(filter(bool, [0, 1, 0, 2, 0])) == [1, 2]
...
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    ...
    assert list(pickle.loads(pickle.dumps(c, proto))) == ans
    next(c)

    assert list(pickle.loads(pickle.dumps(c, proto))) == ans[1:]
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    c = filter(isEven, range(6))
    pickletest(proto, c)
print("TestBasicOps::test_filter: ok")
