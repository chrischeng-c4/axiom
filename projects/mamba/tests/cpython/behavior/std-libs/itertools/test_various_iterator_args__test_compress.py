...
# lib = "itertools"
# dimension = "behavior"
# case = "test_various_iterator_args__test_compress"
# subject = "cpython.test_itertools.TestVariousIteratorArgs.test_compress"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestVariousIteratorArgs::test_compress
"""Auto-ported test: TestVariousIteratorArgs::test_compress (CPython 3.12 oracle)."""
...
for s in ('123', '', range(1000), ('do', 1.2), range(2000, 2200, 5)):
    ...
    for g in (G, I, Ig, S, L, R):
        ...
        assert list(compress(g(s), repeat(1))) == list(g(s))

    try:
        compress(X(s), repeat(1))
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        compress(N(s), repeat(1))
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        list(compress(E(s), repeat(1)))
        raise AssertionError('expected ZeroDivisionError')
    except ZeroDivisionError:
        pass
print("TestVariousIteratorArgs::test_compress: ok")
