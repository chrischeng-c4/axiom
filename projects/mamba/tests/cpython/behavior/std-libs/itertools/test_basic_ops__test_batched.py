...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_batched"
# subject = "cpython.test_itertools.TestBasicOps.test_batched"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_batched
"""Auto-ported test: TestBasicOps::test_batched (CPython 3.12 oracle)."""
...
assert list(batched('ABCDEFG', 2)) == [('A', 'B'), ('C', 'D'), ('E', 'F'), ('G',)]

assert list(batched('ABCDEFG', 1)) == [('A',), ('B',), ('C',), ('D',), ('E',), ('F',), ('G',)]
try:
...
for n in range(1, 6):
    for i in range(len(data)):
        ...
        assert ''.join(chain(*batches)) == s

        assert all((type(batch) is tuple for batch in batches))
        if batches:
            last_batch = batches.pop()

            assert all((len(batch) == n for batch in batches))

            assert len(last_batch) <= n
            batches.append(last_batch)
print("TestBasicOps::test_batched: ok")
