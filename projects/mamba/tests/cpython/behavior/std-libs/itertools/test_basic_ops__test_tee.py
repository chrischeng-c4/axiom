...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_tee"
# subject = "cpython.test_itertools.TestBasicOps.test_tee"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_tee
"""Auto-ported test: TestBasicOps::test_tee (CPython 3.12 oracle)."""
...
assert list(copy.deepcopy(a)) == ans

assert list(copy.deepcopy(b)) == ans
...
assert list(copy.deepcopy(a)) == long_ans

assert list(copy.deepcopy(b)) == long_ans

assert list(a) == long_ans

assert list(b) == long_ans
a, b = tee('abc')
take(2, a)
take(1, b)

assert list(copy.deepcopy(a)) == ans[2:]

assert list(copy.deepcopy(b)) == ans[1:]
...
assert list(copy.deepcopy(a)) == long_ans[100:]

assert list(copy.deepcopy(b)) == long_ans[60:]

assert list(a) == long_ans[100:]

assert list(b) == long_ans[60:]
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    pickletest(proto, iter(tee('abc')))
    a, b = tee('abc')
    pickletest(proto, a, compare=ans)
    pickletest(proto, b, compare=ans)
print("TestBasicOps::test_tee: ok")
