...
# lib = "posix"
# dimension = "behavior"
# case = "test_posix_weaklinking__test_access"
# subject = "cpython.test_posix.TestPosixWeaklinking.test_access"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_posix.py::TestPosixWeaklinking::test_access
"""Auto-ported test: TestPosixWeaklinking::test_access (CPython 3.12 oracle)."""
...
_verify_available('HAVE_FACCESSAT')
if self_mac_ver >= (10, 10):

    assert 'HAVE_FACCESSAT' in posix._have_functions
else:

    assert 'HAVE_FACCESSAT' not in posix._have_functions
    try:
    ...
    try:
        ...
        import re as _re_aR
        assert _re_aR.search('follow_symlinks unavailable', str(_aR_e))
    try:
        os.access('file', os.R_OK, effective_ids=True)
        raise AssertionError('expected NotImplementedError')
    except NotImplementedError as _aR_e:
        import re as _re_aR
        assert _re_aR.search('effective_ids unavailable', str(_aR_e))
print("TestPosixWeaklinking::test_access: ok")
