...
# lib = "posix"
# dimension = "behavior"
# case = "test_posix_weaklinking__test_rename_replace"
# subject = "cpython.test_posix.TestPosixWeaklinking.test_rename_replace"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_posix.py::TestPosixWeaklinking::test_rename_replace
"""Auto-ported test: TestPosixWeaklinking::test_rename_replace (CPython 3.12 oracle)."""
...
_verify_available('HAVE_RENAMEAT')
if self_mac_ver >= (10, 10):

    assert 'HAVE_RENAMEAT' in posix._have_functions
else:

    assert 'HAVE_RENAMEAT' not in posix._have_functions
    try:
        os.rename('a', 'b', src_dir_fd=0)
        raise AssertionError('expected NotImplementedError')
    except NotImplementedError as _aR_e:
        import re as _re_aR
        assert _re_aR.search('src_dir_fd and dst_dir_fd unavailable', str(_aR_e))
    try:
    ...
    try:
        ...
        import re as _re_aR
        assert _re_aR.search('src_dir_fd and dst_dir_fd unavailable', str(_aR_e))
print("TestPosixWeaklinking::test_rename_replace: ok")
