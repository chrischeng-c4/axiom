...
# lib = "format"
# dimension = "behavior"
# case = "format_test__test_better_error_message_format"
# subject = "cpython.test_format.FormatTest.test_better_error_message_format"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_format.py::FormatTest::test_better_error_message_format
"""Auto-ported test: FormatTest::test_better_error_message_format (CPython 3.12 oracle)."""
...
for value in [12j, 12, 12.0, '12']:
    bad_format_spec = '%M'
    err = re.escape(f"Invalid format specifier '{bad_format_spec}' for object of type '{type(value).__name__}'")
    try:
        f'xx{{value:{bad_format_spec}}}yy'.format(value=value)
        raise AssertionError('expected ValueError')
    except ValueError as _aR_e:
        import re as _re_aR
        assert _re_aR.search(err, str(_aR_e))
    try:
    ...
    try:
        ...
        import re as _re_aR
        assert _re_aR.search(err, str(_aR_e))
print("FormatTest::test_better_error_message_format: ok")
