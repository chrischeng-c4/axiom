...
# lib = "cmd_line_script"
# dimension = "behavior"
# case = "cmd_line_test__test_consistent_sys_path_for_direct_execution"
# subject = "cpython.test_cmd_line_script.CmdLineTest.test_consistent_sys_path_for_direct_execution"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_cmd_line_script.py::CmdLineTest::test_consistent_sys_path_for_direct_execution
"""Auto-ported test: CmdLineTest::test_consistent_sys_path_for_direct_execution (CPython 3.12 oracle)."""
...
with os_helper.temp_dir() as work_dir, os_helper.temp_dir() as script_dir:
    ...
    assert out_by_name[0] == script_dir

    assert work_dir not in out_by_name
    p = spawn_python('-Es', script_dir, cwd=work_dir)
    out_by_dir = kill_python(p).decode().splitlines()

    assert out_by_dir == out_by_name
    p = spawn_python('-I', script_dir, cwd=work_dir)
    out_by_dir_isolated = kill_python(p).decode().splitlines()

    assert out_by_dir_isolated == out_by_dir
print("CmdLineTest::test_consistent_sys_path_for_direct_execution: ok")
