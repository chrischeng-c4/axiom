...
# lib = "xmlrpc"
# dimension = "behavior"
# case = "cgi_handler_test_case__test_cgi_xmlrpc_response"
# subject = "cpython.test_xmlrpc.CGIHandlerTestCase.test_cgi_xmlrpc_response"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_xmlrpc.py::CGIHandlerTestCase::test_cgi_xmlrpc_response
"""Auto-ported test: CGIHandlerTestCase::test_cgi_xmlrpc_response (CPython 3.12 oracle)."""
...
# --- test body ---
self_cgi = xmlrpc.server.CGIXMLRPCRequestHandler()
data = "<?xml version='1.0'?>\n        <methodCall>\n            <methodName>test_method</methodName>\n            <params>\n                <param>\n                    <value><string>foo</string></value>\n                </param>\n                <param>\n                    <value><string>bar</string></value>\n                </param>\n            </params>\n        </methodCall>\n        "
with os_helper.EnvironmentVarGuard() as env, captured_stdout(encoding=self_cgi.encoding) as data_out, support.captured_stdin() as data_in:
    data_in.write(data)
    data_in.seek(0)
    env['CONTENT_LENGTH'] = str(len(data))
    self_cgi.handle_request()
data_out.seek(0)
handle = data_out.read()

try:
    xmlrpclib.loads(handle[44:])
    raise AssertionError('expected xmlrpclib.Fault')
except xmlrpclib.Fault:
    pass
content = handle[handle.find('<?xml'):]

assert int(re.search('Content-Length: (\\d+)', handle).group(1)) == len(content)
print("CGIHandlerTestCase::test_cgi_xmlrpc_response: ok")
