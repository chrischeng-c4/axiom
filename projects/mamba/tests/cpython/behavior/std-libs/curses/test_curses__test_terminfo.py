...
# lib = "curses"
# dimension = "behavior"
# case = "test_curses__test_terminfo"
# subject = "cpython.test_curses.TestCurses.test_terminfo"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_curses.py::TestCurses::test_terminfo
"""Auto-ported test: TestCurses::test_terminfo (CPython 3.12 oracle)."""
...
assert isinstance(curses.tigetflag('hc'), int)

assert curses.tigetflag('cols') == -1

assert curses.tigetflag('cr') == -1

assert isinstance(curses.tigetnum('cols'), int)

assert curses.tigetnum('hc') == -2

assert curses.tigetnum('cr') == -2

assert isinstance(curses.tigetstr('cr'), (bytes, type(None)))

assert curses.tigetstr('hc') is None

assert curses.tigetstr('cols') is None
cud = curses.tigetstr('cud')
if cud is not None:

    assert isinstance(cud, bytes)
    curses.tparm(cud, 2)
    cud_2 = curses.tparm(cud, 2)

    assert isinstance(cud_2, bytes)
    curses.putp(cud_2)
curses.putp(b'abc\n')
print("TestCurses::test_terminfo: ok")
