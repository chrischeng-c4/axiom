...
# lib = "curses"
# dimension = "behavior"
# case = "test_curses__test_clear"
# subject = "cpython.test_curses.TestCurses.test_clear"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_curses.py::TestCurses::test_clear
"""Auto-ported test: TestCurses::test_clear (CPython 3.12 oracle)."""
...
assert win.instr(0, 0).rstrip() == b'Lorem ip'

assert win.instr(1, 0).rstrip() == b'dolor sit amet,'
win.move(0, 3)
win.clrtobot()

assert win.instr(0, 0).rstrip() == b'Lor'

assert win.instr(1, 0).rstrip() == b''
for func in [win.erase, win.clear]:
    lorem_ipsum(win)
    func()

    assert win.instr(0, 0).rstrip() == b''

    assert win.instr(1, 0).rstrip() == b''
print("TestCurses::test_clear: ok")
