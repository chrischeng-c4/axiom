...
# lib = "time"
# dimension = "behavior"
# case = "time_test_case__test_strftime_special"
# subject = "cpython.test_time.TimeTestCase.test_strftime_special"
# kind = "semantic"
# xfail = ""
# mem_carveout = ""
...
# status = "filled"
# ///
# Auto-ported from CPython 3.12 test_time.py::TimeTestCase::test_strftime_special
"""Auto-ported test: TimeTestCase::test_strftime_special (CPython 3.12 oracle)."""
...
# --- test body ---
self_t = time.time()
tt = time.gmtime(self_t)
s1 = time.strftime('%c', tt)
s2 = time.strftime('%B', tt)

assert time.strftime('🐍', tt) == '🐍'

assert time.strftime('💻%c🐍%B', tt) == f'💻{s1}🐍{s2}'
...
assert time.strftime('\x00', tt) == '\x00'

assert time.strftime('\x00' * 1000, tt) == '\x00' * 1000

assert time.strftime('\x00%c\x00%B', tt) == f'\x00{s1}\x00{s2}'

assert time.strftime('%c\x00%B\x00', tt) == f'{s1}\x00{s2}\x00'
print("TimeTestCase::test_strftime_special: ok")
