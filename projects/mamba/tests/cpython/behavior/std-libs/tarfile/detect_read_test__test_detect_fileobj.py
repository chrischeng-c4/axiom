...
# lib = "tarfile"
# dimension = "behavior"
# case = "detect_read_test__test_detect_fileobj"
# subject = "cpython.test_tarfile.DetectReadTest.test_detect_fileobj"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_tarfile.py::DetectReadTest::test_detect_fileobj
"""Auto-ported test: DetectReadTest::test_detect_fileobj (CPython 3.12 oracle)."""
...
def _test_modes(testfunc):
    if suffix:
        try:
            tarfile.open(tarname, mode='r:' + suffix)
        ...
        try:
            ...
            pass
    testfunc(tarname, 'r')
    testfunc(tarname, 'r:' + suffix)
    testfunc(tarname, 'r:*')
    testfunc(tarname, 'r|' + suffix)
    testfunc(tarname, 'r|*')

def _testfunc_file(name, mode):
    try:
...
def _testfunc_fileobj(name, mode):
    try:
...
def mode():
    return self_prefix + suffix
_test_modes(_testfunc_fileobj)
print("DetectReadTest::test_detect_fileobj: ok")
