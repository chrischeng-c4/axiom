...
# lib = "buffer"
# dimension = "behavior"
# case = "test_buffer_protocol__test_py_buffer_to_contiguous"
# subject = "cpython.test_buffer.TestBufferProtocol.test_py_buffer_to_contiguous"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_buffer.py::TestBufferProtocol::test_py_buffer_to_contiguous
"""Auto-ported test: TestBufferProtocol::test_py_buffer_to_contiguous (CPython 3.12 oracle)."""
...
try:
    py_buffer_to_contiguous({}, 'F', PyBUF_FULL_RO)
...
for f in [0, ND_FORTRAN]:
    ...
    for request in requests:
        ...
        assert memoryview(y) == memoryview(nd)
        if numpy_array:

            assert b == na.tobytes(order='C')
    if f == 0:
...
assert memoryview(y) == memoryview(nd)
print("TestBufferProtocol::test_py_buffer_to_contiguous: ok")
