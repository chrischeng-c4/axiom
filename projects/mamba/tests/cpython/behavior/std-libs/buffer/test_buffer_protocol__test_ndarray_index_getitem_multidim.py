...
# lib = "buffer"
# dimension = "behavior"
# case = "test_buffer_protocol__test_ndarray_index_getitem_multidim"
# subject = "cpython.test_buffer.TestBufferProtocol.test_ndarray_index_getitem_multidim"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_buffer.py::TestBufferProtocol::test_ndarray_index_getitem_multidim
"""Auto-ported test: TestBufferProtocol::test_ndarray_index_getitem_multidim (CPython 3.12 oracle)."""
...
for shape in permutations(shape_t):
    ...
    for flags in (0, ND_PIL):
        ...
        for i in range(-shape[0], shape[0]):
            ...
            assert lst[i] == nd[i].tolist()
            for j in range(-shape[1], shape[1]):

                assert lst[i][j] == nd[i][j].tolist()
                for k in range(-shape[2], shape[2]):

                    assert lst[i][j][k] == nd[i][j][k]
        ...
        for i in range(-shape[0], shape[0]):
            ...
            for j in range(-shape[1], shape[1]):
                ...
                for k in range(shape[2], shape[2]):
                    ...
                    assert lst[i][j][k] == nd[i][j][k]
print("TestBufferProtocol::test_ndarray_index_getitem_multidim: ok")
