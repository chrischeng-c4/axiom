...
# lib = "genericalias"
# dimension = "behavior"
# case = "base_test__test_copy"
# subject = "cpython.test_genericalias.BaseTest.test_copy"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_genericalias.py::BaseTest::test_copy
"""Auto-ported test: BaseTest::test_copy (CPython 3.12 oracle)."""
...
class X(list):
    ...
    def __deepcopy__(self, memo):
        return self
aliases = [GenericAlias(list, T), GenericAlias(deque, T), GenericAlias(X, T)] + _UNPACKED_TUPLES
for alias in aliases:
    copied = copy.copy(alias)

    assert copied.__origin__ == alias.__origin__

    assert copied.__args__ == alias.__args__

    assert copied.__parameters__ == alias.__parameters__
