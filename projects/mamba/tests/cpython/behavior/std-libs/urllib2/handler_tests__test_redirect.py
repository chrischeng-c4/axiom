...
# lib = "urllib2"
# dimension = "behavior"
# case = "handler_tests__test_redirect"
# subject = "cpython.test_urllib2.HandlerTests.test_redirect"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_urllib2.py::HandlerTests::test_redirect
"""Auto-ported test: HandlerTests::test_redirect (CPython 3.12 oracle)."""
...
for code in (301, 302, 303, 307, 308):
    for data in (None, 'blah\nblah\n'):
        ...
        try:
            ...
            assert code in (307, 308)

            assert data is not None
            err.close()

        assert o.req.get_full_url() == to_url
        try:

            assert o.req.get_method() == 'GET'
        except AttributeError:

            assert not o.req.data
        ...
        assert o.req.headers['Nonsense'] == 'viking=withhold'

        assert 'Spam' not in o.req.headers

        assert 'Spam' not in o.req.unredirected_hdrs
...
try:
    ...
    assert count == urllib.request.HTTPRedirectHandler.max_repeats
    err.close()
req = Request(from_url, origin_req_host='example.com')
count = 0
req.timeout = socket._GLOBAL_DEFAULT_TIMEOUT
try:
    ...
    assert count == urllib.request.HTTPRedirectHandler.max_redirections
    err.close()
print("HandlerTests::test_redirect: ok")
