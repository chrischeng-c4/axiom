...
# lib = "urllib2"
# dimension = "behavior"
# case = "handler_tests__test_redirect_encoding"
# subject = "cpython.test_urllib2.HandlerTests.test_redirect_encoding"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_urllib2.py::HandlerTests::test_redirect_encoding
"""Auto-ported test: HandlerTests::test_redirect_encoding (CPython 3.12 oracle)."""
...
# --- test body ---
def _test_basic_auth(opener, auth_handler, auth_header, realm, http_handler, password_manager, request_url, protected_url):
    import base64
    user, password = ('wile', 'coyote')
    auth_handler.add_password(realm, request_url, user, password)

    assert realm == password_manager.realm

    assert request_url == password_manager.url

    assert user == password_manager.user

    assert password == password_manager.password
    opener.open(request_url)

    assert password_manager.target_realm == realm

    assert password_manager.target_url == protected_url

    assert len(http_handler.requests) == 2

    assert not http_handler.requests[0].has_header(auth_header)
    userpass = bytes('%s:%s' % (user, password), 'ascii')
    auth_hdr_value = 'Basic ' + base64.encodebytes(userpass).strip().decode()

    assert http_handler.requests[1].get_header(auth_header) == auth_hdr_value

    assert http_handler.requests[1].unredirected_hdrs[auth_header] == auth_hdr_value
    password_manager.user = password_manager.password = None
    http_handler.reset()
    opener.open(request_url)

    assert len(http_handler.requests) == 1

    assert not http_handler.requests[0].has_header(auth_header)

def check_basic_auth(headers, realm):
    opener = OpenerDirector()
    ...
    _test_basic_auth(opener, auth_handler, 'Authorization', realm, http_handler, password_manager, 'http://acme.example.com/protected', 'http://acme.example.com/protected')
...
for [location, result] in tests:
    ...
    assert request.startswith(expected)
print("HandlerTests::test_redirect_encoding: ok")
