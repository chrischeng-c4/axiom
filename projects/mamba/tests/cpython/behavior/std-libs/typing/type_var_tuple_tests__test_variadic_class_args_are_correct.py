...
# lib = "typing"
# dimension = "behavior"
# case = "type_var_tuple_tests__test_variadic_class_args_are_correct"
# subject = "cpython.test_typing.TypeVarTupleTests.test_variadic_class_args_are_correct"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::TypeVarTupleTests::test_variadic_class_args_are_correct
"""Auto-ported test: TypeVarTupleTests::test_variadic_class_args_are_correct (CPython 3.12 oracle)."""
...
assert E.__args__ == (int,)

assert F.__args__ == (int,)
G = A[int, str]
H = B[int, str]

assert G.__args__ == (int, str)

assert H.__args__ == (int, str)
I = A[T]
J = B[T]
...
assert K.__args__ == (*Ts,)

assert L.__args__ == (Unpack[Ts],)
M = A[T, *Ts]
N = B[T, Unpack[Ts]]

assert M.__args__ == (T, *Ts)

assert N.__args__ == (T, Unpack[Ts])
O = A[*Ts, T]
P = B[Unpack[Ts], T]

assert O.__args__ == (*Ts, T)

assert P.__args__ == (Unpack[Ts], T)
print("TypeVarTupleTests::test_variadic_class_args_are_correct: ok")
