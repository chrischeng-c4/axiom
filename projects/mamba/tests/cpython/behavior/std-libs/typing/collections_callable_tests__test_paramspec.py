...
# lib = "typing"
# dimension = "behavior"
# case = "collections_callable_tests__test_paramspec"
# subject = "cpython.test_typing.CollectionsCallableTests.test_paramspec"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::CollectionsCallableTests::test_paramspec
"""Auto-ported test: CollectionsCallableTests::test_paramspec (CPython 3.12 oracle)."""
...
assert C1[[int], str] == Callable[[int], str]

assert C1[[int, str], str] == Callable[[int, str], str]

assert C1[[], str] == Callable[[], str]

assert C1[..., str] == Callable[..., str]

assert C1[P2, str] == Callable[P2, str]

assert C1[Concatenate[int, P2], str] == Callable[Concatenate[int, P2], str]

assert repr(C1) == f'{fullname}[~P, ~T]'

assert repr(C1[[int, str], str]) == f'{fullname}[[int, str], str]'
try:
    C1[int, str]
...
assert C2[[int]] == Callable[[int], int]

assert C2[[int, str]] == Callable[[int, str], int]

assert C2[[]] == Callable[[], int]

assert C2[...] == Callable[..., int]

assert C2[P2] == Callable[P2, int]

assert C2[Concatenate[int, P2]] == Callable[Concatenate[int, P2], int]

assert C2[int] == Callable[[int], int]

assert C2[int, str] == Callable[[int, str], int]

assert repr(C2) == f'{fullname}[~P, int]'

assert repr(C2[int, str]) == f'{fullname}[[int, str], int]'
print("CollectionsCallableTests::test_paramspec: ok")
