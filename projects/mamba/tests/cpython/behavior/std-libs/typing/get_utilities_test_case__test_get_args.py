...
# lib = "typing"
# dimension = "behavior"
# case = "get_utilities_test_case__test_get_args"
# subject = "cpython.test_typing.GetUtilitiesTestCase.test_get_args"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::GetUtilitiesTestCase::test_get_args
"""Auto-ported test: GetUtilitiesTestCase::test_get_args (CPython 3.12 oracle)."""
...
assert get_args(C[int]) == (int,)

assert get_args(C[T]) == (T,)

assert get_args(typing.SupportsAbs[int]) == (int,)

assert get_args(typing.SupportsAbs[T]) == (T,)

assert get_args(Point2DGeneric[int]) == (int,)

assert get_args(Point2DGeneric[T]) == (T,)
...
assert get_args(Self) == ()

assert get_args(LiteralString) == ()

assert get_args(ClassVar[int]) == (int,)

assert get_args(Union[int, str]) == (int, str)

assert get_args(Literal[42, 43]) == (42, 43)

assert get_args(Final[List[int]]) == (List[int],)
...
assert get_args(typing.Dict[int, Tuple[T, T]][Optional[int]]) == (int, Tuple[Optional[int], Optional[int]])

assert get_args(Callable[[], T][int]) == ([], int)

assert get_args(Callable[..., int]) == (..., int)

assert get_args(Callable[[int], str]) == ([int], str)

assert get_args(Union[int, Callable[[Tuple[T, ...]], str]]) == (int, Callable[[Tuple[T, ...]], str])

assert get_args(Tuple[int, ...]) == (int, ...)

assert get_args(Tuple[()]) == ()

assert get_args(Annotated[T, 'one', 2, ['three']]) == (T, 'one', 2, ['three'])

assert get_args(List) == ()

assert get_args(Tuple) == ()

assert get_args(Callable) == ()

assert get_args(list[int]) == (int,)

assert get_args(list) == ()

assert get_args(collections.abc.Callable[[int], str]) == ([int], str)

assert get_args(collections.abc.Callable[..., str]) == (..., str)

assert get_args(collections.abc.Callable[[], str]) == ([], str)

assert get_args(collections.abc.Callable[[int], str]) == get_args(Callable[[int], str])
P = ParamSpec('P')

assert get_args(P) == ()

assert get_args(P.args) == ()

assert get_args(P.kwargs) == ()

assert get_args(Callable[P, int]) == (P, int)

assert get_args(collections.abc.Callable[P, int]) == (P, int)

assert get_args(Callable[Concatenate[int, P], int]) == (Concatenate[int, P], int)

assert get_args(collections.abc.Callable[Concatenate[int, P], int]) == (Concatenate[int, P], int)

assert get_args(Concatenate[int, str, P]) == (int, str, P)

assert get_args(list | str) == (list, str)

assert get_args(Required[int]) == (int,)

assert get_args(NotRequired[int]) == (int,)

assert get_args(TypeAlias) == ()

assert get_args(TypeGuard[int]) == (int,)
Ts = TypeVarTuple('Ts')

assert get_args(Ts) == ()

assert get_args((*Ts,)[0]) == (Ts,)

assert get_args(Unpack[Ts]) == (Ts,)

assert get_args(tuple[*Ts,]) == (*Ts,)

assert get_args(tuple[Unpack[Ts]]) == (Unpack[Ts],)

assert get_args((*tuple[*Ts,],)[0]) == (*Ts,)

assert get_args(Unpack[tuple[Unpack[Ts]]]) == (tuple[Unpack[Ts]],)
print("GetUtilitiesTestCase::test_get_args: ok")
