...
# lib = "typing"
# dimension = "behavior"
# case = "named_tuple_tests__test_generic"
# subject = "cpython.test_typing.NamedTupleTests.test_generic"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::NamedTupleTests::test_generic
"""Auto-ported test: NamedTupleTests::test_generic (CPython 3.12 oracle)."""
...
assert X.__bases__ == (tuple, Generic)

assert X.__orig_bases__ == (NamedTuple, Generic[T])

assert X.__mro__ == (X, tuple, Generic, object)

class Y(Generic[T], NamedTuple):
    x: T

assert Y.__bases__ == (Generic, tuple)

assert Y.__orig_bases__ == (Generic[T], NamedTuple)

assert Y.__mro__ == (Y, Generic, tuple, object)
for G in (X, Y):

    assert G.__parameters__ == (T,)

    assert G[T].__args__ == (T,)

    assert get_args(G[T]) == (T,)
    A = G[int]

    assert A.__origin__ is G
    ...
    try:
        ...
        pass
print("NamedTupleTests::test_generic: ok")
