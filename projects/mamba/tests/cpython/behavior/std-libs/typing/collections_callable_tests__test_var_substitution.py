...
# lib = "typing"
# dimension = "behavior"
# case = "collections_callable_tests__test_var_substitution"
# subject = "cpython.test_typing.CollectionsCallableTests.test_var_substitution"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::CollectionsCallableTests::test_var_substitution
"""Auto-ported test: CollectionsCallableTests::test_var_substitution (CPython 3.12 oracle)."""
...
assert C1[str] == Callable[[int, str], str]

assert C1[None] == Callable[[int, type(None)], type(None)]

assert C2[int, float, str] == Callable[[int, float], str]

assert C3[int] == Callable[..., int]

assert C3[NoReturn] == Callable[..., NoReturn]
C4 = C2[int, VT, str]

assert repr(C4) == f'{fullname}[[int, ~VT], str]'

assert repr(C4[dict]) == f'{fullname}[[int, dict], str]'

assert C4[dict] == Callable[[int, dict], str]
C5 = Callable[[typing.List[T], tuple[KT, T], VT], int]

assert C5[int, str, float] == Callable[[typing.List[int], tuple[str, int], float], int]
print("CollectionsCallableTests::test_var_substitution: ok")
