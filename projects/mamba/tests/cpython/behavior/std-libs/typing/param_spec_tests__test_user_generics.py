...
# lib = "typing"
# dimension = "behavior"
# case = "param_spec_tests__test_user_generics"
# subject = "cpython.test_typing.ParamSpecTests.test_user_generics"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::ParamSpecTests::test_user_generics
"""Auto-ported test: ParamSpecTests::test_user_generics (CPython 3.12 oracle)."""
...
assert G1.__args__ == (int, P_2)

assert G1.__parameters__ == (P_2,)
try:
    X[int]
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
    import re as _re_aR
    assert _re_aR.search('few arguments for', str(_aR_e))
try:
    X[int, P_2, str]
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
    import re as _re_aR
    assert _re_aR.search('many arguments for', str(_aR_e))
G2 = X[int, Concatenate[int, P_2]]

assert G2.__args__ == (int, Concatenate[int, P_2])

assert G2.__parameters__ == (P_2,)
G3 = X[int, [int, bool]]

assert G3.__args__ == (int, (int, bool))

assert G3.__parameters__ == ()
G4 = X[int, ...]

assert G4.__args__ == (int, Ellipsis)

assert G4.__parameters__ == ()

class Z(Generic[P]):
    f: Callable[P, int]
G5 = Z[[int, str, bool]]

assert G5.__args__ == ((int, str, bool),)

assert G5.__parameters__ == ()
G6 = Z[int, str, bool]

assert G6.__args__ == ((int, str, bool),)

assert G6.__parameters__ == ()

assert G5.__args__ == G6.__args__

assert G5.__origin__ == G6.__origin__

assert G5.__parameters__ == G6.__parameters__

assert G5 == G6
G7 = Z[int]

assert G7.__args__ == ((int,),)

assert G7.__parameters__ == ()
try:
    Z[[int, str], bool]
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search('many arguments for', str(_aR_e))
print("ParamSpecTests::test_user_generics: ok")
