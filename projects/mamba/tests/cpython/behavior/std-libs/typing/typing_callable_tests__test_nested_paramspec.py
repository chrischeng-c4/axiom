...
# lib = "typing"
# dimension = "behavior"
# case = "typing_callable_tests__test_nested_paramspec"
# subject = "cpython.test_typing.TypingCallableTests.test_nested_paramspec"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::TypingCallableTests::test_nested_paramspec
"""Auto-ported test: TypingCallableTests::test_nested_paramspec (CPython 3.12 oracle)."""
...
assert My.__parameters__ == (P, T)
C1 = My[[int, T2], Callable[P2, T2]]

assert C1.__args__ == ((int, T2), Callable[P2, T2])

assert C1.__parameters__ == (T2, P2)

assert C1[str, [list[int], bytes]] == My[[int, str], Callable[[list[int], bytes], str]]
C2 = My[[Callable[[T2], int], list[T2]], str]

assert C2.__args__ == ((Callable[[T2], int], list[T2]), str)

assert C2.__parameters__ == (T2,)

assert C2[list[str]] == My[[Callable[[list[str]], int], list[list[str]]], str]
C3 = My[[Callable[P2, T2], T2], T2]

assert C3.__args__ == ((Callable[P2, T2], T2), T2)

assert C3.__parameters__ == (P2, T2)

assert C3[[], int] == My[[Callable[[], int], int], int]

assert C3[[str, bool], int] == My[[Callable[[str, bool], int], int], int]

assert C3[[str, bool], T][int] == My[[Callable[[str, bool], int], int], int]
C4 = My[[Callable[[int, *Ts, str], T2], T2], T2]

assert C4.__args__ == ((Callable[[int, *Ts, str], T2], T2), T2)

assert C4.__parameters__ == (Ts, T2)

assert C4[bool, bytes, float] == My[[Callable[[int, bool, bytes, str], float], float], float]
print("TypingCallableTests::test_nested_paramspec: ok")
