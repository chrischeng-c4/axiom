...
# lib = "typing"
# dimension = "behavior"
# case = "type_var_tuple_tests__test_callable_args_are_correct"
# subject = "cpython.test_typing.TypeVarTupleTests.test_callable_args_are_correct"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::TypeVarTupleTests::test_callable_args_are_correct
"""Auto-ported test: TypeVarTupleTests::test_callable_args_are_correct (CPython 3.12 oracle)."""
...
def tests_tuple_arg_ordering_matters():
    Ts1 = TypeVarTuple('Ts1')
    Ts2 = TypeVarTuple('Ts2')

    assert tuple[*Ts1, *Ts2] != tuple[*Ts2, *Ts1]

    assert Tuple[Unpack[Ts1], Unpack[Ts2]] != Tuple[Unpack[Ts2], Unpack[Ts1]]
Ts = TypeVarTuple('Ts')
Ts1 = TypeVarTuple('Ts1')
Ts2 = TypeVarTuple('Ts2')
a = Callable[[*Ts], None]
...
assert a.__args__ == (*Ts, type(None))

assert b.__args__ == (Unpack[Ts], type(None))
c = Callable[[int, *Ts], None]
d = Callable[[int, Unpack[Ts]], None]

assert c.__args__ == (int, *Ts, type(None))

assert d.__args__ == (int, Unpack[Ts], type(None))
e = Callable[[*Ts, int], None]
f = Callable[[Unpack[Ts], int], None]

assert e.__args__ == (*Ts, int, type(None))

assert f.__args__ == (Unpack[Ts], int, type(None))
g = Callable[[str, *Ts, int], None]
h = Callable[[str, Unpack[Ts], int], None]

assert g.__args__ == (str, *Ts, int, type(None))

assert h.__args__ == (str, Unpack[Ts], int, type(None))
i = Callable[[None], *Ts]
j = Callable[[None], Unpack[Ts]]

assert i.__args__ == (type(None), *Ts)

assert j.__args__ == (type(None), Unpack[Ts])
k = Callable[[None], tuple[int, *Ts]]
l = Callable[[None], Tuple[int, Unpack[Ts]]]

assert k.__args__ == (type(None), tuple[int, *Ts])

assert l.__args__ == (type(None), Tuple[int, Unpack[Ts]])
m = Callable[[None], tuple[*Ts, int]]
n = Callable[[None], Tuple[Unpack[Ts], int]]

assert m.__args__ == (type(None), tuple[*Ts, int])

assert n.__args__ == (type(None), Tuple[Unpack[Ts], int])
o = Callable[[None], tuple[str, *Ts, int]]
p = Callable[[None], Tuple[str, Unpack[Ts], int]]

assert o.__args__ == (type(None), tuple[str, *Ts, int])

assert p.__args__ == (type(None), Tuple[str, Unpack[Ts], int])
q = Callable[[*Ts], *Ts]
r = Callable[[Unpack[Ts]], Unpack[Ts]]

assert q.__args__ == (*Ts, *Ts)

assert r.__args__ == (Unpack[Ts], Unpack[Ts])
s = Callable[[*Ts1], *Ts2]
u = Callable[[Unpack[Ts1]], Unpack[Ts2]]

assert s.__args__ == (*Ts1, *Ts2)

assert u.__args__ == (Unpack[Ts1], Unpack[Ts2])
print("TypeVarTupleTests::test_callable_args_are_correct: ok")
