...
# lib = "typing"
# dimension = "behavior"
# case = "generic_tests__test_copy_generic_instances"
# subject = "cpython.test_typing.GenericTests.test_copy_generic_instances"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::GenericTests::test_copy_generic_instances
"""Auto-ported test: GenericTests::test_copy_generic_instances (CPython 3.12 oracle)."""
...
assert copy(c).attr == 42

assert deepcopy(c).attr == 42

assert copy(c) is not c

assert deepcopy(c) is not c
c.attr = 1

assert copy(c).attr == 1

assert deepcopy(c).attr == 1
ci = C[int](42)

assert copy(ci).attr == 42

assert deepcopy(ci).attr == 42

assert copy(ci) is not ci

assert deepcopy(ci) is not ci
ci.attr = 1

assert copy(ci).attr == 1

assert deepcopy(ci).attr == 1

assert ci.__orig_class__ == C[int]
print("GenericTests::test_copy_generic_instances: ok")
