...
# lib = "memoryio"
# dimension = "behavior"
# case = "py_string_io_test__test_readlines"
# subject = "cpython.test_memoryio.PyStringIOTest.test_readlines"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_memoryio.py::PyStringIOTest::test_readlines
"""Auto-ported test: PyStringIOTest::test_readlines (CPython 3.12 oracle)."""
...
def write_ops(f, t):
    ...
    assert f.tell() == 1
buf = buftype('1234567890\n')
memio = ioclass(buf * 10)

assert memio.readlines() == [buf] * 10
memio.seek(5)

assert memio.readlines() == [buf[5:]] + [buf] * 9
memio.seek(0)

assert memio.readlines(15) == [buf] * 2
memio.seek(0)

assert memio.readlines(-1) == [buf] * 10
memio.seek(0)

assert memio.readlines(0) == [buf] * 10
memio.seek(0)

assert type(memio.readlines()[0]) == type(buf)
memio.seek(0)

assert memio.readlines(None) == [buf] * 10
...
try:
    ...
    pass
print("PyStringIOTest::test_readlines: ok")
