...
# lib = "dis"
# dimension = "behavior"
# case = "dis_with_file_tests__test_big_offsets"
# subject = "cpython.test_dis.DisWithFileTests.test_big_offsets"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_dis.py::DisWithFileTests::test_big_offsets
"""Auto-ported test: DisWithFileTests::test_big_offsets (CPython 3.12 oracle)."""
...
def do_disassembly_test(func, expected, with_offsets=False):
    maxDiff = None
    got = get_disassembly(func, depth=0)
    do_disassembly_compare(got, expected, with_offsets)
    if hasattr(func, '__code__'):
        got_disco = io.StringIO()
        with contextlib.redirect_stdout(got_disco):
            dis.disco(func.__code__)
        do_disassembly_compare(got_disco.getvalue(), expected, with_offsets)
...
def get_cached_values(quickened, adaptive):
    ...
    for instruction in dis.get_instructions(f, show_caches=True, adaptive=adaptive):
        if instruction.opname == 'CACHE':
            yield instruction.argrepr

def get_disassemble_as_string(func, lasti=-1):
    return get_disassembly(func, lasti, False)

def get_disassembly(func, lasti=-1, wrapper=True, **kwargs):
    output = io.StringIO()
    if wrapper:
...
def expected(count, w):
    s = ['  1        %*d RESUME                   0\n\n' % (w, 0)]
    s += ['           %*d LOAD_FAST                0 (x)\n           %*d LOAD_CONST               1 (1)\n           %*d BINARY_OP                0 (+)\n           %*d STORE_FAST               0 (x)\n' % (w, 10 * i + 2, w, 10 * i + 4, w, 10 * i + 6, w, 10 * i + 10) for i in range(count)]
    s += ['\n  3        %*d LOAD_FAST                0 (x)\n           %*d RETURN_VALUE\n' % (w, 10 * count + 2, w, 10 * count + 4)]
    s[1] = '  2' + s[1][3:]
    return ''.join(s)
for i in range(1, 5):
    do_disassembly_test(func(i), expected(i, 4), True)
do_disassembly_test(func(999), expected(999, 4), True)
do_disassembly_test(func(1000), expected(1000, 5), True)
print("DisWithFileTests::test_big_offsets: ok")
