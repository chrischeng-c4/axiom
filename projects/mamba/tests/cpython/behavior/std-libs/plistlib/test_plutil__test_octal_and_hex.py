...
# lib = "plistlib"
# dimension = "behavior"
# case = "test_plutil__test_octal_and_hex"
# subject = "cpython.test_plistlib.TestPlutil.test_octal_and_hex"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_plistlib.py::TestPlutil::test_octal_and_hex
"""Auto-ported test: TestPlutil::test_octal_and_hex (CPython 3.12 oracle)."""
...
properties = {'fname': 'H', 'lname': 'A', 'marks': {'a': 100, 'b': 16}}
exptected_properties = {'fname': 'H', 'lname': 'A', 'marks': {'a': 100, 'b': 16}}
pl = {'HexType': 16777228, 'IntType': 83}
...
def check_content():
    ...
    with open(file_name) as f:
        ...
        assert ff == exptected_properties

def check_plistlib_parse():
    convert_to_bin()
    with open(file_name, 'rb') as f:

        assert plistlib.load(f) == exptected_properties

def convert_to_bin():
    """Convert file to binary using plutil
        """
    subprocess.run(['plutil', '-convert', 'binary1', file_name])

def convert_to_json():
    """Convert binary file to json using plutil
        """
    subprocess.run(['plutil', '-convert', 'json', file_name])

def get_lint_status():
    return subprocess.run(['plutil', '-lint', file_name], capture_output=True, text=True).stdout
...
with open(file_name, 'r') as f:
    ...
    assert p.get('HexType') == 16777228

    assert p.get('IntType') == 83
print("TestPlutil::test_octal_and_hex: ok")
