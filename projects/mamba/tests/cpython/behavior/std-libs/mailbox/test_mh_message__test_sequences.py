...
# lib = "mailbox"
# dimension = "behavior"
# case = "test_mh_message__test_sequences"
# subject = "cpython.test_mailbox.TestMHMessage.test_sequences"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_mailbox.py::TestMHMessage::test_sequences
"""Auto-ported test: TestMHMessage::test_sequences (CPython 3.12 oracle)."""
...
assert msg.get_sequences() == []
msg.set_sequences(['foobar'])

assert msg.get_sequences() == ['foobar']
msg.set_sequences([])

assert msg.get_sequences() == []
msg.add_sequence('unseen')

assert msg.get_sequences() == ['unseen']
msg.add_sequence('flagged')

assert msg.get_sequences() == ['unseen', 'flagged']
msg.add_sequence('flagged')

assert msg.get_sequences() == ['unseen', 'flagged']
msg.remove_sequence('unseen')

assert msg.get_sequences() == ['flagged']
msg.add_sequence('foobar')

assert msg.get_sequences() == ['flagged', 'foobar']
msg.remove_sequence('replied')

assert msg.get_sequences() == ['flagged', 'foobar']
msg.set_sequences(['foobar', 'replied'])

assert msg.get_sequences() == ['foobar', 'replied']
print("TestMHMessage::test_sequences: ok")
