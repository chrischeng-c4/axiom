...
# lib = "re"
# dimension = "behavior"
# case = "re_tests__test_multiple_repeat"
# subject = "cpython.test_re.ReTests.test_multiple_repeat"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_re.py::ReTests::test_multiple_repeat
"""Auto-ported test: ReTests::test_multiple_repeat (CPython 3.12 oracle)."""
    ...
    for outer_mod in ('', '?', '+'):
        ...
        for inner_reps in ('*', '+', '?', '{1,2}'):
            for inner_mod in ('', '?', '+'):
                ...
                checkPatternError('x%s%s' % (inner_op, outer_op), 'multiple repeat', 1 + len(inner_op))
print("ReTests::test_multiple_repeat: ok")
