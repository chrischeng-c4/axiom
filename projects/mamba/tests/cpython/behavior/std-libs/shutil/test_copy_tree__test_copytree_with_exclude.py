...
# lib = "shutil"
# dimension = "behavior"
# case = "test_copy_tree__test_copytree_with_exclude"
# subject = "cpython.test_shutil.TestCopyTree.test_copytree_with_exclude"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_shutil.py::TestCopyTree::test_copytree_with_exclude
"""Auto-ported test: TestCopyTree::test_copytree_with_exclude (CPython 3.12 oracle)."""
...
src_dir = mkdtemp()
try:
    dst_dir = join(mkdtemp(), 'destination')
    ...
    try:
        ...
        assert exists(join(dst_dir, 'test.txt'))

        assert not exists(join(dst_dir, 'test.tmp'))

        assert not exists(join(dst_dir, 'test_dir2'))
    finally:
    ...
    try:
        ...
        assert not exists(join(dst_dir, 'test_dir2', 'subdir2'))

        assert not exists(join(dst_dir, 'test_dir2', 'subdir'))
    finally:
        shutil.rmtree(dst_dir)
    try:
        ...
        assert not exists(join(dst_dir, 'test_dir2', 'subdir2', 'test.py'))
...
print("TestCopyTree::test_copytree_with_exclude: ok")
