...
# lib = "weakset"
# dimension = "behavior"
# case = "test_weak_set__test_intersection"
# subject = "cpython.test_weakset.TestWeakSet.test_intersection"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_weakset.py::TestWeakSet::test_intersection
"""Auto-ported test: TestWeakSet::test_intersection (CPython 3.12 oracle)."""
...
s = WeakSet(self_letters)
i = s.intersection(self_items2)
for c in self_letters:

    assert (c in i) == (c in self_items2 and c in self_letters)

assert s == WeakSet(self_letters)

assert type(i) == WeakSet
for C in (set, frozenset, dict.fromkeys, list, tuple):
    x = WeakSet([])

    assert i.intersection(C(self_items)) == x

assert len(i) == len(self_items2)
self_items2.pop()
gc.collect()

assert len(i) == len(self_items2)
print("TestWeakSet::test_intersection: ok")
