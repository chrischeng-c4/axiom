...
# lib = "subprocess"
# dimension = "behavior"
# case = "run_func_test_case__test_encoding_warning"
# subject = "cpython.test_subprocess.RunFuncTestCase.test_encoding_warning"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_subprocess.py::RunFuncTestCase::test_encoding_warning
"""Auto-ported test: RunFuncTestCase::test_encoding_warning (CPython 3.12 oracle)."""
...
code = textwrap.dedent('            from subprocess import *\n            run("echo hello", shell=True, text=True)\n            check_output("echo hello", shell=True, text=True)\n            ')
cp = subprocess.run([sys.executable, '-Xwarn_default_encoding', '-c', code], capture_output=True)
lines = cp.stderr.splitlines()

assert len(lines) == 2

assert lines[0].startswith(b'<string>:2: EncodingWarning: ')

assert lines[1].startswith(b'<string>:3: EncodingWarning: ')
print("RunFuncTestCase::test_encoding_warning: ok")
