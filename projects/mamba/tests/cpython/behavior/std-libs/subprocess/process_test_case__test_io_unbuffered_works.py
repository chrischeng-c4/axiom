...
# lib = "subprocess"
# dimension = "behavior"
# case = "process_test_case__test_io_unbuffered_works"
# subject = "cpython.test_subprocess.ProcessTestCase.test_io_unbuffered_works"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_subprocess.py::ProcessTestCase::test_io_unbuffered_works
"""Auto-ported test: ProcessTestCase::test_io_unbuffered_works (CPython 3.12 oracle)."""
...
try:
    ...
    assert isinstance(p.stdin, io.RawIOBase)

    assert isinstance(p.stdout, io.RawIOBase)

    assert isinstance(p.stderr, io.RawIOBase)
finally:
    p.stdin.close()
    p.stdout.close()
    p.stderr.close()
    p.wait()
print("ProcessTestCase::test_io_unbuffered_works: ok")
