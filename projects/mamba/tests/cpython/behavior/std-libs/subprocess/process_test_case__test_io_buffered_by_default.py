...
# lib = "subprocess"
# dimension = "behavior"
# case = "process_test_case__test_io_buffered_by_default"
# subject = "cpython.test_subprocess.ProcessTestCase.test_io_buffered_by_default"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_subprocess.py::ProcessTestCase::test_io_buffered_by_default
"""Auto-ported test: ProcessTestCase::test_io_buffered_by_default (CPython 3.12 oracle)."""
...
try:
    ...
    assert isinstance(p.stdin, io.BufferedIOBase)

    assert isinstance(p.stdout, io.BufferedIOBase)

    assert isinstance(p.stderr, io.BufferedIOBase)
finally:
    p.stdin.close()
    p.stdout.close()
    p.stderr.close()
    p.wait()
print("ProcessTestCase::test_io_buffered_by_default: ok")
