...
# lib = "functools"
# dimension = "behavior"
# case = "test_partial_py__test_repr"
# subject = "cpython.test_functools.TestPartialPy.test_repr"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_functools.py::TestPartialPy::test_repr
"""Auto-ported test: TestPartialPy::test_repr (CPython 3.12 oracle)."""
...
if partial in _partial_types:
    name = 'functools.partial'
else:
    name = partial.__name__
f = partial(capture)

assert f'{name}({capture!r})' == repr(f)
f = partial(capture, *args)

assert f'{name}({capture!r}, {args_repr})' == repr(f)
f = partial(capture, **kwargs)

assert repr(f) in [f'{name}({capture!r}, {kwargs_repr})' for kwargs_repr in kwargs_reprs]
f = partial(capture, *args, **kwargs)

assert repr(f) in [f'{name}({capture!r}, {args_repr}, {kwargs_repr})' for kwargs_repr in kwargs_reprs]
print("TestPartialPy::test_repr: ok")
