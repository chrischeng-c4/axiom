...
# lib = "decimal"
# dimension = "behavior"
# case = "c_usability_test__test_none_args"
# subject = "cpython.test_decimal.CUsabilityTest.test_none_args"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::CUsabilityTest::test_none_args
"""Auto-ported test: CUsabilityTest::test_none_args (CPython 3.12 oracle)."""
...
with localcontext(Context()) as c:
    ...
    assert str(x.exp(context=None)) == '1.609487E+48'

    assert c.flags[Inexact]

    assert c.flags[Rounded]
    c.clear_flags()
    ...
    assert z.is_normal(context=None) is False

    assert z.is_subnormal(context=None) is True
    c.clear_flags()

    assert str(x.ln(context=None)) == '4.709530'
    ...
    try:
        Decimal(-1).ln(context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass

    assert c.flags[InvalidOperation]
    c.clear_flags()

    assert str(x.log10(context=None)) == '2.045323'
    ...
    assert str(x.logb(context=None)) == '2'

    try:
        Decimal(0).logb(context=None)
        raise AssertionError('expected DivisionByZero')
    except DivisionByZero:
        pass

    assert c.flags[DivisionByZero]
    c.clear_flags()

    assert str(x.logical_invert(context=None)) == '1111000'

    try:
        y.logical_invert(context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    assert str(y.next_minus(context=None)) == '9.999999E+999'

    try:
        Decimal('sNaN').next_minus(context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    assert str(y.next_plus(context=None)) == 'Infinity'

    try:
        Decimal('sNaN').next_plus(context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    assert str(z.normalize(context=None)) == '0'

    try:
        y.normalize(context=None)
        raise AssertionError('expected Overflow')
    except Overflow:
        pass
    ...
    assert str(z.number_class(context=None)) == '+Subnormal'
    c.clear_flags()

    assert str(z.sqrt(context=None)) == '0E-1005'

    assert c.flags[Clamped]

    assert c.flags[Inexact]

    assert c.flags[Rounded]

    assert c.flags[Subnormal]

    assert c.flags[Underflow]
    c.clear_flags()
    ...
    assert str(z.to_eng_string(context=None)) == '1e-9999'
    c.capitals = 1
    c.clear_flags()
    ans = str(x.compare(Decimal('Nan891287828'), context=None))

    assert ans == 'NaN1287828'

    try:
        x.compare(Decimal('sNaN'), context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.compare_signal(Decimal('NaN'), context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.logical_and(123, context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.logical_xor(123, context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.max(Decimal('sNaN'), context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.max_mag(Decimal('sNaN'), context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.min(Decimal('sNaN'), context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.min_mag(Decimal('sNaN'), context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        y.remainder_near(101, context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        x.fma(Decimal('1e9999'), 3, context=None)
        raise AssertionError('expected Overflow')
    except Overflow:
        pass
    ...
    try:
        Decimal('sNaN').to_integral(context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        Decimal('sNaN').to_integral_value(context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        Decimal('sNaN').to_integral_exact(context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
    ...
    try:
        y.quantize(Decimal('1e-10'), rounding=ROUND_UP, context=None)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
...
with localcontext(Context()) as context:
    ...
    with localcontext(ctx=None) as c:
        ...
        assert c.Emin == -999
print("CUsabilityTest::test_none_args: ok")
