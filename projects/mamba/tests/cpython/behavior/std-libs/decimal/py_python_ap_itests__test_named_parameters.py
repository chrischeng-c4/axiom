...
# lib = "decimal"
# dimension = "behavior"
# case = "py_python_ap_itests__test_named_parameters"
# subject = "cpython.test_decimal.PyPythonAPItests.test_named_parameters"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::PyPythonAPItests::test_named_parameters
"""Auto-ported test: PyPythonAPItests::test_named_parameters (CPython 3.12 oracle)."""
...
with localcontext() as c:
    ...
    assert D(9, context=xc) == 9

    assert D(context=xc, value=9) == 9

    assert D(context=xc) == 0
    xc.clear_flags()
    ...
    assert xc.flags[InvalidOperation]

    assert not c.flags[InvalidOperation]
    xc.clear_flags()

    assert D(2).exp(context=xc) == 7
    ...
    assert xc.flags[Overflow]

    assert not c.flags[Overflow]
    xc.clear_flags()

    assert D(2).ln(context=xc) == D('0.7')
    ...
    assert D(0).log10(context=xc) == D('-inf')

    assert D(-1).next_minus(context=xc) == -2

    assert D(-1).next_plus(context=xc) == D('-0.9')

    assert D('9.73').normalize(context=xc) == D('1E+1')

    assert D('9999').to_integral(context=xc) == 9999

    assert D('-2000').to_integral_exact(context=xc) == -2000

    assert D('123').to_integral_value(context=xc) == 123

    assert D('0.0625').sqrt(context=xc) == D('0.2')

    assert D('0.0625').compare(context=xc, other=3) == -1
    xc.clear_flags()
    ...
    assert D('0.01').max(D('0.0101'), context=xc) == D('0.0')

    assert D('0.01').max(D('0.0101'), context=xc) == D('0.0')

    assert D('0.2').max_mag(D('-0.3'), context=xc) == D('-0.3')

    assert D('0.02').min(D('-0.03'), context=xc) == D('-0.0')

    assert D('0.02').min_mag(D('-0.03'), context=xc) == D('0.0')

    assert D('0.2').next_toward(D('-1'), context=xc) == D('0.1')
    xc.clear_flags()
    ...
    assert D('9.99').remainder_near(D('1.5'), context=xc) == D('-0.5')

    assert D('9.9').fma(third=D('0.9'), context=xc, other=7) == D('7E+1')

    try:
        D(1).is_canonical(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).is_finite(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).is_infinite(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).is_nan(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).is_qnan(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).is_snan(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).is_signed(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).is_zero(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    assert not D('0.01').is_normal(context=xc)

    assert D('0.01').is_subnormal(context=xc)

    try:
        D(1).adjusted(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).conjugate(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).radix(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    assert D(-111).logb(context=xc) == 2

    assert D(0).logical_invert(context=xc) == 1

    assert D('0.01').number_class(context=xc) == '+Subnormal'

    assert D('0.21').to_eng_string(context=xc) == '0.21'

    assert D('11').logical_and(D('10'), context=xc) == 0

    assert D('11').logical_or(D('10'), context=xc) == 1

    assert D('01').logical_xor(D('10'), context=xc) == 1

    assert D('23').rotate(1, context=xc) == 3
    ...
    assert D('23').shift(-1, context=xc) == 0

    try:
        D.from_float(1.1, context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(0).as_tuple(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    assert D(1).canonical() == 1

    try:
        D('-1').copy_abs(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D('-1').copy_negate(context=xc)
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        D(1).canonical(context='x')
        raise AssertionError('expected TypeError')
    except TypeError:
        pass
    ...
    try:
        ...
        pass
print("PyPythonAPItests::test_named_parameters: ok")
