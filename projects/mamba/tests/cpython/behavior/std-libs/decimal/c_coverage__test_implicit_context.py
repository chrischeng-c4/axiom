...
# lib = "decimal"
# dimension = "behavior"
# case = "c_coverage__test_implicit_context"
# subject = "cpython.test_decimal.CCoverage.test_implicit_context"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::CCoverage::test_implicit_context
"""Auto-ported test: CCoverage::test_implicit_context (CPython 3.12 oracle)."""
...
with localcontext() as c:
    ...
    assert abs(Decimal('-10')) == 10

    assert Decimal('7') + 1 == 8

    assert Decimal('10') / 5 == 2

    assert Decimal('10') // 7 == 1

    assert Decimal('1.2').fma(Decimal('0.01'), 1) == 1

    assert Decimal('NaN').fma(7, 1).is_nan() is True

    assert pow(Decimal(10), 2, 7) == 2
    if decimal == C:

        assert pow(10, Decimal(2), 7) == 2

        assert pow(10, 2, Decimal(7)) == 2
    else:
    ...
    assert Decimal('1.01').exp() == 3

    assert Decimal('0.01').is_normal() is False

    assert Decimal('0.01').is_subnormal() is True

    assert Decimal('20').ln() == 3

    assert Decimal('20').log10() == 1

    assert Decimal('580').logb() == 2

    assert Decimal('10').logical_invert() == 1

    assert -Decimal('-10') == 10

    assert Decimal('2') * 4 == 8

    assert Decimal('10').next_minus() == 9

    assert Decimal('10').next_plus() == Decimal('2E+1')

    assert Decimal('-10').normalize() == Decimal('-1E+1')

    assert Decimal('10').number_class() == '+Normal'

    assert +Decimal('-1') == -1

    assert Decimal('10') % 7 == 3

    assert Decimal('10') - 7 == 3

    assert Decimal('1.12345').to_integral_exact() == 1

    assert Decimal('1').is_canonical()
...
with localcontext() as c:
    ...
    assert z == x
print("CCoverage::test_implicit_context: ok")
