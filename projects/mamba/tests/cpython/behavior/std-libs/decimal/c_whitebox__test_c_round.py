...
# lib = "decimal"
# dimension = "behavior"
# case = "c_whitebox__test_c_round"
# subject = "cpython.test_decimal.CWhitebox.test_c_round"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::CWhitebox::test_c_round
"""Auto-ported test: CWhitebox::test_c_round (CPython 3.12 oracle)."""
...
with localcontext() as c:
    ...
    try:
        Decimal('1.23').__round__(-int_max - 1)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass

    try:
        Decimal('1.23').__round__(int_max)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass

    try:
        Decimal('1').__round__(int(MAX_EMAX + 1))
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass

    try:
        Decimal('1').__round__(-int(MIN_ETINY - 1))
        raise AssertionError('expected C.InvalidOperation')
    except C.InvalidOperation:
        pass

    try:
        Decimal('1.23').__round__(-int_max - 2)
    ...
    try:
        ...
        pass
print("CWhitebox::test_c_round: ok")
