...
# lib = "decimal"
# dimension = "behavior"
# case = "py_context_subclassing__test_context_subclassing"
# subject = "cpython.test_decimal.PyContextSubclassing.test_context_subclassing"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::PyContextSubclassing::test_context_subclassing
"""Auto-ported test: PyContextSubclassing::test_context_subclassing (CPython 3.12 oracle)."""
...
class MyContext(Context):
    ...
    def __init__(self, prec=None, rounding=None, Emin=None, Emax=None, capitals=None, clamp=None, flags=None, traps=None):
...
for attr in ('prec', 'rounding', 'Emin', 'Emax', 'capitals', 'clamp', 'flags', 'traps'):
    ...
    assert getattr(c, attr) == getattr(d, attr)
...
try:
    c.quantize(Decimal('9e2'), 0)
    raise AssertionError('expected InvalidOperation')
except InvalidOperation:
    pass

try:
    MyContext(**{'rounding': 'XYZ'})
    raise AssertionError('expected TypeError')
except TypeError:
    pass
c = MyContext(rounding=ROUND_DOWN, prec=1)

assert c.rounding == ROUND_DOWN

assert c.plus(Decimal('9.9')) == 9
...
try:
    c.add(Decimal('1e99'), Decimal('2.234e2000'))
...
assert c.to_sci_string(x) == '1e+222'
...
assert str(x) == '1.000000000000000000000000000E+99'
...
for signal in (Rounded, DivisionByZero):
    ...
    assert c.traps[signal]
c.clear_traps()
for signal in OrderedSignals[decimal]:

    assert not c.traps[signal]
print("PyContextSubclassing::test_context_subclassing: ok")
