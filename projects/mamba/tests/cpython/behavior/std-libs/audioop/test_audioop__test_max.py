...
# lib = "audioop"
# dimension = "behavior"
# case = "test_audioop__test_max"
# subject = "cpython.test_audioop.TestAudioop.test_max"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_audioop.py::TestAudioop::test_max
"""Auto-ported test: TestAudioop::test_max (CPython 3.12 oracle)."""
...
for w in (1, 2, 3, 4):
    ...
    assert audioop.max(b'', w) == 0

    assert audioop.max(bytearray(), w) == 0

    assert audioop.max(memoryview(b''), w) == 0
    p = packs[w]

    assert audioop.max(p(5), w) == 5

    assert audioop.max(p(5, -8, -1), w) == 8

    assert audioop.max(p(maxvalues[w]), w) == maxvalues[w]

    assert audioop.max(p(minvalues[w]), w) == -minvalues[w]

    assert audioop.max(datas[w], w) == -minvalues[w]
print("TestAudioop::test_max: ok")
