...
# lib = "pydoc"
# dimension = "behavior"
# case = "test_internal_utilities__test_sys_path_adjustment_when_curdir_already_included"
# subject = "cpython.test_pydoc.TestInternalUtilities.test_sys_path_adjustment_when_curdir_already_included"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_pydoc.py::TestInternalUtilities::test_sys_path_adjustment_when_curdir_already_included
"""Auto-ported test: TestInternalUtilities::test_sys_path_adjustment_when_curdir_already_included (CPython 3.12 oracle)."""
...
# --- test body ---
def _get_revised_path(given_path, argv0=None):
    if argv0 is None:
        argv0 = self_argv0
...
def _get_starting_path():
    ...
    for spelling in self_curdir_spellings:
        for __ in range(clean_path.count(spelling)):
            clean_path.remove(spelling)
    return clean_path
tmpdir = tempfile.TemporaryDirectory()
self_argv0dir = tmpdir.name
self_argv0 = os.path.join(tmpdir.name, 'nonexistent')
pass
self_abs_curdir = abs_curdir = os.getcwd()
self_curdir_spellings = ['', os.curdir, abs_curdir]
clean_path = _get_starting_path()
for spelling in self_curdir_spellings:
    leading_curdir = [spelling] + clean_path

    assert _get_revised_path(leading_curdir) is None
    trailing_curdir = clean_path + [spelling]

    assert _get_revised_path(trailing_curdir) is None
    leading_argv0dir = [self_argv0dir] + leading_curdir

    assert _get_revised_path(leading_argv0dir) is None
    trailing_argv0dir = trailing_curdir + [self_argv0dir]

    assert _get_revised_path(trailing_argv0dir) is None
print("TestInternalUtilities::test_sys_path_adjustment_when_curdir_already_included: ok")
