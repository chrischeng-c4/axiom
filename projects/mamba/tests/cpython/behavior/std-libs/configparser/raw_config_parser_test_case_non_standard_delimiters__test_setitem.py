...
# lib = "configparser"
# dimension = "behavior"
# case = "raw_config_parser_test_case_non_standard_delimiters__test_setitem"
# subject = "cpython.test_configparser.RawConfigParserTestCaseNonStandardDelimiters.test_setitem"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_configparser.py::RawConfigParserTestCaseNonStandardDelimiters::test_setitem
"""Auto-ported test: RawConfigParserTestCaseNonStandardDelimiters::test_setitem (CPython 3.12 oracle)."""
...
def parse_error(cf, exc, src):
    ...
    try:
        ...
        cm = _types_aR.SimpleNamespace(exception=_aR_e)
    return cm.exception
cf = fromstring('\n            [section1]\n            name1 {0[0]} value1\n            [section2]\n            name2 {0[0]} value2\n            [section3]\n            name3 {0[0]} value3\n        '.format(delimiters), defaults={'nameD': 'valueD'})

assert set(cf['section1'].keys()) == {'name1', 'named'}

assert set(cf['section2'].keys()) == {'name2', 'named'}

assert set(cf['section3'].keys()) == {'name3', 'named'}

assert cf['section1']['name1'] == 'value1'

assert cf['section2']['name2'] == 'value2'

assert cf['section3']['name3'] == 'value3'

assert cf.sections() == ['section1', 'section2', 'section3']
cf['section2'] = {'name22': 'value22'}

assert set(cf['section2'].keys()) == {'name22', 'named'}

assert cf['section2']['name22'] == 'value22'

assert 'name2' not in cf['section2']

assert cf.sections() == ['section1', 'section2', 'section3']
cf['section3'] = {}

assert set(cf['section3'].keys()) == {'named'}

assert 'name3' not in cf['section3']

assert cf.sections() == ['section1', 'section2', 'section3']
cf[default_section] = cf[default_section]

assert set(cf[default_section].keys()) != set()
cf[default_section] = {}

assert set(cf[default_section].keys()) == set()

assert set(cf['section1'].keys()) == {'name1'}

assert set(cf['section2'].keys()) == {'name22'}

assert set(cf['section3'].keys()) == set()

assert cf.sections() == ['section1', 'section2', 'section3']
cf['section2'] = cf['section2']

assert set(cf['section2'].keys()) == {'name22'}
print("RawConfigParserTestCaseNonStandardDelimiters::test_setitem: ok")
