...
# lib = "traceback"
# dimension = "behavior"
# case = "traceback_cases__test_caret"
# subject = "cpython.test_traceback.TracebackCases.test_caret"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_traceback.py::TracebackCases::test_caret
"""Auto-ported test: TracebackCases::test_caret (CPython 3.12 oracle)."""
...
def tokenizer_error_with_caret_range():
    compile('blech  (  ', '?', 'exec')
err = get_exception_format(syntax_error_with_caret, SyntaxError)

assert len(err) == 4

assert err[1].strip() == 'return x!'

assert '^' in err[2]

assert err[1].find('!') == err[2].find('^')

assert err[2].count('^') == 1
err = get_exception_format(syntax_error_with_caret_2, SyntaxError)

assert '^' in err[2]

assert err[2].count('\n') == 1

assert err[1].find('+') + 1 == err[2].find('^')

assert err[2].count('^') == 1
err = get_exception_format(syntax_error_with_caret_non_ascii, SyntaxError)
...
assert err[2].count('^') == 1
err = get_exception_format(syntax_error_with_caret_range, SyntaxError)

assert '^' in err[2]

assert err[2].count('\n') == 1

assert err[1].find('y') == err[2].find('^')

assert err[2].count('^') == len('y for y in range(30)')
err = get_exception_format(tokenizer_error_with_caret_range, SyntaxError)
...
assert err[2].count('^') == 1
print("TracebackCases::test_caret: ok")
