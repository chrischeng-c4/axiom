...
def tokenizer_error_with_caret_range():
    compile('blech  (  ', '?', 'exec')
err = get_exception_format(syntax_error_bad_indentation, IndentationError)

assert len(err) == 4

assert err[1].strip() == 'print(2)'

assert '^' in err[2]

assert err[1].find(')') + 1 == err[2].find('^')
err = get_exception_format(syntax_error_bad_indentation2, IndentationError)

assert len(err) == 3

assert err[1].strip() == 'print(2)'
print("TracebackCases::test_bad_indentation: ok")
