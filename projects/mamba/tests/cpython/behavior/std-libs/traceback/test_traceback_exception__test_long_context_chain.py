...
assert len(res) > sys.getrecursionlimit()

assert len([l for l in res if 'ZeroDivisionError:' in l]) > sys.getrecursionlimit() * 0.5

assert 'RecursionError: maximum recursion depth exceeded' in res[-1]
print("TestTracebackException::test_long_context_chain: ok")
