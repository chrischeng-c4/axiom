...
# lib = "ntpath"
# dimension = "behavior"
# case = "test_ntpath__test_ismount"
# subject = "cpython.test_ntpath.TestNtpath.test_ismount"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_ntpath.py::TestNtpath::test_ismount
"""Auto-ported test: TestNtpath::test_ismount (CPython 3.12 oracle)."""
...
with os_helper.temp_dir() as d:
    ...
    assert not ntpath.ismount(d)
if sys.platform == 'win32':
    test_cwd = os.getenv('SystemRoot')
    drive, path = ntpath.splitdrive(test_cwd)
    with os_helper.change_cwd(test_cwd):

        assert not ntpath.ismount(drive.lower())

        assert not ntpath.ismount(drive.upper())

    assert ntpath.ismount('\\\\localhost\\c$')

    assert ntpath.ismount('\\\\localhost\\c$\\')

    assert ntpath.ismount(b'\\\\localhost\\c$')

    assert ntpath.ismount(b'\\\\localhost\\c$\\')
print("TestNtpath::test_ismount: ok")
