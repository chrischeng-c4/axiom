...
# lib = "ntpath"
# dimension = "behavior"
# case = "path_like_tests__test_path_normcase"
# subject = "cpython.test_ntpath.PathLikeTests.test_path_normcase"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_ntpath.py::PathLikeTests::test_path_normcase
"""Auto-ported test: PathLikeTests::test_path_normcase (CPython 3.12 oracle)."""
...
def _check_function(func):
    assertPathEqual(func(self_file_path), func(self_file_name))
...
def assertPathIn(path, pathset):

    assert _norm(path) in _norm(pathset)
self_file_name = os_helper.TESTFN
self_file_path = FakePath(os_helper.TESTFN)
pass
with open(self_file_name, 'xb', 0) as file:
    file.write(b'test_ntpath.PathLikeTests')
_check_function(path.normcase)
if sys.platform == 'win32':

    assert ntpath.normcase('ΩΩ') == 'ωΩ'

    assert ntpath.normcase('abc\x00def') == 'abc\x00def'
print("PathLikeTests::test_path_normcase: ok")
