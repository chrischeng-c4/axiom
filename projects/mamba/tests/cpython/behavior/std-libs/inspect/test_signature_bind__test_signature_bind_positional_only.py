...
# lib = "inspect"
# dimension = "behavior"
# case = "test_signature_bind__test_signature_bind_positional_only"
# subject = "cpython.test_inspect.TestSignatureBind.test_signature_bind_positional_only"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_inspect.py::TestSignatureBind::test_signature_bind_positional_only
"""Auto-ported test: TestSignatureBind::test_signature_bind_positional_only (CPython 3.12 oracle)."""
...
assert call(test, 1, 2, 4, 5, bar=6) == (1, 2, 4, 5, 6, {})

assert call(test, 1, 2) == (1, 2, 3, 42, 50, {})

assert call(test, 1, 2, foo=4, bar=5) == (1, 2, 3, 4, 5, {})

assert call(test, 1, 2, foo=4, bar=5, c_po=10) == (1, 2, 3, 4, 5, {'c_po': 10})

assert call(test, 1, 2, 30, c_po=31, foo=4, bar=5) == (1, 2, 30, 4, 5, {'c_po': 31})

assert call(test, 1, 2, 30, foo=4, bar=5, c_po=31) == (1, 2, 30, 4, 5, {'c_po': 31})

assert call(test, 1, 2, c_po=4) == (1, 2, 3, 42, 50, {'c_po': 4})
try:
    call(test, a_po=1, b_po=2)
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
    import re as _re_aR
    assert _re_aR.search("missing a required positional-only argument: 'a_po'", str(_aR_e))

def without_var_kwargs(c_po=3, d_po=4, /):
    return (c_po, d_po)
try:
    call(without_var_kwargs, c_po=33, d_po=44)
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
    import re as _re_aR
    assert _re_aR.search("positional-only arguments passed as keyword arguments: 'c_po, d_po'", str(_aR_e))
print("TestSignatureBind::test_signature_bind_positional_only: ok")
