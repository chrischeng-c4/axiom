...
# lib = "inspect"
# dimension = "behavior"
# case = "test_parameter_object__test_signature_parameter_object"
# subject = "cpython.test_inspect.TestParameterObject.test_signature_parameter_object"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_inspect.py::TestParameterObject::test_signature_parameter_object
"""Auto-ported test: TestParameterObject::test_signature_parameter_object (CPython 3.12 oracle)."""
...
assert p.annotation is p.empty

assert p.kind == inspect.Parameter.POSITIONAL_ONLY
try:
    inspect.Parameter('foo', default=10, kind='123')
    raise AssertionError('expected ValueError')
except ValueError as _aR_e:
    import re as _re_aR
    assert _re_aR.search("value '123' is not a valid Parameter.kind", str(_aR_e))
try:
    inspect.Parameter('1', kind=inspect.Parameter.VAR_KEYWORD)
    raise AssertionError('expected ValueError')
except ValueError as _aR_e:
    import re as _re_aR
    assert _re_aR.search('not a valid parameter name', str(_aR_e))
try:
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search('name must be a str', str(_aR_e))
try:
    inspect.Parameter('$', kind=inspect.Parameter.VAR_KEYWORD)
    raise AssertionError('expected ValueError')
except ValueError as _aR_e:
    import re as _re_aR
    assert _re_aR.search('is not a valid parameter name', str(_aR_e))
try:
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search('cannot have default values', str(_aR_e))
try:
...
assert repr(p).startswith('<Parameter')

assert '"a=42"' in repr(p)
print("TestParameterObject::test_signature_parameter_object: ok")
