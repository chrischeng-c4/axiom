...
# lib = "inspect"
# dimension = "behavior"
# case = "test_signature_bind__test_signature_bind_kwonly"
# subject = "cpython.test_inspect.TestSignatureBind.test_signature_bind_kwonly"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_inspect.py::TestSignatureBind::test_signature_bind_kwonly
"""Auto-ported test: TestSignatureBind::test_signature_bind_kwonly (CPython 3.12 oracle)."""
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search('too many positional arguments', str(_aR_e))

assert call(test, foo=1) == 1
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search("missing a required argument: 'bar'", str(_aR_e))

def test(foo, *, bar):
    return (foo, bar)

assert call(test, 1, bar=2) == (1, 2)

assert call(test, bar=2, foo=1) == (1, 2)
try:
    call(test, bar=2, foo=1, spam=10)
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
    import re as _re_aR
    assert _re_aR.search("got an unexpected keyword argument 'spam'", str(_aR_e))
try:
    call(test, 1, 2)
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search("missing a required keyword-only argument: 'bar'", str(_aR_e))
...
assert call(test, foo=1, bar=2) == (1, 2, {})

assert call(test, 1, bar=2, spam='ham') == (1, 2, {'spam': 'ham'})

assert call(test, spam='ham', foo=1, bar=2) == (1, 2, {'spam': 'ham'})
try:
    call(test, spam='ham', bar=2)
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
    import re as _re_aR
    assert _re_aR.search("missing a required argument: 'foo'", str(_aR_e))

assert call(test, 1, bar=2, bin=1, spam=10) == (1, 2, {'bin': 1, 'spam': 10})
print("TestSignatureBind::test_signature_bind_kwonly: ok")
