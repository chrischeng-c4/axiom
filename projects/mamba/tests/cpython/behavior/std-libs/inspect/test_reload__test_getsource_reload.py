...
# lib = "inspect"
# dimension = "behavior"
# case = "test_reload__test_getsource_reload"
# subject = "cpython.test_inspect.TestReload.test_getsource_reload"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_inspect.py::TestReload::test_getsource_reload
"""Auto-ported test: TestReload::test_getsource_reload (CPython 3.12 oracle)."""
...
# --- test body ---
src_before = textwrap.dedent('def foo():\n    print("Bla")\n    ')
src_after = textwrap.dedent('def foo():\n    print("Oh no!")\n    ')

def assertInspectEqual(path, source):
    inspected_src = inspect.getsource(source)
    with open(path, encoding='utf-8') as src:

        assert src.read().splitlines(True) == inspected_src.splitlines(True)
with ready_to_import('reload_bug', src_before) as (name, path):
    module = importlib.import_module(name)
    assertInspectEqual(path, module)
    with open(path, 'w', encoding='utf-8') as src:
        src.write(src_after)
    assertInspectEqual(path, module)
print("TestReload::test_getsource_reload: ok")
