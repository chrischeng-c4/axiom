...
# lib = "inspect"
# dimension = "behavior"
# case = "test_signature_object__test_signature_annotations_with_local_namespaces"
# subject = "cpython.test_inspect.TestSignatureObject.test_signature_annotations_with_local_namespaces"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_inspect.py::TestSignatureObject::test_signature_annotations_with_local_namespaces
"""Auto-ported test: TestSignatureObject::test_signature_annotations_with_local_namespaces (CPython 3.12 oracle)."""
...
for signature_func in (inspect.signature, inspect.Signature.from_callable):
    ...
    assert sig1.return_annotation == int

    assert sig1.parameters['foo'].annotation == Foo
    sig2 = signature_func(func, locals=locals())

    assert sig2.return_annotation == int

    assert sig2.parameters['foo'].annotation == Foo
    sig3 = signature_func(func2, globals={'Bar': int}, locals=locals())

    assert sig3.return_annotation == int

    assert sig3.parameters['foo'].annotation == Foo

    assert sig3.parameters['bar'].annotation == 'Bar'
print("TestSignatureObject::test_signature_annotations_with_local_namespaces: ok")
