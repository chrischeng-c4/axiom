...
# lib = "pathlib"
# dimension = "behavior"
# case = "walk_tests__test_walk_bottom_up"
# subject = "cpython.test_pathlib.WalkTests.test_walk_bottom_up"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_pathlib.py::WalkTests::test_walk_bottom_up
"""Auto-ported test: WalkTests::test_walk_bottom_up (CPython 3.12 oracle)."""
...
for path, dirnames, filenames in self_walk_path.walk(top_down=False):
    if path == self_walk_path:

        assert not seen_testfn

        assert seen_sub1

        assert seen_sub2

        assert sorted(dirnames) == ['SUB1', 'SUB2']

        assert filenames == ['tmp1']
        seen_testfn = True
    elif path == self_sub1_path:

        assert not seen_testfn

        assert not seen_sub1

        assert seen_sub11

        assert dirnames == ['SUB11']

        assert filenames == ['tmp2']
        seen_sub1 = True
    elif path == self_sub11_path:

        assert not seen_sub1

        assert not seen_sub11

        assert dirnames == []

        assert filenames == []
        seen_sub11 = True
    elif path == self_sub2_path:

        assert not seen_testfn

        assert not seen_sub2

        assert sorted(dirnames) == sorted(self_sub2_tree[1])

        assert sorted(filenames) == sorted(self_sub2_tree[2])
        seen_sub2 = True
    else:
        raise AssertionError(f'Unexpected path: {path}')

assert seen_testfn
print("WalkTests::test_walk_bottom_up: ok")
