...
# lib = "pathlib"
# dimension = "behavior"
# case = "walk_tests__test_walk_topdown"
# subject = "cpython.test_pathlib.WalkTests.test_walk_topdown"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_pathlib.py::WalkTests::test_walk_topdown
"""Auto-ported test: WalkTests::test_walk_topdown (CPython 3.12 oracle)."""
...
walker = self_walk_path.walk()
entry = next(walker)
entry[1].sort()

assert entry == (self_walk_path, ['SUB1', 'SUB2'], ['tmp1'])
entry = next(walker)

assert entry == (self_sub1_path, ['SUB11'], ['tmp2'])
entry = next(walker)

assert entry == (self_sub11_path, [], [])
entry = next(walker)
entry[1].sort()
entry[2].sort()

assert entry == self_sub2_tree
try:
    next(walker)
    raise AssertionError('expected StopIteration')
except StopIteration:
    pass
print("WalkTests::test_walk_topdown: ok")
