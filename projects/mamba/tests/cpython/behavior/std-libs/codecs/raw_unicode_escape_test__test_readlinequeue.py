...
# lib = "codecs"
# dimension = "behavior"
# case = "raw_unicode_escape_test__test_readlinequeue"
# subject = "cpython.test_codecs.RawUnicodeEscapeTest.test_readlinequeue"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_codecs.py::RawUnicodeEscapeTest::test_readlinequeue
"""Auto-ported test: RawUnicodeEscapeTest::test_readlinequeue (CPython 3.12 oracle)."""
...
assert reader.readline(keepends=False) == 'foo'
writer.write('\nbar\r')

assert reader.readline(keepends=False) == ''

assert reader.readline(keepends=False) == 'bar'
writer.write('baz')

assert reader.readline(keepends=False) == 'baz'

assert reader.readline(keepends=False) == ''
writer.write('foo\r')

assert reader.readline(keepends=True) == 'foo\r'
writer.write('\nbar\r')

assert reader.readline(keepends=True) == '\n'

assert reader.readline(keepends=True) == 'bar\r'
writer.write('baz')

assert reader.readline(keepends=True) == 'baz'

assert reader.readline(keepends=True) == ''
writer.write('foo\r\n')

assert reader.readline(keepends=True) == 'foo\r\n'
print("RawUnicodeEscapeTest::test_readlinequeue: ok")
