...
# lib = "collections"
# dimension = "behavior"
# case = "test_one_trick_pony_ab_cs__test_callable"
# subject = "cpython.test_collections.TestOneTrickPonyABCs.test_Callable"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_collections.py::TestOneTrickPonyABCs::test_Callable
"""Auto-ported test: TestOneTrickPonyABCs::test_Callable (CPython 3.12 oracle)."""
...
for x in non_samples:
    ...
    assert not isinstance(x, Callable)

    assert not issubclass(type(x), Callable)
samples = [lambda: None, type, int, object, len, list.append, [].append]
for x in samples:

    assert isinstance(x, Callable)

    assert issubclass(type(x), Callable)
validate_abstract_methods(Callable, '__call__')
validate_isinstance(Callable, '__call__')
print("TestOneTrickPonyABCs::test_Callable: ok")
