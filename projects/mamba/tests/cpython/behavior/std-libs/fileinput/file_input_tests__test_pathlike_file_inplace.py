...
# lib = "fileinput"
# dimension = "behavior"
# case = "file_input_tests__test_pathlike_file_inplace"
# subject = "cpython.test_fileinput.FileInputTests.test_pathlike_file_inplace"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_fileinput.py::FileInputTests::test_pathlike_file_inplace
"""Auto-ported test: FileInputTests::test_pathlike_file_inplace (CPython 3.12 oracle)."""
...
def writeTmp(content, *, mode='w'):
    ...
    with open(fd, mode, encoding=encoding) as f:
        f.write(content)
    return name
t1 = FakePath(writeTmp('Path-like file.'))
with FileInput(t1, inplace=True, encoding='utf-8') as fi:
    line = fi.readline()

    assert line == 'Path-like file.'
    print('Modified %s' % line)
with open(t1, encoding='utf-8') as f:

    assert f.read() == 'Modified Path-like file.\n'
print("FileInputTests::test_pathlike_file_inplace: ok")
