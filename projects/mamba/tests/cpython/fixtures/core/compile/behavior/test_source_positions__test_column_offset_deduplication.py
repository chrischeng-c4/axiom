...
# lib = "compile"
# dimension = "behavior"
# case = "test_source_positions__test_column_offset_deduplication"
# subject = "cpython.test_compile.TestSourcePositions.test_column_offset_deduplication"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_compile.py::TestSourcePositions::test_column_offset_deduplication
"""Auto-ported test: TestSourcePositions::test_column_offset_deduplication (CPython 3.12 oracle)."""
...
for source in ['lambda: a', '(a for b in c)']:
    code = compile(f'{source}, {source}', '<test>', 'eval')

    assert len(code.co_consts) == 2

    assert isinstance(code.co_consts[0], types.CodeType)

    assert isinstance(code.co_consts[1], types.CodeType)

    assert code.co_consts[0] != code.co_consts[1]

    assert list(code.co_consts[0].co_positions()) != list(code.co_consts[1].co_positions())
print("TestSourcePositions::test_column_offset_deduplication: ok")
