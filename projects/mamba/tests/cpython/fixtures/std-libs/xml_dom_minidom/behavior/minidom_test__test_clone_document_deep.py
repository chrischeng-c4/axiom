...
# lib = "xml_dom_minidom"
# dimension = "behavior"
# case = "minidom_test__test_clone_document_deep"
# subject = "cpython.test_minidom.MinidomTest.testCloneDocumentDeep"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_minidom.py::MinidomTest::testCloneDocumentDeep
"""Auto-ported test: MinidomTest::testCloneDocumentDeep (CPython 3.12 oracle)."""
...
confirm(not (doc.isSameNode(doc2) or doc2.isSameNode(doc)), 'testCloneDocumentDeep: document objects not distinct')
confirm(len(doc.childNodes) == len(doc2.childNodes), 'testCloneDocumentDeep: wrong number of Document children')
confirm(doc2.documentElement.nodeType == Node.ELEMENT_NODE, 'testCloneDocumentDeep: documentElement not an ELEMENT_NODE')
confirm(doc2.documentElement.ownerDocument.isSameNode(doc2), 'testCloneDocumentDeep: documentElement owner is not new document')
confirm(not doc.documentElement.isSameNode(doc2.documentElement), 'testCloneDocumentDeep: documentElement should not be shared')
if doc.doctype is not None:
    confirm(doc2.doctype.nodeType == Node.DOCUMENT_TYPE_NODE, 'testCloneDocumentDeep: doctype not a DOCUMENT_TYPE_NODE')
    confirm(doc2.doctype.ownerDocument.isSameNode(doc2))
    confirm(not doc.doctype.isSameNode(doc2.doctype))
print("MinidomTest::testCloneDocumentDeep: ok")
