...
# lib = "posix"
# dimension = "behavior"
# case = "test_posix_weaklinking__test_listdir_scandir"
# subject = "cpython.test_posix.TestPosixWeaklinking.test_listdir_scandir"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_posix.py::TestPosixWeaklinking::test_listdir_scandir
"""Auto-ported test: TestPosixWeaklinking::test_listdir_scandir (CPython 3.12 oracle)."""
...
_verify_available('HAVE_FDOPENDIR')
if self_mac_ver >= (10, 10):

    assert 'HAVE_FDOPENDIR' in posix._have_functions
else:

    assert 'HAVE_FDOPENDIR' not in posix._have_functions
    try:
        os.listdir(0)
        raise AssertionError('expected TypeError')
    except TypeError as _aR_e:
        import re as _re_aR
        assert _re_aR.search('listdir: path should be string, bytes, os.PathLike or None, not int', str(_aR_e))
    try:
        os.scandir(0)
        raise AssertionError('expected TypeError')
    except TypeError as _aR_e:
        import re as _re_aR
        assert _re_aR.search('scandir: path should be string, bytes, os.PathLike or None, not int', str(_aR_e))
print("TestPosixWeaklinking::test_listdir_scandir: ok")
