...
# lib = "strptime"
# dimension = "behavior"
# case = "strptime_tests__test_date_locale"
# subject = "cpython.test_strptime.StrptimeTests.test_date_locale"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_strptime.py::StrptimeTests::test_date_locale
"""Auto-ported test: StrptimeTests::test_date_locale (CPython 3.12 oracle)."""
...
now = time.time()
roundtrip('%x', slice(0, 3), time.localtime(now))
for i in range(1, 7):
    roundtrip('%x', slice(0, 3), time.localtime(now - i * 24 * 3600))
for i in range(1, 12):
    roundtrip('%x', slice(0, 3), time.localtime(now - i * 30 * 24 * 3600))
roundtrip('%x', slice(0, 3), time.localtime(now - 366 * 24 * 3600))
print("StrptimeTests::test_date_locale: ok")
