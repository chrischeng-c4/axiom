...
# lib = "fileinput"
# dimension = "behavior"
# case = "file_input_tests__test_readline"
# subject = "cpython.test_fileinput.FileInputTests.test_readline"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_fileinput.py::FileInputTests::test_readline
"""Auto-ported test: FileInputTests::test_readline (CPython 3.12 oracle)."""
...
with FileInput(files=TESTFN, openhook=hook_encoded('ascii')) as fi:
    try:
        ...
        assert fi.readline() == 'A\n'

        assert fi.readline() == 'B\n'

        assert fi.readline() == 'C\n'
    except UnicodeDecodeError:

        raise AssertionError('Read to end of file')
    try:
        list(fi)
    ...
    assert fi.readline() == ''
print("FileInputTests::test_readline: ok")
