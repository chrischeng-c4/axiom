...
# lib = "inspect"
# dimension = "behavior"
# case = "test_parameter_object__test_signature_parameter_replace"
# subject = "cpython.test_inspect.TestParameterObject.test_signature_parameter_replace"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_inspect.py::TestParameterObject::test_signature_parameter_replace
"""Auto-ported test: TestParameterObject::test_signature_parameter_replace (CPython 3.12 oracle)."""
...
assert p is not p.replace()

assert p == p.replace()
p2 = p.replace(annotation=1)

assert p2.annotation == 1
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search('name is a required attribute', str(_aR_e))
...
assert p2.default is p2.empty
p2 = p2.replace(default=42, kind=p2.POSITIONAL_OR_KEYWORD)

assert p2.kind == p2.POSITIONAL_OR_KEYWORD
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search("value <class 'inspect._empty'> is not a valid Parameter.kind", str(_aR_e))
p2 = p2.replace(kind=p2.KEYWORD_ONLY)

assert p2 == p
print("TestParameterObject::test_signature_parameter_replace: ok")
