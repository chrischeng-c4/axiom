...
# lib = "shutil"
# dimension = "behavior"
# case = "test_archives__test_make_zipfile_rootdir_nodir"
# subject = "cpython.test_shutil.TestArchives.test_make_zipfile_rootdir_nodir"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_shutil.py::TestArchives::test_make_zipfile_rootdir_nodir
"""Auto-ported test: TestArchives::test_make_zipfile_rootdir_nodir (CPython 3.12 oracle)."""
...
# --- test body ---
def _create_files(base_dir='dist'):
    root_dir = mkdtemp()
...
def check_unpack_archive(format, **kwargs):
    check_unpack_archive_with_converter(format, lambda path: path, **kwargs)
    check_unpack_archive_with_converter(format, FakePath, **kwargs)
    check_unpack_archive_with_converter(format, FakePath, **kwargs)

def check_unpack_archive_with_converter(format, converter, **kwargs):
    root_dir, base_dir = _create_files()
    expected = rlistdir(root_dir)
    expected.remove('outer')
    base_name = os.path.join(mkdtemp(), 'archive')
    ...
    tmpdir3 = mkdtemp()
    unpack_archive(converter(filename), converter(tmpdir3), format=format, **kwargs)

    assert rlistdir(tmpdir3) == expected
    try:
        unpack_archive(converter(TESTFN), **kwargs)
        raise AssertionError('expected shutil.ReadError')
...
def check_unpack_tarball(format):
    check_unpack_archive(format, filter='fully_trusted')
    check_unpack_archive(format, filter='data')
    with warnings_helper.check_warnings(('Python 3.14', DeprecationWarning)):
        check_unpack_archive(format)
...
for dry_run in (False, True):
    ...
    assert not os.path.exists(f'{TESTFN}.zip')
    ...
    except NotADirectoryError as _aR_e:
        import types as _types_aR
        cm = _types_aR.SimpleNamespace(exception=_aR_e)

    assert cm.exception.errno == errno.ENOTDIR

    assert cm.exception.filename == tmp_file

    assert not os.path.exists(f'{TESTFN}.zip')
print("TestArchives::test_make_zipfile_rootdir_nodir: ok")
