...
# lib = "shutil"
# dimension = "behavior"
# case = "test_copy_tree__test_copytree_retains_permissions"
# subject = "cpython.test_shutil.TestCopyTree.test_copytree_retains_permissions"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_shutil.py::TestCopyTree::test_copytree_retains_permissions
"""Auto-ported test: TestCopyTree::test_copytree_retains_permissions (CPython 3.12 oracle)."""
...
assert os.stat(src_dir).st_mode == os.stat(dst_dir).st_mode

assert os.stat(os.path.join(src_dir, 'permissive.txt')).st_mode == os.stat(os.path.join(dst_dir, 'permissive.txt')).st_mode

assert os.stat(os.path.join(src_dir, 'restrictive.txt')).st_mode == os.stat(os.path.join(dst_dir, 'restrictive.txt')).st_mode
restrictive_subdir_dst = os.path.join(dst_dir, os.path.split(restrictive_subdir)[1])

assert os.stat(restrictive_subdir).st_mode == os.stat(restrictive_subdir_dst).st_mode
print("TestCopyTree::test_copytree_retains_permissions: ok")
