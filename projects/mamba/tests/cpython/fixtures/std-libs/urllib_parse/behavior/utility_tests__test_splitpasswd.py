...
# lib = "urllib_parse"
# dimension = "behavior"
# case = "utility_tests__test_splitpasswd"
# subject = "cpython.test_urlparse.Utility_Tests.test_splitpasswd"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_urlparse.py::Utility_Tests::test_splitpasswd
"""Auto-ported test: Utility_Tests::test_splitpasswd (CPython 3.12 oracle)."""
...
assert splitpasswd('user:ab') == ('user', 'ab')

assert splitpasswd('user:a\nb') == ('user', 'a\nb')

assert splitpasswd('user:a\tb') == ('user', 'a\tb')

assert splitpasswd('user:a\rb') == ('user', 'a\rb')

assert splitpasswd('user:a\x0cb') == ('user', 'a\x0cb')

assert splitpasswd('user:a\x0bb') == ('user', 'a\x0bb')

assert splitpasswd('user:a:b') == ('user', 'a:b')

assert splitpasswd('user:a b') == ('user', 'a b')

assert splitpasswd('user 2:ab') == ('user 2', 'ab')

assert splitpasswd('user+1:a+b') == ('user+1', 'a+b')

assert splitpasswd('user:') == ('user', '')

assert splitpasswd('user') == ('user', None)

assert splitpasswd(':ab') == ('', 'ab')
print("Utility_Tests::test_splitpasswd: ok")
