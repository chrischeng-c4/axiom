...
# lib = "urllib2"
# dimension = "behavior"
# case = "request_hdrs_tests__test_request_headers_methods"
# subject = "cpython.test_urllib2.RequestHdrsTests.test_request_headers_methods"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_urllib2.py::RequestHdrsTests::test_request_headers_methods
"""Auto-ported test: RequestHdrsTests::test_request_headers_methods (CPython 3.12 oracle)."""
...
assert req.has_header('Spam-eggs')

assert req.header_items() == [('Spam-eggs', 'blah')]
req.add_header('Foo-Bar', 'baz')

assert sorted(req.header_items()) == [('Foo-bar', 'baz'), ('Spam-eggs', 'blah')]

assert not req.has_header('Not-there')

assert req.get_header('Not-there') is None

assert req.get_header('Not-there', 'default') == 'default'
req.remove_header('Spam-eggs')

assert not req.has_header('Spam-eggs')
req.add_unredirected_header('Unredirected-spam', 'Eggs')

assert req.has_header('Unredirected-spam')
req.remove_header('Unredirected-spam')

assert not req.has_header('Unredirected-spam')
print("RequestHdrsTests::test_request_headers_methods: ok")
