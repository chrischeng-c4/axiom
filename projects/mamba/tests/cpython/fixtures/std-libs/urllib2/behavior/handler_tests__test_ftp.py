...
# lib = "urllib2"
# dimension = "behavior"
# case = "handler_tests__test_ftp"
# subject = "cpython.test_urllib2.HandlerTests.test_ftp"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_urllib2.py::HandlerTests::test_ftp
"""Auto-ported test: HandlerTests::test_ftp (CPython 3.12 oracle)."""
...
h.parent = MockOpener()
for url, host, port, user, passwd, type_, dirs, filename, mimetype in [('ftp://localhost/foo/bar/baz.html', 'localhost', ftplib.FTP_PORT, '', '', 'I', ['foo', 'bar'], 'baz.html', 'text/html'), ('ftp://parrot@localhost/foo/bar/baz.html', 'localhost', ftplib.FTP_PORT, 'parrot', '', 'I', ['foo', 'bar'], 'baz.html', 'text/html'), ('ftp://%25parrot@localhost/foo/bar/baz.html', 'localhost', ftplib.FTP_PORT, '%parrot', '', 'I', ['foo', 'bar'], 'baz.html', 'text/html'), ('ftp://%2542parrot@localhost/foo/bar/baz.html', 'localhost', ftplib.FTP_PORT, '%42parrot', '', 'I', ['foo', 'bar'], 'baz.html', 'text/html'), ('ftp://localhost:80/foo/bar/', 'localhost', 80, '', '', 'D', ['foo', 'bar'], '', None), ('ftp://localhost/baz.gif;type=a', 'localhost', ftplib.FTP_PORT, '', '', 'A', [], 'baz.gif', 'image/gif')]:
    req = Request(url)
    req.timeout = None
    r = h.ftp_open(req)
    ...
    assert h.passwd == passwd

    assert h.host == socket.gethostbyname(host)

    assert h.port == port

    assert h.dirs == dirs

    assert h.ftpwrapper.filename == filename

    assert h.ftpwrapper.filetype == type_
    headers = r.info()

    assert headers.get('Content-type') == mimetype

    assert int(headers['Content-length']) == len(data)
    r.close()
print("HandlerTests::test_ftp: ok")
