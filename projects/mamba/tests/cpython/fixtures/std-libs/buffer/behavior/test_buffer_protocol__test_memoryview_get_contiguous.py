...
# lib = "buffer"
# dimension = "behavior"
# case = "test_buffer_protocol__test_memoryview_get_contiguous"
# subject = "cpython.test_buffer.TestBufferProtocol.test_memoryview_get_contiguous"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_buffer.py::TestBufferProtocol::test_memoryview_get_contiguous
"""Auto-ported test: TestBufferProtocol::test_memoryview_get_contiguous (CPython 3.12 oracle)."""
...
try:
    get_contiguous({}, PyBUF_READ, 'F')
    raise AssertionError('expected TypeError')
except TypeError:
    pass

try:
    get_contiguous(b'x', PyBUF_WRITE, 'C')
...
try:
    get_contiguous(nd, PyBUF_WRITE, 'A')
    raise AssertionError('expected BufferError')
except BufferError:
    pass
...
for order in ['C', 'A']:
    ...
    assert ndarray(m).tolist() == nd.tolist()

try:
    get_contiguous(nd, PyBUF_WRITE, 'F')
    raise AssertionError('expected BufferError')
except BufferError:
    pass
m = get_contiguous(nd, PyBUF_READ, order)
...
try:
    get_contiguous(nd, PyBUF_WRITE, 'C')
    raise AssertionError('expected BufferError')
except BufferError:
    pass
m = get_contiguous(nd, PyBUF_READ, order)
...
for order in ['C', 'F', 'A']:
    ...
    try:
        get_contiguous(nd, PyBUF_WRITE, order)
        raise AssertionError('expected BufferError')
    except BufferError:
        pass
    m = get_contiguous(nd, PyBUF_READ, order)
...
assert m.c_contiguous
print("TestBufferProtocol::test_memoryview_get_contiguous: ok")
