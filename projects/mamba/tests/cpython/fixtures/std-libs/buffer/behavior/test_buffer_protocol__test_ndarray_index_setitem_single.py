...
# lib = "buffer"
# dimension = "behavior"
# case = "test_buffer_protocol__test_ndarray_index_setitem_single"
# subject = "cpython.test_buffer.TestBufferProtocol.test_ndarray_index_setitem_single"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_buffer.py::TestBufferProtocol::test_ndarray_index_setitem_single
"""Auto-ported test: TestBufferProtocol::test_ndarray_index_setitem_single (CPython 3.12 oracle)."""
...
for fmt, items, single_item in iter_format(5):
    ...
    assert nd.tolist() == items

    try:
        nd.__setitem__(-6, single_item)
        raise AssertionError('expected IndexError')
    except IndexError:
        pass

    try:
        nd.__setitem__(5, single_item)
        raise AssertionError('expected IndexError')
    except IndexError:
        pass
    ...
    assert mv.tolist() == items

    try:
        mv.__setitem__(-6, single_item)
        raise AssertionError('expected IndexError')
    except IndexError:
        pass

    try:
        mv.__setitem__(5, single_item)
        raise AssertionError('expected IndexError')
    except IndexError:
        pass
for fmt, items, single_item in iter_format(5):
    ...
    assert mv.tolist() == items
print("TestBufferProtocol::test_ndarray_index_setitem_single: ok")
