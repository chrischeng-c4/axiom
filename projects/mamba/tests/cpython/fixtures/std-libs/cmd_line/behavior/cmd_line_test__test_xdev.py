...
# lib = "cmd_line"
# dimension = "behavior"
# case = "cmd_line_test__test_xdev"
# subject = "cpython.test_cmd_line.CmdLineTest.test_xdev"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_cmd_line.py::CmdLineTest::test_xdev
"""Auto-ported test: CmdLineTest::test_xdev (CPython 3.12 oracle)."""
...
out = run_xdev('-c', code, xdev=False)
...
out = run_xdev('-b', '-c', code)

assert out == f'default::BytesWarning {expected_filters}'
out = run_xdev('-bb', '-c', code)

assert out == f'error::BytesWarning {expected_filters}'
out = run_xdev('-Werror', '-c', code)

assert out == f'error::Warning {expected_filters}'
try:
    import _testcapi
except ImportError:
    pass
    ...
    with support.SuppressCrashReport():
        out = run_xdev('-c', code, check_exitcode=False)
    if support.with_pymalloc():
        alloc_name = 'pymalloc_debug'
    else:
        alloc_name = 'malloc_debug'

    assert out == alloc_name
try:
    import faulthandler
except ImportError:
    pass
    ...
    assert out == 'True'
print("CmdLineTest::test_xdev: ok")
