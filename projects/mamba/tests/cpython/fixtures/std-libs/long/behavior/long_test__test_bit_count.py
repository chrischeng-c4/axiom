...
# lib = "long"
# dimension = "behavior"
# case = "long_test__test_bit_count"
# subject = "cpython.test_long.LongTest.test_bit_count"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_long.py::LongTest::test_bit_count
"""Auto-ported test: LongTest::test_bit_count (CPython 3.12 oracle)."""
...
for a in range(-1000, 1000):
    ...
    assert a.bit_count() == bin(a).count('1')
for exp in [10, 17, 63, 64, 65, 1009, 70234, 1234567]:
    a = 2 ** exp

    assert a.bit_count() == 1

    assert (a - 1).bit_count() == exp

    assert (a ^ 63).bit_count() == 7

    assert (a - 1 ^ 510).bit_count() == exp - 8
print("LongTest::test_bit_count: ok")
