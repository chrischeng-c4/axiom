...
# lib = "re"
# dimension = "behavior"
# case = "re_tests__test_re_escape_bytes"
# subject = "cpython.test_re.ReTests.test_re_escape_bytes"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_re.py::ReTests::test_re_escape_bytes
"""Auto-ported test: ReTests::test_re_escape_bytes (CPython 3.12 oracle)."""
...
for i in p:
    ...
    assertMatch(b'[' + re.escape(b) + b']', b)
    assertMatch(b'(?x)' + re.escape(b), b)
assertMatch(re.escape(p), p)
for i in b'-.]{}':
    b = bytes([i])

    assert re.escape(b)[:1] == b'\\'
literal_chars = LITERAL_CHARS.encode('ascii')

assert re.escape(literal_chars) == literal_chars
print("ReTests::test_re_escape_bytes: ok")
