...
# lib = "re"
# dimension = "behavior"
# case = "re_tests__test_re_groupref_exists_errors"
# subject = "cpython.test_re.ReTests.test_re_groupref_exists_errors"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_re.py::ReTests::test_re_groupref_exists_errors
"""Auto-ported test: ReTests::test_re_groupref_exists_errors (CPython 3.12 oracle)."""
...
checkPatternError('(?P<a>)(?(0)a|b)', 'bad group number', 10)
checkPatternError('()(?(-1)a|b)', "bad character in group name '-1'", 5)
checkPatternError('()(?(+1)a|b)', "bad character in group name '+1'", 5)
checkPatternError('()' * 10 + '(?(1_0)a|b)', "bad character in group name '1_0'", 23)
checkPatternError('()(?( 1 )a|b)', "bad character in group name ' 1 '", 5)
checkPatternError('()(?(㊀)a|b)', "bad character in group name '㊀'", 5)
checkPatternError('()(?(¹)a|b)', "bad character in group name '¹'", 5)
checkPatternError('()(?(१)a|b)', "bad character in group name '१'", 5)
checkPatternError('()(?(1', 'missing ), unterminated name', 5)
checkPatternError('()(?(1)a', 'missing ), unterminated subpattern', 2)
checkPatternError('()(?(1)a|b', 'missing ), unterminated subpattern', 2)
checkPatternError('()(?(1)a|b|c', 'conditional backref with more than two branches', 10)
checkPatternError('()(?(1)a|b|c)', 'conditional backref with more than two branches', 10)
checkPatternError('()(?(2)a)', 'invalid group reference 2', 5)
print("ReTests::test_re_groupref_exists_errors: ok")
