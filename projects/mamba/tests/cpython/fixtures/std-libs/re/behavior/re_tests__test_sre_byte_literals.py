...
# lib = "re"
# dimension = "behavior"
# case = "re_tests__test_sre_byte_literals"
# subject = "cpython.test_re.ReTests.test_sre_byte_literals"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_re.py::ReTests::test_sre_byte_literals
"""Auto-ported test: ReTests::test_sre_byte_literals (CPython 3.12 oracle)."""
...
for i in [0, 8, 16, 32, 64, 127, 128, 255]:
    ...
    assert re.match(('\\%03o0' % i).encode(), bytes([i]) + b'0')

    assert re.match(('\\%03o8' % i).encode(), bytes([i]) + b'8')

    assert re.match(('\\x%02x' % i).encode(), bytes([i]))

    assert re.match(('\\x%02x0' % i).encode(), bytes([i]) + b'0')

    assert re.match(('\\x%02xz' % i).encode(), bytes([i]) + b'z')
...
assert re.match(b'\\0', b'\x00')

assert re.match(b'\\08', b'\x008')

assert re.match(b'\\01', b'\x01')

assert re.match(b'\\018', b'\x018')
checkPatternError(b'\\567', 'octal escape value \\567 outside of range 0-0o377', 0)
checkPatternError(b'\\911', 'invalid group reference 91', 1)
checkPatternError(b'\\x1', 'incomplete escape \\x1', 0)
checkPatternError(b'\\x1z', 'incomplete escape \\x1', 0)
print("ReTests::test_sre_byte_literals: ok")
