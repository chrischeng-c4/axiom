...
# lib = "statistics"
# dimension = "behavior"
# case = "test_quantiles__test_specific_cases"
# subject = "cpython.test_statistics.TestQuantiles.test_specific_cases"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_statistics.py::TestQuantiles::test_specific_cases
"""Auto-ported test: TestQuantiles::test_specific_cases (CPython 3.12 oracle)."""
...
for n, expected in [(1, []), (2, [250.0]), (3, [200.0, 320.0]), (4, [160.0, 250.0, 335.0]), (5, [136.0, 220.0, 292.0, 344.0]), (6, [120.0, 200.0, 250.0, 320.0, 350.0]), (8, [100.0, 160.0, 212.5, 250.0, 302.5, 335.0, 357.5]), (10, [88.0, 136.0, 184.0, 220.0, 250.0, 292.0, 326.0, 344.0, 362.0]), (12, [80.0, 120.0, 160.0, 200.0, 225.0, 250.0, 285.0, 320.0, 335.0, 350.0, 365.0]), (15, [72.0, 104.0, 136.0, 168.0, 200.0, 220.0, 240.0, 264.0, 292.0, 320.0, 332.0, 344.0, 356.0, 368.0])]:
    ...
    assert expected == quantiles(data, n=n)

    assert len(quantiles(data, n=n)) == n - 1
    ...
    if len(expected) >= 2:
        ...
        assert quantiles(expected, n=n) == expected
    sdata = sorted(data)
    lo = 2 * sdata[0] - sdata[1]
    hi = 2 * sdata[-1] - sdata[-2]
    padded_data = data + [lo, hi]

    assert quantiles(data, n=n) == quantiles(padded_data, n=n, method='inclusive')
...
print("TestQuantiles::test_specific_cases: ok")
