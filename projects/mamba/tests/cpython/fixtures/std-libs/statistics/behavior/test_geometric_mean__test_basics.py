...
# lib = "statistics"
# dimension = "behavior"
# case = "test_geometric_mean__test_basics"
# subject = "cpython.test_statistics.TestGeometricMean.test_basics"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_statistics.py::TestGeometricMean::test_basics
"""Auto-ported test: TestGeometricMean::test_basics (CPython 3.12 oracle)."""
...
assert abs(geometric_mean([54, 24, 36]) - 36.0) < 1e-07

assert abs(geometric_mean([4.0, 9.0]) - 6.0) < 1e-07

assert abs(geometric_mean([17.625]) - 17.625) < 1e-07
...
for rng in [range(1, 100), range(1, 1000), range(1, 10000), range(500, 10000, 3), range(10000, 500, -3), [12, 17, 13, 5, 120, 7], [random.expovariate(50.0) for i in range(1000)], [random.lognormvariate(20.0, 3.0) for i in range(2000)], [random.triangular(2000, 3000, 2200) for i in range(3000)]]:
    ...
    assert math.isclose(gm_float, float(gm_decimal))
print("TestGeometricMean::test_basics: ok")
