...
# lib = "httpservers"
# dimension = "behavior"
# case = "base_http_request_handler_test_case__test_unprintable_not_logged"
# subject = "cpython.test_httpservers.BaseHTTPRequestHandlerTestCase.test_unprintable_not_logged"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_httpservers.py::BaseHTTPRequestHandlerTestCase::test_unprintable_not_logged
"""Auto-ported test: BaseHTTPRequestHandlerTestCase::test_unprintable_not_logged (CPython 3.12 oracle)."""
...
self_handler.client_address = ('127.0.0.1', 1337)
log_message = BaseHTTPRequestHandler.log_message
with mock.patch.object(sys, 'stderr', StringIO()) as fake_stderr:
    log_message(self_handler, '/foo')
    log_message(self_handler, '/\x1bbar\x00\x1b')
    log_message(self_handler, '/spam %s.', 'a')
    log_message(self_handler, '/spam %s.', '\x1b\x7f\x9f\xa0beans')
    log_message(self_handler, '"GET /foo\\b"ar\x07 HTTP/1.0"')
...
assert '/\\x1bbar\\x00\\x1b' in lines[1]

assert '/spam a.' in lines[2]

assert '/spam \\x1b\\x7f\\x9f\xa0beans.' in lines[3]

assert '"GET /foo\\\\b"ar\\x07 HTTP/1.0"' in lines[4]
print("BaseHTTPRequestHandlerTestCase::test_unprintable_not_logged: ok")
