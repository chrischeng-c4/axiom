...
# lib = "format"
# dimension = "behavior"
# case = "format_test__test_optimisations"
# subject = "cpython.test_format.FormatTest.test_optimisations"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_format.py::FormatTest::test_optimisations
"""Auto-ported test: FormatTest::test_optimisations (CPython 3.12 oracle)."""
...
assert '%s' % text is text

assert '%.5s' % text is text

assert '%.10s' % text is text

assert '%1s' % text is text

assert '%5s' % text is text

assert '{0}'.format(text) is text

assert '{0:s}'.format(text) is text

assert '{0:.5s}'.format(text) is text

assert '{0:.10s}'.format(text) is text

assert '{0:1s}'.format(text) is text

assert '{0:5s}'.format(text) is text

assert text % () is text

assert text.format() is text
print("FormatTest::test_optimisations: ok")
