...
# lib = "audioop"
# dimension = "behavior"
# case = "test_audioop__test_tomono"
# subject = "cpython.test_audioop.TestAudioop.test_tomono"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_audioop.py::TestAudioop::test_tomono
"""Auto-ported test: TestAudioop::test_tomono (CPython 3.12 oracle)."""
...
for w in (1, 2, 3, 4):
    ...
    assert audioop.tomono(data2, w, 1, 0) == data1

    assert audioop.tomono(data2, w, 0, 1) == b'\x00' * len(data1)
    for k in range(w):
        data2[k + w::2 * w] = data1[k::w]

    assert audioop.tomono(data2, w, 0.5, 0.5) == data1

    assert audioop.tomono(bytearray(data2), w, 0.5, 0.5) == data1

    assert audioop.tomono(memoryview(data2), w, 0.5, 0.5) == data1
print("TestAudioop::test_tomono: ok")
