...
# lib = "audioop"
# dimension = "behavior"
# case = "test_audioop__test_maxpp"
# subject = "cpython.test_audioop.TestAudioop.test_maxpp"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_audioop.py::TestAudioop::test_maxpp
"""Auto-ported test: TestAudioop::test_maxpp (CPython 3.12 oracle)."""
...
for w in (1, 2, 3, 4):
    ...
    assert audioop.maxpp(b'', w) == 0

    assert audioop.maxpp(bytearray(), w) == 0

    assert audioop.maxpp(memoryview(b''), w) == 0

    assert audioop.maxpp(packs[w](*range(100)), w) == 0

    assert audioop.maxpp(packs[w](9, 10, 5, 5, 0, 1), w) == 10

    assert audioop.maxpp(datas[w], w) == maxvalues[w] - minvalues[w]
print("TestAudioop::test_maxpp: ok")
