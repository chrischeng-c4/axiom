...
# lib = "collections"
# dimension = "behavior"
# case = "test_counter__test_multiset_operations"
# subject = "cpython.test_collections.TestCounter.test_multiset_operations"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_collections.py::TestCounter::test_multiset_operations
"""Auto-ported test: TestCounter::test_multiset_operations (CPython 3.12 oracle)."""
...
for i in range(1000):
    ...
    for counterop, numberop in [(Counter.__add__, lambda x, y: max(0, x + y)), (Counter.__sub__, lambda x, y: max(0, x - y)), (Counter.__or__, lambda x, y: max(0, x, y)), (Counter.__and__, lambda x, y: max(0, min(x, y)))]:
        ...
        for x in elements:
            ...
            assert numberop(p[x], q[x]) == result[x]

        assert (x > 0 for x in result.values())
elements = 'abcdef'
for i in range(100):
...
print("TestCounter::test_multiset_operations: ok")
