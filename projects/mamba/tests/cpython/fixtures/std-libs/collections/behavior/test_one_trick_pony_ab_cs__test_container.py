...
# lib = "collections"
# dimension = "behavior"
# case = "test_one_trick_pony_ab_cs__test_container"
# subject = "cpython.test_collections.TestOneTrickPonyABCs.test_Container"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_collections.py::TestOneTrickPonyABCs::test_Container
"""Auto-ported test: TestOneTrickPonyABCs::test_Container (CPython 3.12 oracle)."""
...
for x in non_samples:
    ...
    assert not issubclass(type(x), Container)
samples = [bytes(), str(), tuple(), list(), set(), frozenset(), dict(), dict().keys(), dict().items()]
for x in samples:

    assert isinstance(x, Container)

    assert issubclass(type(x), Container)
validate_abstract_methods(Container, '__contains__')
validate_isinstance(Container, '__contains__')
print("TestOneTrickPonyABCs::test_Container: ok")
