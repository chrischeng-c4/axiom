...
# lib = "itertools"
# dimension = "behavior"
# case = "test_various_iterator_args__test_starmap"
# subject = "cpython.test_itertools.TestVariousIteratorArgs.test_starmap"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestVariousIteratorArgs::test_starmap
"""Auto-ported test: TestVariousIteratorArgs::test_starmap (CPython 3.12 oracle)."""
...
for s in (range(10), range(0), range(100), (7, 11), range(20, 50, 5)):
    for g in (G, I, Ig, S, L, R):
        ...
        assert list(starmap(operator.pow, g(ss))) == [x ** x for x in g(s)]

    try:
        starmap(operator.pow, X(ss))
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        starmap(operator.pow, N(ss))
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        list(starmap(operator.pow, E(ss)))
        raise AssertionError('expected ZeroDivisionError')
    except ZeroDivisionError:
        pass
print("TestVariousIteratorArgs::test_starmap: ok")
