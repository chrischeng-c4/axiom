...
# lib = "itertools"
# dimension = "behavior"
# case = "test_various_iterator_args__test_filterfalse"
# subject = "cpython.test_itertools.TestVariousIteratorArgs.test_filterfalse"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestVariousIteratorArgs::test_filterfalse
"""Auto-ported test: TestVariousIteratorArgs::test_filterfalse (CPython 3.12 oracle)."""
...
for s in (range(10), range(0), range(1000), (7, 11), range(2000, 2200, 5)):
    for g in (G, I, Ig, S, L, R):
        ...
        assert list(filterfalse(isEven, g(s))) == [x for x in g(s) if isOdd(x)]

    try:
        filterfalse(isEven, X(s))
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        filterfalse(isEven, N(s))
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    try:
        list(filterfalse(isEven, E(s)))
        raise AssertionError('expected ZeroDivisionError')
    except ZeroDivisionError:
        pass
print("TestVariousIteratorArgs::test_filterfalse: ok")
