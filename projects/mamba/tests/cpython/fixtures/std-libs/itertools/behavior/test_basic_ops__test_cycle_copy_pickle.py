...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_cycle_copy_pickle"
# subject = "cpython.test_itertools.TestBasicOps.test_cycle_copy_pickle"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_cycle_copy_pickle
"""Auto-ported test: TestBasicOps::test_cycle_copy_pickle (CPython 3.12 oracle)."""
...
assert take(10, copy.deepcopy(c)) == list('bcabcabcab')
for proto in range(pickle.HIGHEST_PROTOCOL + 1):

    assert take(10, pickle.loads(pickle.dumps(c, proto))) == list('bcabcabcab')
    next(c)

    assert take(10, pickle.loads(pickle.dumps(c, proto))) == list('cabcabcabc')
    next(c)
    next(c)
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    pickletest(proto, cycle('abc'))
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    it = iter('abcde')
    c = cycle(it)
    ...
    assert take(20, d) == list('cdeabcdeabcdeabcdeab')
    it = iter('abcde')
    c = cycle(it)
    ...
    assert take(20, d) == list('cdeabcdeabcdeabcdeab')
print("TestBasicOps::test_cycle_copy_pickle: ok")
