...
# lib = "itertools"
# dimension = "behavior"
# case = "test_basic_ops__test_starmap"
# subject = "cpython.test_itertools.TestBasicOps.test_starmap"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_itertools.py::TestBasicOps::test_starmap
"""Auto-ported test: TestBasicOps::test_starmap (CPython 3.12 oracle)."""
...
assert list(starmap(operator.pow, zip(range(3), range(1, 7)))) == [0 ** 1, 1 ** 2, 2 ** 3]

assert take(3, starmap(operator.pow, zip(count(), count(1)))) == [0 ** 1, 1 ** 2, 2 ** 3]

assert list(starmap(operator.pow, [])) == []

assert list(starmap(operator.pow, [iter([4, 5])])) == [4 ** 5]

try:
    list(starmap(operator.pow, [None]))
...
try:
    starmap(operator.pow, [(4, 5)], 'extra')
    raise AssertionError('expected TypeError')
except TypeError:
    pass
...
try:
    next(starmap(errfunc, [(4, 5)]))
    raise AssertionError('expected ValueError')
except ValueError:
    pass

try:
    next(starmap(onearg, [(4, 5)]))
    raise AssertionError('expected TypeError')
except TypeError:
    pass
ans = [0 ** 1, 1 ** 2, 2 ** 3]
...
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    c = starmap(operator.pow, zip(range(3), range(1, 7)))
    pickletest(proto, c)
print("TestBasicOps::test_starmap: ok")
