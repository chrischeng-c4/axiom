...
# lib = "typing"
# dimension = "behavior"
# case = "type_var_tests__test_attributes"
# subject = "cpython.test_typing.TypeVarTests.test_attributes"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::TypeVarTests::test_attributes
"""Auto-ported test: TypeVarTests::test_attributes (CPython 3.12 oracle)."""
...
assert T_bound.__name__ == 'T_bound'

assert T_bound.__constraints__ == ()

assert T_bound.__bound__ is int
T_constraints = TypeVar('T_constraints', int, str)

assert T_constraints.__name__ == 'T_constraints'

assert T_constraints.__constraints__ == (int, str)

assert T_constraints.__bound__ is None
T_co = TypeVar('T_co', covariant=True)

assert T_co.__name__ == 'T_co'

assert T_co.__covariant__ is True

assert T_co.__contravariant__ is False

assert T_co.__infer_variance__ is False
T_contra = TypeVar('T_contra', contravariant=True)

assert T_contra.__name__ == 'T_contra'

assert T_contra.__covariant__ is False

assert T_contra.__contravariant__ is True

assert T_contra.__infer_variance__ is False
T_infer = TypeVar('T_infer', infer_variance=True)

assert T_infer.__name__ == 'T_infer'

assert T_infer.__covariant__ is False

assert T_infer.__contravariant__ is False

assert T_infer.__infer_variance__ is True
print("TypeVarTests::test_attributes: ok")
