...
# lib = "typing"
# dimension = "behavior"
# case = "typed_dict_tests__test_generic_inheritance"
# subject = "cpython.test_typing.TypedDictTests.test_generic_inheritance"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::TypedDictTests::test_generic_inheritance
"""Auto-ported test: TypedDictTests::test_generic_inheritance (CPython 3.12 oracle)."""
...
assert A.__bases__ == (Generic, dict)

assert A.__orig_bases__ == (TypedDict, Generic[T])

assert A.__mro__ == (A, Generic, dict, object)

assert A.__parameters__ == (T,)

assert A[str].__parameters__ == ()

assert A[str].__args__ == (str,)

class A2(Generic[T], TypedDict):
    a: T

assert A2.__bases__ == (Generic, dict)

assert A2.__orig_bases__ == (Generic[T], TypedDict)

assert A2.__mro__ == (A2, Generic, dict, object)

assert A2.__parameters__ == (T,)

assert A2[str].__parameters__ == ()

assert A2[str].__args__ == (str,)

class B(A[KT], total=False):
    b: KT

assert B.__bases__ == (Generic, dict)

assert B.__orig_bases__ == (A[KT],)

assert B.__mro__ == (B, Generic, dict, object)

assert B.__parameters__ == (KT,)

assert B.__total__ == False

assert B.__optional_keys__ == frozenset(['b'])

assert B.__required_keys__ == frozenset(['a'])

assert B[str].__parameters__ == ()

assert B[str].__args__ == (str,)

assert B[str].__origin__ == B

class C(B[int]):
    c: int

assert C.__bases__ == (Generic, dict)

assert C.__orig_bases__ == (B[int],)

assert C.__mro__ == (C, Generic, dict, object)

assert C.__parameters__ == ()

assert C.__total__ == True

assert C.__optional_keys__ == frozenset(['b'])

assert C.__required_keys__ == frozenset(['a', 'c'])

assert C.__annotations__ == {'a': T, 'b': KT, 'c': int}
try:
    C[str]
...
assert Point3D.__bases__ == (Generic, dict)

assert Point3D.__orig_bases__ == (Point2DGeneric[T], Generic[T, KT])

assert Point3D.__mro__ == (Point3D, Generic, dict, object)

assert Point3D.__parameters__ == (T, KT)

assert Point3D.__total__ == True

assert Point3D.__optional_keys__ == frozenset()

assert Point3D.__required_keys__ == frozenset(['a', 'b', 'c'])

assert Point3D.__annotations__ == {'a': T, 'b': T, 'c': KT}

assert Point3D[int, str].__origin__ == Point3D
try:
    Point3D[int]
    raise AssertionError('expected TypeError')
except TypeError:
...
try:
    ...
    pass
print("TypedDictTests::test_generic_inheritance: ok")
