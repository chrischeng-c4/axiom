...
# lib = "typing"
# dimension = "behavior"
# case = "get_type_hint_tests__test_get_type_hints_classes"
# subject = "cpython.test_typing.GetTypeHintTests.test_get_type_hints_classes"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::GetTypeHintTests::test_get_type_hints_classes
"""Auto-ported test: GetTypeHintTests::test_get_type_hints_classes (CPython 3.12 oracle)."""
...
assert gth(ann_module.C) == {'y': Optional[ann_module.C]}

assert isinstance(gth(ann_module.j_class), dict)

assert gth(ann_module.M) == {'123': 123, 'o': type}

assert gth(ann_module.D) == {'j': str, 'k': str, 'y': Optional[ann_module.C]}

assert gth(ann_module.Y) == {'z': int}

assert gth(ann_module.h_class) == {'y': Optional[ann_module.C]}

assert gth(ann_module.S) == {'x': str, 'y': str}

assert gth(ann_module.foo) == {'x': int}

assert gth(NoneAndForward) == {'parent': NoneAndForward, 'meaning': type(None)}

assert gth(HasForeignBaseClass) == {'some_xrepr': XRepr, 'other_a': mod_generics_cache.A, 'some_b': mod_generics_cache.B}

assert gth(XRepr.__new__) == {'x': int, 'y': int}

assert gth(mod_generics_cache.B) == {'my_inner_a1': mod_generics_cache.B.A, 'my_inner_a2': mod_generics_cache.B.A, 'my_outer_a': mod_generics_cache.A}
print("GetTypeHintTests::test_get_type_hints_classes: ok")
