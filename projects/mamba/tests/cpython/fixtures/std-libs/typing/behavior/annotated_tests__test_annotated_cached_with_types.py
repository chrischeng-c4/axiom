...
# lib = "typing"
# dimension = "behavior"
# case = "annotated_tests__test_annotated_cached_with_types"
# subject = "cpython.test_typing.AnnotatedTests.test_annotated_cached_with_types"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::AnnotatedTests::test_annotated_cached_with_types
"""Auto-ported test: AnnotatedTests::test_annotated_cached_with_types (CPython 3.12 oracle)."""
...
assert type(a1_metadata) is A

assert a1_metadata == A('X')

assert type(a2_metadata) is B

assert a2_metadata == B('X')

assert type(a1_metadata) is not type(a2_metadata)
...
assert type(b1_metadata) is A

assert b1_metadata == A('Y')

assert type(b2_metadata) is B

assert b2_metadata == B('Y')

assert type(b1_metadata) is not type(b2_metadata)
field_c1 = Annotated[int, 1]
field_c2 = Annotated[int, 1.0]
field_c3 = Annotated[int, True]

assert type(field_c1.__metadata__[0]) is int

assert type(field_c2.__metadata__[0]) is float

assert type(field_c3.__metadata__[0]) is bool
print("AnnotatedTests::test_annotated_cached_with_types: ok")
