...
# lib = "typing"
# dimension = "behavior"
# case = "collections_callable_tests__test_pickle"
# subject = "cpython.test_typing.CollectionsCallableTests.test_pickle"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_typing.py::CollectionsCallableTests::test_pickle
"""Auto-ported test: CollectionsCallableTests::test_pickle (CPython 3.12 oracle)."""
...
def clear_caches():
    for f in typing._cleanups:
        f()
global T_pickle, P_pickle, TS_pickle
Callable = Callable
T_pickle = TypeVar('T_pickle')
P_pickle = ParamSpec('P_pickle')
TS_pickle = TypeVarTuple('TS_pickle')
samples = [Callable[[int, str], float], Callable[P_pickle, int], Callable[P_pickle, T_pickle], Callable[Concatenate[int, P_pickle], int], Callable[Concatenate[*TS_pickle, P_pickle], int]]
for alias in samples:
    for proto in range(pickle.HIGHEST_PROTOCOL + 1):
        s = pickle.dumps(alias, proto)
        loaded = pickle.loads(s)

        assert alias.__origin__ == loaded.__origin__

        assert alias.__args__ == loaded.__args__

        assert alias.__parameters__ == loaded.__parameters__
del T_pickle, P_pickle, TS_pickle
print("CollectionsCallableTests::test_pickle: ok")
