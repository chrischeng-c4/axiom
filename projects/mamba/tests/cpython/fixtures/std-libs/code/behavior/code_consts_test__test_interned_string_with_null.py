...
# lib = "code"
# dimension = "behavior"
# case = "code_consts_test__test_interned_string_with_null"
# subject = "cpython.test_code.CodeConstsTest.test_interned_string_with_null"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_code.py::CodeConstsTest::test_interned_string_with_null
"""Auto-ported test: CodeConstsTest::test_interned_string_with_null (CPython 3.12 oracle)."""
...
# --- test body ---
def assertIsInterned(s):
    if not isinterned(s):

        raise AssertionError('String %r is not interned' % (s,))

def assertIsNotInterned(s):
    if isinterned(s):

        raise AssertionError('String %r is interned' % (s,))

def find_const(consts, value):
    for v in consts:
        if v == value:
            return v

    assert value in consts

    raise AssertionError('Should never be reached')
co = compile('res = "str\\0value!"', '?', 'exec')
v = find_const(co.co_consts, 'str\x00value!')
assertIsNotInterned(v)
print("CodeConstsTest::test_interned_string_with_null: ok")
