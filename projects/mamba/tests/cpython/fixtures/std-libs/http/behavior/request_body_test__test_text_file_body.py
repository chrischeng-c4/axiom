...
# lib = "http"
# dimension = "behavior"
# case = "request_body_test__test_text_file_body"
# subject = "cpython.test_httplib.RequestBodyTest.test_text_file_body"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_httplib.py::RequestBodyTest::test_text_file_body
"""Auto-ported test: RequestBodyTest::test_text_file_body (CPython 3.12 oracle)."""
...
def get_headers_and_fp():
    f = io.BytesIO(self_sock.data)
    f.readline()
    message = client.parse_headers(f)
    return (message, f)
self_conn = client.HTTPConnection('example.com')
self_conn.sock = self_sock = FakeSocket('')
self_conn.sock = self_sock
pass
with open(os_helper.TESTFN, 'w', encoding='utf-8') as f:
    f.write('body')
with open(os_helper.TESTFN, encoding='utf-8') as f:
    self_conn.request('PUT', '/url', f)
    message, f = get_headers_and_fp()

    assert 'text/plain' == message.get_content_type()

    assert message.get_charset() is None

    assert message.get('content-length') is None

    assert 'chunked' == message.get('transfer-encoding')

    assert b'4\r\nbody\r\n0\r\n\r\n' == f.read()
print("RequestBodyTest::test_text_file_body: ok")
