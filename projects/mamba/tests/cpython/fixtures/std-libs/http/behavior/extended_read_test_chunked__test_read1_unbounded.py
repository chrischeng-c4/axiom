...
# lib = "http"
# dimension = "behavior"
# case = "extended_read_test_chunked__test_read1_unbounded"
# subject = "cpython.test_httplib.ExtendedReadTestChunked.test_read1_unbounded"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_httplib.py::ExtendedReadTestChunked::test_read1_unbounded
"""Auto-ported test: ExtendedReadTestChunked::test_read1_unbounded (CPython 3.12 oracle)."""
...
def _verify_readline(readline, expected, limit=5):
    all = []
    while True:
        line = readline(limit)
    ...
    assert b''.join(all) == expected

    assert self_resp.isclosed()
...
assert b''.join(all) == lines_expected

assert resp.isclosed()
print("ExtendedReadTestChunked::test_read1_unbounded: ok")
