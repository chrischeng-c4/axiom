...
# lib = "http"
# dimension = "behavior"
# case = "header_tests__test_parse_all_octets"
# subject = "cpython.test_httplib.HeaderTests.test_parse_all_octets"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_httplib.py::HeaderTests::test_parse_all_octets
"""Auto-ported test: HeaderTests::test_parse_all_octets (CPython 3.12 oracle)."""
...
assert resp.msg['Content-Length'] == '0'

assert resp.getheader("!#$%&'*+-.^_`|~") == 'value'

assert resp.msg["!#$%&'*+-.^_`|~"] == 'value'
vchar = ''.join(map(chr, range(33, 126 + 1)))

assert resp.getheader('VCHAR') == vchar

assert resp.msg['VCHAR'] == vchar

assert resp.getheader('obs-text') is not None

assert 'obs-text' in resp.msg
for folded in (resp.getheader('obs-fold'), resp.msg['obs-fold']):

    assert folded.startswith('text')

    assert ' folded with space' in folded

    assert folded.endswith('folded with tab')
print("HeaderTests::test_parse_all_octets: ok")
