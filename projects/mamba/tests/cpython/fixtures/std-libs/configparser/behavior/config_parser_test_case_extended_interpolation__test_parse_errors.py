...
# lib = "configparser"
# dimension = "behavior"
# case = "config_parser_test_case_extended_interpolation__test_parse_errors"
# subject = "cpython.test_configparser.ConfigParserTestCaseExtendedInterpolation.test_parse_errors"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_configparser.py::ConfigParserTestCaseExtendedInterpolation::test_parse_errors
"""Auto-ported test: ConfigParserTestCaseExtendedInterpolation::test_parse_errors (CPython 3.12 oracle)."""
...
def fromstring(string, defaults=None, optionxform=None):
...
def parse_error(cf, exc, src):
    ...
    return cm.exception
cf = newconfig()
parse_error(cf, configparser.ParsingError, '[Foo]\n{}val-without-opt-name\n'.format(delimiters[0]))
parse_error(cf, configparser.ParsingError, '[Foo]\n{}val-without-opt-name\n'.format(delimiters[1]))
e = parse_error(cf, configparser.MissingSectionHeaderError, 'No Section!\n')

assert e.args == ('<???>', 1, 'No Section!\n')
if not allow_no_value:
    e = parse_error(cf, configparser.ParsingError, '[Foo]\n  wrong-indent\n')
    ...
    with open(tricky, encoding='utf-8') as f:
        e = parse_error(cf, error, f)

    assert e.args == expected
print("ConfigParserTestCaseExtendedInterpolation::test_parse_errors: ok")
