...
# lib = "configparser"
# dimension = "behavior"
# case = "config_parser_test_case_no_interpolation__test_weird_errors"
# subject = "cpython.test_configparser.ConfigParserTestCaseNoInterpolation.test_weird_errors"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_configparser.py::ConfigParserTestCaseNoInterpolation::test_weird_errors
"""Auto-ported test: ConfigParserTestCaseNoInterpolation::test_weird_errors (CPython 3.12 oracle)."""
...
def assertMatchesIni(cf):

    assert cf['numbers']['one'] == '1'

    assert cf['numbers']['two'] == '%(one)s * 2'

    assert cf['numbers']['three'] == '${common:one} * 3'

    assert cf['hexen']['sixteen'] == '${numbers:two} * 8'
...
assert str(e) == "Section 'Foo' already exists"

assert e.args == ('Foo', None, None)
if strict:
    try:
        cf.read_string(textwrap.dedent("                    [Foo]\n                    will this be added{equals}True\n                    [Bar]\n                    what about this{equals}True\n                    [Foo]\n                    oops{equals}this won't\n                ".format(equals=delimiters[0])), source='<foo-bar>')
        raise AssertionError('expected configparser.DuplicateSectionError')
    except configparser.DuplicateSectionError as _aR_e:
        import types as _types_aR
        cm = _types_aR.SimpleNamespace(exception=_aR_e)
    ...
    assert str(e) == "While reading from '<foo-bar>' [line  5]: section 'Foo' already exists"

    assert e.args == ('Foo', '<foo-bar>', 5)
    try:
    ...
    assert str(e) == "While reading from '<dict>': option 'opt' in section 'Bar' already exists"

    assert e.args == ('Bar', 'opt', '<dict>', None)
print("ConfigParserTestCaseNoInterpolation::test_weird_errors: ok")
