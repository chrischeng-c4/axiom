...
# lib = "configparser"
# dimension = "behavior"
# case = "config_parser_test_case_non_standard_default_section__test_safe_interpolation"
# subject = "cpython.test_configparser.ConfigParserTestCaseNonStandardDefaultSection.test_safe_interpolation"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_configparser.py::ConfigParserTestCaseNonStandardDefaultSection::test_safe_interpolation
"""Auto-ported test: ConfigParserTestCaseNonStandardDefaultSection::test_safe_interpolation (CPython 3.12 oracle)."""
...
def parse_error(cf, exc, src):
    ...
    try:
        ...
        cm = _types_aR.SimpleNamespace(exception=_aR_e)
    return cm.exception
cf = fromstring('[section]\noption1{eq}xxx\noption2{eq}%(option1)s/xxx\nok{eq}%(option1)s/%%s\nnot_ok{eq}%(option2)s/%%s'.format(eq=delimiters[0]))

assert cf.get('section', 'ok') == 'xxx/%s'
if interpolation == configparser._UNSET:

    assert cf.get('section', 'not_ok') == 'xxx/xxx/%s'
    ...
    try:
        ...
        pass
print("ConfigParserTestCaseNonStandardDefaultSection::test_safe_interpolation: ok")
