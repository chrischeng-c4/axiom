...
# lib = "memoryio"
# dimension = "behavior"
# case = "py_bytes_io_test__test_truncate"
# subject = "cpython.test_memoryio.PyBytesIOTest.test_truncate"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_memoryio.py::PyBytesIOTest::test_truncate
"""Auto-ported test: PyBytesIOTest::test_truncate (CPython 3.12 oracle)."""
...
try:
    memio.truncate(IntLike(-1))
    raise AssertionError('expected ValueError')
except ValueError:
    pass
memio.seek(6)

assert memio.truncate(IntLike(8)) == 8

assert memio.getvalue() == buf[:8]

assert memio.truncate() == 6

assert memio.getvalue() == buf[:6]

assert memio.truncate(4) == 4

assert memio.getvalue() == buf[:4]

assert memio.tell() == 6
memio.seek(0, 2)
memio.write(buf)

assert memio.getvalue() == buf[:4] + buf
pos = memio.tell()

assert memio.truncate(None) == pos

assert memio.tell() == pos
...
try:
    memio.truncate(IntLike(0))
    raise AssertionError('expected ValueError')
except ValueError:
    pass
print("PyBytesIOTest::test_truncate: ok")
