...
# lib = "decimal"
# dimension = "behavior"
# case = "c_whitebox__test_c_funcs"
# subject = "cpython.test_decimal.CWhitebox.test_c_funcs"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::CWhitebox::test_c_funcs
"""Auto-ported test: CWhitebox::test_c_funcs (CPython 3.12 oracle)."""
...
assert Decimal('9.99e10').to_eng_string() == '99.9E+9'
...
try:
    Decimal(9).number_class('x', 'y')
    raise AssertionError('expected TypeError')
except TypeError:
    pass

try:
    Decimal(9).same_quantum(3, 'x', 'y')
    raise AssertionError('expected TypeError')
except TypeError:
    pass
...
with localcontext() as c:
    ...
    try:
        c.same_quantum(9, 3, 'x', 'y')
        raise AssertionError('expected TypeError')
    except TypeError:
        pass

    assert str(c.canonical(Decimal(200))) == '200'

    assert c.radix() == 10
    c.traps[DivisionByZero] = True

    try:
        Decimal(9).__divmod__(0)
    ...
    try:
        pow(Decimal(1000), 1, 501)
        raise AssertionError('expected InvalidOperation')
    except InvalidOperation:
        pass
print("CWhitebox::test_c_funcs: ok")
