...
# lib = "decimal"
# dimension = "behavior"
# case = "py_context_flags__test_float_operation"
# subject = "cpython.test_decimal.PyContextFlags.test_float_operation"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_decimal.py::PyContextFlags::test_float_operation
"""Auto-ported test: PyContextFlags::test_float_operation (CPython 3.12 oracle)."""
...
with localcontext() as c:
    ...
    assert Decimal(7.5) == 7.5

    assert c.flags[FloatOperation]
    c.clear_flags()

    assert c.create_decimal(7.5) == 7.5

    assert c.flags[FloatOperation]
    c.clear_flags()
    x = Decimal.from_float(7.5)

    assert not c.flags[FloatOperation]
    ...
    try:
        Decimal(7.5)
        raise AssertionError('expected FloatOperation')
    ...
    assert not c.flags[FloatOperation]
print("PyContextFlags::test_float_operation: ok")
