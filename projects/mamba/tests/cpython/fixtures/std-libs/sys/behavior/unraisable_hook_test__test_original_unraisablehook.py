...
# lib = "sys"
# dimension = "behavior"
# case = "unraisable_hook_test__test_original_unraisablehook"
# subject = "cpython.test_sys.UnraisableHookTest.test_original_unraisablehook"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_sys.py::UnraisableHookTest::test_original_unraisablehook
"""Auto-ported test: UnraisableHookTest::test_original_unraisablehook (CPython 3.12 oracle)."""
...
for err_msg in (None, 'original hook'):
    ...
    with test.support.captured_output('stderr') as stderr:
        with test.support.swap_attr(sys, 'unraisablehook', sys.__unraisablehook__):
            write_unraisable_exc(ValueError(42), err_msg, obj)
    err = stderr.getvalue()
    if err_msg is not None:

        assert f'Exception ignored {err_msg}: {obj!r}\n' in err
    else:

        assert f'Exception ignored in: {obj!r}\n' in err

    assert 'Traceback (most recent call last):\n' in err

    assert 'ValueError: 42\n' in err
print("UnraisableHookTest::test_original_unraisablehook: ok")
