...
# lib = "codecs"
# dimension = "behavior"
# case = "utf7_test__test_ascii"
# subject = "cpython.test_codecs.UTF7Test.test_ascii"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_codecs.py::UTF7Test::test_ascii
"""Auto-ported test: UTF7Test::test_ascii (CPython 3.12 oracle)."""
...
assert set_d.encode(encoding) == set_d.encode('ascii')

assert set_d.encode('ascii').decode(encoding) == set_d
set_o = ' !"#$%&*;<=>@[]^_`{|}'

assert set_o.encode(encoding) == set_o.encode('ascii')

assert set_o.encode('ascii').decode(encoding) == set_o

assert 'a+b'.encode(encoding) == b'a+-b'

assert b'a+-b'.decode(encoding) == 'a+b'
ws = ' \t\n\r'

assert ws.encode(encoding) == ws.encode('ascii')

assert ws.encode('ascii').decode(encoding) == ws
other_ascii = ''.join(sorted(set(bytes(range(128)).decode()) - set(set_d + set_o + '+' + ws)))

assert other_ascii.encode(encoding) == b'+AAAAAQACAAMABAAFAAYABwAIAAsADAAOAA8AEAARABIAEwAUABUAFgAXABgAGQAaABsAHAAdAB4AHwBcAH4Afw-'
print("UTF7Test::test_ascii: ok")
