...
# lib = "math"
# dimension = "behavior"
# case = "math_tests__test_prod"
# subject = "cpython.test_math.MathTests.test_prod"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_math.py::MathTests::test_prod
"""Auto-ported test: MathTests::test_prod (CPython 3.12 oracle)."""
...
assertIsNaN(prod(iterable))
assertIsNaN(prod([1, 2, 3, float('nan'), 2, 3]))
assertIsNaN(prod([1, 0, float('nan'), 2, 3]))
assertIsNaN(prod([1, float('nan'), 0, 3]))
assertIsNaN(prod([1, float('inf'), float('nan'), 3]))
assertIsNaN(prod([1, float('-inf'), float('nan'), 3]))
assertIsNaN(prod([1, float('nan'), float('inf'), 3]))
assertIsNaN(prod([1, float('nan'), float('-inf'), 3]))

assert prod([1, 2, 3, float('inf'), -3, 4]) == float('-inf')

assert prod([1, 2, 3, float('-inf'), -3, 4]) == float('inf')
assertIsNaN(prod([1, 2, 0, float('inf'), -3, 4]))
assertIsNaN(prod([1, 2, 0, float('-inf'), -3, 4]))
assertIsNaN(prod([1, 2, 3, float('inf'), -3, 0, 3]))
assertIsNaN(prod([1, 2, 3, float('-inf'), -3, 0, 2]))

assert type(prod([1, 2, 3, 4, 5, 6])) == int

assert type(prod([1, 2.0, 3, 4, 5, 6])) == float

assert type(prod(range(1, 10000))) == int

assert type(prod(range(1, 10000), start=1.0)) == float

assert type(prod([1, decimal.Decimal(2.0), 3, 4, 5, 6])) == decimal.Decimal
print("MathTests::test_prod: ok")
