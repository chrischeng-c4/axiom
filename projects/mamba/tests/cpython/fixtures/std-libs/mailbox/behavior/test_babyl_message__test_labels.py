...
# lib = "mailbox"
# dimension = "behavior"
# case = "test_babyl_message__test_labels"
# subject = "cpython.test_mailbox.TestBabylMessage.test_labels"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_mailbox.py::TestBabylMessage::test_labels
"""Auto-ported test: TestBabylMessage::test_labels (CPython 3.12 oracle)."""
...
assert msg.get_labels() == []
msg.set_labels(['foobar'])

assert msg.get_labels() == ['foobar']
msg.set_labels([])

assert msg.get_labels() == []
msg.add_label('filed')

assert msg.get_labels() == ['filed']
msg.add_label('resent')

assert msg.get_labels() == ['filed', 'resent']
msg.add_label('resent')

assert msg.get_labels() == ['filed', 'resent']
msg.remove_label('filed')

assert msg.get_labels() == ['resent']
msg.add_label('foobar')

assert msg.get_labels() == ['resent', 'foobar']
msg.remove_label('unseen')

assert msg.get_labels() == ['resent', 'foobar']
msg.set_labels(['foobar', 'answered'])

assert msg.get_labels() == ['foobar', 'answered']
print("TestBabylMessage::test_labels: ok")
