...
# lib = "mailbox"
# dimension = "behavior"
# case = "test_mbox_message__test_from"
# subject = "cpython.test_mailbox.TestMboxMessage.test_from"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_mailbox.py::TestMboxMessage::test_from
"""Auto-ported test: TestMboxMessage::test_from (CPython 3.12 oracle)."""
...
def _check_from(msg, sender=None):
    if sender is None:
        sender = 'MAILER-DAEMON'

    assert re.match(sender + ' \\w{3} \\w{3} [\\d ]\\d [\\d ]\\d:\\d{2}:\\d{2} \\d{4}', msg.get_from()) is not None
...
assert msg.get_unixfrom() is None
msg.set_from('foo bar')

assert msg.get_from() == 'foo bar'

assert msg.get_unixfrom() is None
msg.set_from('foo@bar', True)
_check_from(msg, 'foo@bar')

assert msg.get_unixfrom() is None
msg.set_from('blah@temp', time.localtime())
_check_from(msg, 'blah@temp')

assert msg.get_unixfrom() is None
print("TestMboxMessage::test_from: ok")
