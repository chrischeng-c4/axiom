...
# lib = "mailbox"
# dimension = "behavior"
# case = "test_babyl_message__test_visible"
# subject = "cpython.test_mailbox.TestBabylMessage.test_visible"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_mailbox.py::TestBabylMessage::test_visible
"""Auto-ported test: TestBabylMessage::test_visible (CPython 3.12 oracle)."""
...
assert visible.get_payload() is None
visible['User-Agent'] = 'FooBar 1.0'
visible['X-Whatever'] = 'Blah'

assert msg.get_visible().keys() == []
msg.set_visible(visible)
visible = msg.get_visible()

assert visible.keys() == ['User-Agent', 'X-Whatever']

assert visible['User-Agent'] == 'FooBar 1.0'

assert visible['X-Whatever'] == 'Blah'
...
assert visible.keys() == ['User-Agent', 'Date', 'From', 'To', 'Subject']
for header in ('User-Agent', 'Date', 'From', 'To', 'Subject'):

    assert visible[header] == msg[header]
print("TestBabylMessage::test_visible: ok")
