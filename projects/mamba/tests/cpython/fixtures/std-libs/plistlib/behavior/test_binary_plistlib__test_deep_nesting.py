...
# lib = "plistlib"
# dimension = "behavior"
# case = "test_binary_plistlib__test_deep_nesting"
# subject = "cpython.test_plistlib.TestBinaryPlistlib.test_deep_nesting"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_plistlib.py::TestBinaryPlistlib::test_deep_nesting
"""Auto-ported test: TestBinaryPlistlib::test_deep_nesting (CPython 3.12 oracle)."""
...
tests = [50, 100000] if support.is_wasi else [50, 600, 100000]
for N in tests:
    chunks = [b'\xa1' + (i + 1).to_bytes(4, 'big') for i in range(N)]
    try:
        result = decode(*chunks, b'Tseed', offset_size=4, ref_size=4)
    except RecursionError:

        assert N > sys.getrecursionlimit() // 2
    else:
        for i in range(N):
        ...
        assert result == 'seed'
print("TestBinaryPlistlib::test_deep_nesting: ok")
