...
# lib = "csv"
# dimension = "behavior"
# case = "test_csv__test_roundtrip_escaped_unquoted_newlines"
# subject = "cpython.test_csv.Test_Csv.test_roundtrip_escaped_unquoted_newlines"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_csv.py::Test_Csv::test_roundtrip_escaped_unquoted_newlines
"""Auto-ported test: Test_Csv::test_roundtrip_escaped_unquoted_newlines (CPython 3.12 oracle)."""
...
for lineterminator in ('\r\n', '\n', '\r'):
    with TemporaryFile('w+', encoding='utf-8', newline='') as fileobj:
        writer = csv.writer(fileobj, lineterminator=lineterminator, quoting=csv.QUOTE_NONE, escapechar='\\')
        writer.writerows(rows)
        fileobj.seek(0)
        for i, row in enumerate(csv.reader(fileobj, quoting=csv.QUOTE_NONE, escapechar='\\')):

            assert row == rows[i]
print("Test_Csv::test_roundtrip_escaped_unquoted_newlines: ok")
