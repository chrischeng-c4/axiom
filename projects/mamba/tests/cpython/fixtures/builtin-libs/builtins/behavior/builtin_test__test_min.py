...
# lib = "builtins"
# dimension = "behavior"
# case = "builtin_test__test_min"
# subject = "cpython.test_builtin.BuiltinTest.test_min"
# kind = "semantic"
...
# Auto-ported from CPython 3.12 test_builtin.py::BuiltinTest::test_min
"""Auto-ported test: BuiltinTest::test_min (CPython 3.12 oracle)."""
...
assert min(1.0, 2, 3) == 1.0
try:
    min()
    raise AssertionError('expected TypeError')
except TypeError as _aR_e:
    import re as _re_aR
    assert _re_aR.search('min expected at least 1 argument, got 0', str(_aR_e))
...
try:
    ...
    import re as _re_aR
    assert _re_aR.search('min\\(\\) iterable argument is empty', str(_aR_e))
...
assert min((1, 2), default=None) == 1

assert min((), default=1, key=neg) == 1

assert min((1, 2), default=1, key=neg) == 2

assert min((1, 2), key=None) == 1
data = [random.randrange(200) for i in range(100)]
keys = dict(((elem, random.randrange(50)) for elem in data))
f = keys.__getitem__

assert min(data, key=f) == sorted(data, key=f)[0]
print("BuiltinTest::test_min: ok")
