...
fn wheel_is_minimal_pure_python_py3_none_any() {
    ...
    assert_eq!(
        w.get("must_be_pure_python").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        w.get("must_be_py3_none_any").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        w.get("must_be_minimal").and_then(|v| v.as_bool()),
        Some(true)
    );
    let sentinel_attr = w
...
fn missing_wheel_file_fails_fixture_validation() {
    ...
    assert_eq!(
        v.get("must_validate_wheel_file_exists_on_disk")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        v.get("must_validate_wheel_file_matches_index_metadata")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        v.get("validation_must_run_before_install_attempt")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        v.get("missing_wheel_failure_kind").and_then(|v| v.as_str()),
        Some("wheel_file_missing")
    );
    let exit = v
        .get("missing_wheel_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 23);
    assert_eq!(
        v.get("missing_wheel_diagnostic_must_name_path")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        v.get("must_distinguish_from_resolver_failures")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn install_succeeds_offline() {
    let doc = load_toml(&manifest_path());
    let c = doc
        .get("offline_install_case")
        .and_then(|v| v.as_table())
        .expect("[offline_install_case] missing — acceptance: \"Install succeeds offline.\"");
    assert_eq!(
        c.get("must_run_install_command").and_then(|v| v.as_bool()),
        Some(true)
    );
    let tmpl = c
        .get("install_command_template")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(
        tmpl.starts_with("mamba "),
        "install_command_template must invoke mamba"
    );
    let pkg = doc
        .get("wheel")
    ...
    assert!(
        tmpl.contains(pkg),
        "install_command_template must reference the package"
    );
    assert_eq!(
        c.get("must_be_offline").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("forbid_network_access").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_extract_wheel_into_temp_env")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_capture_install_stdout")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_capture_install_stderr")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_capture_install_exit_status")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("expected_install_exit_code")
            .and_then(|v| v.as_integer()),
        Some(0)
    );
    assert_eq!(
        c.get("must_assert_dist_info_present_after_install")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let dist_info = c
        .get("dist_info_dir_pattern")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(dist_info.contains(pkg));
    assert!(dist_info.contains(".dist-info"));
    assert_eq!(
        c.get("must_assert_record_file_present")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("record_file_path_within_dist_info")
            .and_then(|v| v.as_str()),
        Some("RECORD")
    );
}
...
fn imported_package_returns_expected_sentinel_value() {
    ...
    assert_eq!(
        c.get("must_run_import_script_after_install")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_assert_sentinel_attribute_present")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_assert_sentinel_value_equals_expected")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("sentinel_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("sentinel_value_mismatch")
    );
    let mismatch_exit = c
    ...
    assert_eq!(
        c.get("import_failure_failure_kind")
            .and_then(|v| v.as_str()),
    ...
    assert_ne!(
        mismatch_exit, import_exit,
        "sentinel-mismatch and import-failure exit codes must differ"
    );
    assert_eq!(
        c.get("must_be_deterministic").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_distinguish_sentinel_mismatch_from_import_failure")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("native_wheel_support").and_then(|v| v.as_bool()),
        Some(true)
    );
}
