...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("third_party_sqlalchemy_expression_behavioral")
    );
...
fn surface_covers_sqlalchemy() {
    ...
    let modules: Vec<&str> = s
        ...
        .unwrap_or_default();
    assert!(
        modules.contains(&"sqlalchemy"),
        "covered_modules must include sqlalchemy"
    );
    for f in &[
...
fn expression_definition_pins_canonical_table() {
    let doc = load_toml(&manifest_path());
    let e = doc
        .get("expression_definition")
        .and_then(|v| v.as_table())
        .expect("[expression_definition] missing");
    assert_eq!(
        e.get("metadata_variable").and_then(|v| v.as_str()),
        Some("metadata_2644")
    );
    assert_eq!(
        e.get("table_name").and_then(|v| v.as_str()),
        Some("mamba_2644")
    );
    assert_eq!(e.get("column_a_name").and_then(|v| v.as_str()), Some("id"));
    assert_eq!(
        e.get("column_a_type").and_then(|v| v.as_str()),
        Some("Integer")
    );
    assert_eq!(
        e.get("column_a_primary_key").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        e.get("column_b_name").and_then(|v| v.as_str()),
        Some("label")
    );
    assert_eq!(
        e.get("column_b_type").and_then(|v| v.as_str()),
        Some("String")
    );
    assert_eq!(
        e.get("select_expression_python_repr")
            .and_then(|v| v.as_str()),
        Some("select(mamba_2644)")
    );
}
...
fn fixture_fails_if_sqlalchemy_cannot_import() {
    ...
    let exit = i
        .get("sqlalchemy_import_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 151);
    assert_eq!(
        i.get("sqlalchemy_import_failure_kind")
            .and_then(|v| v.as_str()),
...
fn fixture_does_not_open_a_database_connection() {
    let doc = load_toml(&manifest_path());
    let n = doc
        .get("no_database_connection_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let conn = n
        .get("engine_connect_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let dbapi = n
        .get("dbapi_driver_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(conn, 152);
    assert_eq!(dbapi, 153);
    assert_ne!(
        conn, dbapi,
        "engine-connect and dbapi-driver exit codes must differ"
    );
    assert_eq!(
        n.get("engine_connect_failure_kind")
            .and_then(|v| v.as_str()),
...
fn summary_records_sqlalchemy_as_orm_tooling_coverage() {
    let doc = load_toml(&manifest_path());
    let h = doc
        .get("orm_tooling_coverage_reporting_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        h.get("orm_tooling_coverage_field_name")
            .and_then(|v| v.as_str()),
        Some("orm_tooling_dependencies_covered"),
    );
    let req: Vec<&str> = h
        .get("required_orm_tooling_dependencies_covered")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    assert!(
        req.contains(&"sqlalchemy"),
        "required_orm_tooling_dependencies_covered must include sqlalchemy"
    );
    assert_eq!(
    ...
    let exit = h
        .get("missing_orm_tooling_coverage_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 154);
    assert_eq!(
        h.get("missing_orm_tooling_coverage_failure_kind")
            .and_then(|v| v.as_str()),
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("db_driver_integration").and_then(|v| v.as_bool()),
        Some(true)
    );
}
