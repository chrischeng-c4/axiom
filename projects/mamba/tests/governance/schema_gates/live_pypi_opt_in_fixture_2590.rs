...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("live_pypi_opt_in"));
...
fn default_index_cross_references_frozen_local_simple_index() {
    let doc = crate::common::load_toml(&manifest_path());
    let i = doc.get("default_index").and_then(|v| v.as_table()).expect("[default_index] missing");
...
fn opt_in_gate_pins_both_feature_and_env_var_names() {
    let doc = crate::common::load_toml(&manifest_path());
    let g = doc.get("opt_in_gate").and_then(|v| v.as_table()).expect("[opt_in_gate] missing");
    assert_eq!(g.get("must_be_opt_in").and_then(|v| v.as_bool()), Some(true));
    let feature = g.get("opt_in_cargo_feature_name").and_then(|v| v.as_str()).unwrap();
    assert!(!feature.is_empty());
    assert!(feature.contains("pypi") || feature.contains("live"), "feature name must mention live or pypi: {feature}");
    let env_var = g.get("opt_in_env_var_name").and_then(|v| v.as_str()).unwrap();
    assert!(env_var.contains("PYPI") || env_var.contains("LIVE"), "env var must mention LIVE or PYPI: {env_var}");
    assert!(env_var.chars().all(|c| c.is_ascii_uppercase() || c == '_' || c.is_ascii_digit()),
        "env var name must be SCREAMING_SNAKE_CASE: {env_var}");
    assert_eq!(g.get("forbid_running_under_default_profile").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(g.get("must_attach_ignore_marker_when_not_opted_in").and_then(|v| v.as_bool()), Some(true));
}
...
fn default_package_manager_gate_runs_without_network() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("default_profile_network_assertion").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(d.get("default_profile_network_violation_failure_kind").and_then(|v| v.as_str()), Some("network_in_default_profile"));
    let exit = d.get("default_profile_network_violation_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 22);
}
...
fn live_pypi_tests_still_have_explicit_command_path() {
    let doc = crate::common::load_toml(&manifest_path());
    let l = doc.get("live_command_path_contract").and_then(|v| v.as_table()).expect(
        "[live_command_path_contract] missing — acceptance: \
         \"Live PyPI tests still have an explicit command path.\"",
    );
    assert_eq!(l.get("must_document_live_command_separately").and_then(|v| v.as_bool()), Some(true));
    let template = l.get("documented_command_template").and_then(|v| v.as_str()).unwrap();
    assert!(template.contains("cargo test"), "documented_command_template must use cargo test");
    let feature = doc.get("opt_in_gate").and_then(|v| v.get("opt_in_cargo_feature_name")).and_then(|v| v.as_str()).unwrap();
    assert!(template.contains(feature), "documented_command_template must mention opt-in feature {feature}");
    assert!(template.contains("--ignored"), "documented_command_template must include --ignored filter");
    assert_eq!(l.get("documented_command_must_include_feature_flag").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(l.get("documented_command_must_include_ignore_filter").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(l.get("live_command_must_be_excluded_from_default_test_run").and_then(|v| v.as_bool()), Some(true));
}
...
fn ci_summary_shows_live_checks_as_opt_in() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc.get("summary_visibility_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(s.get("summary_must_record_live_checks_as_opt_in").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("summary_must_record_default_profile_as_offline").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("summary_must_be_machine_readable").and_then(|v| v.as_bool()), Some(true));
    ...
    let allowed: Vec<&str> = s.get("allowed_default_profile_network_mode_values").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    assert!(allowed.contains(&"offline"));
    assert!(!allowed.contains(&"online"), "default profile must not allow online network mode");
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for required in &[
        "outcome", "case", "profile",
        "live_pypi_opt_in", "live_pypi_executed",
        "network_access_observed", "failure_kind", "exit_code",
    ] {
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("removing_live_integration_coverage").and_then(|v| v.as_bool()), Some(true));
}
