...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("stdlib_collections_itertools_functools_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("stdlib_collections_itertools_functools_behavioral"));
...
fn surface_covers_collections_itertools_functools() {
    ...
    let families: Vec<&str> = s.get("helper_families").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
...
fn deterministic_sample_covers_every_helper() {
    ...
    assert!(max <= 64, "sample_max_records must stay small enough for per-run check");

    let helpers = doc.get("helper_cases").and_then(|v| v.as_array()).expect("[[helper_cases]] missing");
    let mut seen: Vec<&str> = Vec::new();
    for c in helpers {
        let t = c.as_table().expect("helper case must be a table");
        let family = t.get("family").and_then(|v| v.as_str()).expect("helper.family missing");
        let helper = t.get("helper").and_then(|v| v.as_str()).expect("helper.helper missing");
        assert!(t.get("input_python_repr").and_then(|v| v.as_str()).is_some(), "helper.input_python_repr missing");
        assert!(t.get("expected_python_repr").and_then(|v| v.as_str()).is_some(), "helper.expected_python_repr missing");
        assert!(["collections", "itertools", "functools"].contains(&family), "unexpected family={family}");
        seen.push(helper);
    }
    for h in &["Counter", "defaultdict_list", "chain", "islice", "lru_cache", "partial"] {
...
fn fixture_fails_on_any_helper_behavior_mismatch() {
    ...
    sorted.sort();
    sorted.dedup();
    assert_eq!(sorted.len(), exits.len(), "helper family exit codes must differ");
}
...
fn failure_output_identifies_helper_family() {
    ...
    assert_eq!(n.get("must_emit_helper_family_in_failure_output").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(n.get("helper_family_field_name").and_then(|v| v.as_str()), Some("helper_family"));
    let allowed: Vec<&str> = n.get("allowed_helper_family_values").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for fam in &["collections", "itertools", "functools"] {
        assert!(allowed.contains(fam), "allowed_helper_family_values must include {fam}");
    }
    assert_eq!(n.get("must_emit_helper_name_in_failure_output").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(n.get("helper_name_field_name").and_then(|v| v.as_str()), Some("helper"));
    assert_eq!(n.get("forbid_generic_unnamed_helper_failure").and_then(|v| v.as_bool()), Some(true));
    let exit = n.get("unnamed_helper_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 92);
    assert_eq!(n.get("unnamed_helper_failure_kind").and_then(|v| v.as_str()), Some("helper_failure_missing_family"));
}
...
fn fixture_remains_fast_enough_for_ecosystem_gate() {
    let doc = crate::common::load_toml(&manifest_path());
    let p = doc.get("performance_budget_contract").and_then(|v| v.as_table()).expect(
    ...
    for k in &[
        ...
        assert_eq!(p.get(*k).and_then(|v| v.as_bool()), Some(true), "{k} must be true");
    }
    assert_eq!(p.get("ecosystem_profile_fixture_issue").and_then(|v| v.as_integer()), Some(2814));
    let total = p.get("max_total_runtime_ms").and_then(|v| v.as_integer()).unwrap();
    let per = p.get("max_per_helper_runtime_ms").and_then(|v| v.as_integer()).unwrap();
    assert!(total > 0 && per > 0 && per <= total, "runtime bounds sane");
    assert!(total <= 5000, "total runtime must stay light for ecosystem gate, got {total}ms");
    let recur = p.get("forbid_long_running_recursion_above_iteration_count").and_then(|v| v.as_integer()).unwrap();
    assert!(recur > 0 && recur <= 100_000, "recursion bound must be sane, got {recur}");
    let exit = p.get("performance_budget_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 93);
    assert_eq!(p.get("performance_budget_failure_kind").and_then(|v| v.as_str()), Some("helper_fixture_over_budget"));
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "helper_family", "helper",
        "input_python_repr", "expected_python_repr", "actual_python_repr",
        "duration_ms", "failure_kind", "exit_code",
    ] {
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("full_iterator_algebra_compatibility").and_then(|v| v.as_bool()), Some(true));
}
