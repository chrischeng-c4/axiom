...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("third_party_urllib3_response_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("third_party_urllib3_response_behavioral"));
...
fn surface_covers_urllib3() {
    ...
    assert!(modules.contains(&"urllib3"), "covered_modules must include urllib3");
    for f in &[
    ...
    assert_eq!(s.get("import_statement").and_then(|v| v.as_str()), Some("import urllib3"));
}
...
fn deterministic_sample_covers_response_construction() {
    ...
    let arr = doc.get("response_cases").and_then(|v| v.as_array()).expect("[[response_cases]] missing");
    assert!(!arr.is_empty(), "response_cases must not be empty");
    for c in arr {
        let t = c.as_table().expect("case must be a table");
        for f in &[
            "body_bytes_python_repr", "status", "headers_python_repr",
            "expected_status", "expected_data_bytes_python_repr",
            "expected_content_type_header", "expected_content_length_header",
        ] {
...
fn fixture_fails_if_urllib3_cannot_import() {
    ...
    let exit = i.get("urllib3_import_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 147);
    assert_eq!(
        i.get("urllib3_import_failure_kind").and_then(|v| v.as_str()),
...
fn no_external_network_io_occurs() {
    ...
    let pool = n.get("connection_pool_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(net, 148);
    assert_eq!(pool, 149);
    ...
    assert_eq!(
        n.get("connection_pool_failure_kind").and_then(|v| v.as_str()),
...
fn runner_records_under_http_client_dependency_coverage() {
    let doc = crate::common::load_toml(&manifest_path());
    let h = doc.get("http_client_coverage_reporting_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(
        h.get("http_client_coverage_field_name").and_then(|v| v.as_str()),
        Some("http_client_dependencies_covered"),
    );
    let req: Vec<&str> = h.get("required_http_client_dependencies_covered").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    assert!(req.contains(&"urllib3"), "required_http_client_dependencies_covered must include urllib3");
    assert_eq!(h.get("summary_record_format").and_then(|v| v.as_str()), Some("json"));
    let exit = h.get("missing_http_client_coverage_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 150);
    assert_eq!(
        h.get("missing_http_client_coverage_failure_kind").and_then(|v| v.as_str()),
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("live_connection_pools").and_then(|v| v.as_bool()), Some(true));
