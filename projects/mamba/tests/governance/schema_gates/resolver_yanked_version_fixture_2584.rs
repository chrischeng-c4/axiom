...
fn test_runs_without_network() {
    ...
    let n = doc
        .get("network_gate")
        .and_then(|v| v.as_table())
        .expect("[network_gate] missing — acceptance: \"Test runs without network.\"");
    for f in &[
...
fn yanked_version_behavior_is_deterministic() {
    ...
    assert_eq!(
        y.get("must_avoid_yanked_versions_by_default")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        y.get("must_select_yanked_version_only_when_explicitly_allowed")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let flag = y
        .get("allow_yanked_flag_name")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(
        flag.starts_with("--"),
        "allow_yanked_flag_name must be a long flag"
    );
    assert!(
        flag.contains("yanked"),
        "allow_yanked_flag_name must mention yanked"
    );
    assert_eq!(
        y.get("deterministic_resolution_across_runs")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        y.get("must_record_yanked_decision_in_resolution_log")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        y.get("yanked_decision_log_format").and_then(|v| v.as_str()),
        Some("json")
    );
}
...
fn failure_output_names_selected_or_rejected_version() {
    ...
    assert_eq!(
        d.get("failure_message_must_name_rejected_version")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("failure_message_must_name_selected_version_when_available")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("failure_message_must_include_yanked_marker")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("failure_message_must_be_deterministic")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("yanked_rejection_failure_kind")
            .and_then(|v| v.as_str()),
        Some("yanked_version_rejected")
    );
    let yanked_exit = d
        .get("yanked_rejection_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(yanked_exit, 0);
    assert_eq!(
        d.get("no_candidate_failure_kind").and_then(|v| v.as_str()),
        Some("no_acceptable_version")
    );
    let no_cand_exit = d
        .get("no_candidate_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(no_cand_exit, 0);
    assert_ne!(
        yanked_exit, no_cand_exit,
        "yanked-rejection and no-candidate exit codes must differ"
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("every_pypi_metadata_edge_case")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
