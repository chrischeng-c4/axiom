...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("stdlib_urllib_parse_email_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("stdlib_urllib_parse_email_behavioral"));
...
fn surface_registers_urllib_parse_and_email() {
    ...
    assert_eq!(
        s.get("urllib_parse_import_statement").and_then(|v| v.as_str()),
...
fn deterministic_sample_covers_urlparse_urlencode_email() {
    ...
    let specs: &[(&str, &[&str])] = &[
        ("urlparse_cases", &["input_url", "expected_scheme", "expected_netloc", "expected_path"]),
        ("urlencode_cases", &["input_pairs", "expected_output"]),
        ("email_message_cases", &[
            "header_from", "header_to", "header_subject",
            "content_type", "charset", "payload_text",
            "expected_get_content_value", "expected_get_content_type_value",
        ]),
    ];
...
fn fixture_fails_on_wrong_url_component_parsing() {
    let doc = crate::common::load_toml(&manifest_path());
    let f = doc.get("failure_on_wrong_url_component_contract").and_then(|v| v.as_table()).expect(
    ...
    let parse_exit = f.get("url_component_mismatch_exit_code").and_then(|v| v.as_integer()).unwrap();
    let encode_exit = f.get("urlencode_mismatch_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(parse_exit, 116);
    assert_eq!(encode_exit, 117);
    assert_ne!(parse_exit, encode_exit, "urlparse and urlencode exit codes must differ");
    assert_eq!(
        f.get("url_component_mismatch_failure_kind").and_then(|v| v.as_str()),
        Some("urlparse_component_mismatch"),
    );
    assert_eq!(
        f.get("urlencode_mismatch_failure_kind").and_then(|v| v.as_str()),
...
fn fixture_fails_on_wrong_email_header_or_payload_behavior() {
    let doc = crate::common::load_toml(&manifest_path());
    let f = doc.get("failure_on_wrong_email_behavior_contract").and_then(|v| v.as_table()).expect(
    ...
    sorted.sort();
    sorted.dedup();
    assert_eq!(sorted.len(), exits.len(), "email failure exit codes must all differ");
    for (k, v) in &[
        ("email_header_mismatch_failure_kind", "email_header_mismatch"),
        ("email_payload_mismatch_failure_kind", "email_payload_mismatch"),
        ("email_content_type_mismatch_failure_kind", "email_content_type_mismatch"),
    ] {
...
fn no_network_io_is_used() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc.get("no_network_io_contract").and_then(|v| v.as_table()).expect(
        "[no_network_io_contract] missing — acceptance: \"No network I/O is used.\"",
    );
    for k in &[
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "input_url", "expected_scheme", "expected_netloc", "expected_path",
        "input_pairs", "expected_output",
        "header_from", "header_to", "header_subject", "payload_text",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("http_client_behavior").and_then(|v| v.as_bool()), Some(true));
}
