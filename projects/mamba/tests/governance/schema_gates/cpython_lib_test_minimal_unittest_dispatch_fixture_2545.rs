...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("cpython_lib_test_minimal_unittest_dispatch"));
    assert_eq!(doc.get("issue").and_then(|v| v.as_integer()), Some(2545));
    assert_eq!(doc.get("parent_issue").and_then(|v| v.as_integer()), Some(2528));
    assert_eq!(doc.get("profile").and_then(|v| v.as_str()), Some("cpython_lib_test"));
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("cpython_lib_test_minimal_unittest_dispatch"));
...
fn determinism_pins_no_external_state() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("determinism").and_then(|v| v.as_table()).expect("[determinism] missing");
    for k in &[
...
fn seed_module_definition_pins_canonical_assertion() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc.get("seed_module_definition").and_then(|v| v.as_table()).expect("[seed_module_definition] missing");
    assert_eq!(s.get("module_name").and_then(|v| v.as_str()), Some("test_mamba_2545_minimal"));
    assert_eq!(s.get("testcase_class_name").and_then(|v| v.as_str()), Some("Mamba2545MinimalTestCase"));
    assert_eq!(s.get("testcase_method_name").and_then(|v| v.as_str()), Some("test_two_plus_two_equals_four"));
    assert_eq!(s.get("assertion_kind").and_then(|v| v.as_str()), Some("assertEqual"));
    assert_eq!(s.get("assertion_lhs_python_repr").and_then(|v| v.as_str()), Some("2 + 2"));
    assert_eq!(s.get("assertion_rhs_python_repr").and_then(|v| v.as_str()), Some("4"));
    assert_eq!(s.get("sentinel_inversion_lhs_python_repr").and_then(|v| v.as_str()), Some("2 + 2"));
    assert_eq!(s.get("sentinel_inversion_rhs_python_repr").and_then(|v| v.as_str()), Some("5"));
}

#[test]
fn runner_command_pins_canonical_invocation() {
    let doc = crate::common::load_toml(&manifest_path());
    let r = doc.get("runner_command").and_then(|v| v.as_table()).expect("[runner_command] missing");
    for k in &[
    ...
    assert_eq!(r.get("runner_command_field_name").and_then(|v| v.as_str()), Some("runner_command"));
    assert_eq!(
...
fn fixture_fails_when_sentinel_assertion_is_inverted() {
    let doc = crate::common::load_toml(&manifest_path());
    let c = doc.get("sentinel_failure_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(
        c.get("original_assertion_expected_outcome").and_then(|v| v.as_str()),
        Some("pass"),
    );
    assert_eq!(
        c.get("inverted_assertion_expected_outcome").and_then(|v| v.as_str()),
        Some("fail"),
    );
    let assert_exit = c.get("inverted_assertion_exit_code").and_then(|v| v.as_integer()).unwrap();
    let runner_exit = c.get("runner_error_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(assert_exit, 176);
    assert_eq!(runner_exit, 177);
    assert_ne!(assert_exit, runner_exit, "assertion-failure and runner-error exit codes must differ");
    assert_eq!(
        c.get("inverted_assertion_failure_kind").and_then(|v| v.as_str()),
...
fn fixture_is_not_classified_as_stub() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc.get("non_stub_classification_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(n.get("classification_field_name").and_then(|v| v.as_str()), Some("classification"));
    let allowed: Vec<&str> = n.get("allowed_classification_values").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for v in &["real", "executed"] {
        assert!(allowed.contains(v), "allowed_classification_values must include {v}");
    }
    assert!(!allowed.contains(&"stub"), "allowed_classification_values must NOT include 'stub'");
    assert!(!allowed.contains(&"passthrough"), "allowed_classification_values must NOT include 'passthrough'");
    assert!(!allowed.contains(&"placeholder"), "allowed_classification_values must NOT include 'placeholder'");
    let exit = n.get("stub_classification_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 178);
    assert_eq!(
        n.get("stub_classification_failure_kind").and_then(|v| v.as_str()),
...
fn no_broad_unittest_implementation_work_is_included_here() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc.get("narrow_dispatch_scope_contract").and_then(|v| v.as_table()).expect(
    ...
    let required: Vec<&str> = s.get("required_dispatch_surface").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for v in &[
        "unittest.TestCase",
        ...
        assert!(required.contains(v), "required_dispatch_surface must include {v}");
    }
    let forbidden: Vec<&str> = s.get("forbidden_surface").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for v in &[
    ...
    let exit = s.get("scope_overshoot_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 179);
    assert_eq!(
        s.get("scope_overshoot_failure_kind").and_then(|v| v.as_str()),
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "testcase_class_name", "testcase_method_name",
        "assertion_kind", "runner_command", "classification",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("full_unittest_compatibility_beyond_dispatch_path").and_then(|v| v.as_bool()), Some(true));
}
