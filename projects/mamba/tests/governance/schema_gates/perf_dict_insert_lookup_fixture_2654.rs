...
fn workload_is_deterministic_dict_insert_lookup() {
    ...
    assert_eq!(w.get("must_pre_generate_keys").and_then(|v| v.as_bool()), Some(true));
}
...
fn timing_contract_declares_tier_and_mode() {
    ...
    assert!(allowed.contains(&mode), "timing_mode {mode} must be in allowed_timing_modes");
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    for k in &[
        ...
        "expected_checksum", "speedup", "outcome", "exit_code",
