...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("third_party_pluggy_hook_behavioral")
    );
...
fn surface_covers_pluggy() {
    ...
    let modules: Vec<&str> = s
        ...
        .unwrap_or_default();
    assert!(
        modules.contains(&"pluggy"),
        "covered_modules must include pluggy"
    );
    for f in &[
...
fn plugin_definition_pins_canonical_plugin() {
    ...
    let p = doc
        ...
        .and_then(|v| v.as_table())
        .expect("[plugin_definition] missing");
    assert_eq!(
        p.get("project_name").and_then(|v| v.as_str()),
        Some("mamba2647")
    );
    assert_eq!(
        p.get("hook_name").and_then(|v| v.as_str()),
        Some("mamba_2647_compute")
    );
    assert_eq!(
        p.get("hookspec_signature").and_then(|v| v.as_str()),
        Some("(x: int, y: int) -> int")
    );
    assert_eq!(
        p.get("hookimpl_returns_python_repr")
            .and_then(|v| v.as_str()),
        Some("x + y")
    );
    assert_eq!(
        p.get("plugin_class_name").and_then(|v| v.as_str()),
        Some("Mamba2647Plugin")
    );
}
...
fn deterministic_sample_covers_hook_calls() {
    ...
    let arr = doc
        .get("hook_call_cases")
        .and_then(|v| v.as_array())
        .expect("[[hook_call_cases]] missing");
...
fn fixture_fails_if_pluggy_cannot_import() {
    ...
    let exit = i
        .get("pluggy_import_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 163);
    assert_eq!(
...
fn fixture_asserts_hook_call_result_exactly() {
    ...
    let mismatch = c
        .get("hook_result_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let invoc = c
        .get("hook_invocation_error_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    ...
    assert_ne!(
        mismatch, invoc,
        "mismatch and invocation-error exit codes must differ"
    );
    assert_eq!(
        c.get("hook_result_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("pluggy_hook_result_mismatch"),
    );
    assert_eq!(
        c.get("hook_invocation_error_failure_kind")
            .and_then(|v| v.as_str()),
...
fn runner_records_pluggy_as_plugin_framework_coverage() {
    let doc = load_toml(&manifest_path());
    let h = doc
        .get("plugin_framework_coverage_reporting_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        h.get("plugin_framework_coverage_field_name")
            .and_then(|v| v.as_str()),
        Some("plugin_framework_dependencies_covered"),
    );
    let req: Vec<&str> = h
        .get("required_plugin_framework_dependencies_covered")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    assert!(
        req.contains(&"pluggy"),
        "required_plugin_framework_dependencies_covered must include pluggy"
    );
    assert_eq!(
    ...
    let exit = h
        .get("missing_plugin_framework_coverage_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 166);
    assert_eq!(
        h.get("missing_plugin_framework_coverage_failure_kind")
            .and_then(|v| v.as_str()),
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "project_name",
        "hook_name",
        "call_args_python_repr",
        "expected_result_python_repr",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("pytest_behavior_beyond_pluggy_smoke")
