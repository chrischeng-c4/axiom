...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_subprocess_smoke_behavioral")
    );
...
fn surface_registers_subprocess() {
    ...
    let modules: Vec<&str> = s
        ...
        .unwrap_or_default();
    assert!(
        modules.contains(&"subprocess"),
        "covered_modules must include subprocess"
    );
    for f in &[
...
fn deterministic_sample_pins_smoke_commands() {
    ...
    for c in arr {
        ...
        assert_eq!(
            t.get("shell").and_then(|v| v.as_bool()),
            Some(false),
...
fn fixture_does_not_invoke_shell_with_untrusted_input() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc
        .get("no_shell_untrusted_input_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let sh = n
        .get("shell_used_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let unt = n
        .get("untrusted_input_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(sh, 137);
    assert_eq!(unt, 138);
    assert_ne!(
        sh, unt,
        "shell-used and untrusted-input exit codes must differ"
    );
    assert_eq!(
    ...
    assert_eq!(
        n.get("untrusted_input_failure_kind")
            .and_then(|v| v.as_str()),
...
fn unsupported_behavior_is_visible_as_blocker_not_silent_skip() {
    ...
    let allowed: Vec<&str> = u
        .get("allowed_unsupported_subprocess_paths")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    assert!(
        !allowed.is_empty(),
        "allowed_unsupported_subprocess_paths must list at least one entry"
    );
}
...
fn runner_records_subprocess_status_separately() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc
        .get("separate_status_recording_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        s.get("status_field_name").and_then(|v| v.as_str()),
        Some("subprocess_status")
    );
    assert_eq!(
    ...
    let exit = s
        .get("missing_status_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 140);
    assert_eq!(
        s.get("missing_status_failure_kind")
            .and_then(|v| v.as_str()),
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("shell_pipelines").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        o.get("signal_handling").and_then(|v| v.as_bool()),
        Some(true)
    );
}
