...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_import_smoke_matrix")
    );
...
fn cross_references_required_module_manifest_fixture() {
    let doc = crate::common::load_toml(&manifest_path());
    let x = doc
        .get("required_module_manifest_cross_reference")
        .and_then(|v| v.as_table())
        .expect("[required_module_manifest_cross_reference] missing");
    assert_eq!(
        x.get("fixture_issue").and_then(|v| v.as_integer()),
        Some(2624)
    );
    assert_eq!(
        x.get("must_consume_required_module_manifest")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        x.get("must_match_required_module_set_exactly")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let shared: Vec<&str> = x
        .get("shared_required_modules")
    ...
    for m in &["json", "datetime", "pathlib", "logging", "argparse"] {
        assert!(
            shared.contains(m),
            "shared_required_modules must include {m}"
        );
    }
}
...
fn matrix_record_format_is_pinned() {
    ...
    let m = doc
        .get("matrix_record")
        .and_then(|v| v.as_table())
        .expect("[matrix_record] missing");
    assert_eq!(
        m.get("record_format").and_then(|v| v.as_str()),
        Some("json")
    );
    let required: Vec<&str> = m
        .get("record_required_fields")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for f in &[
        ...
        assert!(
            required.contains(f),
            "record_required_fields must include {f}"
        );
    }
    let outcomes: Vec<&str> = m
        .get("outcome_values")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    ...
    assert_eq!(
        m.get("must_emit_one_record_per_required_module")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        m.get("must_emit_one_record_per_optional_module")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        m.get("must_distinguish_required_from_optional_in_record")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        m.get("required_flag_field_name").and_then(|v| v.as_str()),
        Some("required")
    );
}
...
fn smoke_budget_is_bounded() {
    ...
    let s = doc
        .get("smoke_budget")
        .and_then(|v| v.as_table())
        .expect("[smoke_budget] missing");
    assert_eq!(
        s.get("must_run_in_smoke_profile").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        s.get("smoke_profile_fixture_issue")
    ...
    let total = s
        ...
        .unwrap();
    let per = s
        .get("max_per_module_runtime_ms")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert!(
        total > 0 && per > 0 && per <= total,
        "runtime bounds must be positive and per≤total"
    );
    assert!(
        total <= 10_000,
        "smoke total runtime must stay light, got {total}ms"
    );
    assert_eq!(
        s.get("must_run_imports_in_isolated_subprocess_or_subinterpreter")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn removing_required_module_from_matrix_fails_validation() {
    let doc = crate::common::load_toml(&manifest_path());
    let c = doc
        .get("required_module_coverage_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let exit = c
        .get("missing_required_module_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 67);
    assert_eq!(
        c.get("missing_required_module_failure_kind")
            .and_then(|v| v.as_str()),
        Some("required_stdlib_module_missing_from_import_matrix")
    );
}
...
fn import_failure_names_the_exact_module() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc
        .get("import_failure_naming_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let required: Vec<&str> = n
        .get("import_failure_record_required_fields")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for f in &[
        ...
        assert!(
            required.contains(f),
            "import_failure_record_required_fields must include {f}"
        );
    }
    let exit = n
        .get("import_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 68);
    assert_eq!(
        n.get("import_failure_failure_kind")
            .and_then(|v| v.as_str()),
        Some("stdlib_module_import_failed")
    );
}
...
fn smoke_summary_includes_stdlib_import_totals() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc
        .get("smoke_summary_totals_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        s.get("must_emit_stdlib_import_totals_in_smoke_summary")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        s.get("totals_record_format").and_then(|v| v.as_str()),
        Some("json")
    );
    ...
    assert_eq!(
        s.get("must_be_consumable_by_smoke_summary_emitter")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        s.get("smoke_summary_fixture_issue")
            .and_then(|v| v.as_integer()),
        Some(2527)
    );
    let exit = s
        .get("missing_stdlib_totals_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 69);
    assert_eq!(
        s.get("missing_stdlib_totals_failure_kind")
            .and_then(|v| v.as_str()),
        Some("smoke_summary_missing_stdlib_import_totals")
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("behavioral_correctness_for_each_module")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
