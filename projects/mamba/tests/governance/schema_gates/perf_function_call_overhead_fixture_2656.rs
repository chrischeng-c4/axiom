//! Schema gate for the function-call overhead benchmark fixture —
//! closes #2656.
...
fn manifest_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("gates")
        .join("performance")
        .join("function_call_overhead")
        .join("manifest.toml")
}

const BENCH_ID: &str = "function_call_overhead";
const ISSUE: i64 = 2656;
const FIXTURE: &str = "perf_function_call_overhead";
const OOS_FLAG: &str = "call_dispatch_optimization";
...
#[test]
fn workload_is_pure_python_tight_loop() {
    let doc = crate::common::load_toml(&manifest_path());
    let w = doc.get("workload").and_then(|v| v.as_table()).unwrap();
    assert_eq!(w.get("function_kind").and_then(|v| v.as_str()), Some("pure_python"));
    assert_eq!(w.get("must_be_pure").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(w.get("must_not_allocate_per_call").and_then(|v| v.as_bool()), Some(true));
    assert!(w.get("n_iterations").and_then(|v| v.as_integer()).unwrap() > 0);
    assert_eq!(w.get("deterministic").and_then(|v| v.as_bool()), Some(true));
}

// Acceptance: "Wrong computed result fails before timing is accepted."
#[test]
fn wrong_result_fails_before_timing() {
    let doc = crate::common::load_toml(&manifest_path());
    let c = doc.get("checksum_contract").and_then(|v| v.as_table()).expect(
        "[checksum_contract] missing — acceptance: \
         \"Wrong computed result fails before timing is accepted.\"",
    );
...
fn timing_contract_declares_tier_and_mode() {
    ...
    assert!(t.get("measured_iterations").and_then(|v| v.as_integer()).unwrap() >= 1);
}
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    for k in &[
        ...
        "expected_checksum", "baseline_label", "baseline_version",
        "speedup", "outcome", "exit_code",
