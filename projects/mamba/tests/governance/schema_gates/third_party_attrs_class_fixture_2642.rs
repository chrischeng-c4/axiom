...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("third_party_attrs_class_behavioral")
    );
...
fn attrs_class_definition_pins_canonical_class() {
    let doc = load_toml(&manifest_path());
    let a = doc
        .get("attrs_class_definition")
        .and_then(|v| v.as_table())
        .expect("[attrs_class_definition] missing");
    assert_eq!(
        a.get("class_name").and_then(|v| v.as_str()),
        Some("Point2642")
    );
    assert_eq!(a.get("field_a_name").and_then(|v| v.as_str()), Some("x"));
    assert_eq!(
        a.get("field_b_name").and_then(|v| v.as_str()),
        Some("label")
    );
    assert_eq!(a.get("field_c_name").and_then(|v| v.as_str()), Some("n"));
    assert_eq!(
        a.get("field_c_converter").and_then(|v| v.as_str()),
        Some("int")
    );
}
...
fn fixture_fails_if_attrs_cannot_import() {
    ...
    let exit = i
        .get("attrs_import_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 141);
    assert_eq!(
...
fn fixture_asserts_constructor_default_and_equality_or_repr() {
    let doc = load_toml(&manifest_path());
    let c = doc
        .get("constructor_default_equality_repr_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    .iter()
    .map(|k| c.get(*k).and_then(|v| v.as_integer()).unwrap())
    .collect();
    ...
    assert_eq!(
        sorted.len(),
        exits.len(),
        "attrs behavior exit codes must all differ"
    );
}
...
fn unsupported_behavior_visible_as_xfail_or_blocker_with_issue_reference() {
    let doc = load_toml(&manifest_path());
    let u = doc
        .get("unsupported_behavior_xfail_or_blocker_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let allowed: Vec<&str> = u
        .get("allowed_unsupported_outcome_values")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for v in &["xfail", "blocker"] {
        assert!(
            allowed.contains(v),
            "allowed_unsupported_outcome_values must include {v}"
        );
    }
    assert_eq!(
        u.get("unsupported_outcome_link_field_name")
            .and_then(|v| v.as_str()),
        Some("blocker_issue")
    );
    let exit = u
        .get("missing_link_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 146);
    assert_eq!(
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("full_attrs_feature_coverage")
