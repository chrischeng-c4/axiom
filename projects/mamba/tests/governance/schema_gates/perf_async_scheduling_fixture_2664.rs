...
fn workload_avoids_real_time_sleeps_and_awaits_many_tiny_coroutines() {
    ...
    assert_eq!(w.get("must_use_event_loop").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(w.get("forbid_asyncio_sleep_with_nonzero_delay").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(w.get("forbid_time_sleep").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(w.get("forbid_real_time_dependency").and_then(|v| v.as_bool()), Some(true));
...
fn timing_contract_does_not_depend_on_real_time_sleep() {
    ...
    assert_eq!(t.get("must_not_depend_on_real_time_sleep_duration").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(t.get("forbid_wall_clock_sleep_in_workload").and_then(|v| v.as_bool()), Some(true));
}
...
fn status_contract_links_unsupported_to_blocker() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc.get("status_contract").and_then(|v| v.as_table()).expect(
        "[status_contract] missing — acceptance: \
         \"Unsupported async behavior is visible and linked to a blocker.\"",
    );
    let states: Vec<&str> = s.get("allowed_status_values").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for st in &["pass", "xfail", "blocker"] {
        assert!(states.contains(st), "allowed_status_values must include {st}");
    }
    assert_eq!(s.get("must_link_blocker_to_issue_when_status_is_blocker").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("unsupported_async_behavior_must_be_visible").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("must_appear_in_summary").and_then(|v| v.as_bool()), Some(true));
}
...
fn pass_path_reports_speedup_against_cpython_3_12() {
    ...
    assert_eq!(s.get("must_only_record_speedup_on_pass").and_then(|v| v.as_bool()), Some(true));
    let req: Vec<&str> = s.get("required_record_keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for k in &["benchmark_id", "status", "baseline_label", "baseline_version", "speedup", "outcome"] {
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for k in &[
        "benchmark_id", "status", "tier", "timing_mode", "n_coroutines",
        "result_checksum", "expected_result_checksum",
        "completion_count", "expected_completion_count",
        "baseline_label", "baseline_version",
        "speedup", "outcome", "blocker_issue", "exit_code",
