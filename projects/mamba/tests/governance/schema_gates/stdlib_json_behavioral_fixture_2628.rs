...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("stdlib_json_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("stdlib_json_behavioral"));
...
fn surface_covers_dumps_loads_and_all_value_kinds() {
    ...
    assert_eq!(s.get("import_statement").and_then(|v| v.as_str()), Some("import json"));
    for f in &[
        "must_cover_dumps", "must_cover_loads",
        "must_cover_dict", "must_cover_list",
        "must_cover_string_escaping", "must_cover_numeric",
        "must_cover_boolean", "must_cover_null",
    ] {
...
fn deterministic_sample_covers_all_value_kinds_and_escaping() {
    ...
    let roundtrips = doc.get("roundtrip_cases").and_then(|v| v.as_array()).expect("[[roundtrip_cases]] missing");
    let mut kinds: Vec<&str> = Vec::new();
    for c in roundtrips {
        let t = c.as_table().expect("roundtrip case must be a table");
        let k = t.get("kind").and_then(|v| v.as_str()).expect("roundtrip.kind missing");
        assert!(t.get("input_python_repr").and_then(|v| v.as_str()).is_some(), "roundtrip.input_python_repr missing");
        assert!(t.get("expected_dumps").and_then(|v| v.as_str()).is_some(), "roundtrip.expected_dumps missing");
        kinds.push(k);
    }
    ...
    let escapings = doc.get("escaping_cases").and_then(|v| v.as_array()).expect("[[escaping_cases]] missing");
    assert!(!escapings.is_empty(), "escaping_cases must not be empty");
    for c in escapings {
        let t = c.as_table().expect("escaping case must be a table");
        assert!(t.get("input_python_string").and_then(|v| v.as_str()).is_some(), "escaping.input_python_string missing");
        assert!(t.get("ensure_ascii").and_then(|v| v.as_bool()).is_some(), "escaping.ensure_ascii missing");
        assert!(t.get("expected_dumps").and_then(|v| v.as_str()).is_some(), "escaping.expected_dumps missing");
    }
}
...
fn fixture_fails_on_wrong_roundtrip_or_escaping() {
    ...
    let rt = f.get("roundtrip_mismatch_exit_code").and_then(|v| v.as_integer()).unwrap();
    let esc = f.get("escaping_mismatch_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(rt, 70);
    assert_eq!(esc, 71);
    assert_ne!(rt, esc, "roundtrip and escaping exit codes must differ");
    assert_eq!(f.get("roundtrip_mismatch_failure_kind").and_then(|v| v.as_str()), Some("json_roundtrip_mismatch"));
    assert_eq!(f.get("escaping_mismatch_failure_kind").and_then(|v| v.as_str()), Some("json_escaping_mismatch"));
}
...
fn fixture_is_part_of_required_ecosystem_gate() {
    ...
    assert_eq!(r.get("manifest_module_name").and_then(|v| v.as_str()), Some("json"));
...
fn failure_output_names_json_as_the_module() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc.get("failure_naming_contract").and_then(|v| v.as_table()).expect(
        "[failure_naming_contract] missing — acceptance: \
         \"Failure output names json as the module.\"",
    );
    assert_eq!(n.get("must_emit_module_name_in_failure_output").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(n.get("module_name_field_name").and_then(|v| v.as_str()), Some("module_name"));
    assert_eq!(n.get("module_name_value").and_then(|v| v.as_str()), Some("json"));
    assert_eq!(n.get("forbid_generic_unnamed_json_failure").and_then(|v| v.as_bool()), Some(true));
    let exit = n.get("unnamed_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 73);
    assert_eq!(n.get("unnamed_failure_failure_kind").and_then(|v| v.as_str()), Some("json_failure_missing_module_name"));
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "roundtrip_kind", "input_python_repr", "expected_dumps", "actual_dumps",
        "ensure_ascii", "input_python_string",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("full_json_compliance_suite").and_then(|v| v.as_bool()), Some(true));
}
