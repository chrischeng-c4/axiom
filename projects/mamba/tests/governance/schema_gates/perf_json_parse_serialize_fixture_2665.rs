...
fn workload_round_trips_a_fixed_payload() {
    ...
    assert_eq!(
        w.get("payload_must_be_fixed").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        w.get("payload_must_be_deterministic")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        w.get("must_exercise_json_loads").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        w.get("must_exercise_json_dumps").and_then(|v| v.as_bool()),
        Some(true)
    );
...
fn wrong_json_behavior_fails_benchmark() {
    ...
    assert_eq!(
        c.get("must_validate_parsed_field_set")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_validate_parsed_field_values")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_validate_serialized_shape")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_validate_round_trip_equality")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
...
fn timing_contract_declares_tier_and_mode() {
    ...
    assert_eq!(
        t.get("must_separate_loads_and_dumps_timings")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn speedup_reported_against_cpython_3_12() {
    ...
    assert_eq!(
        s.get("must_report_loads_and_dumps_speedups_separately")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let req: Vec<&str> = s
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    for k in &[
        ...
        "n_iterations",
        "parsed_field_set",
        "expected_parsed_field_set",
        "serialized_shape",
        "expected_serialized_shape",
        "baseline_label",
        "baseline_version",
        "loads_seconds",
        "dumps_seconds",
