...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("venv_site_packages_activation"));
...
fn package_under_test_is_pure_python() {
    let doc = crate::common::load_toml(&manifest_path());
    let p = doc.get("package_under_test").and_then(|v| v.as_table()).expect("[package_under_test] missing");
    let name = p.get("name").and_then(|v| v.as_str()).unwrap();
    assert!(!name.is_empty());
    assert!(!p.get("version").and_then(|v| v.as_str()).unwrap().is_empty());
    ...
    assert_eq!(import_name, name, "import_name must match package name for a pure-Python package");
...
fn import_succeeds_only_after_install_or_sync() {
    let doc = crate::common::load_toml(&manifest_path());
    let c = doc.get("install_then_import_case").and_then(|v| v.as_table()).expect(
    ...
    let allowed: Vec<&str> = c.get("allowed_activation_commands").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for cmd in &["install", "sync"] {
        assert!(allowed.contains(cmd), "allowed_activation_commands must include {cmd}");
    }
    assert_eq!(c.get("pre_install_failure_kind").and_then(|v| v.as_str()), Some("import_succeeded_before_install"));
    let pre_exit = c.get("pre_install_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(pre_exit, 0);
    assert_eq!(pre_exit, 19);
    assert_eq!(c.get("post_install_failure_kind").and_then(|v| v.as_str()), Some("import_failed_after_install"));
    let post_exit = c.get("post_install_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(post_exit, 0);
    assert_eq!(post_exit, 20);
    assert_ne!(pre_exit, post_exit, "pre- and post-install failure exit codes must differ");
}
...
fn test_fails_if_site_packages_not_on_runtime_path() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc.get("site_packages_path_contract").and_then(|v| v.as_table()).expect(
        "[site_packages_path_contract] missing — acceptance: \
         \"Test fails if site-packages is not on the runtime path.\"",
    );
    assert_eq!(s.get("must_assert_site_packages_on_runtime_path").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("must_assert_site_packages_path_is_under_temp_env").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("site_packages_missing_failure_kind").and_then(|v| v.as_str()), Some("site_packages_not_on_runtime_path"));
    let exit = s.get("site_packages_missing_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 21);
    assert_eq!(s.get("diagnostic_must_print_sys_path_when_missing").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("must_distinguish_from_import_failed_after_install").and_then(|v| v.as_bool()), Some(true));

    let post_exit = doc.get("install_then_import_case")
        .and_then(|v| v.get("post_install_failure_exit_code"))
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(exit, post_exit, "site-packages-missing exit code must differ from import-failed-after-install");
}
...
fn test_avoids_global_python_or_user_cache_state() {
    let doc = crate::common::load_toml(&manifest_path());
    let g = doc.get("global_state_isolation").and_then(|v| v.as_table()).expect(
    ...
    for f in &[
        ...
        assert_eq!(g.get(*f).and_then(|v| v.as_bool()), Some(true), "{f} must be true");
    }
    let forbidden: Vec<&str> = g.get("forbidden_env_vars").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for required in &["PYTHONPATH", "PYTHONHOME", "PIP_CACHE_DIR", "MAMBA_CACHE_DIR"] {
        assert!(forbidden.contains(required), "forbidden_env_vars must include {required}");
    }
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for required in &[
        "outcome", "case", "package", "version", "import_statement",
        "phase", "sys_path", "site_packages_path",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("virtualenv_ux_polish").and_then(|v| v.as_bool()), Some(true));
}
