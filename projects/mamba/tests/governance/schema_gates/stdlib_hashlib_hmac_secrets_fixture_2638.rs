...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_hashlib_hmac_secrets_behavioral")
    );
    ...
    let specs: &[(&str, &[&str])] = &[
        ...
        (
            ...
            &[
                ...
                "expected_output_len",
                "must_assert_shape_only",
                "forbid_assert_exact_value",
            ],
        ),
    ];
...
fn fixture_fails_on_wrong_digest_or_comparison_result() {
    let doc = load_toml(&manifest_path());
    let f = doc
        .get("failure_on_wrong_digest_or_comparison_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let sha = f
        .get("sha256_digest_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let hmac = f
        .get("hmac_compare_digest_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(sha, 122);
    assert_eq!(hmac, 123);
    ...
    assert_eq!(
        f.get("sha256_digest_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("sha256_digest_mismatch"),
    );
    assert_eq!(
        f.get("hmac_compare_digest_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
...
fn random_output_is_tested_only_by_shape_or_length() {
    let doc = load_toml(&manifest_path());
    let s = doc
        .get("shape_only_random_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let exit = s
        .get("exact_value_assertion_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 124);
    assert_eq!(
        s.get("exact_value_assertion_failure_kind")
            .and_then(|v| v.as_str()),
...
fn no_external_services_are_used() {
    let doc = load_toml(&manifest_path());
    let n = doc
        .get("no_external_services_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let exit = n
        .get("external_service_use_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 125);
    assert_eq!(
        n.get("external_service_use_failure_kind")
            .and_then(|v| v.as_str()),
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("cryptographic_performance").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        o.get("openssl_provider_coverage").and_then(|v| v.as_bool()),
        Some(true)
    );
}
