...
fn workload_exercises_single_and_multi_group_patterns() {
    ...
    assert_eq!(w.get("must_exercise_single_group").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(w.get("must_exercise_multi_group").and_then(|v| v.as_bool()), Some(true));
    let pats = w.get("patterns").and_then(|v| v.as_array()).unwrap();
    assert!(pats.len() >= 2, "must declare at least one single-group and one multi-group pattern");
    let groups: Vec<i64> = pats.iter()
        .filter_map(|p| p.as_table().and_then(|t| t.get("group_count")).and_then(|v| v.as_integer()))
        .collect();
    assert!(groups.iter().any(|&g| g == 1), "must include a single-group pattern");
    assert!(groups.iter().any(|&g| g >= 2), "must include a multi-group pattern");
...
fn wrong_match_fails_before_speedup_accepted() {
    ...
    assert_eq!(c.get("must_validate_match_count").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("must_validate_match_checksum").and_then(|v| v.as_bool()), Some(true));
    ...
    assert_eq!(c.get("on_mismatch_must_fail_before_speedup_accepted").and_then(|v| v.as_bool()), Some(true));
...
fn appears_in_machine_readable_summary() {
    ...
    let req: Vec<&str> = s.get("required_record_keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for k in &["benchmark_id", "match_count", "match_checksum", "speedup", "outcome"] {
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    for k in &[
        ...
        "match_checksum", "expected_match_checksum",
