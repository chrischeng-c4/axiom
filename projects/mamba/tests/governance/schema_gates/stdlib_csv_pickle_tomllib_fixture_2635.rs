...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("stdlib_csv_pickle_tomllib_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("stdlib_csv_pickle_tomllib_behavioral"));
...
fn deterministic_sample_covers_csv_pickle_tomllib() {
    ...
    let specs: &[(&str, &[&str])] = &[
        ...
        ("csv_write_cases", &["input_rows", "expected_output_text", "line_terminator"]),
        ("pickle_roundtrip_cases", &["input_python_repr", "protocol", "must_match_after_loads"]),
        ("tomllib_parse_cases", &["input_text", "expected_python_repr", "runtime_outcome"]),
    ];
    ...
    let pickle = doc.get("pickle_roundtrip_cases").and_then(|v| v.as_array()).unwrap();
    for c in pickle {
        let t = c.as_table().unwrap();
        assert_eq!(t.get("protocol").and_then(|v| v.as_str()), Some("HIGHEST_PROTOCOL"));
        assert_eq!(t.get("must_match_after_loads").and_then(|v| v.as_bool()), Some(true));
    }
}
...
fn fixture_fails_on_wrong_parsed_or_roundtripped_data() {
    ...
    sorted.sort();
    sorted.dedup();
    assert_eq!(sorted.len(), exits.len(), "serializer mismatch exit codes must all differ");

    for (k, v) in &[
        ("csv_read_mismatch_failure_kind", "csv_read_mismatch"),
        ("csv_write_mismatch_failure_kind", "csv_write_mismatch"),
        ("pickle_roundtrip_mismatch_failure_kind", "pickle_roundtrip_mismatch"),
        ("tomllib_parse_mismatch_failure_kind", "tomllib_parse_mismatch"),
    ] {
        assert_eq!(f.get(*k).and_then(|v| v.as_str()), Some(*v), "{k} must be {v}");
    }
}
...
fn tomllib_behavior_is_explicit_pass_xfail_or_blocker() {
    let doc = crate::common::load_toml(&manifest_path());
    let t = doc.get("tomllib_runtime_status_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(t.get("runtime_status_field_name").and_then(|v| v.as_str()), Some("runtime_outcome"));
    assert_eq!(t.get("blocker_link_field_name").and_then(|v| v.as_str()), Some("blocker_issue"));

    let allowed: Vec<&str> = t.get("allowed_runtime_status_values").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for v in &["pass", "xfail", "blocker"] {
        assert!(allowed.contains(v), "allowed_runtime_status_values must include {v}");
    }

    let exit = t.get("implicit_tomllib_status_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 109);
    assert_eq!(
        t.get("implicit_tomllib_status_failure_kind").and_then(|v| v.as_str()),
...
fn no_network_or_locale_dependency() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc.get("no_network_or_locale_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(n.get("pinned_csv_dialect_name").and_then(|v| v.as_str()), Some("excel"));

    let net = n.get("network_io_exit_code").and_then(|v| v.as_integer()).unwrap();
    let loc = n.get("locale_dependency_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(net, 110);
    assert_eq!(loc, 111);
    assert_ne!(net, loc, "network and locale dependency exit codes must differ");
    ...
    assert_eq!(
        n.get("locale_dependency_failure_kind").and_then(|v| v.as_str()),
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "csv_input_text", "csv_expected_rows", "csv_expected_output_text",
        "pickle_input_python_repr",
        "tomllib_input_text", "tomllib_expected_python_repr",
        "runtime_outcome", "blocker_issue",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("binary_protocol_compatibility_matrix").and_then(|v| v.as_bool()), Some(true));
}
