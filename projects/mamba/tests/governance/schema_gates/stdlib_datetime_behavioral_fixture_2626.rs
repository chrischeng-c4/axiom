...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_datetime_behavioral")
    );
...
fn deterministic_sample_arrays_are_bounded_and_consistent() {
    ...
    let arithmetic = doc
        .get("arithmetic_cases")
        .and_then(|v| v.as_array())
        .expect("[[arithmetic_cases]] missing");
    assert!(!arithmetic.is_empty(), "arithmetic_cases must not be empty");
    for c in arithmetic {
        let t = c.as_table().expect("arithmetic case must be a table");
        assert!(
            t.get("left").and_then(|v| v.as_str()).is_some(),
            "arithmetic.left missing"
        );
        assert!(
            t.get("delta_days").and_then(|v| v.as_integer()).is_some(),
            "arithmetic.delta_days missing"
        );
        assert!(
            t.get("expected_right").and_then(|v| v.as_str()).is_some(),
            "arithmetic.expected_right missing"
        );
        assert_eq!(
            t.get("operator").and_then(|v| v.as_str()),
            Some("add_timedelta_days")
        );
    }

    let comparison = doc
        .get("comparison_cases")
        .and_then(|v| v.as_array())
        .expect("[[comparison_cases]] missing");
    assert!(!comparison.is_empty(), "comparison_cases must not be empty");
    for c in comparison {
        let t = c.as_table().expect("comparison case must be a table");
        assert!(
            t.get("left").and_then(|v| v.as_str()).is_some(),
            "comparison.left missing"
        );
        assert!(
            t.get("right").and_then(|v| v.as_str()).is_some(),
            "comparison.right missing"
        );
        let rel = t
            .get("expected_relation")
            .and_then(|v| v.as_str())
            .expect("comparison.expected_relation missing");
        assert!(
    ...
    for c in iso {
        ...
        for f in &[
            ...
            assert!(
                t.get(*f).and_then(|v| v.as_integer()).is_some(),
                "isoformat.{f} missing"
            );
        }
    }
...
fn fixture_fails_on_incorrect_arithmetic_or_formatting() {
    ...
    let arith = f
        .get("arithmetic_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let comp = f
        .get("comparison_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let iso = f
        .get("isoformat_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(arith, 62);
    assert_eq!(comp, 63);
    ...
    assert_ne!(
        arith, comp,
        "arithmetic and comparison exit codes must differ"
    );
    assert_ne!(
        arith, iso,
        "arithmetic and isoformat exit codes must differ"
    );
    assert_ne!(comp, iso, "comparison and isoformat exit codes must differ");
    assert_eq!(
        f.get("arithmetic_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("datetime_arithmetic_mismatch")
    );
    assert_eq!(
        f.get("comparison_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("datetime_comparison_mismatch")
    );
    assert_eq!(
        f.get("isoformat_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("datetime_isoformat_mismatch")
    );
}
...
fn timezone_and_locale_behavior_is_deterministic_or_avoided() {
    let doc = crate::common::load_toml(&manifest_path());
    let t = doc
        .get("timezone_locale_safety_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    for m in &["utc_only", "fixed_offset", "naive_in_utc_for_arithmetic"] {
        assert!(
            allowed.contains(m),
            "allowed_timezone_modes must include {m}"
        );
    }
    let forbidden: Vec<&str> = t
        .get("forbidden_timezone_modes")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for m in &[
        "system_local_timezone",
        "implicit_dst_dependent",
        "locale_dependent_strftime",
    ] {
        assert!(
            forbidden.contains(m),
            "forbidden_timezone_modes must include {m}"
        );
    }
    for m in &allowed {
        assert!(
            !forbidden.contains(m),
            "{m} cannot be both allowed and forbidden"
        );
    }
    let exit = t
        .get("nondeterministic_path_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 66);
    assert_eq!(
        t.get("nondeterministic_path_failure_kind")
            .and_then(|v| v.as_str()),
        Some("datetime_nondeterministic_path_used")
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("full_timezone_database_compatibility")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
