...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_importlib_metadata_behavioral")
    );
...
fn surface_covers_importlib_and_importlib_metadata() {
    ...
    assert_eq!(
        ...
        Some("import importlib")
    );
    assert_eq!(
        s.get("metadata_import_statement").and_then(|v| v.as_str()),
        Some("from importlib import metadata as importlib_metadata")
    );
    for f in &[
...
fn deterministic_sample_covers_import_and_not_found() {
    ...
    let specs: &[(&str, &[&str])] = &[
        (
            "import_by_name_cases",
            &["target_module_name", "expected_module_attribute"],
        ),
        (
            "metadata_not_found_cases",
            &[
                "target_package_name",
    ...
    let not_found = doc
        .get("metadata_not_found_cases")
        .and_then(|v| v.as_array())
        .unwrap();
    for c in not_found {
        let t = c.as_table().unwrap();
        assert_eq!(
            t.get("expected_exception_type").and_then(|v| v.as_str()),
            Some("PackageNotFoundError")
        );
        assert_eq!(
            t.get("must_not_perform_network_io")
...
fn fixture_fails_when_dynamic_import_by_name_is_broken() {
    let doc = crate::common::load_toml(&manifest_path());
    let f = doc
        .get("failure_on_broken_dynamic_import_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        sorted.len(),
        exits.len(),
        "dynamic import failure exit codes must all differ"
    );
}
...
fn metadata_not_found_behavior_is_deterministic() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc
        .get("deterministic_not_found_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        d.get("not_found_exception_type_field_name")
            .and_then(|v| v.as_str()),
        Some("exception_type")
    );
    assert_eq!(
        d.get("not_found_exception_type_value")
            .and_then(|v| v.as_str()),
        Some("PackageNotFoundError")
    );
    let exit = d
        .get("nondeterministic_not_found_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 103);
    assert_eq!(
        d.get("nondeterministic_not_found_failure_kind")
            .and_then(|v| v.as_str()),
        Some("metadata_not_found_nondeterministic")
    );
}
...
fn failure_output_distinguishes_importlib_from_metadata() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc
        .get("failure_kind_separation_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        s.get("subject_module_field_name").and_then(|v| v.as_str()),
        Some("subject_module")
    );
    let allowed: Vec<&str> = s
        .get("allowed_subject_module_values")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for m in &["importlib", "importlib.metadata"] {
        assert!(
            allowed.contains(m),
            "allowed_subject_module_values must include {m}"
        );
    }
    let exit = s
        .get("mixed_subject_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 104);
    assert_eq!(
        s.get("mixed_subject_failure_kind").and_then(|v| v.as_str()),
        Some("import_failure_subject_module_mixed")
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("full_installed_distribution_metadata_coverage")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
