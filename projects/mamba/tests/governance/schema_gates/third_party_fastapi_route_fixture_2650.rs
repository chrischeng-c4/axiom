...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("third_party_fastapi_route_behavioral")
    );
...
fn surface_covers_fastapi() {
    ...
    let modules: Vec<&str> = s
        ...
        .unwrap_or_default();
    assert!(
        modules.contains(&"fastapi"),
        "covered_modules must include fastapi"
    );
    for f in &[
...
fn route_definition_pins_canonical_route() {
    let doc = load_toml(&manifest_path());
    let r = doc
        .get("route_definition")
        .and_then(|v| v.as_table())
        .expect("[route_definition] missing");
    assert_eq!(
        r.get("app_variable").and_then(|v| v.as_str()),
        Some("app_2650")
    );
    assert_eq!(
        r.get("route_path").and_then(|v| v.as_str()),
        Some("/mamba/2650")
    );
    assert_eq!(r.get("route_method").and_then(|v| v.as_str()), Some("GET"));
    assert_eq!(
        r.get("handler_name").and_then(|v| v.as_str()),
        Some("mamba_2650_handler")
    );
    assert_eq!(
        r.get("handler_response_status")
            .and_then(|v| v.as_integer()),
        Some(200)
    );
    assert_eq!(
        r.get("handler_response_body_python_repr")
            .and_then(|v| v.as_str()),
        Some("{'ok': True, 'n': 2650}"),
    );
}
...
fn deterministic_sample_covers_route_calls() {
    ...
    let arr = doc
        .get("route_call_cases")
        .and_then(|v| v.as_array())
        .expect("[[route_call_cases]] missing");
...
fn fixture_appears_in_ecosystem_reports_even_if_blocked() {
    let doc = load_toml(&manifest_path());
    let e = doc
        .get("ecosystem_reporting_when_blocked_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        e.get("ecosystem_report_field_name")
            .and_then(|v| v.as_str()),
        Some("ecosystem_dependencies_covered"),
    );
    let req: Vec<&str> = e
        .get("required_ecosystem_dependencies_covered")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    assert!(
        req.contains(&"fastapi"),
        "required_ecosystem_dependencies_covered must include fastapi"
    );
    let exit = e
        .get("missing_ecosystem_report_entry_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 171);
    assert_eq!(
        e.get("missing_ecosystem_report_entry_failure_kind")
            .and_then(|v| v.as_str()),
...
fn import_and_route_setup_failures_are_distinguishable() {
    let doc = load_toml(&manifest_path());
    let c = doc
        .get("import_vs_route_setup_failure_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let import_exit = c
        .get("fastapi_import_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let route_exit = c
        .get("fastapi_route_setup_failure_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(import_exit, 172);
    assert_eq!(route_exit, 173);
    assert_ne!(
        import_exit, route_exit,
        "import and route-setup exit codes must differ"
    );
    assert_eq!(
        c.get("fastapi_import_failure_kind")
            .and_then(|v| v.as_str()),
        Some("third_party_fastapi_import_failed"),
    );
    assert_eq!(
        c.get("fastapi_route_setup_failure_kind")
            .and_then(|v| v.as_str()),
        Some("fastapi_route_setup_failed"),
    );
    assert_eq!(
        c.get("failure_phase_field_name").and_then(|v| v.as_str()),
        Some("failure_phase")
    );
...
fn no_external_network_or_server_process_is_required() {
    let doc = load_toml(&manifest_path());
    let n = doc
        .get("no_external_network_or_server_process_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let net = n
        .get("external_network_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let proc_ = n
        .get("server_process_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(net, 174);
    assert_eq!(proc_, 175);
    assert_ne!(
        net, proc_,
        "external-network and server-process exit codes must differ"
    );
    assert_eq!(
        n.get("external_network_failure_kind")
            .and_then(|v| v.as_str()),
        Some("fastapi_external_network_used"),
    );
    assert_eq!(
        n.get("server_process_failure_kind")
            .and_then(|v| v.as_str()),
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &["pass", "fail", "xfail", "blocker", "missing", "skip"] {
        assert!(
            ...
            "runner_contract.outcome_values must include {required}"
        );
    }
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("full_pydantic_coverage").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        o.get("openapi_generation_coverage")
