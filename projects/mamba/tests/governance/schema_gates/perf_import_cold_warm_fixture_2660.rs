...
fn workload_imports_local_and_stdlib_modules() {
    ...
    assert!(w.get("local_module_name").and_then(|v| v.as_str()).is_some());
    assert!(w.get("stdlib_module_name").and_then(|v| v.as_str()).is_some());
    assert!(w.get("local_module_sentinel_name").and_then(|v| v.as_str()).is_some());
    assert!(w.get("stdlib_sentinel_attribute").and_then(|v| v.as_str()).is_some());
    assert_eq!(w.get("must_invalidate_module_cache_between_cold_runs").and_then(|v| v.as_bool()), Some(true));
...
fn sentinel_values_are_validated() {
    ...
    assert_eq!(c.get("must_validate_local_sentinel_value").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("must_validate_stdlib_sentinel_callable").and_then(|v| v.as_bool()), Some(true));
...
fn timing_contract_declares_startup_cost_inclusion() {
    ...
    assert_eq!(t.get("must_declare_startup_cost_included").and_then(|v| v.as_bool()), Some(true));
    assert!(
        t.get("startup_cost_included").and_then(|v| v.as_bool()).is_some(),
        "startup_cost_included must be explicitly declared as a bool",
    );
    assert_eq!(t.get("must_distinguish_cold_and_warm_timings").and_then(|v| v.as_bool()), Some(true));
}
...
fn summary_distinguishes_cold_from_warm() {
    ...
    assert_eq!(s.get("must_emit_cold_and_warm_speedups_separately").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("must_be_keyed_by_benchmark_id_and_phase").and_then(|v| v.as_bool()), Some(true));
    let phases: Vec<&str> = s.get("allowed_phase_labels").and_then(|v| v.as_array())
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for k in &[
        "benchmark_id", "phase", "tier", "timing_mode", "startup_cost_included",
        "checksum", "expected_checksum",
        "cold_speedup", "warm_speedup", "outcome", "exit_code",
