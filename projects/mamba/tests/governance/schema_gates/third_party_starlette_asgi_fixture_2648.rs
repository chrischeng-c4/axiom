...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("third_party_starlette_asgi_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("third_party_starlette_asgi_behavioral"));
...
fn surface_covers_starlette() {
    ...
    assert!(modules.contains(&"starlette"), "covered_modules must include starlette");
    for f in &[
    ...
    assert_eq!(s.get("import_statement").and_then(|v| v.as_str()), Some("import starlette"));
}

#[test]
fn asgi_application_definition_pins_canonical_app() {
    let doc = crate::common::load_toml(&manifest_path());
    let a = doc.get("asgi_application_definition").and_then(|v| v.as_table()).expect("[asgi_application_definition] missing");
    assert_eq!(a.get("app_variable").and_then(|v| v.as_str()), Some("app_2648"));
    assert_eq!(a.get("route_path").and_then(|v| v.as_str()), Some("/mamba/2648"));
    assert_eq!(a.get("route_method").and_then(|v| v.as_str()), Some("GET"));
    assert_eq!(a.get("handler_name").and_then(|v| v.as_str()), Some("mamba_2648_handler"));
    assert_eq!(a.get("handler_response_status").and_then(|v| v.as_integer()), Some(200));
    assert_eq!(
        a.get("handler_response_body_python_repr").and_then(|v| v.as_str()),
        Some("{'ok': True, 'n': 2648}"),
    );
}
...
fn deterministic_sample_covers_asgi_calls() {
    ...
    let arr = doc.get("asgi_call_cases").and_then(|v| v.as_array()).expect("[[asgi_call_cases]] missing");
...
fn fixture_fails_if_starlette_cannot_import() {
    ...
    let exit = i.get("starlette_import_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 167);
    assert_eq!(
        i.get("starlette_import_failure_kind").and_then(|v| v.as_str()),
...
fn fixture_avoids_external_sockets() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc.get("no_external_sockets_contract").and_then(|v| v.as_table()).expect(
    ...
    let sock = n.get("external_socket_exit_code").and_then(|v| v.as_integer()).unwrap();
    let life = n.get("server_lifecycle_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(sock, 168);
    assert_eq!(life, 169);
    assert_ne!(sock, life, "external-socket and server-lifecycle exit codes must differ");
    assert_eq!(
        n.get("external_socket_failure_kind").and_then(|v| v.as_str()),
        Some("starlette_external_socket_used"),
    );
    assert_eq!(
        n.get("server_lifecycle_failure_kind").and_then(|v| v.as_str()),
...
fn summary_records_asgi_framework_coverage() {
    let doc = crate::common::load_toml(&manifest_path());
    let h = doc.get("asgi_framework_coverage_reporting_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(
        h.get("asgi_framework_coverage_field_name").and_then(|v| v.as_str()),
        Some("asgi_framework_dependencies_covered"),
    );
    let req: Vec<&str> = h.get("required_asgi_framework_dependencies_covered").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    assert!(req.contains(&"starlette"), "required_asgi_framework_dependencies_covered must include starlette");
    assert_eq!(h.get("summary_record_format").and_then(|v| v.as_str()), Some("json"));
    let exit = h.get("missing_asgi_framework_coverage_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 170);
    assert_eq!(
        h.get("missing_asgi_framework_coverage_failure_kind").and_then(|v| v.as_str()),
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("full_server_lifecycle").and_then(|v| v.as_bool()), Some(true));
}
