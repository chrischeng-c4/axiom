...
fn workload_is_deterministic_fixed_string() {
    ...
    assert_eq!(w.get("input_kind").and_then(|v| v.as_str()), Some("fixed_string"));
    assert!(w.get("input_size_bytes").and_then(|v| v.as_integer()).unwrap() > 0);
    assert_eq!(w.get("must_use_same_input_each_iteration").and_then(|v| v.as_bool()), Some(true));
...
fn wrong_split_or_join_fails_benchmark() {
    ...
    assert_eq!(c.get("must_validate_output_length").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("must_validate_content_checksum").and_then(|v| v.as_bool()), Some(true));
...
fn speedup_in_ten_x_suite_candidate_set() {
    ...
    assert_eq!(s.get("ten_x_suite_candidate").and_then(|v| v.as_bool()), Some(true));
    assert!(
        s.get("ten_x_suite_candidate_set_field_name").and_then(|v| v.as_str()).is_some(),
        "ten_x_suite_candidate_set_field_name must be declared",
    );
    let req: Vec<&str> = s.get("required_record_keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for k in &["benchmark_id", "speedup", "ten_x_suite_candidate", "outcome"] {
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    for k in &[
        ...
        "expected_checksum", "joined_length", "expected_joined_length",
        "speedup", "ten_x_suite_candidate", "outcome", "exit_code",
