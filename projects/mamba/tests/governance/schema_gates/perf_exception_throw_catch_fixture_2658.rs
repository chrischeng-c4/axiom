...
fn workload_raises_and_catches_at_interval() {
    ...
    assert!(w.get("raise_interval").and_then(|v| v.as_integer()).unwrap() > 0);
    assert!(w.get("n_iterations").and_then(|v| v.as_integer()).unwrap() > 0);
    assert_eq!(w.get("must_count_raises").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(w.get("must_count_catches").and_then(|v| v.as_bool()), Some(true));
...
fn wrong_control_flow_fails_benchmark() {
    ...
    assert_eq!(c.get("must_assert_raise_count_equals_catch_count").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(c.get("must_assert_counts_match_interval_schedule").and_then(|v| v.as_bool()), Some(true));
...
fn speedup_reported_separately_with_tier() {
    ...
    assert_eq!(s.get("must_emit_tier_in_summary").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("tier_field_name").and_then(|v| v.as_str()), Some("tier"));
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for k in &[
        "benchmark_id", "tier", "timing_mode", "raise_interval", "checksum",
        "expected_checksum", "raise_count", "catch_count",
