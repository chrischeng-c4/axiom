...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_logging_behavioral")
    );
...
fn surface_covers_logger_handler_formatter_with_inmemory_sink() {
    ...
    assert_eq!(
        s.get("in_memory_stream_class_name")
            .and_then(|v| v.as_str()),
        Some("io.StringIO")
    );
}
...
fn deterministic_sample_covers_level_filtering_and_formatting() {
    ...
    assert_eq!(
        d.get("must_use_isolated_logger_per_case")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("logger_name_prefix_value").and_then(|v| v.as_str()),
        Some("mamba_stdlib_2631.")
    );
    assert_eq!(
        d.get("formatter_format_string").and_then(|v| v.as_str()),
        Some("%(levelname)s:%(name)s:%(message)s")
    );
    let max = d
    ...
    let specs: &[(&str, &[&str])] = &[
        (
            "level_filtering_cases",
            &[
                "logger_level",
                "log_call_level",
                "message",
                "expected_captured_messages",
            ],
        ),
        (
            "formatting_cases",
            &[
                ...
                "log_call_level",
                "message",
                "formatter_format_string",
                "expected_formatted_line",
            ],
        ),
    ];
...
fn fixture_fails_on_wrong_level_filtering_or_formatting() {
    ...
    let lf = f
        .get("level_filtering_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    let fm = f
        .get("formatting_mismatch_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(lf, 85);
    assert_eq!(fm, 86);
    assert_ne!(
        lf, fm,
        "level-filtering and formatting exit codes must differ"
    );
    assert_eq!(
        f.get("level_filtering_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("logging_level_filtering_mismatch")
    );
    assert_eq!(
        f.get("formatting_mismatch_failure_kind")
            .and_then(|v| v.as_str()),
        Some("logging_formatting_mismatch")
    );
}
...
fn no_global_logging_state_leaks_across_tests() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc
        .get("no_global_state_leak_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    let forbidden: Vec<&str> = n
        .get("forbidden_global_mutations")
        .and_then(|v| v.as_array())
    ...
    for m in &[
        ...
        assert!(
            ...
            "forbidden_global_mutations must include {m}"
        );
    }
    let exit = n
        .get("global_state_leak_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 87);
    assert_eq!(
        n.get("global_state_leak_failure_kind")
            .and_then(|v| v.as_str()),
        Some("logging_global_state_leaked")
    );
}
...
fn fixture_output_is_deterministic() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc
        .get("deterministic_output_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        d.get("nondeterministic_output_failure_kind")
            .and_then(|v| v.as_str()),
        Some("logging_nondeterministic_output")
    );
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("multiprocessing_or_file_logging")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
