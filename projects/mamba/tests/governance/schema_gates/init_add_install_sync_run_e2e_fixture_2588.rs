...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("init_add_install_sync_run_e2e"));
...
fn temp_project_is_per_test_and_isolated() {
    let doc = crate::common::load_toml(&manifest_path());
    let t = doc.get("temp_project").and_then(|v| v.as_table()).expect("[temp_project] missing");
    for f in &[
...
fn steps_declare_canonical_uv_like_workflow_in_order() {
    let doc = crate::common::load_toml(&manifest_path());
    let steps = doc.get("steps").and_then(|v| v.as_array()).expect("[[steps]] missing");
    assert_eq!(steps.len(), 5, "must declare exactly 5 canonical workflow steps");
    ...
    for s in steps {
        ...
        assert!(order > prev_order, "steps must be declared in strictly increasing order");
        prev_order = order;
        let name = t.get("name").and_then(|v| v.as_str()).unwrap();
        let status = t.get("status").and_then(|v| v.as_str()).unwrap();
        assert!(allowed_statuses.contains(&status), "status {status} not allowed");
        if status == "blocked" {
            blocked_count += 1;
            let blocker = t.get("blocker_issue").and_then(|v| v.as_integer());
            assert!(blocker.is_some(), "blocked step {name} must declare blocker_issue");
            let reason = t.get("blocker_reason").and_then(|v| v.as_str());
            assert!(reason.is_some(), "blocked step {name} must declare blocker_reason");
        }
        names.push(name);
    }

    for required in &["init", "add", "install", "sync", "run"] {
        assert!(names.contains(required), "steps must include canonical step {required}");
    }
    assert!(blocked_count >= 1, "fixture must declare at least one blocked step with follow-up issue");
}
...
fn test_names_every_workflow_step_and_status() {
    let doc = crate::common::load_toml(&manifest_path());
    let s = doc.get("step_naming_contract").and_then(|v| v.as_table()).expect(
        "[step_naming_contract] missing — acceptance: \
         \"Test names every workflow step and current status.\"",
    );
    assert_eq!(s.get("must_name_every_workflow_step").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("must_record_status_per_step").and_then(|v| v.as_bool()), Some(true));
    ...
    assert_eq!(s.get("forbid_silent_skip_for_blocked_step").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("forbid_silent_skip_for_unknown_step").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("must_print_step_name_and_status_in_order").and_then(|v| v.as_bool()), Some(true));
}
...
fn supported_steps_execute_in_order_in_temp_project() {
    let doc = crate::common::load_toml(&manifest_path());
    let e = doc.get("step_execution_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(e.get("expected_outcome_when_all_supported_steps_pass").and_then(|v| v.as_str()), Some("pass"));
    assert_eq!(e.get("expected_outcome_when_any_supported_step_fails").and_then(|v| v.as_str()), Some("fail"));
    assert_eq!(e.get("supported_step_failure_kind").and_then(|v| v.as_str()), Some("supported_step_failed"));
    let exit = e.get("supported_step_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 17);
}
...
fn unsupported_steps_link_to_followup_issues_or_blockers() {
    let doc = crate::common::load_toml(&manifest_path());
    let b = doc.get("blocker_contract").and_then(|v| v.as_table()).expect(
        "[blocker_contract] missing — acceptance: \
         \"Unsupported steps are linked to follow-up issues or blockers.\"",
    );
    assert_eq!(b.get("must_link_each_blocked_step_to_followup_issue").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(b.get("must_emit_blocker_diagnostic_for_each_blocked_step").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(b.get("forbid_silent_skip_of_blocked_step").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(b.get("blocker_step_failure_kind").and_then(|v| v.as_str()), Some("blocked_step_without_followup"));
    let exit = b.get("blocker_step_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 18);
    assert_eq!(b.get("must_distinguish_blocked_from_supported_failures").and_then(|v| v.as_bool()), Some(true));

    let supported_exit = doc.get("step_execution_contract")
        .and_then(|v| v.get("supported_step_failure_exit_code"))
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(exit, supported_exit, "blocker exit code must differ from supported-step-failure exit code");
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for required in &[
        "outcome", "case", "step_order", "step_name", "step_status",
        "blocker_issue", "captured_output", "project_file_diff",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("making_every_step_fully_implemented_in_this_issue").and_then(|v| v.as_bool()),
        Some(true),
    );
}
