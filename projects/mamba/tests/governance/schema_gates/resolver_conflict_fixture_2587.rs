...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("resolver_conflict"));
...
fn requirements_and_conflict_pair_are_consistent() {
    let doc = crate::common::load_toml(&manifest_path());
    let reqs = doc.get("requirements").and_then(|v| v.as_array()).expect("[[requirements]] missing");
    assert_eq!(reqs.len(), 2, "must declare exactly two conflicting requirements");
    let names: Vec<&str> = reqs.iter()
        .filter_map(|r| r.as_table().and_then(|t| t.get("name")).and_then(|v| v.as_str()))
        .collect();
    let constraints: Vec<&str> = reqs.iter()
        .filter_map(|r| r.as_table().and_then(|t| t.get("version_constraint")).and_then(|v| v.as_str()))
        .collect();
    assert_eq!(names.len(), 2);
    assert_eq!(constraints.len(), 2);
    assert_ne!(names[0], names[1]);
    assert_ne!(constraints[0], constraints[1], "version constraints must disagree");

    let c = doc.get("conflict").and_then(|v| v.as_table()).expect("[conflict] missing");
    assert_eq!(c.get("must_be_unsatisfiable").and_then(|v| v.as_bool()), Some(true));
    let pair: Vec<&str> = c.get("incompatible_pair").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    assert_eq!(pair.len(), 2);
    for n in &names {
        assert!(pair.contains(n), "incompatible_pair must include requirement name {n}");
    }
    let ca = c.get("incompatible_constraint_a").and_then(|v| v.as_str()).unwrap();
    let cb = c.get("incompatible_constraint_b").and_then(|v| v.as_str()).unwrap();
    assert_ne!(ca, cb);
    assert!(constraints.contains(&ca));
    assert!(constraints.contains(&cb));
    assert_eq!(c.get("constraints_must_disagree").and_then(|v| v.as_bool()), Some(true));
}
...
fn test_runs_in_default_package_manager_profile() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("default_profile_case").and_then(|v| v.as_table()).expect(
        "[default_profile_case] missing — acceptance: \
         \"Test runs in the default package-manager profile.\"",
    );
    assert_eq!(d.get("must_be_default_profile").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("profile_name").and_then(|v| v.as_str()), Some("package_manager"));
    assert_eq!(d.get("must_run_in_default_profile_without_extra_flags").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("must_not_require_opt_in_marker").and_then(|v| v.as_bool()), Some(true));
}
...
fn conflict_result_is_deterministic_and_offline() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("determinism_and_offline_contract").and_then(|v| v.as_table()).expect(
...
fn diagnostic_is_stable_enough_for_regression_checks() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("diagnostic_stability_contract").and_then(|v| v.as_table()).expect(
        "[diagnostic_stability_contract] missing — acceptance: \
         \"Diagnostic is stable enough for regression checks.\"",
    );
    assert_eq!(d.get("must_name_incompatible_packages").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("must_name_incompatible_constraints").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("must_be_machine_readable").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("diagnostic_record_format").and_then(|v| v.as_str()), Some("json"));
    assert_eq!(d.get("conflict_failure_kind").and_then(|v| v.as_str()), Some("resolver_conflict"));
    let exit = d.get("conflict_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 16);
    let yanked_exit = d.get("yanked_rejection_exit_code_from_2584").and_then(|v| v.as_integer()).unwrap();
    let no_cand_exit = d.get("no_candidate_exit_code_from_2584").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(yanked_exit, 10);
    assert_eq!(no_cand_exit, 11);
    assert_ne!(exit, yanked_exit, "conflict exit must differ from yanked-rejection exit (#2584)");
    assert_ne!(exit, no_cand_exit, "conflict exit must differ from no-candidate exit (#2584)");
    assert_eq!(d.get("must_distinguish_from_yanked_rejection_exit_code").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("must_distinguish_from_no_candidate_exit_code").and_then(|v| v.as_bool()), Some(true));
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        "outcome", "case",
        "incompatible_package_a", "incompatible_package_b",
        "incompatible_constraint_a", "incompatible_constraint_b",
        "diagnostic_message", "failure_kind", "exit_code",
    ] {
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("implementing_full_sat_resolver_redesign").and_then(|v| v.as_bool()), Some(true));
}
