...
fn workload_uses_single_small_class() {
    ...
    assert_eq!(
        w.get("must_use_single_instance").and_then(|v| v.as_bool()),
        Some(true)
    );
...
fn timing_contract_has_tier_and_internal_marker() {
    ...
    assert_eq!(
        t.get("must_emit_internal_timing_marker")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert!(
        t.get("internal_timing_marker_field_name")
            .and_then(|v| v.as_str())
            .is_some(),
