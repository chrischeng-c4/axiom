...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("frozen_local_simple_index"));
...
fn index_kind_is_frozen_local_simple_pep503() {
    ...
    assert_eq!(i.get("must_serve_from_local_disk").and_then(|v| v.as_bool()), Some(true));
...
fn index_layout_declares_required_files() {
    let doc = crate::common::load_toml(&manifest_path());
    let l = doc.get("index_layout").and_then(|v| v.as_table()).expect("[index_layout] missing");
    assert_eq!(l.get("root_relative_path").and_then(|v| v.as_str()), Some("simple/"));
    assert_eq!(l.get("per_package_subdir_kind").and_then(|v| v.as_str()), Some("normalized_project_name"));
    assert_eq!(l.get("must_include_root_index_html").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(l.get("must_include_per_package_index_html_for_each_package").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(l.get("must_include_at_least_one_wheel_per_package").and_then(|v| v.as_bool()), Some(true));
}

#[test]
fn graph_declares_two_packages_and_one_dependency_edge() {
    let doc = crate::common::load_toml(&manifest_path());
    let pkgs = doc.get("packages").and_then(|v| v.as_array()).expect("[[packages]] missing");
    ...
    let g = doc.get("graph").and_then(|v| v.as_table()).expect("[graph] missing");
    assert_eq!(g.get("must_declare_at_least_two_packages").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(g.get("must_declare_at_least_one_dependency_edge").and_then(|v| v.as_bool()), Some(true));
    let edges = g.get("edges").and_then(|v| v.as_array()).expect("graph.edges missing");
    assert!(!edges.is_empty(), "must declare at least one dependency edge");
    for e in edges {
        let t = e.as_table().unwrap();
        let from = t.get("from").and_then(|v| v.as_str()).unwrap();
        let to = t.get("to").and_then(|v| v.as_str()).unwrap();
        assert!(names.contains(&from), "edge.from must be a declared package: {from}");
        assert!(names.contains(&to), "edge.to must be a declared package: {to}");
...
fn default_package_manager_tests_do_not_hit_pypi() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("default_offline_gate").and_then(|v| v.as_table()).expect(
...
fn fixture_serves_or_loads_metadata_deterministically() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("determinism_contract").and_then(|v| v.as_table()).expect(
        "[determinism_contract] missing — acceptance: \
         \"Fixture can serve or load package metadata deterministically.\"",
    );
    assert_eq!(d.get("must_load_metadata_deterministically_across_runs").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("must_serve_metadata_deterministically_when_run_as_server").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("load_mode_allowed").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("serve_mode_allowed").and_then(|v| v.as_bool()), Some(true));
    let modes: Vec<&str> = d.get("allowed_modes").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for m in &["load", "serve"] {
        assert!(modes.contains(m), "allowed_modes must include {m}");
    }
    assert_eq!(d.get("checksum_must_be_recorded_for_each_artifact").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(d.get("checksum_algorithm").and_then(|v| v.as_str()), Some("sha256"));
    assert_eq!(d.get("checksum_record_format").and_then(|v| v.as_str()), Some("json"));
}
...
fn missing_fixture_files_fail_validation() {
    ...
    assert_eq!(v.get("must_validate_all_declared_files_exist").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(v.get("must_validate_per_package_index_html_exists").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(v.get("must_validate_root_index_html_exists").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(v.get("missing_file_failure_kind").and_then(|v| v.as_str()), Some("fixture_file_missing"));
    let exit = v.get("missing_file_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_ne!(exit, 0);
    assert_eq!(exit, 12);
    assert_eq!(v.get("missing_file_diagnostic_must_name_path").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(v.get("validation_must_run_before_any_resolver_use").and_then(|v| v.as_bool()), Some(true));
}
...
fn runner_contract_declares_keys_and_cases() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for required in &[
        "outcome", "case", "package", "version", "filename",
        "checksum", "missing_file_path", "failure_kind", "exit_code",
    ] {
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("live_pypi_integration").and_then(|v| v.as_bool()), Some(true));
}
