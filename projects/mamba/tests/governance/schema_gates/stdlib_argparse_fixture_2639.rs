...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("stdlib_argparse_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("stdlib_argparse_behavioral"));
...
fn surface_registers_argparse() {
    ...
    assert!(modules.contains(&"argparse"), "covered_modules must include argparse");
    for f in &[
    ...
    assert_eq!(s.get("import_statement").and_then(|v| v.as_str()), Some("import argparse"));
}

#[test]
fn parser_definition_pins_canonical_parser() {
    let doc = crate::common::load_toml(&manifest_path());
    let p = doc.get("parser_definition").and_then(|v| v.as_table()).expect("[parser_definition] missing");
    assert_eq!(p.get("prog").and_then(|v| v.as_str()), Some("mamba_2639"));
    assert_eq!(p.get("flag_name").and_then(|v| v.as_str()), Some("--verbose"));
    assert_eq!(p.get("flag_action").and_then(|v| v.as_str()), Some("store_true"));
    assert_eq!(p.get("flag_default").and_then(|v| v.as_bool()), Some(false));
    assert_eq!(p.get("positional_name").and_then(|v| v.as_str()), Some("name"));
    assert_eq!(p.get("positional_required").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(p.get("exit_on_error").and_then(|v| v.as_bool()), Some(false));
}
...
fn deterministic_sample_covers_success_and_failure() {
    ...
    let specs: &[(&str, &[&str])] = &[
        ("parse_success_cases", &["argv", "expected_name", "expected_verbose"]),
        ("parse_failure_cases", &["argv", "expected_error_kind", "must_not_call_systemexit_on_host"]),
    ];
    ...
    let fail = doc.get("parse_failure_cases").and_then(|v| v.as_array()).unwrap();
    for c in fail {
        let t = c.as_table().unwrap();
        assert_eq!(t.get("must_not_call_systemexit_on_host").and_then(|v| v.as_bool()), Some(true));
    }
}
...
fn fixture_fails_if_option_parsing_is_wrong() {
    let doc = crate::common::load_toml(&manifest_path());
    let f = doc.get("failure_on_wrong_parsing_contract").and_then(|v| v.as_table()).expect(
    ...
    sorted.sort();
    sorted.dedup();
    assert_eq!(sorted.len(), exits.len(), "argparse parsing-failure exit codes must all differ");
}
...
fn error_path_does_not_terminate_host_process_unexpectedly() {
    let doc = crate::common::load_toml(&manifest_path());
    let e = doc.get("error_path_non_terminating_contract").and_then(|v| v.as_table()).expect(
    ...
    let exit = e.get("host_process_terminated_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 130);
    assert_eq!(
        e.get("host_process_terminated_failure_kind").and_then(|v| v.as_str()),
...
fn output_is_deterministic_and_locale_independent() {
    let doc = crate::common::load_toml(&manifest_path());
    let d = doc.get("deterministic_locale_independent_output_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(d.get("pinned_prog_name").and_then(|v| v.as_str()), Some("mamba_2639"));
    let loc = d.get("locale_dependency_exit_code").and_then(|v| v.as_integer()).unwrap();
    let nd = d.get("nondeterministic_output_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(loc, 131);
    assert_eq!(nd, 132);
    assert_ne!(loc, nd, "locale and nondeterminism exit codes must differ");
    assert_eq!(
        d.get("locale_dependency_failure_kind").and_then(|v| v.as_str()),
        Some("argparse_locale_dependency_used"),
    );
    assert_eq!(
        d.get("nondeterministic_output_failure_kind").and_then(|v| v.as_str()),
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "expected_error_kind", "actual_error_kind",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("full_help_formatting_compatibility").and_then(|v| v.as_bool()), Some(true));
}
