...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("ignore_xfail_skip_inventory_command")
    );
...
fn invocation_pins_single_canonical_entry_point() {
    ...
    let i = doc
        ...
        .expect("[invocation] missing");
    assert_eq!(
        i.get("must_have_single_canonical_entry_point")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let tmpl = i
        .get("canonical_command_template")
        .and_then(|v| v.as_str())
        .unwrap();
    ...
    assert_eq!(
        i.get("must_be_runnable_offline").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
    ...
    assert_eq!(
        i.get("must_not_require_cargo_build")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn scan_scope_pins_three_marker_kinds_within_mamba_tests() {
    ...
    assert_eq!(
        s.get("scan_root_relative_path").and_then(|v| v.as_str()),
        Some("projects/mamba/tests")
    );

    let x = doc
        .get("summary_cross_reference")
        .and_then(|v| v.as_table())
        .expect("[summary_cross_reference] missing");
    assert_eq!(
        x.get("smoke_summary_fixture_issue")
            .and_then(|v| v.as_integer()),
        Some(2600)
    );
    assert_eq!(
        x.get("must_be_consumable_by_smoke_summary")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
...
fn inventory_reports_debt_counts() {
    ...
    assert_eq!(
        d.get("must_emit_counts_grouped_by_path")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("must_emit_counts_grouped_by_category")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        d.get("counts_record_format").and_then(|v| v.as_str()),
        Some("json")
    );
    let required: Vec<&str> = d
        .get("counts_required_fields")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for f in &["path", "category", "marker_kind", "count", "total"] {
        assert!(
            required.contains(f),
            "counts_required_fields must include {f}"
        );
    }
    assert_eq!(
        d.get("must_emit_counts_for_every_scanned_marker_kind")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let exit = d
        .get("missing_counts_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 52);
}
...
fn command_exits_nonzero_on_parse_or_unknown_marker() {
    ...
    assert_eq!(
        p.get("parse_error_must_exit_nonzero")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        p.get("unknown_marker_format_must_exit_nonzero")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let parse = p
        .get("parse_error_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(parse, 0);
    assert_eq!(parse, 53);
    let unknown = p
        .get("unknown_marker_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_ne!(unknown, 0);
    ...
    assert_ne!(
        parse, unknown,
        "parse-error and unknown-marker exits must differ"
    );
    assert_eq!(
        p.get("must_distinguish_parse_error_from_unknown_marker")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        p.get("must_emit_exact_file_and_line_for_parse_error")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn summary_linkable_from_smoke_report() {
    ...
    assert_eq!(
        s.get("must_emit_link_artifact").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        s.get("link_artifact_format").and_then(|v| v.as_str()),
        Some("json")
    );
    let required: Vec<&str> = s
        .get("link_artifact_required_fields")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for f in &[
        ...
        assert!(
            required.contains(f),
            "link_artifact_required_fields must include {f}"
        );
    }
    let path = s
        .get("link_artifact_relative_path_within_repo")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(path.starts_with("projects/mamba/tests/"));
    assert!(path.ends_with(".json"));
    assert_eq!(
        s.get("must_be_stable_across_runs_on_identical_input")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        s.get("must_emit_schema_version").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        s.get("schema_version_field_name").and_then(|v| v.as_str()),
        Some("schema_version")
    );
    assert_eq!(
        s.get("checksum_field_name").and_then(|v| v.as_str()),
        Some("checksum")
    );
    let exit = s
        .get("missing_link_artifact_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 55);
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("removing_existing_debt").and_then(|v| v.as_bool()),
        Some(true)
    );
}
