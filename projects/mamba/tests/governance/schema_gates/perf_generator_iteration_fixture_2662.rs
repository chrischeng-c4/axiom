...
fn workload_yields_and_consumes_deterministic_integers() {
    ...
    assert_eq!(w.get("generator_kind").and_then(|v| v.as_str()), Some("yield_integers"));
    assert!(w.get("n_yields").and_then(|v| v.as_integer()).unwrap() > 0);
    assert_eq!(w.get("must_consume_fully").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(w.get("must_aggregate_yielded_values_into_checksum").and_then(|v| v.as_bool()), Some(true));
...
fn wrong_generator_output_fails_benchmark() {
    ...
    assert_eq!(c.get("must_validate_yield_count").and_then(|v| v.as_bool()), Some(true));
...
fn summary_categorizes_generator_separately() {
    ...
    assert_eq!(s.get("must_categorize_separately").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("benchmark_category").and_then(|v| v.as_str()), Some("generator"));
    assert_eq!(s.get("must_not_pool_with_other_benchmarks").and_then(|v| v.as_bool()), Some(true));
    let cats: Vec<&str> = s.get("allowed_categories").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    assert!(cats.contains(&"generator"), "allowed_categories must include 'generator'");
...
fn runner_contract_declares_keys_and_outcomes() {
    ...
    let keys: Vec<&str> = c.get("keys").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for k in &[
        "benchmark_id", "category", "tier", "timing_mode", "n_yields",
        "yielded_checksum", "expected_yielded_checksum",
        "yield_count", "expected_yield_count",
