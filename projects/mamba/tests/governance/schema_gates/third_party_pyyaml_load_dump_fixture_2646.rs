...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("third_party_pyyaml_load_dump_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("third_party_pyyaml_load_dump_behavioral"));
...
fn surface_covers_pyyaml() {
    ...
    assert!(modules.contains(&"yaml"), "covered_modules must include yaml");
    for f in &[
    ...
    assert_eq!(s.get("import_statement").and_then(|v| v.as_str()), Some("import yaml"));
}
...
fn deterministic_sample_covers_load_and_dump() {
    ...
    let load = doc.get("load_cases").and_then(|v| v.as_array()).expect("[[load_cases]] missing");
    ...
    let dump = doc.get("dump_cases").and_then(|v| v.as_array()).expect("[[dump_cases]] missing");
    assert!(!dump.is_empty(), "dump_cases must not be empty");
    for c in dump {
        let t = c.as_table().expect("case must be a table");
        assert!(t.get("python_value_repr").is_some(), "dump_cases.python_value_repr missing");
    }
}
...
fn fixture_fails_if_yaml_cannot_import() {
    ...
    let exit = i.get("yaml_import_failure_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 159);
    assert_eq!(
...
fn fixture_fails_on_wrong_parsed_or_dumped_shape() {
    let doc = crate::common::load_toml(&manifest_path());
    let c = doc.get("shape_mismatch_contract").and_then(|v| v.as_table()).expect(
    ...
    let parsed = c.get("parsed_shape_mismatch_exit_code").and_then(|v| v.as_integer()).unwrap();
    let dumped = c.get("dumped_shape_mismatch_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(parsed, 160);
    assert_eq!(dumped, 161);
    assert_ne!(parsed, dumped, "parsed and dumped exit codes must differ");
    assert_eq!(
        c.get("parsed_shape_mismatch_failure_kind").and_then(|v| v.as_str()),
        Some("pyyaml_parsed_shape_mismatch"),
    );
    assert_eq!(
        c.get("dumped_shape_mismatch_failure_kind").and_then(|v| v.as_str()),
...
fn native_extension_limitations_recorded_explicitly_if_present() {
    let doc = crate::common::load_toml(&manifest_path());
    let n = doc.get("native_extension_limitation_reporting_contract").and_then(|v| v.as_table()).expect(
    ...
    assert_eq!(
        n.get("libyaml_availability_field_name").and_then(|v| v.as_str()),
        Some("libyaml_available"),
    );
    let allowed: Vec<&str> = n.get("allowed_libyaml_availability_values").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for v in &["available", "unavailable"] {
        assert!(allowed.contains(v), "allowed_libyaml_availability_values must include {v}");
    }
    assert_eq!(
        n.get("native_extension_limitation_field_name").and_then(|v| v.as_str()),
        Some("native_extension_limitations"),
    );
    let exit = n.get("missing_native_extension_report_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 162);
    assert_eq!(
        n.get("missing_native_extension_report_failure_kind").and_then(|v| v.as_str()),
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "yaml_input", "expected_parsed_python_repr",
        "python_value_repr",
        "libyaml_available", "native_extension_limitations",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("yaml_spec_edge_cases").and_then(|v| v.as_bool()), Some(true));
}
