...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("ignored_test_categorization")
    );
...
fn categories_block_lists_four_required_values() {
    ...
    for v in &["opt_in", "blocker", "flaky", "obsolete"] {
        assert!(
            values.contains(v),
            "categories.allowed_values must include {v}"
        );
    }
    assert_eq!(values.len(), 4, "exactly the four required categories");
    assert_eq!(
        c.get("required_for_every_ignored_marker")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("case_sensitive").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        c.get("must_be_lowercase").and_then(|v| v.as_bool()),
        Some(true)
    );
    for v in &values {
        assert!(
            v.chars().all(|ch| ch.is_ascii_lowercase() || ch == '_'),
            "category value {v} must be lowercase snake_case"
        );
    }
    let exit = c
        .get("unknown_category_exit_code")
...
fn inventory_summary_shows_counts_by_category() {
    ...
    assert_eq!(
        s.get("must_emit_count_for_every_category")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        s.get("must_emit_total_count").and_then(|v| v.as_bool()),
        Some(true)
    );
    ...
    let exit = s
        .get("missing_category_count_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 37);
}
...
fn unclassified_ignore_markers_fail_validation() {
    ...
    assert_eq!(
        v.get("must_validate_every_ignored_marker_has_category")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        v.get("must_validate_category_is_in_allowed_values")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        v.get("must_validate_blocker_links_work_item")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        v.get("unclassified_marker_failure_kind")
            .and_then(|v| v.as_str()),
        Some("ignore_marker_unclassified")
    );
    let unclassified = v
        .get("unclassified_marker_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(unclassified, 38);
    let blocker = v
        .get("blocker_without_work_item_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(blocker, 39);
    assert_ne!(
        unclassified, blocker,
        "unclassified and blocker-link-missing exits must differ"
    );
    assert_eq!(
        v.get("must_distinguish_unclassified_from_blocker_link_missing")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn mvp_required_tests_cannot_be_only_opt_in_unless_justified() {
    let doc = crate::common::load_toml(&manifest_path());
    let g = doc
        .get("mvp_required_guard_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        g.get("must_block_opt_in_only_for_mvp_required")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        g.get("must_require_explicit_justification_when_mvp_required_is_opt_in")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let field = g
        .get("justification_field_name")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(field.contains("justification"));
    assert!(field.contains("opt_in"));
    assert_eq!(
        g.get("must_require_justification_to_link_work_item")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let unjust = g
        .get("mvp_required_opt_in_without_justification_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(unjust, 40);
    let no_wi = g
        .get("justification_missing_work_item_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(no_wi, 41);
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("deleting_obsolete_tests").and_then(|v| v.as_bool()),
        Some(true)
    );
}
