...
fn header_is_well_formed() {
    ...
    assert_eq!(
        ...
        Some("stdlib_required_module_manifest")
    );
...
fn modules_table_array_declares_required_and_optional_with_consistent_fields() {
    ...
    assert!(
        modules.len() >= 2,
        "must declare at least one required and one optional entry"
    );
    ...
    for m in modules {
        ...
        let required = t
            ...
            .expect("module.required missing");
        let mvp = t
            .get("mvp_objective")
            .and_then(|v| v.as_str())
            .expect("module.mvp_objective missing");
        ...
        if required {
            assert!(
                fixture_issue > 0,
                "required module {name} must point to a real fixture_issue, got {fixture_issue}"
            );
            required_count += 1;
        } else {
            optional_count += 1;
    ...
    assert!(
        required_count >= 1,
        "must declare at least one required module"
    );
    assert!(
        optional_count >= 1,
        "must declare at least one optional module"
    );
}
...
fn required_module_without_fixture_reference_fails_validation() {
    let doc = load_toml(&manifest_path());
    let r = doc
        .get("required_fixture_reference_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        r.get("must_validate_every_required_module_has_fixture_reference")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let required: Vec<&str> = r
        .get("required_module_required_fields")
        .and_then(|v| v.as_array())
    ...
    for f in &["name", "fixture_issue", "required", "mvp_objective"] {
        assert!(
            required.contains(f),
            "required_module_required_fields must include {f}"
        );
    }
    let exit = r
        .get("missing_fixture_reference_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 59);
    assert_eq!(
        r.get("must_emit_module_name_in_diagnostic")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        r.get("must_emit_owning_mvp_objective_in_diagnostic")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        r.get("fixture_issue_zero_is_treated_as_missing_for_required")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn optional_modules_are_reported_separately() {
    let doc = load_toml(&manifest_path());
    let o = doc
        .get("optional_modules_reporting_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        o.get("must_separate_optional_from_required_in_report")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        o.get("must_not_fail_validation_for_optional_without_fixture")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        o.get("optional_report_record_format")
            .and_then(|v| v.as_str()),
        Some("json")
    );
    let required: Vec<&str> = o
        .get("optional_report_required_fields")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for f in &["name", "required", "fixture_issue", "mvp_objective"] {
        assert!(
            required.contains(f),
            "optional_report_required_fields must include {f}"
        );
    }
    let exit = o
        .get("missing_optional_section_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 60);
    assert_eq!(
        o.get("must_distinguish_required_from_optional_validation_path")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn manifest_is_consumed_by_ecosystem_summary() {
    let doc = load_toml(&manifest_path());
    let e = doc
        .get("ecosystem_summary_consumption_contract")
        .and_then(|v| v.as_table())
        .expect(
    ...
    assert_eq!(
        e.get("must_be_consumable_by_ecosystem_summary")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        e.get("must_emit_consumption_artifact")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        e.get("consumption_artifact_format")
            .and_then(|v| v.as_str()),
        Some("json")
    );
    let path = e
        .get("consumption_artifact_relative_path_within_repo")
        .and_then(|v| v.as_str())
        .unwrap();
    assert!(path.starts_with("projects/mamba/tests/"));
    assert!(path.ends_with(".json"));
    let required: Vec<&str> = e
        .get("consumption_artifact_required_fields")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    for f in &[
        ...
        assert!(
            required.contains(f),
            "consumption_artifact_required_fields must include {f}"
        );
    }
    assert_eq!(
        e.get("must_be_stable_across_runs_on_identical_input")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
    let exit = e
        .get("missing_consumption_artifact_exit_code")
        .and_then(|v| v.as_integer())
        .unwrap();
    assert_eq!(exit, 61);
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(
        o.get("implementing_every_module_fixture")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
