...
fn header_is_well_formed() {
    ...
    assert_eq!(doc.get("fixture").and_then(|v| v.as_str()), Some("stdlib_pathlib_behavioral"));
    ...
    assert_eq!(doc.get("family").and_then(|v| v.as_str()), Some("stdlib_pathlib_behavioral"));
...
fn surface_covers_purepath_join_suffix_name_parent_relative() {
    ...
    assert_eq!(s.get("module_name").and_then(|v| v.as_str()), Some("pathlib"));
    assert_eq!(s.get("must_be_importable_via_import_statement").and_then(|v| v.as_bool()), Some(true));
    assert_eq!(s.get("import_statement").and_then(|v| v.as_str()), Some("import pathlib"));
    for f in &[
        "must_cover_purepath",
...
fn deterministic_sample_covers_every_component_kind() {
    ...
    assert_eq!(d.get("purepath_class_name").and_then(|v| v.as_str()), Some("PurePosixPath"));
    ...
    let specs: &[(&str, &[&str])] = &[
        ...
        ("relative_to_cases", &["input_path", "base_path", "expected_relative_str"]),
    ];
...
fn fixture_fails_on_wrong_path_component_behavior() {
    ...
    sorted.sort();
    sorted.dedup();
    assert_eq!(sorted.len(), exits.len(), "all component-mismatch exit codes must differ");
}
...
fn fixture_avoids_machine_specific_absolute_path_expectations() {
    let doc = crate::common::load_toml(&manifest_path());
    let m = doc.get("machine_agnostic_path_contract").and_then(|v| v.as_table()).expect(
    ...
    for k in &[
        ...
        assert_eq!(m.get(*k).and_then(|v| v.as_bool()), Some(true), "{k} must be true");
    }
    let allowed: Vec<&str> = m.get("allowed_path_classes").and_then(|v| v.as_array())
    ...
    let forbidden: Vec<&str> = m.get("forbidden_assertion_patterns").and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
    for p in &[
        "starts_with_slash",
        ...
        assert!(forbidden.contains(p), "forbidden_assertion_patterns must include {p}");
    }
    let exit = m.get("machine_specific_assertion_exit_code").and_then(|v| v.as_integer()).unwrap();
    assert_eq!(exit, 79);
    assert_eq!(m.get("machine_specific_assertion_failure_kind").and_then(|v| v.as_str()), Some("pathlib_machine_specific_absolute_path"));
}
...
fn runner_records_pathlib_under_required_stdlib_coverage() {
    ...
    assert_eq!(r.get("manifest_module_name").and_then(|v| v.as_str()), Some("pathlib"));
    assert_eq!(r.get("runner_must_record_module_name").and_then(|v| v.as_str()), Some("pathlib"));
    assert_eq!(r.get("runner_coverage_field_name").and_then(|v| v.as_str()), Some("stdlib_required_module_covered"));
...
fn runner_contract_declares_keys_and_cases() {
    ...
    for required in &[
        ...
        "input_path", "expected_joined_str", "expected_suffix",
        "expected_name", "expected_parent_str", "expected_relative_str",
...
fn pins_out_of_scope_per_issue() {
    ...
    assert_eq!(o.get("platform_specific_filesystem_edge_cases").and_then(|v| v.as_bool()), Some(true));
}
