...
def discover_authoritative_source_sets(repo_root: pathlib.Path) -> dict:
    ...
    manifests_dir = repo_root / "projects/mamba/tests/harness/cpython/config/manifests"
    ...
    gaps_path = repo_root / "projects/mamba/tests/harness/cpython/config/behavior_gaps.txt"
    ...
    div_path = repo_root / "projects/mamba/tests/harness/cpython/config/type_divergences.txt"
    if div_path.is_file():
    ...
    ec_dirs = [
        repo_root / "projects/mamba/external-contracts/behavior",
        repo_root / "projects/mamba/external-contracts/stability",
        repo_root / "projects/mamba/external-contracts/efficiency",
    ]
    ...
    gates_dir = repo_root / "projects/mamba/tests/governance/gates"
    ...
    if gates_dir.is_dir():
        for entry in sorted(gates_dir.iterdir(), key=lambda x: x.name):
            ...
            if entry.is_dir() and dir_name.startswith("t1_") and dir_name.endswith("_denominator"):
                ...
                manifest_rel = f"projects/mamba/tests/governance/gates/{dir_name}/manifest.toml"
                denom_rel = f"projects/mamba/tests/governance/gates/{dir_name}/denominator.txt"
    ...
    for name in source_set_names:
        ...
        for identity in info["source_identities"]:
            if (name, identity) in represented_pairs:
                continue
            reviewed_against = "#2022" if identity == "projects/mamba/tests/governance/gates/t1_multicore_scaling_denominator/denominator.txt" else "#2010"
