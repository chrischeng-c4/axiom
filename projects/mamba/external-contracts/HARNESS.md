...
| Dimension keyword | `runner.rs:is_type_strict_path` | ANY path component `== "type"` ⇒ strict-type rules (mamba must reject); all other dimensions are positive contracts (a compile reject there = type-system false positive, per README.md dimension rule; cf. `tech-design/type-system/1595-ingress-overwalling-shapes.md`). |
