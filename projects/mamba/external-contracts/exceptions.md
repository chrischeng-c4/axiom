...
- **Subclass-init shapes OPEN (#1557)**: unbound `Exception.__init__(self,…)` loses attrs; `__new__` args not pre-stored (str() falls to generic repr); composite NoneType-callable crash — red lines in `tech-design/exceptions/exc-subclass-init-shapes.md`.
...
```bash
# inner loop (seconds; paths relative to tests/cpython) — from projects/mamba/tests/harness/cpython/
