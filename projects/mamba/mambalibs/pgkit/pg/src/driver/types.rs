...
//! similar to cclab-nebula's BSON type handling.

use chrono::{DateTime, NaiveDate, NaiveDateTime, NaiveTime, Utc};
use rust_decimal::Decimal;
use serde_json::Value as JsonValue;
use sqlx::postgres::{PgArguments, PgRow};
use sqlx::{Arguments, Column, Postgres, Row as SqlxRow, Type, TypeInfo};
    ...
    pub fn bind_to_arguments(&self, arguments: &mut PgArguments) -> Result<()> {
        match self {
            ...
            ExtractedValue::SmallInt(v) => {
                arguments.add(*v).map_err(|e| {
                    DataBridgeError::Query(format!("Failed to bind SMALLINT: {}", e))
                })?;
            }
            ...
            ExtractedValue::Timestamp(v) => {
                arguments.add(*v).map_err(|e| {
                    DataBridgeError::Query(format!("Failed to bind TIMESTAMP: {}", e))
                })?;
            }
            ExtractedValue::TimestampTz(v) => {
                arguments.add(*v).map_err(|e| {
                    DataBridgeError::Query(format!("Failed to bind TIMESTAMPTZ: {}", e))
                })?;
            }
            ...
            ExtractedValue::Array(values) => {
                ...
                if values.is_empty() {
                    ...
                    arguments.add(Option::<Vec<i32>>::None).map_err(|e| {
                        DataBridgeError::Query(format!("Failed to bind empty ARRAY: {}", e))
                    })?;
                } else {
                    ...
                    match element_type {
                        Some(ExtractedValue::Int(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::Int(_) | ExtractedValue::Null)
                            });

                            if is_homogeneous {
                                // Check if contains nulls
                                let has_nulls =
                                    values.iter().any(|v| matches!(v, ExtractedValue::Null));

                                if has_nulls {
                                    // Bind as Option<Vec<Option<i32>>> to handle NULL elements
                                    let int_array: Vec<Option<i32>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(int_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind INT4[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<i32> (no nulls)
                                    let int_array: Vec<i32> = values
                                        .iter()
                                        .map(|v| match v {
                        ...
                        Some(ExtractedValue::String(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::String(_) | ExtractedValue::Null)
                            });
                            ...
                            if is_homogeneous {
                                ...
                                if has_nulls {
                                    ...
                                    let str_array: Vec<Option<String>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(str_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind TEXT[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<String> (no nulls)
                                    let str_array: Vec<String> = values
                                        .iter()
                                        .map(|v| match v {
                        ...
                        Some(ExtractedValue::Bool(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::Bool(_) | ExtractedValue::Null)
                            });
                            ...
                            if is_homogeneous {
                                ...
                                if has_nulls {
                                    ...
                                    let bool_array: Vec<Option<bool>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(bool_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind BOOL[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<bool> (no nulls)
                                    let bool_array: Vec<bool> = values
                                        .iter()
                                        .map(|v| match v {
                        ...
                        Some(ExtractedValue::BigInt(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::BigInt(_) | ExtractedValue::Null)
                            });
                            ...
                            if is_homogeneous {
                                ...
                                if has_nulls {
                                    ...
                                    let bigint_array: Vec<Option<i64>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(bigint_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind INT8[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<i64> (no nulls)
                                    let bigint_array: Vec<i64> = values
                                        .iter()
                                        .map(|v| match v {
                        ...
                        Some(ExtractedValue::SmallInt(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::SmallInt(_) | ExtractedValue::Null)
                            });
                            ...
                            if is_homogeneous {
                                ...
                                if has_nulls {
                                    ...
                                    let smallint_array: Vec<Option<i16>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(smallint_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind INT2[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<i16> (no nulls)
                                    let smallint_array: Vec<i16> = values
                                        .iter()
                                        .map(|v| match v {
                        ...
                        Some(ExtractedValue::Float(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::Float(_) | ExtractedValue::Null)
                            });
                            ...
                            if is_homogeneous {
                                ...
                                if has_nulls {
                                    ...
                                    let float_array: Vec<Option<f32>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(float_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind FLOAT4[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<f32> (no nulls)
                                    let float_array: Vec<f32> = values
                                        .iter()
                                        .map(|v| match v {
                        ...
                        Some(ExtractedValue::Double(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::Double(_) | ExtractedValue::Null)
                            });
                            ...
                            if is_homogeneous {
                                ...
                                if has_nulls {
                                    ...
                                    let double_array: Vec<Option<f64>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(double_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind FLOAT8[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<f64> (no nulls)
                                    let double_array: Vec<f64> = values
                                        .iter()
                                        .map(|v| match v {
                        ...
                        Some(ExtractedValue::Uuid(_)) => {
                            ...
                            let is_homogeneous = values.iter().all(|v| {
                                matches!(v, ExtractedValue::Uuid(_) | ExtractedValue::Null)
                            });
                            ...
                            if is_homogeneous {
                                ...
                                if has_nulls {
                                    ...
                                    let uuid_array: Vec<Option<Uuid>> = values
                                        .iter()
                                        .map(|v| match v {
                                        ...
                                        .collect();
                                    arguments.add(uuid_array).map_err(|e| {
                                        DataBridgeError::Query(format!(
                                            "Failed to bind UUID[] with nulls: {}",
                                            e
                                        ))
                                    })?;
                                } else {
                                    // Bind as Vec<Uuid> (no nulls)
                                    let uuid_array: Vec<Uuid> = values
                                        .iter()
                                        .map(|v| match v {
            ...
            ExtractedValue::Decimal(v) => {
                ...
                arguments.add(*v).map_err(|e| {
                    DataBridgeError::Query(format!("Failed to bind DECIMAL: {}", e))
                })?;
            }
        }
        Ok(())
    ...
    for (idx, column) in row.columns().iter().enumerate() {
        ...
        let value = match type_name {
            "BOOL" | "BOOLEAN" => match row.try_get::<Option<bool>, _>(idx) {
                ...
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract BOOL from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "INT2" | "SMALLINT" => match row.try_get::<Option<i16>, _>(idx) {
                ...
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract SMALLINT from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "INT4" | "INTEGER" | "INT" => match row.try_get::<Option<i32>, _>(idx) {
                Ok(Some(v)) => ExtractedValue::Int(v),
                Ok(None) => ExtractedValue::Null,
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract INT from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "INT8" | "BIGINT" => match row.try_get::<Option<i64>, _>(idx) {
                ...
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract BIGINT from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "FLOAT4" | "REAL" => match row.try_get::<Option<f32>, _>(idx) {
                ...
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract FLOAT from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "FLOAT8" | "DOUBLE PRECISION" | "DOUBLE" => match row.try_get::<Option<f64>, _>(idx) {
                Ok(Some(v)) => ExtractedValue::Double(v),
                Ok(None) => ExtractedValue::Null,
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract DOUBLE from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "VARCHAR" | "TEXT" | "CHAR" | "BPCHAR" | "NAME" => {
                match row.try_get::<Option<String>, _>(idx) {
                    ...
                    Err(e) => {
                        return Err(DataBridgeError::Query(format!(
                            "Failed to extract STRING from column '{}': {}",
                            column_name, e
                        )))
                    }
                }
            ...
            "BYTEA" => match row.try_get::<Option<Vec<u8>>, _>(idx) {
                ...
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract BYTES from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            ...
            "TIMESTAMP" => match row.try_get::<Option<NaiveDateTime>, _>(idx) {
                ...
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract TIMESTAMP from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "TIMESTAMPTZ" | "TIMESTAMP WITH TIME ZONE" => {
                match row.try_get::<Option<DateTime<Utc>>, _>(idx) {
                    ...
                    Err(e) => {
                        return Err(DataBridgeError::Query(format!(
                            "Failed to extract TIMESTAMPTZ from column '{}': {}",
                            column_name, e
                        )))
                    }
                }
            }
            "JSON" | "JSONB" => match row.try_get::<Option<JsonValue>, _>(idx) {
                Ok(Some(v)) => ExtractedValue::Json(v),
                Ok(None) => ExtractedValue::Null,
                Err(e) => {
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract JSON from column '{}': {}",
                        column_name, e
                    )))
                }
            },
            "NUMERIC" | "DECIMAL" => {
                ...
                if let Ok(v) = row.try_get::<Option<Decimal>, _>(idx) {
                    ...
                    return Err(DataBridgeError::Query(format!(
                        "Failed to extract DECIMAL from column '{}': incompatible type",
                        column_name
                    )));
                }
            }
            ...
            "_TEXT" | "_VARCHAR" => {
                extract_array::<String>(row, idx, &column_name, ExtractedValue::String)?
            }
            ...
            unknown => {
                tracing::warn!(
                    "Unknown PostgreSQL type '{}' for column '{}', attempting string extraction",
                    unknown,
                    column_name
                );
                match row.try_get::<Option<String>, _>(idx) {
                    ...
                    Err(e) => {
                        return Err(DataBridgeError::Query(format!(
                            "Failed to extract unknown type '{}' from column '{}': {}",
                            unknown, column_name, e
                        )))
                    }
                }
            }
    ...
    T: for<'a> sqlx::Decode<'a, Postgres> + Type<Postgres> + sqlx::postgres::PgHasArrayType,
{
    match row.try_get::<Option<Vec<T>>, _>(idx) {
        Ok(Some(vec)) => {
            let values: Vec<ExtractedValue> = vec.into_iter().map(convert).collect();
            Ok(ExtractedValue::Array(values))
        }
        Ok(None) => Ok(ExtractedValue::Null),
        Err(e) => Err(DataBridgeError::Query(format!(
...
fn bind_array_as_json(values: &[ExtractedValue], arguments: &mut PgArguments) -> Result<()> {
    ...
    arguments
        .add(JsonValue::Array(json_array))
...
mod tests {
    ...
    fn test_extracted_value_type_names() {
        ...
        assert_eq!(
            ExtractedValue::Double(3.14).pg_type_name(),
            "DOUBLE PRECISION"
        );
        assert_eq!(
            ExtractedValue::String("test".to_string()).pg_type_name(),
            "TEXT"
        );
        assert_eq!(ExtractedValue::Bytes(vec![1, 2, 3]).pg_type_name(), "BYTEA");
        assert_eq!(ExtractedValue::Uuid(Uuid::nil()).pg_type_name(), "UUID");
        assert_eq!(
            ExtractedValue::Json(JsonValue::Null).pg_type_name(),
            "JSONB"
        );
        assert_eq!(ExtractedValue::Array(vec![]).pg_type_name(), "ARRAY");
        assert_eq!(
            ExtractedValue::Decimal(Decimal::from_str("123.45").unwrap()).pg_type_name(),
            "NUMERIC"
        );
    }
    ...
    fn test_bind_array_values() {
        ...
        let value = ExtractedValue::Array(vec![ExtractedValue::Int(1), ExtractedValue::Int(2)]);
        assert!(value.bind_to_arguments(&mut args).is_ok());
    }
    ...
    fn test_string_type_conversion() {
        ...
        assert!(empty_value
            .bind_to_arguments(&mut PgArguments::default())
            .is_ok());
    ...
    fn test_numeric_type_conversions() {
        ...
        // Test zero values
        assert_eq!(
            extracted_to_json(&ExtractedValue::SmallInt(0)).unwrap(),
            json!(0)
        );
        assert_eq!(
            extracted_to_json(&ExtractedValue::Int(0)).unwrap(),
            json!(0)
        );
        assert_eq!(
            extracted_to_json(&ExtractedValue::BigInt(0)).unwrap(),
            json!(0)
        );
    }
    ...
    fn test_array_type_conversions() {
        ...
        assert!(string_array
            ...
            .is_ok());
        assert_eq!(
            extracted_to_json(&string_array).unwrap(),
            json!(["hello", "world"])
        );
        ...
        assert!(mixed_array
            ...
            .is_ok());
        assert_eq!(
            extracted_to_json(&mixed_array).unwrap(),
            json!([42, "test", true, null])
        );

        // Test nested array
        let nested_array = ExtractedValue::Array(vec![
            ExtractedValue::Array(vec![ExtractedValue::Int(1), ExtractedValue::Int(2)]),
            ExtractedValue::Array(vec![ExtractedValue::Int(3), ExtractedValue::Int(4)]),
        ]);
        assert!(nested_array
            .bind_to_arguments(&mut PgArguments::default())
            .is_ok());
        assert_eq!(
            extracted_to_json(&nested_array).unwrap(),
            json!([[1, 2], [3, 4]])
        );
        ...
        assert!(array_with_nulls
            .bind_to_arguments(&mut PgArguments::default())
            .is_ok());
        assert_eq!(
            extracted_to_json(&array_with_nulls).unwrap(),
            json!([1, null, 3])
        );
    }
    ...
    fn test_type_name_mapping() {
        ...
        assert_eq!(
            ExtractedValue::SmallInt(i16::MAX).pg_type_name(),
            "SMALLINT"
        );
        assert_eq!(
            ExtractedValue::SmallInt(i16::MIN).pg_type_name(),
            "SMALLINT"
        );
        ...
        assert_eq!(
            ExtractedValue::Double(0.0).pg_type_name(),
            "DOUBLE PRECISION"
        );
        assert_eq!(
            ExtractedValue::Double(f64::MAX).pg_type_name(),
            "DOUBLE PRECISION"
        );
        assert_eq!(
            ExtractedValue::Double(f64::MIN).pg_type_name(),
            "DOUBLE PRECISION"
        );
        ...
        // JSON
        assert_eq!(
            ExtractedValue::Json(serde_json::json!(null)).pg_type_name(),
            "JSONB"
        );
        assert_eq!(
            ExtractedValue::Json(serde_json::json!({})).pg_type_name(),
            "JSONB"
        );
        assert_eq!(
            ExtractedValue::Json(serde_json::json!([])).pg_type_name(),
        ...
        // Decimal
        assert_eq!(
            ExtractedValue::Decimal(Decimal::from_str("0").unwrap()).pg_type_name(),
            "NUMERIC"
        );
        assert_eq!(
            ExtractedValue::Decimal(Decimal::from_str("123.45").unwrap()).pg_type_name(),
    ...
    fn test_decimal_type_conversion() {
        ...
        // Test large decimal
        let large_decimal =
            ExtractedValue::Decimal(Decimal::from_str("999999999999999.999999").unwrap());
        assert!(large_decimal
            .bind_to_arguments(&mut PgArguments::default())
            .is_ok());
        assert_eq!(
            extracted_to_json(&large_decimal).unwrap(),
            json!("999999999999999.999999")
        );

        // Test negative decimal
        let negative_decimal = ExtractedValue::Decimal(Decimal::from_str("-123.45").unwrap());
        assert_eq!(
            extracted_to_json(&negative_decimal).unwrap(),
            json!("-123.45")
        );
    ...
    fn test_numeric_boundaries() {
        ...
        assert!(big_max
            ...
            .is_ok());
        assert_eq!(
            extracted_to_json(&big_max).unwrap(),
        ...
        assert!(big_min
            ...
            .is_ok());
        assert_eq!(
            extracted_to_json(&big_min).unwrap(),
        ...
        assert_eq!(
            extracted_to_json(&int_near_min).unwrap(),
            json!(-2147483647)
        );
    }
    ...
    fn test_array_of_primitives() {
        ...
        assert!(int_array
            ...
            .is_ok());
        assert_eq!(
            extracted_to_json(&int_array).unwrap(),
            json!([1, 2, 3, 4, 5])
        );
        ...
        assert!(bool_array
            ...
            .is_ok());
        assert_eq!(
            extracted_to_json(&bool_array).unwrap(),
        ...
        assert!(bigint_array
            ...
            .is_ok());
        assert_eq!(
            extracted_to_json(&bigint_array).unwrap(),
            json!([1000000, 2000000, 3000000])
        );
    ...
    fn test_extracted_value_all_variants() {
        ...
        for variant in all_variants {
            let mut args = PgArguments::default();
            assert!(
                variant.bind_to_arguments(&mut args).is_ok(),
                "Failed to bind variant: {}",
                variant.pg_type_name()
            );
        }
    }
    ...
    fn test_native_array_binding_other_types() {
        ...
        let float_array =
            ExtractedValue::Array(vec![ExtractedValue::Float(1.1), ExtractedValue::Float(2.2)]);
    ...
    fn test_type_display_formatting() {
        ...
        assert_eq!(
            ExtractedValue::Double(1.0).pg_type_name(),
            "DOUBLE PRECISION"
        );
        assert_eq!(
            ExtractedValue::String("test".to_string()).pg_type_name(),
        ...
        assert_eq!(ExtractedValue::Array(vec![]).pg_type_name(), "ARRAY");
        assert_eq!(
            ExtractedValue::Decimal(Decimal::from_str("1.0").unwrap()).pg_type_name(),
            "NUMERIC"
        );
        ...
        for extracted_value in all_types {
            ...
            for ch in type_name.chars() {
                assert!(
                    ch.is_ascii_uppercase() || ch.is_whitespace(),
                    "Type name '{}' contains non-uppercase character: '{}'",
                    type_name,
                    ch
                );
            }
