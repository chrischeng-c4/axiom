...
// Locks the shape of the T2 Generators Async Denominator Gate fixture
// pinned by tests/governance/gates/t2_generators_async_denominator/manifest.toml.
// Closes #2699.
...
fn manifest_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/governance/gates/t2_generators_async_denominator/manifest.toml")
}

fn denominator_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/governance/gates/t2_generators_async_denominator/denominator.txt")
}
...
fn header_is_well_formed() {
    ...
    assert_eq!(m["fixture"].as_str(), Some("t2_generators_async_denominator"));
    assert_eq!(m["issue"].as_integer(), Some(2646));
    assert_eq!(m["capability"].as_str(), Some("mamba-language-core"));
    assert_eq!(m["work_root_wi"].as_integer(), Some(2039));
    assert_eq!(m["profile"].as_str(), Some("regression-prevention"));
    assert_eq!(m["family"].as_str(), Some("t2_generators_async_denominator"));
}
