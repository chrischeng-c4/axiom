...
fn surface_pins_the_five_required_axes() {
    ...
    assert!(b(sf, "must_cover_module_level_dispatcher_workaround_recorded"));
    assert!(b(sf, "must_cover_hasattr_conditional_binding_refused"));
    assert!(b(sf, "must_cover_phase_3_fix_paths_enumerated"));
    assert!(b(sf, "must_cover_method_call_only_libs_explicitly_unaffected"));
...
fn r1_jit_must_not_fold_handle_arith_to_native() {
    ...
    assert!(b(r, "forbid_folding_to_native_i64_add_for_typed_handle_operands"));
    assert!(b(r, "forbid_silently_succeeding_with_wrong_arithmetic_result"));
    ...
    assert_eq!(s(r, "expected_dispatch_path"), "dunder_via_class_rs_mb_call_method");
...
fn r2_module_level_dispatcher_workaround_recorded() {
    let m = m();
    let r = get(&m, "r2_module_level_dispatcher_workaround_recorded_contract");
    ...
    assert_eq!(s(r, "expected_worked_example_call"), "fractions.fraction_add");
    assert_eq!(i(r, "workaround_dropped_exit_code"), 469);
    assert_eq!(i(r, "prefix_mismatch_exit_code"), 470);
    assert!(b(r, "must_distinguish_workaround_dropped_from_prefix_mismatch"));
}
...
fn r3_hasattr_conditional_binding_refused() {
    ...
    assert!(b(r, "must_pin_sys_implementation_name_branch_as_correct_pattern"));
    assert!(b(r, "forbid_using_hasattr_conditional_binding_in_cross_runtime_fixtures"));
    assert!(b(r, "forbid_silently_dropping_hasattr_branch_due_to_jit_type_confusion"));
...
fn r4_phase_3_fix_paths_enumerated() {
    ...
    assert!(b(r, "must_distinguish_phase_3_implemented_from_fix_path_dropped"));
}

#[test]
fn r5_method_call_only_libs_explicitly_unaffected() {
    let m = m();
    let r = get(&m, "r5_method_call_only_libs_explicitly_unaffected_contract");
    ...
    assert!(b(r, "must_distinguish_widened_into_gate_from_dropped_from_record"));
}
...
fn pins_out_of_scope_per_issue() {
    ...
    assert!(b(oos, "phase_3_implementation_of_typed_handle_tag_or_wrapper_instance"));
