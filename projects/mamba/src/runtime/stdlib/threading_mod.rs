...
//!     surfaced through the dispatcher table. `Thread.start` and
//!     `Thread.join` flip `started` / `alive` flags but do not run the
//!     target in another OS thread.
...
//!   - `get_ident` / `get_native_id`: always return 1.
...
unsafe extern "C" fn d_thread(args_ptr: *const MbValue, nargs: usize) -> MbValue {
    ...
    mb_threading_thread(target, name)
}
    ...
    if let Some(ptr) = thread.as_ptr() {
        unsafe {
            if let ObjData::Instance { ref fields, .. } = (*ptr).data {
                ...
                if !target.is_none() {
                    let _ = super::super::class::mb_call0(target);
                }
...
/// threading.current_thread() -> the singleton MainThread Instance.
...
/// threading.get_ident() -> int (always 1).
pub fn mb_threading_get_ident() -> MbValue {
    MbValue::from_int(1)
}

/// threading.get_native_id() -> int (always 1).
...
mod tests {
    ...
    fn dict_get(val: MbValue, key: &str) -> Option<MbValue> {
        val.as_ptr().and_then(|ptr| unsafe {
    ...
    fn test_thread_with_str_name() {
        ...
        assert_eq!(
            get_str(dict_get(t, "name").unwrap()),
    ...
    fn test_thread_start_join_lifecycle() {
        ...
        assert_eq!(dict_get(t, "started").unwrap().as_bool(), Some(true));
        assert_eq!(dict_get(t, "alive").unwrap().as_bool(), Some(true));
        mb_threading_thread_join(t);
        assert_eq!(dict_get(t, "alive").unwrap().as_bool(), Some(false));
    }
    ...
    #[test]
    fn test_local_is_dict() {
        let l = mb_threading_local();
        assert!(l.as_ptr().is_some());
        unsafe {
            if let ObjData::Dict(ref lock) = (*l.as_ptr().unwrap()).data {
