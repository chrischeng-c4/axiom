...
unsafe extern "C" fn dispatch_normalize_encoding(
...
pub fn register() {
    ...
    let dispatchers: &[(&str, usize)] = &[
        (
            "normalize_encoding",
            dispatch_normalize_encoding as *const () as usize,
        ),
        (
            "search_function",
            dispatch_search_function as *const () as usize,
        ),
    ];
    ...
    super::super::module::NATIVE_FUNC_ADDRS.with(|s| {
        s.borrow_mut().insert(shell_addr as u64);
    });
    ...
    super::super::module::NATIVE_FUNC_ADDRS.with(|s| {
        s.borrow_mut().insert(empty_addr as u64);
    });
