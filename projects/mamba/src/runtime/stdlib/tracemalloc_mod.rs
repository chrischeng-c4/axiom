...
fn make_frame(filename: &str, lineno: i64) -> MbValue {
    make_instance("tracemalloc.Frame", vec![
...
fn make_traceback(raw_frames: &[RawFrame], total_nframe: Option<i64>) -> MbValue {
    let frames: Vec<MbValue> = raw_frames.iter().rev()
        .map(|(f, l)| make_frame(f, *l))
        .collect();
    make_instance("tracemalloc.Traceback", vec![
        ("_entries", MbValue::from_ptr(MbObject::new_list(frames))),
        ("total_nframe",
            total_nframe.map(MbValue::from_int).unwrap_or_else(MbValue::none)),
    ])
}

fn make_trace(raw: &RawTrace) -> MbValue {
    let total = if raw.3 < 0 { None } else { Some(raw.3) };
    make_instance("tracemalloc.Trace", vec![
...
fn raw_trace_value(raw: &RawTrace) -> MbValue {
    let frames: Vec<MbValue> = raw.2.iter()
        .map(|(f, l)| MbValue::from_ptr(MbObject::new_tuple(vec![
            new_str(f), MbValue::from_int(*l),
        ])))
        .collect();
...
fn make_snapshot(raws: Vec<RawTrace>, limit: i64) -> MbValue {
    ...
    let traces_seq = make_instance("tracemalloc._Traces", vec![
        ("_entries", MbValue::from_ptr(MbObject::new_list(traces))),
    ]);
    make_instance("tracemalloc.Snapshot", vec![
...
unsafe extern "C" fn dispatch_snapshot(args_ptr: *const MbValue, nargs: usize) -> MbValue {
    ...
    let raws: Vec<RawTrace> = a.first().copied()
        .map(list_items)
        .unwrap_or_default()
        .into_iter()
        .filter_map(parse_raw_trace)
...
unsafe extern "C" fn dispatch_traceback(args_ptr: *const MbValue, nargs: usize) -> MbValue {
    let a = unsafe { arg_slice(args_ptr, nargs) };
    let frames: Vec<RawFrame> = a.first().copied()
        .map(list_items)
        .unwrap_or_default()
        .into_iter()
        .filter_map(parse_raw_frame)
...
unsafe extern "C" fn dispatch_filter(args_ptr: *const MbValue, nargs: usize) -> MbValue {
            ...
            if let Some(k) = kv.first().copied().and_then(extract_str) {
                    ...
                    "inclusive" | "filename_pattern" | "lineno" | "all_frames" | "domain")
                {
                    return raise("TypeError", &format!(
                        "Filter.__init__() got an unexpected keyword argument '{k}'"
                    ));
                }
            }
        }
    ...
    let getk = |name: &str, idx: usize| -> MbValue {
        kwarg(kw, name).or_else(|| pos.get(idx).copied()).unwrap_or_else(MbValue::none)
    };
    ...
    make_instance("tracemalloc.Filter", vec![
        ("inclusive", MbValue::from_bool(inclusive.as_bool().unwrap_or(false))),
        ("filename_pattern", pattern),
        ("lineno", lineno),
        ("all_frames", MbValue::from_bool(all_frames.as_bool().unwrap_or(false))),
        ("domain", domain),
    ])
}
...
unsafe extern "C" fn dispatch_domain_filter(args_ptr: *const MbValue, nargs: usize) -> MbValue {
    ...
    let kw = a.iter().copied().find(|v| is_dict_value(*v)).unwrap_or_else(MbValue::none);
    let pos: Vec<MbValue> = a.iter().copied().filter(|v| !is_dict_value(*v)).collect();
    let inclusive = kwarg(kw, "inclusive").or_else(|| pos.first().copied())
        .unwrap_or_else(MbValue::none);
    let domain = kwarg(kw, "domain").or_else(|| pos.get(1).copied())
        .unwrap_or_else(MbValue::none);
    make_instance("tracemalloc.DomainFilter", vec![
        ("inclusive", MbValue::from_bool(inclusive.as_bool().unwrap_or(false))),
        ("domain", domain),
    ])
}
...
fn traceback_str_of(tb: MbValue) -> String {
    let frames = get_field(tb, "_entries").map(list_items).unwrap_or_default();
    match frames.first() {
        Some(f) => {
            let file = get_field(*f, "filename").and_then(extract_str).unwrap_or_default();
            let line = get_field(*f, "lineno").and_then(|v| v.as_int()).unwrap_or(0);
            format!("{file}:{line}")
        }
        None => String::new(),
    }
...
fn statistic_str(stat: MbValue, diff: bool) -> String {
    ...
    let size = get_field(stat, "size").and_then(|v| v.as_int()).unwrap_or(0);
    let count = get_field(stat, "count").and_then(|v| v.as_int()).unwrap_or(0);
    let mut out = if diff {
        let sd = get_field(stat, "size_diff").and_then(|v| v.as_int()).unwrap_or(0);
        let cd = get_field(stat, "count_diff").and_then(|v| v.as_int()).unwrap_or(0);
        format!(
            "{}: size={} ({}), count={} ({:+})",
            traceback_str_of(tb),
    ...
    if count != 0 {
        out.push_str(&format!(", average={}", format_size(size as f64 / count as f64, false)));
    }
    out
}
...
fn group_stats(
    ...
    if cumulative && !matches!(key_type, "lineno" | "filename") {
        raise("ValueError", &format!(
            "cumulative mode cannot by used with key type {key_type:?}"
        ));
        return Err(());
    }
...
unsafe extern "C" fn snapshot_statistics(self_v: MbValue, args: MbValue) -> MbValue {
    ...
    let key_type = items.iter().copied().find(|v| !is_dict_value(*v))
    ...
    let stats: Vec<MbValue> = groups.into_iter()
        .map(|(frames, size, count)| {
            ...
            make_instance("tracemalloc.Statistic", vec![
                ("traceback", make_traceback(&frames.iter().rev().cloned().collect::<Vec<_>>(), None)),
...
unsafe extern "C" fn snapshot_compare_to(self_v: MbValue, args: MbValue) -> MbValue {
    ...
    let key_type = items.get(1).copied().and_then(extract_str)
        .unwrap_or_else(|| "lineno".to_string());
    ...
    let key_of = |frames: &Vec<RawFrame>| -> String {
        frames.iter().map(|(f, l)| format!("{f}\x00{l}")).collect::<Vec<_>>().join("\x01")
    };
    ...
    diffs.sort_by(|a, b| {
        (b.2.abs(), b.1, b.4.abs(), b.3).cmp(&(a.2.abs(), a.1, a.4.abs(), a.3))
    });
    let out: Vec<MbValue> = diffs.into_iter()
        .map(|(frames, size, size_diff, count, count_diff)| {
            make_instance("tracemalloc.StatisticDiff", vec![
                ("traceback", make_traceback(&frames.iter().rev().cloned().collect::<Vec<_>>(), None)),
                ("size", MbValue::from_int(size)),
...
fn filter_matches(filter: MbValue, raw: &RawTrace) -> bool {
    let cls = filter.as_ptr().map(|p| unsafe {
        if let ObjData::Instance { ref class_name, .. } = (*p).data {
            class_name.clone()
        } else { String::new() }
    }).unwrap_or_default();
    let inclusive = get_field(filter, "inclusive").and_then(|v| v.as_bool()).unwrap_or(false);
    if cls == "tracemalloc.DomainFilter" {
        let domain = get_field(filter, "domain").and_then(|v| v.as_int()).unwrap_or(-1);
        return (raw.0 == domain) == inclusive;
    }
    let pattern = get_field(filter, "filename_pattern").and_then(extract_str).unwrap_or_default();
    let lineno = get_field(filter, "lineno").and_then(|v| v.as_int());
    let all_frames = get_field(filter, "all_frames").and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    let res = if all_frames {
        if frames.iter().any(match_impl) { inclusive } else { !inclusive }
    } else {
        match_impl(&frames[0]) == inclusive
    };
    match domain {
        Some(d) => {
            if inclusive { res && raw.0 == d } else { res || raw.0 != d }
        }
        None => res,
    }
}
...
unsafe extern "C" fn snapshot_filter_traces(self_v: MbValue, args: MbValue) -> MbValue {
    ...
    let is_seq = filters_v.as_ptr().map(|p| unsafe {
    ...
    let limit = get_field(self_v, "traceback_limit").and_then(|v| v.as_int()).unwrap_or(1);
    let raws = snapshot_raws(self_v);
...
unsafe extern "C" fn frame_str(self_v: MbValue, _args: MbValue) -> MbValue {
    let file = get_field(self_v, "filename").and_then(extract_str).unwrap_or_default();
    let line = get_field(self_v, "lineno").and_then(|v| v.as_int()).unwrap_or(0);
...
unsafe extern "C" fn traceback_repr(self_v: MbValue, _args: MbValue) -> MbValue {
    let frames = get_field(self_v, "_entries").map(list_items).unwrap_or_default();
    let parts: Vec<String> = frames.iter().map(|f| {
        let file = get_field(*f, "filename").and_then(extract_str).unwrap_or_default();
        let line = get_field(*f, "lineno").and_then(|v| v.as_int()).unwrap_or(0);
    ...
    if total.is_none() {
        ...
        new_str(&format!("<Traceback {body} total_nframe={}>", total.as_int().unwrap_or(0)))
    }
}
...
unsafe extern "C" fn trace_str(self_v: MbValue, _args: MbValue) -> MbValue {
    ...
    let size = get_field(self_v, "size").and_then(|v| v.as_int()).unwrap_or(0);
    new_str(&format!("{}: {}", traceback_str_of(tb), format_size(size as f64, false)))
}
...
fn register_tracemalloc_classes() {
    ...
    {
        let mut m: Map<String, MbValue> = Map::new();
        m.insert("__str__".into(), var(statistic_dunder_str as *const () as usize));
        super::super::class::mb_class_register("tracemalloc.Statistic", vec![], m);
        let mut d: Map<String, MbValue> = Map::new();
        d.insert("__str__".into(), var(statistic_diff_dunder_str as *const () as usize));
        super::super::class::mb_class_register("tracemalloc.StatisticDiff", vec![], d);
    }
    {
        let mut m: Map<String, MbValue> = Map::new();
        m.insert("statistics".into(), var(snapshot_statistics as *const () as usize));
        m.insert("compare_to".into(), var(snapshot_compare_to as *const () as usize));
        m.insert("filter_traces".into(), var(snapshot_filter_traces as *const () as usize));
