                ...
                if let Some(ptr) = w.as_ptr() {
                    unsafe {
                        if let ObjData::Instance { ref class_name, .. } = (*ptr).data {
                            return class_name == "ReferenceType"
...
fn make_class_stub(name: &str) -> MbValue {
    ...
    fields.insert("__qualname__".to_string(),
        MbValue::from_ptr(MbObject::new_str(name.to_string())));
    ...
    let module_name = if name == "WeakSet" { "_weakrefset" } else { "weakref" };
    ...
    if let Some(ptr) = wref.as_ptr() {
        unsafe {
            if let ObjData::Instance { ref fields, .. } = (*ptr).data {
                ...
                if let Some(t) = f.get("_target") { return *t; }
            }
        }
    }
    MbValue::none()
...
pub fn reference_repr(wref: MbValue) -> MbValue {
    let Some(wptr) = wref.as_ptr() else { return MbValue::none() };
    let target = unsafe {
        match &(*wptr).data {
            ObjData::Instance { class_name, fields } if class_name == "ReferenceType" => {
                fields.read().unwrap().get("_target").copied().unwrap_or_else(MbValue::none)
            }
    ...
    let s = if target.is_none() {
        ...
        format!(
            ...
            wref.to_bits(),
            cls,
            target.as_ptr().map(|p| p as u64).unwrap_or_else(|| target.to_bits()),
        )
    };
...
fn referent_type_name(target: MbValue) -> String {
    ...
    if target.as_int().is_some() { return "int".to_string(); }
    if target.as_float().is_some() { return "float".to_string(); }
    if target.as_bool().is_some() { return "bool".to_string(); }
    "object".to_string()
}
...
pub fn mb_weakref_getweakrefcount(obj: MbValue) -> MbValue {
    let key = referent_key(obj);
    let n = WEAKREF_REGISTRY.with(|r| {
        r.borrow().get(&key).map(|v| v.len()).unwrap_or(0)
    });
...
pub fn mb_weakref_getweakrefs(obj: MbValue) -> MbValue {
    let key = referent_key(obj);
    let items = WEAKREF_REGISTRY.with(|r| {
        r.borrow().get(&key).cloned().unwrap_or_default()
    });
    for &w in &items {
        unsafe { super::super::rc::retain_if_ptr(w); }
    }
...
pub fn mb_weakref_weak_set() -> MbValue {
    let mut fields = FxHashMap::default();
    fields.insert("data".to_string(), MbValue::from_ptr(MbObject::new_set(Vec::new())));
    fields.insert("__class__".to_string(),
...
pub fn register() {
    ...
    let dispatchers: Vec<(&str, usize)> = vec![
        ...
        ("getweakrefcount",     dispatch_getweakrefcount  as *const () as usize),
        ("getweakrefs",         dispatch_getweakrefs      as *const () as usize),
        ("WeakSet",             dispatch_weak_set         as *const () as usize),
        ("WeakKeyDictionary",   dispatch_weak_key_dict    as *const () as usize),
        ("WeakValueDictionary", dispatch_weak_value_dict  as *const () as usize),
    ...
    for (name, addr) in dispatchers {
        ...
        super::super::closure::mb_func_set_name(
            fval, MbValue::from_ptr(MbObject::new_str(name.to_string())));
        super::super::closure::mb_func_set_module(
            fval, MbValue::from_ptr(MbObject::new_str("weakref".to_string())));
    }
    ...
    attrs.insert("ReferenceType".to_string(),     make_class_stub("ReferenceType"));
    attrs.insert("ProxyType".to_string(),         make_class_stub("ProxyType"));
    attrs.insert("CallableProxyType".to_string(), make_class_stub("CallableProxyType"));
...
mod tests {
    ...
    fn test_weak_set_is_instance_with_set_data() {
        ...
        assert_eq!(get_str(get_field(s, "__class__")), Some("WeakSet".to_string()));
    ...
    fn test_weak_key_dict_is_instance_with_dict_data() {
        ...
        assert_eq!(get_str(get_field(d, "__class__")), Some("WeakKeyDictionary".to_string()));
    ...
    fn test_weak_value_dict_is_instance_with_dict_data() {
        ...
        assert_eq!(get_str(get_field(d, "__class__")), Some("WeakValueDictionary".to_string()));
        let data = get_field(d, "data");
        unsafe {
            assert!(matches!(&(*data.as_ptr().unwrap()).data, ObjData::Dict(_)));
        }
    ...
    fn test_weak_method_stores_method() {
        ...
        assert_eq!(get_str(get_field(wm, "__class__")), Some("WeakMethod".to_string()));
    ...
    fn test_finalize_returns_alive_instance() {
        ...
        assert_eq!(get_str(get_field(f, "__class__")), Some("finalize".to_string()));
    ...
    fn test_class_stub_has_name_and_module() {
        ...
        assert_eq!(get_str(get_field(c, "__name__")), Some("ReferenceType".to_string()));
        assert_eq!(get_str(get_field(c, "__module__")), Some("weakref".to_string()));
    }
    ...
    fn test_class_stubs_have_distinct_names() {
        ...
        assert_eq!(get_str(get_field(r, "__name__")), Some("ReferenceType".to_string()));
        assert_eq!(get_str(get_field(p, "__name__")), Some("ProxyType".to_string()));
        assert_eq!(get_str(get_field(c, "__name__")), Some("CallableProxyType".to_string()));
        assert_eq!(get_str(get_field(k, "__name__")), Some("KeyedRef".to_string()));
    }
