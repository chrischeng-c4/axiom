...
fn register_marker(name: &str) {
    ...
    let mut attrs = HashMap::new();
    attrs.insert("__name__".into(), MbValue::from_ptr(MbObject::new_str(name.to_string())));
    super::register_module(name, attrs);
}
...
fn register_codec_module(name: &str) {
    ...
    register_with(name,
        &["Codec","IncrementalEncoder","IncrementalDecoder","StreamWriter","StreamReader"],
...
fn register_xml_sax_package() {
    ...
    let shell = dispatch_class_shell as *const () as usize;
    let mut attrs = HashMap::new();
    for cn in &["ContentHandler","ErrorHandler","InputSource","SAXException",
                "SAXParseException","SAXNotRecognizedException",
                "SAXNotSupportedException","SAXReaderNotAvailable"] {
        attrs.insert((*cn).into(), MbValue::from_func(shell));
    }
    for fn_name in &["make_parser","parse","parseString"] {
        attrs.insert((*fn_name).into(), MbValue::from_func(shell));
    }
    attrs.insert("default_parser_list".into(),
        MbValue::from_ptr(MbObject::new_list(Vec::new())));
...
fn register_xml_remainder() {
    ...
    register_with("xml.etree.ElementPath",
        &[],
        ...
        &[], &[]);
    register_with("xml.etree.ElementInclude",
    ...
    register_with("xml.etree.cElementTree",
        &["Element","ElementTree","SubElement","Comment","ProcessingInstruction",
          "QName","XMLParser","TreeBuilder","iselement"],
...
fn register_email_internals() {
    register_with("email.base64mime",
        &[],
        ...
        &[("CRLF", 0)],
        &[]);
    register_with("email.quoprimime",
        &[],
        ...
        &[], &[]);
    register_with("email._policybase",
        &["Policy","Compat32"],
        &[], &[], &[]);
    register_with("email._encoded_words",
        &[],
        &[("decode",  dispatch_empty_str as *const () as usize),
          ("encode",  dispatch_empty_str as *const () as usize),
        ...
        &[], &[]);
    register_with("email._header_value_parser",
        &["TokenList","WhiteSpaceTerminal","UnstructuredTokenList","Phrase",
          "Word","CFWSList","Atom","Token","EncodedWord","DotAtomText","DotAtom",
          "AddrSpec","LocalPart","DomainLiteral","Domain","Address","AddressList",
          "MailboxList","Mailbox","NameAddr","AngleAddr","GroupList","Group",
          "DisplayName","Identifier","HeaderLabel","Header","ParameterizedHeaderValue",
          "Parameter","MimeParameters","MIMEVersion","ContentType","ContentDisposition",
          "ContentTransferEncoding","BareQuotedString","QuotedString","Comment"],
        &[], &[], &[]);
    register_with("email._parseaddr",
...
fn register_asyncio_remainder() {
    register_with("asyncio.format_helpers",
        &[],
        &[("extract_stack",        dispatch_empty_list as *const () as usize),
          ...
          ("_format_callback_source", dispatch_empty_str as *const () as usize)],
        &[], &[]);
    register_with("asyncio.log",
        ...
        &[],
        &[]);
    register_with("asyncio.windows_events", &["ProactorEventLoop","WindowsSelectorEventLoopPolicy","WindowsProactorEventLoopPolicy"], &[], &[], &[]);
    register_with("asyncio.windows_utils", &["PipeHandle"], &[("pipe", dispatch_empty_list as *const () as usize)], &[], &[]);
    register_with("asyncio.unix_events",
        &["SelectorEventLoop","AbstractChildWatcher","SafeChildWatcher",
          "FastChildWatcher","PidfdChildWatcher","MultiLoopChildWatcher",
          "ThreadedChildWatcher","DefaultEventLoopPolicy"],
        &[], &[], &[]);
    register_with("asyncio.selector_events",
        &["BaseSelectorEventLoop"],
        &[], &[], &[]);
    register_with("asyncio.proactor_events",
        &["BaseProactorEventLoop"],
        &[], &[], &[]);
    register_with("asyncio.taskgroups",
        &["TaskGroup"],
        &[], &[], &[]);
    register_with("asyncio.timeouts",
        &["Timeout","_State"],
        &[("timeout",     dispatch_class_shell as *const () as usize),
          ("timeout_at",  dispatch_class_shell as *const () as usize)],
        &[], &[]);
    register_with("asyncio.staggered",
        &[],
        &[("staggered_race", dispatch_class_shell as *const () as usize)],
        &[], &[]);
    register_with("asyncio.threads",
        &[],
        &[("to_thread", dispatch_class_shell as *const () as usize)],
        &[], &[]);
    register_with("asyncio.constants",
        &[],
        &[],
        ...
        &[]);
    register_with("asyncio.mixins",
        &["_LoopBoundMixin"],
        &[], &[], &[]);
}

fn register_concurrent_futures_subs() {
    register_with("concurrent.futures.thread",
        &["ThreadPoolExecutor","BrokenThreadPool","_WorkItem"],
        &[], &[], &[]);
    register_with("concurrent.futures.process",
        &["ProcessPoolExecutor","BrokenProcessPool","_ResultItem","_CallItem"],
        &[("EXTRA_QUEUED_CALLS", dispatch_int_zero as *const () as usize)],
        &[], &[]);
    register_with("concurrent.futures._base",
        &["Future","Executor","CancelledError","TimeoutError","InvalidStateError",
          "BrokenExecutor","FIRST_COMPLETED","FIRST_EXCEPTION","ALL_COMPLETED"],
        &[("wait",          dispatch_class_shell as *const () as usize),
          ("as_completed",  dispatch_empty_list  as *const () as usize)],
        &[("PENDING",1),("RUNNING",2),("CANCELLED",3),("CANCELLED_AND_NOTIFIED",4),
          ("FINISHED",5),("LOG_LEVEL",30)],
...
fn register_collections_underscore() {
    ...
    register_with("collections._collections_abc",
...
fn register_codec_shims() {
    for name in &["encodings.ascii", "encodings.utf_16", "encodings.latin_1",
                  "encodings.cp1252", "encodings.cp437", "encodings.cp850",
                  "encodings.mbcs"] {
...
fn register_third_party_probe_shells() {
    ...
    for name in &["yaml", "numpy", "pandas", "matplotlib", "scipy",
...
fn register_c_extensions() {
    ...
    register_with("_string",
        &[],
        &[("formatter_field_name_split", dispatch_empty_list as *const () as usize),
          ("formatter_parser",            dispatch_empty_list as *const () as usize)],
        &[], &[]);
    register_with("_decimal",
        &["Decimal","Context","DecimalException","Clamped","DivisionByZero",
          "InvalidOperation","Overflow","Rounded","Subnormal","Underflow",
          "Inexact","FloatOperation","DefaultContext","BasicContext","ExtendedContext"],
    ...
    register_with("_json",
        ...
        &[], &[]);
    register_with("_pickle",
        &["Pickler","Unpickler","PicklingError","UnpicklingError","PickleError"],
        &[("dump",  dispatch_noop      as *const () as usize),
          ("dumps", dispatch_empty_str as *const () as usize),
          ("load",  dispatch_noop      as *const () as usize),
          ("loads", dispatch_noop      as *const () as usize)],
        &[], &[]);
    register_with("_random",
        &["Random"],
        &[], &[], &[]);
    register_with("_socket",
    ...
    register_with("_signal",
        ...
        &[("SIG_DFL",0),("SIG_IGN",1),
          ...
          ("SIGPIPE",13),("SIGALRM",14),("SIGTERM",15),("SIGUSR1",10),("SIGUSR2",12),
    ...
    register_with("_io",
        &["IOBase","RawIOBase","BufferedIOBase","TextIOBase","FileIO",
          ...
          "BufferedRandom","TextIOWrapper","UnsupportedOperation","BlockingIOError",
    ...
    register_with("_csv",
        ...
        &[("QUOTE_MINIMAL",0),("QUOTE_ALL",1),("QUOTE_NONNUMERIC",2),
          ("QUOTE_NONE",3),("QUOTE_STRINGS",4),("QUOTE_NOTNULL",5)],
        &[("__version__","1.0")]);
    register_with("_codecs",
        &["__doc__"],
        &[("encode",                 dispatch_empty_str as *const () as usize),
          ("decode",                 dispatch_empty_str as *const () as usize),
