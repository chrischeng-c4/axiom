...
pub fn register() {
    ...
    attrs.insert("Binnumber".into(), MbValue::from_ptr(MbObject::new_str("0[bB](?:_?[01])+".to_string())));
    ...
    attrs.insert("Comment".into(), MbValue::from_ptr(MbObject::new_str("#[^\\r\\n]*".to_string())));
    ...
    attrs.insert("Decnumber".into(), MbValue::from_ptr(MbObject::new_str("(?:0(?:_?0)*|[1-9](?:_?[0-9])*)".to_string())));
    attrs.insert("Double".into(), MbValue::from_ptr(MbObject::new_str("[^\"\\\\]*(?:\\\\.[^\"\\\\]*)*\"".to_string())));
    attrs.insert("Double3".into(), MbValue::from_ptr(MbObject::new_str("[^\"\\\\]*(?:(?:\\\\.|\"(?!\"\"))[^\"\\\\]*)*\"\"\"".to_string())));
    ...
    attrs.insert("Expfloat".into(), MbValue::from_ptr(MbObject::new_str("[0-9](?:_?[0-9])*[eE][-+]?[0-9](?:_?[0-9])*".to_string())));
    attrs.insert("Exponent".into(), MbValue::from_ptr(MbObject::new_str("[eE][-+]?[0-9](?:_?[0-9])*".to_string())));
    ...
    attrs.insert("Hexnumber".into(), MbValue::from_ptr(MbObject::new_str("0[xX](?:_?[0-9a-fA-F])+".to_string())));
    attrs.insert("Ignore".into(), MbValue::from_ptr(MbObject::new_str("[ \\f\\t]*(\\\\\\r?\\n[ \\f\\t]*)*(#[^\\r\\n]*)?".to_string())));
    ...
    attrs.insert("Name".into(), MbValue::from_ptr(MbObject::new_str("\\w+".to_string())));
    attrs.insert("Octnumber".into(), MbValue::from_ptr(MbObject::new_str("0[oO](?:_?[0-7])+".to_string())));
    ...
    attrs.insert("Single".into(), MbValue::from_ptr(MbObject::new_str("[^'\\\\]*(?:\\\\.[^'\\\\]*)*'".to_string())));
    attrs.insert("Single3".into(), MbValue::from_ptr(MbObject::new_str("[^'\\\\]*(?:(?:\\\\.|'(?!''))[^'\\\\]*)*'''".to_string())));
    ...
    attrs.insert("StringPrefix".into(), MbValue::from_ptr(MbObject::new_str("(|Rb|Br|RF|RB|R|br|Rf|Fr|rb|F|B|BR|bR|r|rB|u|rf|b|fR|U|f|FR|rF|fr)".to_string())));
    ...
    attrs.insert("Whitespace".into(), MbValue::from_ptr(MbObject::new_str("[ \\f\\t]*".to_string())));
...
pub fn mb_tokenize_TokenInfo(
    ...
    MbValue::from_ptr(MbObject::new_tuple(vec![tok_type, string, start, end, line]))
}
...
pub fn mb_tokenize_tok_name() -> MbValue {
    ...
    unsafe {
        ...
        if let ObjData::Dict(ref lock) = (*dict).data {
            ...
            let entries = &[
                (ENDMARKER, "ENDMARKER"), (NAME, "NAME"), (NUMBER, "NUMBER"),
                (STRING, "STRING"), (NEWLINE, "NEWLINE"), (NL, "NL"),
                (COMMENT, "COMMENT"), (INDENT, "INDENT"), (DEDENT, "DEDENT"),
                (OP, "OP"), (ERRORTOKEN, "ERRORTOKEN"), (ENCODING, "ENCODING"),
            ];
            for (code, name) in entries {
                map.insert(code.to_string().into(),
    ...
    fn test_tokenize_bytes_input_no_silent_drop() {
        ...
        unsafe {
            if let ObjData::List(ref lock) = (*ptr).data {
                ...
                assert!(list.len() >= 2,
                    "tokenize(bytes) yielded only {} tokens — silent drop?", list.len());
            } else {
