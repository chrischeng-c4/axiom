...
unsafe extern "C" fn dispatch_add_history(args_ptr: *const MbValue, nargs: usize) -> MbValue {
    ...
    let max = *history_length_setting().lock().unwrap_or_else(|e| {
        history_length_setting().clear_poison();
        e.into_inner()
    });
    let mut h = history().lock().unwrap_or_else(|e| {
...
unsafe extern "C" fn dispatch_get_history_length(_a: *const MbValue, _n: usize) -> MbValue {
    ...
    MbValue::from_int(*history_length_setting().lock().unwrap_or_else(|e| {
        history_length_setting().clear_poison();
        e.into_inner()
    }))
}

unsafe extern "C" fn dispatch_set_history_length(
    ...
    *history_length_setting().lock().unwrap_or_else(|e| {
        history_length_setting().clear_poison();
        e.into_inner()
    }) = n;
    if n >= 0 {
...
unsafe extern "C" fn dispatch_get_history_item(args_ptr: *const MbValue, nargs: usize) -> MbValue {
    ...
    let h = history().lock().unwrap_or_else(|e| {
        history().clear_poison();
        e.into_inner()
    });
...
unsafe extern "C" fn dispatch_remove_history_item(
    args_ptr: *const MbValue,
    nargs: usize,
) -> MbValue {
    let args = args_slice(args_ptr, nargs);
...
unsafe extern "C" fn dispatch_replace_history_item(
    args_ptr: *const MbValue,
    nargs: usize,
) -> MbValue {
    let args = args_slice(args_ptr, nargs);
...
unsafe extern "C" fn dispatch_write_history_file(
    args_ptr: *const MbValue,
    nargs: usize,
) -> MbValue {
    let args = args_slice(args_ptr, nargs);
...
unsafe extern "C" fn dispatch_append_history_file(
    args_ptr: *const MbValue,
    nargs: usize,
) -> MbValue {
    let args = args_slice(args_ptr, nargs);
...
pub fn register() {
    ...
    let dispatchers: &[(&str, usize)] = &[
        ...
        (
            "clear_history",
            dispatch_clear_history as *const () as usize,
        ),
        (
            "get_history_length",
            dispatch_get_history_length as *const () as usize,
        ),
        (
            "set_history_length",
            dispatch_set_history_length as *const () as usize,
        ),
        (
            "get_current_history_length",
            dispatch_get_current_history_length as *const () as usize,
        ),
        (
            "get_history_item",
            dispatch_get_history_item as *const () as usize,
        ),
        (
            "remove_history_item",
            dispatch_remove_history_item as *const () as usize,
        ),
        (
            "replace_history_item",
            dispatch_replace_history_item as *const () as usize,
        ),
        (
            "read_history_file",
            dispatch_read_history_file as *const () as usize,
        ),
        (
            "write_history_file",
            dispatch_write_history_file as *const () as usize,
        ),
        (
            "append_history_file",
            dispatch_append_history_file as *const () as usize,
        ),
...
mod tests {
    ...
    fn reset_history() {
        ...
        h.clear();
        drop(h);
        let mut l = history_length_setting().lock().unwrap_or_else(|e| {
            history_length_setting().clear_poison();
            e.into_inner()
        });
        *l = -1;
