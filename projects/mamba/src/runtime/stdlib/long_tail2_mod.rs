...
pub fn register() {
    // json submodules
    register_with("json.decoder",
        &["JSONDecoder", "JSONDecodeError"],
        &[("scanstring", dispatch_empty_str as *const () as usize)],
        &[], &[]);
    register_with("json.encoder",
        &["JSONEncoder"],
        &[("encode_basestring", dispatch_empty_str as *const () as usize),
          ("encode_basestring_ascii", dispatch_empty_str as *const () as usize),
          ("py_encode_basestring_ascii", dispatch_empty_str as *const () as usize)],
        &[("INFINITY", 0)], &[]);
    register_with("json.scanner",
        &["JSONDecodeError"],
        &[("py_make_scanner", dispatch_noop as *const () as usize),
          ("make_scanner",    dispatch_noop as *const () as usize)],
        &[], &[]);
    register_with("json.tool",
        &[],
        &[("main", dispatch_noop as *const () as usize)],
        &[], &[]);

    // logging submodules
    register_with("logging.handlers",
        &[
            "StreamHandler","FileHandler","NullHandler","WatchedFileHandler",
            "BaseRotatingHandler","RotatingFileHandler","TimedRotatingFileHandler",
            "SocketHandler","DatagramHandler","SysLogHandler","NTEventLogHandler",
            "SMTPHandler","HTTPHandler","MemoryHandler","BufferingHandler",
            ...
            "copy","io","logging","os","pickle","queue","re","socket","struct",
            "threading","time",
        ],
        &[],
        &[("DEFAULT_TCP_LOGGING_PORT",9020),
          ...
          ("ST_MTIME",8)],
        &[]);
    register_with("logging.config",
        &[
            // classes / configurators
            "BaseConfigurator","ConvertingDict","ConvertingList",
            "ConvertingMixin","ConvertingTuple","DictConfigurator",
            "dictConfigClass","StreamRequestHandler","ThreadingTCPServer",
            // re-exported submodules (present-only; hasattr probes)
            "errno","functools","io","logging","os","queue","re",
    ...
    register_with("asyncio.subprocess",
        &[
            "Process","SubprocessStreamProtocol","PIPE","STDOUT","DEVNULL",
        ],
        &[("create_subprocess_shell", dispatch_class_shell as *const () as usize),
          ("create_subprocess_exec",  dispatch_class_shell as *const () as usize)],
        &[("PIPE",-1),("STDOUT",-2),("DEVNULL",-3)],
        &[]);
    register_with("asyncio.queues",
        &["Queue","PriorityQueue","LifoQueue","QueueFull","QueueEmpty",
          "QueueShutDown"],
        &[], &[], &[]);
    register_with("asyncio.streams",
        &["StreamReader","StreamWriter","StreamReaderProtocol",
          "FlowControlMixin","LimitOverrunError","IncompleteReadError"],
        &[("open_connection",        dispatch_class_shell as *const () as usize),
          ("start_server",            dispatch_class_shell as *const () as usize),
          ("open_unix_connection",    dispatch_class_shell as *const () as usize),
          ("start_unix_server",       dispatch_class_shell as *const () as usize)],
        &[], &[]);
    register_with("asyncio.locks",
        &["Lock","Event","Condition","Semaphore","BoundedSemaphore","Barrier",
          "BrokenBarrierError"],
        &[], &[], &[]);
    register_with("asyncio.futures",
        &["Future","CancelledError","InvalidStateError","TimeoutError"],
        &[("wrap_future", dispatch_class_shell as *const () as usize),
          ("isfuture",     dispatch_false       as *const () as usize)],
        &[], &[]);
    register_with("asyncio.tasks",
        &["Task"],
        &[("ensure_future",   dispatch_class_shell as *const () as usize),
          ...
          ("run_coroutine_threadsafe", dispatch_class_shell as *const () as usize),
        ...
        &[], &[]);
    register_with("asyncio.protocols",
        &["BaseProtocol","Protocol","DatagramProtocol","SubprocessProtocol",
          "BufferedProtocol"],
        &[], &[], &[]);
    register_with("asyncio.transports",
        &["BaseTransport","ReadTransport","WriteTransport","Transport",
          "DatagramTransport","SubprocessTransport"],
        &[], &[], &[]);
    register_with("asyncio.events",
        &["AbstractEventLoopPolicy","AbstractEventLoop","Handle","TimerHandle",
          "AbstractServer"],
        &[("get_event_loop_policy", dispatch_class_shell as *const () as usize),
          ...
          ("get_running_loop",      dispatch_class_shell as *const () as usize),
          ("_get_running_loop",     dispatch_class_shell as *const () as usize)],
        &[], &[]);
    register_with("asyncio.exceptions",
        &["CancelledError","InvalidStateError","TimeoutError",
          "IncompleteReadError","LimitOverrunError","SendfileNotAvailableError",
          "BrokenBarrierError"],
        &[], &[], &[]);
    register_with("asyncio.coroutines",
        &[],
        ...
        &[], &[]);
    register_with("asyncio.base_events",
        &["BaseEventLoop","Server"],
        &[], &[], &[]);
    register_with("asyncio.runners",
        &["Runner"],
        &[("run", dispatch_noop as *const () as usize)],
        &[], &[]);

    // multiprocessing submodules
    register_with("multiprocessing.pool",
        &["Pool","ThreadPool","ApplyResult","MapResult","IMapIterator",
          "IMapUnorderedIterator","AsyncResult"],
        &[], &[], &[]);
    register_with("multiprocessing.queues",
        &["Queue","SimpleQueue","JoinableQueue"],
        &[], &[], &[]);
    register_with("multiprocessing.shared_memory",
        &["SharedMemory","ShareableList"],
        &[], &[], &[]);
    register_with("multiprocessing.connection",
        &["Connection","Listener","Client","Pipe","wait","answer_challenge",
          "deliver_challenge"],
        &[], &[], &[]);
    register_with("multiprocessing.context",
        &["BaseContext","Process","ProcessError","BufferTooShort",
          "AuthenticationError","TimeoutError","DefaultContext",
          "ForkContext","SpawnContext","ForkServerContext"],
        &[("get_context",       dispatch_class_shell as *const () as usize),
          ...
          ("get_all_start_methods", dispatch_empty_list as *const () as usize)],
        &[], &[]);
    register_with("multiprocessing.managers",
        &["BaseManager","SyncManager","Namespace","Token","RemoteError",
          "Server","State","BaseProxy","MakeProxyType"],
        &[], &[], &[]);
    register_with("multiprocessing.process",
        &["BaseProcess","Process"],
        &[("current_process",   dispatch_class_shell as *const () as usize),
          ("active_children",   dispatch_empty_list  as *const () as usize),
          ("parent_process",    dispatch_class_shell as *const () as usize)],
        &[], &[]);
    register_with("multiprocessing.synchronize",
        &["Lock","RLock","Semaphore","BoundedSemaphore","Condition","Event",
          "Barrier","SemLock"],
        &[], &[], &[]);
    register_with("multiprocessing.util",
        &[],
        ...
        &[("SUBDEBUG",5),("SUBWARNING",25),("DEBUG",10),("INFO",20),
          ("NOTSET",0)],
        &[]);
    ...
    register_with("tkinter",
        &["Tk","Frame","Label","Button","Entry","Text","Canvas","Menu",
          "Listbox","Scale","Scrollbar","Toplevel","Widget","Variable",
          "StringVar","IntVar","DoubleVar","BooleanVar","TclError"],
        &[("mainloop", dispatch_noop as *const () as usize)],
        &[("NORMAL",0),("DISABLED",1),("HIDDEN",2),("ACTIVE",3),
          ("HORIZONTAL",0),("VERTICAL",1)],
        &[]);
    register_with("tkinter.ttk",
        &["Style","Widget","Button","Combobox","Entry","Frame","Label",
          "LabelFrame","Notebook","Panedwindow","Progressbar","Radiobutton",
          "Scale","Scrollbar","Separator","Sizegrip","Treeview"],
        &[], &[], &[]);
    register_with("tkinter.messagebox",
        &[],
        ...
        &[], &[]);
    register_with("tkinter.filedialog",
        &[],
        &[("askopenfilename",   dispatch_empty_str as *const () as usize),
          ("asksaveasfilename", dispatch_empty_str as *const () as usize),
          ("askopenfile",       dispatch_class_shell as *const () as usize),
          ("askopenfilenames",  dispatch_empty_list as *const () as usize),
          ("askdirectory",      dispatch_empty_str as *const () as usize)],
        &[], &[]);

    register_with("turtle",
    ...
    register_with("curses",
        ...
        &[("COLOR_BLACK",0),("COLOR_RED",1),("COLOR_GREEN",2),
          ...
          ("A_NORMAL",0),("A_STANDOUT",65536),("A_UNDERLINE",131072),
          ("A_REVERSE",262144),("A_BLINK",524288),("A_DIM",1048576),
          ("A_BOLD",2097152),("KEY_UP",259),("KEY_DOWN",258),
          ("KEY_LEFT",260),("KEY_RIGHT",261),("KEY_ENTER",343),
          ("KEY_BACKSPACE",263)],
        &[]);
    register_with("curses.ascii",
        &[],
        ...
        &[("NUL",0),("SOH",1),("STX",2),("ETX",3),("EOT",4),("ENQ",5),
          ("ACK",6),("BEL",7),("BS",8),("TAB",9),("LF",10),("VT",11),
          ("FF",12),("CR",13),("SO",14),("SI",15),("ESC",27),("SP",32),
          ("DEL",127)],
        &[]);
    register_with("curses.panel",
        &["panel"],
    ...
    register_with("imp",
        ...
        &[("SEARCH_ERROR",0),("PY_SOURCE",1),("PY_COMPILED",2),("C_EXTENSION",3),
          ("PY_RESOURCE",4),("PKG_DIRECTORY",5),("C_BUILTIN",6),("PY_FROZEN",7)],
        &[]);
    register_with("formatter",
        &["DumbWriter","AbstractFormatter","NullFormatter","AbstractWriter",
          "NullWriter"],
    ...
    register_with("pydoc_data.topics",
        &[],
        &[],
        &[],
