...
disp_binary!(d_pthread_sigmask, mb_signal_pthread_sigmask);
...
///
/// Returns the SIG_DFL sentinel (`0`) regardless of what was installed
/// before — mamba does not track per-process signal dispositions.
pub fn mb_signal_signal(_signum: MbValue, _handler: MbValue) -> MbValue {
    MbValue::from_int(0)
}

/// signal.getsignal(signum) -> handler.
///
/// **Hot path (#1470 Gate 2).** CPython walks a Python-level dict and
/// constructs a `Signals(signum)` IntEnum on the return path; mamba
/// returns the SIG_DFL sentinel int (`0`) directly. Keep this body
/// trivially small — every additional allocation regresses the gate.
pub fn mb_signal_getsignal(_signum: MbValue) -> MbValue {
    MbValue::from_int(0)
}
...
pub fn mb_signal_set_wakeup_fd(_fd: MbValue) -> MbValue {
    MbValue::from_int(-1)
}
...
///
/// CPython returns a localized description; surface-presence callers
/// only check callability, so we return None.
pub fn mb_signal_strsignal(_signum: MbValue) -> MbValue {
    MbValue::none()
}

/// signal.valid_signals() -> set[int].
///
/// CPython returns the set of usable signals on the host. Mamba returns
/// an empty list (callers usually iterate; an empty iterable is the
/// safest no-op).
...
/// signal.pthread_sigmask(how, mask) -> previous mask (empty list).
pub fn mb_signal_pthread_sigmask(_how: MbValue, _mask: MbValue) -> MbValue {
    MbValue::from_ptr(MbObject::new_list(Vec::new()))
}
...
mod tests {
    ...
    #[test]
    fn test_getsignal_hot_path_returns_sig_dfl() {
        // Perf-gate path: must remain a single from_int — no allocations,
        // no dict lookups. Any indirection here regresses #1470 Gate 2.
        let r = mb_signal_getsignal(MbValue::from_int(15));
        assert_eq!(r.as_int(), Some(0));
    }

    #[test]
    fn test_signal_returns_sig_dfl() {
        let r = mb_signal_signal(MbValue::from_int(2), MbValue::none());
        assert_eq!(r.as_int(), Some(0));
    }
    ...
    fn test_noop_callables_return_none() {
        ...
        assert!(mb_signal_strsignal(MbValue::from_int(2)).is_none());
