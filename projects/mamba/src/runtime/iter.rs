...
pub enum IterKind {
    ...
    Enumerate { inner: Box<MbIterator>, count: i64 },
...
pub fn mb_enumerate(iterable: MbValue, start: MbValue) -> MbValue {
    ...
    if let Some(inner_id_val) = inner_id.as_int() {
        let inner = ITERATORS.with(|iters| iters.borrow_mut().remove(&(inner_id_val as u64)));
...
pub fn mb_zip(a: MbValue, b: MbValue) -> MbValue {
    ...
    for id_val in [id_a, id_b] {
        if let Some(id) = id_val.as_int() {
            let inner = ITERATORS.with(|iters| iters.borrow_mut().remove(&(id as u64)));
            if let Some(inner) = inner {
                inners.push(Box::new(inner));
            }
        }
    }
    if inners.len() != 2 {
...
fn release_iter(iter: &MbIterator) {
    match &iter.kind {
        ...
        IterKind::Enumerate { inner, .. } => release_iter(inner),
        IterKind::Zip { inners, .. } => {
            for i in inners {
                release_iter(i);
            }
...
fn advance_iter(iter: &mut MbIterator) -> MbValue {
    ...
    match &mut iter.kind {
        ...
        IterKind::Enumerate { inner, count } => {
            let next_val = advance_iter(inner);
            if next_val.is_none() && inner.exhausted {
                iter.exhausted = true;
                MbValue::none()
            } else {
                let idx = MbValue::from_int(*count);
                *count += 1;
                MbValue::from_ptr(MbObject::new_tuple(vec![idx, next_val]))
            }
        }
        IterKind::Reversed { items, index } => {
            if *index < items.len() {
        ...
        IterKind::Zip {
            ...
            for i in 0..n {
                let val = advance_iter(&mut inners[i]);
                if val.is_none() && inners[i].exhausted {
                    exhausted_at = Some(i);
                    break;
                }
                vals.push(val);
            ...
            if let Some(i) = exhausted_at {
                ...
                if strict {
                    if i > 0 {
                        // iter[0..i] all yielded values for this row, but
                        // iter[i] just exhausted → arg{i+1} is shorter
                        // than arg 1 (CPython's exact wording).
                        ...
                        // iter[0] exhausted at row start. CPython advances
                        // each remaining iter once to confirm they're also
                        // empty; the first one that still has a value names
                        // the "longer" argument.
                        for j in 1..n {
                            let v = advance_iter(&mut inners[j]);
                            if !(v.is_none() && inners[j].exhausted) {
                                let typ =
                                    MbValue::from_ptr(MbObject::new_str("ValueError".to_string()));
                                let msg = MbValue::from_ptr(MbObject::new_str(format!(
