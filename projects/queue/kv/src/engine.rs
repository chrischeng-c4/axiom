...
impl Shard {
    ...
    fn track_access(&self, key: &str) {
        self.lru_times.write().insert(key.to_string(), Instant::now());
    ...
    /// Returns WRONGTYPE error if key holds wrong type
    pub fn hgetall(&self, key: &str) -> Result<std::collections::HashMap<String, KvValue>, KvError> {
        let guard = self.data.read();
        match guard.get(key) {
            Some(entry) if !entry.is_expired() => {
                self.track_access(key);
                ...
                match &mut entry.value {
                    KvValue::Map(map) => {
                        let current = match map.get(field) {
                            ...
                            Some(_) => {
                                return Err(KvError::Storage(
                                    "hash value is not a number".to_string(),
                                ));
                            }
                            None => 0.0,
                        };
    ...
    fn estimate_value_size(value: &KvValue) -> usize {
        match value {
            ...
            KvValue::Map(m) => m.iter().map(|(k, v)| k.len() + Self::estimate_value_size(v)).sum::<usize>() + 48,
...
impl KvEngine {
    ...
    fn check_memory_and_evict(&self) -> Result<(), KvError> {
        let maxmem = *self.maxmemory.read();
        if maxmem == 0 {
            return Ok(()); // No limit
        }
    ...
    pub fn setnx(&self, key: &KvKey, value: KvValue, ttl: Option<Duration>) -> Result<bool, KvError> {
        // Check memory limit first
        self.check_memory_and_evict()?;
        ...
        Ok(self.shard_for_key(key.as_str())
    ...
    /// # Example
    /// ```
    /// use cclab_kv::engine::KvEngine;
    /// use cclab_kv::types::{KvKey, KvValue};
    ///
    ...
    /// Returns WRONGTYPE error if key holds wrong type
    pub fn hgetall(&self, key: &KvKey) -> Result<std::collections::HashMap<String, KvValue>, KvError> {
    ...
    pub fn hexists(&self, key: &KvKey, field: &str) -> Result<bool, KvError> {
        self.shard_for_key(key.as_str()).hexists(key.as_str(), field)
    }
    ...
    pub fn lrange(&self, key: &KvKey, start: i64, stop: i64) -> Result<Vec<KvValue>, KvError> {
        self.shard_for_key(key.as_str()).lrange(key.as_str(), start, stop)
    }
...
mod tests {
    ...
    fn test_basic_set_get() {
        ...
        engine.set(&key, KvValue::String("hello".to_string()), None).unwrap();
    ...
    fn test_incr_type_mismatch() {
        ...
        engine.set(&key, KvValue::String("not a number".to_string()), None).unwrap();
    ...
    fn test_cas_success() {
        ...
        engine.set(&key, KvValue::String("initial".to_string()), None).unwrap();
    ...
    fn test_cas_failure() {
        ...
        engine.set(&key, KvValue::String("actual".to_string()), None).unwrap();
    ...
    fn test_ttl_expiration() {
        ...
        // Set with 10ms TTL
        engine.set(&key, KvValue::Int(42), Some(Duration::from_millis(10))).unwrap();
    ...
    fn test_cleanup_expired() {
        ...
        for i in 0..10 {
            ...
            engine.set(&key, KvValue::Int(i), Some(Duration::from_millis(5))).unwrap();
        }
    ...
    fn test_setnx_success() {
        ...
        assert!(engine.setnx(&key, KvValue::String("value1".to_string()), None).unwrap());

        // Second SETNX should fail (key exists)
        assert!(!engine.setnx(&key, KvValue::String("value2".to_string()), None).unwrap());
    ...
    fn test_setnx_expired() {
        ...
        // Set with short TTL
        engine.setnx(&key, KvValue::String("old".to_string()), Some(Duration::from_millis(10))).unwrap();

        // Wait for expiration
        thread::sleep(Duration::from_millis(20));

        // SETNX should now succeed (expired)
        assert!(engine.setnx(&key, KvValue::String("new".to_string()), None).unwrap());
    ...
    fn test_expire_and_ttl() {
        ...
        engine.set(&key, KvValue::String("value".to_string()), None).unwrap();
    ...
    fn test_persist() {
        ...
        // Set with TTL
        engine.set(&key, KvValue::String("value".to_string()), Some(Duration::from_secs(60))).unwrap();
    ...
    fn test_hset_hget() {
        ...
        // Get single field
        assert_eq!(engine.hget(&key, "field1").unwrap(), Some(KvValue::String("value1".to_string())));
        ...
        assert_eq!(engine.hget(&key, "field1").unwrap(), Some(KvValue::String("updated".to_string())));
    }
    ...
    fn test_hash_type_mismatch() {
        ...
        engine.set(&key, KvValue::String("not a hash".to_string()), None).unwrap();
    ...
    fn test_hincrby_type_error() {
        ...
        // Set a string field
        engine.hset(&key, vec![("name".to_string(), KvValue::String("test".to_string()))]).unwrap();
    ...
    fn test_lpop_rpop() {
        ...
        // LPOP from head
        assert_eq!(engine.lpop(&key), Some(KvValue::String("first".to_string())));

        // RPOP from tail
        assert_eq!(engine.rpop(&key), Some(KvValue::String("third".to_string())));

        // Only "second" remains
        assert_eq!(engine.llen(&key).unwrap(), 1);
        assert_eq!(engine.lpop(&key), Some(KvValue::String("second".to_string())));
    ...
    fn test_lrange() {
        ...
        // Positive indices
        assert_eq!(engine.lrange(&key, 0, 2).unwrap(), vec![KvValue::Int(0), KvValue::Int(1), KvValue::Int(2)]);

        // Negative indices
        assert_eq!(engine.lrange(&key, -3, -1).unwrap(), vec![KvValue::Int(2), KvValue::Int(3), KvValue::Int(4)]);

        // Mixed
        assert_eq!(engine.lrange(&key, 1, -2).unwrap(), vec![KvValue::Int(1), KvValue::Int(2), KvValue::Int(3)]);

        // Out of bounds
        assert_eq!(engine.lrange(&key, 0, 100).unwrap(), vec![
            KvValue::Int(0), KvValue::Int(1), KvValue::Int(2), KvValue::Int(3), KvValue::Int(4)
        ]);
    }
    ...
    fn test_list_type_mismatch() {
        ...
        engine.set(&key, KvValue::String("string".to_string()), None).unwrap();
    ...
    fn test_eviction_lru() {
        ...
        // Add first key
        let key1 = KvKey::new("key1").unwrap();
        engine.set(&key1, KvValue::String("value1".to_string()), None).unwrap();
        ...
        for i in 2..10 {
            ...
            let _ = engine.hset(&key, vec![
                ("f".to_string(), KvValue::String("x".repeat(50))),
            ]);
        }
    ...
    fn test_eviction_noeviction_oom() {
        ...
        for i in 0..5 {
            ...
            engine.set(&key, KvValue::String("x".repeat(100)), None).unwrap();
        }
        ...
        let result = engine.hset(&key, vec![
            ("f".to_string(), KvValue::String("x".repeat(200))),
        ]);
