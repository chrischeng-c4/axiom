...
impl KvClient {
    ...
    /// # }
    /// ```
    pub async fn scan(&mut self, prefix: Option<&str>, limit: usize) -> Result<Vec<String>, ClientError> {
        let mut payload = Vec::new();
    ...
    /// # }
    /// ```
    pub async fn cas(&mut self, key: &str, expected: &KvValue, new_value: KvValue, ttl: Option<Duration>) -> Result<bool, ClientError> {
        let prefixed_key = self.prefix_key(key);
        let mut payload = Vec::new();
...
mod tests {
    ...
    async fn test_nested_namespace() {
        ...
        let mut client = KvClient::connect("127.0.0.1:7117/prod/cache").await.unwrap();
