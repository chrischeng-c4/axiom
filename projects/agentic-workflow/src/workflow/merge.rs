// SPEC-MANAGED: projects/agentic-workflow/tech-design/core/interfaces/workflow/merge.md#source
// CODEGEN-BEGIN
...
pub fn handle(
    ...
    let action = match &phase {
        ...
        StatePhase::ChangeMergeCreated => {
            let all_merged = specs_to_merge.is_empty() || specs_merged >= specs_to_merge.len();
            if !all_merged {
                Action::ResumeMerge
    ...
    let archive_path = format!(".aw/archive/{}-{}", Utc::now().format("%Y%m%d"), change_id);
