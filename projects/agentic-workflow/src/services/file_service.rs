// SPEC-MANAGED: projects/agentic-workflow/tech-design/core/interfaces/services/file_service.md#source
// CODEGEN-BEGIN
...
fn list_main_specs_scoped(group: Option<&str>, project_root: &Path) -> Result<String> {
    ...
    if !specs_dir.exists()
        && !project_root
            ...
            .exists()
        && project_td_paths.iter().all(|(_, path)| !path.exists())
    {
    ...
    if let Some(group) = group {
        ...
        let agentic_workflow_td = project_root.join("projects/agentic-workflow/tech-design");
        if agentic_workflow_td.exists() {
            entries.push(("agentic-workflow".to_string(), agentic_workflow_td));
        }
...
mod tests {
    ...
    fn test_read_main_spec_with_config_scopes() {
        ...
        // Write config.toml with [specs.scopes] mapping agentic-workflow → crates
        ...
        let result = read_file(
            "any-change",
            "main_spec:agentic-workflow/core/logic/state-machine",
            project_root,
        )
        .unwrap();
        assert!(
...
"#,
        ...
        let result = read_file(
            "any-change",
            "main_spec:agentic-workflow/logic/state-machine",
            project_root,
        )
        .unwrap();
...
"#,
        ...
        let result = read_file(
            "any-change",
            "list:main_specs:agentic-workflow",
            project_root,
        )
        .unwrap();
    ...
    fn test_read_main_spec_fallback_no_config() {
        ...
        let result = read_file(
            "any-change",
            "main_spec:agentic-workflow/core/workflow",
            project_root,
        )
        .unwrap();
    ...
    fn test_read_main_spec_subpath() {
        ...
        // Write config.toml so agentic-workflow resolves to crates/
        let config_dir = project_root.join("cclab");
        std::fs::create_dir_all(&config_dir).unwrap();
        std::fs::write(
            config_dir.join("config.toml"),
