// SPEC-MANAGED: projects/agentic-workflow/tech-design/core/logic/spec_store.md#source
// CODEGEN-BEGIN
...
impl FileSystemSpecStore {
    ...
    fn resolve_group_dir(&self, group: &str) -> Option<PathBuf> {
        ...
        if group == "agentic-workflow" {
            let candidate = self.root.join("projects/agentic-workflow/tech-design");
            if candidate.exists() {
    ...
    fn candidate_files(&self) -> Vec<PathBuf> {
        ...
        let agentic_workflow_td = self.root.join("projects/agentic-workflow/tech-design");
...
mod tests {
    ...
    async fn test_read_returns_file_content() {
        ...
        let content = store
            .read("projects/agentic-workflow/core/state-machine.md")
            .await
            .unwrap();
...
"#,
        ...
        assert_eq!(
            results[0].path,
            "projects/agentic-workflow/logic/runtime.md"
        );
        let content = store
            .read("projects/agentic-workflow/logic/runtime.md")
            .await
            .unwrap();
    ...
    fn test_resolve_group_dir_config_hit() {
        let temp = tempfile::TempDir::new().unwrap();
        let root = temp.path().to_path_buf();
        let dir = root.join("projects/agentic-workflow/tech-design");
