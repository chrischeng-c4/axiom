// SPEC-MANAGED: projects/agentic-workflow/tech-design/surface/interfaces/src/validate_spec_structure.md#source
// CODEGEN-BEGIN
...
pub fn run_all() -> Result<()> {
    let project_root = crate::find_project_root()?;
    let target = crate::shared::workspace::tech_design_path(&project_root);

    let shape = crate::validate::PathShape::Prefix(target.clone());
    let files = match crate::validate::resolve_spec_files(&shape) {
        Ok(f) => f,
        Err(e) => {
...
fn discover_spec_roots(project_root: &Path) -> Vec<PathBuf> {
    let mut roots: Vec<PathBuf> = crate::shared::workspace::project_tech_design_paths(project_root)
        .into_iter()
        .map(|(_, path)| path)
    ...
    if roots.is_empty() {
        let specs_dir = crate::shared::workspace::tech_design_path(project_root).join("crates");
