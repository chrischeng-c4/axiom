    ...
    for line in content.lines() {
        ...
        if let Some(rest) = trimmed.strip_prefix("**Pillar — ") {
            ...
            if let Some(inner) = trimmed.strip_prefix('|') {
                ...
                if !first.is_empty()
                    && !first.eq_ignore_ascii_case("Capability")
                    && !is_separator
                {
...
pub fn upsert_capability_type(
    ...
    let table = doc
        ...
        .with_context(|| {
            format!(
                "`capability_types` in {} is not a table",
                path.display()
            )
        })?;
    table.insert(
...
mod tests {
    ...
    fn dims(capability_type: CapabilityType) -> BTreeSet<&'static str> {
        required_ec_dimensions(&capability_type).iter().copied().collect()
    }
    ...
    fn required_ec_dimensions_agent_first_is_behavior_only() {
        assert_eq!(dims(CapabilityType::AgentFirst), BTreeSet::from(["behavior"]));
    }
