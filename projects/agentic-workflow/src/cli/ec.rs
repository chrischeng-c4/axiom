...
fn run_draft(project: &str, args: EcDraftArgs) -> Result<()> {
    ...
    if args.json {
        ...
        println!("next: aw ec fill --project {} {} --section e2e-test --body-file <file>", ctx.project, rel);
...
fn run_fill(project: &str, args: EcFillArgs) -> Result<()> {
    ...
    if args.json {
        ...
        println!("ec fill {}: filled {} in {}", ctx.project, args.section, rel);
...
"#,
    ...
    fn ec_draft_fill_markdown_drives_manifest() {
        ...
        let draft = render_ec_draft(&ctx, "search-indexing", "efficiency", "Search Indexing", &args);
