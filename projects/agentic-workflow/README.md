...
EC Dimensions:
- behavior: `cargo test -p agentic-workflow --lib markdown_capability_tables` - Markdown README contract parsing, migration, and readiness reporting.
Root WI: -
Status: verified
Required Verification: smoke
Promise:
...
EC Dimensions:
- behavior: `./target/debug/aw view --snapshot` - repo catalog, terminal status, focused README detail, capability list, EC inventory, TD summary, and renderer-neutral surface snapshot are present.
- behavior: `./target/debug/aw view --check` - headless contract check contains the terminal pane, repo catalog, semantic layout toggle, and selected README, capability, EC, and TD detail panes.
...
- behavior: `./target/debug/aw view --layout top-bottom --screenshot /private/tmp/aw-view-app-top-bottom.png` - project list stays fixed while the terminal/detail region can switch to top-bottom layout with a visible toggle control.
...
Required Verification: smoke
Promise:
AW presents the repository as a visual-reader model: projects/libs navigation is the root, selected repo items expose README capability detail, EC contract status, and TD relation summary, and the desktop artifact has stable semantic surface IDs so tests do not depend on a toolkit-private tree. The project list stays fixed on the left; the right-side workspace can place terminal/status and current EC/capability detail either left-right or top-bottom, defaulting to left-right. A native toggle button switches that workspace layout in the running desktop app.
Gate Inventory:
