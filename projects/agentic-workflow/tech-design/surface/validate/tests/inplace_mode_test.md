...
# Standardized projects/agentic-workflow/tests/cli/tests/inplace_mode_test.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `projects/agentic-workflow/tests/cli/tests/inplace_mode_test.rs` generated from AST during Score force-regeneration standardization.
...
<!-- source-snapshot: path=projects/agentic-workflow/tests/cli/tests/inplace_mode_test.rs -->
````rust
...
//!
//! These tests require the `score` binary; cargo wires `CARGO_BIN_EXE_score`
...
fn skip_unless_ready() -> Option<(std::path::PathBuf, String)> {
    ...
    let bin = std::env::var("CARGO_BIN_EXE_score").ok().or_else(|| {
        let exe = std::env::current_exe().ok()?;
        let debug_dir = exe.parent()?.parent()?;
        let bin = debug_dir.join(format!("score{}", std::env::consts::EXE_SUFFIX));
...
fn write_td_review_fixture(root: &std::path::Path, slug: &str, spec_rel: &str) {
    ...
    std::fs::create_dir_all(root.join("projects/agentic-workflow/tech-design/surface/specs")).unwrap();
    std::fs::write(root.join(".aw/config.toml"), "").unwrap();
    std::fs::write(
        root.join(format!(".aw/issues/open/{}.md", slug)),
        format!(
            "---\n\
             type: bug\n\
...
fn inplace_td_init_switches_branch_no_worktree_dir() {
    let Some((git, bin)) = skip_unless_ready() else {
        eprintln!("skipping: git or CARGO_BIN_EXE_score missing");
        return;
    };
...
"#,
    ...
    let updated_issue =
        std::fs::read_to_string(root.join(format!(".aw/issues/open/{}.md", slug))).unwrap();
    assert!(
...
fn td_create_numeric_id_uses_tracker_id_branch_with_legacy_cache_file() {
    ...
    std::fs::write(
        root.join(format!(".aw/issues/open/{}.md", legacy_slug)),
        issue_body,
    )
    .unwrap();
    Command::new(&git)
    ...
    assert!(
        !root.join(".aw/issues/open/1887.md").exists(),
        "td create should update the existing cache file instead of inventing a second one"
    );
    let updated =
        std::fs::read_to_string(root.join(format!(".aw/issues/open/{}.md", legacy_slug)))
            .unwrap();
...
#[test]
fn td_review_apply_accepts_dirty_spec_on_td_branch() {
    let Some((git, bin)) = skip_unless_ready() else {
        eprintln!("skipping: git or CARGO_BIN_EXE_score missing");
        return;
    };
    ...
    let slug = "review-dirty";
    let spec_rel = "projects/agentic-workflow/tech-design/surface/specs/review-dirty.md";
    write_td_review_fixture(root, slug, spec_rel);
    Command::new(&git)
        .arg("-C")
        .arg(root)
        .args(["add", "."])
    ...
    Command::new(&git)
        .arg("-C")
        .arg(root)
        .args(["commit", "-m", "bootstrap td review"])
        .status()
        .unwrap();
    Command::new(&git)
        .arg("-C")
        .arg(root)
        .args(["switch", "-c", &format!("td-{}", slug)])
        .status()
        .unwrap();
    ...
    assert!(
        out.status.success(),
        "td review --apply should accept dirty spec path:\nstdout={}\nstderr={}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("aw td validate"),
        "review apply should dispatch validate:\n{}",
        stdout,
    );
    ...
    assert!(
        out.status.success(),
        "td validate should commit dirty review spec:\nstdout={}\nstderr={}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("aw td revise"),
        "needs-revision review should dispatch revise:\n{}",
        stdout,
    );
    assert_eq!(
        git_status(&git, root).trim(),
        "",
        "validate must commit the dirty spec and issue update",
    );
}

#[test]
fn td_review_apply_rejects_unrelated_dirty_file() {
    let Some((git, bin)) = skip_unless_ready() else {
        eprintln!("skipping: git or CARGO_BIN_EXE_score missing");
        return;
    };
    ...
    let slug = "review-dirty-extra";
    let spec_rel = "projects/agentic-workflow/tech-design/surface/specs/review-dirty-extra.md";
    write_td_review_fixture(root, slug, spec_rel);
    Command::new(&git)
        .arg("-C")
        .arg(root)
    ...
    assert!(
        !out.status.success(),
        "td review --apply should reject unrelated dirty files"
    );
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&out.stdout),
    ...
    assert!(
        ...
        "expected dirty-spec-only error, got:\n{}",
        combined,
    );
}
...
fn wi_validate_accepts_apply_dirty_issue_file_on_issue_branch() {
    ...
    let slug = "demo";
    let issue_path = root.join(format!(".aw/issues/open/{}.md", slug));
    std::fs::write(
        &issue_path,
        format!(
            "---\n\
    ...
    std::fs::write(
        ...
        format!(
            "---\n\
             ...
             ## Scope\n\n\
             ### In Scope\n\
             - Allow only the matching dirty issue file on the issue branch.\n\n\
             ### Out of Scope\n\
    ...
    assert!(
        out.status.success(),
        "wi validate should accept apply-produced dirty issue file:\nstdout={}\nstderr={}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("Validation passed for 'demo'."),
        "WI validate should use backend projection rather than issue-branch CRRR:\n{}",
        stdout,
    );
    assert!(
        git_status(&git, root).contains(".aw/issues/open/demo.md"),
        "WI validate must not commit or clean issue-branch working tree state",
    );
}
````
