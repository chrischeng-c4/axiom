...
changes:
  - path: projects/agentic-workflow/tests/cli/tests/init_doc_projection_test.rs
    ...
    description: |
      ...
      Issue #986 (init-projector slice 3/3) adds four more real-binary smoke
      tests covering the `.claude/skills` + `.agents/skills` dual-tree
      contract. `fresh_init_installs_aw_skills_into_both_claude_and_agents_trees`
      proves AC1/AC3: a fresh `aw init` installs every sampled `aw-*` skill
      (a no-divergence skill, a path-transform skill, a doc-ref-transform
      skill, and the new `aw-guard`) into BOTH trees, with the `.agents`
      body equal to `doc_mirror::agents_skill_body_from_claude_skill_body`
      applied to the `.claude` body, companion scripts installed identically
      into both trees, and `aw init --check` clean immediately after.
      `init_check_detects_claude_skill_tamper_and_init_restores_it` and
      `init_check_detects_agents_skill_tamper_and_init_restores_it` prove
      AC2 independently for each tree: hand-editing an installed
      `aw-wi/SKILL.md` in either `.claude/skills` or `.agents/skills` makes
      `aw init --check` fail and name the stale path without writing, and a
      follow-up write-mode `aw init` restores it (verified clean again via a
      second `--check`). `init_prunes_deprecated_skill_from_agents_tree`
