---
id: projects-score-tests-merge-target-branch-rs
...
# Standardized projects/agentic-workflow/tests/cli/tests/merge_target_branch.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `projects/agentic-workflow/tests/cli/tests/merge_target_branch.rs` generated from AST during Score force-regeneration standardization.
...
<!-- source-snapshot: path=projects/agentic-workflow/tests/cli/tests/merge_target_branch.rs -->
```rust
...
fn case_a_feature_branch_detected() {
    ...
    let result = agentic_workflow::cli::merge_target::resolve_merge_target(None, None, tmp.path()).unwrap();
    assert_eq!(
        result, "feature-abc",
...
fn case_detached_head_no_config_returns_error() {
    ...
    let err = agentic_workflow::cli::merge_target::resolve_merge_target(None, None, tmp.path()).unwrap_err();
    assert!(
