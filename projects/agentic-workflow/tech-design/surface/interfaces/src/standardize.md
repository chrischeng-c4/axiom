---
id: projects-score-src-standardize-rs
...
# Standardized projects/agentic-workflow/src/cli/standardize.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `projects/agentic-workflow/src/cli/standardize.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `CommandTraceabilityCoverage` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 281 |  |
| `CoverageMapEntry` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 532 |  |
| `DependencyPolicyFinding` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 419 |  |
| `DeploymentFacetFinding` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 428 |  |
| `FrontendEcosystemAst` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 488 |  |
| `FrontendSourceNode` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 504 |  |
| `FrontendWorkspaceNode` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 495 |  |
| `GeneratorPrimitiveGap` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 541 |  |
| `ImportEdge` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 524 |  |
| `MarkerCounts` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 181 |  |
| `RegenerabilityCoverage` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 188 |  |
| `SemanticCoverage` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 216 |  |
| `SemanticGap` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 550 |  |
| `SourceEvidenceDomain` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 462 |  |
| `SourceEvidenceGraph` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 455 |  |
| `SourceEvidenceNode` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 471 |  |
| `SourceSymbol` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 516 |  |
| `SourceUnit` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 440 |  |
| `StackMigrationCoverage` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 380 |  |
| `StandardizationCoverage` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 169 |  |
| `StandardizeAction` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 559 |  |
| `StandardizeActionKind` | projects/agentic-workflow/src/cli/standardize.rs | enum | pub | 572 |  |
| `StandardizeArgs` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 88 |  |
| `StandardizeAuditArgs` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 105 |  |
| `StandardizeAuditCheckArgs` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 127 |  |
| `StandardizeAuditCommand` | projects/agentic-workflow/src/cli/standardize.rs | enum | pub | 112 |  |
| `StandardizeAuditRecordArgs` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 152 |  |
| `StandardizeCommand` | projects/agentic-workflow/src/cli/standardize.rs | enum | pub | 98 |  |
| `TraceabilityBlocker` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 312 |  |
| `TraceabilityBlockerKind` | projects/agentic-workflow/src/cli/standardize.rs | enum | pub | 323 |  |
| `TraceabilityCoverage` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 240 |  |
| `WorkspaceStackMigration` | projects/agentic-workflow/src/cli/standardize.rs | struct | pub | 401 |  |
| `as_str` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 346 | as_str(self) -> &'static str |
| `project_managed_coverage` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 1098 | project_managed_coverage(project: &str) -> Result<StandardizationCoverage> |
| `project_regenerability_coverage` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 1175 | project_regenerability_coverage(project: &str) -> Result<RegenerabilityCoverage> |
| `project_regenerability_fast_coverage` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 1183 | project_regenerability_fast_coverage(project: &str) -> Result<RegenerabilityCoverage> |
| `project_semantic_coverage` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 1206 | project_semantic_coverage(project: &str) -> Result<SemanticCoverage> |
| `project_stack_migration_coverage` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 1236 | project_stack_migration_coverage(project: &str) -> Result<StackMigrationCoverage> |
| `project_traceability_coverage` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 1215 | project_traceability_coverage(project: &str) -> Result<TraceabilityCoverage> |
| `ready_fixture` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 259 | ready_fixture(project: &str) -> Self |
| `ready_fixture` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 295 | ready_fixture() -> Self |
| `run` | projects/agentic-workflow/src/cli/standardize.rs | function | pub | 707 | run(args: StandardizeArgs) -> Result<()> |

<!-- #920 (epic #914 slice F): symbol manifest resynced against the current
     `src/cli/standardize.rs` line numbers and public surface. Removed
     entries for symbols deleted with the retired
     `managed`/`semantic`/`traceability` layer-driver plumbing
     (`StandardizeStageArgs`, `StandardizeStageCommand`,
     `StandardizeReportArgs`, `StandardizeNextArgs`, `StandardizeRunArgs`,
     `StandardizeTraceabilityArgs`, `StandardizeTraceabilityCommand`,
     `StandardizeTraceabilityReportArgs`, `StandardizeTraceabilityRunArgs`,
     `CodegenCoverage`) and for `ProjectHealthStandardizeCoverage` /
     `project_health_standardize_coverage`, which are now `pub(crate)` (only
     `aw health`/`src/cli/project.rs` call them) rather than fully `pub`. The
     three new worker-command helpers
     (`managed_health_worker_command`/`semantic_health_worker_command`/
     `traceability_health_worker_command`) are likewise `pub(crate)` and so
     are intentionally not listed in this `pub`-only manifest. -->
## Source
<!-- type: source lang: rust -->
<!-- source-from-target: strip-handwrite -->

<!-- source-snapshot: path=projects/agentic-workflow/src/cli/standardize.rs -->
`````rust
...
changes:
  ...
  - path: projects/agentic-workflow/src/cli/standardize.rs
    ...
    section: source
    description: |
      Standardize next/run summaries expose the aw.cli.v1 agent contract with
      schema_version, status, completion, and canonical next fields while
      retaining next_action for compatibility.
  - path: projects/agentic-workflow/src/cli/standardize.rs
    action: modify
    impl_mode: codegen
    section: source
    description: |
      #920 (epic #914 slice F): `aw standardize` is retired down to `audit`
      only. `StandardizeCommand` drops the `Managed`/`Semantic`/`Traceability`
      variants and their `report`/`next`/`run` stage plumbing
      (`StandardizeStageArgs`/`StandardizeStageCommand`/
      `StandardizeReportArgs`/`StandardizeNextArgs`/`StandardizeRunArgs`/
      `StandardizeTraceabilityArgs` and their dispatch arms, envelope
      builders, and print/emit helpers); `StandardizeArgs::command` is now a
      required `StandardizeCommand` (not `Option`), so a bare `aw standardize
      --project <p>` fails to parse instead of falling through to a removed
      parent-workflow shorthand -- `aw health --project <p>` is the read-only
      successor. `DELETED_COMMAND_PATHS` gained `"aw standardize managed"`,
      `"aw standardize semantic"`, and `"aw standardize traceability"` so
      `aw standardize audit`/bare `aw standardize` do not false-positive
      against the deleted-path scan.

      The underlying inventory/coverage library code (`build_inventory`,
      `choose_action`, `choose_semantic_action_with_project`,
      `choose_traceability_action`, `Inventory`, `StandardizationCoverage`,
      `SemanticCoverage`, `TraceabilityCoverage`,
      `TouchedScopeStandardization`, `active_doc_command_blockers`,
      `DELETED_COMMAND_PATHS` scanning, and
      `project_health_standardize_coverage`) is retained -- `aw health`
      (`src/cli/project.rs`) and the #932 gate consume it directly, so only
      the CLI verb surface (parse/dispatch/print/envelope/execute for the
      three removed layer drivers) is deleted. `choose_action` and its sibling
      selectors are now exercised only by this file's own unit tests (no
      remaining production caller); their internal `.command` string literals
      (which still name the retired `aw standardize managed|semantic|
      traceability run ...` shape) are unchanged in this pass since they have
      no live caller to break.

      Three new `pub(crate)` functions --
      `managed_health_worker_command`/`semantic_health_worker_command`/
      `traceability_health_worker_command` -- let
      `project_health_next_command` (`src/cli/project.rs`) name the concrete
      slice-E worker verb for the project's top-priority gap directly instead
      of looping back through a removed layer-run driver:
      `managed_health_worker_command` emits `aw td code-claim <path>` when a
      managed-layer uncovered file is known (the only managed gap kind
      mechanically derivable from `ProjectHealthReport`'s existing summary
      data without adding a live filesystem inventory scan to a pure,
      already-covered report-summary function), and falls back -- like the
      semantic and traceability helpers -- to a read-only `aw health
      --project <p> metrics|traceability --verbose` pointer for gap kinds
