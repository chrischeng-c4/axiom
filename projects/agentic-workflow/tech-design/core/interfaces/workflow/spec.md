---
id: projects-sdd-src-workflow-spec-rs
...
# Standardized projects/agentic-workflow/src/workflow/spec.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `projects/agentic-workflow/src/workflow/spec.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `handle` | projects/agentic-workflow/src/workflow/spec.rs | function | pub | 16 | handle(change_dir: &Path, change_id: &str, project_path: &str) -> Result<Value> |
## Source
<!-- type: source lang: rust -->
<!-- source-from-target: strip-handwrite -->

<!-- source-snapshot: path=projects/agentic-workflow/src/workflow/spec.rs -->
````rust
                    ...
                    if let Some(spec_id) = &pending_review_spec {
                        ...
                        if sm.revision_count(&key) >= 1 {
                            if let Some(spec) = missing_specs.first() {
                                ...
                                return super::tasks::handle(
                                    change_dir, change_id, project_path,
                                );
                            }
                        } else {
                            Action::ReviseSpec {
                    ...
                    if let Some(spec_id) = &pending_review_spec {
                        ...
                        if sm.revision_count(&key) >= 1 {
                            return Ok(helpers::mainthread_must_fix(
                                change_id, &phase, &format!("change_spec:{}", spec_id),
...
#[derive(Debug)]
enum Action {
    CreateSpec { spec_id: String, depends: Vec<String> },
...
```yaml
changes:
  - path: projects/agentic-workflow/src/workflow/spec.rs
    action: modify
    section: source
    impl_mode: codegen
    description: |
      Source template owns the complete spec workflow router.
```
