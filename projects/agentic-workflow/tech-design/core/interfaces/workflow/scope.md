...
Public API manifest for `projects/agentic-workflow/src/workflow/scope.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `ScopeInfo` | projects/agentic-workflow/src/workflow/scope.rs | struct | pub | 17 |  |
| `build_spec_dir_tree` | projects/agentic-workflow/src/workflow/scope.rs | function | pub | 265 | build_spec_dir_tree(     spec_groups: &[String],     project_root: &Path,     config: Option<&SddConfig>, ) -> String |
| `extract_crate_labels_from_content` | projects/agentic-workflow/src/workflow/scope.rs | function | pub | 494 | extract_crate_labels_from_content(content: &str) -> Option<Vec<String>> |
| `extract_scope` | projects/agentic-workflow/src/workflow/scope.rs | function | pub | 33 | extract_scope(change_dir: &Path) -> ScopeInfo |
| `pre_filter_specs` | projects/agentic-workflow/src/workflow/scope.rs | function | pub | 206 | pre_filter_specs(     spec_groups: &[String],     project_root: &Path,     config: Option<&SddConfig>, ) -> String |
| `resolve_spec_dir` | projects/agentic-workflow/src/workflow/scope.rs | function | pub | 137 | resolve_spec_dir(     group: &str,     specs_base: &Path,     scopes: &HashMap<String, String>, ) -> Option<PathBuf> |
| `resolve_spec_dir_for_root` | projects/agentic-workflow/src/workflow/scope.rs | function | pub | 171 | resolve_spec_dir_for_root(     group: &str,     project_root: &Path,     specs_base: &Path,     scopes: &HashMap<String, String>, ) -> Option<PathBuf> |
| `suggest_topics` | projects/agentic-workflow/src/workflow/scope.rs | function | pub | 369 | suggest_topics(description: &str) -> Vec<&'static str> |
## Schema
...
definitions:
  ScopeInfo:
    type: object
    required: [affected_crates, spec_groups, affected_paths, keywords, is_unknown, source]
    description: Scope information extracted from issue labels and clarifications.
    properties:
      affected_crates:
        type: array
        items: { type: string }
        x-rust-type: "Vec<String>"
        description: "Affected crate names."
      spec_groups:
        type: array
        items: { type: string }
        x-rust-type: "Vec<String>"
        description: "Spec groups derived from crate names."
      affected_paths:
        type: array
        items: { type: string }
        x-rust-type: "Vec<String>"
        description: "Specific paths mentioned in clarifications."
      keywords:
        type: array
        items: { type: string }
        x-rust-type: "Vec<String>"
        description: "Keywords extracted from scope answers."
      is_unknown:
        type: boolean
        description: "True when user answered unknown or no scope info available."
      source:
        type: string
        description: "Source of scope info."
    x-rust-struct:
...
<!-- source-snapshot: path=projects/agentic-workflow/src/workflow/scope.rs -->
```rust
...
mod tests {
    ...
    fn test_extract_scope_from_requirements_patterns() {
        ...
        let result = extract_scope_from_requirements("Modify projects/agentic-workflow/src/tools/agent.rs");
...
"#,
    ...
    fn test_pre_filter_specs_with_project_td_path() {
        let temp = TempDir::new().unwrap();
        let specs_dir = temp.path().join("projects/agentic-workflow/tech-design/logic");
        std::fs::create_dir_all(&specs_dir).unwrap();
        std::fs::write(specs_dir.join("state.md"), "# State\n").unwrap();
        std::fs::create_dir_all(temp.path().join(".aw")).unwrap();
        std::fs::write(
...
"#,
    ...
    fn test_pre_filter_specs_config_present_but_group_not_in_scopes() {
        ...
        let specs_dir = temp.path().join("projects/agentic-workflow/tech-design/core");
        std::fs::create_dir_all(&specs_dir).unwrap();
...
```yaml
changes:
  - path: projects/agentic-workflow/src/workflow/scope.rs
    action: modify
    section: source
    impl_mode: codegen
    description: |
      Source template owns the complete workflow scope module.
  - action: annotate
    section: schema
...
- [overview] Single struct, all Vec<String> + bool + String.
