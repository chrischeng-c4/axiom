...
Public API manifest for `projects/agentic-workflow/src/agents/crr.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `CRRCycle` | projects/agentic-workflow/src/agents/crr.rs | struct | pub | 97 |  |
| `CRRCycleBuilder` | projects/agentic-workflow/src/agents/crr.rs | struct | pub | 112 |  |
| `CRREvent` | projects/agentic-workflow/src/agents/crr.rs | enum | pub | 63 |  |
| `CRRResult` | projects/agentic-workflow/src/agents/crr.rs | struct | pub | 86 |  |
| `CRRVerdictType` | projects/agentic-workflow/src/agents/crr.rs | enum | pub | 54 |  |
| `build` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 345 | build(self) -> NovaResult<CRRCycle> |
| `creator` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 291 | creator(mut self, agent: A) -> Self |
| `creator_arc` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 297 | creator_arc(mut self, agent: Arc<dyn Agent>) -> Self |
| `max_revisions` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 329 | max_revisions(mut self, n: u32) -> Self |
| `new` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 156 | new() -> CRRCycleBuilder |
| `on_event` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 335 | on_event(mut self, handler: F) -> Self |
| `reviewer` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 303 | reviewer(mut self, reviewer: R) -> Self |
| `reviewer_arc` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 309 | reviewer_arc(mut self, reviewer: Arc<dyn Reviewer>) -> Self |
| `reviser` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 315 | reviser(mut self, agent: A) -> Self |
| `reviser_arc` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 321 | reviser_arc(mut self, agent: Arc<dyn Agent>) -> Self |
| `run` | projects/agentic-workflow/src/agents/crr.rs | function | pub | 167 | run(&self, input: &str) -> NovaResult<CRRResult> |
## Schema
...
definitions:
  CRRVerdictType:
    type: string
    enum: [Approved, NeedsRevision, Rejected]
    description: Compact discriminant carried in CRREvent::Reviewed.
    x-rust-enum:
      derive: [Debug, Clone, PartialEq, Eq]

  CRREvent:
    type: string
    enum: [Creating, Created, Reviewing, Reviewed, Revising, Revised, Completed]
    description: Events emitted during a CRRCycle run.
    x-rust-enum:
      derive: [Debug, Clone]
      variants:
        - { name: Creating, doc: "Creator agent is about to run." }
        - name: Created
          kind: struct
          doc: "Creator produced an artifact."
          fields:
            - { name: artifact, rust_type: String }
        - name: Reviewing
          kind: struct
          doc: "Reviewer is about to review the current artifact."
          fields:
            - { name: revision, rust_type: u32 }
        - name: Reviewed
          kind: struct
          doc: "Reviewer produced a verdict."
          fields:
            - { name: verdict_type, rust_type: CRRVerdictType }
        - name: Revising
          kind: struct
          doc: "Reviser is about to address issues from a revision."
          fields:
            - { name: revision, rust_type: u32 }
            - { name: issue_count, rust_type: usize }
        - name: Revised
          kind: struct
          doc: "Reviser produced a revised artifact."
          fields:
            - { name: artifact, rust_type: String }
        - name: Completed
          kind: struct
          doc: "Cycle completed successfully."
          fields:
            - { name: artifact, rust_type: String }
            - { name: revision_count, rust_type: u32 }

  CRRResult:
    type: object
    required: [verdict, artifact, revision_count]
    description: Output of a completed CRRCycle run.
    properties:
      verdict:
        type: object
      ...
      artifact:
        type: string
        description: "The final artifact string produced by the cycle."
      revision_count:
        type: integer
        x-rust-type: "u32"
        description: "Number of revisions performed."
    x-rust-struct:
      derive: [Debug]

  CRRCycle:
    type: object
    required: [creator, reviewer, reviser, max_revisions, event_handler]
    description: Generic Create-Review-Revise orchestrator.
    properties:
      creator:
        type: object
        x-rust-type: "Arc<dyn Agent>"
      ...
      event_handler:
        type: object
        x-rust-type: "Option<Arc<dyn Fn(CRREvent) + Send + Sync>>"
        x-rust-visibility: private
        description: "Optional event handler closure."
    x-rust-struct:
      derive: []
  ...
  CRRCycleBuilder:
    type: object
    required: [creator, reviewer, reviser, max_revisions, event_handler]
    description: Builder for CRRCycle.
    properties:
      creator:
        type: object
        x-rust-type: "Option<Arc<dyn Agent>>"
        x-rust-visibility: private
        description: "Optional creator agent."
      reviewer:
        type: object
      ...
      reviser:
        ...
        x-rust-type: "Option<Arc<dyn Agent>>"
        x-rust-visibility: private
        description: "Optional reviser agent."
      max_revisions:
        type: integer
        x-rust-type: "u32"
        x-rust-visibility: private
...
<!-- source-snapshot: path=projects/agentic-workflow/src/agents/crr.rs -->
````rust
...
```yaml
changes:
  - path: projects/agentic-workflow/src/agents/crr.rs
    action: modify
    section: source
    impl_mode: codegen
    description: |
      Source template owns the complete CRR module, including schema-derived
      types, verdict conversion, orchestration behavior, builder methods, and
      tests.
  - action: annotate
    section: schema
...
- [overview] 5 types: 2 enums (one with struct variants), 3 structs.
- [schema] CRREvent uses struct variants for payload; closure trait-object via x-rust-type Option<Arc<dyn Fn(...)>>; visibility private.
...
- [overview] Promotes the CRR behavior and tests into source-template ownership while preserving schema as the language-neutral type contract.
- [source] Uses `strip-managed-markers` to retain current Rust behavior and remove mixed CODEGEN/HANDWRITE boundaries.
