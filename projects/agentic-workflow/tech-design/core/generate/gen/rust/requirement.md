---
id: sdd-gen-rust-requirement-types
...
# Requirement Annotations Generator Types

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `projects/agentic-workflow/src/generate/gen/rust/requirement.rs` generated from AST during Score force-regeneration standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `ImplAtLocation` | projects/agentic-workflow/src/generate/gen/rust/requirement.rs | struct | pub | 20 |  |
| `RequirementAnnotation` | projects/agentic-workflow/src/generate/gen/rust/requirement.rs | struct | pub | 28 |  |
| `RequirementAnnotationOutput` | projects/agentic-workflow/src/generate/gen/rust/requirement.rs | struct | pub | 38 |  |
| `generate_requirement_annotations` | projects/agentic-workflow/src/generate/gen/rust/requirement.rs | function | pub | 106 | generate_requirement_annotations(     annotations: &[RequirementAnnotation],     lang: Lang, ) -> RequirementAnnotationOutput |
| `parse_requirement_annotations` | projects/agentic-workflow/src/generate/gen/rust/requirement.rs | function | pub | 48 | parse_requirement_annotations(     frontmatter: &Value,     spec_path: &str, ) -> Vec<RequirementAnnotation> |
## Schema
...
definitions:
  ImplAtLocation:
    type: object
    required: [file]
    description: A single impl_at location (file + optional symbol).
    properties:
      file: { type: string }
      symbol: { type: string }
    x-rust-struct:
  ...
  RequirementAnnotation:
    type: object
    required: [id, text, spec_path, impl_at]
    description: A single requirement with optional impl_at locations.
    properties:
      id: { type: string }
      text: { type: string }
      ...
      impl_at:
        type: array
        items: { $ref: "#/definitions/ImplAtLocation" }
    x-rust-struct:
      derive: [Debug, Clone]

  RequirementAnnotationOutput:
    type: object
    required: [injections, skipped, warnings]
    description: Output from the requirements annotation generator.
    properties:
      injections:
        type: object
        x-rust-type: "HashMap<String, Vec<String>>"
      skipped:
        type: array
        items: { type: string }
      warnings:
...
<!-- source-snapshot: path=projects/agentic-workflow/src/generate/gen/rust/requirement.rs -->
```rust
...
"#;
        ...
        let annotations = parse_requirement_annotations(&value, "spec.md");
...
changes:
  - path: projects/agentic-workflow/src/generate/gen/rust/requirement.rs
    ...
    section: source
    impl_mode: codegen
    description: Source template owns the complete requirements generator module.
  - action: annotate
    section: schema
