...
**Callers**: `/cclab:sdd:agent` skill (via CLI) and internally by workflow tools (via `run_agent()`). Agents cannot call `sdd_delegate_agent` — recursion blocked.
...
params:
  ...
  - name: prompt
    ...
    schema:
      type: string
      description: Inline prompt text. One of prompt or prompt_path is required.
  - name: prompt_path
    required: false
    schema:
      type: string
      description: Relative path to prompt file (e.g. .aw/changes/{id}/prompts/{action}.md). One of prompt or prompt_path is required.
result:
  name: result
  schema:
    type: object
...
files:
  ...
  - path: .aw/tech-design/crates/cclab-sdd/tools/utils/delegate-agent.md
    action: modify
    section: doc
    impl_mode: codegen
    desc: |
