...
// @claim vat-cloud-workflows-lean-build
...
// Contract: vat compiles without the emulator feature; the cloud-workflows emulator verb then errors cleanly, never a panic.
#[test]
#[ignore = "AW EC gate: run via `aw health --verify-ec` or `cargo test -- --ignored`"]
fn vat_cloud_workflows_lean_build() {
    let command = "cargo build -p vat --no-default-features";
    let id = "vat-cloud-workflows-lean-build";
