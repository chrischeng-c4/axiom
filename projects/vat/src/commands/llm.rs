...
- Built-in emulators: `preset = "pubsub"`, `"firebase-auth"`, `"cloud-tasks"`,
  and `"cloud-scheduler"` run vat's OWN in-process Rust emulator under
  `runtime = auto` — no gcloud, Java, firebase-tools, or Docker, and instant
  start. They export `PUBSUB_EMULATOR_HOST` / `FIREBASE_AUTH_EMULATOR_HOST` /
  ...
  `jobs/{j}:run`. `pubsub` still accepts `runtime = native` (gcloud) /
  `runtime = docker` (image) as a fidelity fallback; the others are built-in
  only (Cloud Tasks / Scheduler have no official emulator at all).
