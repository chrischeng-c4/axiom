#!/usr/bin/env sh
# <HANDWRITE gap="project-root-install-script" tracker="aw-arena" reason="project-specific repository installer dispatch contract">
# arena installer — downloads the right prebuilt binary from GitHub
# Releases and drops it on your PATH.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/chrischeng-c4/axiom/main/projects/arena/install.sh | sh
#
# Env overrides:
#   ARENA_VERSION   tag to install (default: latest arena@* release, e.g. arena@1.0)
#   ARENA_INSTALL   install dir (default: $HOME/.local/bin)
#   ARENA_REPO      gh repo (default: chrischeng-c4/axiom)
...
REPO="${ARENA_REPO:-chrischeng-c4/axiom}"
INSTALL_DIR="${ARENA_INSTALL:-$HOME/.local/bin}"
VERSION="${ARENA_VERSION:-latest}"
TOKEN="${GH_TOKEN:-${GITHUB_TOKEN:-}}"

say() { printf 'arena-install: %s\n' "$*" >&2; }
...
if [ "${VERSION}" = "latest" ]; then
  ...
  # API filter so we only pick `arena@*` tags (not mamba/jet/etc).
  api="https://api.github.com/repos/${REPO}/releases?per_page=30"
  tag="$(
    auth_curl_api "${api}" \
      | grep -E '"tag_name": "arena@[^"]+"' \
      | head -n 1 \
      | sed -E 's/.*"tag_name": "([^"]+)".*/\1/'
  )" || true
  if [ -z "${tag}" ]; then
    if [ -z "${TOKEN}" ]; then
      die "could not find a arena@* release in ${REPO} (repo may be private — export GH_TOKEN or \`gh auth login\`)"
    fi
    die "could not find a arena@* release in ${REPO}"
  fi
else
  tag="${VERSION}"
fi
...
tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t arena-install)"
...
bin="${tmpdir}/arena-${target}/arena"
[ -f "${bin}" ] || die "binary not found in archive: ${bin}"
chmod +x "${bin}"

mkdir -p "${INSTALL_DIR}"
mv "${bin}" "${INSTALL_DIR}/arena"
say "installed: ${INSTALL_DIR}/arena"
...
# ---- show version ------------------------------------------------------
if "${INSTALL_DIR}/arena" --version >/dev/null 2>&1; then
  ver="$("${INSTALL_DIR}/arena" --version 2>/dev/null || echo unknown)"
  say "ready: ${ver}"
fi
# </HANDWRITE>
