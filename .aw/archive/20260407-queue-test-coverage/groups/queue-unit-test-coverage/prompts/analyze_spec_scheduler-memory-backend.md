# Task: Analyze Spec 'scheduler-memory-backend' for Change 'queue-test-coverage'
...
2. Read the skeleton via `cclab sdd workflow read-artifact queue-test-coverage` with scope="scheduler-memory-backend"
