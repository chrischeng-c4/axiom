# Task: Review Implementation of Spec 'merge-3way' for Change 'sdd-merge-3way'

## Pre-Review Step (MANDATORY)

Before evaluating any checklist items:
1. Read spec: `cclab/changes/sdd-merge-3way/groups/merge-3way/specs/merge-3way.md`
2. Find the `## Test Plan` section (if present) and note whether it exists and how many test cases it defines.

## Instructions

3. Read implementation diff: `cclab/changes/sdd-merge-3way/implementation.md`
4. List changed files via `cclab sdd workflow list-changed-files sdd-merge-3way`
...
Read file: cclab/changes/sdd-merge-3way/implementation.md

# List changed files
cclab sdd workflow list-changed-files sdd-merge-3way

# Write review (write payload JSON first, then run)
cclab sdd artifact review-change-implementation sdd-merge-3way cclab/changes/sdd-merge-3way/payloads/review-change-implementation.json
```