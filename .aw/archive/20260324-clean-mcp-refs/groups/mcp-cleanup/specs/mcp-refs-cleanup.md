...
| `logic/implement-task.md` | `mcp/tools/change_impl/{common,create,review,revise}.rs` | `tools/{common_change_impl,create_change_impl,review_change_impl,revise_change_impl}.rs` |
...
| `logic/change-merge.md` | `mcp/tools/change_merge/create.rs` | `tools/create_change_merge.rs` |
...
The spec `generate/template-mcp-configs.md` documents MCP config generation for `.mcp.json`, `.gemini/settings.json`, `.codex/config.toml`. If `cclab-mcp` server is no longer used, this spec is fully obsolete. Archive by moving to `cclab/archive/` or adding `status: archived` frontmatter.
...
After all changes, no spec file under `cclab/specs/crates/cclab-sdd/` should contain text implying the MCP server is in active use. Validation: `grep -r 'MCP' cclab/specs/crates/cclab-sdd/` returns zero matches except in archived files or historical notes explicitly marked as deprecated.
## Scenarios
...
- **WHEN** the refactor is applied
- **THEN** the file is archived (moved to `cclab/archive/` or marked `status: archived`) and no longer appears in active spec listings

### Scenario: Post-refactor validation
- **GIVEN** all changes have been applied
- **WHEN** `grep -r 'MCP' cclab/specs/crates/cclab-sdd/` is run
...
| Spec File | Action |
|-----------|--------|
| `generate/template-mcp-configs.md` | Archive — move to `cclab/archive/crates/cclab-sdd/generate/template-mcp-configs.md` or add `status: archived` to frontmatter |
