# Task: Review Implementation of Spec 'e2e-test-infrastructure' for Change 'e2e-test-reorg'

## Pre-Review Step (MANDATORY)

Before evaluating any checklist items:
1. Read spec: `cclab/changes/e2e-test-reorg/groups/e2e-test-reorg/specs/e2e-test-infrastructure.md`
2. Find the `## Test Plan` section (if present) and note whether it exists and how many test cases it defines.

## Instructions

3. Read implementation diff: `cclab/changes/e2e-test-reorg/implementation.md`
4. List changed files via `cclab sdd workflow list-changed-files e2e-test-reorg`
...
Read file: cclab/changes/e2e-test-reorg/implementation.md

# List changed files
cclab sdd workflow list-changed-files e2e-test-reorg

# Write review (write payload JSON first, then run)
cclab sdd artifact review-change-implementation e2e-test-reorg cclab/changes/e2e-test-reorg/payloads/review-change-implementation.json
```