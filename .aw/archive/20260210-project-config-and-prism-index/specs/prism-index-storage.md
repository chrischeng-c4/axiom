...
type: spec
title: "Prism Index Storage & Resolution"
version: 1
spec_type: utility
...
# Prism Index Storage & Resolution

## Overview

A utility specification for resolving and managing the persistent storage location of Prism code indexes. It ensures that indexes are stored in a consistent, user-local directory structure (`~/.cclab/projects/{path_hash}/prism/`), derived from the canonical path of the project root. This enables index persistence across server restarts and supports multiple projects and monorepo modules without collision.
...
The system must resolve the persistent storage root for a project at `~/.cclab/projects/{path_hash}/prism/`, where `~` is the user's home directory.
...
- **THEN** A unique SHA256 hash is generated from the canonical path and the directory `~/.cclab/projects/{hash}/prism/` is returned.
...
- **THEN** The returned path includes the module's identifier (e.g., `.../prism/backend.idx`).
...
flowchart TB
    ...
    construct[Construct ~/.cclab/projects/{hash}/prism/]
