---
id: change-spec-section-optionality
type: spec
...
Modify the section selection rule engine in `change-spec.md` to support **conditional section optionality** based on inferred tech stack.

| Aspect | Detail |
|--------|--------|
| Target | Section Selection § Rule engine |
| Trigger | `tech_stack.design_system` detected via `tech-stack-inference` |
| Effect | `design-token` and `component` sections become **optional** (suggested, not required) |
| Scope | Rule engine output classification only — no new section types added |

### Current behavior

Keyword rules produce a flat list of suggested sections. All matched sections are treated equally — the spec_plan includes them and agent must fill all.

### New behavior

After keyword matching, an **optionality filter** checks `tech_stack.design_system`:

| Condition | `design-token` | `component` |
|-----------|---------------|-------------|
| No design system detected | required (if keyword-matched) | required (if keyword-matched) |
| Design system detected, `provides_tokens: true` | optional | required |
| Design system detected, `provides_components: true` | required | optional |
| Design system detected, both `true` | optional | optional |

Optional sections are included in `spec_plan.sections` with an `optional: true` annotation. The fill loop skips them unless the agent explicitly decides they are needed for the specific change.

### Constraint

- `overview` and `changes` are never optional (always-required rule)
- Optionality only applies to sections matched by keyword rules, not to always-required or conditional-count sections
- The `tech-stack-inference` spec (sibling in this group) defines the detection logic
...
```yaml
_sdd:
  id: section-optionality-changes
  refs:
    - $ref: "#section-optionality-filter"
    - $ref: "tech-stack-inference#tech-stack-detect"
changes:
  - path: crates/cclab-sdd/src/services/spec_service.rs
    action: modify
    description: "Add optionality filter after keyword rule matching in resolve_section_rules()"
  - path: crates/cclab-sdd/src/models/spec_rules.rs
    action: modify
    description: "Extend SectionEntry enum to support optional annotation (string | {type, optional})"
  - path: crates/cclab-sdd/src/tools/create_change_spec.rs
    action: modify
    description: "Update fill loop to handle optional sections — skip if agent decides not needed"
  - path: crates/cclab-sdd/src/tools/common_change_spec.rs
    action: modify
    description: "Update prune_todo_sections to remove skipped optional sections cleanly"
  - path: cclab/specs/crates/cclab-sdd/logic/change-spec.md
    action: modify
    description: "Update Section Selection and Section Fill Order sections with optionality rules"
```
