...
Remove all `merge_strategy` references from the spec — the field is dead code. Actual merge behavior is always: write to `cclab/specs/{main_spec_ref}` (create if absent, overwrite if exists). No variant is needed.
