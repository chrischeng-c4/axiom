# Task: Revise Reference Context for Group 'jet-install-optimizations' (Change 'all-jet-issues')

## Instructions

1. **Read artifact + review feedback**:
   `/Users/chris.cheng/cclab/cclab-jet/cclab/changes/all-jet-issues/groups/jet-install-optimizations/reference_context.md`
   Focus on the `# Reviews` section — list each issue to address.
2. **Read pre-clarifications** (confirm scope):
   `/Users/chris.cheng/cclab/cclab-jet/cclab/changes/all-jet-issues/groups/jet-install-optimizations/pre_clarifications.md`
...
cclab sdd artifact revise-reference-context all-jet-issues cclab/changes/all-jet-issues/payloads/revise-reference-context.json
```