# Task: Review Implementation of Spec 'xfail-zero' for Change 'mamba-xfail-zero'

## Pre-Review Step (MANDATORY)

Before evaluating any checklist items:
1. Read spec: `cclab/changes/mamba-xfail-zero/groups/xfail-zero/specs/xfail-zero.md`
2. Find the `## Test Plan` section (if present) and note whether it exists and how many test cases it defines.

## Instructions

3. Read implementation diff: `cclab/changes/mamba-xfail-zero/implementation.md`
4. List changed files via `cclab sdd workflow list-changed-files mamba-xfail-zero`
...
Read file: cclab/changes/mamba-xfail-zero/implementation.md

# List changed files
cclab sdd workflow list-changed-files mamba-xfail-zero

# Write review (write payload JSON first, then run)
cclab sdd artifact review-change-implementation mamba-xfail-zero cclab/changes/mamba-xfail-zero/payloads/review-change-implementation.json
```