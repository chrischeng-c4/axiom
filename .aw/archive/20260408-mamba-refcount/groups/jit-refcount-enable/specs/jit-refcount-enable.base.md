...
**Status**: JIT refcount infrastructure is committed (immortal refcount, `mb_retain_value`/`mb_release_value` wrappers, release-before-overwrite for all dest-writing instructions, Copy retain, return cleanup, container retain-on-store, cascading release-on-free). The ownership audit pass added `retain_if_ptr` calls to ~22 borrowed-reference runtime functions. **`EMIT_REFCOUNT_CALLS = false`** in `mod.rs` — enabling still causes SIGBUS in sequential tests.

**Known Issue (KI-refcount-closure)**: `mb_closure_release` in `runtime/closure.rs` is asymmetric — it removes the closure from the thread-local HashMap but does not release captured values. When sequential tests share runtime state, captured strings/lists that were retained via `mb_closure_get_capture` can be prematurely freed, causing use-after-free (SIGBUS in `test_closure_get_capture_owned_ref` when run after other tests).

**Re-enable checklist:**
1. Fix closure ownership symmetry: `mb_closure_release` must cascade-release captured values
2. Audit remaining borrowed-reference returns not caught by first pass
3. Run `cargo test -p mamba --test jit_refcount_audit_tests -- --test-threads=1` — must pass without SIGBUS
4. Run full conformance suite under ASan — zero heap-use-after-free reports
5. Flip `EMIT_REFCOUNT_CALLS` and `GcState.enabled` to `true` in one commit
