---
id: init-mcp-json
type: spec
title: "Generate .mcp.json with Project Header in cclab init"
version: 1
spec_type: utility
created_at: 2026-02-24T02:58:52.541177+00:00
updated_at: 2026-02-24T02:58:52.541177+00:00
codebase_paths:
  - crates/cclab-sdd/src/cli/init.rs
knowledge_refs:
  - cclab/knowledge/40-mcp/dynamic-config.md
requirements:
  total: 5
  ids:
    - R1
    - R2
    - R3
    - R4
    - R5
---

# Generate .mcp.json with Project Header in cclab init

## Overview

Update cclab init to generate or upgrade a .mcp.json file with X-Cclab-Project header pointing to the project root. Add .mcp.json to .gitignore since it contains local-specific paths. Track .mcp.json.example as a template for onboarding.

## Requirements

### R1 - Generate .mcp.json with header
During cclab init (fresh install), generate .mcp.json at project root with the cclab-mcp server entry including headers.X-Cclab-Project set to the current working directory.

### R2 - Add .mcp.json to .gitignore
Ensure .mcp.json is in .gitignore since it contains local-specific paths and ports.

### R3 - Track .mcp.json.example
Generate .mcp.json.example alongside .mcp.json as a tracked template without local-specific values, using placeholder paths.

### R4 - Update mode preservation
During cclab init update mode, preserve existing .mcp.json entries (other servers, custom settings). Only merge missing fields.

### R5 - Upgrade existing .mcp.json
When .mcp.json exists but cclab-mcp entry lacks X-Cclab-Project header, merge the header into the existing config without overwriting other settings.

## Acceptance Criteria

### Scenario: Fresh install generates .mcp.json
- **GIVEN** Project has no .mcp.json
- **WHEN** User runs cclab init
- **THEN** .mcp.json created with X-Cclab-Project header, .mcp.json added to .gitignore

### Scenario: Update merges header into existing
- **GIVEN** Project has existing .mcp.json with cclab-mcp but no X-Cclab-Project header
- **WHEN** User runs cclab init (update mode)
- **THEN** X-Cclab-Project header is added, other entries preserved

### Scenario: Update skips if header exists
- **GIVEN** Project has existing .mcp.json with X-Cclab-Project header already set
- **WHEN** User runs cclab init (update mode)
- **THEN** No changes made to .mcp.json

### Scenario: Example file tracked
- **GIVEN** Fresh install
- **WHEN** cclab init completes
- **THEN** .mcp.json.example exists with placeholder values, suitable for git tracking
