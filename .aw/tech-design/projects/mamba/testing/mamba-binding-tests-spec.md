...
Add integration test suites for Mamba Python binding crates and the `cclab-mamba-registry` foundation crate. Each binding crate exposes `mb_*` FFI functions callable from the Mamba JIT via the native-call ABI (`extern "C" fn(*const MbValue, usize) -> MbValue`). Currently each crate has 5–9 inline unit tests in `methods.rs` but no `tests/` integration test directory.

This change creates `crates/cclab-{name}-mamba/tests/methods_test.rs` for each remaining binding crate. The removed legacy API binding crate is superseded by `projects/mamba/mambalibs/httpkit/` and is not part of this test matrix.
...
|----|-------------|----------|
| R1 | Each remaining binding crate MUST have `crates/cclab-{name}-mamba/tests/methods_test.rs` | P0 |
...
| R9 | Total new integration tests: ≥ 110 (≥2 per function across the remaining binding functions) | P1 |
...
Coverage: R1, R2, R3, R4, R5, R6, R7, R8, R9, R10.
...
cargo test -p cclab-pg-mamba -p cclab-runtime-mamba \
           -p cclab-agent-mamba -p cclab-fetch-mamba \
           -p cclab-log-mamba -p cclab-mcp-mamba -p mamba-registry

# Count total tests (should be ≥ 110 new + existing inline)
