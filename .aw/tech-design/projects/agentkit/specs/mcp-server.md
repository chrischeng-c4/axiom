...
```mermaid
---
id: mcp_server_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — MCP server expose agentkit tools as MCP server" }
  build: { kind: process, label: "construct typed surface per #2064" }
...
```mermaid
---
id: mcp_server_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "MCP server expose agentkit tools as MCP server" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/mcp-server.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2064"

  - path: projects/agentkit/tools/src/mcp_server.rs
    action: create
    section: changes
    note: "Module stub for MCP server expose agentkit tools as MCP server — codegen marker block, implementation lands in this issue's PR"
```
