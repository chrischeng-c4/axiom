...
```mermaid
---
id: graph_multi_agent_subgraph_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Multi-agent subgraph composition" }
  build: { kind: process, label: "construct typed surface per #2043" }
...
```mermaid
---
id: graph_multi_agent_subgraph_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Multi-agent subgraph composition" }
...
```mermaid
---
id: graph_multi_agent_subgraph_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-multi-agent-subgraph.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2043"

  - path: projects/agentkit/graph/src/graph_multi_agent_subgraph.rs
    action: create
    section: changes
    note: "Module stub for Multi-agent subgraph composition — codegen marker block, implementation lands in this issue's PR"
```
