...
```mermaid
---
id: mcp_client_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — MCP client consume external MCP servers as tool sources" }
  build: { kind: process, label: "construct typed surface per #2063" }
...
```mermaid
---
id: mcp_client_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "MCP client consume external MCP servers as tool sources" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/mcp-client.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2063"

  - path: projects/agentkit/tools/src/mcp_client.rs
    action: create
    section: changes
    note: "Module stub for MCP client consume external MCP servers as tool sources — codegen marker block, implementation lands in this issue's PR"
```
