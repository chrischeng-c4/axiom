...
```mermaid
---
id: graph_event_streaming_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Graph event streaming state delta node events" }
  build: { kind: process, label: "construct typed surface per #2044" }
...
```mermaid
---
id: graph_event_streaming_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Graph event streaming state delta node events" }
...
```mermaid
---
id: graph_event_streaming_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-event-streaming.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2044"

  - path: projects/agentkit/graph/src/graph_event_streaming.rs
    action: create
    section: changes
    note: "Module stub for Graph event streaming state delta node events — codegen marker block, implementation lands in this issue's PR"
```
