...
```mermaid
---
id: vector_store_trait_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — VectorStore trait + in-memory + sqlite impls" }
  build: { kind: process, label: "construct typed surface per #2050" }
...
```mermaid
---
id: vector_store_trait_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "VectorStore trait + in-memory + sqlite impls" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/vector-store-trait.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2050"

  - path: projects/agentkit/llm/src/vector_store_trait.rs
    action: create
    section: changes
    note: "Module stub for VectorStore trait + in-memory + sqlite impls — codegen marker block, implementation lands in this issue's PR"
```
