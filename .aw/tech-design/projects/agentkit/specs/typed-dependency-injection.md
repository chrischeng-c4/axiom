...
```mermaid
---
id: typed_dependency_injection_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Typed dependency injection (deps_type equivalent)" }
  build: { kind: process, label: "construct typed surface per #2034" }
...
```mermaid
---
id: typed_dependency_injection_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Typed dependency injection (deps_type equivalent)" }
...
```mermaid
---
id: typed_dependency_injection_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/typed-dependency-injection.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2034"

  - path: projects/agentkit/core/src/typed_dependency_injection.rs
    action: create
    section: changes
    note: "Module stub for Typed dependency injection (deps_type equivalent) — codegen marker block, implementation lands in this issue's PR"
```
