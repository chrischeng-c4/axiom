...
```mermaid
---
id: frontend_skeleton_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — projects agentkit frontend skeleton jet wasm bundling" }
  build: { kind: process, label: "construct typed surface per #2051" }
...
```mermaid
---
id: frontend_skeleton_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "projects agentkit frontend skeleton jet wasm bundling" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/frontend-skeleton.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2051"

  - path: projects/agentkit/frontend/src/frontend_skeleton.rs
    action: create
    section: changes
    note: "Module stub for projects agentkit frontend skeleton jet wasm bundling — codegen marker block, implementation lands in this issue's PR"
```
