...
```mermaid
---
id: rate_limiting_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Rate limiting per-provider token-bucket queueing" }
  build: { kind: process, label: "construct typed surface per #2068" }
...
```mermaid
---
id: rate_limiting_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Rate limiting per-provider token-bucket queueing" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/rate-limiting.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2068"

  - path: projects/agentkit/llm/src/rate_limiting.rs
    action: create
    section: changes
    note: "Module stub for Rate limiting per-provider token-bucket queueing — codegen marker block, implementation lands in this issue's PR"
```
