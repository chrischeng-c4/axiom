...
```mermaid
---
id: embeddings_trait_providers_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Embeddings trait + 3 provider impls OpenAI Cohere local" }
  build: { kind: process, label: "construct typed surface per #2048" }
...
```mermaid
---
id: embeddings_trait_providers_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Embeddings trait + 3 provider impls OpenAI Cohere local" }
...
```mermaid
---
id: embeddings_trait_providers_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/embeddings-trait-providers.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2048"

  - path: projects/agentkit/llm/src/embeddings_trait_providers.rs
    action: create
    section: changes
    note: "Module stub for Embeddings trait + 3 provider impls OpenAI Cohere local — codegen marker block, implementation lands in this issue's PR"
```
