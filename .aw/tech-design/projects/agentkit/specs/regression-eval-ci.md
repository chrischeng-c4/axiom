...
```mermaid
---
id: regression_eval_ci_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Regression eval CI integration fail PR on score drop" }
  build: { kind: process, label: "construct typed surface per #2062" }
...
```mermaid
---
id: regression_eval_ci_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Regression eval CI integration fail PR on score drop" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/regression-eval-ci.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2062"

  - path: projects/agentkit/core/src/regression_eval_ci.rs
    action: create
    section: changes
    note: "Module stub for Regression eval CI integration fail PR on score drop — codegen marker block, implementation lands in this issue's PR"
```
