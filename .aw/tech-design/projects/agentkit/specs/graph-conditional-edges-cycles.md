---
id: graph-conditional-edges-cycles
...
```mermaid
---
id: graph_conditional_edges_cycles_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Conditional edges and cycles" }
  build: { kind: process, label: "construct typed surface per #2039" }
...
```mermaid
---
id: graph_conditional_edges_cycles_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Conditional edges and cycles" }
...
```mermaid
---
id: graph_conditional_edges_cycles_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-conditional-edges-cycles.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2039"

  - path: projects/agentkit/graph/src/graph_conditional_edges_cycles.rs
    action: create
    section: changes
    note: "Module stub for Conditional edges and cycles — codegen marker block, implementation lands in this issue's PR"
```
