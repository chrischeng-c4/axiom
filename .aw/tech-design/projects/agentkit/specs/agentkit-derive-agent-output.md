---
id: agentkit-derive-agent-output
...
```mermaid
---
id: agentkit_derive_agent_output_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — agentkit-derive derive AgentOutput proc macro" }
  build: { kind: process, label: "construct typed surface per #2032" }
...
```mermaid
---
id: agentkit_derive_agent_output_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "agentkit-derive derive AgentOutput proc macro" }
...
```mermaid
---
id: agentkit_derive_agent_output_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/agentkit-derive-agent-output.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2032"

  - path: projects/agentkit/derive/src/agentkit_derive_agent_output.rs
    action: create
    section: changes
    note: "Module stub for agentkit-derive derive AgentOutput proc macro — codegen marker block, implementation lands in this issue's PR"
```
