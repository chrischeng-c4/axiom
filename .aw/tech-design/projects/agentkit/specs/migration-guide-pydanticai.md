...
```mermaid
---
id: migration_guide_pydanticai_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Migration guide PydanticAI to agentkit" }
  build: { kind: process, label: "construct typed surface per #2073" }
...
```mermaid
---
id: migration_guide_pydanticai_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Migration guide PydanticAI to agentkit" }
...
```mermaid
---
id: migration_guide_pydanticai_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/migration-guide-pydanticai.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2073"

  - path: projects/agentkit/core/src/migration_guide_pydanticai.rs
    action: create
    section: changes
    note: "Module stub for Migration guide PydanticAI to agentkit — codegen marker block, implementation lands in this issue's PR"
```
