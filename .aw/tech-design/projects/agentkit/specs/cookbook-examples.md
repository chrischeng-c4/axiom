...
```mermaid
---
id: cookbook_examples_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Cookbook examples 10+ runnable agent recipes" }
  build: { kind: process, label: "construct typed surface per #2072" }
...
```mermaid
---
id: cookbook_examples_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Cookbook examples 10+ runnable agent recipes" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/cookbook-examples.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2072"

  - path: projects/agentkit/core/src/cookbook_examples.rs
    action: create
    section: changes
    note: "Module stub for Cookbook examples 10+ runnable agent recipes — codegen marker block, implementation lands in this issue's PR"
```
