...
```mermaid
---
id: cancellation_timeout_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Cancellation timeout cooperative through the run loop" }
  build: { kind: process, label: "construct typed surface per #2070" }
...
```mermaid
---
id: cancellation_timeout_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Cancellation timeout cooperative through the run loop" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/cancellation-timeout.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2070"

  - path: projects/agentkit/core/src/cancellation_timeout.rs
    action: create
    section: changes
    note: "Module stub for Cancellation timeout cooperative through the run loop — codegen marker block, implementation lands in this issue's PR"
```
