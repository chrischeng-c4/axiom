...
```mermaid
---
id: usage_cost_tracking_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Usage + cost tracking per provider" }
  build: { kind: process, label: "construct typed surface per #2036" }
...
```mermaid
---
id: usage_cost_tracking_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Usage + cost tracking per provider" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/usage-cost-tracking.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2036"

  - path: projects/agentkit/core/src/usage_cost_tracking.rs
    action: create
    section: changes
    note: "Module stub for Usage + cost tracking per provider — codegen marker block, implementation lands in this issue's PR"
```
