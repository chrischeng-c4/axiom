...
```mermaid
---
id: graph_parallel_execution_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Parallel node execution fan-out fan-in" }
  build: { kind: process, label: "construct typed surface per #2042" }
...
```mermaid
---
id: graph_parallel_execution_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Parallel node execution fan-out fan-in" }
...
```mermaid
---
id: graph_parallel_execution_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-parallel-execution.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2042"

  - path: projects/agentkit/graph/src/graph_parallel_execution.rs
    action: create
    section: changes
    note: "Module stub for Parallel node execution fan-out fan-in — codegen marker block, implementation lands in this issue's PR"
```
