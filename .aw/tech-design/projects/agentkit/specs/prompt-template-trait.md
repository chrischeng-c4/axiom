...
```mermaid
---
id: prompt_template_trait_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — PromptTemplate trait + Jinja-style impl" }
  build: { kind: process, label: "construct typed surface per #2045" }
...
```mermaid
---
id: prompt_template_trait_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "PromptTemplate trait + Jinja-style impl" }
...
```mermaid
---
id: prompt_template_trait_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/prompt-template-trait.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2045"

  - path: projects/agentkit/llm/src/prompt_template_trait.rs
    action: create
    section: changes
    note: "Module stub for PromptTemplate trait + Jinja-style impl — codegen marker block, implementation lands in this issue's PR"
```
