...
```mermaid
---
id: opentelemetry_tracing_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — OpenTelemetry tracing integration spans agent tool LLM" }
  build: { kind: process, label: "construct typed surface per #2057" }
...
```mermaid
---
id: opentelemetry_tracing_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "OpenTelemetry tracing integration spans agent tool LLM" }
...
```mermaid
---
id: opentelemetry_tracing_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/opentelemetry-tracing.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2057"

  - path: projects/agentkit/core/src/opentelemetry_tracing.rs
    action: create
    section: changes
    note: "Module stub for OpenTelemetry tracing integration spans agent tool LLM — codegen marker block, implementation lands in this issue's PR"
```
