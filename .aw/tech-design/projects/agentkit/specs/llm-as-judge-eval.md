...
```mermaid
---
id: llm_as_judge_eval_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — LLM-as-judge eval primitive rubric scoring" }
  build: { kind: process, label: "construct typed surface per #2061" }
...
```mermaid
---
id: llm_as_judge_eval_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "LLM-as-judge eval primitive rubric scoring" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/llm-as-judge-eval.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2061"

  - path: projects/agentkit/core/src/llm_as_judge_eval.rs
    action: create
    section: changes
    note: "Module stub for LLM-as-judge eval primitive rubric scoring — codegen marker block, implementation lands in this issue's PR"
```
