...
```mermaid
---
id: graph_node_edge_core_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Graph State Node Edge core linear path" }
  build: { kind: process, label: "construct typed surface per #2038" }
...
```mermaid
---
id: graph_node_edge_core_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Graph State Node Edge core linear path" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-node-edge-core.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2038"

  - path: projects/agentkit/graph/src/graph_node_edge_core.rs
    action: create
    section: changes
    note: "Module stub for Graph State Node Edge core linear path — codegen marker block, implementation lands in this issue's PR"
```
