...
```mermaid
---
id: structured_event_log_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Structured event log typed events streamed to subscribers" }
  build: { kind: process, label: "construct typed surface per #2058" }
...
```mermaid
---
id: structured_event_log_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Structured event log typed events streamed to subscribers" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/structured-event-log.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2058"

  - path: projects/agentkit/core/src/structured_event_log.rs
    action: create
    section: changes
    note: "Module stub for Structured event log typed events streamed to subscribers — codegen marker block, implementation lands in this issue's PR"
```
