...
```mermaid
---
id: agentkit_derive_agent_tool_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — agentkit-derive derive AgentTool proc macro" }
  build: { kind: process, label: "construct typed surface per #2031" }
...
```mermaid
---
id: agentkit_derive_agent_tool_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "agentkit-derive derive AgentTool proc macro" }
...
```mermaid
---
id: agentkit_derive_agent_tool_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/agentkit-derive-agent-tool.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2031"

  - path: projects/agentkit/derive/src/agentkit_derive_agent_tool.rs
    action: create
    section: changes
    note: "Module stub for agentkit-derive derive AgentTool proc macro — codegen marker block, implementation lands in this issue's PR"
```
