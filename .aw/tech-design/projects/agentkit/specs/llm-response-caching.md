...
```mermaid
---
id: llm_response_caching_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — LLM response caching provider-agnostic TTL key strategy" }
  build: { kind: process, label: "construct typed surface per #2067" }
...
```mermaid
---
id: llm_response_caching_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "LLM response caching provider-agnostic TTL key strategy" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/llm-response-caching.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2067"

  - path: projects/agentkit/llm/src/llm_response_caching.rs
    action: create
    section: changes
    note: "Module stub for LLM response caching provider-agnostic TTL key strategy — codegen marker block, implementation lands in this issue's PR"
```
