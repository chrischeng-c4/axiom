...
```mermaid
---
id: agent_run_visualizer_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Agent run visualizer single-agent trace" }
  build: { kind: process, label: "construct typed surface per #2052" }
...
```mermaid
---
id: agent_run_visualizer_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Agent run visualizer single-agent trace" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/agent-run-visualizer.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2052"

  - path: projects/agentkit/frontend/src/agent_run_visualizer.rs
    action: create
    section: changes
    note: "Module stub for Agent run visualizer single-agent trace — codegen marker block, implementation lands in this issue's PR"
```
