---
id: result-validator-retry-loop
...
```mermaid
---
id: result_validator_retry_loop_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Result validator + retry-with-feedback loop" }
  build: { kind: process, label: "construct typed surface per #2033" }
...
```mermaid
---
id: result_validator_retry_loop_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Result validator + retry-with-feedback loop" }
...
```mermaid
---
id: result_validator_retry_loop_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/result-validator-retry-loop.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2033"

  - path: projects/agentkit/core/src/result_validator_retry_loop.rs
    action: create
    section: changes
    note: "Module stub for Result validator + retry-with-feedback loop — codegen marker block, implementation lands in this issue's PR"
```
