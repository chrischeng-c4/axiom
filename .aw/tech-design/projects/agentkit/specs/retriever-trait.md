...
```mermaid
---
id: retriever_trait_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Retriever trait + interface" }
  build: { kind: process, label: "construct typed surface per #2047" }
...
```mermaid
---
id: retriever_trait_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Retriever trait + interface" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/retriever-trait.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2047"

  - path: projects/agentkit/llm/src/retriever_trait.rs
    action: create
    section: changes
    note: "Module stub for Retriever trait + interface — codegen marker block, implementation lands in this issue's PR"
```
