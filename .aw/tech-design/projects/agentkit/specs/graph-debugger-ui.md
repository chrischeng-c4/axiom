...
```mermaid
---
id: graph_debugger_ui_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Graph debugger UI state inspector edge traversal checkpoint timeline" }
  build: { kind: process, label: "construct typed surface per #2053" }
...
```mermaid
---
id: graph_debugger_ui_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Graph debugger UI state inspector edge traversal checkpoint timeline" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-debugger-ui.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2053"

  - path: projects/agentkit/frontend/src/graph_debugger_ui.rs
    action: create
    section: changes
    note: "Module stub for Graph debugger UI state inspector edge traversal checkpoint timeline — codegen marker block, implementation lands in this issue's PR"
```
