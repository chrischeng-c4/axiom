---
id: logfire-local-observability-ui
...
```mermaid
---
id: logfire_local_observability_ui_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Logfire-equivalent local observability UI jet-based" }
  build: { kind: process, label: "construct typed surface per #2059" }
...
```mermaid
---
id: logfire_local_observability_ui_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Logfire-equivalent local observability UI jet-based" }
...
```mermaid
---
id: logfire_local_observability_ui_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/logfire-local-observability-ui.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2059"

  - path: projects/agentkit/frontend/src/logfire_local_observability_ui.rs
    action: create
    section: changes
    note: "Module stub for Logfire-equivalent local observability UI jet-based — codegen marker block, implementation lands in this issue's PR"
```
