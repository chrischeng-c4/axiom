...
```mermaid
---
id: memory_abstractions_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Memory abstractions ConversationMemory SummaryMemory VectorMemory" }
  build: { kind: process, label: "construct typed surface per #2065" }
...
```mermaid
---
id: memory_abstractions_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Memory abstractions ConversationMemory SummaryMemory VectorMemory" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/memory-abstractions.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2065"

  - path: projects/agentkit/core/src/memory_abstractions.rs
    action: create
    section: changes
    note: "Module stub for Memory abstractions ConversationMemory SummaryMemory VectorMemory — codegen marker block, implementation lands in this issue's PR"
```
