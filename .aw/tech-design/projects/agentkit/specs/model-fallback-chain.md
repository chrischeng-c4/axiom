...
```mermaid
---
id: model_fallback_chain_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Model fallback chain provider A fails to B to C" }
  build: { kind: process, label: "construct typed surface per #2069" }
...
```mermaid
---
id: model_fallback_chain_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Model fallback chain provider A fails to B to C" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/model-fallback-chain.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2069"

  - path: projects/agentkit/llm/src/model_fallback_chain.rs
    action: create
    section: changes
    note: "Module stub for Model fallback chain provider A fails to B to C — codegen marker block, implementation lands in this issue's PR"
```
