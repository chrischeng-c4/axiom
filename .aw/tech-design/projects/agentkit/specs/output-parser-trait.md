...
```mermaid
---
id: output_parser_trait_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — OutputParser trait JSON regex typed" }
  build: { kind: process, label: "construct typed surface per #2046" }
...
```mermaid
---
id: output_parser_trait_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "OutputParser trait JSON regex typed" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/output-parser-trait.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2046"

  - path: projects/agentkit/llm/src/output_parser_trait.rs
    action: create
    section: changes
    note: "Module stub for OutputParser trait JSON regex typed — codegen marker block, implementation lands in this issue's PR"
```
