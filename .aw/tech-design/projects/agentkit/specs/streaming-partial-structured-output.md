---
id: streaming-partial-structured-output
...
```mermaid
---
id: streaming_partial_structured_output_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Streaming with partial structured output" }
  build: { kind: process, label: "construct typed surface per #2035" }
...
```mermaid
---
id: streaming_partial_structured_output_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Streaming with partial structured output" }
...
```mermaid
---
id: streaming_partial_structured_output_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/streaming-partial-structured-output.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2035"

  - path: projects/agentkit/core/src/streaming_partial_structured_output.rs
    action: create
    section: changes
    note: "Module stub for Streaming with partial structured output — codegen marker block, implementation lands in this issue's PR"
```
