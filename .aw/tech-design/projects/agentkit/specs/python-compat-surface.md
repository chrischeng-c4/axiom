...
```mermaid
---
id: python_compat_surface_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Python-compat surface PydanticAI LangGraph signature mirroring" }
  build: { kind: process, label: "construct typed surface per #2056" }
...
```mermaid
---
id: python_compat_surface_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Python-compat surface PydanticAI LangGraph signature mirroring" }
...
```mermaid
---
id: python_compat_surface_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/python-compat-surface.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2056"

  - path: projects/agentkit/mamba-binding/src/python_compat_surface.rs
    action: create
    section: changes
    note: "Module stub for Python-compat surface PydanticAI LangGraph signature mirroring — codegen marker block, implementation lands in this issue's PR"
```
