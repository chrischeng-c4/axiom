...
```mermaid
---
id: graph_interrupt_hitl_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Interrupt human-in-the-loop resumption" }
  build: { kind: process, label: "construct typed surface per #2041" }
...
```mermaid
---
id: graph_interrupt_hitl_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Interrupt human-in-the-loop resumption" }
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-interrupt-hitl.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2041"

  - path: projects/agentkit/graph/src/graph_interrupt_hitl.rs
    action: create
    section: changes
    note: "Module stub for Interrupt human-in-the-loop resumption — codegen marker block, implementation lands in this issue's PR"
```
