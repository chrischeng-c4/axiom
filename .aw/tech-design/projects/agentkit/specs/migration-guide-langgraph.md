...
```mermaid
---
id: migration_guide_langgraph_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Migration guide LangGraph to agentkit" }
  build: { kind: process, label: "construct typed surface per #2074" }
...
```mermaid
---
id: migration_guide_langgraph_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Migration guide LangGraph to agentkit" }
...
```mermaid
---
id: migration_guide_langgraph_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/migration-guide-langgraph.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2074"

  - path: projects/agentkit/core/src/migration_guide_langgraph.rs
    action: create
    section: changes
    note: "Module stub for Migration guide LangGraph to agentkit — codegen marker block, implementation lands in this issue's PR"
```
