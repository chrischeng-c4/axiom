...
```mermaid
---
id: expose_graph_api_mamba_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Expose Graph API via mamba binding" }
  build: { kind: process, label: "construct typed surface per #2055" }
...
```mermaid
---
id: expose_graph_api_mamba_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Expose Graph API via mamba binding" }
...
```mermaid
---
id: expose_graph_api_mamba_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/expose-graph-api-mamba.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2055"

  - path: projects/agentkit/mamba-binding/src/expose_graph_api_mamba.rs
    action: create
    section: changes
    note: "Module stub for Expose Graph API via mamba binding — codegen marker block, implementation lands in this issue's PR"
```
