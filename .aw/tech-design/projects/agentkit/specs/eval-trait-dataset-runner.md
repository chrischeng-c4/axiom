...
```mermaid
---
id: eval_trait_dataset_runner_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Eval trait + dataset runner deterministic agent testing" }
  build: { kind: process, label: "construct typed surface per #2060" }
...
```mermaid
---
id: eval_trait_dataset_runner_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Eval trait + dataset runner deterministic agent testing" }
...
```mermaid
---
id: eval_trait_dataset_runner_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/eval-trait-dataset-runner.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2060"

  - path: projects/agentkit/core/src/eval_trait_dataset_runner.rs
    action: create
    section: changes
    note: "Module stub for Eval trait + dataset runner deterministic agent testing — codegen marker block, implementation lands in this issue's PR"
```
