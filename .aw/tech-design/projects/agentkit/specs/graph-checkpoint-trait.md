...
```mermaid
---
id: graph_checkpoint_trait_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — Checkpoint trait memory file SQL impls" }
  build: { kind: process, label: "construct typed surface per #2040" }
...
```mermaid
---
id: graph_checkpoint_trait_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "Checkpoint trait memory file SQL impls" }
...
```mermaid
---
id: graph_checkpoint_trait_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/graph-checkpoint-trait.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2040"

  - path: projects/agentkit/graph/src/graph_checkpoint_trait.rs
    action: create
    section: changes
    note: "Module stub for Checkpoint trait memory file SQL impls — codegen marker block, implementation lands in this issue's PR"
```
