...
```mermaid
---
id: document_loader_trait_lifecycle
entry: start
nodes:
  start: { kind: start, label: "entry — DocumentLoader trait" }
  build: { kind: process, label: "construct typed surface per #2049" }
...
```mermaid
---
id: document_loader_trait_surface
nodes:
  Caller: { kind: class, label: "Caller" }
  Surface: { kind: class, label: "DocumentLoader trait" }
...
```mermaid
---
id: document_loader_trait_tests
nodes:
...
```yaml
files:
  - path: .aw/tech-design/projects/agentkit/specs/document-loader-trait.md
    action: create
    section: changes
    note: "This TD spec — source of truth for #2049"

  - path: projects/agentkit/llm/src/document_loader_trait.rs
    action: create
    section: changes
    note: "Module stub for DocumentLoader trait — codegen marker block, implementation lands in this issue's PR"
```
