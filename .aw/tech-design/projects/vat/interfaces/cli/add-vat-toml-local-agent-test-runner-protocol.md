...
---
mindmap
  root((vat.toml runner))
    Contract
      "project-local vat.toml"
      setup
      services
      runners
      env
    ...
    Boundaries
      ...
      "not OCI"
      "not daemon"
      "not long-lived process manager"
```
## State Machine
...
properties:
  ...
  retention: { type: string, enum: [failed, always, never] }
  services:
    type: array
    items:
      type: object
...
- [contract] The TD contract matches the implemented vat scope: project-local `vat.toml`, ephemeral run-scoped setup/services/runner execution, structured evidence, failed-only retention, and `vat logs`.
- [compatibility] Legacy `vat run -- <cmd>` remains explicitly preserved.
- [boundary] Docker, OCI image runtime, Linux runtime on macOS, Compose compatibility, daemon mode, restart policy, and production scheduling remain out of scope.
- [verification] `aw td check .aw/tech-design/projects/vat/interfaces/cli/add-vat-toml-local-agent-test-runner-protocol.md` reports 0 findings.
- [tests] The contract points to `cargo test -p vat` and `cargo test -p vat vat_toml_runner -- --nocapture`, both already exercised by vat health.
