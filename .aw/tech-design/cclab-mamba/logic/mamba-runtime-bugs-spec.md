...
> **Audit note (2026-05-08, #1270):** all 5 R-groups (R1 semicolon
> separator, R2 ZeroDivisionError on `//`/%, R3 decorator return,
> R4 nested f-string, R5 json.dumps return) are **shipped** on
> `main`. Spot-check evidence:
>
> - R1: `parser/stmt.rs` consumes `TokenKind::Semicolon` between
>   simple statements (`while self.peek_kind() == Some(TokenKind::Semicolon)`).
> - R2: `runtime/builtins.rs:1375` raises `ZeroDivisionError` for
>   floor-div by zero; line 1394 covers modulo by zero; line 1411
>   handles the float case.
> - R3: `lower/hir_to_mir.rs:99` checks `func.decorators.is_empty()`
>   and threads them via `pending_decorators`.
> - R4: `parser/expr.rs:495` defines `parse_fstring_parts`; line 560
>   recurses on inner `f'...'` literals as documented.
> - R5: `runtime/stdlib/json_mod.rs:11` `dispatch_dumps` returns the
>   `mb_json_dumps*` result (line 56), no longer drops it.
>
> Spec stays in place as a historical bug catalog but is no longer a
> live to-do list. New runtime bugs go to fresh issues, not against
...
**Status:** shipped — `parser/stmt.rs` consumes `TokenKind::Semicolon` between simple statements (peek + while-consume loops).
...
**Status:** shipped — `runtime/builtins.rs:1375` (floor-div), 1394 (modulo), 1411 (float modulo) all `mb_raise` with `ZeroDivisionError`.
...
**Status:** shipped — `lower/hir_to_mir.rs:99` checks `func.decorators.is_empty()` and threads decorators via `pending_decorators` so the wrapper's return value reaches the caller.
...
**Status:** shipped — `parser/expr.rs:495` defines `parse_fstring_parts`; line 560 explicitly recurses on inner `f'...'` literals.
...
**Status:** shipped — `runtime/stdlib/json_mod.rs:11` `dispatch_dumps` returns the `mb_json_dumps*` result (line 56) instead of dropping it.
