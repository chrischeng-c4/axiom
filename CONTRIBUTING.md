...
| **`libs/service-http`** | the **HTTP service policy shell**: standard probe/admin routes, lifecycle readiness/signal adapters, metrics provider seam, tracing policy, runtime delegation, and the shared **HTTP error envelope** (`ErrorEnvelope` + the `ApiErr` status/kind builder). It owns no listener or drain state. |
