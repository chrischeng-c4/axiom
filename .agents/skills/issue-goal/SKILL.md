...
2. Load config from `apps/<project>/issue-loop.md` (frontmatter), or `projects/<project>/issue-loop.md` for retained mamba/lumen roots. Required keys: `branch`, `label`, `repo`, `verify`, `done_gates`, `pr`.
...
- `.score/handoffs/<n>-patrol-handoff.md` exists with mtime < 24h (recent failure).
...
1. Commit on the working branch. Conventional format. **No `Co-Authored-By: Codex` / `🤖 Generated with Codex` trailer** (global rule).
...
- **Never** commit with the `Co-Authored-By: Codex` trailer (global rule).
...
- If `score td` / `score cb` lifecycle is required for an issue, run it on **mainthread** — the PostToolUse hook lockfile chain doesn't reach subagent Bash (see memory `feedback_no_subagent_for_score_crrr.md`).
