...
workbench_settings="$(WORKBENCH_CORE_BIN="${workbench_core_bin}" xcodebuild \
...
workbench_app="${workbench_build_dir}/Workbench.app"
if [ ! -d "${workbench_app}" ]; then
    echo "Workbench.app was not produced at ${workbench_app}" >&2
    exit 66
fi

workbench_process="${workbench_app}/Contents/MacOS/Workbench"
if pgrep -f "${workbench_process}" >/dev/null 2>&1; then
    pkill -f "${workbench_process}"
    workbench_wait_count=0
    while pgrep -f "${workbench_process}" >/dev/null 2>&1 && [ "${workbench_wait_count}" -lt 30 ]; do
        sleep 0.1
        workbench_wait_count=$((workbench_wait_count + 1))
    done
fi
