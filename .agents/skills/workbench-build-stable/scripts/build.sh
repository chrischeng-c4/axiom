#!/bin/sh
# HANDWRITE-BEGIN gap="missing-generator:contract:83c52f04" tracker="#2445" reason="Build and restart only the stable native product."
set -eu
...
workbench_settings="$(WORKBENCH_CORE_BIN="${workbench_core_bin}" xcodebuild -project "${workbench_project}" -scheme WorkbenchMac -configuration Release -showBuildSettings)"
...
WORKBENCH_CORE_BIN="${workbench_core_bin}" xcodebuild -project "${workbench_project}" -scheme WorkbenchMac -configuration Release build
workbench_app="${workbench_build_dir}/Axiom Workbench.app"
workbench_process="${workbench_app}/Contents/MacOS/Axiom Workbench"

if [ ! -d "${workbench_app}" ]; then
    echo "stable product was not produced at ${workbench_app}" >&2
    exit 66
fi
