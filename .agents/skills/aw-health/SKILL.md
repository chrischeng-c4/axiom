---
name: aw:health
description: Run AW project health/readiness gates. If no project is supplied, infer it from the current project-<name> branch and run aw health with the resolved project token.
user-invocable: true
...
2. If no project is supplied, infer from the current branch:
   ...
   ```
   - `project-<token>` -> run `aw health <token>`.
   - Example: branch `project-aw` -> `aw health aw`.
...
`aw health` emits JSON by default and owns the readiness gate. Do not add
`--human` or `--pretty` unless the user asks for a human-readable report.

```bash
aw health <project>
```

Use the JSON envelope as authoritative:
...
- If `aw health` prints progress events before the final result, wait for the
  final `event=result` envelope before answering.
