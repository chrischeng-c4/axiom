...
- External behavior belongs in the canonical Python EC project under `external-contracts/`; never add an app-level Rust EC wrapper or delegate a Python EC to Cargo. A rule that genuinely requires crate-private state is a colocated Rust invariant under its semantic `src/**` owner and runs via the library test gate.
...
- **Never run `aw cb gen --force-regen`**: lifecycle generation can auto-commit a regen that reverts unsynced source. Not your tool.
...
- Smoke the real surface: `./target/debug/aw <verb> --help` after clap changes, and run the actual changed verb against a throwaway fixture when feasible. Reuse the project-fixture helpers in `external-contracts/src/` for Python EC and colocated `src/**` helpers for Rust invariants.
...
- `src/cli/loop_state.rs` — `decide_next_action` :161 (terminal command is slugged `aw cb check <id>`); loop-state block lives in the WI issue body.
...
- LINEAR: `aw wi` → `aw ec` → `aw td create` (td_inited→td_created) → `aw cb gen` (→cb_genned) → `aw cb fill` (→cb_filled) → `aw cb check` (→td_merged, terminal). No merge/review/revise — `aw td merge` was REMOVED; never reference or re-add it.
...
- Epic #914 (chain integrity, router unification onto td_phase, runner
  re-homing `aw wi run`/`aw capability run`, `aw run` deprecation,
  standardize dissolution) is DONE. Current epic: #1899 unifies all loop
  drivers onto `aw goal`'s closed four-leaf root-type enum (`wi` /
  `capability` / `backlog` / `adhoc`), retiring `aw wi run` and
  `aw capability run` themselves in favor of `aw goal wi <id>` and
  `aw goal capability [<cap-id>] --project <p>`. Each issue body carries its
  own Scope/AC — the issue is the contract; the file map above tells you
  where its subjects live.
- `aw llm --topic goal` is the orientation doc for the four-leaf enum; use
  it when a task needs the loop-verb mental model instead of re-deriving it.
