...
```
uv run --python 3.13 --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/e2e.py" --project <name> review-prompt <iid> > <prompt-path>
codex exec - < <prompt-path> > <transcript-path> 2>&1
uv run --python 3.13 --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/e2e.py" --project <name> verdict <iid> --transcript <transcript-path>
```
...
is 3.11+, while a bare `python3` is 3.9 on at least one machine this runs on. If
`${CLAUDE_PLUGIN_ROOT}` does not resolve, the plugin is not loaded; the script
is in the checkout at `plugins/aw/scripts/e2e.py`. For readability this skill
...
```
uv run --python 3.13 --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/e2e.py" review-prompt > <prompt-path>
codex exec - < <prompt-path> > <transcript-path> 2>&1
```
