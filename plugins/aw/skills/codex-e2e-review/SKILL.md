...
```
uv run --python 3.13 --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/e2e.py" review-prompt <iid> > <prompt-path>
codex exec - < <prompt-path> > <transcript-path> 2>&1
uv run --python 3.13 --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/e2e.py" verdict <iid> --transcript <transcript-path>
```
