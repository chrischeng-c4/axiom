...
is in the checkout at `plugins/aw/scripts/logic.py`. For readability this skill
...
```
uv run --python 3.13 --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/logic.py" review-prompt > <prompt-path>
codex exec - < <prompt-path> > <transcript-path> 2>&1
```
