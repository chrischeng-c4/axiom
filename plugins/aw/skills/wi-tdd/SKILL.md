...
```
uv run --python 3.13 --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/epic.py" order <iid> --open-only
```
...
rather than a wrong interpreter. If `${CLAUDE_PLUGIN_ROOT}` does not resolve
the plugin is not loaded; the scripts are in the checkout at
`plugins/aw/scripts/e2e.py`, `plugins/aw/scripts/unit.py`, and
`plugins/aw/scripts/logic.py`. For readability the table below writes the short
