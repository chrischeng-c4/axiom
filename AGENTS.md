...
`hitl_question.tool_hint=ask_user_question`, invoke the host's native
user-question tool immediately with its question, choices, and freeform prompt;
do not treat the envelope as terminal output or fabricate human approval. If the
host has no such tool, present every field as a blocking human question. (The
