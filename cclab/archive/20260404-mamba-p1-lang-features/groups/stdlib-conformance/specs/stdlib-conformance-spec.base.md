---
id: mamba-merge-tests-spec
main_spec_ref: "cclab-mamba/testing/cpython-compliance.md"
---
...
### R1 - Code Migration
Move `crates/cclab-mamba-tests/tests/cpython_compat.rs` to `crates/cclab-mamba/tests/cpython_compat.rs`. Update module references and crate imports as needed to align with being an internal integration test.

### R2 - Fixture Migration
Relocate all Python fixture files from `crates/cclab-mamba-tests/tests/fixtures/cpython/` to `crates/cclab-mamba/tests/fixtures/cpython/`. Maintain existing directory structures for fixture discovery.

### R3 - XFail Manifest Migration and Renaming
Move `crates/cclab-mamba-tests/known_failures.toml` to `crates/cclab-mamba/cpython_known_failures.toml`. Rename the file to explicitly associate it with the CPython compliance suite and prevent collisions with other test manifests.
...
Update `crates/cclab-mamba/Cargo.toml` to register `cpython_compat` as an integration test target with `harness = false`, ensuring it continues to use its custom `datatest-stable` harness.
...
- **WHEN** `cargo test -p cclab-mamba --test cpython_compat` is run
...
requirementDiagram
  ...
  element TestCpythonCompatSuite {
    ...
    when: "cargo test -p cclab-mamba --test cpython_compat"
...
- `crates/cclab-mamba-tests/tests/cpython_compat.rs` (DELETE)
- `crates/cclab-mamba-tests/tests/fixtures/cpython/` (DELETE)
- `crates/cclab-mamba-tests/known_failures.toml` (DELETE)
- `crates/cclab-mamba-tests/Cargo.toml` (DELETE)
- `crates/cclab-mamba-tests/src/lib.rs` (DELETE)
- `crates/cclab-mamba-tests/` (DELETE)

- `Cargo.toml` (MODIFY)
  - Remove `"crates/cclab-mamba-tests"` from `members`.

- `crates/cclab-mamba/Cargo.toml` (MODIFY)
  - Add `[[test]] name = "cpython_compat" harness = false`.
  - Ensure `toml` and `datatest-stable` are in `dev-dependencies`.

- `crates/cclab-mamba/tests/cpython_compat.rs` (CREATE)
  - Copy from `crates/cclab-mamba-tests/tests/cpython_compat.rs`.
  - Update `load_xfail_map()` manifest path to `cpython_known_failures.toml`.
  - Update `harness!` fixture path.

- `crates/cclab-mamba/tests/fixtures/cpython/` (CREATE)
  - Move contents from `crates/cclab-mamba-tests/tests/fixtures/cpython/`.

- `crates/cclab-mamba/cpython_known_failures.toml` (CREATE)
  - Rename/Move from `crates/cclab-mamba-tests/known_failures.toml`.