...
project_build_print_release_next_steps() {
  ...
  echo ""
  echo "Complete the release with build-release ${project}:"
  echo "  1. git-land: land this release commit on main"
  echo "  2. build-release ${project}: candidate from the landed sha -> GKE gate -> annotated ${tag} at that sha -> promotion -> public verification"
  echo "Never create or push ${tag} by hand: the candidate proves the bytes first, and the tag exists only at the verified candidate commit."
}
