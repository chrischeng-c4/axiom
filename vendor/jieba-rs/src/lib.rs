...
/// A caller-owned route for [`Jieba::cut_no_hmm_visit`].
///
/// The visitor needs random access to one dynamic-programming result for each
/// input byte.  Keeping that route behind this trait lets callers place it in
/// bounded external storage instead of retaining it in a `Vec`.
pub trait RouteStore {
    /// Prepare slots indexed by input byte offsets.  `slots` includes the
    /// terminal offset.
    fn reset(&mut self, slots: usize) -> io::Result<()>;

    /// Read the score and next byte offset for one input byte offset.
    fn get(&mut self, index: usize) -> io::Result<(f64, usize)>;

    /// Write the score and next byte offset for one input byte offset.
...
impl Jieba {
    ...
    /// Visit the tokens from `cut(sentence, false)` without retaining them in
    /// an output vector.
    ///
    /// `route` owns the dynamic-programming route.  The caller may use a
    /// file-backed implementation when an input block is too large for RAM.
    /// This deliberately implements only the no-HMM path used by Lumen.
    pub fn cut_no_hmm_visit(
        &self,
        sentence: &str,
        route: &mut impl RouteStore,
                ...
                for state in SplitMatches::new(re_han, sentence) {
                    match state {
                        SplitState::Matched(_) => {
                            self.cut_dag_no_hmm_visit(state.as_str(), route, &mut emit)?;
                        }
                        SplitState::Unmatched(_) => {
                            ...
                            for skip_state in SplitMatches::new(re_skip, block) {
                                ...
                                if skip_state.is_matched() {
                                    ...
                                    let mut indices = word.char_indices().map(|entry| entry.0).peekable();
                                    while let Some(byte_start) = indices.next() {
                                        let byte_end = indices.peek().copied().unwrap_or(word.len());
                                        emit(&word[byte_start..byte_end])?;
                                    }
                                }
                            }
                        }
    ...
    fn cut_dag_no_hmm_visit(
        &self,
        sentence: &str,
        route: &mut impl RouteStore,
        emit: &mut impl FnMut(&str) -> io::Result<()>,
    ) -> io::Result<()> {
        let slots = sentence
            .len()
            .checked_add(1)
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "sentence is too large"))?;
        route.reset(slots)?;

        let result = (|| {
            // The terminal is part of the algorithm, not an assumption about
            // how the caller initializes or reuses its backing storage.
            route.set(sentence.len(), (0.0, 0))?;
            let logtotal = (self.total as f64).ln();
            let mut previous_start = sentence.len();

            for byte_start in sentence.char_indices().map(|entry| entry.0).rev() {
                let mut best: Option<(f64, usize)> = None;
                for (_, end_index) in self.cedar.common_prefix_iter(&sentence[byte_start..]) {
                    let byte_end = end_index
                        .checked_add(byte_start)
                        .and_then(|offset| offset.checked_add(1))
                        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "dictionary edge overflows"))?;
                    let word = &sentence[byte_start..byte_end];
                    let freq = self.get_word_freq(word, 1);
                    let suffix_score = route.get(byte_end)?.0;
                    let candidate = ((freq as f64).ln() - logtotal + suffix_score, byte_end);

                    if best.as_ref().is_none_or(|current| {
                        candidate.partial_cmp(current).unwrap_or(Ordering::Equal) != Ordering::Less
                    }) {
                        best = Some(candidate);
                    }
                }
                ...
                let value = match best {
                    ...
                    None => {
                        let suffix_score = route.get(previous_start)?.0;
                        ((1f64).ln() - logtotal + suffix_score, previous_start)
                    }
                };
                route.set(byte_start, value)?;
            ...
            while byte_start < sentence.len() {
                let (_, byte_end) = route.get(byte_start)?;
                if byte_end <= byte_start || byte_end > sentence.len() || !sentence.is_char_boundary(byte_end) {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "route contains an invalid byte offset",
                    ));
                }
                let word = &sentence[byte_start..byte_end];
                if word.chars().count() == 1 && word.chars().all(|character| character.is_ascii_alphanumeric()) {
                    if ascii_start.is_none() {
                        ascii_start = Some(byte_start);
                    }
                } else {
                    if let Some(start) = ascii_start.take() {
                        emit(&sentence[start..byte_start])?;
                    }
                    emit(word)?;
                }
                byte_start = byte_end;
...
#[cfg(test)]
mod tests {
    use super::{Jieba, RouteStore, SplitMatches, SplitState, Tag, Token, TokenizeMode, RE_HAN_DEFAULT};
    ...
    impl RouteStore for MemoryRoute {
        fn reset(&mut self, slots: usize) -> io::Result<()> {
            self.slots.clear();
        ...
        fn get(&mut self, index: usize) -> io::Result<(f64, usize)> {
            self.slots
                .get(index)
                .copied()
                .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "route read outside slots"))
        }

        fn set(&mut self, index: usize, value: (f64, usize)) -> io::Result<()> {
            if self.fail_set {
                return Err(io::Error::new(io::ErrorKind::Other, "injected route write failure"));
            }
            let slot = self
                .slots
                .get_mut(index)
                .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "route write outside slots"))?;
            *slot = value;
            Ok(())
        }
    }

    fn visited(jieba: &Jieba, input: &str) -> Vec<String> {
        let mut route = MemoryRoute::default();
        let mut tokens = Vec::new();
        jieba
            .cut_no_hmm_visit(input, &mut route, |token| {
                tokens.push(token.to_owned());
                Ok(())
            })
            .expect("visitor must finish");
        tokens
    }

    fn assert_visit_matches_cut(jieba: &Jieba, input: &str) {
        let expected = jieba
            .cut(input, false)
            .into_iter()
            .map(str::to_owned)
            .collect::<Vec<_>>();
        assert_eq!(visited(jieba, input), expected, "input: {input:?}");
    }

    #[test]
    fn test_no_hmm_visit_matches_cut_for_default_boundaries() {
        let jieba = Jieba::new();
        for input in [
            "我们中出了一个叛徒",
            "南京市长江大桥",
            "abc123+_#&.%-XYZ",
            "中文, punctuation!\n\tEnglish42",
            "𠀀𪛔，emoji👪 and Ελληνικά",
            "  \r\n\t ",
        ] {
            assert_visit_matches_cut(&jieba, input);
        }
    }

    #[test]
    fn test_no_hmm_visit_matches_cut_after_dictionary_changes() {
        let mut jieba = Jieba::empty();
        jieba.add_word("南京", Some(10), None);
        jieba.add_word("南京市", Some(10), None);
        jieba.add_word("市长", Some(10), None);
        jieba.add_word("长江", Some(10), None);
        jieba.add_word("长江大桥", Some(10), None);
        assert_visit_matches_cut(&jieba, "南京市长江大桥");

        let dictionary = "中出 10000\n出了 10000\n";
        jieba.load_dict(&mut BufReader::new(dictionary.as_bytes())).unwrap();
        assert_visit_matches_cut(&jieba, "我们中出了一个叛徒");
    }

    #[test]
    fn test_no_hmm_visit_propagates_callback_and_route_errors() {
        let jieba = Jieba::new();
        let mut route = MemoryRoute::default();
        let callback_error = jieba
            .cut_no_hmm_visit("我们中出了一个叛徒", &mut route, |_| {
                Err(io::Error::new(io::ErrorKind::Interrupted, "stop after first token"))
            })
            .expect_err("callback error must stop visiting");
        assert_eq!(callback_error.kind(), io::ErrorKind::Interrupted);

        let mut route = MemoryRoute {
            fail_set: true,
            ..MemoryRoute::default()
        };
        let route_error = jieba
            .cut_no_hmm_visit("我们", &mut route, |_| Ok(()))
            .expect_err("route write error must escape");
        assert_eq!(route_error.kind(), io::ErrorKind::Other);
    }
