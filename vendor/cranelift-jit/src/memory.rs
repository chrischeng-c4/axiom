...
impl Memory {
    ...
    pub(crate) fn allocate(&mut self, size: usize, align: u64) -> io::Result<*mut u8> {
        ...
        // Mamba vendor patch: enforce a minimum 256 MB chunk on aarch64 so all
        // JIT'd functions in a JITModule remain within ±128 MB of each other —
        // sidesteps Reloc::Arm64Call's 26-bit BL immediate-range bug
        // (compiled_blob.rs:90 assertion) without needing PIC/PLT (PLT is
        // x86_64-only in this crate).
        //
        // Memory is OS demand-paged: address-space reservation is large but
        // RSS only grows as code is actually emitted. On 64-bit hosts the
        // address-space cost is negligible.
        let min_chunk = 256 * 1024 * 1024;
        let alloc_size = std::cmp::max(size, min_chunk);
        self.current = PtrLen::with_size(alloc_size)?;
        self.position = size;
