---
id: libs-service-backup-src-llm-rs
summary: Lossless rust-source-unit coverage for `libs/service-backup/src/llm.rs`.
capability_refs:
...
# Standardized libs/service-backup/src/llm.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/service-backup/src/llm.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `TOPIC` | libs/service-backup/src/llm.rs | const | pub | 17 | pub const TOPIC: cli_std::llm::Topic = cli_std::llm::Topic { |
| `topic` | libs/service-backup/src/llm.rs | function | pub | 54 | pub fn topic() -> &'static cli_std::llm::Topic { |
| `SECTIONED_TOPICS` | libs/service-backup/src/llm.rs | const | pub | 110 | pub const SECTIONED_TOPICS: &[cli_std::llm::SectionedTopic] = &[cli_std::llm::SectionedTopic { |
| `sectioned_topic` | libs/service-backup/src/llm.rs | function | pub | 124 | pub fn sectioned_topic() -> &'static cli_std::llm::SectionedTopic { |

#2494: `TOPIC`/`topic` are the original static form and are unchanged —
every existing CLI consumer (e.g. `apps/lumen/src/spec.rs`'s
`service_backup::llm::topic().body`) keeps compiling and behaving exactly
as before. `SECTIONED_TOPICS`/`sectioned_topic` are the parallel
`cli_std::llm::SectionedTopic` form: the destination-contract block is a
`TopicSection::Generated` section rendered from
`crate::SUPPORTED_SCHEMES` at call time instead of a hand-copied scheme
list, so it can't drift from what `BackupDestination::from_uri` and
`sink_from_destination` actually accept in this build.
...
changes:
  - path: "libs/service-backup/src/llm.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/service-backup/src/llm.rs` captured during libs codegen standardization.
      #2494: added `SECTIONED_TOPICS`/`sectioned_topic`, a
      `cli_std::llm::SectionedTopic` companion to the existing static
      `TOPIC`/`topic`. Its destination-contract section is
      `TopicSection::Generated`, rendered from `crate::SUPPORTED_SCHEMES`
      at call time instead of a hand-copied scheme list — additive; `TOPIC`
      and `topic()` keep their exact prior signature and behavior.
```
