---
id: libs-cli-std-src-connect-rs
summary: Lossless rust-source-unit coverage for `libs/cli-std/src/connect.rs`.
capability_refs:
...
# Standardized libs/cli-std/src/connect.rs
...
`select_token`, `resolve_token`) are universal to every k8s-native service
CLI, not lumen-specific. Each adopter keeps only its own flag surface,
CR-kind lookup convention (the `resource_kind` string passed to
`resolve_cr_tokens_secret`, e.g. `"lumen"`), and role mapping into [`Role`]
(structurally identical to `service_auth::Role` but kept independent here —
`service-auth` itself depends on `cli-std`, so `cli-std` cannot depend back
on `service-auth` without a cycle).

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `TOKEN_REGISTRY_SECRET_KEY` | libs/cli-std/src/connect.rs | const | pub | 23 | pub const TOKEN_REGISTRY_SECRET_KEY: &str = "token-registry.json"; |
| `ChildGuard` | libs/cli-std/src/connect.rs | struct | pub | 31 | pub struct ChildGuard { |
| `spawn` | libs/cli-std/src/connect.rs | function | pub | 37 | pub fn spawn(command: &mut Command) -> Result<Self> { |
| `free_local_port` | libs/cli-std/src/connect.rs | function | pub | 56 | pub fn free_local_port() -> Result<u16> { |
| `wait_for_local_port_ready` | libs/cli-std/src/connect.rs | function | pub | 66 | pub fn wait_for_local_port_ready(port: u16, timeout: Duration) -> Result<()> { |
| `Role` | libs/cli-std/src/connect.rs | enum | pub | 89 | pub enum Role { |
| `covers` | libs/cli-std/src/connect.rs | function | pub | 98 | pub fn covers(self, needed: Role) -> bool { |
| `TokenClaims` | libs/cli-std/src/connect.rs | struct | pub | 109 | pub struct TokenClaims { |
| `cr_tokens_secret` | libs/cli-std/src/connect.rs | function | pub | 119 | pub fn cr_tokens_secret(cr_json: &serde_json::Value) -> Option<String> { |
| `kubectl_get_json` | libs/cli-std/src/connect.rs | function | pub | 126 | pub fn kubectl_get_json( |
| `resolve_cr_tokens_secret` | libs/cli-std/src/connect.rs | function | pub | 154 | pub fn resolve_cr_tokens_secret( |
| `secret_data_bytes` | libs/cli-std/src/connect.rs | function | pub | 167 | pub fn secret_data_bytes(secret_json: &serde_json::Value, key: &str) -> Result<Vec<u8>> { |
| `select_token` | libs/cli-std/src/connect.rs | function | pub | 182 | pub fn select_token( |
| `resolve_token` | libs/cli-std/src/connect.rs | function | pub | 205 | pub fn resolve_token( |
...
changes:
  - path: "libs/cli-std/src/connect.rs"
    ...
    impl_mode: codegen
    description: |
      New shared module (#1376), lifted from `lumen connect`'s #1321
      implementation: the `kubectl port-forward` process lifecycle
      (ChildGuard, free_local_port, wait_for_local_port_ready) and the
      token-registry Secret resolution chain (kubectl_get_json,
      cr_tokens_secret, resolve_cr_tokens_secret, secret_data_bytes,
      select_token, resolve_token), now parameterized by a `resource_kind`
      string and its own `Role`/`TokenClaims` types instead of a `Lumen` CR
      type / `service_auth::Role` (avoiding a cli-std -> service-auth
      dependency cycle), so no k8s-native service CLI needs a lumen or
      service-auth dependency to adopt `connect`. Gated behind the `k8s`
      feature so non-k8s consumers build unchanged. Behavior and tests moved
      byte-identical from the lumen implementation.
```
