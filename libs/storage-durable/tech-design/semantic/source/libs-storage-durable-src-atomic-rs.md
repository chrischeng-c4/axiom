---
id: libs-storage-durable-src-atomic-rs
summary: Lossless rust-source-unit coverage for `libs/storage-durable/src/atomic.rs`.
capability_refs:
...
# Standardized libs/storage-durable/src/atomic.rs

## Overview
<!-- type: overview lang: markdown -->

Lossless rust-source-unit coverage for `libs/storage-durable/src/atomic.rs`.

### Symbols

| Name | Target | Kind | Visibility | Signature |
|------|--------|------|------------|-----------|
| `atomic_write` | libs/storage-durable/src/atomic.rs | function | pub | atomic_write(path: impl AsRef<Path>, bytes: &[u8], policy: FsyncPolicy) -> Result<()> |
| `sync_parent_dir` | libs/storage-durable/src/atomic.rs | function | pub | sync_parent_dir(path: impl AsRef<Path>) -> Result<()> |
...
changes:
  - path: "libs/storage-durable/src/atomic.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/storage-durable/src/atomic.rs`.
```
