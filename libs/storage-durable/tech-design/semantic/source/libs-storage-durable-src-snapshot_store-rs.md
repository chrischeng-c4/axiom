---
id: libs-storage-durable-src-snapshot-store-rs
summary: Lossless rust-source-unit coverage for `libs/storage-durable/src/snapshot_store.rs`.
capability_refs:
...
# Standardized libs/storage-durable/src/snapshot_store.rs

## Overview
<!-- type: overview lang: markdown -->

Lossless rust-source-unit coverage for `libs/storage-durable/src/snapshot_store.rs`.

### Symbols

| Name | Target | Kind | Visibility | Signature |
|------|--------|------|------------|-----------|
| `SnapshotFile` | libs/storage-durable/src/snapshot_store.rs | struct | pub | SnapshotFile { seq, path } |
| `SnapshotFileStore` | libs/storage-durable/src/snapshot_store.rs | struct | pub | SnapshotFileStore |
| `new` | libs/storage-durable/src/snapshot_store.rs | method | pub | new(root, prefix, extension, policy) -> Result<Self> |
| `save` | libs/storage-durable/src/snapshot_store.rs | method | pub | save(&self, seq, bytes) -> Result<PathBuf> |
| `load_latest` | libs/storage-durable/src/snapshot_store.rs | method | pub | load_latest(&self) -> Result<Option<Vec<u8>>> |
| `snapshots` | libs/storage-durable/src/snapshot_store.rs | method | pub | snapshots(&self) -> Result<Vec<SnapshotFile>> |
| `prune` | libs/storage-durable/src/snapshot_store.rs | method | pub | prune(&self, keep) -> Result<usize> |
...
changes:
  - path: "libs/storage-durable/src/snapshot_store.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/storage-durable/src/snapshot_store.rs`.
```
