...
//! render toolkit for the common sharded-HA objects, so a service supplies only
//! a [`ManagedService`] (its CRD type + `render`/`status_patch`/readiness) and
//! its service-specific rendering.
//!
//! See `CONTRIBUTING.md` "Service archetype" — this is the deploy-layer member of
//! the shared service kit (`raft-core` + `raft-host` + `h2c` + `cli-std` + this).
