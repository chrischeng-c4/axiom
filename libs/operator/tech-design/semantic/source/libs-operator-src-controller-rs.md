...
summary: Lossless rust-source-unit coverage for `libs/operator/src/controller.rs`.
capability_refs:
...
# Standardized libs/operator/src/controller.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/operator/src/controller.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `Error` | libs/operator/src/controller.rs | enum | pub | 26 | pub enum Error { |
| `run` | libs/operator/src/controller.rs | function | pub | 55 | pub async fn run<S: ManagedService>() -> anyhow::Result<()> { |
...
changes:
  - path: "libs/operator/src/controller.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/operator/src/controller.rs` captured during libs codegen standardization.
```
