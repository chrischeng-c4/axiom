# Service Observability Capabilities

## Brief

`service-observability` is what every axiom service installs so its stdout, its
metrics, and its traces look the same. It owns one versioned log envelope, the
rules for which correlation fields may enter it, the bound on how much a caller
can put in it, and the decision of whether tracing export is available at all.

The envelope is the contract. A collector reads `axiom.service.log.v1` lines
from a service it has never seen and knows where the severity is, where the
service identity is, that `trace_id` is either a real W3C id or absent, and that
nothing under `attributes` is an `Authorization` header. That guarantee has to
hold for a caller who logs carelessly, which is why containment lives here and
not in each service.
