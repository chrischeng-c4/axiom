---
id: libs-peer-tls-src-lib-rs
summary: Lossless rust-source-unit coverage for `libs/peer-tls/src/lib.rs`.
capability_refs:
...
# Standardized libs/peer-tls/src/lib.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/peer-tls/src/lib.rs` captured during libs codegen standardization.
...
"#;
    ...
    fn write_tls_fixture(name: &str) -> PeerTlsConfig {
        let dir =
            std::env::temp_dir().join(format!("peer-tls-rustls-{}-{name}", std::process::id()));
...
changes:
  - path: "libs/peer-tls/src/lib.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/peer-tls/src/lib.rs` captured during libs codegen standardization.
```
