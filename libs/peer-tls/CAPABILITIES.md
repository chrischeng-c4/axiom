# Peer Tls Capabilities

## Brief

`peer-tls` is the shared mutual-TLS material library for service-to-service
ports. It validates PEM material against an expected identity, builds the
rustls server and client configs that enforce that identity, and keeps both
usable across certificate rotation. It is not a certificate authority, not an
issuer, and not a transport: callers own where material comes from and who
