...
| Kubernetes delegated auth | `kubernetes-delegated-auth` | Supported | Rust services can validate an explicit service audience or explicitly select Kubernetes-default audiences through TokenReview, accept only ServiceAccount principals, and authorize app-supplied attributes through SubjectAccessReview. | `DelegatedAuthConfig::new` rejects an empty audience. The app must name the Kubernetes-default constructor when default Pod tokens are its intended credential. The app also owns its API group, resource, name, verb, and namespace mapping. Kubernetes RBAC owns the answer. | `cargo test -p service-auth --features k8s` |
