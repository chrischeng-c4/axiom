---
id: libs-service-k8s-src-lease-rs
summary: Lossless rust-source-unit coverage for `libs/service-k8s/src/lease.rs`.
capability_refs:
...
# Standardized libs/service-k8s/src/lease.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/service-k8s/src/lease.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `Election` | libs/service-k8s/src/lease.rs | struct | pub | 30 | pub struct Election { |
| `new` | libs/service-k8s/src/lease.rs | function | pub | 36 | pub fn new(identity: String) -> Arc<Self> { |
| `spawn` | libs/service-k8s/src/lease.rs | function | pub | 114 | pub fn spawn(client: Client, namespace: String, lease_name: String, election: Arc<Election>) { |
...
changes:
  - path: "libs/service-k8s/src/lease.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/service-k8s/src/lease.rs` captured during libs codegen standardization.
```
