---
id: libs-compass-src-type-inference-env-rs
summary: Lossless rust-source-unit coverage for `libs/compass/src/type_inference/env.rs`.
capability_refs:
...
# Standardized libs/compass/src/type_inference/env.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/compass/src/type_inference/env.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `VenvType` | libs/compass/src/type_inference/env.rs | enum | pub | 22 | pub enum VenvType { |
| `DetectedEnv` | libs/compass/src/type_inference/env.rs | struct | pub | 49 | pub struct DetectedEnv { |
| `EnvInfo` | libs/compass/src/type_inference/env.rs | struct | pub | 60 | pub struct EnvInfo { |
| `detect_python_environment` | libs/compass/src/type_inference/env.rs | function | pub | 77 | pub fn detect_python_environment(project_root: &Path) -> EnvInfo { |
| `detect_with_config` | libs/compass/src/type_inference/env.rs | function | pub | 83 | pub fn detect_with_config(project_root: &Path, config: &ArgusConfig) -> EnvInfo { |
| `detect_all_venvs` | libs/compass/src/type_inference/env.rs | function | pub | 169 | pub fn detect_all_venvs(project_root: &Path) -> Vec<DetectedEnv> { |
| `is_venv_directory` | libs/compass/src/type_inference/env.rs | function | pub | 235 | pub fn is_venv_directory(path: &Path) -> bool { |
| `find_site_packages` | libs/compass/src/type_inference/env.rs | function | pub | 256 | pub fn find_site_packages(venv_path: &Path, python_version: Option<&str>) -> Option<PathBuf> { |
| `get_venv_python_version` | libs/compass/src/type_inference/env.rs | function | pub | 407 | pub fn get_venv_python_version(venv_path: &Path) -> Option<String> { |
...
changes:
  - path: "libs/compass/src/type_inference/env.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/compass/src/type_inference/env.rs` captured during libs codegen standardization.
```
