...
|------|--------|------|------------|------|-----------|
| `init_tracing` | libs/service-http/src/logging.rs | function | pub | 27 | pub fn init_tracing(cfg: &HttpConfig) -> anyhow::Result<()> { |
...
changes:
  - path: "libs/service-http/src/logging.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/service-http/src/logging.rs` captured during libs codegen standardization.
```
