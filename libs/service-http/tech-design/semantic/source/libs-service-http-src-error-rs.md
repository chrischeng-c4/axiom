---
id: libs-service-http-src-error-rs
summary: Lossless rust-source-unit coverage for `libs/service-http/src/error.rs`.
capability_refs:
...
# Standardized libs/service-http/src/error.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/service-http/src/error.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `ErrorEnvelope` | libs/service-http/src/error.rs | struct | pub | 27 | pub struct ErrorEnvelope { |
| `ApiErr` | libs/service-http/src/error.rs | struct | pub | 35 | pub struct ApiErr { |
| `new` | libs/service-http/src/error.rs | function | pub | 43 | pub fn new(status: StatusCode, kind: &'static str, message: impl Into<String>) -> Self { |
...
changes:
  - path: "libs/service-http/src/error.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/service-http/src/error.rs` captured during libs codegen standardization.
```
