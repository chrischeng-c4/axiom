...
| Static request auth values | `static-request-auth-values` | Limited | TypeScript accepts fixed default headers. Python accepts a fixed token or fixed default headers at construction. | Rust has no generated default Authorization input. No language refreshes a token per request. See [Dynamic request auth provider](ROADMAP.md#dynamic-request-auth-provider). | `cargo test -p openapi-codegen` |
...
| Dynamic per-request auth provider | `dynamic-per-request-auth-provider` | Not supported | Generated clients do not call a credential or header provider before each request. | A rotated projected token cannot replace a construction-time value. | [Dynamic request auth provider](ROADMAP.md#dynamic-request-auth-provider) |
