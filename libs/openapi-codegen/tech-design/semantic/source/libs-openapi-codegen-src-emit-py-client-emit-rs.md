---
id: libs-openapi-codegen-src-emit-py-client-emit-rs
summary: Lossless rust-source-unit coverage for `libs/openapi-codegen/src/emit/py/client_emit.rs`.
capability_refs:
...
# Standardized libs/openapi-codegen/src/emit/py/client_emit.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/openapi-codegen/src/emit/py/client_emit.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `HEADER` | libs/openapi-codegen/src/emit/py/client_emit.rs | const | pub | 12 | pub const HEADER: &str = "# Code generated by openapi-codegen. DO NOT EDIT.\n"; |
| `emit` | libs/openapi-codegen/src/emit/py/client_emit.rs | function | pub | 14 | pub fn emit(ops: &[OperationIR], tm: &TypeMap) -> String { |
...
pub fn emit(ops: &[OperationIR], tm: &TypeMap, target: Option<PythonTarget>) -> String {
    ...
    out.push_str("    context built from this replaces the root store rather than extending it.\n\n");
    ...
    out.push_str("def _private_trust_context(trust: PrivateTrust, base_url: str) -> ssl.SSLContext:\n");
    ...
    out.push_str("            f\"this client would verify {trust.server_name} while addressing \"\n");
    out.push_str("            f\"{addressed}; point the base URL at the name the certificate asserts\"\n");
    out.push_str("        )\n");
    out.push_str("    # Naming a cafile makes this the *only* trust anchor: create_default_context\n");
    ...
    out.push_str("            # An injected transport carries its own TLS configuration; silently\n");
    ...
    for ir in ops {
        out.push('\n');
        out.push_str(&emit_method(ir, tm, &mut reg, false, target));
    }
    ...
    for ir in ops {
        out.push('\n');
        out.push_str(&emit_method(ir, tm, &mut async_reg, true, target));
    }
    out
}
...
mod tests {
    ...
    fn init_signatures_accept_use_post_fallback() {
        ...
        let out = emit(&operations::build(&s), &tm, Some(PythonTarget::Py311));
    ...
    fn no_generated_client_offers_to_skip_verification() {
        ...
        let tm = build_type_map(&s);
        let out = emit(&operations::build(&s), &tm, Some(PythonTarget::Py311));
        for weakening in ["CERT_NONE", "check_hostname = False", "verify=False", "insecure"] {
            assert!(
...
changes:
  - path: "libs/openapi-codegen/src/emit/py/client_emit.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/openapi-codegen/src/emit/py/client_emit.rs` captured during libs codegen standardization.
```
