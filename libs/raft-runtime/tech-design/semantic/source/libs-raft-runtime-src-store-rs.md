---
id: libs-raft-runtime-src-store-rs
summary: Lossless rust-source-unit coverage for `libs/raft-runtime/src/store.rs`.
capability_refs:
...
# Standardized libs/raft-runtime/src/store.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/raft-runtime/src/store.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `FsyncPolicy` | libs/raft-runtime/src/store.rs | re-export | pub | 14 | pub use storage_durable::FsyncPolicy; |
| `RaftStore` | libs/raft-runtime/src/store.rs | struct | pub | 18 | pub struct RaftStore { |
| `open` | libs/raft-runtime/src/store.rs | function | pub | 25 | pub fn open(dir: &str, node_id: NodeId, fsync: FsyncPolicy) -> io::Result<RaftStore> { |
| `save` | libs/raft-runtime/src/store.rs | function | pub | 36 | pub fn save(&self, state: &PersistedState) -> io::Result<()> { |
| `load` | libs/raft-runtime/src/store.rs | function | pub | 44 | pub fn load(&self) -> io::Result<Option<PersistedState>> { |
...
impl RaftStore {
    ...
    pub fn save(&self, state: &PersistedState) -> io::Result<()> {
        ...
        storage_durable::atomic_write(&self.path, &bytes, self.fsync)
...
changes:
  - path: "libs/raft-runtime/src/store.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/raft-runtime/src/store.rs` captured during libs codegen standardization.
```
