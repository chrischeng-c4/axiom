---
id: libs-raft-runtime-src-config-rs
summary: Lossless rust-source-unit coverage for `libs/raft-runtime/src/config.rs`.
capability_refs:
...
# Standardized libs/raft-runtime/src/config.rs

## Overview
<!-- type: overview lang: markdown -->

Public API manifest for `libs/raft-runtime/src/config.rs` captured during libs codegen standardization.

### Symbols

| Name | Target | Kind | Visibility | Line | Signature |
|------|--------|------|------------|------|-----------|
| `SnapshotPolicy` | libs/raft-runtime/src/config.rs | enum | pub | 7 | pub enum SnapshotPolicy { |
| `HostConfig` | libs/raft-runtime/src/config.rs | struct | pub | 20 | pub struct HostConfig { |
...
changes:
  - path: "libs/raft-runtime/src/config.rs"
    ...
    impl_mode: codegen
    description: |
      rust-source-unit (td_ast) source for `libs/raft-runtime/src/config.rs` captured during libs codegen standardization.
```
