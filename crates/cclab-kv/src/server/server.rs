...
async fn handle_connection(
    ...
    loop {
        ...
        // Process request
        let response =
            match process_request(&buf[..5 + payload_len], &engine, &waiter_manager).await {
                Ok(resp) => resp,
                Err(e) => {
                    let msg = e.to_string();
    ...
    match cmd {
        ...
        Command::Incr => {
            ...
            match engine.incr(&key, delta) {
                Ok(new_val) => Ok(write_response(Status::Ok, &new_val.to_be_bytes())),
                Err(e) => Ok(write_response(Status::Error, e.to_string().as_bytes())),
            }
        }
        Command::Decr => {
        ...
        Command::MGet => {
            ...
            let kv_keys: Result<Vec<_>, _> = keys.iter().map(|k| KvKey::new(k)).collect();

            let kv_keys = kv_keys.map_err(|e| {
                ProtocolError::Io(std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
                    e.to_string(),
                ))
        ...
        Command::MSet => {
            ...
            let kv_pairs: Result<Vec<_>, _> = pairs
                .iter()
                .map(|(k, v)| KvKey::new(k).map(|key| (key, v.clone())))
                .collect();

            let kv_pairs = kv_pairs.map_err(|e| {
                ProtocolError::Io(std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
                    e.to_string(),
                ))
            ...
            let pair_refs: Vec<(&KvKey, KvValue)> =
                kv_pairs.iter().map(|(k, v)| (k, v.clone())).collect();
        ...
        Command::BLPop => blocking_pop_handler(engine, waiter_manager, &payload, true).await,
        Command::BRPop => blocking_pop_handler(engine, waiter_manager, &payload, false).await,
    }
}
