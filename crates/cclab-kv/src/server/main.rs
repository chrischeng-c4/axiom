...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (engine_arc, persistence_handle) = if args.disable_persistence {
        ...
        let (recovered_engine, stats) =
            cclab_kv::persistence::recovery::RecoveryManager::recover(&args.data_dir, args.shards)?;

        if stats.snapshot_loaded {
            info!(
                "Recovered from snapshot: {} entries in {:?}",
                stats.snapshot_entries, stats.recovery_duration
            );
        }
        ...
        if stats.wal_entries_replayed > 0 {
            info!(
                ...
                stats.wal_entries_replayed, stats.recovery_duration
            );
        }
        ...
        info!(
            ...
            args.fsync_interval_ms, args.snapshot_interval_secs, args.snapshot_ops_threshold
        );
