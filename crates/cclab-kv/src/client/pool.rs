...
use super::client::{ClientError, KvClient};
...
///
/// ```no_run
/// use cclab_kv::client::{KvPool, PoolConfig, KvValue};
///
/// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
/// let pool = KvPool::connect(
///     PoolConfig::new("127.0.0.1:7117/cache")
///         .min_size(2)
///         .max_size(10)
/// ).await?;
///
...
impl KvPool {
    ...
    pub fn namespace(&self) -> Option<&str> {
        self.config
            .addr
            .find('/')
            .map(|idx| &self.config.addr[idx + 1..])
    }
...
mod tests {
    ...
    fn test_pool_config_builder() {
        let config = PoolConfig::new("127.0.0.1:7117")
            .min_size(5)
            .max_size(20)
            .idle_timeout(Duration::from_secs(600))
            .acquire_timeout(Duration::from_secs(10));

        assert_eq!(config.addr, "127.0.0.1:7117");
    ...
    fn test_namespace_extraction() {
        let pool = KvPool::new(PoolConfig::new("127.0.0.1:7117/cache"));
        assert_eq!(pool.namespace(), Some("cache"));

        let pool = KvPool::new(PoolConfig::new("127.0.0.1:7117"));
    ...
    async fn test_pool_basic() {
        let pool = KvPool::connect(PoolConfig::new("127.0.0.1:7117").min_size(2).max_size(5))
            .await
            .unwrap();
    ...
    async fn test_pool_max_size() {
        let pool = KvPool::connect(PoolConfig::new("127.0.0.1:7117").min_size(1).max_size(3))
            .await
            .unwrap();
    ...
    async fn test_pool_concurrent() {
        let pool = KvPool::connect(PoolConfig::new("127.0.0.1:7117").min_size(2).max_size(10))
            .await
            .unwrap();
        ...
        for i in 0..20 {
            ...
            let handle = tokio::spawn(async move {
                let mut conn = pool.acquire().await.unwrap();
                conn.client()
                    .set(&format!("key_{}", i), crate::KvValue::Int(i as i64), None)
                    .await
                    .unwrap();
            });
            handles.push(handle);
