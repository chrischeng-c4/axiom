...
fn bench_individual_get(engine: &Arc<KvEngine>, count: usize) -> (f64, f64) {
    ...
    for i in 0..count {
        let key = make_key(i);
        let _ = engine.set(&key, make_string_value(i), None);
    }
...
fn run_comparison_benchmarks() {
    ...
    for &size in &batch_sizes {
        ...
        println!(
            "│   Individual: {:>10.0} ops/sec ({:.6}s)",
            ind_get_ops, ind_get_time
        );
        println!(
            "│   Batch MGET: {:>10.0} ops/sec ({:.6}s)",
            batch_get_ops, batch_get_time
        );
        ...
        println!(
            "│   Individual: {:>10.0} ops/sec ({:.6}s)",
            ind_set_ops, ind_set_time
        );
        println!(
            "│   Batch MSET: {:>10.0} ops/sec ({:.6}s)",
            batch_set_ops, batch_set_time
        );
        ...
        println!(
            "│   Individual: {:>10.0} ops/sec ({:.6}s)",
            ind_del_ops, ind_del_time
        );
        println!(
            "│   Batch MDEL: {:>10.0} ops/sec ({:.6}s)",
            batch_del_ops, batch_del_time
        );
