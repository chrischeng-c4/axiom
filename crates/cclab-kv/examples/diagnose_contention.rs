...
fn analyze_shard_distribution() {
    ...
    println!(
        "  Std deviation: {:.2}",
        calculate_stddev(&counts, avg as f64)
    );
...
fn benchmark_concurrent_same_keys() {
    ...
    for i in 0..100 {
        let key = make_key(i);
        let _ = engine.set(&key, KvValue::Int(0), None);
    }
    ...
    for num_threads in [1, 2, 4, 8] {
        ...
        for _t in 0..num_threads {
            ...
            handles.push(thread::spawn(move || {
                for i in 0..ops_per_thread {
                    ...
                    let _ = engine.set(&key, KvValue::Int(i as i64), None);
                }
            }));
        }
        ...
        println!(
            "{} threads: {:>10.0} ops/sec ({:.3}s for {} ops)",
            num_threads,
            ops_per_sec,
            elapsed.as_secs_f64(),
            total_ops
...
fn benchmark_thread_overhead() {
    ...
    for num_threads in [1, 2, 4, 8] {
        ...
        println!(
            "{} threads: {:>10.0} ops/sec (avg over {} runs)",
