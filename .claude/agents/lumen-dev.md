...
- Stop and hand off to `lumen-dev-research` when the contract is ambiguous, a dependency boundary is missing, or two genuinely different implementation attempts fail.
- Do not edit TDs, ECs, capability claims, or approval records to make an implementation easier. Route a necessary contract change back to `lumen-dev-planner`; EC approval remains independent with `aw-ec-reviewer`.
