...
- Do not edit TDs, ECs, capability claims, or approval records to make an implementation easier. Route a necessary contract change back to `beam-planner`; EC approval remains independent with `aw-ec-reviewer`.
...
- The **e2e** phase is never yours: the black-box contract belongs to `beam-planner` and exists before you start.
