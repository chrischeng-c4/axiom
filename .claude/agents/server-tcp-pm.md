...
Leave `libs/server-tcp/README.md`, `STATUS.md`, `ROADMAP.md`, and `docs/**` as
uncommitted working-tree bytes that state the library's promises, support
matrix, and outcomes, with `aw metadoc check server-tcp` and
`aw meta check --path libs/server-tcp` both clean, so the grill confirms each
...
- Check the write root is yours:
  `git -c core.fsmonitor=false status --short -- libs/server-tcp` must show no
  other writer's uncommitted work. If it does, stop and report; one worktree
  carries one writer.
- Read before drafting: `libs/server-tcp/README.md`, `CONTRIBUTING.md`,
  `STATUS.md`, `ROADMAP.md`, `docs/**`, and `Cargo.toml`; the `e2e/` manifest
  and the `src/` module `//!` blocks for what the code actually does; every
  consumer's `README.md` `Sources` column naming `libs/server-tcp` for what
...
- A performance gap reported by `server-tcp-qa` names a `src/` change point,
...
- Run the checks and fix until both are clean:
  ...
  ```bash
  uv run --project apps/aw aw metadoc check server-tcp
  uv run --project apps/aw aw meta check --path libs/server-tcp
  ```

- Report: `git -c core.fsmonitor=false status --short -- libs/server-tcp`; each
...
- `git -c core.fsmonitor=false status --short` shows changes only under
  `libs/server-tcp/README.md`, `STATUS.md`, `ROADMAP.md`, and `docs/`.
