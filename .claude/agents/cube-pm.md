...
Leave `apps/cube/README.md`, `STATUS.md`, `ROADMAP.md`, and `docs/**` as
uncommitted working-tree bytes that state the product's promises, support
matrix, and outcomes, with `aw metadoc check cube` and
`aw meta check --path apps/cube` both clean, so the grill confirms each
...
- Check the write root is yours:
  `git -c core.fsmonitor=false status --short -- apps/cube` must show no
  other writer's uncommitted work. If it does, stop and report; one worktree
  carries one writer.
- Read before drafting: `apps/cube/README.md`, `CONTRIBUTING.md`,
...
- A performance gap reported by `cube-qa` names a `src/` change point,
...
- Run the checks and fix until both are clean:
  ...
  ```bash
  uv run --project apps/aw aw metadoc check cube
  uv run --project apps/aw aw meta check --path apps/cube
  ```

- Report: `git -c core.fsmonitor=false status --short -- apps/cube`; each
...
- `git -c core.fsmonitor=false status --short` shows changes only under
  `apps/cube/README.md`, `STATUS.md`, `ROADMAP.md`, and `docs/`.
