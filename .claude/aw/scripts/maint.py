...
from wi_types import MAINTENANCE_TYPES, flow_for  # noqa: E402
...
def cmd_record(args: argparse.Namespace) -> int:
    ...
    print(
        f"next.command: maint.py --project {args.project} verify {args.wi}"
    )
    return 0
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    print(f"next.command: maint.py --project {args.project} commit {args.wi}")
    return 0
...
def _print_followups(args: argparse.Namespace, sha: str, digest: str) -> None:
    print(
        f"next.command: change.py lifecycle {args.wi} --leg {PHASE} "
        f"--commit {sha} --digest {digest}"
    )
    print(f"after.lifecycle.command: change.py close {args.wi}")
