...
- Run git through `git -c core.fsmonitor=false`. This checkout enables `core.fsmonitor`, and a stalled daemon blocks every command that reads the index — indefinitely, with no error. `apps/aw/src/aw/scripts/leg.py` does this for the same reason.
