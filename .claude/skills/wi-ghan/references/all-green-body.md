# All green reference body for wi_draft_gate

## Goal

Provide a reference body whose acceptance rows all succeed against the checkout.
...
- `.claude/skills/wi-ghan/scripts/wi_draft_gate.py` - all-green reference body.

### Frozen decisions

- The command exits 0 to serve as an all-green baseline.

## Acceptance

| # | command | current | target | why it cannot hold by accident |
|---|---------|---------|--------|--------------------------------|
| 1 | `python3 -c "import sys; sys.exit(0)"` | exits 1 | exits 0 | command exists and succeeds |
