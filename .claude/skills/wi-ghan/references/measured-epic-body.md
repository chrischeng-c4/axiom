...
- AC13: `aw ec --help`, `aw td --help`, and `aw cb --help` list exactly the target sets and nothing else — 5, 5, and 4 leaves besides `help`. Recompute with:
  ```
...
- AC14: No retired leaf gains a new reference while it is deprecated. The chain validator refuses a new emit site for a deprecated leaf, and the refusal has a case that was red before the fix.
- AC15: Each Tier removal lands with its caller migration in the same change. After a Tier C removal, every `.toml` gate binding that named the removed leaf names its replacement, and a project whose gate was green before the removal is green after it.
