---
name: aw:grill-epic-to-prd
description: Interview the human through AskUserQuestion until an epic work item's product promise is stated as observable behaviour, then write it as one `## <title> (#<iid>)` section of the owning project's `docs/product/<capability-area>.md`. Use after the epic is on the tracker and before any of its ladder runs. Writes only under `docs/product/`; never source, tests, or the tracker.
version: 0.1.0
user-invocable: true
---

# /aw-grill-epic-to-prd

Grill the human, then write one section. This skill owns exactly one thing:
the product-facing statement of what an epic promises, filed under the project
...
That refusal is the answer, not an error to route around: a change's product document is
`/aw-grill-change-to-prd`'s job, and an intent that has no work item yet is `/aw-grill-me-to-epic`'s.
...
- the `Capability:` line of `## Capability Alignment`, which already names the
  README capability this epic advances. That is the topic, unless the human
  says otherwise below.
- the `## Requirements` list and the `## Verification Inventory` table, which
  together are the only promises the section may make: one observable
  requirement per `R<n>`, each with the gate that measures it. A requirement
  whose gate cell is empty is not a promise yet.
...
| Promise | the observable behaviour, in the words a README capability row would use | it names no `R<n>`, or the `R<n>` it names has an empty gate cell |
...
Do not ask what the body already answers. `## Requirements` is the promise's shape and `## Verification
Inventory` is its measurement; the interview supplies who it is for and
what it is not.
...
each requirement's gate from `## Verification Inventory` verbatim under the promise, so a reader can run the thing the
